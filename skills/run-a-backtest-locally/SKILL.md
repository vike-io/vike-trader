...
vike-cli backtest --local --profile sweep.toml --rank-by sharpe   # a [sweep] table SEARCHES
vike-cli backtest --local --profile sweep.toml --optimizer tpe --trials 128
```

⚠ **There is no `sweep` verb.** A profile carrying a non-empty `[sweep]` table IS a parameter
search — the same predicate the engine and the server both branch on — and `--optimizer` names the
method. `vike-cli sweep` fails naming this command.
...
A parameter search takes five more flags, and all five are checked locally so a typo is a usage
error rather than a wasted round trip — the metric and the method are computed where the run
happens:

- `--rank-by sharpe|return|max_dd|equity|multi` — how to ORDER the rows. It is IGNORED, not an
  error, on a profile with no `[sweep]` table: it says how to order results, not what work to do.
- `--optimizer grid|euler|tpe` — the search METHOD (default `grid`, the exhaustive product).
- `--euler-depth N` / `--trials N` / `--seed S` — the per-method knobs, each refused under a method
  that does not own it.

⚠ `--optimizer` other than `grid`, the three knobs, and `--rank-by multi` are **`--local` only**:
the wire verb a remote search runs over carries the profile and a ranking metric and no method
selector. Each is refused BY NAME rather than quietly downgraded to the grid.
