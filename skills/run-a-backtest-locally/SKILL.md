...
```sh
vike-cli backtest run --local --profile run.toml --explain-data
vike-cli data fetch binance:BTCUSDT:1h --from 2026-01-01 --to 2026-02-01   # asks a datahub; --addr
...
| `6` | A DECLARED THRESHOLD WAS BREACHED — the product of this crate's JUDGING verbs, one per plane (`vike-cli backtest gate` and `vike-cli data hist gate`), and the one rung on this ladder that is a RESULT rather than a failure. |
