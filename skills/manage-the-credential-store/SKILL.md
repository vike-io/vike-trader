...
- `vike-cli secrets list` does not list a node key, on purpose. **Do not report that absence as
  "you have no node key"** — it is the wrong file. `vike-cli node status` owns that question.
...
| `vike-cli node` | stand a vike-tradehub node — the running daemon — up, and attach this box to one (setup on the daemon \| connect \| status \| disconnect on the client) |
...
2026-09-07.** `vike-cli node setup` and `vike-cli node connect` now write the two vike-tradehub NODE
...
| ANY node key — both pairs | not even the right FILE (they live in `node.env`), and a 256-bit HMAC key is minted rather than typed. The refusal names the command that owns the pair you asked for — `vike-cli node setup` for the tradehub, `vike-cli datahub setup` for the datahub |
...
- Do not write the file with a shell redirection, an editor script, or any tool other than a verb
  that UPSERTS — `vike-cli secrets set` for a venue credential, `vike-cli node setup` /
  `vike-cli node connect` for the tradehub pair, `vike-cli datahub setup` for the datahub pair.
