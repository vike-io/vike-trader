...
- **No store.** `setup` and `connect --manual` upsert into an EXISTING node store and create none
  (`crates/vike-cli/src/cmd/node/mod.rs`'s `open_store`, called from those two alone). ⚠ WHICH store
  that is, is a property of the box and not of the command: `<project>/settings/node.env` until this
  box has been migrated, and the `node_key` TABLE of `<project>/settings/db/vike.db` from then on.
  One `vike_secrets::backend_in` decides it per RUN for the reader and the writer alike, so they
  cannot disagree — and note the node pair has its own namespace either way, never the venue
  store's: a separate file before the migration and a separate TABLE after it, so a service holding
  no venue key never opens one.
  ...
  `vike-cli secrets template > settings/secrets.env`, then `chmod 600` it — and re-run. On a
  migrated box there is nothing to make: the database already holds both tables.
...
- The daemon also needs `VIKE_TRADEHUB_OBSERVE_KEY` in its OWN project's node store
  (`<project>/settings/node.env`, or that project's `db/vike.db` `node_key` table once migrated)
...
- ⚠ **Which artifact "the node store" is, is a property of the box, and it is not the venue store.**
  `vike_secrets::resolve_node_keys` decides once per run: the `node_key` TABLE of
  `<project>/settings/db/vike.db` when that database exists — wholly, opening no file at all — else
  `<project>/settings/node.env`, else, for a box whose pair was written before 2026-09-08 and never
  moved, `<project>/settings/secrets.env` as a temporary MIGRATION read that says so by name. Never
  a merge: whichever answers first answers wholly, because a pair split across two of them is a
  half-migrated box and merging would hide it behind an opaque `bad mac` at the node.
