...
- write to `policy.toml`, or rewrite `secrets.env` with an editor or a redirection, or ask for
  API keys in the chat;
...
4. **Never arm mainnet from the credential store.** `crates/vike-config/src/arming.rs` refuses
   startup when a `{VENUE}_MAINNET` name in the STORE carries the exact value `1` — refused,
   not ignored, so an operator who wrote it does not read demo fills as real ones. It is handed the
   parsed credential map rather than a file, so the refusal is the same whether that map came out
   of `secrets.env` or out of the database's `credential` table.
...
| `MainnetSwitchUnset` | `{VENUE}_MAINNET` is a conjunct of a `live` ceiling, never a replacement | the flag — and not in the credential store, whichever artifact answers |
