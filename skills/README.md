...
| [`record-and-replay-a-session`](record-and-replay-a-session/SKILL.md) | `list_series`, `run_backtest` | Record a live market feed to a tape with vike-recorder, prove the recorder is really recording, then replay the result — a recorded market TAPE through the backtest engine via list_series + run_backtest, or a live session's COMMAND JOURNAL through the real core via replay_journal. |
...
| [`run-a-backtest-locally`](run-a-backtest-locally/SKILL.md) | none | Run a backtest or a parameter sweep on THIS machine with `vike-cli backtest --local` and `vike-cli sweep --local`, against a local history store, with no vike-datahub server anywhere. |
