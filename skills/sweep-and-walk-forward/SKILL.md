...
| --- | :---: | --- |
| `profile` | yes | the profile TOML text with `[data]`/`[strategy]`/`[sweep]` (+ optional `[engine]`) |
| `script` | no | Rhai source, injected as `[strategy.params].src` |
| `rank_by` | no | `sharpe` (default), `return`, `max_dd` or `equity` - applied server-side |
...
| argument | required | meaning |
| --- | :---: | --- |
| `profile` | yes | the profile TOML text with `[data]`/`[strategy]`/`[walkforward]` (+ optional `[engine]`) |
| `script` | no | Rhai source, injected as `[strategy.params].src` |
...
Human twins: `vike-cli sweep --profile <run.toml> --rank-by sharpe` and
