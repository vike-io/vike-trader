...
public bars with no credentials and no venue account — ⚠ but it asks a DATAHUB to do the fetching,
so it needs one reachable (`--addr`, default `127.0.0.1:7878`). Where there is no server,
`vike-cli data fetch-starter` downloads the published dataset over plain HTTPS instead.
