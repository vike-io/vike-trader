...
description: Get market data into the local history store so a backtest has something to run over. Use when a user says they have no data, asks how to download or fetch bars, asks for BTCUSDT (or any symbol) history, says list_series came back empty, asks what a fresh install can run, or asks for a demo tape to try the tools on. Covers `vike-cli data fetch VENUE:SYMBOL:INTERVAL` for REAL public bars (no credentials, no venue account), `vike-cli data seed-demo` for the SYNTHETIC demo tape, `vike-cli data fetch-starter` for the PUBLISHED dataset where no venue can be reached, `vike-cli data export` for getting a slice back out, choosing a window, where the store lives, and reading the result back with list_series before believing the fetch landed.
metadata:
  tools: "list_series"
...
**Real public bars.** No credentials and no venue account — this is public market data. ⚠ It does
need a **reachable datahub**: this verb asks a server to fetch the window into the store rather than
calling the exchange itself. Default `127.0.0.1:7878`, else `--addr HOST:PORT`. With no server, use
`vike-cli data fetch-starter` (below), which needs nothing but HTTPS.

```sh
vike-cli data fetch binance:BTCUSDT:1h --days 180
vike-cli data fetch okx:BTC-USDT:1h --from 2024-01-01T00 --to 2024-06-01T00 --addr <host>:7878
```
...
```sh
vike-cli data export binance:BTCUSDT:1h --out btc-1h.parquet
vike-cli data export binance:BTCUSDT:1h --out btc-1h.parquet --from 2024-01-01T00
```

⚠ `--from` and `--to` are INDEPENDENT here and neither is required — an export bounds a slice that
is already on disk, so all four combinations are meaningful. That is the one place this grammar
differs from `fetch`'s, where a window is required and `--from` needs a matching `--to`. `--days`
is refused on `export` by name: it counts back from NOW, which says nothing about what a store
holds.

All of these write into (or read from) the store root, which `--store DIR` names outright when the
user wants a particular one.
