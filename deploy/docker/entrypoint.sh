...
#
# The image runs `vike-tradehub --config /project/settings/tradehub.toml`, and
...
#
# The last line must `exec`, so `vike-tradehub` REPLACES this shell and becomes PID 1. Without it
...
# carry, `crates/vike-backfill/src/pmxt/ingest.rs` is
# `clickhouse-client` in a binary this image does not ship (and is the only such site left, since
# the research crate's own ClickHouse sink was deleted with it), `crates/vike-ml/src/cli.rs` is the
...
every venue would stay on paper with nothing in the log to say why.
  ...
  [$problems] the daemon profile '$profile' is missing — this image's CMD names it, and
      vike-tradehub EXITS without it ('bad profile <path>: No such file or directory'). It decides
...
docker run --user \"\$(id -u):\$(id -g)\" ..."
  ...
  # the dependency that would be needed to call it; and `vike-recorder` and `vike-datahub` run this
...
Fix what it named above, or run 'vike-cli config check --json' for the machine-readable report."
  ...
  log "starting vike-tradehub"
  exec "$BIN_DIR/vike-tradehub" "$@"
}
