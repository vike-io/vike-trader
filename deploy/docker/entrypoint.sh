...
# carry, `crates/vike-ml/src/cli.rs` is the LightGBM trainer in a binary this image does not ship,
# and `crates/vike-buildinfo/build.rs` runs at BUILD time. ⚠ A fourth site was here — a
...
# that data is fetched by API or from the venue directly, never by reaching ClickHouse, and with it
# went the last `clickhouse-client` spawn anywhere in the tree (the research crate's own sink had
# gone with that crate, and the pmxt backfill's on 2026-09-09 with its NOT-idempotent write path).
# The one site linked
