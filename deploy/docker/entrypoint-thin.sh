...
# `vike-desktop` does NOT read. It resolves the key from the CREDENTIALS MAP, i.e.
...
# ⚠ THREE HOMES NOW, AND THIS GUARD KNEW ONLY THE OLDEST ONE. It grepped `secrets.env` alone, and
# the node keys LEFT that file at `docs/decisions/0051` — they live in `node.env` beside it, which
# `vike_secrets::resolve_node_keys` reads FIRST and which `vike-cli backend setup`/`connect` write.
# So this refused a correctly configured client and named the wrong file while doing it: an operator
# following `docs/ops/upgrading.md`'s migration recipe could not start this container at all. The
# legacy rung stays because that same resolver still reads it, warned, for a box that has not moved.
#
# ⚠ And the DATABASE is the third: `docs/decisions/0054`'s credential half makes
# `<settings>/db/vike.db` answer WHOLLY when it exists, with both files shadowed — so a hit in
# either one would be a false positive there, which is why it is probed BEFORE them. This shell
# cannot read a row, and it must not pretend to: the GUI performs its own routed resolution
# seconds later, so the honest act is to say the key is beyond this guard's reach and start
# anyway. Refusing would be the same defect wearing the newest store
# (`docs/decisions/0013-degrade-vs-refuse.md` is the standing call, and a viewer that will not run
# is worse than one that warns).
_settings="${VIKE_SETTINGS_DIR:-}"
_store="$_settings/secrets.env"
_nodes="$_settings/node.env"
_db="$_settings/db/vike.db"
if [ -n "${VIKE_TRADEHUB_OBSERVE_KEY:-}" ]; then
    echo "vike-thin: observe key from the environment"
elif [ -n "$_settings" ] && [ -f "$_db" ]; then
    echo "vike-thin: settings database $_db answers for this project, so the node keys are ROWS" >&2
    echo "vike-thin:   this shell cannot read and both files beside it are shadowed. vike-desktop" >&2
    echo "vike-thin:   resolves them itself; starting. If the pair is not in there, the node will" >&2
    echo "vike-thin:   answer 'bad mac' — check with: vike-cli backend status" >&2
elif [ -n "$_settings" ] && grep -qs '^VIKE_TRADEHUB_OBSERVE_KEY=' "$_nodes"; then
    echo "vike-thin: observe key from $_nodes"
elif [ -n "$_settings" ] && grep -qs '^VIKE_TRADEHUB_OBSERVE_KEY=' "$_store"; then
    echo "vike-thin: observe key from $_store — the PRE-0051 home. Still read, and still warned" >&2
    echo "vike-thin:   about: move the VIKE_*_OBSERVE_KEY / VIKE_*_CONTROL_KEY lines to $_nodes," >&2
    echo "vike-thin:   which holds node keys and nothing else. That fallback is a migration." >&2
else
    ...
    echo "vike-thin:      vike-desktop actually reads — node.env holds VIKE_TRADEHUB_OBSERVE_KEY," >&2
    echo "vike-thin:      or secrets.env on a box that has not migrated, or the settings database)" >&2
...
exec /opt/vike/bin/vike-desktop --observe "$NODE"
