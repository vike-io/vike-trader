...
On Windows, with no Rust at all — this repository is also a scoop bucket:

```powershell
scoop bucket add vike https://github.com/vike-io/vike-trader
scoop install vike-cli
```

⚠ Neither path updates itself. `cargo binstall` records the install under a crate version that never
moves, so a re-install needs `--force`; and `scoop update` cannot advance this bucket, because this
mirror is re-created as a single commit on every publish and scoop refreshes a bucket with a plain
`git pull`, which refuses an unrelated history. A later release is installed by removing the bucket
and adding it again:

```powershell
scoop bucket rm vike
scoop bucket add vike https://github.com/vike-io/vike-trader
scoop update vike-cli
```

⚠ That is a property of where the bucket lives rather than of scoop: this repository is a snapshot
by design, so it has no history for a `git pull` to follow. A bucket published as a repository of
its own keeps ordinary history, and `scoop update vike-cli` alone is then the whole update. When
one exists this section will say so.
