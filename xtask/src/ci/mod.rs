...
/// for a reader of an API reference: `vike-app` is the egui/wgpu GUI BINARY — a composition root
...
mod docs_roster_tests {
    ...
    fn names() -> BTreeSet<String> {
        set(&["vike-model", "vike-core", "vike-app", "vike-backfill", "ibapi", "xtask"])
    }
    ...
    fn the_documented_roster_is_the_ci_roster() {
        ...
        for excluded in ["vike-app", "vike-backfill", "ibapi"] {
            assert!(
    ...
    fn the_docs_trigger_reads_the_owning_crate_not_the_closure() {
        ...
        assert!(
            !docs_affected(&set(&["vike-app"]), &n),
            "vike-app is not documented, so a change confined to it cannot move a published page"
        );
        assert!(
            !docs_affected(&set(&[]), &n),
