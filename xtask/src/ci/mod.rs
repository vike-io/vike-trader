    ...
    let (affected, core, docs) = match &files {
        ...
        Some(files) => {
            let is_global = |f: &String| {
                // ⚠ The exemption is tested FIRST, and it is a per-FILE decision rather than a
                // per-commit one: a change touching `scripts/mirror_readme.md` AND
                // `scripts/gen_skills.sh` still escalates, because the shell script is global on
                // its own. Only a change whose every `scripts/` file is markdown ends up with an
                // empty `global` set.
                //
                // ⚠ Those two examples are REAL files on purpose. `crates/vike-ops/tests/citation_gate.rs`
                // reads a BACKTICKED path as a citation and requires it to resolve, so an invented
                // placeholder name under a real directory reddens CI — which is exactly what it did
                // here, on the first draft of this very comment. An illustrative path must either
                // name a file that exists or carry no backticks at all.
                if tables::is_global_exempt(f) {
                    return false;
                }
                f.as_str() == "Cargo.toml"
            ...
            // `tables::GLOBAL_PREFIX_EXEMPT` stopped escalating it — at which point this job, the
