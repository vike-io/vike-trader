...
//! # The output contract
//!
//! [`Plan::render`] emits, in this order, the seven `key=value` lines
...
pub use git::Env;
use graph::{affected_from, Graph, Rdeps};
...
///
/// Both entries walk a whole-tree input rather than a crate's own sources, so the reverse-dep
/// closure — derived from changed CRATES — structurally cannot select them. A gate that cannot run on
/// its own change class is not a gate: measured, a docs-only PR selected zero crates and not one of
/// the five prose gates ran.
...
pub fn compute(env: &Env, cwd: &Path) -> Result<Plan, String> {
    ...
    // `ordered` is the subset the `test` job runs — plus the store-kind gate's crate, which joins the
    // LANE without joining `affected` (see `tables::STORE_KIND_GATE_CRATE` for why).
    let rs_touched = files.as_ref().is_none_or(|f| f.iter().any(|p| p.ends_with(".rs")));
    let ordered: Vec<String> = roster
        .into_iter()
        .filter(|c| affected.contains(c) || (rs_touched && c == tables::STORE_KIND_GATE_CRATE))
        .collect();
