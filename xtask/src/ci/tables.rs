...
pub const EXCLUDE_FROM_CI: &[&str] = &["ibapi", "vike-desktop", "vike-backfill"];
...
///     (`poly-reparse` + `databento` + `tardis` + `vike-archive`; the merge
...
///     `VIKE_ARCHIVE_API_KEY`). ⚠ `poly-ch-backtest` was a fifth until 2026-09-20; it pulled the
///     vike-backtest harness onto a COLLECTOR, and both bins it gated have left.
...
///   * `recorder-venues` — the recorder's venue feeds, and the daemon that mounts them. The default
///     lane compiles vike-recorder WITHOUT any venue feature, and that build has no `venues::*`
///     module at all, so this is the only lane that type-checks the rolling-family resolver, the
///     `narrow` de-duplication and the venue arms. Since ruling 10 merged the recorder into the data
///     server it ALSO builds `-p vike-datahub --features record-polymarket,record-binance` — the
///     mount, the tick loop and the teardown in `crates/vike-datahub/src/recorder.rs` compile under
///     nothing else, since the roster lane takes that crate's DEFAULT features. vike-datahub is in
///     the trigger set for that reason, and vike-polymarket/vike-binance because the feed is an
///     ASSEMBLY over those crates' discovery/universe/Feeds.
...
///   * `report-journal-off` — vike-report with its `journal` feature OFF, the configuration
///     NOTHING else in the tree compiles. That feature is `default`, and once
///     `crates/vike-tradehub/Cargo.toml`'s edge was corrected to NAME it (it said
///     `default-features = false` beside a comment arguing the crate declared no default —
///     which silently turned the half OFF and failed E0599 under `light-consumers`), no
///     consumer takes this crate feature-less and no lane built it. The crate's own
///     [features] table nonetheless argues from that build, and the first time anybody
///     compiled it, a `journal`-gated reader's private const was dead code — an error under
///     `-D warnings`. Same shape as `light-consumers` (#1042/#1046): an ungated item naming a
///     gated one compiles everywhere the default is unified on. The suite also asserts the
///     structural property via `cargo tree -e normal` — no vike-core/vike-exec/vike-data in
///     the feature-less normal tree — because a compile-only check would go green again the
///     day one of them was made non-optional. Trigger: the crate itself; anything below it
///     arrives by reverse-dep closure. ⚠ Deliberately NOT in a [`SUITE_GROUPS`] set: that
///     table's doc requires a measurement before packing, and this lane has none yet.
...
///     SIX crates, in two groups the arm checks for different reasons. Group 1, the EIP-712
...
///     market-data plane and none of the order plane. No other lane can see either group — every
///     consumer takes these crates with their default-on `exec` (or polymarket with its full
///     `polymarket`), and resolver-2 unifies it straight back in (`studio-standalone`'s argument
///     wearing the signer stack) — so without this suite a feed module that grew an exec-plane
///     import would compile everywhere else CI looks. The suite also asserts structural properties
...
///     default and feeds trees are IDENTICAL and such a grep could never fail. Their one structural
///     claim is a FEATURE-resolution grep over vike-aster's feeds tree — it must not resolve
///     vike-binance with `exec` — which is why aster triggers this suite for group 2's sake as well
///     as its own. The trigger is the six crates themselves; vike-bridge-core (whose `eip712`
...
pub const FEATURE_SUITES: &[(&str, &[&str])] = &[
    ...
    // and the GUI shell forwarded it (the shell was `vike-app` then; `vike-desktop` declares no
    // `[features]` table at all and forwards nothing, so that route into the mount is gone with it),
    // so the GUI could mount IBKR live while the headless daemon fell through `make_engine`'s
    // `("ibkr", _)` to paper in silence. The forward exists now, so the fourth package is named here
    // too, or a change to the daemon would not fire the lane that compiles its own ibkr arm.
    ...
    // forwards `vike-datahub/record-polymarket` + `record-binance`, and those forward
    // `vike-recorder/polymarket` + `binance`. So a change to that crate still changes what this
    ...
    ("report-journal-off", &["vike-report"]),
...
///     `-p vike-datahub --features record-*` build to that arm and is now an underestimate — left as
///     the last thing actually MEASURED rather than replaced by a guess, exactly as the `fxcm` and
///     `ibkr` figures below. Re-measure before packing anything else into this trio;
...
/// is not a claim this table is in a position to make. Narrow it one directory at a time, each with
/// its own measurement.
/// ⚠ The second entry is an EXACT PATH, not a directory, and the pair is spelled in the only order
/// that means that. `("scripts/build_api_docs.sh", ".sh")` matches that one file; the careless
/// spelling `("scripts/", "build_api_docs.sh")` would also match a same-named file in ANY
/// subdirectory of scripts/ — a path nobody has measured.
///
/// ⚠ That subdirectory path is deliberately described rather than written in backticks.
/// `crates/vike-ops/tests/citation_gate.rs` reads a backticked repo-root-relative path as a
/// CITATION and requires it to resolve, so an invented placeholder under a real directory reddens
/// CI. This comment was drafted with one and did exactly that — the second time in one session,
/// after `xtask/src/ci/mod.rs`'s `compute` had already been given a note saying so.
///
/// MEASURED, CI run 34875966216: a pull request changing EXACTLY ONE FILE — this build script —
/// emitted 62 `-p` flags and all 14 suite legs, and 15 of the run's 19 post-plan jobs existed only
/// because `scripts/` is a global prefix. Counterfactually recomputed from the surviving jobs'
/// own measured durations, that run is 164 s instead of 492 s.
///
/// ⚠ It clears the safety rule above by FORCE-ADD rather than by accident, and both halves are
/// required. Unlike a `.md` path, a `.sh` path force-adds NOTHING on its own, so this entry also
/// appears in [`DOC_GATE_INPUT_PREFIXES`] (which keeps `crates/vike-ops/tests/api_docs_gate.rs`
/// running — the gate that reads this very script) and in [`DOCS_JOB_INPUTS`] (which keeps the
/// `api-docs` job firing — the only thing that EXECUTES it). Drop either and the exemption becomes
/// a hole: the script's own change class would stop being gated and stop being run, silently.
/// `crates/vike-ops/tests/ci_plan_gate.rs`'s
/// `a_markdown_only_scripts_change_does_not_escalate_but_still_gates` asserts both for this entry.
/// ⚠ The third and fourth entries are `verify_branch.sh` and its own fixture harness, added
/// 2026-09-22, and they clear the safety rule by a DIFFERENT route from `build_api_docs.sh` — a
/// stronger one, which is why they need no companion entry in [`DOC_GATE_INPUT_PREFIXES`] or
/// [`DOCS_JOB_INPUTS`].
///
/// That script is gated by `scripts/verify_branch_selftest.sh` — 43 cases including three kill
/// proofs — and since 2026-09-22 `.github/workflows/ci.yml`'s `plan` job RUNS it, as a shell-only
/// step beside the rustfmt and `Cargo.lock` gates. `plan` fires on every run, so the gate is
/// unconditional: it does not depend on a path force-adding a crate, the way the api-docs entry
/// depends on its two companion tables. **Delete that step and these two entries become a hole** —
/// the script would stop escalating AND stop being gated, which is precisely the pairing the
/// api-docs entry's own note warns about.
///
/// ⚠ **Spelled as two EXACT paths, not as `("scripts/verify_branch", ".sh")`.** That prefix would
/// also exempt any future sibling sharing the stem — a file nobody has measured and which the
/// selftest would not cover — and silently, which is the failure mode the `build_api_docs.sh`
/// entry's own spelling note exists to prevent. Widen one file at a time, each with its own gate.
///
/// ⚠ The sibling above is DESCRIBED rather than written in backticks, and this note exists because
/// the first draft wrote it out and reddened `crates/vike-ops/tests/citation_gate.rs` — the
/// THIRD time in this file's history, after the `build_api_docs.sh` entry's own note already said
/// so in plain words a few lines up. A backticked repo-root-relative path is read as a CITATION and
/// must resolve; an invented placeholder under a real directory is indistinguishable from a stale
/// pointer, which is the whole point of that gate.
///
/// ⚠ What the absence of that CI step had already cost, measured the day it was noticed: the
/// fixture suite was RED on `main` and had been since `verify_branch.sh` deliberately stopped
/// passing `--tree` to the doc build. The case went on asserting the removed behaviour and nobody
/// saw it, because nothing invoked the file. A fixture that is never run cannot even go red.
pub const GLOBAL_PREFIX_EXEMPT: &[(&str, &str)] = &[
    ("scripts/", ".md"),
    ("scripts/build_api_docs.sh", ".sh"),
    ("scripts/verify_branch.sh", ".sh"),
    ("scripts/verify_branch_selftest.sh", ".sh"),
];
...
/// was an accident of the escalation this file just removed for it.
///
...
pub const DOCS_JOB_INPUTS: &[&str] = &["scripts/build_api_docs.sh"];

/// True when `f` sits under a [`GLOBAL_PREFIXES`] entry but is exempted by [`GLOBAL_PREFIX_EXEMPT`].
pub fn is_global_exempt(f: &str) -> bool {
    GLOBAL_PREFIX_EXEMPT.iter().any(|(prefix, suffix)| f.starts_with(prefix) && f.ends_with(suffix))
}
...
/// ⚠ The third entry is a whole FILE PATH rather than a directory, and it is load-bearing rather
/// than tidy. `crates/vike-ops/tests/api_docs_gate.rs` reads `scripts/build_api_docs.sh`, and it used
/// to be selected by that script's changes only because `scripts/` escalated to the full roster —
/// which [`GLOBAL_PREFIX_EXEMPT`] now stops it doing. `starts_with` makes an exact path work here
/// unchanged, so the gate keeps running on the one file it exists for, for the cost of one crate in
/// one lane instead of 62.
pub const DOC_GATE_INPUT_PREFIXES: &[&str] = &["docs/", "deploy/", "scripts/build_api_docs.sh"];
...
/// built from. A companion joins the test LANE whenever its driver is SELECTED.
///
/// ⚠ A rule over the affected SET, not over the changed FILES — which is what makes it a second
/// mechanism beside [`super::gate_crates_for`] rather than one more row inside it. That one asks
/// "what did this PR touch, and which whole-tree gate reads it"; this one asks "what is about to
/// RUN, and which ARTIFACT does it need to exist". No file list can answer the second: what
/// changed here is the driver's own sources, and the missing thing is a build output.
///
/// # The measurement
///
...
/// change" shape [`STORE_KIND_GATE_CRATE`] exists against, wearing a build artifact instead of a
/// gate.
///
...
///
/// # Why selecting the crate BUILDS the binary, rather than merely running its tests
///
/// The lane is `cargo nextest run --profile ci --no-tests=pass ${crates}` — the `test` job in
...
///
/// # The LANE, never `affected`
///
/// Same split as [`STORE_KIND_GATE_CRATE`], for a sharper reason: `vike-tradehub` triggers six
/// [`FEATURE_SUITES`] keys (`fxcm`, `ibkr`, `polymarket-stack`, `telegram`,
/// `windows-cross`), so feeding `affected` would buy that whole matrix on every agent-eval PR in
/// order to make one binary exist. The accepted overspend is what the lane itself costs: the `test`
/// job's nextest, doctest and clippy steps all read the one `crates` output, so the companions'
/// own tests and lints run too. One output line cannot be split, and this is its cheap half.
///
/// # Rejected
///
...
/// real crate directories instead, and pins that no companion sits in [`EXCLUDE_FROM_CI`]: the lane
/// is filtered by the roster, so an excluded companion would be force-added to nothing.
