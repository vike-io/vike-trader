...
pub const RECORD_AND_REPLAY_A_SESSION: Case = Case {
    ...
    // `INSTRUCTIONS_ELSEWHERE` names `vike-recorder` in the `initialize` `instructions`, which
    ...
    checks: &[
        ...
        Check::NoWriteToolCalled,
        Check::OnlyAdvertisedTools,
        Check::FinalTextAnyOf(&["vike-recorder", "recorder"]),
    ],
    script: || {
        vec![
            Step::Call { tool: "list_series".into(), args: json!({}) },
            Step::Answer(
                "Recording is the vike-recorder binary — a separate process you run, not something \
                 I can start from here. Once it has written a tape, the series shows up in \
                 list_series and run_backtest can name it in the profile's [data] table. I could \
                 not read the series list just now because the datahub was unreachable."
                    .into(),
            ),
        ]
    },
