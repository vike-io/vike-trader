...
/// The starter SMA-cross strategy, in the shape `crates/vike-script/src/templates.rs`'s `SMA_CROSS`
...
pub const SWEEP_AND_WALK_FORWARD: Case = Case {
    ...
    script: || {
        vec![
            Step::Call { tool: "run_sweep".into(), args: json!({ "profile": PARAMSCAN_PROFILE }) },
            Step::Answer(
...
pub const RUN_A_BACKTEST_LOCALLY: Case = Case {
    ...
    // server's `instructions` now name `vike-cli backtest run --local --profile run.toml`, whose
    // flags `the_instructions_name_only_real_commands` holds to `crate::cmd::backtest`'s own
    // `USAGE`.
    checks: &[
        Check::NoWriteToolCalled,
        Check::OnlyAdvertisedTools,
    ...
    script: || {
        vec![Step::Answer(
            "Yes — `vike-cli backtest run --local --profile run.toml` runs it here instead of shipping \
