...
fn the_bridge_forwards_the_servers_instructions() {
    ...
    let mut left = 4;
    let served =
        "This is `vike-cli`'s agent surface. Recording is `vike-backend datahub --record`.";
...
/// The argument the settle probe passes, and the only one the stand-in's guard line answers.
///
/// ⚠ **The script's guard and the probe's argument are ONE spelling for a reason.** They sit in
/// different functions, and two independent literals would fail SILENTLY in the worst direction if
/// they ever drifted apart: the probe would miss the guard, fall through to `exec sleep 120`, block
/// for two minutes, and then exit 0 — so `settle_planted_exec` would report the file as settled.
/// Nothing downstream could catch it either, because the case's own timing assertion starts its
/// clock AFTER the settle. `crates/vike-ops/tests/common/mod.rs`'s `guard_line` derives the pair the
/// same way and for the same reason.
#[cfg(unix)]
const SETTLE_ARG: &str = "--vike-settle";

/// Exec a freshly planted script once, retrying ONLY `ETXTBSY`, so the DRIVER's exec of it cannot
/// lose that race.
///
/// # The race
///
/// `std::fs::write` closes its own descriptor before it returns, but a test binary's cases run as
/// parallel THREADS of ONE process, and a `fork`/`posix_spawn` in any other thread hands the forked
/// child an inherited duplicate; `O_CLOEXEC` closes that duplicate AT `execve`, not at `fork`. So
/// for a window of microseconds some process on the box holds the freshly planted file open for
/// WRITING, and exec'ing a file that is open for write is `ETXTBSY`.
/// `crates/vike-cli/tests/common/mod.rs` is the authority for the mechanism.
///
/// ⚠ **This site is NOT reachable in an ordinary run of this binary, and saying otherwise was
/// wrong.** The race needs a SIBLING thread forking during the write, and measured here
/// `std::process::Command::new` appears three times: one built and never spawned
/// (`does-not-need-to-exist`, which only reads `get_envs()`), one this probe, and one inside the
/// `#[ignore]`d `the_bridge_really_connects_the_real_cli_to_this_harness`. A default
/// `cargo test -p vike-agent-eval` therefore has no forking sibling at all — this file's own module
/// doc says as much ("a `Command` that is built and never spawned — except the last one").
///
/// It is kept, and the honest reason is narrower than the one first written here: the sibling
/// EXISTS under `--ignored`, where the live smoke spawns the real CLI beside this case, and the
/// cost of insurance is one `execve` of a script that answers the guard and exits. What it must not
/// do is claim to be closing a live hole.
///
/// ⚠ Losing it here would not have read as a race. `drive` would return a SPAWN error rather than a
/// kill, and the case would fail on its `err.contains("KILLED")` assertion — reporting a wrong
/// answer about the deadline, which is the one property this case exists to prove.
///
/// # Why a local copy rather than a shared helper
///
/// `vike-agent-eval` declares NO `vike-*` dependency, normal or dev. Taking one so a test could
/// share nine lines would trade a structural property of the crate for a convenience, so the
/// mechanism is CITED rather than shared — and the spelling deliberately matches
/// `crates/vike-ops/tests/common/mod.rs`'s `plant_exec`, so the three sites read as one idiom.
///
/// # Why a probe rather than a retry of the real call
///
/// Re-running `drive` would repeat its MCP-server setup and restart the clock this case measures.
/// A probe closes the window PERMANENTLY instead of sampling it: after the write returns, nothing
/// opens this path for writing again, so the only descriptors that can refuse the exec belong to
/// children forked DURING the write, and every one of those reaches `execve` microseconds later,
/// where its duplicate closes. A probe that succeeds proves they all have.
#[cfg(unix)]
fn settle_planted_exec(path: &std::path::Path) {
    const ETXTBSY: i32 = 26;
    const ATTEMPTS: u32 = 8;
    let mut backoff = std::time::Duration::from_millis(5);

    for attempt in 1..=ATTEMPTS {
        match std::process::Command::new(path).arg(SETTLE_ARG).output() {
            Ok(out) if out.status.success() => return,
            Ok(out) => panic!(
                "settle {}: the probe RAN and exited {:?}, so this is not the ETXTBSY race — the \
                 stand-in answered `{SETTLE_ARG}` with work instead of the guard, or its `#!` \
                 interpreter refused it.\nstderr: {}",
                path.display(),
                out.status.code(),
                String::from_utf8_lossy(&out.stderr).trim(),
            ),
            Err(e) if e.raw_os_error() == Some(ETXTBSY) => {}
            // NOT the race. A missing, unreadable or non-executable stand-in is a real fixture bug
            // and is reported NOW, with its errno, rather than waited out and then blamed on it.
            Err(e) => panic!(
                "settle {}: the probe could not be spawned and it is NOT the ETXTBSY ({ETXTBSY}) \
                 race — errno {:?}: {e}",
                path.display(),
                e.raw_os_error(),
            ),
    ...
    panic!(
        "settle {}: still ETXTBSY after {ATTEMPTS} attempts. The plant-then-exec window is \
         microseconds, so this is no longer that race — suspect a descriptor genuinely held open \
         for write on the planted file.",
        path.display(),
    );
}
...
fn a_cli_that_never_finishes_is_killed_at_the_deadline() {
    ...
    // It ignores every argument the DRIVER sends and outlives the deadline by a wide margin, so a
    // driver that waited for it could not finish inside this test's own budget. The one argument it
    // does answer is `--vike-settle`, which the driver never sends — see `settle_planted_exec`.
    std::fs::write(
        &slow,
        format!("#!/bin/sh\nif [ \"$1\" = \"{SETTLE_ARG}\" ]; then exit 0; fi\nexec sleep 120\n"),
    )
    .expect("write the stand-in");
    std::fs::set_permissions(&slow, std::fs::Permissions::from_mode(0o755))
        .expect("make the stand-in executable");
    settle_planted_exec(&slow);
...
fn the_bridge_really_connects_the_real_cli_to_this_harness() {
    ...
    let ctx = DriveContext {
        ...
        instructions: "This is `vike-cli`'s agent surface. Recording is `vike-backend datahub --record`.",
    };
