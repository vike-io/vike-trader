...
/// ⚠ **0084's SEVENTH verb, and the one that record did not price.** It counted six because it
/// measured the `HistStore` trait; `series_facts` was INHERENT to `DataFusionHist` and therefore
/// invisible to the count.
///
/// The consequence is what makes this worth a test rather than a line: without the verb, a
/// wire-routed backtest records `fingerprint: null` and READS GREEN — the reproducibility record
/// disappears and nothing fails. So the assertion is not "it answers" but "it answers the same
/// thing the local store does", which is the only form that would have caught a fabricated empty.
#[test]
fn series_facts_carries_coverage_and_commit_keys_over_the_composed_path() {
    let (_dir, store) = seeded_store();
    let addr = spawn_server(store.clone());
    let remote = RemoteHistStore::new(addr.to_string());

    let (wire_cov, wire_keys) =
        remote.series_facts(&bar_series_id()).expect("series_facts is served");
    let (local_cov, local_keys) = store.series_facts(&bar_series_id()).unwrap();

    assert_eq!(wire_cov, local_cov, "coverage: wire == local");
    assert_eq!(wire_keys, local_keys, "commit keys: wire == local");

    // ...and neither half is vacuous: the seed committed under a named key and holds real rows.
    assert!(wire_cov.rows > 0, "the seeded series has rows: {wire_cov:?}");
    assert!(!wire_keys.is_empty(), "...and the commit key it was written under: {wire_keys:?}");
}

/// The server ADVERTISES it — the whole of the negotiation for a plain trait verb, and what stops a
/// client older than the verb sending one.
#[test]
fn the_server_advertises_series_facts() {
    let (_dir, store) = seeded_store();
    let addr = spawn_server(store);
    let client = vike_datahub_client::DatahubClient::connect(addr).expect("handshake on connect");
    assert!(
        client.features().iter().any(|f| f == vike_datahub_client::FEATURE_SERIES_FACTS),
        "every serving build advertises `{}`: {:?}",
        vike_datahub_client::FEATURE_SERIES_FACTS,
        client.features()
    );
}
