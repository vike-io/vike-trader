...
//! profile-shaped sibling of `run_sweep_walkforward_roundtrip.rs`'s Studio-verb gate.
//!
...
//!
//! Both verbs are served on EVERY build (the harness is DataFusion-free — proven by
//! `roundtrip.rs`'s lean-build tests). This file is behind `serve-datafusion` only because it needs
