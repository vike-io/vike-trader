...
fn the_verb_scope_classification_is_pinned() {
    ...
    // `crates/vike-ops/tests/named_run_closure_gate.rs` holds — so a `src` is UNREAD rather than
    // refused.
    //
...
fn a_foreign_domain_mac_is_denied() {
    ...
    let mac = node_auth::sign(tradehub_domain, OBSERVE_KEY, &nonce, PROTO_VERSION, Scope::Read);
...
fn a_replayed_mac_from_another_connection_is_denied() {
    ...
    let captured =
        node_auth::sign(DATAHUB_DOMAIN, OBSERVE_KEY, &nonce_a, PROTO_VERSION, Scope::Read);
...
fn a_client_refuses_a_named_run_against_a_server_that_does_not_advertise_it() {
    ...
    assert!(matches!(client.ping(), Ok(())), "a frame was sent, so this stream is desynced");
}
