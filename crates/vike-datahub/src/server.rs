...
use vike_datahub_client::proto::{
    ...
    FEATURE_SCAN_PERP_METRICS, FEATURE_SEED_CLASS, FEATURE_SEED_SERIES, FEATURE_SERIES_FACTS,
    FEATURE_VENUE_CATALOG, PROTO_VERSION, Plane, Request, Response, SeedDone, VerbScope,
...
fn handle_request(
    ...
    match request {
        ...
        Request::ScanQuotes { venue, symbol, start, end, limit } => {
            match store.scan_quotes_capped(&venue, &symbol, TsRange { start, end }, limit.map(|l| l as usize)) {
                Ok(quotes) => Response::Quotes(cap_to_whole_ts(quotes, limit, |r| r.ts)),
                Err(e) => Response::Error(e.to_string()),
            }
        }
        Request::ScanTrades { venue, symbol, start, end, limit } => {
            match store.scan_trades_capped(&venue, &symbol, TsRange { start, end }, limit.map(|l| l as usize)) {
        ...
        //
        // ⚠ **TWO caps, and they do different jobs — neither is redundant.** The `_capped` verb
        // passes `limit` down as a READ budget: `vike_data`'s `narrow_to_budget` selects whole
        // manifest parts under it and clamps the range, which is what bounds the ALLOCATION (a
        // whole-range depth scan is ~27 GB for the live store's worst single day). `cap_to_whole_ts`
        // below then bounds the FRAME. A store that inherits the trait DEFAULT narrows nothing, so
        // the frame cap is the only one that fires there — which is exactly why it stays.
        //
        ...
        Request::ScanBookUpdates { venue, symbol, start, end, limit } => {
            match store.scan_book_updates_capped(&venue, &symbol, TsRange { start, end }, limit.map(|l| l as usize)) {
                Ok(rows) => Response::BookUpdates(cap_to_whole_ts(rows, limit, |r| r.ts)),
                Err(e) => Response::Error(e.to_string()),
            }
        }
        Request::ScanDepth { venue, symbol, start, end, limit } => {
            match store.scan_depth_capped(&venue, &symbol, TsRange { start, end }, limit.map(|l| l as usize)) {
