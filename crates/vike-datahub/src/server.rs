...
use vike_datahub_client::proto::{
    ...
    FEATURE_DELETE_SERIES, FEATURE_MARKET_DATA, FEATURE_SEED_SERIES, FEATURE_VENUE_CATALOG,
...
fn handle_request(
    ...
    match request {
        ...
        // on this wire.
        Request::SeedSeries { venue, symbol, interval } => {
            seed_series_verb(&venue, &symbol, &interval, backfill, seed, store)
        }
...
///    `VerbScope::Control` client. They run BEFORE the venue lookup so a bad interval is refused
