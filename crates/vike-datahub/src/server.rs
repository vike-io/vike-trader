...
use vike_datahub_client::proto::{
    ...
    FEATURE_DELETE_SERIES, PROTO_VERSION, Request, Response, read_frame_raw, read_frame_raw_capped,
    write_frame,
};
...
pub enum ServerAuth {
    ...
    /// not is [`Request::DeleteSeries`]: `crates/vike-datahub/src/server.rs`'s `delete_series_verb`
    /// answers [`KEYLESS_DELETE_REFUSAL`] here before the selector is validated, and
    /// `served_features` withholds the advertisement. `Backfill` KEEPING its place in that list
    /// beside it is the argument rather than an inconsistency — both are [`Scope::Control`] on a
...
impl ServerAuth {
    ...
    /// `Request::DeleteSeries` arm returns [`KEYLESS_DELETE_REFUSAL`], so key ABSENCE decides WHICH
...
pub fn required_scope(request: &Request) -> VerbScope {
    match request {
        ...
        // The WRITE verb...
        Request::Backfill { .. }
        // ...and the DESTRUCTIVE one. Control is necessary and NOT sufficient: `handle_request`
        // refuses it outright on a KEY-LESS server, where `Scope::Control` is a word nothing
        // enforces — see `vike_datahub_client::proto`'s `FEATURE_DELETE_SERIES`. This arm is what
        // stops an OBSERVE connection on a keyed server reaching it.
            ...
            Ok(request) => match authed {
                ...
                Some(scope) => {
                    ...
                    if scope_admits(scope, needed) {
                        handle_request(request, &store, backfill.as_deref(), true)
                    } else {
                ...
                None => handle_request(request, &store, backfill.as_deref(), false),
            },
...
/// # The sweep rule
///
/// A SWEEP (any wildcarded dimension) must carry `produced_by`. A fully-named single series need
...
/// `crates/vike-datahub/tests/run_sweep_walkforward_profile_roundtrip.rs`'s
