...
pub struct RecordArgs {
    /// **WHERE the profile came from, as one line for a message** — `--record <path>` or
    /// `--record-profile <name>`. A `String` rather than the `PathBuf` it was until 2026-09-16,
    /// because a profile now has TWO possible sources and a path could only ever describe one of
    /// them. [`record`] reads the profile itself from the already-validated `RecorderProfile` it is
    /// handed; this field is provenance, never an input to a read.
    pub profile: String,
...
pub fn one_store_root(
    ...
    Err(format!(
        "the recording profile and this server disagree about the store root, and since the \
         ...
         `store` (⚠ WHICH profile: a `--record` file, or the `recorder` ROW a `--record-profile` \
...
/// migrated must behave as it did. It is wrong here: `--record-profile` is an explicit, required
/// argument with no default, so "you named a profile and it is not there" must be a REFUSAL rather
...
/// path: it lands in a journal hours later, where `--record-profile default` is a line in a unit
/// nobody has open, and "which store answered" is the question an operator cannot reconstruct.
/// `vike-cli recorder show` prints the same `source:` line from the other side.
///
...
pub fn load_and_check_profile_row(
    ...
    // library — `crates/vike-ops/tests/settings_registry.rs`'s `LIBRARY_PIN` is a ratchet.
    ...
    let stored = profiles.by_name(name).ok_or_else(|| {
        format!(
            "{store}: no recorder profile named `{name}`. A `--record-profile` is an explicit \
             argument with no default, so an absent profile is a refusal rather than a daemon that \
             binds its port, answers every query and records nothing. `vike-cli recorder show` \
             lists what this store holds; `vike-cli config mirror --recorder` writes one from an \
             existing settings/recorder.toml"
        )
    })?;
...
mod tests {
    ...
    /// **A `--record-profile` naming a profile the store does not hold is a REFUSAL, and the
    ...
    /// here, because `--record-profile` is explicit and required. A daemon that treated the empty
    ...
    fn a_profile_name_the_store_does_not_hold_is_refused_before_anything_binds() {
        ...
        assert!(
            ...
            "…and what to run about it, because `--record-profile` has no file to go and look \
             at: {err}"
        );
    }
