    ...
    let stored = profiles.by_name(name).ok_or_else(|| {
        format!(
            "{store}: no recorder profile named `{name}`. A `--recorder-profile NAME` is an \
             ...
             what this store holds; `vike-cli config mirror --recorder` writes one from an \
...
/// `crates/vike-ops/tests/profile_writer_gate.rs`), and
/// `crates/vike-cli/src/cmd/config_mirror_recorder.rs` consults `plan_active_row` with
...
fn no_settings_database(store: &str) -> String {
    format!(
        "{store}: this box has no settings database, so there is no ACTIVE recorder profile for \
         ...
         create the database; `vike-cli config mirror --recorder <project>/settings/recorder.toml` \
         then stores a profile in it. Until then, name a TOML file outright with `--record <path>` \
         — the two are SOURCES, not a precedence chain"
    )
}
...
fn no_recorder_profile_stored(store: &str) -> String {
    format!(
        "{store}: the settings database holds NO recorder profile, so `--recorder-profile` with no \
         value resolves nothing. `vike-cli config mirror --recorder \
         <project>/settings/recorder.toml` writes one (run it with --dry-run first); `vike-cli \
         config recorder` lists what this store holds"
    )
}
...
fn no_recorder_profile_selected(store: &str, stored: usize) -> String {
    format!(
        "{store}: this store holds {stored} recorder profile(s) and NONE of them is marked active, \
         ...
         ⚠ There is no CLI verb that sets the active row yet: `vike-cli config mirror --recorder` \
         PRESERVES an existing selection and deliberately never creates one, so naming the profile \
         is the whole answer today"
    )
}
...
mod tests {
    ...
    fn each_no_names_a_different_next_command() {
        ...
        // Database, no recorder profile -> the command that WRITES a body.
        assert!(none_stored.contains("config mirror --recorder"), "{none_stored}");
        assert!(
            !none_stored.contains("secrets migrate"),
            "a store that answered does not need migrating: {none_stored}"
        );
        // Profiles stored, none selected -> name one, and say the selection verb does not exist.
        ...
        assert!(
            none_active.contains("no CLI verb that sets the active row"),
            "⚠ the operator must not be sent to a verb that does not exist — the §6 defect this \
             very change fixes elsewhere in this file: {none_active}"
        );
    }
