...
//! `RunSlice`, `RunParamscan`, `RunWalkforward`, `RunParamscanProfile`, `RunWalkforwardProfile` and
...
// The CHART-GAP SEED lane (`docs/decisions/0058-a-chart-gap-fetch-is-an-observe-verb.md`): the
// per-venue token bucket and the per-process series ledger that let `Request::SeedSeries` be a
// `VerbScope::Observe` write. Feature-free like `backfill`'s table type and for the same reason —
// the verb must DECODE and answer on every build, armed or not — and it REUSES that table for the
// fetch itself rather than naming a collector of its own, so it drags nothing new into the tree.
pub mod seed;

// The VENUE-CATALOG lane
// (`docs/decisions/0062-a-venue-catalog-fetch-is-an-observe-verb-and-not-a-write.md`): the provider
// table, the per-venue bucket and the TTL memo behind `Request::VenueCatalog`. Feature-free like
// `backfill`'s and `seed`'s table types and for the same reason — the verb must DECODE and answer
// on every build, armed or not — with only `real_catalog_table` (which NAMES the six bridge
// providers) behind `catalog-serve`.
//
// ⚠ Unlike `seed`, it does NOT reuse the backfill table: that table is kline COLLECTORS writing the
// store, and this verb writes nothing. The two would have been one seam only if a catalog were a
// time series, which 0062's decision 1 argues at length it is not.
pub mod catalog;
