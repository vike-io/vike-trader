...
are served by `vike-backend backtest --addr`, and this daemon refuses them by name.)
  ...
  --record PATH     the same thing from a TOML FILE at PATH (store root + subscriptions +
                    ...
                    ⚠ --record and --recorder-profile are two SOURCES, never a precedence chain.
...
const REFUSE_TWO_SOURCES: &str = "--record and --recorder-profile name DIFFERENT SOURCES for one profile (a file and a database \
...
mod tests {
    ...
    fn the_two_profile_sources_are_named_never_chained() {
        let Ok(Parsed::Run(Some(req))) = parse_args(argv(&["--recorder-profile", "default"]))
        else {
    ...
    fn the_profile_flag_takes_an_optional_value() {
        ...
        let Ok(Parsed::Run(Some(named))) =
            parse_args(argv(&["--recorder-profile", "prod", "--once"]))
        else {
    ...
    fn the_retired_profile_flag_still_starts_this_release_and_says_it_will_not_next() {
        ...
        let Ok(Parsed::Run(Some(new))) = parse_args(argv(&["--recorder-profile", "default"]))
        else {
