...
//!   behaves exactly as every build before them did. With either set, EVERY connection must
//!   complete the `Hello`/`Auth` handshake, Observe reads history and the catalog, and Control
//!   additionally admits the `Backfill` WRITE and every `Run*` verb — those COMPILE
//!   CLIENT-SUPPLIED RHAI server-side, so they are not reads whatever they return.
...
credential store (<project>/settings/secrets.env — `vike-cli secrets path` prints it):
  ...
  VIKE_DATAHUB_CONTROL_KEY   write scope: the above, plus the Backfill store write and every
                             Run* verb (those COMPILE CLIENT-SUPPLIED RHAI on this server).
                       With NEITHER set this server authenticates nothing and serves every verb
                       to whoever can reach the socket, exactly as it always has — which is why
                       that build is CONFINED to a loopback address (see VIKE_DATAHUB_ADDR).
                       Setting either makes the handshake mandatory on every connection.
...
pub fn run(
    ...
    let credentials: std::collections::HashMap<String, String> =
        match vike_secrets::resolve_project(booted.settings_dir_override.as_deref()) {
            Ok(resolved) => {
    ...
    let repo_default =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).ancestors().nth(2).map_or_else(
            || std::path::PathBuf::from("market_data").join("hist"),
            |r| r.join("market_data").join("hist"),
        );
    //
