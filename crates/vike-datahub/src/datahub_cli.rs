...
const USAGE: &str = "\
usage: vike-datahub [--record PATH | --record-profile NAME] [--tick-secs N] [--silent-secs N]
...
are served by `vike-backend backtest --addr`, and this daemon refuses them by name.)
  ...
  --record-profile NAME
                    ...
                    daemon that comes up healthy and records nothing. `vike-cli recorder show`
                    lists what the store holds; `vike-cli config mirror --recorder` writes a row
                    from an existing settings/recorder.toml.
  --record PATH     the same thing from a TOML FILE at PATH (store root + subscriptions +
                    maintenance + alerting). ⚠ NOT deprecated: it is what a commissioning dry run
                    points at a throwaway profile beside a temp store, and it is what a box that
                    has not migrated still uses.
                    ⚠ --record and --record-profile are two SOURCES, never a precedence chain.
                    Giving both is a refusal; the operator names which.
...
/// WHERE the recording profile comes from — **two SOURCES, never a precedence chain.**
///
/// ⚠ This is `vike_secrets::store::Backend`'s rule applied one layer up, and the rule is the point:
/// "where is my recorder profile" must have ONE answer, and the operator names which. A fallback —
/// *look for the row, else the file* — would make a half-migrated box read half from each and would
/// turn a typo'd profile name into a silent file read.
///
/// ⚠ **`--record PATH` is NOT deprecated and did not become a legacy rung**, and the reason is a
/// measured deployment hazard rather than kindness to old habits:
/// `deploy/sbin/vike-trader-ci-deploy` replaces the BINARY unattended on a tag while a unit file is
/// installed out of band by an operator, so a release that re-pointed `--record` at a profile NAME
/// would take a live recorder down on the next tag with an argument its own unit still spells as a
/// path. The two flags let the flip be an operator act, sequenced with seeding the row. It is also
/// the input `docs/ops/recorder-deploy.md`'s dry-run recipe needs: a throwaway profile file pointed
/// at a temp store, so a commissioning run is not a second writer.
#[derive(Debug, Clone, PartialEq, Eq)]
enum ProfileSource {
    /// `--record PATH` — a TOML file, read and parsed exactly as it always was.
    File(std::path::PathBuf),
    /// `--record-profile NAME` — a `recorder` profile row in `<project>/settings/db/vike.db`.
    Row(String),
}
...
/// Naming BOTH profile sources is a refusal, not a precedence question — see [`ProfileSource`].
const REFUSE_TWO_SOURCES: &str = "--record and --record-profile name DIFFERENT SOURCES for one profile (a file and a database \
     row), so giving both is ambiguous rather than a precedence question. Pick one. The file is \
     what a commissioning dry run points at a throwaway store; the row is what a deployed unit \
     names.";
    ...
    while let Some(a) = it.next() {
        match a.as_str() {
            ...
            "--record" => {
                ...
                profile = Some(ProfileSource::File(std::path::PathBuf::from(v)));
            }
            "--record-profile" => {
                let v = it.next().ok_or("--record-profile needs a profile NAME".to_string())?;
                if profile.is_some() {
                    return Err(REFUSE_TWO_SOURCES.to_string());
                }
                profile = Some(ProfileSource::Row(v));
            }
            "--tick-secs" => {
        ...
        return match companion {
            Some(flag) => Err(format!(
                "{flag} configures a recording, so it needs --record PATH or \
                 --record-profile NAME. Without one this daemon only serves the store"
            )),
...
pub fn run(
        ...
        Some(req) => {
            ...
            let loaded = match &req.profile {
                ProfileSource::File(path) => {
                    crate::recorder::load_and_check_profile(path, &resolved.root, cwd)
                }
    ...
    // having written nothing — which is the leg that makes a WRITE verb's `VerbScope::Observe`
    ...
    let catalog_table = crate::catalog::real_catalog_table();
    ...
    match record {
        ...
        Some(req) => format!(
            "vike-datahub: RECORDING AND SERVING — venue feeds fill the very store this server \
             answers from ({})",
            match &req.profile {
                ProfileSource::File(p) => format!("--record {}", p.display()),
                ProfileSource::Row(n) =>
                    format!("--record-profile {n} (a row in the settings store)"),
            }
        ),
        None => {
            "vike-datahub: SERVE-ONLY — no --record or --record-profile, so this daemon subscribes \
             to NO venue and writes NO rows; it answers from a store some other process fills. Add \
             `--record-profile <name>` to the unit's ExecStart= to make it record"
                .to_string()
        }
    }
}
...
fn to_record_args(req: &RecordRequest) -> crate::recorder::RecordArgs {
    crate::recorder::RecordArgs {
        profile: match &req.profile {
            ProfileSource::File(p) => p.display().to_string(),
...
mod tests {
    ...
    fn the_record_flags_parse() {
        ...
        assert_eq!(req.profile, ProfileSource::File(std::path::PathBuf::from("r.toml")));
    ...
    fn the_two_profile_sources_are_named_never_chained() {
        let Ok(Parsed::Run(Some(req))) = parse_args(argv(&["--record-profile", "default"])) else {
            panic!("`--record-profile NAME` is a run that records");
        };
        assert_eq!(req.profile, ProfileSource::Row("default".to_string()));

        let err = parse_args(argv(&["--record", "r.toml", "--record-profile", "default"]))
            .expect_err("naming both sources must be refused, not resolved by precedence");
        assert!(err.contains("--record-profile"), "{err}");
        assert!(err.contains("SOURCES"), "the message says WHY it is ambiguous: {err}");
        // …and in the other order, so the refusal is not an artefact of which came first.
        assert!(
            parse_args(argv(&["--record-profile", "default", "--record", "r.toml"])).is_err(),
            "order must not decide an ambiguity"
        );
        assert!(
            parse_args(argv(&["--record-profile"])).is_err(),
            "--record-profile needs a name — a flag that takes a value and does not get one is an \
             error, never a silently-defaulted knob"
        );
    ...
    fn the_mode_line_says_which_mode() {
        ...
        let Ok(Parsed::Run(Some(row))) = parse_args(argv(&["--record-profile", "prod"])) else {
            panic!("`--record-profile NAME` is a run that records");
        };
