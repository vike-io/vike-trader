...
fn write_granularity_per_symbol_vs_per_family() {
    ...
    eprintln!("A  per-symbol (today)   {a_ms:9.1} ms   {SYMBOLS} commits   {:6.2} ms/commit   {:8} KB on disk",
        a_ms / SYMBOLS as f64, du(dir_a.path()) / 1024);
    eprintln!("B  per-family (item #5) {b_ms:9.1} ms   1 commit      {b_ms:6.2} ms/commit   {:8} KB on disk",
        du(dir_b.path()) / 1024);
