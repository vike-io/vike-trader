...
use vike_data::datafusion_hist::RepairPlan;
use vike_data::{
...
use vike_model::{
    AssetClass, Bar, BookLevel, BookUpdate, BookUpdateKind, QuoteTick, SymbolProperties,
    TickScheme, TickTier, TradeTick, consolidate_trades,
};
...
// "vikearchive:…", poly_reparse) target the SAME
...
/// `crates/vike-data/src/datafusion_hist/manifest.rs`'s `SeriesLock` doc spends a paragraph on.
///
...
fn hold_the_series_lock(series_dir: &Path) -> std::fs::File {
    ...
    // `crates/vike-data/src/datafusion_hist/manifest.rs` itself uses.
...
/// 5,000 rows, `crates/vike-data/src/live_rec.rs`). So the contended case is `Ok(None)` and the
...
fn the_plan_shows_the_base_less_state_and_quotes_the_refusal() {
    ...
    assert!(err.contains("vike-cli data repair"), "the refusal must name the COMMAND now: {err}");
    let text = plan.lines().join("\n");
