...
///
/// A part's source is derived from its commit-key namespace: the Polymarket writers each key
/// their appends with a DISJOINT prefix on the SAME `venue=polymarket/symbol=<token_id>` series
/// (`RecorderSink` → `live-…`, `pmxt_backfill` → `pmxt:…`, `vike_archive_backfill` →
/// `vikearchive:…`, `poly_reparse` → its own), so an operator expresses "live beats pmxt beats
/// archive" as an ordered prefix list. ⚠ `clickhouse:…` is a FIFTH prefix and is still worth
/// ranking, but it has no writer any more: `clickhouse_poly_backfill` minted it and was retired on
/// 2026-09-19 for reaching ClickHouse directly, so the only parts carrying it are ones already on
/// disk from before that date. (This sentence said "the four Polymarket writers" and named
/// `clickhouse_poly_backfill` in place of `vike_archive_backfill`; the archive writer was already
/// missing from the list when the retirement made the count wrong in the other direction, so no
/// count is written here now.) This is why supersession is a maintenance operation the
...
impl SourceRankPolicy {
    ...
    ///
    /// The match itself is [`crate::store_kind::key_matches_prefix`] — the store's ONE answer to
    /// "does this key belong to this producer", shared with the provenance classifier a deletion
    /// asserts against. Two spellings of `starts_with` would be two chances for a precedence
    /// decision and a delete to disagree about which writer owns a series.
    pub fn rank_of(&self, commit_keys: &[String]) -> usize {
        commit_keys
            .iter()
            .filter_map(|k| {
                self.prefixes.iter().position(|p| crate::store_kind::key_matches_prefix(k, p))
            })
            .min()
