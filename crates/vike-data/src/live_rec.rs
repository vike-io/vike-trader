...
//! stream-health transitions (`GapStart`/`Stale`/`Live`) for the `"book"` stream ONLY (quote/trade
//! gap provenance is a noted follow-up) as single-row, zero-level `BookUpdate`s whose `kind` is one
//! of the three status kinds (`GapStart`/`Stale`/`LiveResume`) rather than `Delta`/`Snapshot` —
//! replay treats them as markers, not book content.
//!
...
impl LiveDataSink for RecorderSink {
    ...
    fn stream_status(&self, venue: &str, symbol: &str, stream: &str, status: StreamStatus) {
        // §B disclosure → recorded stream-health markers, BOOK lane only (the recorded book
        // stream is the one whose replay integrity depends on knowing where live data was
        // missing/stale; quote/trade gap provenance is a noted follow-up). Status events are
        // single-row, zero-level BookUpdates (kind ≠ Delta/Snapshot) with seq 0.
        if stream != "book" {
            return;
        }
...
mod tests {
    ...
    fn book_stream_status_recorded_as_markers() {
        ...
        // NON-book lanes are not recorded here (quotes/trades gap provenance = follow-up):
        sink.stream_status(
            "polymarket",
            "TOK",
            "quotes",
    ...
    fn append_retry_recovers_a_transient_fast_failure() {
        ...
        let (res, attempts) = append_with_retry(
            || {
                calls += 1;
                if calls == 1 {
                    Err(DataError::Io("transient".into()))
                } else {
                    Ok(7)
                }
            },
