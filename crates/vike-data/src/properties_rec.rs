...
impl PropertiesRecorder {
    ...
    pub fn from_env(store: Arc<dyn HistStore + Send + Sync>) -> Self {
        let enabled = std::env::var(RECORD_PROPERTIES_ENV).ok().as_deref() == Some("1");
        Self { store, enabled }
    }
    ...
    pub fn open_from_env(tick_store_root: &std::path::Path) -> Option<Arc<PropertiesRecorder>> {
        if std::env::var(RECORD_PROPERTIES_ENV).ok().as_deref() != Some("1") {
            return None;
        }
...
mod tests {
    ...
    fn disabled_recorder_writes_nothing() {
        ...
        assert!(store
            .scan_symbol_properties("bybit", "BTCUSDT", TsRange::all())
            .unwrap()
            .is_empty());
    }
    ...
    /// `None`. One self-contained test: nothing else in this crate reads the var, so the in-test
    /// set/remove can't race a sibling test.
    #[test]
    fn open_from_env_gates_on_exact_1() {
        let dir = tempfile::tempdir().unwrap();

        std::env::remove_var(RECORD_PROPERTIES_ENV);
        assert!(PropertiesRecorder::open_from_env(dir.path()).is_none(), "unset → None");

        std::env::set_var(RECORD_PROPERTIES_ENV, "1");
        let rec = PropertiesRecorder::open_from_env(dir.path()).expect("\"1\" → Some");
        assert!(rec.enabled(), "enabled recorder when env == \"1\"");

        std::env::set_var(RECORD_PROPERTIES_ENV, "true");
        assert!(
            PropertiesRecorder::open_from_env(dir.path()).is_none(),
