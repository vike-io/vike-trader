...
//!
//! ⚠ **NO PRODUCTION WIRING — and this paragraph claimed one until the claim was audited.** It
//! read: "`vike-app` calls [`ChainRecorder::open_from_env`] with its `tick_store_root()` and
//! threads the result into `vike_app_core::tools::spawn_tool_fetchers`". That root is gone with the
//! desktop's local market-data plane — `crates/vike-desktop/src/main.rs` binds its `chain_rec` to
//! `None` — and NEITHER impure constructor has a call site anywhere in the tree:
//! `ChainRecorder::open_from_env` and [`ChainRecorder::from_env`] are both dead doors. So
//! `VIKE_RECORD_CHAINS=1` records nothing, in any binary, and the file key `flags.record_chains` is
//! inert for the same reason rather than a different one. `vike_config::CONSUMPTION`'s row for that
//! key is the gated authority (a `Reader::Uncalled` naming both constructors, so the day either
//! gains a caller the row goes red); this doc is deliberately not a second one.
//!
//! What the constructors still ARE, for whoever mounts this next: [`ChainRecorder::open_from_env`]
//! is the app-root shape — it opens the `DataFusionHist` at the caller-resolved store root ONLY
//! when the env gate is on (so a disabled startup never touches the store) and hands back a
//! shareable `Arc`. Consumers reach the Deribit options provider via `with_chain_recorder`, the
//! same shape `vike-mount` uses to reach venues with [`crate::PropertiesRecorder`], with the store
//! open living HERE (one CI-tested helper) instead of being re-inlined per call site. The recorder,
//! its idempotency rule and its `kind=chain` series are all intact; only the mount is missing.
