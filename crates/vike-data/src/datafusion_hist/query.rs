...
use super::codec::{bars_from_batch, quotes_from_batch, str_values_required, trades_from_batch};
...
impl DataFusionHist {
    ...
    pub(super) fn collect_for_symbol(
        ...
        let mut sel: Vec<&FileEntry> = m.files.iter().filter(|f| f.overlaps(range)).collect();
        // ⚠ A ts ORDER before the budget, because without one "the first N parts" names an
        // ARBITRARY set — `read_manifest` returns APPEND order. A narrowed read has to be a PREFIX
        // of the range, or the caller continues past rows it never received.
        sel.sort_by_key(|f| (f.ts_min, f.ts_max));
        let range = narrow_to_budget(&mut sel, range, budget);
        let urls: Vec<String> =
