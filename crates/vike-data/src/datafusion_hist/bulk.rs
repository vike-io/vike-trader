...
//! (`SeriesLock` / `read_manifest` / `seal_into_manifest` / `publish`) that `commit_rows`
...
//! CURRENT (pre-flush) manifest does not reference, then [`super::manifest::publish`] appends a
//! framed delta record naming them. A crash at any point up to (but not including) that frame
//! reaching disk leaves the OLD manifest intact and the new part file(s) simply unreferenced —
//! invisible to every read, and safe to overwrite (a retry recomputes the exact same per-date part
//! name from the manifest it re-reads, not from a directory listing, so it lands on the same path
//! and replaces the orphan). A crash AFTER it is a completed, durable commit. There is no state
//! where a reader observes a half-applied window: one series' flush is all-or-nothing at the
//! publish granularity, exactly like a live `commit_rows` call minus its extra WAL safety net.
//!
//! ⚠ **The all-or-nothing used to come from `rename` and now comes from FRAMING**, which is the
//! one line of this section manifest v3 changed. A frame that did not reach its fsync is torn, and
//! `super::delta`'s reader stops at it — so a half-written publish is indistinguishable from one
//! that never happened, which is the same property the rename gave. `write_manifest`'s
//! remove-then-retry rename fallback (load-bearing on Windows, where `rename` fails outright when
//! the destination exists) still guards the FOLD and the v2 migration, the two whole-file writes
//! that remain.
//!
    ...
    pub fn flush(&mut self, key_prefix: &str) -> Result<BulkFlushReport, DataError> {
        ...
        let target = |kind: &str, venue: &str, symbol: &str| -> (PathBuf, bool) {
            match g.as_ref().and_then(|f| f(venue, symbol)) {
                Some(group) => (self.store.group_dir(kind, venue, &group, None), true),
...
fn commit_rows_bulk<C: SeriesCodec>(
    ...
    // The same framed append the live path makes, at `Durability::Bulk` — which skips the frame's
    // fsync for exactly the reason it skipped the manifest's: bulk keeps no WAL and its source file
    // is still on disk, so a lost publish costs a re-run of this `commit_key`, not data.
    publish(series_dir, &mut m, frame, Durability::Bulk, store.fold_bytes())?;
    Ok(written)
}
