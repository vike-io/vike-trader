...
pub const SERIES_CADENCE: &[SeriesCadence] = &[
    ...
    SeriesCadence {
        ...
        notes: "⚠ `interval=funding` is a reserved PARTITION label on this kind, not a cadence — \
                the market funding-RATE history is a bar series while the account's realized \
                payments are `kind=exec_funding`. `crates/vike-data/src/store_kind.rs`'s \
                `STORE_KINDS` pins both; decision 0080 resolved the naming half by \
                qualifying the ACCOUNT side and leaving this label untouched.",
    },
    SeriesCadence {
        kind: "quote",
        venue: None,
