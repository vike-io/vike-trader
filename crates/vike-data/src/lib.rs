...
//
// ⚠ Declared AFTER `datafusion_hist` because rustfmt sorts a CONTIGUOUS run of `mod` declarations
// and these two form one: a comment between them would split the run, and there is no comment to
// put there. Nothing about the order is semantic.
