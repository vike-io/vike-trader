...
use crate::hist_maint::{
    CompactionConfig, CompactionReport, Durability, GROUPED_ROW_GROUP_ROWS, MaintenanceConfig,
    MaintenanceReport, PruneReport, RetentionPolicy, SeriesMaintenance, SourceRankPolicy,
    WriteOpts, WriteProfile,
};
...
/// operator reads it: `vike-cli data repair` (through the engine's `backtest data repair`) renders
...
use codec::{
    ...
    book_rows, book_updates_from_rows, f64_col, i64_col,
};
use delta::DeltaFrame;
use gaps::{day_gap_to_ms_range, parse_utc_date};
use manifest::{
    FOLD_BYTES, Manifest, SeriesLock, fold_base, merge_tmp_name, publish, read_manifest,
    remove_series_contents, seal_into_manifest,
};
...
impl DataFusionHist {
    ...
    /// [`SeriesLock`] and re-seals + re-publishes any un-published append. Against a LIVE store
    ...
    /// stage 8, which is not accepted — see [`crate::series::SeriesId::source`].
    fn series_dir(
        &self,
        kind: &str,
        venue: &str,
    ...
    /// `vike_model::store_path::refuse_a_path_hostile_symbol` carries both measurements and is the
    ...
    fn refuse_a_hostile_symbol(symbol: &str) -> Result<(), DataError> {
        vike_model::store_path::refuse_a_path_hostile_symbol(symbol).map_err(DataError::Query)
    }
    ...
    /// all still come from `vike_model::store_path` — only the noun is this crate's.
    fn refuse_a_hostile_group(group: &str) -> Result<(), DataError> {
        vike_model::store_path::refuse_a_path_hostile_symbol(group)
    ...
    /// double-count that `crate::datafusion_hist::sources`'s module doc calls the hazard, made
    ...
    fn scan_symbol_across_layouts<C: SeriesCodec>(
        ...
        // ⚠ The budget is applied PER SERIES, not shared across them. A grouped layout answers from
        // the per-symbol series plus every group dir, and splitting one budget between them would
        // need to know each one's row count before reading any. The effect is that a grouped read
        // may return up to (1 + groups) budgets' worth — bounded, just not by one budget, and the
        // clamp each narrowing returns still makes every part of the answer complete.
        let mut out = self.scan_series_for::<C>(
            &self.ticks_dir(kind, venue, symbol),
            range,
            symbol,
            None,
            budget,
        ...
        for dir in groups {
            ...
            let rows = self.scan_series_for::<C>(&dir, range, symbol, Some(symbol), budget)?;
        ...
        F: Fn(&[usize]) -> Vec<ArrayRef>,
    {
        ...
        let (written, frame) = seal_into_manifest(
            series_dir,
            &mut m,
            commit_key,
            ts,
        ...
        // (2b) publish by APPENDING a framed delta record and fsyncing it — the durability
        // boundary. It was a whole-manifest rewrite until v3; on the live box's largest series that
        // was 104 MB per commit, and `manifest.rs`'s module doc carries the measurement. The
        // atomicity the rewrite bought comes from the framing instead (`delta.rs`), and the fsync
        // this waits on is a few hundred bytes rather than the whole file.
        ...
        C::Row: Clone,
    {
        ...
        // live recorder: `crates/vike-data/src/live_rec.rs`'s `flush_buf_with` retries once,
        ...
        // (`vike_recorder::venues::binance::group_name_for`). This catches the caller that types a
    ...
    fn scan_series<C: SeriesCodec>(
        ...
        self.scan_series_for::<C>(series_dir, range, ctx, None, None)
    }
    ...
    fn scan_series_for<C: SeriesCodec>(
        ...
        let batches = self.collect_for_symbol(series_dir, range, symbol, budget)?;
        let mut out = Vec::new();
        for b in &batches {
    ...
    fn compact_roundtrip(
        ...
        match kind {
            ...
            "exec_funding" => self.compact_roundtrip_generic::<FundingCodec>(urls, ""),
    ...
    fn compact_roundtrip_superseding(
        ...
        match kind {
            ...
            "exec_funding" => {
                self.compact_roundtrip_superseding_generic::<FundingCodec>(ranked, "")
            }
    ...
    /// rebuilding a migrated series: `RebuildReport::parts_without_keys` is the related signal.
    ...
    /// `crate::live_rec`'s `RecorderSink` discards its buffer — up to 5,000 rows — when its own
    ...
    /// plan/write gap is bounded the same way `crate::removal`'s is.
    ///
    ...
    pub fn plan_series_manifest_rebuild(&self, id: &SeriesId) -> Result<RepairPlan, DataError> {
        ...
        Ok(RepairPlan {
            ...
            delta_frames,
            delta_error,
            orphan_commits: base.as_ref().map(|m| m.orphan_commits.len()).unwrap_or(0),
            report,
    ...
    pub fn migrate_series_to_group(
        ...
        let landed = match kind {
            "quote" => self
                .scan_series_for::<QuoteCodec>(&gdir, TsRange::all(), symbol, Some(symbol), None)?
                .len(),
            "trade" => self
                .scan_series_for::<TradeCodec>(&gdir, TsRange::all(), symbol, Some(symbol), None)?
                .len(),
            _ => self
                .scan_series_for::<BookCodec>(&gdir, TsRange::all(), symbol, Some(symbol), None)?
                .len(),
        };
        if landed == 0 {
            return Err(DataError::Query(format!(
    ...
    pub fn delete_series_checked(
        ...
        }
        {
            ...
            if let Some(prefix) = require_produced_by {
                ...
                if let Some(foreign) =
                    commits.iter().find(|k| !crate::store_kind::key_matches_prefix(k, prefix))
                {
                    return Err(DataError::Query(format!(
    ...
    /// Write one series' bars to a standalone Parquet file — the symmetric twin of
    /// [`Self::append_bars_from_parquet`].
    ///
    /// # Why the store gained a WRITER
    ///
    /// The reader has existed since bulk loading did, and its asymmetry was invisible until
    /// something needed to hand a slice to somebody who does not have this store: the starter
    /// dataset published as a release asset, a slice shared between boxes, a reproducer attached to
    /// a bug report. Each of those was previously "copy the store directory", which ships the
    /// manifest, the commit keys and every other series in the same part — the store's INTERNALS,
    /// to somebody who wanted six months of one instrument.
    ///
    /// The output carries exactly the columns [`Self::append_bars_from_parquet`] selects, so the
    /// pair round-trips: export a slice, load it into a fresh store, get the same bars back. That
    /// is asserted rather than assumed (`crates/vike-data/tests/parquet_export.rs`).
    ///
    /// ⚠ It writes a PLAIN file, not a part: no manifest entry, no commit key, nothing under the
    /// store root changes. A file this produces is data, and the store that later ingests it
    /// decides for itself what commit key that ingest gets.
    ///
    /// `WriteProfile::Sealed` deliberately — an exported file is read start to finish by whoever
    /// receives it and is never appended to, which is exactly the shape compaction output has, and
    /// the heavier compression is worth it for something that will be downloaded many times.
    ///
    /// # Errors
    ///
    /// The store's own read error, or any Arrow/Parquet failure while encoding. Returns the number
    /// of rows written, which is ZERO for a range the store holds nothing in — a valid empty file,
    /// because the alternative (an error) would make "this slice is empty" indistinguishable from
    /// "the export broke".
    pub fn export_bars_parquet(
        &self,
        path: &Path,
        venue: &str,
        symbol: &str,
...
fn parse_series_id(root: &Path, dir: &Path) -> Option<SeriesId> {
    ...
    for comp in rel.components() {
        ...
        if let Some(v) = seg.strip_prefix("kind=") {
            ...
            // `crates/vike-data/src/datafusion_hist.rs`'s `series_dir` carries why the position is
...
impl HistStore for DataFusionHist {
    ...
    fn scan_depth_capped(
        ...
        // data-dependent. Applied per series for `scan_symbol_across_layouts`' stated reason.
        let mut rows = self.scan_series_for::<BookCodec>(
            &self.ticks_dir("depth", venue, symbol),
            range,
            symbol,
            None,
        ...
        for dir in self.group_dirs("depth", venue)? {
            let g = self.scan_series_for::<BookCodec>(&dir, range, symbol, Some(symbol), budget)?;
