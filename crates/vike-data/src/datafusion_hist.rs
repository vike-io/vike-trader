...
use manifest::{
    Manifest, SeriesLock, merge_tmp_name, read_manifest, remove_series_contents,
    seal_into_manifest, write_manifest,
};
...
impl DataFusionHist {
    ...
    pub fn series_facts(&self, id: &SeriesId) -> Result<(SeriesCoverage, Vec<String>), DataError> {
        ...
        Ok((coverage_of(&dir, &m), m.commits))
    }

    /// One series' INGEST COMMIT KEYS — the manifest's `commits` log, verbatim and in the order it
    /// records them.
    ///
    ...
    pub fn series_commits(&self, id: &SeriesId) -> Result<Vec<String>, DataError> {
        Ok(read_manifest(&self.series_dir_of(id))?.commits)
    }
    ...
    pub fn delete_series_checked(
        ...
        }
        {
            ...
            if let Some(prefix) = require_produced_by {
                let commits = read_manifest(&dir)?.commits;
                if commits.is_empty() {
                    return Err(DataError::Query(format!(
