...
pub trait HistStore {
    ...
    /// [`Self::scan_quotes`] under a ROW BUDGET — and the doc for all four capped reads
    /// ([`Self::scan_trades_capped`], [`Self::scan_book_updates_capped`],
    /// [`Self::scan_depth_capped`] point here).
    ///
    ...
    ///
    /// ⚠ **A shorter answer is not a partial one.** An impl honouring the budget must return rows
    /// that are COMPLETE for some prefix range — never a sample of the asked-for range. The caller
    /// pages by continuing past the last row's `ts`, so a gap inside the answered span is lost in
    /// silence. `DataFusionHist::narrow_to_budget`'s own doc carries the argument.
    ///
    ...
    ///
    /// ⚠ **Four verbs, not thirteen, and the four were MEASURED.** `kind=bar`, `kind=cohort` and
    /// `kind=perp_metrics` hold thousands of rows per series on the live store — a budget buys
    /// nothing there. `scan_equity`/`scan_exec_fills` hold zero rows today; they join this family
    /// when they stop being empty, and `scan_exec_fills` needs a range parameter first.
    ...
    // `crates/vike-data/src/archive_store.rs`'s `the_last_two_kinds_still_answer_empty_rather_than_refusing`
