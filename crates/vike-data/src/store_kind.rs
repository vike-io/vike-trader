...
//! and `crate::properties_rec`/`crate::chain_rec`, vike-backfill's venue / pmxt /
...
pub struct CommitKey {
    ...
    /// The producer LANE this key's rows came down — NOT the vendor and NOT the venue.
    ///
    /// It is the vocabulary of the `source=` path segment
    /// (`docs/superpowers/specs/2026-09-07-store-source-dimension-design.md` §4.1), whose whole
    /// purpose is that an archive copy and this box's own recorded tape of ONE series can coexist
    /// instead of sharing a leaf, a `_manifest.json`, a commit log and a lock. ⚠ **No leaf carries
    /// one yet** — stages 0-3 build the IDENTITY half only, and scoping a producer is stage 8,
    /// which is not accepted. So today this field is read by the classifier and by nothing that
    /// touches disk.
    ///
    /// ⚠ **DECLARED, not derived from [`CommitKey::template`]'s prefix, and that is measured
    /// rather than preferred.** Four declared templates have NO prefix at all
    /// ([`PREFIXLESS_TEMPLATES`]), so derivation would hand all four the same empty answer — and
    /// `key_matches_prefix` is `starts_with`, so an empty source id would claim every key in the
    /// store. The declaration's own drift protection is that it sits on the row
    /// `crates/vike-data/tests/store_kind_gate.rs`'s
    /// `every_declared_commit_key_template_exists_in_its_producer` already holds VERBATIM against
    /// the file that builds it, beside the reverse gate that forces every verb-calling file into
    /// this table at all.
    ///
    /// ⚠ **A source id may equal a venue slug, and two of them do.** `dukascopy` and `ibkr` are
    /// both `vike_model::VENUES` rows and both name a real data LANE; the pairing is declared in
    /// `crates/vike-data/tests/store_kind_gate.rs`'s `SOURCE_IDS_THAT_ARE_VENUE_SLUGS`, gated both
    /// directions so a third cannot appear unnoticed. It is not an ambiguity on disk — `venue=` and
    /// `source=` are adjacent, labelled segments — but it IS the confusion
    /// `docs/decisions/0030-data-vike-io-is-a-vendor-not-a-venue.md` names, wearing a second hat.
...
pub const STORE_KINDS: &[StoreKind] = &[
    StoreKind {
        ...
        commit_keys: &[
            ...
            // ⚠ **A `crates/vike-backtest/src/fetch.rs` row stood HERE, writing a `fetch:` commit
            // key, and it is DELETED with its producer (2026-09-19).** It was the engine's own
            // `backtest data fetch` — the one venue call in the compute plane, and the producer a
            // new user's first write went through, because `vike-cli data fetch` spawned it. That
            // verb now asks a DATAHUB instead, so a first write arrives through
            // `vike_backfill::klines::ingest_klines` and carries that path's commit key.
            //
            // ⚠ **Existing stores keep working and nothing migrates.** A `fetch:`-prefixed key is
            // still on disk wherever that verb ran, `--produced-by` still accepts the literal
            // prefix, and `append_*`'s idempotency compares the key it is GIVEN against the commit
            // log rather than against this table. What this row's removal changes is only that no
            // code MINTS a new one. A re-fetch of the same window through the datahub writes under
            // the collector's key and is therefore not deduplicated against the old rows — which is
            // the ordinary cross-producer case `SourceRankPolicy` exists for, not a new hazard.
        ...
        notes: "⚠ TWO different facts are called funding, and THIS row is the market one. The \
                MARKET funding-RATE history is a BAR series under the reserved label \
                ...
                symbol, \"funding\", …)`. The ACCOUNT's realized payments are a different series \
                entirely, and since decision 0080 they no longer share a word: that kind \
                is `exec_funding`. The market side was deliberately NOT renamed — `interval` is \
                part of `SeriesId` and bar series exist, so only the account side was free to \
                move. The label is deliberately NOT a cadence, because \
    ...
    StoreKind {
        ...
        commit_keys: &[
            ...
            // ⚠ **TWO `clickhouse:` rows stood HERE and BOTH are DELETED with their producers.**
            // A `clickhouse_spot` row keyed `clickhouse:spot:{symbol}:{day}` (2026-09-19) and a
            // `clickhouse_poly` row keyed `clickhouse:quote:{token}:{day}` (2026-09-20). The first
            // shelled `clickhouse-client` against a local `data_history` database — which is not an
            // API and not a venue, and only worked on the one box whose
            // `~/.clickhouse-client/config.xml` was already authenticated; the second did the same
            // against the recorder's `polymarket` database. The owner ruled that data is fetched by
            // API or from the venue directly, never by reaching ClickHouse. `clickhouse_poly`'s
            // three lanes are served by `crates/vike-backfill/src/vike_archive.rs` over HTTP (the
            // `vikearchive:` row just below), which is why its deletion cost no LANE — what it did
            // cost is named at `crates/vike-data/src/backtest_store.rs`'s module doc.
            //
            // MEASURED before the first removal, because "unused" is the claim that rots: the
            // deployed store holds `venue=binance` and `venue=polymarket` and no spot series at all,
            // and `vike_strategy::CheapNp` — the stated consumer — takes its spot reference series
            // as a SYMBOL handed to the engine rather than reading one from a fixed venue partition.
            //
            // ⚠ Existing `clickhouse:` keys on any disk keep working, for BOTH prefixes:
            // `--produced-by` accepts the literal prefix and idempotency compares the key it is
            // GIVEN against the commit log, never against this table. `vike_data::hist_maint`'s
            // `SourceRankPolicy` can still rank a part carrying one. What ends is that anything
            // MINTS a new one.
            CommitKey {
        ...
        notes: "⚠ NO clickhouse producer writes this kind any more — both are deleted (spot \
                2026-09-19, poly 2026-09-20: data is fetched by API or from the venue, never by \
                reaching ClickHouse) — and the RULE they left behind is what this note is for: a \
                `clickhouse:` producer had to pick a second segment no sibling used, because the \
                namespace is shared and only that segment told two apart. One keyed a Polymarket \
                TOKEN (`clickhouse:quote:{token}:{day}`), the other a SYMBOL \
                (`clickhouse:spot:{symbol}:{day}`), and nothing but that segment separated them. \
                Rows written under either key are still readable and `--produced-by` still accepts \
                the prefix; nothing mints a new one. The hazard is dormant rather than gone: a \
                second producer re-creates it. \
    ...
    StoreKind {
        ...
        commit_keys: &[
            ...
            // ⚠ A `clickhouse_poly` row keyed `clickhouse:trade:{token}:{day}` stood HERE and is
            // DELETED with its producer (2026-09-20) — the `quote` kind above carries the argument
            // and the read-compatibility note for all three of this collector's lanes.
            CommitKey {
                producer: "crates/vike-backfill/src/tardis/ingest.rs",
                template: "tardis:{}:{symbol}:{day}",
                source: "tardis",
    ...
    StoreKind {
        ...
        identity: "venue = the bridge venue id; symbol = the instrument. The LOSSLESS L2 lane: \
                   every delta present, `seq` contiguous, gaps detectable. ⚠ CONTRADICTED by the \
                   pmxt Polymarket archive, and PINNED here rather than quietly fixed: \
                   `crates/vike-data/src/cheap_np_book.rs` — which moved into this crate in #2017 \
                   and so now sits beside this row — records that the pmxt mapper assigns `seq` \
                   per asset per HOURLY FILE, so it restarts at 0 every hour. This row describes \
                   the LIVE recorder's output; the archive ingested from pmxt does not satisfy it, \
                   and that file's `a_recorded_seq_restart_does_not_drop_later_deltas` is what \
                   holds the fold correct across the restart",
        commit_keys: &[
            CommitKey {
                producer: "crates/vike-data/src/live_rec.rs",
                template: "live-{venue}-{symbol}-{}-{first_ts}-{last_ts}-{seq}",
            ...
            // ⚠ A `clickhouse_poly` row keyed `clickhouse:book:{token}:{day}` stood HERE and is
            // DELETED with its producer (2026-09-20). The `vikearchive:` row directly below is the
            // lane-for-lane replacement — the same `book_events`, over HTTP instead of a
    ...
    StoreKind {
        ...
        notes: "⚠ A `SymbolProperties` field with no column here is SILENTLY DROPPED on every \
                store round-trip — no error, just loss. `contract_size`, `taker_hold_ms`, \
                `tick_scheme` and `asset_class` are each an additive column added to close exactly \
                that hole, so adding a field to the model without adding its column here is the \
                trap, not the exception. ⚠ `asset_class` is the one column here whose cell is a \
                WORD off a closed vocabulary (`vike_model::AssetClass::sql_word`) rather than a \
                number or a JSON blob: an unrecognised word makes the row REFUSE to decode rather \
                than degrading to unclassified, so renaming a variant is a tape-compatibility \
                decision, not a rename. ⚠ Its producer's commit key carries NO namespace prefix \
    ...
    StoreKind {
        ...
        commit_keys: &[CommitKey {
            ...
            template: "{VENUE}:exec_funding:{account}:{coin}:{start_ms}-{end_ms}",
            source: "venue",
            spender: None,
        }],
        notes: "The ACCOUNT's REALIZED funding — what this account actually paid or received. \
                ⚠ The NAME COLLISION this row used to carry is RESOLVED by \
                decision 0080: the bare word `funding` sat on this, the SPECIALISED \
                case, while the MARKET funding-RATE history lives in the `bar` row above under \
                `interval=funding`. The account plane's prefix now does the work, so \
                `exec_fill`/`exec_order`/`exec_funding` share one and a reader can see which \
                side a kind is on. The market rate was deliberately NOT renamed — `interval` is \
                part of `SeriesId` and bar series exist, so that migration is not free. \
                ⚠ The partition key is (venue, coin) and the account is NOT in it; the ACCOUNT \
                COLUMN is what makes two accounts' payments for one coin separable, because \
                `hash` is the venue TRANSACTION hash and names no account — before that column \
                a main account's -1200 and a test account's -30 read back as -1230, \
                unsubtractable. Whether to ALSO partition per account is left open by 0079 and \
                is now a convenience question. ⚠ `hist rm` REFUSES this kind by name, as it does \
                every account kind, because it deletes by SERIES and one series holds both \
                accounts. ⚠ The trait default no longer lies: `append_funding`/`scan_funding` \
                return `DataError::Unsupported` rather than `Ok(0)`/`Ok(vec![])`, so a store \
                that cannot hold this kind says so instead of reporting a successful write of \
                nothing.",
    },
    StoreKind {
        kind: "chain",
        ...
        notes: "⚠ Volume warning, not a contradiction: at the recorder's default cadence one \
                ...
                ⚠ The trait default no longer lies: `append_chain_snapshot`/`scan_chain` return \
                `DataError::Unsupported` rather than `Ok(0)`/`Ok(vec![])`, so a store that cannot \
                hold this kind says so instead of reporting a successful write of nothing — the \
                `exec_funding` row's verdict-6 change, carried onto this pair. `chain_as_of` and \
                `chain_as_of_within` are derived over the read half and propagate it.",
    },
    StoreKind {
        kind: "cohort",
        ...
        notes: "⚠ A LONG ROW on purpose — the label taxonomy is DATA, not schema. \
                ...
                ⚠ THIS ROW AND `perp_metrics` ARE THE LAST TWO whose trait defaults still answer \
                `Ok(0)`/`Ok(vec![])` from a store that cannot hold the kind — the collision the \
                `exec_funding` row's verdict 6 closed, and the `chain` row's pair after it. A \
                DECLARED residual, not an oversight: \
                `crates/vike-data/src/hist.rs`'s `scan_cohort` carries the argument for why the \
                flip is its own decision, and \
                `crates/vike-data/src/archive_store.rs`'s \
                `the_last_two_kinds_still_answer_empty_rather_than_refusing` pins it.",
    },
    StoreKind {
        kind: "perp_metrics",
        ...
        notes: "⚠ THE NARROWEST KIND IN THE STORE — one timestamp, one number — and both of the \
                ...
                ⚠ It is the SECOND of the last two kinds whose trait defaults still answer \
                `Ok(0)`/`Ok(vec![])` from a store that cannot hold them; the `cohort` row above \
                carries that declared residual and the argument for leaving it, and this sentence \
                exists so the rule is not written in only one of the pair.",
    },
];
...
/// Every `source=` value this kind's DECLARED producers could write, in table order, deduped.
///
/// The set is the whole point rather than a convenience: a kind whose set has **one** member can
/// never hold two sources in a leaf, and a kind whose set has **two or more** is one whose history
/// has to be split before its producers can be scoped (the design's §14 stage 8, which is not
/// accepted — nothing here creates a `source=` leaf).
///
/// ⚠ It is derived from [`StoreKind::commit_keys`], which that field's own doc declares OBSERVED
/// rather than exhaustive. So a set of one is "no SECOND producer is declared", never "no second
/// producer exists" — and the check that narrows that gap is the reverse gate
/// (`crates/vike-data/tests/store_kind_gate.rs`'s
/// `every_caller_of_a_write_verb_is_a_declared_producer`), which walks from call site to row.
pub fn sources_for_kind(kind: &str) -> Vec<&'static str> {
    let Some(k) = store_kind(kind) else { return Vec::new() };
    let mut out: Vec<&'static str> = Vec::new();
    for ck in k.commit_keys {
        if !out.contains(&ck.source) {
            out.push(ck.source);
        }
    }
    out
...
/// The whole `source=` VOCABULARY, derived from every declared row and sorted.
///
/// Written down NOWHERE — not in this file's prose, not in the design's §8.3, which listed two
/// ClickHouse lanes that were deleted on 2026-09-20 under the owner's rule that data is fetched by
/// API or from the venue and never by reaching into ClickHouse. A roster restated beside its table
/// is the rot this crate's gates exist to end, so the answer is this call.
pub fn source_ids() -> Vec<&'static str> {
    let mut out: Vec<&'static str> =
        STORE_KINDS.iter().flat_map(|k| k.commit_keys.iter().map(|c| c.source)).collect();
    out.sort_unstable();
    out.dedup();
    out
}

/// Which declared LANE wrote `key` — [`commit_key_prefix`] against every row, **LONGEST prefix
/// wins**.
///
/// Longest-wins is load-bearing, not tidiness. `pmxt:quote:` and `pmxt:trade:` are both `pmxt`, so
/// a shortest-wins or first-wins rule would agree with this one today and diverge the moment two
/// lanes ever share a namespace — which has happened in this table before: the design's own §8.2
/// argues the case from `clickhouse:quote:` vs `clickhouse:spot:`, two DIFFERENT sources under one
/// namespace, where only the second segment told them apart. Those two rows are gone; the rule they
/// argued for is kept, because the next shared namespace costs nothing to survive and a silent
/// mis-attribution costs a wrong split.
///
/// `None` has THREE causes and they are not interchangeable, which is why this returns an option
/// rather than a fallback string: the key belongs to a producer that has been REMOVED from the tree
/// (the ordinary case — `panel_bars:` is nobody's), or to a producer that was never declared, or to
/// one of the [`PREFIXLESS_TEMPLATES`] whose keys carry no namespace to match. A caller that needs
/// to tell them apart asks [`producers_for_key`] as well.
pub fn source_for_key(key: &str) -> Option<&'static str> {
    let mut best: Option<(&'static str, usize)> = None;
    for k in STORE_KINDS {
        for ck in k.commit_keys {
            let Some(prefix) = commit_key_prefix(ck.template) else { continue };
            if !key_matches_prefix(key, prefix) {
                continue;
            }
            if best.is_none_or(|(_, len)| prefix.len() > len) {
                best = Some((ck.source, prefix.len()));
            }
        }
    }
    best.map(|(s, _)| s)
...
mod tests {
    ...
    fn perp_metrics_still_names_two_lanes_that_share_one_leaf() {
        ...
        // The dedup is real work, not an identity: `bar` declares seven rows and SIX lanes, because
        // `klines.rs` and `funding_rate.rs` are both the venue's own historical API.
        let bars = sources_for_kind("bar");
        assert_eq!(bars.len(), 6, "{bars:?}");
