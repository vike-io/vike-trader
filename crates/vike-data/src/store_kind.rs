...
pub const STORE_KINDS: &[StoreKind] = &[
    ...
    StoreKind {
        ...
        notes: "⚠ A `SymbolProperties` field with no column here is SILENTLY DROPPED on every \
                ...
                (`{venue}:{symbol}:{date}`) — one of two in this table that do not, the other \
                being the `bar` row's klines window key (`{venue}:{symbol}:{interval}:\
                {start_ms}-{end_ms}`), which it therefore matches segment-for-segment up to the \
                third. Harmless only because the two never write the same series.",
    },
    StoreKind {
        kind: "equity",
