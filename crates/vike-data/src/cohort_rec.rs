...
//! spelling its own key — is how two `vike-backfill` ClickHouse collectors came to share the
//! `clickhouse:` namespace on `kind=quote` with only their second segment telling them apart, which
//! `crates/vike-data/src/store_kind.rs`'s `quote` row still pins as a standing hazard.
//!
//! ⚠ BOTH of those collectors are gone — `clickhouse_spot` on 2026-09-19 and `clickhouse_poly` on
//! 2026-09-20, for an unrelated reason (data is fetched by API or from the venue, never by reaching
//! ClickHouse). The COLLISION they were two halves of is therefore not live today — but the lesson
//! is why this type exists, so it is stated rather than swept: two callers each spelling their own
//! `clickhouse:` key is what made the namespace ambiguous, and nothing structural stopped a third
//! from doing it again.
//!
