...
impl HistStore for ArchiveParquetHistStore {
    ...
    /// `recent_order_statuses`, and `crates/vike-report/src/equity.rs`'s `equity_curve_from_store`
    ...
    // perp_metrics stay on the two defaults that still answer empty; `crate::hist`'s cohort section
