...
//! (2026-09-20) — see `crates/vike-data/src/backtest_store.rs`'s module doc — so this is
...
fn parse_levels_json(json: &str) -> Vec<BookLevel> {
    if json.is_empty() {
        return Vec::new();
    }
...
mod tests {
    ...
    fn bars_and_account_series_are_always_empty_not_faked() {
        ...
        // ⚠ `scan_funding` LEFT this list on 2026-09-21, and it left for the reason the NOTE at the
        // top of this test already gives about quotes: an "always empty" lane is indistinguishable
        // from a real one that happens to hold nothing. `docs/decisions/0080-the-account-funding-kind-takes-the-qualified-name.md` verdict 6 made the
        // trait default REFUSE instead, so this store — which holds market Parquet and no account
        // series at all — now says so rather than answering `[]`.
        assert!(
            matches!(
                store.scan_funding(VENUE, "T", TsRange::all()),
                Err(DataError::Unsupported(_))
            ),
            "an archive of market Parquet must SAY it holds no account funding, not answer empty"
        );
    }

    /// ⚠ **The same latent defect is still live on FOUR other account lanes**, and this test records
    /// it rather than leaving the asymmetry to be rediscovered. `docs/decisions/0080-the-account-funding-kind-takes-the-qualified-name.md` scoped itself
    /// to one `STORE_KINDS` row, per the per-roster playbook, so `scan_equity`, `scan_exec_fills`,
    /// `scan_exec_orders` and `scan_chain` still answer `[]` from a store that cannot hold them —
    /// the exact shape the funding pair just stopped doing, and the exact shape the quotes lane was
    /// corrected out of before that.
    ///
    ...
    #[test]
    fn the_other_account_lanes_still_answer_empty_rather_than_refusing() {
        let store = ArchiveParquetHistStore::from_files(Vec::new());
        assert!(store.scan_equity(VENUE, "T", TsRange::all()).is_ok());
        assert!(store.scan_exec_fills(VENUE, "T").is_ok());
        assert!(store.scan_exec_orders(VENUE, "T").is_ok());
        assert!(store.scan_chain(VENUE, "T", TsRange::all()).is_ok());
    }
