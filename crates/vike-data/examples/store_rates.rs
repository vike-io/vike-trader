...
/// JSON bytes of one update's level payload — both sides, as `serde` renders `Vec<BookLevel>`.
/// An empty pair of vectors is 4 bytes (`[]` twice), which is what the envelope already carries.
fn level_json_bytes(bids: &[BookLevel], asks: &[BookLevel]) -> usize {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "sizeof           TradeTick={}B  QuoteTick={}B  BookUpdate={}B  BookLevel={}B",
        std::mem::size_of::<TradeTick>(),
        std::mem::size_of::<QuoteTick>(),
        std::mem::size_of::<BookUpdate>(),
        std::mem::size_of::<BookLevel>(),
    );
