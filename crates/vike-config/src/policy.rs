...
use crate::venue_mode::{did_you_mean, legal_modes, roster_id, VenueMode, VenuePolicy};
...
/// The number of fields [`Policy`]'s `Serialize` emits — five real ones plus the `accounts`
/// PROJECTION. Named so the impl below and its own gate cannot disagree about the count.
const POLICY_SERIALIZED_FIELDS: usize = 6;

/// ⚠ **Hand-written rather than derived, and the sixth field is why.**
///
/// The five real fields serialize exactly as `#[derive(Serialize)]` emitted them — same names, same
...
///
/// The sixth, `accounts`, is a **projection of [`VenuePolicy`]'s per-account ceilings, not a second
