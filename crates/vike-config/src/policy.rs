...
use std::path::Path;
use vike_model::account_keys::{AccountLabel, parse_wire_account};
...
/// to sit through. ⚠ The first derivation read the wrong driver: it cited
/// `vike_bridge_core::pump_spec`'s backoff and the subscribe-ack watchdog, which govern the
/// kline/trade lanes — and those lanes disclose no `StreamStatus` at all, so nothing they do can
/// ever reach this switch. The emitting lanes are two:
///
...
///   dial plus the nap, 11 s. ⚠ Those two dialled UNBOUNDED (a bare `tungstenite::connect`, so a
///   black-holed route pinned the thread for the OS's own SYN ladder — longer than this whole
///   grace) until the pumps became this switch's emitter; bounding them is what makes this bullet
///   a derivation rather than an estimate.
...
pub struct Policy {
    ...
    /// `vike_exec::RiskLimits` and judges no strategy-emitted order. The profile's twin is the one
    ...
    /// the order's own symbol re-valued as it will be AFTER the order. `vike_exec::RiskLimits`'s
    ...
    ///   `max_total_exposure` reaches `vike_exec::RiskLimits` through `vike_exec::ProfileRisk`,
    ...
    ///   same reasoning that took the notional ceiling out of the environment in Phase 5 applies to
    ///   a number an operator can swap by pointing at another file.
    /// * **The account vocabulary is already here and nowhere else.** `policy.toml` is the one file
    ///   that speaks in accounts at all — [`Policy::venues`]'s `[accounts]` sub-table names a
    ///   venue's second account by the label its credential keys carry
    ///   ([`vike_model::account_keys`]). A `[risk]` table has no way to say which account it means.
    ///
    ...
    /// (`vike_model::account_keys::AccountLabel`) and each carries its own copy. So
    ...
    /// per-account override table, and the reason is the same one that makes [`crate::VenueMode`]'s
    /// `cap` a `min`: a per-account line could only be useful by RAISING the number for one
    /// account, and a ceiling with a widening surface on it is not a ceiling. A deployment that
    /// genuinely needs different budgets per account runs one project folder per account, which is
    /// the shape every unit in `deploy/` already has.
    ///
    ...
    /// (`crates/vike-core/src/runtime/mod.rs`'s `reconcile_reports`). Under
    ...
    /// MEASURED on the CI box 2026-08-17: one bybit block carrying 53647.10600813 of a shared demo
    /// wallet beside nine paper blocks at 1000 seed each.
    ///
    ...
    ///
    /// A scalar, folded onto each engine's `vike_exec::RiskLimits` at the mount, so every
    ...
    ///
    /// **The correct dead-man observes the CONNECTION state, not silence** — and it has SHIPPED
    /// (M13): [`Self::link_deadman_grace_ms`], ON by default, tripping on a socket the bridge
    ...
    /// THIS key is now the OPTIONAL EXTRA for a 24/7 mount (a crypto perp venue that never closes
    ...
    /// re-derive the morning's ruling from the safety argument alone; the argument was right and
    /// the mechanism was the wrong one for it.
    ///
    ...
    /// its sibling; `crates/vike-tradehub/src/tradehub_cli.rs`'s `link_deadman_config_from_policy`
...
impl Default for Policy {
    fn default() -> Self {
        Policy {
            ...
            // Uncapped, like its per-order sibling above: a deployment with no `policy.toml` gets
            ...
            // account from a file the operator never wrote. `Policy::venues` is the precedent for
            // the opposite call and its own doc argues why it could afford to bite.
            ...
            // file nobody wrote. `Policy::max_sizing_equity` argues the axis.
...
/// The number of fields [`Policy`]'s `Serialize` emits — the real ones plus the `accounts` and
/// `account_exposure` PROJECTIONS. Named so the impl below and its own gate cannot disagree about
/// the count.
const POLICY_SERIALIZED_FIELDS: usize = 12;
...
impl Serialize for Policy {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        s.serialize_field("account_exposure", &self.venues.account_exposure_by_venue())?;
        s.end()
    }
}
...
pub struct PolicyPatch {
    ...
    /// weaker.
    ///
    /// ⚠ **The refusal now offers BOTH, because the account-aggregate ceiling exists.** This doc
    /// said "no account-aggregate ceiling exists in this workspace today" and that is no longer
    /// true: [`Policy::max_account_exposure`] is the aggregate this key's NAME reads as, in this
    ...
    /// **Per-ACCOUNT exposure ceilings** — the `[account_exposure]` table, `venue -> LABEL -> figure`:
    ///
    /// ```toml
    /// max_account_exposure = 50000   # the BOX-WIDE ceiling, every engine carries it
    ///
    /// [account_exposure.binance]
    /// DEFAULT = 50000                # the unlabelled account's own figure
    /// ALT     = 5000                 # …and a tighter one for the second account
    /// ```
    ///
    ...
    /// difference: the LABEL goes through [`vike_model::account_keys::parse_wire_account`] rather
        ...
        if let Some(v) = patch.max_total_exposure {
            return Err(ConfigError::value(
                ...
                "is no longer a policy key — it was read by NOTHING here, so it capped nothing. \
                 Two ceilings replace it, and WHICH ONE YOU WANT DEPENDS ON WHAT YOU MEANT. Per \
                 INSTRUMENT: `max_total_exposure` in your RUN PROFILE's `[risk]` table \
                 (vike_exec::ProfileRisk -> RiskLimits), which RiskGate evaluates on every order \
                 and which a live mount already refuses to start without — it caps ONE SYMBOL at \
                 ONE VENUE, so size it per instrument. Per ACCOUNT — the aggregate this key's NAME \
                 reads as: rename this line to `max_account_exposure`, in this same file, which \
                 caps the whole account's projected open notional across every symbol and denies \
                 with `over-account-exposure`. Delete this line and write one or both of those",
            ));
        }
        ...
        if let Some(caps) = patch.account_exposure {
            for (name, labelled) in caps {
                ...
                for (label, cap) in labelled {
                    ...
                    let text =
                        parsed.text().unwrap_or(vike_model::account_keys::RESERVED_DEFAULT_LABEL);
...
fn unarmed_exposure_account(file: &Path, venue: &str, label: &str, cap: f64) -> ConfigError {
    ConfigError::Value {
        ...
        key: format!("account_exposure.{venue}.{label}"),
        message: format!(
            "caps `{label}` at {cap}, but no `[accounts.{venue}]` line arms it — a labelled \
             account with no mode line resolves to `paper`, so this ceiling would cap an account \
             that trades nothing, and the store has no row to carry it on. Add the mode line \
             first:\n\n    [accounts.{venue}]\n    {label} = \"demo\"   # or \"live\"\n\nThe \
             unlabelled account needs no such line — its mode is the venue's own."
        ),
    }
}
...
mod tests {
    ...
    fn a_link_grace_below_one_reconnect_is_rejected_naming_the_cycle_and_both_moves() {
        for v in [1u64, 500, 23_000, MIN_LINK_DEADMAN_GRACE_MS - 1] {
            ...
            assert!(
                msg.starts_with(&format!("policy.toml: link_deadman_grace_ms = {v} ")),
                "{msg}"
            );
    ...
    fn the_removed_max_total_exposure_key_is_refused_and_names_where_it_lives_now() {
        ...
        // ⚠ The second half of this arm was `msg.contains("not the account")`, back when the
        // per-symbol key was the ONLY replacement and the refusal's whole job was to stop it being
        // read as an aggregate. It now offers the aggregate too, so the disclaimer is carried by
        // the CONTRAST — "Per INSTRUMENT" against "Per ACCOUNT" — rather than by a negation, and
        // the assertion follows the message instead of pinning a phrase that no longer earns its
        // place. What must not weaken is that the per-symbol redirect still declares its scope.
        assert!(
        ...
        // ⚠ This arm asserted `msg.contains("no account-aggregate ceiling")` — "say plainly that
        // the protection this operator wanted does not exist". It DOES exist now, in this very
        // file, so the assertion is INVERTED rather than deleted: the refusal must hand the
        // operator the aggregate key by name, or this tombstone sends someone who asked for a
        // book-wide ceiling to a per-symbol one and stops there.
        assert!(
            msg.contains("max_account_exposure") && msg.contains("this same file"),
            "must name the ACCOUNT-aggregate ceiling that now exists, and say it is here: {msg}"
        );
        assert!(
    ...
    fn the_account_ceiling_loads_validates_and_is_absent_by_default() {
        assert_eq!(
            ...
            None,
            "no policy.toml ⇒ no account ceiling ⇒ the gate is byte-identical to before it existed"
        );
        let mut p = Policy::default();
        ...
        for bad in [0.0, -1.0, f64::NAN, f64::INFINITY] {
            let err = Policy::default()
                .apply(
                    PolicyPatch { max_account_exposure: Some(bad), ..Default::default() },
                    file(),
                )
