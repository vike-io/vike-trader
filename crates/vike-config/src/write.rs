...
pub struct SettingsWrite {
    ...
    /// **Is this write what the next boot will RESOLVE?**
    ///
    /// `true` on every box that has not run `vike-cli config adopt`, and on an adopted box whose
    /// caller re-mirrored the rows ([`RowSync::Mirror`]). `false` on an ADOPTED box in TWO states,
    /// both of which mean *the value is on disk and is being read by nothing*: a caller that may
    /// not write a row ([`RowSync::ReportOnly`]), and a `Mirror` caller whose re-derive would have
    /// promoted a drifted file OUTSIDE the section it was writing — see [`sync_rows`]'s `Mirror`
    /// arm for the measurement that added the second one.
    ///
    /// ⚠ This doc said "in exactly one state" and the count is the kind of claim this tree has
    /// watched rot before; the arm's own code is the authority.
    ///
    /// ⚠ A surface that renders a bare success for `false` tells the operator something false —
    /// which is the same defect `vike_config::CONSUMPTION` exists to refuse one level down, where
    /// it is a KEY nothing reads rather than a WRITE nothing applies.
    pub in_force: bool,
}
...
pub fn set_setting(
    ...
    set_setting_within(settings_dir, file, dotted_key, raw_value, LockBudget::DEFAULT, rows)
}

/// **What this write does about the settings ROWS**, now that a box can resolve from them.
///
/// ⚠ **There is no default, and that is the whole design of this parameter.** Every one of the
/// three production callers writes a settings FILE; on an ADOPTED box a settings file decides
/// NOTHING. Leaving them alone would produce one of two unacceptable outcomes, and both were on
/// the table:
///
/// * a CAPABILITY REGRESSION — three confirm-gated surfaces report success for a write that changes
///   no value anybody reads; or
/// * a DELAYED OUTAGE — the file is written, nothing applies it, and the divergence detonates at the
///   next restart, which on the deployed box is the 06:40 unattended-upgrade window.
///
/// So each caller states its arm and the arm is PINNED by a gate, which makes pointing a
/// daemon-side writer at a row an act somebody performs on a PR rather than one that happens.
#[derive(Debug, Clone, Copy)]
pub enum RowSync {
    /// **Re-derive the touched sections and rewrite the tables, inside the settings lock this write
    /// already holds.** One extra transaction on a path that already did a read-modify-write.
    ///
    /// For a writer that is an OPERATOR ACT outside every daemon's mount namespace. On an unadopted
    /// store it is byte-identical to today's Phase-1 mirror semantics — the rows are a copy either
    /// way — so this arm changes nothing on a box that has not crossed.
    Mirror,
    /// **This caller may not write a row, and says why.** The file is written and
    /// [`SettingsWrite::in_force`] is `false` on an adopted box, so the surface can render *written
    /// to policy.toml — NOT in force until `vike-cli config mirror`* rather than a bare success.
    ///
    /// ⚠ **The one production instance is the DAEMON's control channel, and
    /// `docs/decisions/0068` is why** — not because that record is undecided, but because of what
    /// it decided ON. It rules that NO layer is placed above 0054's `settings/db` grant, and the
    /// reason it can is stated in its own *What would reopen this*: the daemon **writes no settings
    /// row** — *"which is what makes the grant safe while the bound is absent. That is a property
    /// of the current code, not a rule, and nothing enforces it."*
    ///
    /// This arm is that rule, and `crates/vike-ops/tests/settings_row_writer_gate.rs` is the
    /// enforcement 0068 says nothing provides. A daemon-side writer pointed at a row is one of the
    /// three conditions that record names as REOPENING it, so it may not arrive as a side effect
    /// of a diff about something else.
    ///
    /// The arm is also DEAD today for an independent reason, and the two must not be confused: the
    /// daemon's settings directory is read-only in its own mount namespace, so the write refuses at
    /// the LOCK before any database handle is opened. That fence is a MOUNT OPTION, and the grant
    /// it depends on moved once already — 0068 measures `settings/db` as WRITABLE on the CI box since
    /// 2026-09-18. The hazard arrives by the carrier moving, not by anyone deciding, which is why
    /// the second fence is a pin and not a comment.
...
pub fn set_setting_within(
    ...
    // ...and the ROWS, INSIDE the lock this write already holds — see [`RowSync`] for why the
    // parameter has no default, and why the daemon's arm is the one that may not reach here.
    //
    // ⚠ **The ORDERING is a structural fence on `docs/decisions/0068`**: the FILE write happened
    // above and this is reached only on its success. The deployed daemons run
    // `ProtectSystem=strict` with a `ReadWritePaths=` that does not name `<project>/settings`, so
    // their `SettingsLock::acquire_within` fails with `EROFS` before any database handle is opened
    // at all. A daemon cannot reach this line, and it cannot reach it by accident.
    let in_force = sync_rows(settings_dir, rows, file);
    Ok(SettingsWrite {
        file: file.file_name(),
        key: dotted_key.to_string(),
        old_value,
...
/// Re-derive the settings rows after a file write, and answer whether the write is IN FORCE.
///
/// `true` means what an operator expects a settings write to mean: this value is what the next boot
/// will resolve. It is `false` in exactly one state — an ADOPTED box whose caller may not write a
/// row — and a surface that renders a bare success there tells the operator something false.
///
/// ⚠ **A row-sync failure is NOT an error return**, and that is deliberate. The FILE write has
/// already landed and cannot be unwound; turning a mirror failure into an `Err` would report a
/// write that did happen as one that did not, which is the worse of the two wrong answers. It
/// comes back as `in_force = false`, which is TRUE — the value is on disk and is not being read.
fn sync_rows(settings_dir: &Path, rows: RowSync, file: SettingsFile) -> bool {
    // An unadopted box is unchanged in every arm: the rows are a copy, so a write is in force the
    // moment the file lands, exactly as it was before the seal existed.
    let adopted = matches!(
        vike_secrets::read_settings_in(settings_dir),
        Ok(ref s) if s.adoption().is_some()
    );
    if !adopted {
        if let RowSync::Mirror = rows {
            // Phase-1 mirror semantics, unchanged: keep the copy in step where we can, and say
            // nothing when we cannot (there may be no database on this box at all, which is the
            // ordinary state and not a failure).
            let _ = crate::mirror::rows_from_files(settings_dir)
                .ok()
                .map(|derived| vike_secrets::write_settings_in(settings_dir, &derived));
        }
        return true;
    }
    match rows {
        RowSync::Mirror => {
            let Ok(derived) = crate::mirror::rows_from_files(settings_dir) else { return false };
            // ⚠ **THE COLLATERAL-PROMOTION GUARD.** `rows_from_files` is `load(Some(dir))` over ALL
            // FOUR files, and `write_settings` then DELETEs both tables and re-inserts — so on an
            // adopted box this arm promoted every drifted file wholesale, on the back of whatever
            // single key was being written. Measured at `650907a37` in a the latency box lane, all three from
            // one `set_setting(Preferences, "preferences.log_file_level", "warn", Mirror)`:
            // a hand-edited `policy.toml` moved the ceiling `250.0 -> 100000.0` and binance
            // `Demo -> Live`; a DELETED ceiling line booted the box clean with no ceiling and no
            // refusal; a deleted `policy.toml` destroyed the rows outright.
            //
            // The write that carries it need not even be confirm-gated — `typed_confirm_reason` is
            // `Some` for `policy.max_notional_per_order` and `None` for
            // `preferences.log_file_level` — so the key requiring a retype was promoted by the key
            // that does not, and `settings_write.rs`'s journal records only the key the operator
            // named. Nothing downstream could see it.
            //
            // So an adopted box mirrors a write only when the re-derive touches NOTHING BUT the
            // section being written. Anything else means the files have drifted, and a write aimed
            // at one key may not be the act that lands somebody else's stale draft.
            // `in_force` is then `false`, which is exactly what it means — the file was written and
            // no row moved — and the surfaces already render that honestly.
            let Some(section) = crate::mirror::SECTION_FILES
                .iter()
                .find(|(_, f)| *f == file.file_name())
                .map(|(s, _)| *s)
            else {
                return false;
            };
            let stored = vike_secrets::read_settings_in(settings_dir)
                .ok()
                .and_then(|s| s.rows().cloned())
                .unwrap_or_default();
            if collateral_change(&stored, &derived, section) {
                return false;
            }
            vike_secrets::write_settings_in(settings_dir, &derived).is_ok()
        }
        RowSync::ReportOnly(_) => false,
    }
}

/// Would re-deriving the rows change anything OUTSIDE `section`? See [`sync_rows`]'s `Mirror` arm.
///
/// Arming rows belong to `policy`, so a write aimed at any other file may not move one at all.
fn collateral_change(
    stored: &vike_secrets::StoredSettings,
    derived: &vike_secrets::StoredSettings,
    section: &str,
) -> bool {
    let key_of = |r: &vike_secrets::SettingRow| (r.section.clone(), r.key.clone(), r.value.clone());
    let others = |rows: &vike_secrets::StoredSettings| {
        let mut v: Vec<_> =
            rows.settings.iter().filter(|r| r.section != section).map(key_of).collect();
        v.sort();
        v
    };
    if others(stored) != others(derived) {
        return true;
    }
    section != "policy" && stored.arming != derived.arming
}
...
mod tests {
    ...
    /// **The two writers with the [`RowSync`] arm every test in this module wants**, SHADOWING the
    /// glob-imported originals (a local item beats a glob import).
    ///
    /// ⚠ The arm is not arbitrary, and it is not "whatever compiles": every test below runs against
    /// a `tempfile::TempDir` with NO settings database in it, so [`sync_rows`] takes its unadopted
    /// branch and writes nothing at all. `Mirror` is therefore what these tests were measuring
    /// before the parameter existed — the FILE half, alone — and using it keeps them measuring
    /// exactly that.
    ///
    /// Two wrappers rather than a defaulted parameter, because the parameter having no default is
    /// the whole design of it: each of the three PRODUCTION call sites must state its arm, and a
    /// default is how one of them would silently inherit the wrong one. A test module is not a
    /// production call site, and `crates/vike-ops/tests/settings_row_writer_gate.rs` scans `src/`
    /// call sites rather than this one.
    fn set_setting(
        settings_dir: &Path,
        file: SettingsFile,
        dotted_key: &str,
        ...
        super::set_setting(settings_dir, file, dotted_key, raw_value, RowSync::Mirror)
    }

    /// [`set_setting`]'s budget-taking twin — see that wrapper for the arm's argument.
    fn set_setting_within(
        settings_dir: &Path,
        file: SettingsFile,
        dotted_key: &str,
        raw_value: &str,
...
";
    ...
    fn the_live_arm_and_the_safety_overrides_are_inside_the_ceremony() {
        ...
        for key in ["flags.reconcile", "flags.record_properties"] {
