...
pub struct FileRow {
    ...
    pub origin: String,
    /// The machine word: `default` / `file` / `env` / `db`.
    ///
    /// ⚠ This doc read *"`default` / `file` / `env`"* and never gained `db`, which
    /// `vike_config::Origin` grew when the settings database became a layer — so a reader of this
    /// field's contract was told the domain was one word short of what the shipped `--json` already
    /// emitted. `Origin::kind` is the authority; this line is a copy and is now a correct one.
    ///
    /// Which words a given box can actually produce is a fact about THAT BOX since 0057's crossing:
    /// an unadopted one reports `file` and never `db` for a key a file sets, an adopted one reports
    /// `db` and never `file`. `vike_config::Authority` is what says which, and
    /// `vike_config::precedence_line` renders it.
    ...
    ///
    /// It exists because [`Self::why_unread`] alone was not enough and was actively misleading:
    /// six rows described a library that reads the variable without saying that nothing calls that
    /// library, so the paragraph read as *export the variable instead* when exporting it does
    /// nothing. Deriving the line from the variant rather than from the prose is what keeps the two
