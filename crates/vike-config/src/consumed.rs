...
pub const CONSUMPTION: &[Consumption] = &[
    ...
    Consumption {
        ...
        by: Consumer::Not {
            // ⚠ CORRECTED, and the correction is the reason this row is still `Not` while its
            // nine siblings from the same sweep were wired. The old text said "the vike-backtest
            // bins and vike-studio", which understated the problem in the direction that matters:
            // there are THREE consuming processes and they do not share a composition root.
            //
            //   1. the Studio inside `vike-desktop`, through `vike_studio_core::run`'s
            //      `spawn_paramscan` -> `run_paramscan_slice` -> `map_bounded`, on the
            //      `Backend::Local` arm of `crates/vike-studio/src/studio.rs`'s `start_sweep`;
            //   2. `vike-backtest`'s `backtest --addr` COMPUTE SERVER — `crate::compute_server`,
            //      which serves the seven compute verbs and is where ruling 7 put them;
            //   3. a ONE-SHOT `backtest` run, which is also what
            //      `crates/vike-cli/src/cmd/backtest.rs`'s `execute_local` SPAWNS as a child rather
            //      than running in-process.
            //
            // ⚠ CORRECTED, and the correction is why this row is worth reading twice. This list
            // said "(2) the compute daemon's backtest serve arm in `vike-datahub`, which loads NO
            // settings at all" — and `vike-datahub` cannot reach this code AT ALL. Ruling 7 of
            // `docs/superpowers/specs/2026-09-09-datahub-market-data-wire-design.md` moved the
            // seven compute verbs to `vike-backend backtest --addr` and DELETED that crate's
            // `vike-backtest` and `vike-studio-core` edges outright (its `Cargo.toml` says so at
            // the removed dependency block). So the one row this table's own report certified as
            // "the one whose stated obstacle is the ACTUAL obstacle" named a consumer that does
            // not link the crate.
            //
            // ⚠ And the obstacle was mis-attributed with it. The settings-load half is real but it
            // is not where the old text put it: (1) and (2) BOTH load settings — (2)'s
            // `run_serve` is the one `vike_config::load` call in
            // `crates/vike-backtest/src/backtest_cli.rs` — while (3), the one-shot run, loads none
            // at all, which is the arm `execute_local`'s child actually is. So the split is two
            // roots WITH a load and one without, not "vike-datahub has none".
            //
            // The REAL obstacle is the one `vike_ops::settings`' module doc files under family 2,
            // and it blocks all three alike: this is a PROCESS-WIDE RESOURCE KNOB consulted deep
            // inside a fold, so even the two roots that load settings have nowhere to put the
            // value. `sweep_threads` is
            // read at rayon pool construction inside `install_bounded`, which is entered from
            // three different front doors in two crates (`optimize.rs`'s fan-out,
            // `walkforward.rs`, and the public `map_bounded` vike-studio-core takes), none of
            // which carries a settings value. Wiring the FILE key means giving that pool a
            // caller-owned process-wide handle — an `init(spec)` + `OnceLock`, or a config
            // threaded through every entry — which is a design decision, not a threading change,
            // and is the same STEP-2 item `VIKE_PIN_CORES` sits under.
            //
            // ⚠ This is the ONE `Reader::Live` row in the table: its six flag siblings claimed the
            // same shape and were wrong — their reads are reached from nothing — so the difference
            // is now CHECKED rather than asserted. `install_bounded` has a caller in `optimize.rs`'s
            // parameter-search fan-out, outside the file that defines it, the gate opens that file
            // and looks, and since the scanner stopped cutting at the first `#[cfg(test)]` that
            // caller must be PRODUCTION code (a call inside `optimize.rs`'s own test module no
            // longer satisfies it — `a_reader_live_caller_inside_a_test_module_does_not_count`).
            //
            // ⚠ It is NOT "the only one of the eight whose stated obstacle is the actual obstacle",
            // which is what this comment claimed and what the PR that wrote it repeated. That
            // sentence was the licence under which the obstacle above went two corrections without
            // being read: being the row with the live VARIABLE says nothing about whether its
            // stated reason for the FILE key is right.
            why: "`VIKE_SWEEP_THREADS` is read by `crates/vike-backtest/src/harness/sweep.rs`'s \
                  `sweep_threads`, called from `install_bounded` deep inside a rayon fan-out — and \
                  unlike the flag rows below, that read RUNS: exporting the variable still caps \
                  concurrency, only the FILE key is inert. What blocks the file key is not \
                  primarily a missing settings load: vike-desktop's Studio and \
                  `backtest --addr`'s compute server both load settings and still have nowhere to \
                  put the value (only a ONE-SHOT `backtest` run, the arm vike-cli's \
                  `execute_local` spawns, loads none). It is that this is a PROCESS-WIDE RESOURCE \
                  KNOB read at pool construction, entered from three front doors in two crates \
                  (`optimize.rs`'s fan-out, `walkforward.rs`, and the public `map_bounded` \
                  vike-studio-core takes), none of which carries a settings value. Wiring it means \
                  giving that pool a caller-owned process-wide handle — an `init(spec)` + \
                  `OnceLock` — which is `vike_ops::settings`' family-2 STEP-2 item, a design \
                  decision rather than a threading change.",
            reader: Reader::Live {
                file: "crates/vike-backtest/src/harness/sweep.rs",
                needle: "pub fn sweep_threads() -> usize",
                // `install_bounded` is `pub(crate)`; this is the in-crate caller on the live
                // parameter-search path. `map_bounded` is the public front door vike-studio-core
                // takes to the same pool — either would do, and naming the one inside the engine
                // keeps the evidence independent of whether the GUI is built.
                caller: "crates/vike-backtest/src/harness/optimize.rs",
                call: "install_bounded(",
            },
        },
    },
    // ---------------------------------------------------------------------------------------
    ...
    Consumption {
        ...
        by: Consumer::Not {
            ...
            // and it went with the desktop shell's whole local DATA plane, so there is no
            // `if flags.record_dvol {` in the tree any more and the resolved flag reaches nothing.
            // Demoting it back is the only honest answer — an `At` row here would make
            // `vike-cli config show` report `READ: yes` for a key that changes nothing, which is
            // the precise failure this whole table exists to prevent.
            //
            // ⚠ CORRECTED: this said "the desktop opens no venue socket", which is a true sentence
            // about the desktop and the WRONG mechanism for this flag. The DVOL channel is PUBLIC
            // and KEYLESS and belongs to no venue mount — the deleted block's own comment said it
            // "runs whether or not deribit is armed for execution". What the mount needed were the
            // two things the `fat` local data plane owned: a concrete `vike_data::DataFusionHist`
            // to open at the root's tick-store path, and the COMPOSED `LiveDataSink` the venue
            // feeds tee'd through. `fat` was deleted outright and both went with it.
            //
            ...
            why: "NOTHING acts on it — and this is the one row here with no library reader either, \
                  ...
                  it needs a composition root that mounts the feed, and what qualifies a root is \
                  NOT a deribit connection — the channel is public and keyless and opens its own \
                  socket — but a composed `LiveDataSink` and a concrete store to record into. \
                  vike-tradehub is the candidate on those grounds (its `record-feeds` feature \
                  already builds both), and not one line of change in this crate.",
            // The only [`Reader::Nothing`] row, which is what "a stronger admission than its
            // neighbours" means in enum form: its six `Uncalled` siblings at least have a read to
            // point at, and this one has none.
...
mod tests {
    ...
    /// …and the counterexample, so the assertion above is not vacuous: one row IS a live variable.
    #[test]
    fn the_one_live_variable_row_says_so() {
        let c = consumer_of("preferences.sweep_threads").expect("row");
        assert!(
            c.reader().is_some_and(|r| r.env_still_works()),
            "VIKE_SWEEP_THREADS genuinely still caps concurrency — only the file key is inert"
        );
        assert!(c.unread_verdict().is_some_and(|v| v.contains("still works")));
    }
