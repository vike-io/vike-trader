...
// ⚠ `Eq` was dropped when `account_exposure` landed: this type now holds an `f64`, and `Eq` over a
// float is a claim the type cannot honour. `Policy` itself derives only `PartialEq` for the same
// reason, nothing in the tree uses either as a map or set key, and the comparison every caller
// actually performs — two loaded policies in a test — is `PartialEq`.
...
pub struct VenuePolicy {
    ...
    /// **Per-ACCOUNT exposure ceilings** — the `[account_exposure]` table, keyed the same way
    /// [`Self::accounts`] is.
    ///
    /// ⚠ **A SEPARATE table from `[accounts]`, and the reason is `DEFAULT`.** That table refuses
    /// the reserved spelling outright, because an account's MODE ceiling is the venue's own line
    /// and a second place to write it would be two answers to one question. An exposure NUMBER has
    /// no venue line to inherit from — `policy.max_account_exposure` is box-wide, not per venue —
    /// so the unlabelled account needs a way to say its own figure, and folding this into
    /// `[accounts]` would have denied it one forever. Here the key is the label TEXT, with the
    /// unlabelled account spelled [`vike_model::account_keys::RESERVED_DEFAULT_LABEL`], which is
    /// the spelling `parse_wire_account` already admits everywhere else it appears.
    #[serde(skip)]
    account_exposure: BTreeMap<(&'static str, String), f64>,
}
...
impl VenuePolicy {
    ...
    /// [`Self::accounts_by_venue`]'s exposure twin — `{venue: {LABEL: figure}}`, the shape
    /// [`crate::Policy`]'s hand-written `Serialize` emits as its `account_exposure` field so the
    /// provenance walk finds a leaf there and `vike-cli config show` renders a cell for it.
    ///
    /// ⚠ Without this the table would PARSE, VALIDATE and FOLD while appearing on no disclosure
    /// surface — a key that is armed and invisible, which is the failure this workspace engineers
    /// against harder than it engineers against a key that does nothing.
    #[must_use]
    pub fn account_exposure_by_venue(&self) -> BTreeMap<&'static str, BTreeMap<String, f64>> {
        let mut out: BTreeMap<&'static str, BTreeMap<String, f64>> = BTreeMap::new();
        for ((venue, label), cap) in &self.account_exposure {
            out.entry(venue).or_default().insert(label.clone(), *cap);
        }
        out
    }
    ...
    /// **The exposure ceiling this account's own line states**, or `None` where it named no figure.
    ///
    /// ⚠ **It does not INHERIT and it does not RESOLVE** — unlike [`Self::account`], which folds a
    /// venue ceiling in, this returns the stated line and nothing else. The fold belongs to the one
    /// arming site (`vike_mount::make_engine_for_account`), which narrows
    /// `vike_exec::RiskLimits::max_account_exposure` with the box-wide
    /// `policy.max_account_exposure` and then with this, both through `narrow_account_exposure` —
    /// a `min`. Doing the fold there rather than here is what keeps "an account line can only ever
    /// TIGHTEN" a property of the operation instead of a convention two call sites must both keep.
    ///
    /// The unlabelled account is spelled [`vike_model::account_keys::RESERVED_DEFAULT_LABEL`] — see
    /// [`Self::account_exposure`]'s field doc for why this table admits it where `[accounts]`
    /// refuses it.
    #[must_use]
    pub fn account_exposure(&self, venue: &str, label: &AccountLabel) -> Option<f64> {
        let text = label.text().unwrap_or(vike_model::account_keys::RESERVED_DEFAULT_LABEL);
        lookup_owned(&self.account_exposure, venue, text).copied()
    }

    /// Every `(venue, label, figure)` the FILE named, in venue-then-label order — the exposure twin
    /// of [`Self::accounts`], and read by the same disclosure surfaces.
    pub fn account_exposures(&self) -> impl Iterator<Item = (&'static str, &str, f64)> + '_ {
        self.account_exposure.iter().map(|((v, l), x)| (*v, l.as_str(), *x))
    }

    /// **Did a policy FILE name any account under `[account_exposure]`?** — the per-table twin of
    /// [`Self::accounts_declared`], so a disclosure can tell "no table" from "a table that set
    /// nothing".
    #[must_use]
    pub fn account_exposures_declared(&self) -> bool {
        !self.account_exposure.is_empty()
    }

    /// **The mode this labelled account's own line STATED**, or `None` where it has no line.
    ///
    /// ⚠ Deliberately not [`Self::account`], which RESOLVES — folding the venue ceiling in and
    /// answering `Paper` for a labelled account with no line of its own. The one caller
    /// (`Policy::apply`'s `[account_exposure]` check) needs to tell "stated `paper`" from "never
    /// stated", and the resolver cannot: both answer `Paper`.
    pub(crate) fn account_stated(&self, venue: &str, label: &str) -> Option<VenueMode> {
        lookup_owned(&self.accounts, venue, label).copied()
    }

    pub(crate) fn set_account_exposure(&mut self, venue: &'static str, label: &str, cap: f64) {
        self.account_exposure.insert((venue, label.to_string()), cap);
    }
}
...
fn lookup_owned<'a, V>(
    accounts: &'a BTreeMap<(&'static str, String), V>,
    venue: &str,
    label: &str,
) -> Option<&'a V> {
