...
//!
//! ```text
//! code defaults -> <project>/settings/db/vike.db   (0057 Phase 1 — MIRRORED: it LOSES to the files)
//!               -> <project>/settings/*.toml -> env -> CLI
//! ```
//!
...
pub use ceilings::{
    Ceiling, CeilingHome, PRE_TRADE_CEILINGS, Site as CeilingSite, ceilings_named, shared_names,
};
...
//
// ⚠ `DEFAULT_DATAHUB_ADDR` joined it on 2026-09-20, and this line used to explain its absence with
// "nothing outside this crate had reached for it yet". What reached for it is a GUARD rather than a
// dialler, which is why it was not foreseen: `crates/vike-studio/src/remote.rs`'s
// `no_offloading_backend_defaults_to_the_datahub` asserts that neither offloading backend DEFAULTS
// to the data daemon — the exact defect that shipped when `Backend::Remote` pointed at 7878 and the
// `Run*` verbs had moved to the compute plane. A test that asked the question against a local copy
// of the address would drift with the thing it checks, so it asks against this authority, and that
// is only possible if the authority is nameable from outside.
...
pub use drift::{Drift, DriftedKey, compare_sources};
...
pub use load::{
    NO_SETTINGS_DIRECTORY_WARNING, Settings, load, load_with_cli, load_with_source, settings_files,
};
pub use mirror::{
    SECTION_FILES, adoption_integrity, apply_adopted_rows, apply_rows, file_for_section,
    rows_from_files, section_values,
};
pub use source::{Authority, StoreLayer};
...
pub use provenance::{
    Description, FileStatus, Layer, Origin, PRECEDENCE_FILES, PRECEDENCE_STORE, ResolvedSetting,
    SettingKey, describe, describe_with_source, effective_value, precedence, precedence_line,
};
...
pub use write::{
    LockBudget, RowSync, SETTINGS_LOCK_FILE, SettingsFile, SettingsWrite, SettingsWriteError,
    is_policy_plane_key, journal_outcome, requires_typed_confirm, set_setting, set_setting_within,
    typed_confirm_reason, unknown_file_message, validate_settings_text,
};
