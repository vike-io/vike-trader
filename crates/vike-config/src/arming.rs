...
/// gate cannot describe different things; only [`PolyFileArming::source`] differs, and it names
/// BOTH layers for the same reason [`TRADEHUB_LIVE_ARMING`]'s does.
const POLY_FILE_ARMING: &[PolyFileArming] = &[
    PolyFileArming {
        var: "POLY_EXEC",
        on: |f| f.poly_exec,
        source: "poly_exec (settings/flags.toml, or POLY_EXEC)",
    },
    PolyFileArming {
        var: "POLY_RECONCILE",
        on: |f| f.poly_reconcile,
        source: "poly_reconcile (settings/flags.toml, or POLY_RECONCILE)",
    },
];
...
struct PolyFileArming {
    /// The process-env variable this row's gate also answers to. It is what decides whether
    /// [`armed_settings_in`] has ALREADY reported this arm, so one gate is never counted twice.
    var: &'static str,
    /// Reads the RESOLVED flag — file ⊕ environment, environment winning — out of [`Flags`].
    on: fn(Flags) -> bool,
    /// What [`LiveArming::source`] says when the FILE layer is what armed it: both layers named.
    source: &'static str,
}
...
pub fn armed_for_live(flags: Option<Flags>, env: &HashMap<String, String>) -> LiveArmingVerdict {
    ...
    match flags {
        Some(f) => {
            ...
            // flag is the UNION of the file and the environment, so pushing both would report one
            // arm as two pieces of evidence and an operator would go looking for a second setting
            // they never made. The env row wins the tie because it names the spelling that is
            // actually set.
...
mod tests {
    ...
    fn the_polymarket_flags_are_file_evidence_now() {
        ...
        assert!(v.evidence()[0].source.contains("flags.toml"), "it must name BOTH layers: {v:?}");
        assert!(v.refuses());
        // …and the PROCESS-ENV spelling of the SAME gate is not counted twice: the resolved flag is
        // the union of the two layers, so one arm is one row, reported under the spelling that is
        // actually set.
