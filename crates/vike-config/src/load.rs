...
pub struct Settings {
    ...
    /// **WHICH SOURCE answered for every key above** — [`Authority::Files`] on every box that has
    /// not run `vike-cli config adopt`, which is the default and is what [`Settings::default`]
    /// carries.
    ///
    /// A resolved FACT rather than a re-derivable one: `vike-cli config show`'s precedence header
    /// and `vike-cli config check`'s findings both need it, and both would otherwise ask the
    /// filesystem a second time and could then answer differently from the process that is running.
    /// See [`crate::source`].
    ...
    ///
    /// ⚠ **The values in this struct are then NOT necessarily what a daemon on this box would
    /// resolve**, because whether the box is ADOPTED is itself a fact in the store that could not
    /// be opened. The loader resolves from the files and MARKS the result rather than deciding a
    /// policy it cannot see; the per-root disposition is declared in `vike_boot::BootSpec`.
    ///
    /// It is a SECOND channel rather than one more `warnings` line (the refusal is pushed there
    /// too) because a consumer has to be able to REFUSE on it: `vike-cli` keeps `config mirror`,
    /// `config adopt --undo` and every `secrets` verb running in this state — that is the JSON
    /// incident's lesson, that a refusal must not brick the tool that repairs it — while `trade`
    /// and `mcp` refuse, and a scan of free-text warnings is not a thing a verb may gate on.
    pub store_refusal: Option<String>,

    /// **The ADOPTED store opened and said something ILLEGAL** — the seal's counts do not match its
    /// tables, a row will not parse, or a row names a key the section rejects. Set by
    /// [`crate::mirror::apply_adopted_rows`].
    ///
    /// ⚠ **A SEPARATE channel from [`Settings::store_refusal`], because the two mean opposite
    /// things about the same file and want opposite repairs.** `store_refusal` is *this layer could
    /// not be READ*, so the values below fall back to the files and are merely unverified. This one
    /// is *this layer WAS read and is not trustworthy*, and on an adopted box there is no file
    /// underneath to fall back to — so the values below are whatever the rows still resolved,
    /// possibly compiled-in defaults, which for `policy.max_notional_per_order` means NO CEILING.
    /// That is vike-boot's own rule (*could this layer be read at all* degrades; *does this layer
    /// say something illegal* refuses) with the refusal moved off the loader, where it bricked the
    /// repair verbs, and onto the consumers that can act on it.
    ///
    /// **Who must refuse on it, and who must not.** The repair and disclosure verbs — `config
    /// mirror`, `config adopt --undo`, `config show`, `config compare`, every `secrets` verb — keep
    /// running, because a refusal that takes down its own named repair is the JSON incident. The
    /// surfaces that could ACT on an unsound ceiling refuse: `vike-cli trade`, `vike-cli mcp`, and
    /// `vike-cli config check`, which answers [`Level::Fail`] so a deploy pre-flight and the
    /// daemons' `ExecStartPre=` stop a start rather than a running process.
    /// `crates/vike-cli/tests/seal_enforcement.rs` carries both halves of that gate —
    /// `a_broken_seal_still_runs_every_repair_and_disclosure_verb` and
    /// `a_broken_seal_refuses_the_verbs_that_act_on_a_ceiling` — because either alone is satisfied
    /// by doing nothing, and both drive the SHIPPED binary rather than a library call. The
    /// library-level predecessor of the first is what reported green while every real binary on the
    /// box exited 1; that file's own header carries the measurement.
...
/// **[`load_with_cli`] plus the settings DATABASE, and the decision of WHICH SOURCE answers.**
///
/// `docs/decisions/0057-the-seven-settings-files-answered-one-at-a-time.md`'s crossing. The rule is
/// [`crate::source`]'s and is stated there in full; what it means HERE is two shapes, chosen on
/// one probe before any key is looked up:
///
/// ```text
/// UNADOPTED (every box until an operator runs `vike-cli config adopt` — the DEFAULT):
///   code defaults -> the settings database's rows -> policy.toml -> config.toml
///                 -> preferences.toml -> flags.toml -> env -> CLI
///
/// ADOPTED (the store carries a `vike_secrets::Adoption` row):
///   code defaults -> the settings database's rows -> env -> CLI
///                 (the four files are NOT OPENED for resolution at all)
/// ```
///
/// ⚠ **The unadopted shape is byte-identical to what this function did before the seal existed**,
/// and that is the property that makes the release carrying this change safe to deploy anywhere:
/// no box in the world is adopted on the day it ships, so no box changes. The crossing is then an
/// operator act performed with the daemon still running on the old resolution, provable with
/// `vike-cli config show` before any restart, and rolled back by deleting ONE row rather than by
/// redeploying a trading daemon.
///
/// `source` is a PARAMETER and is plain data: this crate never opens the store and must not (its
/// own manifest says so, and 0057 gives the reason under one database — a handle that reaches the
/// settings rows reaches the `credential` table too). The BINARY opens it, through
/// `vike_secrets::read_settings_in`, and hands the ARM in — see [`StoreLayer`], and see why it is
/// the arm rather than an `Option`.
///
...
/// integer outside the store's range — is a `ConfigError` here, on every box, adopted or not, and
/// is unchanged. So is the seal's own integrity check ([`crate::mirror::adoption_integrity`]) on an
/// adopted box.
///
/// A store that **could not be read at all** is different and is NOT an error return: it comes back
/// as [`Settings::store_refusal`], with the store layer dropped and the files applied. The reason is
/// the JSON incident's real lesson — ***a refusal must not brick the tool that repairs it*** — and
/// the measurement that settles it is at the arm itself: on this tree an unreadable store means an
/// empty CREDENTIAL map and therefore an all-paper mount, so a refusal would take a daemon down
/// without preventing anything. `vike-cli config check` answers `Level::Fail` for this state, which
/// puts the stop at a deploy pre-flight rather than at a running daemon.
pub fn load_with_source(
    ...
    //
    // ⚠ It is a probe of the DIRECTORY and has no row analogue, so it survives the crossing
    // unchanged and runs on an adopted box exactly as it does today.
    crate::removed::refuse_removed_project_file(settings_dir)?;

    // Layer 1 — code defaults, plus THE PROBE's answer: which source is about to answer for every
    // key below. It rides the struct from here rather than being recomputed later, so `config
    // show`'s precedence header and `config check`'s findings state a fact about the process that
    // is running instead of asking the filesystem a second question.
    let mut settings = Settings { authority: source.authority(), ..Settings::default() };

    // THE PROBE — asked once, here, before any key is looked up. `adoption` is `Some` only for a
    // store carrying the seal; see `crate::source` for why it is that row and not the database's
    // existence, the tables' existence, or a row count.
    let adoption = source.adoption();

    // Layer 1.5 — the settings DATABASE.
    match source {
        StoreLayer::Rows { rows, adopted: Some(seal) } => {
            // ADOPTED: these rows are the ONLY settings layer. An unreadable row is a refusal here
            // rather than a degrade, and the seal's counts are checked first — `apply_adopted_rows`
            // carries both arguments, including why the inversion is licensed by evidence rather
            // than by preference.
            // ⚠ NO `?` — and the missing one is the repair. See `apply_adopted_rows`, which now
            // marks `Settings::seal_refusal` in every arm that used to return `Err` here: a
            // refusal reaching this `?` took `vike-cli config mirror` down with the box, which is
            // the one thing `resolve_policy`'s arm comment says must never happen.
            crate::mirror::apply_adopted_rows(&mut settings, rows, seal);
        }
        StoreLayer::Rows { rows, adopted: None } => {
            // 0057 Phase 1, unchanged: MIRRORED, below the files. An unreadable row degrades,
            // because the files answer.
            crate::mirror::apply_rows(&mut settings, rows)?;
        }
        StoreLayer::Unreadable(why) => {
            // ⚠ **NOT "so use the files" and NOT a refusal either: RESOLVE WITHOUT IT, and MARK.**
            //
            // Whether this box is adopted is itself a fact IN the store, so a store that will not
            // open cannot answer *am I the authority here*. Falling silently through to the files
            // on that silence is the ladder re-entering through the error path; refusing outright
            // is the other wrong answer, and it was IMPLEMENTED HERE FIRST and then reversed on a
            // measurement. The measurement is worth carrying, because it is the whole reason this
            // arm is a mark rather than a hard stop:
            //
            //   `crates/vike-secrets/src/store.rs`'s `Backend` decides which store answers for a
            //   CREDENTIAL on the same file's existence, and a present-but-unopenable store
            //   returns an EMPTY credential map. An empty credential map IS the live gate. So on
            //   this tree an unreadable settings store ALWAYS co-occurs with an all-paper mount —
            //   today and after the files retire, because 0054 puts both in ONE database. The
            //   hazard a refusal would protect against, *armed and uncapped*, is unreachable from
            //   this arm.
            //
            // What the refusal WOULD have cost is not hypothetical either: it takes down a daemon
            // whose credential half already has an argued, hard-won disposition for exactly this
            // state — degrade and announce, through the `CREDENTIAL STORE UNREADABLE` banner that
            // `crates/vike-tradehub/tests/unreadable_credential_store.rs` exists to pin. Reaching
            // through a shared store to override another owner's decided disposition is the
            // "STOP-AND-REPORT, not a local patch" rule, and this is the report.
            //
            // The ENFORCEMENT POINT is `vike-cli config check`, which answers `Level::Fail` for
            // this state — so it fires at a deploy pre-flight and at the datahub's `ExecStartPre=`,
            // where a settings fault stops a release rather than a running daemon.

            settings.store_refusal = Some(format!(
                "the settings database could not be read ({why}). This process cannot tell whether \
                 this box resolves its settings from the database or from the four files, because \
                 that fact lives IN the database. The values below come from the files, the \
                 environment and the compiled-in defaults, and they are NOT necessarily what a \
                 daemon on this box would resolve."
            ));
    ...
    //
    // ⚠ **SKIPPED WHOLE on an adopted box, and that is the crossing.** Not "read and overridden":
    // a present file on an adopted box is a stale DRAFT and is INERT, and reading it here would be
    // the per-key chain the rule forbids. It is still COMPARED, one block down, so an operator who
    // edits one is told it changed nothing — but a comparison is a warning and never a value.
    if adoption.is_none()
        && let Some(dir) = settings_dir
    {
        ...
        if let Some(patch) = read_toml::<FlagsPatch>(&flags_file)? {
            ...
            if patch.reconcile == Some(false) {
                settings.warnings.push(crate::flags::reconcile_refusal_ignored(
                    &format!("`reconcile = false` in {}", flags_file.display()),
                    settings.authority,
                ));
            }
            settings.flags.apply(patch, &flags_file)?;
        }
    } else if settings_dir.is_none() {
        ...
        //
        // ⚠ The condition is `settings_dir.is_none()` and NOT the `else` of the branch above, which
        // an adopted box would also take: an adopted box HAS a settings directory (the database is
        // under it), so firing this warning there would say "I never found your project" to a box
        // whose project is the whole reason the rows resolved.
        settings.warnings.push(NO_SETTINGS_DIRECTORY_WARNING.to_string());
    }

    // ⚠ **DRIFT — the stale-draft warning, and it is a WARNING rather than a refusal.**
    //
    // On an adopted box a present settings file is inert: it cannot arm anything, cannot widen
    // anything, and can only mislead a human. That is a WARN-class defect in this tree's vocabulary
    // (`docs/decisions/0013-degrade-vs-refuse.md`'s question 2: does it reduce authority? No), and
    // making it a boot refusal was considered and REFUSED — the remedy such a refusal teaches is
    // `rm policy.toml`, and on an adopted box deletion removes nothing: the row keeps deciding, the
    // daemon boots clean, and the operator believes they took a cap off that is still on. Taught at
    // 06:40 under an unattended-upgrade restart, that is worse than the silence it removes.
    //
    // It is also nearly unreachable — `config adopt` refuses unless the two resolutions are
    // identical — so only a hand edit after the crossing produces one.
    if adoption.is_some()
        && let Some(dir) = settings_dir
    {
        settings.warnings.extend(crate::drift::boot_warnings(dir, &settings));
    }
    ...
    //
    // ⚠ The environment layer is ABOVE both sources and is untouched by the crossing. It never
    // reaches `policy`, so no ceiling acquires an override here, and `crate::source`'s doc states
    // why a variable naming the AUTHORITY would be that override wearing a different carrier.
    ...
    if crate::layers::get(env, crate::flags::RECONCILE_ENV) == Some("0") {
        settings.warnings.push(crate::flags::reconcile_refusal_ignored(
            "`VIKE_RECONCILE=0` in the environment",
            settings.authority,
        ));
    }
    ...
    if crate::layers::get(env, crate::flags::VENUE_CATALOG_ENV).is_some_and(|v| v != "1") {
        ...
        settings.warnings.push(crate::flags::venue_catalog_refusal_ignored(
            &format!("a non-`1` `{}` in the environment", crate::flags::VENUE_CATALOG_ENV),
            settings.authority,
        ));
    }
