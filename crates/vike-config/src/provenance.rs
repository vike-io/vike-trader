...
use crate::config::{
    DATAHUB_ADDR_ENV, DATAHUB_ADVERTISE_ADDR_ENV, INSTANCE_ORIGIN_ENV, JOURNAL_DIR_ENV,
    LOG_DIR_ENV, STATE_DIR_ENV, STORE_ROOT_ENV, TRADEHUB_ADDR_ENV,
};
...
pub fn setting_keys() -> Vec<SettingKey> {
    ...
    let mut out = vec![
        ...
        // The CLIENT-side dial address `vike-cli node connect` writes — the one `config` key with
