...
pub const PRECEDENCE: [Layer; 4] = [
    Layer { kind: "env", label: "env" },
    Layer { kind: "file", label: "the file above" },
    // ⚠ BELOW the file, and that ordering IS decision 0057's Phase 1: the store is written and the
    // files still win. When the files retire this row moves up and the one above it goes; nothing
    // else in this table changes, because the header is derived from it.
...
pub fn describe(
    ...
    describe_with_store(settings_dir, None, env)
}

/// [`describe`] plus **the settings DATABASE** — the disclosure twin of
/// [`crate::load_with_store`], and the read-back path
...
///
/// Same rule as the loader: the rows are applied BELOW the files during the mirror period, so a row
/// is reported as the ORIGIN only for a key no file sets — which on a correctly mirrored box is no
/// key at all, and on a hand-edited store is exactly the divergence an operator needs to see.
/// `store` is DATA; this crate never opens the store (see [`crate::mirror`]).
pub fn describe_with_store(
    settings_dir: Option<&Path>,
    store: Option<&vike_secrets::StoredSettings>,
    env: &HashMap<String, String>,
) -> Result<Description, ConfigError> {
    let settings = crate::load::load_with_store(
        settings_dir,
        store,
        env,
    ...
    let db_values: HashMap<&'static str, serde_json::Value> = match store {
