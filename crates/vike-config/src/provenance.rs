...
//! own order: env → the settings file → the settings DATABASE → the code default. There is
//! deliberately no CLI arm —
...
//!
//! **[`Policy`](crate::Policy) rows can only ever report `default`, `policy.toml` or `db`.** That is
//! not a
...
use crate::load::{CONFIG_FILE, FLAGS_FILE, POLICY_FILE, PREFERENCES_FILE, Settings};
...
pub enum Origin {
    ...
    ///
    /// ⚠ **The fourth word, and it is TEMPORARY BY DESIGN.**
    /// `docs/decisions/0057-the-seven-settings-files-answered-one-at-a-time.md`'s Phase 1 is
    /// MIRRORED — the store is written and the FILES STILL WIN — so this origin is reported only
    /// for a key that the store sets and no file sets. On a correctly mirrored box that set is
    /// EMPTY, and a row appearing here is therefore the useful signal: it says the store and the
    /// files have diverged, which during the mirror period means somebody wrote a row by hand.
    /// 0057: *"`Origin` gains its fourth word for the mirror period and narrows back when the files
    /// retire"* — at which point this variant does not go away, it becomes the ordinary answer and
    /// [`Origin::File`] is the one that narrows.
    Db,
}

impl Origin {
    /// The stable machine word: `default` / `file` / `db` / `env`. Pinned by test — a tool reading
    /// `--json`'s `origin` field must keep reading the same words, and `PRECEDENCE` is the set.
    ///
    /// ⚠ **A word `project` is RETIRED.** It was produced only by the per-project
    /// `<project>/vike.toml` layer, and that layer is gone — a present file is now refused at load
    /// (see [`crate::removed`]), so no row can carry it under any configuration. The surviving words
    /// keep their exact meanings and their exact spellings, so the `--json` domain NARROWED rather
    /// than changing: a consumer that matched on `"project"` is left with a branch that can no
    /// longer be taken, which is dead code, not a break. A consumer that matched on `"file"`,
    /// `"env"` or `"default"` sees no difference at all. Narrowing is the only compatible way to
    /// remove a layer; renaming one of them would not have been.
    ///
    /// ⚠ **`db` is a word the domain GREW**, and growth is the other compatible direction: a
    /// consumer that matched the three it knew now meets a fourth, which is why it is a documented
    /// addition rather than a silent one. See [`Origin::Db`] for why it is temporary in MEANING
    /// while the mirror lasts, and permanent in spelling after.
    ...
    /// The layer's own name: the file name, the variable name, the store's own directory, or `""`
    /// for a default.
    ...
    /// One human cell: `default`, `policy.toml`, `env:VIKE_RECONCILE`, or `db`.
...
/// **The layers this crate advertises when the FILES answer, HIGHEST precedence first** — the
/// order [`describe_with_source`] tries them on an unadopted box, which is
/// [`crate::load_with_source`]'s own order reversed.
///
/// **This is the table for every box that has not run `vike-cli config adopt`**, which on the day
/// the crossing shipped was every box in the world. [`PRECEDENCE_STORE`] is its twin.
///
/// Two gates keep the rest honest: `crates/vike-config/tests/layers_are_reachable.rs` pins both
/// tables against [`Origin`]'s variants and against the order [`describe_with_source`] really
/// resolves in, and `crates/vike-cli/tests/settings_layers_reachable.rs` proves every entry
/// actually takes effect through the shipped binary.
///
...
pub const PRECEDENCE_FILES: [Layer; 4] = [
    Layer { kind: "env", label: "env" },
    Layer { kind: "file", label: "the file above" },
    // ⚠ BELOW the file, and that ordering IS decision 0057's Phase 1 mirror: the store is written
    // and the files still win. It does not "move up" when a box adopts — the two tables are
    // MUTUALLY EXCLUSIVE, which a single linear array could not say, and saying it is the whole
    // reason this became a pair. See `PRECEDENCE_STORE`.
    Layer { kind: "db", label: "the settings database" },
    Layer { kind: "default", label: "default" },
];

/// **The layers this crate advertises when the STORE answers** — an ADOPTED box.
///
/// ⚠ **`file` is ABSENT and that is the crossing, not an omission.** On an adopted box the four
/// settings files are not opened for resolution at all, so a `file` row here would advertise a
/// layer nothing applies — which is the exact defect the `project` row above was, and the reason
/// `crates/vike-cli/tests/settings_layers_reachable.rs` exists.
///
/// ⚠ [`Origin::File`] does NOT disappear from the type, and a reader checking whether it should
/// needs the answer here: an unadopted box still reports it, forever, and unadopted is the default.
/// 0057 predicted this shape in as many words — *"`Origin` gains its fourth word for the mirror
/// period and narrows back when the files retire"* — and [`Origin::Db`]'s own doc reads it
/// correctly: the `db` variant does not go away, it becomes the ordinary answer, and `File` is the
/// one that narrows. What changes is which TABLE a given box is described by, which is a fact about
/// that box rather than about this crate.
pub const PRECEDENCE_STORE: [Layer; 3] = [
    Layer { kind: "env", label: "env" },
    Layer { kind: "db", label: "the settings database" },
    Layer { kind: "default", label: "default" },
];

/// The advertised layers for a box whose settings are answered by `authority`.
///
/// One call rather than two constants at every call site, so a surface cannot render the header for
/// one authority and the ROWS for the other.
#[must_use]
pub fn precedence(authority: Authority) -> &'static [Layer] {
    match authority {
        Authority::Files => &PRECEDENCE_FILES,
        Authority::Store => &PRECEDENCE_STORE,
    }
}
...
/// today, one change after the layer was deleted. A layer added to either table shows up here
/// automatically and the reachability gates then refuse to go green until a binary really applies
/// it; a layer removed from one disappears from the header in the same commit, with nothing to
/// remember.
///
/// ⚠ **It takes the authority, so the header is a fact about THIS BOX** rather than a sentence about
/// the crate. The policy parenthetical moves with it — `FILE ONLY` and `STORE ONLY` are the same
/// claim about the same seal (`Policy` implements neither sealed override trait, so it has no env
/// and no CLI layer under either authority); only the artifact the ceilings live in differs.
#[must_use]
pub fn precedence_line(authority: Authority) -> String {
    let layers: Vec<&str> = precedence(authority).iter().map(|l| l.label).collect();
    let policy = match authority {
        Authority::Files => "FILE ONLY",
        Authority::Store => "STORE ONLY",
    };
    format!("precedence: {}   (policy: {policy})", layers.join(" > "))
}
...
pub struct FileStatus {
    ...
    /// [`describe`] runs [`crate::load`] first, which fails naming the file and the key.
    pub present: bool,
...
pub struct Description {
    ...
    /// **WHICH SOURCE answered** — which of [`PRECEDENCE_FILES`] / [`PRECEDENCE_STORE`] describes
    /// this box, and therefore which header [`precedence_line`] renders.
    ///
    /// ⚠ [`FileStatus::present`] still reports what is on DISK, which under [`Authority::Store`] is
    /// a set of INERT drafts rather than layers. A renderer must say so — a file listed as present
    /// beside a value it did not supply is the ambiguity this field exists to remove.
...
pub fn setting_keys() -> Vec<SettingKey> {
    ...
    let mut out = vec![
        ...
        // The daemon's SELF-REPORT override. ⚠ Its blank ENV/file columns in `config show` mean
        ...
        // verified). ⚠ It belongs in `config show` more than most: an operator who has declared a
        // barrier and armed nothing — because the bind disagreed, or because no
        // `VIKE_TRADEHUB_ADMIN_KEY` reached the node-key store — has no other way to see the
        // declaration this box is running under.
        row(
    ...
    //    above gives verbatim: an account LABEL is a name the operator chose and is discovered from
    //    the credential store at runtime, so there is no compile-time key set to derive rows from.
    //
    //    ⚠ Unlike the table above, `DEFAULT` is a legal key here — the unlabelled account has no
    //    venue line to inherit a FIGURE from, only a MODE. That changes nothing about this row,
    //    which reports one origin for one table, but it is why the two tables exist separately at
    //    all (`PolicyPatch::account_exposure` carries the argument).
    out.push(SettingKey {
...
///
/// Takes the same two parameters as [`crate::load`], and calls it — so a description can never disagree
...
/// `Origin::Db` and cannot know whether the box is adopted, so on an adopted box it would describe
/// a resolution nothing is running on.
///
/// ⚠ **This doc asserted "Every shipped disclosure surface calls [`describe_with_source`] with the
/// arm it read", and that sentence was FALSE from the day it was written until 2026-09-22 — which
/// is how the defect it describes survived in the one disclosure every composition root prints.**
/// `vike_config::boot_lines` called this entry point, so a daemon's startup banner was a second,
/// store-blind resolution wearing the first one's clothes: on the live the CI box `vike-tradehub` it
/// credited `policy.toml` for a ceiling the rows set, in the same boot as a WARN saying the
/// database answered. `crates/vike-config/src/boot.rs`'s own module doc carries the journal
/// evidence and what the same read costs once the inert files are deleted.
///
/// So the claim is not restated here in stronger words — a sentence is what failed. It is a
/// property of the TYPE now: `vike_config::boot_lines` takes a [`crate::StoreLayer`] as a required
/// parameter, exactly as `vike-cli`'s `config show` and `config check` already did, so a shipped
/// surface cannot reach a store-blind resolution without writing [`crate::StoreLayer::NotConsulted`]
/// and a reason into its own diff. What survives of the old sentence is the scope: this entry point
/// is for TESTS and for a caller that genuinely has no store to offer, and it has no production
...
/// [`describe`] plus **the settings SOURCE** — the disclosure twin of
/// [`crate::load_with_source`], and the read-back path
/// `docs/decisions/0054-settings-move-into-one-database.md` requires to land BEFORE any file
/// retires (*"an operator who cannot read their settings with the daemon down is worse off than
/// today, and that alone reverses 0054"*).
///
/// Same rule as the loader, and the same probe: on an UNADOPTED box the rows are applied BELOW the
/// files, so a row is reported as the ORIGIN only for a key no file sets — which on a correctly
/// mirrored box is no key at all, and on a hand-edited store is exactly the divergence an operator
/// needs to see. On an ADOPTED box the files are not consulted for resolution at all, so every key
/// the store sets reports [`Origin::Db`] and a present file is reported as a stale draft rather
/// than as a layer. `source` is DATA; this crate never opens the store (see [`crate::mirror`]).
///
/// ⚠ **This function must never inherit the load's SOURCE refusal, and one `?` here would give it
/// one.** `config show` is the disclosure verb an operator reaches for precisely when a box will
/// not start, so a store that could not be read has to render a marked table rather than a second
/// copy of the refusal. It gets that for free today —
/// [`crate::load_with_source`] returns `Ok` with `Settings::store_refusal` set for that case rather
/// than an `Err` — and [`Description::store_refusal`] is where it is carried on to the renderer.
/// The `?` below is reached only by an ILLEGAL row or a failed seal check, which refuse everywhere
/// including here and whose repairs (`config mirror`, `config adopt --undo`) do not go through this
/// function at all.
pub fn describe_with_source(
    ...
    // of the rows.
    //
    // ⚠ **This block used to `?` here, and the comment justifying it said a section whose values do
    // not parse "cannot reach here: `load_with_store` above refused it first".** That premise died
    // when the loader learned to DEGRADE an unreadable store rather than refuse it
    // (`crate::mirror::RowRefusal`), and a `?` left in place would have moved the refusal onto THIS
    // path — `config show`, the disclosure verb an operator reaches for precisely when a box will
    // not start. That is the same brick one door over.
    //
    // So the two dispositions are matched here as well, and they mean the same things they mean in
    // the loader: an UNREADABLE store has no `db` origin to report, which is the disclosure the
    // loader's warning already names; an ILLEGAL row is unreachable (the load above refused it and
    // this function returned) and is propagated rather than swallowed, so the arm stays honest if
    // that ever stops being true.
    let db_values: HashMap<&'static str, serde_json::Value> = match source.rows() {
        Some(s) => match crate::mirror::section_values(s) {
            Ok(values) => values
                .into_iter()
                .filter_map(|(section, object)| {
                    Some((crate::mirror::file_for_section(section)?, object))
                })
                .collect(),
            Err(crate::mirror::RowRefusal::Unreadable(_)) => HashMap::new(),
            Err(crate::mirror::RowRefusal::Illegal(e)) => return Err(e),
        },
        None => HashMap::new(),
    };
    ...
    for spec in setting_keys() {
        ...
        // Precedence, highest first — the mirror of `load_with_source`'s own order, and it BRANCHES
        // on the same probe the loader branched on rather than on a second reading of the store.
        //
        // ⚠ The FILE rung is skipped whole under `Authority::Store`, exactly as the loader skips
        // layer 2 there. Consulting it here would be the worse half of a disagreement: the value
        // column would say what the rows resolved and the ORIGIN column would name a file nothing
        // read — positive confirmation of something false, from the one command an operator runs to
        ...
        let (origin, origin_value) = 'origin: {
            ...
            if authority == Authority::Files
                && let Some(v) = lookup(tables.get(spec.file), &spec.path)
            {
                break 'origin (Origin::File(spec.file), Some(v));
            }
            if let Some(v) = json_lookup(db_values.get(spec.file), &spec.path) {
                break 'origin (Origin::Db, Some(v));
            }
            (Origin::Default, None)
        };

        let adjusted = matches!(origin, Origin::File(_) | Origin::Db)
...
/// vocabulary rather than a second one: a drift message naming a key `config show` cannot show is a
/// message an operator cannot act on. It is the same `sections` + [`lookup`] pair the row loop in
...
/// runs [`crate::load`] first, so by the time this is called the only reachable `None` is "absent".
...
///
/// ⚠ **The rendering must be the FILE's, not JSON's, and that is a correctness requirement rather
/// than a cosmetic one.** [`describe_with_store`] computes `adjusted` by comparing this string
/// against the EFFECTIVE value, which is rendered by [`render`] out of a `toml::Value` — so a leaf
/// this function spells differently is reported as a value *a later rule moved*, which is a lie the
/// boot disclosure and `vike-cli config show` both print. [`crate::Policy`]'s hand-written
/// `Serialize` states the same rule for the same reason: *a rendering that did not match the file's
/// would report every account table as adjusted*.
///
/// It bites on exactly one row today and it bit for real: [`setting_keys`] has a single spec whose
/// leaf is not a scalar — `policy.accounts`, the `{venue: {LABEL: mode}}` table — where a file
/// renders `{ hyperliquid = { ALT = "paper" } }` and `serde_json`'s own `Display` renders
/// `{"hyperliquid":{"ALT":"paper"}}`. MEASURED on one store: every scalar row and all fourteen
/// `policy.venues.*` rows agreed, that one flipped to `adjusted = true`.
///
/// ⚠ So the conversion to `toml::Value` is TRIED and the JSON text is the FALLBACK — which is the
/// shape this doc previously argued AGAINST, on a premise that was half right. The conversion can
/// genuinely fail (`toml` has no unsigned 64-bit integer, so a stored value above `i64::MAX` — a
/// `deadman_timeout_ms` row, say — serializes into nothing at all), and dropping the row on that
/// failure would make `config show` report `default` for a key the loader had just resolved. What
/// it got wrong is that the failure needs a `filter_map`: a fallback costs nothing, keeps the row,
/// and confines the disagreement to the case where no TOML spelling exists to agree with. That case
/// is a SCALAR by construction — an `accounts` subtree holds venue ids, labels and mode words — so
/// the one row that needs the shared renderer can always reach it.
