...
//!
//! Phase 1 is MIRRORED: **the store is written and the files still win.** Nothing in this module
//! changes an effective value on any box, and that is the property that makes it safe to land —
//! so it is PROVEN rather than asserted, by `crates/vike-config/tests/mirror.rs`, whose
//! `the_mirror_changes_no_effective_value` resolves every settings directory it can build twice
//! (files alone, then files over the rows derived from them) and demands the two [`Settings`] be
//! equal, `warnings` included.
//!
...
fn rows_from_loaded(
    ...
    for (section, file) in SECTION_FILES {
        ...
        for (key, value) in &table {
            // The two POLICY maps are arming ceilings, not settings keys — see this module's doc.
            if section == "policy" && (key == VENUES_KEY || key == ACCOUNTS_KEY) {
                continue;
            }
    ...
    Ok(StoredSettings { settings: rows, arming })
}
...
pub fn section_values(
    ...
    for row in &store.settings {
        ...
        let Some(section) = found else {
            ...
            // `config show`'s ORIGIN column can only report a key it resolved. Closing it means the
            // reader refusing an unknown section by name, which is a REFUSAL this mirror period has
            // no appetite for (the files still answer, and a store nobody can read would strand a
            // daemon over a layer that is a copy). Stated here rather than left to be discovered.
            continue;
        };
...
fn parse_row_value(file: &Path, key: &str, raw: &str) -> Result<serde_json::Value, RowRefusal> {
    let value = serde_json::from_str::<serde_json::Value>(raw).map_err(|e| {
        ...
        //
        // Degrading is free here and that is not a compromise: this is 0057 Phase 1, the FILES still
        // win, and `crates/vike-config/tests/mirror.rs`'s `the_mirror_changes_no_effective_value` is
        // the proof that a store layer contributes nothing a file does not already decide. A layer
        // nobody can read costs exactly the disclosure it would have provided.
...
/// **Apply a store's rows to `settings`, through the existing patch types.**
///
...
///
/// ⚠ **Returns `Ok` having applied NOTHING when the store is in a previous encoding**, pushing a
/// warning onto `settings.warnings` instead — [`RowRefusal`] carries why, and why the whole LAYER is
/// dropped rather than the offending row: a half-applied section would answer for the keys whose
/// rows happened to parse and not for the rest, which is a third state nobody reasons about. The
/// files win either way, so the cost is the disclosure and not a value.
        ...
        Err(RowRefusal::Unreadable(why)) => {
            settings.warnings.push(format!(
                "the settings database was NOT read: {why} Nothing this box resolves has changed — \
                 the files still win during the mirror period — but `vike-cli config show` will \
                 report no `db` origin until the store is re-derived."
            ));
            return Ok(());
        }
