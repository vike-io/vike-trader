...
//! [`Flags::poly_auto_redeem`], [`Flags::poly_heartbeat`], [`Flags::poly_redeem_halt`],
//! [`Flags::record_chains`] and [`Flags::record_dvol`] **neither spelling does anything** — the
...
pub struct Flags {
    ...
    /// local DATA plane. It gated a whole FEED (the public keyless `deribit_volatility_index.*`
    /// subscription plus its `DvolRecorder`) from the GUI's `main.rs`, and nothing in the tree
    /// branches on this value now. ⚠ The mechanism is NOT "the GUI opens no venue socket" — this
    /// doc said that, and this channel is keyless and belongs to no venue mount (the deleted block
    /// noted it "runs whether or not deribit is armed for execution"). The mount needed the two
    /// things the `fat` local data plane owned and the desktop no longer has: a concrete
    /// `DataFusionHist` to open, and the composed `LiveDataSink` the venue feeds tee'd through.
    /// `crates/bridges/deribit/src/dvol.rs`'s module doc carries the argument.
...
impl Flags {
    ...
    ///
    /// ⚠ It CAN fail now, and the one way it can is a TOMBSTONE: six keys were deleted from this
    /// type while the variables they mirror stayed live, so a `flags.toml` still carrying one is
    /// refused by name and told which variable to export instead. See [`REMOVED_FLAG_KEYS`].
