...
//!   (`vike_tradehub::reconcile_config`'s module doc is the authority). Merging the two maps for this
...
//!
//! ⚠ **`Consumer::Not` is not one state but three, and reading it as one is how six flags here
//! came to be described as live features.** A `Not` row carries a `vike_config::Reader` saying
//! whether the ENVIRONMENT spelling still works: for `hyperliquid_hip3`-style rows awaiting a
//! thread it does, while for [`Flags::hl_outcome`], [`Flags::pm_resolve`],
//! [`Flags::poly_auto_redeem`], [`Flags::poly_heartbeat`], [`Flags::poly_redeem_halt`] and
//! [`Flags::record_chains`] **neither spelling does anything** — the
//! read sits inside a poller or a recorder constructor no composition root ever builds. Their
//! fields say so individually, and [`crate::CONSUMPTION`] is the gated statement; what re-arms one
//! is a root that MOUNTS the feature, which is a decision rather than a threading task.
//!
//! ⚠ `record_dvol` was the SEVENTH of those and is no longer a field at all: it was the one row
//! with no reader ANYWHERE — not even a library read to point at — so it was deleted with a
//! tombstone rather than carried. [`DEAD_FLAG_KEYS`] is that tombstone and carries the argument,
//! including why neither of the two pre-existing tombstone families could tell the truth about it.
//!
//! ## The file
//!
//! ```toml
...
///
/// ⚠ **`authority` is the source that ANSWERED for this resolution, and it is what the remedy
/// follows.** Without it this message named `<project>/settings/flags.toml` unconditionally — and
/// one of its four call sites is inside `crate::mirror`'s row applier, which on an ADOPTED box is
/// the ONLY settings layer and where that file is not read for resolution at all. So the operator
/// most likely to see this line was the one whose remedy could not possibly work: they would write
/// the key, restart, and get the identical warning back. `crate::remedy` carries the class and the
/// measurement.
#[must_use]
pub fn reconcile_refusal_ignored(origin: &str, authority: crate::Authority) -> String {
    let remedy = authority.write_remedy(crate::SettingsFile::Flags, "reconcile_off");
    format!(
...
/// The warning [`crate::load`] raises when a deployment exported the OLD venue-catalog variable
...
///
/// ⚠ `authority` for the same reason [`reconcile_refusal_ignored`] takes one — see `crate::remedy`.
#[must_use]
pub fn venue_catalog_refusal_ignored(origin: &str, authority: crate::Authority) -> String {
    let remedy = authority.write_remedy(crate::SettingsFile::Flags, "venue_catalog_off");
    format!(
...
/// **Six `flags.toml` keys that were DELETED, with the variable each mirrored still LIVE.**
///
...
///
/// ⚠ **These are NOT [`crate::REMOVED_ENV`] rows, and that distinction is the whole of why this
/// table exists separately.** `REMOVED_ENV` refuses a VARIABLE at startup because the operator's
/// belief in it became false when its last reader went. Every variable below is still read, today,
/// by the venue adapter that owns it (`crates/bridges/binance/src/perp_user_data.rs`'s
/// `TRADE_LITE_FILL_ENV`, `crates/bridges/bybit/src/user_data.rs`'s `fast_exec_enabled`,
/// `crates/bridges/polymarket/src/settlement/chain.rs`'s `CHAIN_WATCH_ENV` / `CHAIN_PROXY_ENV`,
/// `crates/bridges/polymarket/src/mount.rs`'s `POLY_PRESUBMIT_REGISTER_ENV`, and
/// `crates/bridges/polymarket/src/exec.rs`'s `POLY_RATE_GATE_ENV`) — so refusing one at startup
/// would take down a box whose configuration is CORRECT and working. What died is the FILE half:
/// this type mirrored each name into a field nothing downstream consumed.
///
/// The day a variable here stops being read by its adapter, its row graduates to `REMOVED_ENV`.
...
/// the file, the key, the value they wrote, and the variable that still does the job.
///
...
fn removed_flag_key(file: &Path, key: &str, written: bool) -> ConfigError {
...
///
/// ⚠ **Why not [`REMOVED_FLAG_KEYS`]?** That table's whole sentence is *"delete the key and export
/// the variable instead"*, and every row in it is true because the venue adapter that owns the
/// behaviour still reads its own variable. Here the variable's ONLY reader was this type's own
/// `apply_env`, which goes with the field — so that refusal would send an operator to a spelling
/// that has just become as dead as the one they wrote.
///
...
pub const DEAD_FLAG_KEYS: &[(&str, &str, &str)] = &[(
    ...
    // — `vike-tradehub`'s `record-feeds` feature already builds both.
    "`vike_deribit::DvolRecorder::from_flag` and `vike_deribit::spawn_deribit_dvol_feed` still \
     exist and still take the enable gate as a parameter; what is missing is a composition root \
     that mounts the feed with a composed LiveDataSink and a concrete store (vike-tradehub's \
     `record-feeds` build has both)",
)];
...
fn dead_flag_key(file: &Path, key: &str, written: bool) -> ConfigError {
...
pub struct Flags {
    ...
    ///
    /// ⚠ **UNMOUNTED: setting this changes nothing, and neither does `POLY_HEARTBEAT`.** The
    /// poller is PARKED (`crates/bridges/polymarket/src/lib.rs` says so) and no composition root
    /// spawns it. [`crate::CONSUMPTION`]'s row is the gated authority for the claim.
    ///
    ...
    ///
    /// ⚠ **UNMOUNTED: setting this changes nothing, and neither does `POLY_AUTO_REDEEM`.** Nothing
    /// outside tests constructs `AutoRedeemPoller`, and that is a DECISION rather than a gap —
    /// spawning it is new unattended on-chain money movement
    /// (`crates/vike-ops/tests/kill_switch_gate.rs`'s RETIRED row). [`crate::CONSUMPTION`]'s row is
    /// the gated authority.
    ///
    ...
    /// switches off is never spawned, so neither spelling does anything today. That is a reason to
    /// KEEP the field, not to delete it — deleting a guard while its subject survives leaves the
    /// guard behind the thing it guards, and the env half alone would silently narrow the switch
    /// (it has a second trip condition, a halt FILE on disk, that this type does not model).
    /// [`crate::CONSUMPTION`]'s row carries the argument.
    ///
    ...
    ///
    /// ⚠ **UNMOUNTED: setting this changes nothing, and neither does `VIKE_PM_RESOLVE`.** No
    /// composition root constructs `ResolvePoller`; `crates/bridges/polymarket/src/settlement/mod.rs`'s
    /// module doc states it for the whole cluster. [`crate::CONSUMPTION`]'s row is the gated
    /// authority.
    ///
    ...
    ///
    /// ⚠ **UNMOUNTED: setting this changes nothing, and neither does `VIKE_HL_OUTCOME`.** Nothing
    /// outside tests constructs `OutcomePoller`. [`crate::CONSUMPTION`]'s row is the gated
    /// authority.
    ///
    ...
    /// `None`. ⚠ **The mechanism is the STORE, not the fetch, and this doc said the fetch.** It
    /// read "the GUI Options tool that used one went with the desktop cut … what is missing is a
    /// root that FETCHES option chains, and the surviving daemon fetches none". The Options tool
    /// did NOT go: the desktop still calls `vike_app_core::tools::spawn_tool_fetchers` and still
    /// polls chains every 30 s — its own tombstone at that call site says "the Options tool FETCHES
    /// exactly as before, it just records nothing". What went is the local DataFusion store: the
    /// desktop links vike-data with DEFAULT features now (the trait-only `HistStore` seam) and
    /// `open_from_env` is a `hist-datafusion` constructor, so there is no engine to open. Re-arming
    ...
    // ⚠ `record_dvol` stood HERE and is DELETED — see [`DEAD_FLAG_KEYS`] for the tombstone and
    // the argument. It was the tree's one key with no reader on EITHER spelling.
...
impl Flags {
    ...
    ///
    /// ⚠ It CAN fail now, and the ways it can are TOMBSTONES, of TWO kinds. Keys deleted from this
    /// type while the variables they mirror stayed LIVE are refused and told which variable to
    /// export instead ([`REMOVED_FLAG_KEYS`]); keys deleted with NO live variable behind them are
    /// refused and told that neither spelling ever did anything ([`DEAD_FLAG_KEYS`]). The two
    /// messages must not be swapped — the first one, said about a dead variable, sends an operator
    /// to a second spelling that is equally dead.
...
impl EnvOverride for Flags {
    ...
    fn apply_env(&mut self, env: &HashMap<String, String>) -> Result<(), ConfigError> {
        ...
        // ⚠ `RECORD_DVOL_ENV` is NOT folded here any more — the field it set is deleted
...
pub const FLAG_REGISTRY: &[FlagMeta] = &[
    ...
    // ⚠ `record_dvol`'s stewardship row stood HERE and went with the field. The argument for
    // keeping it — that deleting the field deletes the review date that would get the feed
    // re-mounted — is answered by [`DEAD_FLAG_KEYS`], which names the two surviving symbols and
    // what a root must supply. A review date on a key nothing reads is paperwork; a refusal an
    // operator SEES is evidence.
    // --- safety overrides -------------------------------------------------------------------
    FlagMeta {
...
mod tests {
    ...
    fn a_file_true_can_be_turned_off_by_env() {
        let mut f = Flags::default();
        f.apply(FlagsPatch { poly_exec: Some(true), ..Default::default() }, Path::new("f.toml"))
            .unwrap();
        assert!(f.poly_exec);
    ...
    #[test]
    fn every_removed_flag_key_is_refused_and_names_the_variable_that_still_works() {
