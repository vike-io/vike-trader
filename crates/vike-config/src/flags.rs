...
//! [`Flags::preflight_skip`] / [`Flags::allow_withdraw_keys`] (safety OVERRIDES, where `false` is
//! the guarded state).
//!
...
use crate::layers::{get, parse_flag, CliOverride, CliOverrides, EnvOverride};
...
pub struct Flags {
    ...
    ///
    /// WIRED (and the only flag here whose wiring gates a whole FEED): `vike-app`'s `main.rs`
    /// mounts the public keyless `deribit_volatility_index.*` subscription and its `DvolRecorder`
    /// when this is on, and nothing whatsoever when it is off. The gate reaches the recorder as a
    /// PARAMETER (`vike_deribit::DvolRecorder::from_flag`), so this crate's `apply_env` is the one
    /// reader of `VIKE_RECORD_DVOL` in the tree.
    ///
