...
pub enum ArmingBlock {
    ...
    ///
    /// ⚠ This named `dukascopy` as its example — *"whose exec factory is not mounted at all"* — and
    /// that stopped being true on 2026-09-09, when the `("dukascopy", _)` arm landed. No roster
    /// venue answers this today; `vike_mount`'s `venue_arming_under` reaches it through its `_`
    /// catch-all, i.e. for a venue id that is not on the roster at all.
    NoLiveArm,
    ...
    /// `vike_fxcm::sdk_available()` refuses the live mount whatever the store says.
    SdkAbsent,
    ...
    /// venues wide when the variant was written, then one, and since 2026-09-15 **no roster venue
    /// produces it at all** — dukascopy, the last hold-out, addresses its accounts through the
    /// settings database's `account` table (`vike_mount`'s `dukascopy` module).
    ///
    /// ⚠ **That is not the same as unreachable, and the distinction is the reason this variant
    /// stays.** A venue scaffolded by `just new-venue` is deliberately NOT added to
    /// `arm_addresses_accounts` — the marker comment in that list instructs the author not to — so
    /// the first refusal a NEW venue's second account meets is this one. It is the block no venue
    /// produces today, not one nothing can produce.
    ///
    ...
    /// own refusal line says which it was (`vike_mount`'s `DukascopyRefusal::StoreUnreadable`
    ...
    /// choose — an `[accounts.dukascopy]` line above `paper` takes the session, and with no such
    /// line the DEFAULT account keeps it. `vike_mount`'s `dukascopy` module carries the rule
    /// (`sidecar_holder`), the evidence, and the measurement that retires the limit.
    ///
        ...
        match self.block {
            ...
            ArmingBlock::AccountNotInStore => format!(
                "{} cannot be armed: the settings database does not name it, so nothing says WHICH \
                 ...
                 `vike-cli secrets migrate` first. The DEFAULT account is unaffected",
                self.subject()
            ),
            ArmingBlock::SidecarHeldElsewhere => format!(
                "{} cannot be armed: this process opens ONE {v} broker session and another {v} \
                 ...
                 is the policy's to choose: an `[accounts.{v}]` line above `paper` takes it, and \
                 with no such line the DEFAULT account keeps it. To trade both at once, give the \
                 second account its own project folder (its own settings directory and credential \
                 store) and run a second process there",
                self.subject()
            ),
        }
    }
