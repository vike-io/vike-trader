...
use crate::provenance::{describe, Description, Origin, ResolvedSetting};
...
use crate::redact::{is_secret_key, SET, UNSET};
...
fn credential_store_findings(settings_dir: Option<&Path>) -> Vec<String> {
    ...
    if matches!(path.try_exists(), Ok(false)) {
        if let Some(w) = vike_secrets::legacy_store_warning(&path) {
            out.push(format!("credential store: ⚠ {w}"));
        }
    }
    out
