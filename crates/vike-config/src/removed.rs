...
pub const REMOVED_ENV: &[RemovedSetting] = &[
    ...
    RemovedSetting {
        ...
        // now" has to be told the answer is nowhere, in the file they were using, rather than left
        // to guess. The one thing they must NOT conclude is that `VIKE_STATE_ROOT` is the same
        // knob under a new name: that variable is the state ROOT, it is read by
        // vike-tradehub/vike-app-core/vike-studio, and `vike_model::state_path`'s module doc
...
mod tests {
    ...
    /// names the variable, the file it used to live in, and the reason, and it must NOT send the
    /// operator to `VIKE_STATE_ROOT`.
    #[test]
    fn the_unread_state_dir_is_refused_and_offers_no_replacement_line() {
        ...
        // `key: None` ⇒ the "set it in <file> instead:" block, with no TOML line at all.
