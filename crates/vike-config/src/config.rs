...
/// client of the compute verbs (`vike-cli study` today) dials when nothing else answers.
///
...
pub struct Config {
    ...
    /// only when no project sits above the working directory. `crates/vike-desktop/src/main.rs`'s
    ...
    /// this box, and where a client (`vike-cli backtest`/`sweep`/`walkforward`/`study`, the
    /// Studio's Remote backend) DIALS it. `None` = [`DEFAULT_BACKTEST_ADDR`].
    ///
    ...
    /// `crates/vike-cli/src/cmd/study.rs`'s `resolve_addr` is the one place it is folded.
    ///
