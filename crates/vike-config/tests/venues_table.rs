...
use vike_config::{Origin, SettingsFile, StoreLayer, VenueMode, boot_lines, describe, load};
...
/// were written against; `crates/vike-config/tests/boot.rs`'s
/// `a_box_whose_store_supplies_nothing_renders_the_same_block_under_every_arm` is what holds that true.
...
";
    ...
    let err = vike_config::set_setting(
        dir.path(),
        SettingsFile::Policy,
...
// ------------------------------------------------------------------------------------------------
// The write half of the crossing: on an ADOPTED box a file write that does not reach a row is a
// write nothing reads.
// ------------------------------------------------------------------------------------------------

/// **`RowSync::Mirror` makes a settings write take EFFECT on an adopted box; `ReportOnly` says so.**
///
/// This is the half a "read PR" cannot leave alone, and it is measured on the one key where getting
/// it wrong is worst: `policy.venues`, which is what arms a venue at all.
///
/// Once `vike-cli config adopt` has sealed a store, the loader does not open `policy.toml` for
/// resolution. A writer that edits only the file therefore produces one of two unacceptable
/// outcomes, and both were live options before this parameter existed:
///
/// * a CAPABILITY REGRESSION — a confirm-gated surface reports success for a write that changes no
///   value anybody reads; or
/// * a DELAYED OUTAGE — the divergence detonates at the next restart, which on the deployed box is
///   the 06:40 unattended-upgrade window.
///
/// So the assertion is on the RESOLVED ceiling after a restart-shaped reload, not on the file's
/// bytes — the file's bytes are identical under both arms, which is precisely the problem.
#[test]
fn on_an_adopted_box_a_write_reaches_the_ceiling_only_through_the_mirror_arm() {
    for (arm, should_apply) in [
        (vike_config::RowSync::Mirror, true),
        (vike_config::RowSync::ReportOnly("the test's other arm"), false),
    ] {
        let dir = tempfile::tempdir().unwrap();
        write(dir.path(), POLICY_FILE, "[venues]\nbinance = \"demo\"\nbybit = \"paper\"\n");
        ...
        .expect("a store to seal");
        let db = dir.path().join("db").join("vike.db");
        let rows = vike_config::rows_from_files(dir.path()).expect("derive");
        vike_secrets::write_settings(&db, &rows).expect("mirror");
        vike_secrets::write_adoption(&db, "2026-09-18T00:00:00Z", "test", POLICY_FILE, true)
            .expect("seal");
        ...
        .expect("the FILE write lands under BOTH arms — that is what makes this test necessary");
        assert_eq!(w.new_value, "\"demo\"", "the file says `demo` either way");
        assert_eq!(
            w.in_force, should_apply,
            "`SettingsWrite::in_force` is what a surface renders. A bare success for a write \
             nothing reads is positive confirmation of something false."
        );

        // The restart-shaped reload: read the store back and resolve the way a boot does.
        let source = vike_secrets::read_settings_in(dir.path()).expect("read back");
        let seal = source.adoption().expect("still sealed").clone();
        let stored = source.rows().expect("rows").clone();
        let resolved = vike_config::load_with_source(
            Some(dir.path()),
            vike_config::StoreLayer::Rows { rows: &stored, adopted: Some(&seal) },
            &HashMap::new(),
            &vike_config::CliOverrides::default(),
        )
        .expect("an adopted box resolves");

        let ceiling = resolved.policy.venues.get("bybit");
        if should_apply {
            assert_eq!(
                ceiling,
                VenueMode::Demo,
                "`RowSync::Mirror` re-derives the rows inside the lock this write already holds, \
                 so the next boot arms what the operator asked for"
            );
        } else {
            assert_eq!(
                ceiling,
                VenueMode::Paper,
                "⚠ `RowSync::ReportOnly` writes the FILE and nothing else. On an adopted box that \
                 changes no ceiling — which is correct for the daemon's control channel, because \
                 `docs/decisions/0068` rests its no-layer-above-the-grant ruling on the daemon \
                 writing no settings row and names one acquiring the ability as a reopener. It is \
                 also exactly why `in_force` has to be false, so the surface can say so."
            );
        }
        assert!(
            resolved.policy.venues.is_declared(),
            "and the box still knows an arming was stated, under both arms"
        );
    }
}
