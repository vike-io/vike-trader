...
fn an_adopted_box_does_not_open_the_files_even_to_refuse_them() {
    ...
    let rows = StoredSettings { settings: Vec::new(), arming: Vec::new(), venue: Vec::new() };
...
fn an_arming_row_wins_over_a_disagreeing_file_on_an_adopted_box() {
    ...
    let arming: Vec<ArmingRow> = vike_model::VENUES
        ...
        .collect();
    let store = StoredSettings { settings: Vec::new(), arming, venue: Vec::new() };
...
fn losing_rows_after_adoption_refuses_rather_than_resolving_to_the_defaults() {
    ...
    let emptied = StoredSettings { settings: Vec::new(), arming: Vec::new(), venue: Vec::new() };
    let msg = seal_mark(
        dir.path(),
        &emptied,
        &seal,
...
fn an_erased_arming_table_refuses_on_a_box_that_had_stated_one() {
    ...
    let erased = StoredSettings { settings: Vec::new(), arming: Vec::new(), venue: Vec::new() };
...
// ---------------------------------------------------------------------------------------------
// THE REMEDY FOLLOWS THE STORE THAT ANSWERED (`vike_config::remedy`)
//
// A warning's REMEDY is as much a claim about this box as its subject is, and on an adopted box a
// remedy that names a settings FILE is a claim that is false. The shape was measured three times
// in two days; `crates/vike-config/src/remedy.rs` carries the journal evidence and the class.
//
// This is the half of the fix that `crates/vike-ops/tests/settings_remedy_gate.rs` cannot see: the
// gate proves no NEW site spells a file, and these two prove the site that used to reaches the
// right text through the real loader, under both authorities, from the same rows.
// ---------------------------------------------------------------------------------------------

/// **The written-refusal warning on an ADOPTED box.** `reconcile = false` arrives as a ROW here —
/// the files are not read at all — so the way out has to be the verb that writes a row, and the
/// remedy may not name `flags.toml`, which this box would not read if it existed.
#[test]
fn a_written_reconcile_refusal_on_an_adopted_box_names_config_set_and_no_file() {
    let dir = tempfile::tempdir().unwrap();
    let store = StoredSettings {
        venue: Vec::new(),
        settings: vec![SettingRow {
    ...
    .expect("an adopted load with one legal row must not Err");
    assert_eq!(resolved.authority, Authority::Store, "the fixture must BE an adopted box");
    ...
    assert!(
        ...
        "⚠ THE DEFECT: this box does not read the settings files, so naming one as somewhere to \
         write sends the operator to a file nothing reads — they write it, restart, and get this \
         identical warning back: {refusal}"
    );
...
/// **…and the UNADOPTED box's text is unchanged, byte for byte.** The files arm was already
/// correct, and a class fix is not a licence to churn it. This is the half that makes the test
/// above mean something: an assertion that passed under BOTH authorities would prove nothing,
/// which is exactly how the defect shipped.
#[test]
fn the_same_refusal_on_an_unadopted_box_still_names_the_file() {
    let dir = tempfile::tempdir().unwrap();
    let store = StoredSettings {
        venue: Vec::new(),
        settings: vec![SettingRow {
    ...
    .expect("an unadopted load with one legal row must not Err");
    assert_eq!(resolved.authority, Authority::Files);
    ...
    let refusal = resolved
        ...
        .unwrap_or_else(|| panic!("the written refusal must still warn: {:?}", resolved.warnings));
    assert!(
        refusal.contains("`reconcile_off = true` in <project>/settings/flags.toml"),
        "the files arm is what shipped, unchanged: {refusal}"
    );
