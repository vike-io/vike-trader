...
use vike_config::{Origin, PRECEDENCE, REMOVED_PROJECT_FILE, describe_with_store};
...
/// settings directory.
fn winning_origin(env_layer: bool, file_layer: bool, db_layer: bool) -> String {
    ...
    // The DB layer is armed with ROWS rather than with a database file: `describe_with_store` takes
    ...
    let d = describe_with_store(Some(&settings), store.as_ref(), &env).unwrap();
...
fn the_declared_precedence_is_the_resolved_precedence() {
    ...
    let observed = vec![
        winning_origin(true, true, true),    // env over everything
        winning_origin(false, true, true),   // the settings file over the store
        winning_origin(false, false, true),  // the store over the default
        winning_origin(false, false, false), // nothing set
    ];
