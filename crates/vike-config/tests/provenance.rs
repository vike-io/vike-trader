...
fn settings_leaves() -> BTreeSet<String> {
    ...
    // loader RESOLVED, and none of them can be written in a file, an environment variable or a row.
    // `warnings` is the loader's own non-fatal output; `authority` is WHICH SOURCE answered for
    // everything above it (`vike_config::source`); `store_refusal` is set when the store could not
    // be opened at all; `seal_refusal` when it opened and said something ILLEGAL — a seal whose
    // counts do not match its tables, or a row that will not parse on a box with no file underneath
    // it. The last pair are deliberately two fields and not one: they differ in what the OPERATOR
    // must do, and `config show` renders them differently.
    //
    // ⚠ This said "The three" and named three; it is not a count any more, for the reason the rest
    // of this tree stopped writing counts down — the number was wrong the first time a field joined.
    //
    ...
    // legitimate: could an operator write this key in `policy.toml`, in the environment, or as a
    // `setting` row? If yes it needs a provenance row, not a line here.
    out.retain(|k| {
        k != "warnings" && k != "authority" && k != "store_refusal" && k != "seal_refusal"
    });
    out
}
...
/// nothing to adjust.
///
/// ⚠ **It covered the FILE layer only until 2026-09-18, and that blindness was half a defect.** This
/// test called [`describe`], which passes `None` for the store, so no assertion here could ever see
/// the `db` layer — and when the `setting` value column became a JSON scalar, `policy.accounts`
/// (the one [`setting_keys`] spec whose leaf is not a scalar) started resolving from the store as
/// `{"hyperliquid":{"ALT":"paper"}}` against an effective `{ hyperliquid = { ALT = "paper" } }`, and
/// `adjusted` fired on it. Exactly one row, on a flag `vike_config::boot` prints on every daemon
/// boot and `vike-cli config show` prints in its table and its `--json`, whose own comment says no
/// rule moves a value today. So the store half below is an EXTENSION of this test rather than a
/// parallel one: a second test would have left this one just as blind.
#[test]
fn no_row_is_reported_as_adjusted_while_no_clamp_exists() {
    ...
    // The STORE layer, on the same property. The rows are DERIVED from a settings directory by the
    // real mirror rather than hand-written, so the fixture cannot encode a spelling no box would
    // produce; they are then resolved against an EMPTY directory, so every key they carry reports
    // `db` rather than losing to a file.
    let mirrored = tempfile::tempdir().unwrap();
    write(
        mirrored.path(),
        POLICY_FILE,
    ...
    write(mirrored.path(), PREFERENCES_FILE, "log_level = \"warn\"\n");
    let store = rows_from_files(mirrored.path()).expect("the fixture directory must mirror");

    let empty = tempfile::tempdir().unwrap();
    let d = describe_with_source(
        Some(empty.path()),
        StoreLayer::Rows { rows: &store, adopted: None },
        &env(&[]),
    )
    .unwrap();
