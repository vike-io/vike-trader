...
//!
//! It lives in `vike-secrets` — the crate that already OWNS the store and declares no `vike-*`
//! dependency (⚠ this read "and has ZERO dependencies" until 2026-09-13; decision 0054 put
//! `rusqlite` in its manifest, and the `vike-*` half is the half this argument needs) — so every
//! layer can reach it: the GUI editor (`vike_connections::env_write`, which re-exports
...
/// Windows has no mode to carry — the equivalent question is an ACL one, and this no-`vike-*`-dependency
