...
//! ends of being useful: [`Account`] has this reader; `venue_setting` is READ BY THE STORE ITSELF
//! since 2026-09-22, in that [`resolve_store_in`] and [`resolve_store_scoped_in`] FOLD its rows
//! back into the legacy credential names every loader looks up — through the grammar
//! [`venue_setting_names`], which moved down into this crate on the same day so that the fold
//! could run below all three readers rather than above two of them (⚠ this clause said the table
//! was *deliberately EMPTY* and pointed at `crate::schema`'s sequencing rule; that stopped being
//! true when `vike-cli secrets move-venue-config` shipped the writer that fills it); and `venue`
//! is a
//! PROJECTION of the compiled `vike_model::venues::VENUES` roster with no reader of its own AT
...
//!
//! ⚠ **[`Account::id`] is stable for the life of ONE database file, not forever — and since the
//! account lifecycle landed, not unconditionally even within one file.** It is a SQLite rowid with
//! no `AUTOINCREMENT`, assigned in the order the migration meets credential names. TWO things free
//! a number:
//!
//! * the recovery this tree documents — DELETE the database and migrate again, after which the same
//!   accounts come back numbered differently and hold no book at all;
//! * **[`AccountEdit::Remove`]**, which deletes a row. Without `AUTOINCREMENT` a new row is handed
//!   `max(rowid) + 1`, so removing the row with the LARGEST id frees that id for the next `Add`.
//!
...
pub use db::{
    ACCOUNT_LABEL_MAX_LEN, ACCOUNT_TABLE_SCHEMA, Account, AccountEdit, AccountKeys, AccountWrite,
    Accounts, Ambiguity, BookSource, BookWrite, DbError, DbErrorKind, MigrateError, Migration,
    MigrationOutcome, MigrationPlan, MovedRows, NoAccountTable, PlannedOutcome,
    READABLE_SCHEMA_VERSIONS, RESERVED_ACCOUNT_LABEL, SCHEMA_VERSION, SourceReport, Table,
    VENUE_ACCOUNT_ID_MAX_BYTES, database_present, edit_account, is_sqlite_file, migrate,
    move_pending_rows, normalized_account_label, normalized_venue_account_id, preview,
    read_account_keys, read_accounts, read_table, read_table_scoped, set_venue_account_id,
};
#[cfg(feature = "test-support")]
pub use db::{SCHEMA_1, plant_schema_1};
pub use dotenv::{
    DB_DIR, DB_FILE, NODE_FILE, SECRETS_FILE, SETTINGS_DIR, SETTINGS_DIR_ENV, STATE_DIR,
    db_path_for, db_path_in, load_workspace_dotenv, load_workspace_dotenv_from,
    load_workspace_dotenv_scoped, node_path_for, node_path_in, parse_dotenv, project_secrets_path,
    project_secrets_path_from, project_settings_dir, project_settings_dir_for,
    project_settings_dir_from, secrets_path_in, settings_dir_of_store, settings_dir_or_last_resort,
    workspace_db_path_from, workspace_dotenv_path, workspace_dotenv_path_from,
    workspace_node_path_from, workspace_settings_dir_from,
};
pub use env_write::{save_credentials, upsert_env};
pub use schema::{
    ACCOUNT_TIERS, AccountKey, Classification, DDL, FileComments, PendingMove, Placement,
    RowReport, SchemaRefusal, scan_comments,
};
pub use settings::{
    Adoption, ArmingRow, ProfileRiskRow, ProfileRiskSource, ProfileRiskWritten, SETTINGS_SECTIONS,
    SettingRow, SettingsSource, SettingsWritten, StoredProfileRisk, StoredSettings,
    VenueSettingRow, clear_adoption, read_profile_risk, read_profile_risk_in, read_settings,
    read_settings_in, section_is_known, set_venue_setting_in, write_adoption, write_profile_risk,
    write_profile_risk_in, write_settings, write_settings_in,
};

pub use store::{
    Backend, Finding, KeyScope, LEGACY_STORE_FILE, LegacyStoreWarning, Lookup, NodeKeySource,
    PermissionWarning, Resolved, ScopedSecrets, SecretMap, SecretsError, ShadowedStore, Source,
    UndeclaredKey, backend_at, backend_in, edit_account_in, legacy_node_key_notice,
    legacy_store_warning, permission_warning, resolve, resolve_account_keys,
    resolve_account_keys_in, resolve_accounts, resolve_accounts_in, resolve_node_keys,
    resolve_project, resolve_project_scoped, resolve_scoped, resolve_store_in,
    resolve_store_scoped_in, save_credentials_to_store, set_venue_account_id_in,
    workspace_backend_from,
};
// The `venue_setting` NAMING GRAMMAR. It lived in `vike_bridge_core::credentials` until 2026-09-22
// and MOVED here — not a re-export of it, and there is no shim left at the old home: the fold that
// applies it has to run inside the store, and a layer-30 crate is above two of the three readers.
// That module's own doc carries the two measured blindnesses that forced the move.
pub use venue_setting::{
    HAND_MAPPED_ACCOUNTS, HandMappedAccount, hand_mapped_prefix, parse_venue_setting_key,
    venue_setting_key, venue_setting_names,
};
