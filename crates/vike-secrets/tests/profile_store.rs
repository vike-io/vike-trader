...
// ---------------------------------------------------------------------------------------------
// One namespace, three kinds
// ---------------------------------------------------------------------------------------------

/// **the CI box's own shape**: an ACTIVE `recorder` profile called `default`, carrying subscriptions.
/// `deploy/vike-datahub.service` runs `--recorder-profile default`, so this is not a contrived
/// collision — it is the name one flag on the mirror verb reaches.
fn active_recorder_named_default(db: &std::path::Path, w: &OperatorWrite) {
    ...
    store_profile(db, &stored, w, 1, VOCABULARY).expect("store the recorder body");
    set_active(db, ProfileKind::Recorder, "default", w, 2, VOCABULARY).expect("activate it");
}

/// A `run` body under the same name — the CI box's live `run-live.toml` with `--profile-name default`.
fn run_body_named_default() -> StoredProfile {
    StoredProfile {
        row: ProfileRow {
            name: "default".to_string(),
        ...
        params: BTreeMap::new(),
        settings: [
            ("mode".to_string(), "\"live\"".to_string()),
            ("risk.max_notional_per_order".to_string(), "100.0".to_string()),
        ]
        .into_iter()
        .collect(),
        recorder: None,
...
/// **⚠ THE KILL PROOF: a name already held by ANOTHER KIND is refused, and the refusal is what
/// stands between one CLI flag and an armed, un-activated run plane.**
///
/// `profile.name` is `TEXT PRIMARY KEY` — ONE namespace across all three kinds — and
/// [`store_profile`] replaces a body with `DELETE` + `INSERT` while deliberately PRESERVING the
/// `active` bit it finds. Without this refusal,
/// `vike-cli config mirror --no-settings --profile settings/run-live.toml --profile-name default`
/// against the CI box's store would have:
///
/// 1. deleted the ACTIVE `recorder` body and cascaded its `subscription` rows away — the datahub's
///    venue feeds, gone, with the unit still passing `--recorder-profile default`;
/// 2. re-inserted the name as `kind = 'run'` with `active = 1` INHERITED, so `vike-tradehub` would
///    build its `vike_exec::ProfileRisk` from that body at the next boot — the crossing
///    `vike-cli config activate --proves` exists to gate, reached with no `--proves` and no fence;
/// 3. reported `Withhold { NothingSelectedToday }`, i.e. *"No active run row was written … this box
///    resolves its run profile exactly as it does today"*, which would have been false.
///
/// Every one of those three is asserted below as a thing that did NOT happen.
#[test]
fn a_name_held_by_another_kind_is_refused_and_nothing_it_held_is_touched() {
    let fx = Fixture::migrated();
    let db = fx.db();
    let w = OperatorWrite::claim("test");
    active_recorder_named_default(&db, &w);

    let e = store_profile(&db, &run_body_named_default(), &w, 3, VOCABULARY)
        .expect_err("a `run` body may not be stored under a name a `recorder` row holds");
    assert!(
        matches!(&e, ProfileError::NameHeldByAnotherKind { name, held, wanted }
            if name == "default" && held == "recorder" && *wanted == ProfileKind::Run),
        "{e:?}"
    );
    let text = e.to_string();
    for needle in ["recorder", "run", "default", "NOTHING WAS WRITTEN"] {
        assert!(text.contains(needle), "the refusal must name {needle:?}: {text}");
    }

    // 1 — the recorder body and its subscriptions are still there.
    let back = read_profiles(&db).expect("read back");
    let got = back.by_name("default").expect("the recorder profile must still exist");
    assert_eq!(got.row.kind, ProfileKind::Recorder, "the kind was not rewritten");
    let recorder = got.recorder.as_ref().expect("the recorder body must survive the refusal");
    assert_eq!(recorder.subscriptions.len(), 1, "the subscription rows were not cascaded away");
    assert_eq!(recorder.subscriptions[0].family.as_deref(), Some("btc-updown-5m"));
    assert!(got.settings.is_empty(), "and no `run` setting rows landed beside it");

    // 2 — the `active` bit still belongs to the recorder, and NOTHING is active on the run plane.
    assert!(got.row.active, "the recorder row is still the active one");
    assert_eq!(back.active(ProfileKind::Recorder).map(|p| p.row.name.as_str()), Some("default"));
    assert_eq!(
        back.active(ProfileKind::Run),
        None,
        "THE ONE THAT MATTERS: no run row was armed by a verb that only stores bodies"
    );
}

/// The same name, the same KIND, is still a replacement — the refusal above may not have turned an
/// ordinary body edit into a refusal, which is what `store_profile` is for.
#[test]
fn the_same_name_of_the_same_kind_still_replaces_the_body() {
    let fx = Fixture::migrated();
    let db = fx.db();
    let w = OperatorWrite::claim("test");
    store_profile(&db, &body("tradehub", "bybit"), &w, 1, VOCABULARY).expect("store");
    set_active(&db, ProfileKind::Daemon, "tradehub", &w, 2, VOCABULARY).expect("activate");
    store_profile(&db, &body("tradehub", "okx"), &w, 3, VOCABULARY)
        .expect("a same-kind re-store is an EDIT and must still be allowed");
    let back = read_profiles(&db).expect("read");
    let got = back.by_name("tradehub").expect("still there");
    assert_eq!(got.mounts[0].venue, "okx", "the edit landed");
    assert!(got.row.active, "…and the active bit it held is still preserved");
}

/// **A kind word this binary cannot PARSE still holds the name.** The refusal compares the raw
/// column word rather than a [`ProfileKind`], because a guard that first had to understand the
/// squatter would let through exactly the rows nobody can reason about — one written by a schema
/// this binary predates, or by hand.
#[test]
fn an_unparseable_kind_word_holds_the_name_just_as_firmly() {
    let fx = Fixture::migrated();
    let db = fx.db();
    let w = OperatorWrite::claim("test");
    // The `profile` table, created here WITHOUT the three-word `kind` CHECK and then left alone by
    // `profile_ddl`'s `CREATE TABLE IF NOT EXISTS` — the shape a store written by a schema this
    // binary predates is in. Creating it before anything else is what makes the plant possible at
    // all: the shipped CHECK would refuse the row, which is exactly why this state can only arrive
    // from a schema change or a hand edit.
    {
        ...
        conn.execute_batch(
            "CREATE TABLE profile (\
             ...
             INSERT INTO profile (name, kind, active) VALUES ('future', 'backtest', 1);",
        )
        .expect("plant a row of a kind this binary does not read");
    }
    let e = store_profile(&db, &body("future", "bybit"), &w, 4, VOCABULARY)
        .expect_err("an unreadable kind still holds the name");
    assert!(
        matches!(&e, ProfileError::NameHeldByAnotherKind { held, .. } if held == "backtest"),
        "{e:?}"
    );
    let conn = rusqlite::Connection::open(&db).expect("open");
    let (kind, active): (String, i64) = conn
        .query_row("SELECT kind, active FROM profile WHERE name = 'future'", [], |r| {
            Ok((r.get(0)?, r.get(1)?))
        })
        .expect("the planted row is still there");
    assert_eq!((kind.as_str(), active), ("backtest", 1), "untouched, bit and all");
}
