...
// they run as this crate's own `tests/funding_capture_engine.rs`, reaching the simulator through
// the upward DEV edge the manifest declares. (They lived in `vike-backtest`'s `tests/` until the
// Polymarket takers landed and the gates came home with them.) What stays below
