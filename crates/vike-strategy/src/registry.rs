...
pub const SIMULATOR_ONLY: &[(&str, &str)] = &[
    ...
    (
        ...
        "lives in THIS crate now (`crate::cheap_np`) and is still listed here, which is not a \
    ...
    (
        ...
        Liveness::Live {
            why_safe: "mountable, but the daemon does NOT resolve it through this arm and that is \
                       ...
                       before `vike_run::MakerMountConfig::crypto` existed. \
                       `vike_tradehub::config::DaemonProfile::resolve_strategy` builds it from the \
                       profile's own maker fields through `vike_run::build_maker` — the SAME call \
    ...
    (
        ...
        Liveness::Live {
            why_safe: "the same construction and the same refusal as `spread_maker`: the daemon \
                       reaches it through `vike_run::build_maker` from the profile's own maker \
    ...
    (
        ...
        Liveness::NotLive {
            blocker: "TWO-LEG and cross-venue: needs one declared mount leg per carry venue, and \
                      no mount surface declares legs today (`vike_run::MountSpec`'s `legs` is \
...
mod tests {
    ...
    /// `crates/vike-tradehub/src/hot_reload.rs`'s `every_hot_row_carries_a_written_reason`, which
    ...
    /// (`crates/vike-ops/tests/settings_registry.rs`'s `LIBRARY_PIN`).
    ///
