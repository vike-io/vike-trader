...
use vike_model::{
    one_price_bar, order_fill_price, Bar, Broker, Fill, OrderEventKind, OrderKind, OrderLifecycle,
    TradeFold, TripleBarrier, WorkingOrder,
};
...
pub fn evaluate_barriers(
    ...
    if let Some(limit) = barriers.time_limit_ms {
        if time_barrier_hit(entry_ts, limit, now) {
