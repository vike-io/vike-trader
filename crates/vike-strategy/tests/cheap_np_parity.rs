...
//!
//! Runs the Rust gate ([`vike_strategy::fair_value::cheap_gate`], the same predicate
//! [`vike_strategy::CheapNp`] fires on) over the SAME pre-joined taker-BUY prints the Python
...
//! that produced the 12,639-entry reference. ⚠ These fixtures are FROZEN: the generator that
//! re-derived them was deleted with the last Python under `crates/`, on the same rule
//! `docs/decisions/0021-python-oracle-retired-vike-is-the-reference.md` applies to `fixtures/r0..r6/`
//! — "no exporter survives in this tree", so the committed bytes ARE the oracle.
//! That Python reproduced the gate bit-identically (verified: 0 mismatches against
//! `cheap_np_ref.parquet`) and emitted the
...
//!
//!   ```sh
//!   cargo test -p vike-strategy --test cheap_np_parity -- --ignored --nocapture
//!   ```
//!
...
use vike_strategy::fair_value::{H, THETA, cheap_gate, fee, trailing_sigma};
...
/// Fixed-width little-endian records, 48 bytes, `<IIiBbxx dddd`:
///     u32 sts | u32 fts | i32 t | u8 outcome_index | i8 winning_index (-1 = unresolved) | 2 pad
///     f64 ask | f64 s_open | f64 s_now | f64 sigma_sec
/// Rows are in the source parquet's own order; this side re-sorts exactly as polars did. That
/// layout was documented by the now-deleted generator and is recorded HERE so the frozen bytes
/// stay readable without it.
///
/// Plain `std::fs` on purpose: reading parquet from this test would mean adding an
...
/// Python (60 evenly-spaced whole windows out of 28,779, the deleted generator's sample rule):
/// ```text
...
/// ```
#[test]
#[ignore = "reads the gitignored 343 MB .cheapnp_ref/signals.bin — un-re-armable, see the module doc"]
fn full_reference_set_reproduces_every_published_number() {
    ...
    if !sig.exists() || !ent.exists() {
        eprintln!("SKIP: {} not staged, and NO generator survives to re-derive it", sig.display());
        return;
    }
