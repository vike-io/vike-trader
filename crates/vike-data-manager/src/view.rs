...
/// `crates/vike-app-core/src/tool_views/data_rail.rs` sets it from the window's rail instead.
...
// made the window carry two stacked navigations. `crates/vike-app-core/src/tool_views/data_rail.rs`
...
/// `crates/vike-datahub/src/server.rs`'s `delete_series_verb` answers it. What makes remote Delete
/// still wrong FROM THIS SEAT is narrower: that verb is served only by a KEYED datahub, and no
/// crate on this side resolves datahub keys. `crates/vike-app-core/src/stored_mode.rs`'s
...
mod tests {
    ...
    /// is kept in step with `vike_app_core::stored_mode`'s `DELETE_LOCAL_ONLY` so a reader does not
