...
const TOOLS: &[Tool] = &[
    ...
    // ⚠ Same feature as `trade`, and NOT a daemon. See `catalog_main`.
    #[cfg(feature = "vike-tradehub")]
    Tool {
        name: "catalog",
        main: catalog_main,
        summary: "list a credentialed venue's instruments with this box's own credentials",
    },
    // ⚠ Same feature as `trade` and `catalog`, same reason, and NOT a daemon. See `venues_main`.
    #[cfg(feature = "vike-tradehub")]
    Tool {
        name: "venues",
        main: venues_main,
        summary: "which venues and accounts this box would mount, and why the rest stay on paper",
    },
    #[cfg(feature = "vike-cli")]
    Tool {
        name: "vike-cli", main: cli_main, summary: "the operator CLI — config, secrets, trade"
    },
    // ⚠ The daemon that runs `cargo` against an authenticated caller's Rust strategy source
    // (`docs/superpowers/specs/2026-09-21-runtime-loaded-rust-strategies-design.md`, "The builder
    // service"). Its OWN standalone binary (`crates/vike-strategy-builder/src/bin/
    // vike-strategy-builder.rs`) still exists and still works unchanged — this row is a SECOND
    // composition root over the same `vike_strategy_builder::builder::run`, not a replacement.
    #[cfg(feature = "vike-strategy-builder")]
    Tool {
        name: "strategy-builder",
        main: strategy_builder_main,
        summary: "compile an authenticated caller's Rust strategy into a plugin",
    },
];
...
/// ⚠ Takes the context and USES only its `env` — this tool has no argv of its own
/// (`VIKE_STRATEGY_BUILDER_*`-shaped configuration only, same as its standalone binary), so `args`
/// is unused. `vike_strategy_builder::builder::run` is the ENTIRE sequence: every refusal, every
/// default, the socket bind and the (never-returning, on success) serve loop, in one shared
/// function this dispatcher and the crate's own `vike-strategy-builder` binary both call — see
/// that function's doc for why the sequence lives there rather than being copied here. This row
/// exists so `--features full` (what `.github/workflows/release.yml` builds) carries this daemon
/// too; nothing about what it DOES differs from running the standalone binary.
#[cfg(feature = "vike-strategy-builder")]
fn strategy_builder_main(ctx: &ToolCtx<'_>, _args: &[String]) -> ExitCode {
    vike_strategy_builder::builder::run(ctx.env)
}
...
/// ⚠ **A READ-ONLY REPORT behind the daemon's feature, and it starts no daemon and mounts nothing.**
///
/// `vike-backend venues` answers *"I set `live` and it is still on paper — why?"*. It shares
/// `vike-tradehub`'s feature for `catalog_main`'s reason below, one rung sharper: its `EFFECTIVE`
/// column is `vike_mount::venue_account_arming`'s answer, and that function reaches thirteen venue
/// crates because each exchange's tier rule is its own. `vike-cli` — where an operator might look
/// for it first — is held transport-free by `crates/vike-boot/tests/dependency_floor.rs`, the same
/// gate that refuses `catalog` there, so this binary is the only place the screen can be built at
/// all. It installs no signal handler, opens no socket and can place no order.
#[cfg(feature = "vike-tradehub")]
fn venues_main(ctx: &ToolCtx<'_>, args: &[String]) -> ExitCode {
    vike_tradehub::venues_cli::run(ctx.env, ctx.cwd, args)
}

/// ⚠ **A ONE-SHOT COMMAND behind the same feature as the daemon above, and it starts no daemon.**
///
/// `vike-backend catalog` lists a CREDENTIALED venue (alpaca / oanda / ctrader) with THIS BOX'S own
/// credentials — `docs/decisions/0066`'s decision 9. It shares `vike-tradehub`'s feature because it
/// shares that crate's bridge edges and its credential-store read, and for no other reason: it
/// mounts nothing, installs no signal handler and can place no order.
///
/// ⚠ It is a SEPARATE ROW rather than a subcommand of `trade`, deliberately. That verb's summary is
/// "the live trading daemon" and it is the `ExecStart=` of two tracked units; folding a one-shot
/// command into its argv would put a parse ahead of the signal handlers
/// `crates/vike-ops/tests/graceful_stop_pin.rs` checks positionally.
#[cfg(feature = "vike-tradehub")]
fn catalog_main(ctx: &ToolCtx<'_>, args: &[String]) -> ExitCode {
    vike_tradehub::catalog_cli::run(ctx.env, ctx.cwd, args)
}
...
fn backtest_main(ctx: &ToolCtx<'_>, args: &[String]) -> ExitCode {
    ...
    // ⚠ THE BUILD STAMP (`vike_model::runs::BuildStamp`). This dispatcher is a composition root
    // that DOES depend on `vike-buildinfo` — it already prints `version_line` for its own
    // `--version` — so it can answer the question `crates/vike-backtest` is exempted from asking:
    // that crate's build script would rerun on every HEAD move and rebuild the simulator and
    // everything above it. Same shape as the studio table above: the library declares a
    // dependency-free seam and the root fills it.
    //
    // ⚠ `GIT_SHA` is the literal `UNKNOWN` when it could not be resolved (no git, a source tarball,
    // a shallow clone). Converted to `None` rather than written through, because a manifest field a
    // reader takes for a commit must not hold the word "unknown".
    //
    // ⚠ `summary` must OUTLIVE `build`, which borrows it — declared first, because locals drop in
    // reverse declaration order.
    let summary = vike_buildinfo::summary();
    let build = vike_model::runs::BuildStamp {
        git_sha: (vike_buildinfo::GIT_SHA != vike_buildinfo::UNKNOWN)
            .then_some(vike_buildinfo::GIT_SHA),
        summary: Some(&summary),
    };
    // ⚠ THE STUDY MOUNT (ruling R1 of
    // `docs/superpowers/specs/2026-09-12-backtest-cli-surface-design.md`), beside the Studio one and
    // for the identical layer reason: `vike_studio_core::study_run_fn` is at 55, the seam it fills
    // (`vike_backtest::compute_server`'s `StudyRunFactory`) is at 50.
    //
    // ⚠ **The FUNCTION ITEM, not a built runner, and this dispatcher resolving its two paths would
    // be a gate failure rather than a style choice.** `crates/vike-ops/tests/multicall_gate.rs`'s
    // `the_dispatcher_starts_nothing` fails a PR that names `state_path::` here at all — any
    // project-relative resolution in a dispatcher whose whole contract is "one env sweep, one
    // current_dir, hand over" is the second walk in another costume. So the runs root and the
    // pinned trainer are resolved by `backtest_cli`'s `--addr` arm, on the rung that already walks
    // for this daemon's settings, and handed to this constructor there.
    vike_backtest::backtest_cli::run(
        ...
        Some(vike_studio_core::study_run_fn),
    )
}
...
/// ⚠ **This took NO context until 2026-09-23, on the stated ground that `study` "reads no
/// environment variable at all — it takes eight `--flags` and nothing else".** That stopped being
/// true when the study started routing its history
/// (`docs/decisions/0084-only-the-datahub-touches-the-store.md`): the peer comes from
/// `$VIKE_DATAHUB_ADDR` and the credential store from `$VIKE_SETTINGS_DIR`. It is the process's ONE
/// sweep that is handed over, not a second read — which is the whole reason `ToolCtx` carries it.
#[cfg(feature = "study-cli")]
fn study_cli_main(ctx: &ToolCtx<'_>, args: &[String]) -> ExitCode {
    vike_studio_core::study_cli::run(ctx.env, args)
}
