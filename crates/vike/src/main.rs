...
///
/// ⚠ **`name` and the `#[cfg]` feature are two different strings on four of these rows, and that is
/// deliberate.** The FEATURE names the optional dependency it switches on, whose package really is
/// `vike-tradehub`/`vike-recorder`/`vike-datahub`; the NAME is what an operator types after
...
const TOOLS: &[Tool] = &[
    ...
    Tool { name: "recorder", main: recorder_main, summary: "record venue market data" },
    ...
    //
    // The `hub` suffix went from `datahub` and `recorder` in the same cut; leaving it on this one
