//! `vike` — the multi-call dispatcher's pure core: the tool table, the context every tool is
...
//! `OnceLock`s besides. A `vike a && vike b` that ran both in one process would silently give the
//! second tool the first one's logging configuration and signal handlers. There is no safe version
//! of that, so [`Router::dispatch`] refuses a second call rather than documenting the hazard, and
//! `crates/vike-ops/tests/multicall_gate.rs` holds the refusal.
//!
...
pub struct ToolCtx<'a> {
    ...
    /// symlink Linux resolves that to the dispatcher, so every tool would print `vike`.
...
pub struct Tool {
    /// The name this tool answers to, as a verb and as an argv[0] basename. Must equal the
    /// binary name it replaces, or an installed symlink stops resolving.
    pub name: &'static str,
    pub main: ToolMain,
    /// One line for `--help`.
...
pub enum Route<'a> {
    ...
    /// `--version` for itself (through its own parser), so the bare `vike --version` answering a
    /// usage ERROR was the one inconsistency — and the image smoke runs `--version` through every
    /// link, the dispatcher's included.
    Version,
}

/// The basename of an argv[0], with a Windows `.exe` suffix removed.
///
/// ⚠ Both halves matter. Without the basename, an absolute `ExecStart=/srv/vike/bin/vike-tradehub`
...
/// 2. **argv[0] is anything else** — the `vike <verb>` shape. `argv[1]` selects, arguments are
///    `argv[2..]`.
///
/// ⚠ Shape 1 is tried FIRST, and it has to be: the dispatcher will be installed under every tool's
/// name, so `vike-tradehub --config x` must route by argv[0] rather than trying to read `--config`
/// as a verb.
#[must_use]
...
pub fn dispatch(tool: &Tool, ctx: &ToolCtx<'_>, args: &[String]) -> ExitCode {
    assert!(
        ...
        "vike: a second tool was dispatched in one process. Every tool here owns process-global \
         state that assumes it is alone (vike_log::init keeps only the first LogConfig, \
         stop::install_handlers registers once, vike_script's INSTALLED is a one-shot), so the \
         second would silently inherit the first's. Run one tool per process."
    );
    (tool.main)(ctx, args)
}
...
pub fn usage(tools: &[Tool]) -> String {
    let mut out = String::from(
        "vike — one executable, every headless vike tool.\n\n\
         usage: vike <tool> [args...]\n\
...
mod tests {
    ...
    fn table() -> Vec<Tool> {
        vec![
            ...
            Tool { name: "vike-tradehub", main: t, summary: "the live daemon" },
        ]
    }
    ...
    fn a_basename_is_taken_from_a_full_path_on_both_separators() {
        assert_eq!(program_name("/srv/vike/bin/vike-tradehub"), "vike-tradehub");
    ...
    fn argv0_names_the_tool_and_keeps_every_argument() {
        let tools = table();
        let a = argv(["/srv/vike/bin/vike-tradehub", "--config", "x.toml"].as_slice());
        match resolve(&tools, &a) {
            Route::Tool { index, args } => {
                assert_eq!(tools[index].name, "vike-tradehub");
                assert_eq!(args, &a[1..]);
            }
            other => panic!("{other:?}"),
        }
    ...
    /// ⚠ argv[0] must win over the verb shape, or `vike-tradehub --config x` would try to read
    /// `--config` as a tool name and print usage instead of starting the daemon.
    #[test]
    fn argv0_wins_over_the_verb_shape() {
        let tools = table();
        let a = argv(["backtest", "vike-tradehub"].as_slice());
        match resolve(&tools, &a) {
            Route::Tool { index, args } => {
    ...
    fn the_verb_shape_drops_the_program_name_and_the_verb() {
        let tools = table();
        let a = argv(["vike", "backtest", "--from", "2026-01-01"].as_slice());
        match resolve(&tools, &a) {
            Route::Tool { index, args } => {
    ...
    fn help_exits_zero_and_an_unknown_verb_exits_two() {
        let tools = table();
        assert_eq!(resolve(&tools, &argv(["vike", "--help"].as_slice())), Route::Usage(0));
        assert_eq!(resolve(&tools, &argv(["vike", "nope"].as_slice())), Route::Usage(2));
        assert_eq!(resolve(&tools, &argv(["vike"].as_slice())), Route::Usage(2));
        assert_eq!(resolve(&tools, &[]), Route::Usage(2));
    }

    /// The dispatcher answers `--version` like every tool behind it does — the image smoke runs
    /// `--version` through EVERY link including the bare `vike`, so a usage-error answer here
    /// would fail a correctly built image. Under a TOOL's name the flag stays the tool's to
    /// answer: `vike-tradehub --version` must reach tradehub's parser, not this arm.
    #[test]
    fn the_dispatcher_answers_version_and_a_tool_link_keeps_the_flag() {
        let tools = table();
        assert_eq!(resolve(&tools, &argv(["vike", "--version"].as_slice())), Route::Version);
        assert_eq!(resolve(&tools, &argv(["vike", "-V"].as_slice())), Route::Version);
        match resolve(&tools, &argv(["/opt/vike/bin/vike-tradehub", "--version"].as_slice())) {
...
mod arg_passing_tests {
    ...
    fn a_tool_receives_its_own_argv_tail_and_nothing_is_eaten() {
        ...
        let via_verb: Vec<String> =
            ["vike", "vike-cli", "config", "check"].iter().map(|s| (*s).to_string()).collect();
