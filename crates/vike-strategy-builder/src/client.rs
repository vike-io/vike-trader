...
use vike_datahub_client::node_auth::{self, NodeKeys};
...
pub fn build_remote(
    ...
    let mac = node_auth::sign(builder::DOMAIN, keys.key_for(scope), &nonce, PROTO_VERSION, scope);
...
mod tests {
    use super::*;
    use vike_datahub_client::node_auth::Scope;
    ...
    fn a_mac_this_client_signs_is_one_the_service_verifies() {
        ...
        let scope = builder::required_scope();
        let mac =
            node_auth::sign(builder::DOMAIN, keys.key_for(scope), &nonce, PROTO_VERSION, scope);
