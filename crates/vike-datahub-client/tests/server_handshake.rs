...
fn hello_handshake_over_serve_returns_version_and_features() {
    ...
    // ⚠ `backtest` is NOT in this list any more. Ruling 7 of
    // `docs/superpowers/specs/2026-09-09-datahub-market-data-wire-design.md` moved the compute verbs
    // to `vike-backend backtest --addr`, and this harness drives the DATA daemon — the compute
    // daemon's own advertisement is proven in `crates/vike-backtest/tests/compute_plane.rs`.
    for verb in ["load_bars", "scan_quotes", "scan_trades", "properties_as_of"] {
...
fn connect_handshakes_and_exposes_features() {
    ...
    assert!(
        client.features().iter().any(|f| f == "load_bars"),
