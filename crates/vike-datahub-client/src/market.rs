...
//! `vike-model` (`BookLevel`, `TradeTick`, `LiveVerb`), `serde`/`serde_json`, and `rand` (already here
//! for [`vike_node_proto::auth::fresh_nonce`], and what [`MdSessionId::fresh`] mints from). This crate
...
/// anything. On a KEYED server the connection's [`vike_node_proto::auth::Scope`] is the ceiling, and the
...
impl MdSessionId {
    ...
    /// same call, [`vike_node_proto::auth::fresh_nonce`] draws the auth nonce from (`fill_bytes` rather
...
pub struct BookSnapshot {
    ...
    /// `crates/vike-marketdata/src/orderbook.rs`'s `L2Book::top_n` returns (`bids.iter().rev()`), and the
    ...
    /// `crates/vike-marketdata/src/orderbook.rs`'s `L2Book` has `tick_size` and `last_seq` and no time
