...
///
/// The server builds this from a `BacktestResult` at the boundary (`vike_studio_core::wire_run`'s `to_wire_result`);
...
///
/// Only `n_splits` crosses, and the two knobs that do NOT are fixed in different ways. The
/// `WalkMode` (`Anchored`) is hardcoded inside
/// `vike_studio_core::run_walkforward_slice_with_params` and is not wire-configurable. The
...
pub struct WireWfWindow {
    ...
    ///
    /// Today that is EVERY run this verb serves:
    /// `vike_studio_core::run_walkforward_slice_with_params` hands each window a
    /// `WindowOutcome::fixed`, so a `Some` cannot yet come out of the server arm. It is mirrored
    /// anyway because the alternative is a field that vanishes silently the day
    /// an optimizing walk reaches this verb — and a walk-forward optimization whose per-window
    /// choices went missing reports a stitched number with no way to tell a procedure that found
    /// something stable from one that wandered, which is the whole diagnostic the engine field was
    /// added for.
    ///
...
mod tests {
    ...
    fn wire_sweep_result_round_trips_including_none_dsr_pbo() {
        ...
        round_trip(&WireParamscanResult { entries: vec![], dsr: None, pbo: None, best_index: 0 });
    }
    ...
    fn wire_wf_window_round_trips() {
        // the fixed-parameter walk's shape — every window this verb serves today
