...
pub struct RemoteHistStore {
    ...
    /// back on a socket), so two threads sharing a connection would read each other\x27s replies. The
...
impl RemoteHistStore {
    ...
    /// theoretical.** `crates/vike-report/src/excursions.rs`\x27s `backfill_excursions` calls
