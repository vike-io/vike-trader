...
/// serves exactly as it did before this module existed (see `vike_datahub::server`'s
/// `serve_authed`, which is where that contract is stated and tested). Writing the two keys into
/// `<project>/settings/secrets.env` is the whole of turning auth on.
