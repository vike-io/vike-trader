...
pub enum Scope {
    ...
    ///
    /// ⚠ **It is not a superset of [`Scope::Control`] and must never become one.** The two are
    /// different GRANTS rather than two rungs of one ladder: a Control peer can already place real
    /// orders on every armed venue, flatten the book and mount an arbitrary node-local Rhai script
    ...
    /// closed gate an absent control key already is there.
    Admin,
}
...
fn scope_tag(scope: Scope) -> u8 {
    match scope {
        ...
        // capability rather than a claim: a `Control` mac cannot satisfy an `Admin` challenge even
...
/// generator, the same call `vike_tradehub::server`'s own `fresh_nonce` makes.
...
pub struct NodeKeys {
    observe: Vec<u8>,
    control: Vec<u8>,
    /// [`Scope::Admin`]'s key. EMPTY unless a caller used [`NodeKeys::with_admin`] — every
...
impl NodeKeys {
    ...
    /// …and the [`Scope::Admin`] key beside them.
    ///
