...
use crate::proto::{
    ...
    FEATURE_DELETE_SERIES, FEATURE_MARKET_DATA, FEATURE_NAMED_RUN, FEATURE_SEARCH_METHOD,
    FEATURE_SEED_SERIES, FEATURE_STUDY, FEATURE_VENUE_CATALOG, FEATURE_WALKFORWARD_SEARCH,
    PROTO_VERSION, Request, Response, SeedDone, WireSearch, WireStudy, read_frame, write_frame,
};
...
impl DatahubClient {
    ...
    /// scalars (one parser, `vike_backtest::harness::search_select`, on whichever side runs).
    ///
    ...
    /// is the whole point of the record: `crates/vike-app-core/src/backend_registry.rs`'s
    ...
    /// interval set are all the server's. This method is a `VerbScope::Observe` verb, so unlike
    ...
    ///
    /// The request names a VENUE and nothing else. This is a `VerbScope::Observe` verb, so like
        ...
        if !self.features.iter().any(|f| f == FEATURE_VENUE_CATALOG) {
            return Err(format!(
                "datahub server does not advertise `{FEATURE_VENUE_CATALOG}` (advertised: {:?}) — \
                 ...
                 `venue_catalog_off = true` in that box's <project>/settings/flags.toml (or \
                 `VIKE_DATAHUB_VENUE_CATALOG_OFF=1`). Delete that and restart it. The refusal \
                 exists because an armed lane spends that box's venue-API budget on behalf of \
                 read-only clients; it is not what bounds the cost.",
                self.features
            ));
        }
        ...
        if !self.features.iter().any(|f| f == FEATURE_DELETE_SERIES) {
            return Err(format!(
                "datahub server does not advertise `{FEATURE_DELETE_SERIES}` (advertised: {:?}) — \
                 ...
                 or run the delete on the box with `vike-cli data rm --store DIR`.",
                self.features
            ));
        }
