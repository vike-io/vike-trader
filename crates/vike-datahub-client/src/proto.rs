...
pub enum Request {
    ...
    ///
    /// ⚠ **This is `VerbScope::Observe` and it is the only `Run*` verb that is**, which is the
    ...
    ///
    /// ⚠ **This is a WRITE verb classified [`VerbScope::Observe`], and that is a DECISION against a
    ...
    ///
    /// ⚠ **This verb is `VerbScope::Observe` and it is NOT A WRITE**, which is the distinction
...
pub fn plane_of(request: &Request) -> Plane {
    match request {
        ...
        // (`VerbScope::Observe` — `docs/decisions/0064-a-named-run-carries-no-source.md`) and their
...
/// - **⚠ ALMOST every `Run*` verb is [`VerbScope::Control`], and the reason is the SOURCE it can
...
///
///   ⚠ **And [`Request::RunNamed`] is [`VerbScope::Observe`]**, which is 0064's subject. Read the
...
pub fn required_scope(request: &Request) -> VerbScope {
    match request {
        ...
        | Request::NamedStrategies => VerbScope::Observe,
        ...
        | Request::RunStudy(_) => VerbScope::Control,
    }
}
...
///
/// ⚠ **[`Scope::Admin`] admits NOTHING here, and the `matches!` arms are why that is deliberate
...
mod tests {
    ...
    fn the_study_verb_is_a_control_scoped_compute_verb() {
        ...
        assert_eq!(required_scope(&r), VerbScope::Control);
    }
