...
// `vike_node_proto::auth` (the SHARED primitive both localhost services sign with) and re-exported here
// so `proto::Scope` resolves on this protocol exactly as it does on the tradehub node's.
pub use vike_node_proto::auth::Scope;

// The FRAME CODEC, re-exported as part of this protocol's own vocabulary rather than re-spelled.
//
// ⚠ It was DECLARED here until 2026-09-23 and now lives in `vike-node-proto`, below both node
// protocols. This is the identical shape `crates/vike-tradehub-client/src/proto.rs` has always
// used for the same five items, and for the same stated reason: a second codec is a second answer
// to what a frame is. What changed is only WHERE the one answer lives — it used to live in this
// crate, which made the sibling that needed it depend on this one and sit a rank above it for no
// reason but that.
//
// This is public-API surface rather than an alias shim: a protocol module's frame codec IS its
// vocabulary, which is the exception the no-`pub use`-on-a-move convention names outright, and the
// tree already made this exact call once for the tradehub half.
...
///   ([`vike_node_proto::auth::sign`]), so a bump breaks the handshake against every peer not upgraded
...
///   ([`vike_node_proto::auth::sign`]), so a bump breaks the handshake against every keyed peer not
...
///   ([`vike_node_proto::auth::sign`]) against every keyed peer not upgraded in lockstep — the cost
...
/// [`vike_node_proto::auth::sign`] folds the version into the auth mac, so it breaks the handshake
/// against every keyed peer not upgraded in lockstep.
///
...
/// The `Welcome.features` capability string for [`Request::SeriesFacts`] — one series' coverage
/// plus the commit keys that produced it.
///
/// ⚠ **The SEVENTH verb of `docs/decisions/0084-only-the-datahub-touches-the-store.md`'s gap, and
/// the one that record did not price.** It counted six because it measured the `HistStore` TRAIT,
/// and this one was INHERENT to `DataFusionHist` — invisible to that measurement and therefore
/// absent from the table. `crates/vike-backtest/src/backtest_cli.rs`'s `collect_data_fingerprint`
/// is the consumer, and without this verb a wire-routed run loses its reproducibility record while
/// still reading GREEN: the fingerprint is recorded as `null` and nothing fails.
///
/// [`FEATURE_COVERAGE`]'s shape — a plain trait verb, advertised unconditionally, and the
/// negotiation answers one question: is this server older than the verb?
...
/// at least one [`vike_node_proto::auth::NodeKeys`] scope key and therefore REQUIRES a
...
/// ([`vike_node_proto::auth::sign`]), so a bump would break the handshake against every peer that has
...
/// one thing that was asked. A bump also breaks the signed auth mac ([`vike_node_proto::auth::sign`]
...
/// by [`vike_node_proto::auth::sign`]). [`Request`] has no `deny_unknown_fields`, so a daemon predating
...
pub enum Request {
    ...
    ///
    /// - **Key-LESS server** (no [`vike_node_proto::auth::NodeKeys`] configured — the default, and every
    ...
    /// by [`Response::SeriesFacts`] (or [`Response::Error`]).
    ///
    /// ⚠ The SEVENTH verb of 0084's gap, and the one that record did not price: it counted six
    /// because it looked at the `HistStore` trait, and this one was INHERENT to `DataFusionHist`.
    /// A routed backtest without it loses its data fingerprint and still reads green.
    SeriesFacts {
