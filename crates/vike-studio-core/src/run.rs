...
/// (`vike_datahub::wire_run::run_walkforward_local`) takes `n_splits` from its caller and applies
...
pub fn run_walkforward_slice_with_params(
    ...
    //
    // `_train` is ignored deliberately: this door is the FIXED-parameter walk, an out-of-sample
    // stability check rather than an optimize-then-test protocol. The searching driver is a
    // different entry point and carries its own knobs.
    //
    ...
    let rep = walk_forward_strategy(
        ...
        WalkMode::Anchored,
        cash,
        periods_per_year_for_interval(&slice.interval),
        |_train, window| {
...
/// by construction (see the comment beside the DSR block in [`run_sweep_slice_with_params`]).
...
"#;
    ...
    fn ranking_order_is_invariant_to_the_annualization_factor() {
        ...
        let order_at = |ppy: f64| -> Vec<u64> {
            let mut entries: Vec<SweepEntry> = curves.iter().cloned().map(entry).collect();
            entries.sort_by(|a, b| {
                cmp_scores_desc(
