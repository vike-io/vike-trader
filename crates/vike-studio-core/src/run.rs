...
/// **Hand this process's sweep pool the cap its settings resolved** — `vike-backtest`'s
/// `install_sweep_threads`, re-exposed HERE because this crate's callers are the ones that reach
/// the pool and cannot name the crate that owns it.
///
/// `vike-desktop` links `vike-studio` (which re-exports this) and does NOT link `vike-backtest`, so
/// without this seam the GUI's Studio — one of the two settings-loading processes that run sweeps —
/// could not pass `preferences.sweep_threads` at all, and the file key would stay inert exactly
/// where an operator is most likely to set it. Same layering reason `backtest --addr` takes its
/// Studio run table as an injected parameter: only a composition root may name both sides.
///
/// It forwards and adds nothing. `n` is the RESOLVED
/// `vike_config::Preferences::sweep_threads`; `vike_backtest::harness::install_sweep_threads`'s
/// doc is the authority for the precedence and for why it is an `OnceLock` rather than a parameter.
pub fn install_sweep_threads(n: Option<usize>) -> bool {
    vike_backtest::harness::install_sweep_threads(n)
}
