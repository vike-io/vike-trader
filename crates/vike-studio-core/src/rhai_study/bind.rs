...
use vike_model::{Bar, BookLevel, BookUpdate, QuoteTick, TradeTick};
...
/// `crates/vike-marketdata/src/orderbook.rs`'s `BookUpdateKind` already has, so a study comparing
...
///
/// `vike_model::BookLevel` is a bare `(f64, f64)` tuple, which Rhai would render as an opaque value
...
fn levels(side: &[BookLevel]) -> rhai::Array {
    side.iter()
        .map(|BookLevel { price, qty: size }| {
