...
// What a Studio run is COSTED under: the fee schedule DERIVED from the slice the DTO door
// already carries, the explicit flat OVERRIDE that displaces it, and the three terms 0063 declared
// residual because they can flip a WINNER and no slice implies them.
pub mod cost_model;
/// The one implementation of `vike_user_research::StudySim` — the host declares that seam and
/// names no simulator; this crate already links one. See the module doc for why it lives here.
pub mod harness_sim;
pub mod listing;
pub mod ml;
pub mod rhai_study;
...
pub use cost_model::{CostModel, CostSource, FillMix, NOT_MODELLED};
...
pub use run::{
    ...
    SeriesLists, SliceKind, StoreHandle, StudioParamscan, UNREPLAYABLE_TICK_KIND, WindowSearchPlan,
    bar_series, build_strategy, build_strategy_in, build_strategy_with, build_strategy_with_in,
    compare_all_slice, depth_only_series, load_slice_bars, plugins_dir, run_paramscan_slice,
    run_paramscan_slice_with_params, run_slice, run_walkforward_slice,
    run_walkforward_slice_searching, run_walkforward_slice_with_params, series_lists,
    spawn_compare_all, spawn_outcome, spawn_paramscan, spawn_run, spawn_walkforward, tick_series,
};
pub use spec::{
...
pub use wire_run::{
    engine_params_for, run_error_to_wire, run_slice_local, run_sweep_local, run_walkforward_local,
    studio_run_table, study_run_fn, to_data_slice, to_engine_params, to_strategy_spec,
    to_wire_cost_model, to_wire_paramscan_result, to_wire_walkforward_result,
};
