...
// is `#[cfg(feature = "study-cli")]` because it opens a concrete `DataFusionHist`, and the WIRE
// runner needs the same window grammar and the same `[learner]` reader without needing DataFusion.
