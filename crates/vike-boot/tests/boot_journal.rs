...
fn the_anchor_never_claims_a_ceiling_that_is_not_effective() {
    ...
    for key in [
        ...
        "policy.venues",
        // ⚠ The dead-man switch — added when `vike-tradehub`'s live mount started constructing
        // `vike_core::CoreConfig::deadman` from it (`crates/vike-tradehub/src/tradehub_cli.rs`'s
        // `deadman_config_from_policy`) and `policy_is_consumed.rs`'s rows became `Consumed::At`.
        // The KEY is always present; on a file-less box its VALUE is `null` (the switch is OFF —
        // an absent key arms nothing, and the live mount warns once), asserted just below. For
        // one morning this comment said "its compiled-in default is the ARMED end" and the
        // assertion below pinned `"60000"`; `vike_config::Policy::deadman_timeout_ms` records why.
        "policy.deadman_timeout_ms",
        "policy.deadman_action",
        // ⚠ The ACCOUNT-aggregate exposure ceiling — effective wherever it is written
        // (`crates/vike-mount/src/lib.rs`'s `make_engine_for_account` folds it onto
        // `vike_exec::RiskLimits` and `policy_is_consumed.rs`'s row for it is a `Consumed::At`),
        // so the record claims it. On a file-less box its VALUE is `null`, i.e. UNCAPPED —
        // asserted just below, because "no aggregate ceiling was set" is exactly the state an
        // incident review needs to be able to read off a boot record.
        "policy.max_account_exposure",
        // ⚠ The ceiling on the equity FIGURE the sizing and admission lanes see — recorded for the
        // same reason as the exposure ceiling above, and needed by a review for a sharper one: with
        // it absent, every position size and every margin admission on this boot was computed from
        // the VENUE's reported wallet, which on a shared account is a number nobody here wrote.
        "policy.max_sizing_equity",
    ] {
        assert!(line.contains(key), "{key} is effective and must be recorded: {line}");
    }
    assert!(
        line.contains(r#"["policy.deadman_timeout_ms",null]"#),
        "an absent dead-man key is OFF and recorded as null — never as a number nobody wrote: {line}"
    );
    assert!(
        line.contains(r#"["policy.deadman_action",null]"#),
        "an action the unarmed switch will never take is not an effective ceiling: {line}"
    );
    assert!(
    ...
    assert!(
        line.contains(r#"["policy.max_account_exposure",null]"#),
        "an unwritten ACCOUNT ceiling is UNCAPPED and recorded as null — never as a number nobody \
         wrote, and never omitted: {line}"
    );
    assert!(
        line.contains(r#"["policy.max_sizing_equity",null]"#),
        "an unwritten SIZING-EQUITY ceiling is UNCAPPED — the engines sized against the venue's own \
         wallet figure — and that has to be readable off the record: {line}"
    );
}

/// **The dead-man pair is claimed exactly as far as it is effective.** A WRITTEN timeout is
/// recorded as its number with the action beside it; the explicit-off `0` is recorded as `0` (a
/// decision, distinct from the absent key's `null`) with the action `null`, because a switch that
/// is not constructed takes no action. Pure — `boot_ceilings` is the whole of what the record
/// carries, and the serialized `null`-vs-pair shape is pinned by the test above.
#[test]
fn the_deadman_pair_is_claimed_only_as_far_as_it_arms() {
    let claimed = |policy: &Policy, key: &str| -> Option<String> {
        boot_ceilings(policy)
            .into_iter()
            .find(|(k, _)| k == &format!("policy.{key}"))
            .unwrap_or_else(|| panic!("policy.{key} must always be a pair"))
            .1
    };

    let written = Policy { deadman_timeout_ms: Some(60_000), ..Policy::default() };
    assert_eq!(claimed(&written, "deadman_timeout_ms").as_deref(), Some("60000"));
    assert_eq!(claimed(&written, "deadman_action").as_deref(), Some("cancel_all_and_halt"));

    let decided = Policy { deadman_timeout_ms: Some(0), ..Policy::default() };
    assert_eq!(
        claimed(&decided, "deadman_timeout_ms").as_deref(),
        Some("0"),
        "an explicit 0 is the operator's decision and is recorded as the number they wrote"
    );
    assert_eq!(
        claimed(&decided, "deadman_action"),
        None,
        "…and an unconstructed switch acts on nothing"
    );

    let absent = Policy::default();
    assert_eq!(claimed(&absent, "deadman_timeout_ms"), None, "absent is null, not 0 and not 60000");
    assert_eq!(claimed(&absent, "deadman_action"), None);
}
