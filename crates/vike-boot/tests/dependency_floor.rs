...
///
/// Five crates, and each is in it for one reason: `vike-config` (the settings model and the three
...
/// vike-config), `vike-marketdata` (which this crate never names: it arrives BENEATH vike-model,
/// whose market-data vocabulary it holds since
/// `docs/decisions/0067-the-market-data-vocabulary-leaves-the-model.md`, and it declares exactly
/// one external crate — `serde`, already on [`EXTERNAL_FLOOR`] — so the closure got a member
/// without getting heavier) and `vike-buildinfo` (the identity line — ZERO dependencies, and still
/// literally so: it is the only member of this closure that declares nothing at all).
