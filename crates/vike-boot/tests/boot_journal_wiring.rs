...
//! FIFTH root joins this gate the moment it boots and has to be classified before it can merge.
...
//! drains, and the second of the two rows below argues that writing an anchor would make the
//! ledger say something FALSE rather than merely noisy.
//!
...
const NO_BOOT_ANCHOR: &[(&str, &str)] = &[
    ...
    (
        ...
        "it loads no settings at all (`SettingsLoad::Skip`, and its store root and address come \
         ...
         the reading a bracket must never give. It mounts no venue either — and that stayed true \
         when ruling 10 folded the RECORDER into this binary, which is why the retired \
         `crates/vike-recorder` row's argument is carried here rather than deleted with it: a \
         `--record` start subscribes to public venue feeds and writes a tape, but it mounts no \
         `ExecutionClient`, holds no book and can place nothing, so not one of the ceilings is \
         effective in this process and a record claiming otherwise would be a fabrication in an \
         append-only file. A box running it beside `vike-tradehub` is already bracketed by that \
         daemon's anchors, off the same `policy.toml`.",
    ),
];
...
fn the_gate_actually_sees_the_tree() {
    ...
    // The roster anchor still matches: four composition roots run the sequence.
    //
    // ⚠ It was FIVE until ruling 10 retired `vike-recorder`'s `[[bin]]`
    // (`docs/superpowers/specs/2026-09-09-datahub-market-data-wire-design.md` §0.5 merged that
    // daemon into `vike-datahub`), and this floor is deliberately a `>=` so a FIFTH root joining
    // does not have to touch it — only a root LEAVING does, which is the edit that has to be
