...
//!
//! `crates/vike-app/src/main.rs` was already living that: its rolling log file hung off a SECOND
//! `project_log_dir(&cwd)` walk, which does not honour `$VIKE_SETTINGS_DIR`, so under the override
//! the log file and the block describing it named two different projects.
//!
...
fn crate_dir(rel: &str) -> String {
    // `crates/vike-app/src/…` and `crates/bridges/aster/src/…` — the bridges tree is one level
    // deeper, and either way the CRATE is everything above `/src/`.
