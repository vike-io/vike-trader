...
//!
//! # Two declared limitations, and one file measured as falling into the first
//!
...
//!
//!   ⚠ **ONE FILE IS MEASURED AS FALLING INTO THIS, and naming it is better than leaving it as a
//!   property of the cut.** `crates/vike-backtest/src/backtest_cli.rs`'s `run_serve` is a SIXTH,
//!   undeclared composition root: it performs its own `project_settings_dir_from` walk, resolves
//!   settings, and opens a socket. Its `#[cfg(test)] mod` sits roughly 1500 lines ABOVE that call,
//!   so the cut removes the call and the gate reports nothing — in EITHER direction, which is the
//!   half worth knowing: an exemption row added for it is reported as STALE, because the gate
//!   cannot see what it would be excusing. So the row cannot be written and this note is the only
//!   place the hole can live.
//!
//!   It was found by the 0057 crossing, which had to thread the settings SOURCE into every
//!   resolution: that call was store-blind, and on a box that has run `vike-cli config adopt` a
//!   store-blind load reads `config.*` out of files nothing resolves from. It has been threaded —
//!   so the hole costs nothing TODAY, and the next root to fall into it will cost whatever it
//!   costs. Closing it means teaching [`production_half`] to cut at a TRAILING test module rather
//!   than the first one, which is a change to a shared gate's semantics and is owed its own PR.
...
const SEQUENCE: &[(&str, Owned, &str)] = &[
    ...
    (
        ...
        Owned::Called,
        "step 4 — resolve the settings from WHICHEVER SOURCE this box answers from (decision 0057): \
         the four files, or the settings database when it carries an adoption seal. The source is \
         decided on ONE probe inside that call, so it enters the sequence here and at no root: a \
         root that opened the store privately would give one surface a set of layers the rest of \
         the process does not have, which is the whole failure this file exists for. \
         ⚠ This row was keyed on `load_with_store(` until the crossing landed, and the RE-KEY is \
         the point rather than a rename: the needle is a repo-wide pin, so the function moving \
         without it would have left this gate green over a sequence nothing performs",
    ),
    (
        "vike_config::load(",
        Owned::Reserved,
        "step 4's TWO-ARGUMENT public loader. It was the `Called` row until the settings database \
         became a layer; it is now `load_with_source` with `StoreLayer::NotConsulted`, so a root \
         reaching for it gets a `Settings` that silently omits the layer every other surface \
         applied — and, once a box is ADOPTED, one that omits the ONLY layer there is. Libraries \
         that are NOT composition roots still use it legitimately — see NOT_THE_ONE_OWNER",
    ),
    (
...
/// across both copies of the resolver (`vike_model::state_path` and the no-`vike-*`-dependency
...
const NOT_A_SECOND_ANSWER: &[(&str, &str)] = &[
    ...
    (
        "secrets::load_workspace_dotenv_scoped",
        "the CREDENTIAL STORE's LOADER narrowed to a DECLARED name set — the owner's 2026-09-16 \
         ruling that a process materialises only the keys it asked for. It is the \
         `load_workspace_dotenv` row above with a `KeyScope` parameter: the SAME \
         `workspace_dotenv_path`/`backend_in` resolution, so it is not a second answer to *where \
         is this project* — scoping narrows WHAT is materialised and changes nothing about WHERE \
         it is read from. ⚠ That it is not a second answer HERE does not excuse it from \
         `crates/vike-ops/tests/settings_registry.rs`'s `CREDENTIAL_STORE_READERS`, which asks the \
         different question of whether a LIBRARY performs the read at all; it is keyed there, and \
         `every_scoped_store_reader_that_walks_is_keyed` is why it cannot escape",
    ),
    // ── the settings DATABASE, `docs/decisions/0054`'s credential half ──────────────────────────
    //
    // ⚠ The two rows below were OWED and missing: `db_path_for` and `workspace_db_path_from`
    // landed with the database and were classified by neither table, so this gate was already red
    // on that branch. That is the gate working — a new `pub fn` in a resolver module reddens the PR
    // that adds it — and it is recorded here rather than quietly filled in, because the same
    // omission is what the classification exists to catch.
    (
        "secrets::workspace_db_path_from",
        "the CREDENTIAL STORE's DATABASE home — `<project>/settings/db/vike.db`, honouring \
         `$VIKE_SETTINGS_DIR`. The `workspace_dotenv_path_from` row above with one name changed, \
         and it MUST be the same resolution: `docs/decisions/0054`'s whole premise is that the \
         database drains the two files beside it, so a database that resolved its directory \
         differently would migrate one project's credentials into another project's store — the \
         #1089/#1101 family wearing a new artifact. It is a second FILE, not a second ANSWER",
    ),
    (
        "secrets::db_path_for",
        "the PURE half of the row above: the working directory is a PARAMETER rather than read off \
         the process. Same resolver, same accepted case — exactly the pairing \
         `node_path_for` has with `workspace_node_path_from`",
    ),
    (
        "secrets::settings_dir_or_last_resort",
        "resolves NOTHING NEW — it IS the shared half the three rows above were already performing \
         one at a time, extracted so the credential file, the node file and the database cannot \
         resolve their DIRECTORY differently. `settings_file_path_for(o, cwd, F)` is now \
         `settings_dir_or_last_resort(o, cwd).join(F)` by construction, so every path this module \
         answers with is byte-identical to what it answered before the extraction. The working \
         directory is a PARAMETER, like `node_path_for`'s",
    ),
    (
        "secrets::workspace_settings_dir_from",
        "the row above with the working directory read here — the front door a BINARY calls, and \
         the same shape `workspace_dotenv_path_from` has. ⚠ It returns the DIRECTORY rather than a \
         file in it, which is the only reason it needs saying twice: `crates/vike-cli/src/cmd/\
         secrets.rs`'s `settings_dir_of` and `crates/vike-cli/src/cmd/node/mod.rs`'s `settings_dir` \
         are the booting-crate call sites, and both reach it ONLY when the dispatcher's boot \
         resolved no settings directory at all — in which case this resolver, the same \
         `project_settings_dir_for` the boot itself called under the same `None`, cannot answer \
         differently. That is the identical argument the `workspace_dotenv_path` row makes for \
         `store_path`, and these two functions are what `store_path` is now written in terms of",
    ),
    (
        "secrets::secrets_path_in",
        "resolves NOTHING — it takes the already-resolved `<project>/settings` and joins one \
         filename. `state_path::user_data_dir_beside` is the worked example this shape copies, and \
         it exists for the same reason: so a caller that HAS the directory never walks for it again",
    ),
    ("secrets::node_path_in", "resolves NOTHING — the row above with `node.env` for `secrets.env`"),
    (
        "secrets::db_path_in",
        "resolves NOTHING — the same join, and the ONE spelling of the database's shape below the \
    ...
    (
        "secrets::settings_dir_of_store",
        "resolves NOTHING — it takes a store FILE path and returns its PARENT, for the two callers \
         that were handed a `secrets.env` path by their own composition root and never saw the \
         directory it came out of (the cTrader token persister, the GUI's `CredentialHome`). Both \
         built that path as `<settings dir>/<file>`, so taking the parent back is arithmetic on an \
         already-resolved answer and not a second walk",
    ),
];
...
const NOT_THE_ONE_OWNER: &[(&str, &str)] = &[
    ...
    (
     ...
     ride this path. \
     ⚠ **This row's argument was NARROWER than it read, and the crossing is what exposed it.** It \
     justified the exemption on the DIRECTORY being a parameter — which is still true and was \
     never the whole question. The call was ALSO store-blind, and on a box that has run `vike-cli \
     config adopt` a store-blind load reads `preferences.log_level` out of a `preferences.toml` \
     nothing resolves from: a hot apply would have set the level to the compiled-in default rather \
     than to the value the operator had just written. It resolves through the settings SOURCE now. \
     The lesson generalises: an exemption argued on the WALK says nothing about the LAYER SET.",
    ),
    (
     ...
     ⚠ **And the same narrowing as the row above, with a worse consequence.** This call was \
     store-blind until the crossing landed, so on an adopted box it would have answered \
     `Policy::default()` — every roster venue `paper` with `is_declared()` false — and painted a \
     FULL UNMOUNT on the arming screen, every frame that tab was visible, on a box whose arming \
     was correct. A display path, which is why it would have been survivable; the display path for \
     ARMING, which is why it would have been believed. It resolves through the settings SOURCE now.",
    ),
];
...
fn the_gate_actually_sees_the_calls() {
    ...
    let booting: Vec<String> = all
        ...
        .map(|(rel, _)| crate_dir(rel))
        .collect();
    // ⚠ FOUR, and it was five until ruling 10
    // (`docs/superpowers/specs/2026-09-09-datahub-market-data-wire-design.md` §0.5) retired
    // `vike-recorder`'s `[[bin]]` and folded that daemon into `vike-datahub`. The floor is a `>=`
    // so a FIFTH root joining needs no edit here; only a root LEAVING does, and that edit has to be
    // deliberate precisely because "a root retired" and "the anchor stopped matching real code"
    // look identical from inside this assertion.
    // ⚠ It counts FILES, not crates (no dedup), and `production_half` cuts a trailing test module —
    // so `crates/vike-cli/src/cmd/secrets.rs`'s boot call, which lives inside one, is not among
    // them. Four files, four crates, by coincidence rather than by construction.
    assert!(
        booting.len() >= 4,
