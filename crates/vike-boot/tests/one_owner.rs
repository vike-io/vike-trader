...
const SEQUENCE: &[(&str, Owned, &str)] = &[
    ...
    (
        "vike_config::load_with_store(",
        Owned::Called,
        "step 4 — load <project>/settings/*.toml OVER the settings database's mirrored rows \
         (decision 0057 Phase 1). The store is a LAYER, so it enters the sequence here and at no \
         root: a root that opened it privately would give one surface a set of layers the rest of \
    ...
    (
        ...
        "step 4's TWO-ARGUMENT public loader. It was the `Called` row until the settings database \
         became a layer; it is now `load_with_store` with no store, so a root reaching for it gets \
         a `Settings` that silently omits a layer every other surface applied. Libraries that are \
         NOT composition roots still use it legitimately — see NOT_THE_ONE_OWNER",
    ),
    (
