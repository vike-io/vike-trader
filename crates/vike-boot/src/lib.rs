...
pub fn boot(spec: &BootSpec<'_>) -> Result<Booted, String> {
    ...
    //
    //    ⚠ The read sits INSIDE the `Load` arm, not above the match: a root that declares
    //    `SettingsLoad::Skip` has said it resolves no settings at all, and probing a store whose
    //    rows nothing would apply is exactly the kind of work-a-skip-does-anyway that makes a
    //    declared arm stop meaning what it says.
    //
    ...
    let settings = match spec.settings {
        SettingsLoad::Load | SettingsLoad::LoadAndRefuseUnsoundSeal => {
            let read = settings_dir.as_deref().map(vike_secrets::read_settings_in);
