...
//! the code doing the disclosing. `crates/vike-app/src/main.rs` had exactly that bug: its rolling
//! log file hung off a second `project_log_dir(&cwd)` walk, which is `VIKE_SETTINGS_DIR`-blind, so
//! under the override the log file and the block describing it named two different projects.
//!
...
//! also what lets a binary whose STDOUT IS A PROTOCOL (`vike-tradehub`, `vike-recorder`,
...
//! defers the credential read to the two order-write surfaces; `vike-recorder` discloses settings it
//! does not itself consume; `vike-datahub` loads none at all.
//!
...
pub struct Booted {
    ...
    /// on disk (set-but-unhonoured) while merely warning on a walked one, and `vike-recorder`
...
pub fn boot_ceilings(policy: &Policy) -> Vec<(String, Option<String>)> {
    let Policy { max_leverage: _, venues, max_notional_per_order, market_slippage, halt_admit } =
        policy;
    vec![
