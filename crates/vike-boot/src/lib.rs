...
//! 4. **[`vike_config::load_with_source`]** — a missing file is the permissive default; a file that
//!    EXISTS and is broken is an error, because a deployment that wrote a ceiling and typo'd the key
//!    must not silently run without one. ⚠ **The `_with_source` half is WHICH ARTIFACT this box
//!    resolves its settings FROM**, decided on one probe inside that call (decision 0057's
//!    crossing): a settings database carrying an adoption seal answers for every settings key and
//!    the four files are not opened for resolution at all; a store without one leaves the files
//!    answering exactly as they always have, which is every box until an operator runs `vike-cli
//!    config adopt`. This crate hands in the ARM it read (`vike_config::StoreLayer`) rather than an
//!    `Option`, because the probe needs a distinction an `Option` throws away — and it does so with
//!    ONE mapping, replacing five hand-rolled `Err` → *"there is no database"* rewrites across four
//!    crates. A store that will not open is a MARK on [`vike_config::Settings::store_refusal`] and a
//!    warning, never a refusal here: `vike-cli config check` answers `Level::Fail` for that state,
...
//!    binary the moment a subscriber exists. ⚠ It is rendered from step 2's DIRECTORY and step 4's
//!    STORE ARM — both of the answers the settings were actually resolved from, never a second
//!    resolution of either. The arm half was missing until 2026-09-22 and the block described a
//!    source the process was not running on; [`Disclosure::Render`] carries what that printed.
//!
...
pub enum SettingsLoad {
    ...
    /// narrowness is the whole of its justification.** 0013's rule is that a venue connection is a
    /// CAPABILITY, so a fault degrades and announces rather than refusing — a daemon that will not
    /// start helps nobody. That reasoning holds for every settings fault except this one, because
    /// what degrades here is not a capability, it is the CEILING: an adopted box whose rows are
    /// unsound resolves `policy.max_notional_per_order` from the compiled-in default, which is
    /// `None`, which is NO SIZE CAP on the edge every remote control command funnels through. The
    /// degrade is toward MORE exposure, not less, and it is silent.
    ///
    /// So: a root that can PLACE OR GATE AN ORDER takes this arm; everything else takes
    /// [`SettingsLoad::Load`] and reports the mark. The repair verbs must keep running — that is
    /// the 2026-09-18 JSON incident's lesson and the reason the refusal is a mark in the first
    /// place — so `vike-cli` deliberately does NOT take this arm even though it gates orders: it
    /// refuses per-VERB instead (`vike_cli::SEAL_REFUSING_VERBS`), keeping `config mirror` and
    /// `config adopt --undo` alive.
    ///
    /// ⚠ What this COSTS, stated rather than discovered: the unit restart-loops. That is louder
    /// than a silent uncapped daemon and is the intended trade, but it means the box needs an
    /// operator, and `vike-cli config check` is what tells them which row is wrong.
...
pub enum Disclosure {
    ...
    /// rather than an optimisation: [`boot`] hands the arm it already read into `boot_lines`, so
    /// the block cannot describe a source this process did not resolve from. It could until
    /// 2026-09-22 — `crates/vike-config/src/boot.rs`'s module doc carries what the second,
    /// store-blind read printed on the live the CI box daemon.
    Render,
...
pub fn boot(spec: &BootSpec<'_>) -> Result<Booted, String> {
    ...
    //    copy in the crate that declares no `vike-*` dependency. (⚠ That read "the ZERO-dependency
    //    crate" until 2026-09-13, when decision 0054 gave `vike-secrets` a `rusqlite` edge; the
    //    `vike-*` half is the half this choice rests on.)
    ...
    //
    //    ⚠ **The settings DATABASE is a LAYER of that loader, and it enters here rather than at any
    //    root** — `docs/decisions/0057-the-seven-settings-files-answered-one-at-a-time.md`'s Phase
    //    1. It is MIRRORED: the rows are written FROM the four files and the loader applies them
    //    BELOW the files, so on every box today this changes nothing it resolves (proven by
    //    `crates/vike-config/tests/mirror.rs`, not asserted). What it buys before the files retire
    //    is that the GUI's `config show`, the CLI's, and both daemons read the SAME set of layers:
    //    a store this sequence skipped would let one surface report a value another does not have.
    //
    //    The READ needs no unit change and that is measured, not assumed: `settings/` is mounted
    //    read-only to the deployed daemons apart from one grant that does NOT name `settings/db`,
    //    and `vike_secrets::read_settings_in` opens read-only by flag and never creates.
    //
    //    ⚠ A store that will not open is a WARNING, never a refusal. Every ceiling and every flag
    //    still resolves from the files exactly as it did before this layer existed, so refusing
    //    would strand an operator with a daemon that will not boot over a layer that is currently
    //    a copy of something it already read (`docs/decisions/0013-degrade-vs-refuse.md`). The
    //    warning rides `Settings::warnings`, which every root already surfaces once a subscriber
    //    exists.
    //
    //    ⚠ **A store that OPENS and holds an INVALID row is the opposite disposition, and the split
    //    is deliberate rather than an oversight.** `load_with_source` materialises each section's
    //    rows back through the same patch type a file goes through, so a typo'd row key, a
    //    tombstoned one or a row that breaks a bound is a hard startup refusal NAMING THE KEY —
    //    exactly what the same defect in `policy.toml` already is. The two arms answer different
    //    questions: *could this layer be read at all* degrades, because the files still answer;
    //    *does this layer say something illegal* refuses, because a store that says something
    //    illegal is a store somebody edited by hand and believes in. 0057 makes that the point of
    //    the round trip — a row read as nothing is the failure it exists to prevent.
    //
    //    ⚠ The read is GATED ON THE SAME `match spec.settings`, so `SettingsLoad::Skip` reaches
    //    `read_settings_in` on no path: a root that declares it resolves no settings at all must
    //    not probe a store whose rows nothing would apply, which is exactly the
    //    work-a-skip-does-anyway that makes a declared arm stop meaning what it says.
    //    ⚠ This paragraph read "the read sits INSIDE the `Load` arm, not above the match" until
    //    2026-09-22, when the same commit that hoisted it to the declaration (the last ⚠ below)
    //    left both statements standing and a reader hit the false one first. The PROPERTY it
    //    defends is unchanged — only where the binding lives.
    //
    //    ⚠ **The `Err` arm is NOT rewritten into "there is no database" here any more, and that
    //    deletion is the point of the signature change.** Five call sites across four crates each
    //    independently collapsed a `DbError` into `SettingsSource::NoDatabase` and then called
    //    `rows()`, so nothing about WHICH of the five store states answered survived into the
    //    resolved `Settings`. `vike_config::StoreLayer::of` is the ONE mapping now, and
    //    `SettingsLoad::Load`'s own arm is where this root says what an unreadable store costs it.
    //
    //    ⚠ **The read is hoisted to the DECLARATION and not to the WORK, and that split is what
    //    lets step 6 describe the resolution step 4 performed.** The arm used to be built and
    //    dropped inside the `Load` arm below, so the disclosure had nothing to be handed and called
    //    the store-BLIND `vike_config::describe` — a second resolution of the same box, which on an
    //    adopted one credits a file for every value the rows set and, once the inert files are
    //    deleted, announces compiled-in defaults a live daemon is not running on.
    //    `crates/vike-config/src/boot.rs`'s module doc carries the the CI box journal evidence.
    //
    //    The `Skip` arm still performs NO read — `read_settings_in` is not called on that path at
    //    all — so the property the old placement was defending is intact: a root that has declared
    //    it resolves no settings still probes no store. `StoreLayer::of(None, ..)` is
    //    `StoreLayer::NoDatabase`, which `load_with_source` and `describe_with_source` both treat
    //    exactly as they treat the `NotConsulted` arm this replaced.
    ...
    let settings = match spec.settings {
        SettingsLoad::Load | SettingsLoad::LoadAndRefuseUnsoundSeal => {
            ...
            // shared by every root including the repair verbs, and a refusal there is what bricked
            // `vike-cli config mirror` on 2026-09-18. The LOADER marks; the ROOT decides. See
            // `SettingsLoad::LoadAndRefuseUnsoundSeal` for why exactly one disposition refuses.
            seal_gate(
    ...
    let boot_lines = match spec.disclosure {
        ...
        // not load from, nor a SOURCE it did not resolve from. The second half of that sentence is
        // new on 2026-09-22 and the first half is why: they are one rule, and the arm was the half
        // that was missing.
        Disclosure::Render => vike_config::boot_lines(settings_dir.as_deref(), source, spec.env),
...
///
/// `refuses` is whether this root took [`SettingsLoad::LoadAndRefuseUnsoundSeal`]; `seal` is
/// [`vike_config::Settings::seal_refusal`]. Extracted from [`boot`] because the alternative is a
/// test that plants an adopted SQLite store to exercise two branches of an `if`, and because a
/// seam nothing can call is a seam nothing can prove: `crates/vike-cli/tests/seal_enforcement.rs`
/// had a library-level predecessor that returned `Ok` while every real binary exited 1.
fn seal_gate(refuses: bool, seal: Option<&str>) -> Result<(), String> {
    ...
    Err(format!(
        "REFUSING TO START: this box's settings seal is unsound, and this process places or gates \
         ...
         with, and on an adopted box there is no settings FILE underneath the rows to answer \
         instead — so an unreadable row resolves to the compiled-in default, which for \
         `policy.max_notional_per_order` is NO SIZE CAP. Starting anyway would trade uncapped \
         while an operator believed a ceiling was armed.\n\nDiagnose with `vike-cli config check`, \
         which names the row. Repair with `vike-cli config mirror`, or step back to the files with \
         `vike-cli config adopt --undo` — both still run in this state, by design."
    ))
}
...
mod seal_gate_tests {
    ...
    /// The refusing root refuses, and the message carries the operator's way out. ⚠ It must name
    /// BOTH repairs: a refusal whose named repair does not run is the 2026-09-18 JSON incident.
    #[test]
    fn an_unsound_seal_refuses_the_ordering_root_and_names_both_repairs() {
        let err =
            seal_gate(true, Some("the settings rows have CHANGED since this box was adopted"))
        ...
        assert!(err.contains("config mirror"), "{err}");
        assert!(err.contains("config adopt --undo"), "{err}");
    ...
    /// the same unsound seal and still boots, so `vike-cli`'s repair verbs stay reachable.
    ///
    ...
    fn the_same_unsound_seal_does_not_stop_a_reporting_root() {
        assert!(
            seal_gate(false, Some("the settings rows have CHANGED since this box was adopted"))
                .is_ok(),
            "a root that reports the mark must still boot — `config mirror` lives there"
        );
    }
}
