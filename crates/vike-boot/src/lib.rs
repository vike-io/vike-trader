...
//! 4. **[`vike_config::load_with_store`]** — a missing file is the permissive default; a file that
//!    EXISTS and is broken is an error, because a deployment that wrote a ceiling and typo'd the key
//!    must not silently run without one. ⚠ **The `_with_store` half is the SETTINGS DATABASE, and it
//!    is a LAYER of this step rather than a second source** (decision 0057's Phase 1). It is
//!    MIRRORED — the rows are written from the four files and lose to them — so it changes nothing
//!    any box resolves today; what it buys is that every surface reads the same set of layers. A
//!    store that will not open is a WARNING on [`vike_config::Settings::warnings`], never a refusal:
//!    the files still answer, so refusing would strand a daemon over a layer that is a copy of
...
pub fn boot(spec: &BootSpec<'_>) -> Result<Booted, String> {
    ...
    //    is deliberate rather than an oversight.** `load_with_store` materialises each section's
    ...
    let settings = match spec.settings {
        SettingsLoad::Load => {
            let (store_rows, store_warning) = match settings_dir
                .as_deref()
                .map(vike_secrets::read_settings_in)
            {
                Some(Ok(found)) => (found, None),
                Some(Err(e)) => (
                    vike_secrets::SettingsSource::NoDatabase { path: e.path.clone() },
                    Some(format!(
                        "the settings database could not be read ({e}). Every setting on this \
                         process comes from the files, the environment and the compiled-in \
                         defaults, exactly as it would on a box that has never been mirrored."
                    )),
                ),
                None => (vike_secrets::SettingsSource::NoDatabase { path: PathBuf::new() }, None),
            };
            let mut loaded = vike_config::load_with_store(
            ...
            loaded.warnings.extend(store_warning);
            loaded
        }
