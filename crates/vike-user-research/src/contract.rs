...
//!
//! `crates/vike-model/src/runs.rs` already owns run persistence for EVERY kind of run —
...
// ---------------------------------------------------------------------------------------------
// the simulator seam, erased
// ---------------------------------------------------------------------------------------------

/// What one backtest hands back: the three numbers a sweep aggregates, and nothing else.
///
/// ⚠ **Deliberately NOT the simulator's own result type**, and that is the whole seam. Naming it
/// would put `vike-backtest` (layer 50) in this crate's `[dependencies]`, which
/// `crates/vike-ops/tests/layer_gate.rs` refuses at layer 35 — and which this crate's own doc
/// refuses on a second, independent ground: a vectorised signal Sharpe and an event-driven one
/// must not silently become one number. A study gets a RESULT it can aggregate; it never gets
/// the simulator.
...
///
/// The profile arrives as `toml::Value` rather than a typed profile for the same reason
/// [`SimOutcome`] is local: the typed one belongs to a crate this one may not name. The host
/// supplying the implementation owns the deserialization, so a profile this crate cannot describe
/// is that host's `Err` rather than a compile error here.
///
/// ⚠ The store is passed IN, never built. This crate takes `vike-data` at DEFAULT features, so a
/// study can NAME a `HistStore` and has no way to construct one; the seam keeps that property by
/// handing the caller's store straight through rather than opening one of its own.
...
impl StudyContext {
    ...
    ///
    /// OPTIONAL for the same reason the learner is, and the absence is the ORDINARY case:
    /// only a host that already links the simulator can build one, which today is the study
    /// runner in `crates/vike-studio-core`. Every other caller supplies `None`, and a study
    /// that needs a run says so with [`StudyError::NoSim`] and stops rather than degrading
    /// into a number from a different machine.
    #[must_use]
...
impl StudyOutcome {
    ...
    /// `crates/vike-model/src/runs.rs` owns `MANIFEST_FILE` and `REPORT_FILE` and sits at a
...
pub enum StudyError {
    ...
    /// The HOST's refusal, the simulator twin of [`StudyError::NoLearner`]: this study
    /// needs an event-driven run and the caller supplied no [`StudySim`]. Carries what the
    /// study was about to run, so the message names the work rather than the absence.
    NoSim(String),
}
