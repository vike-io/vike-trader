...
//! `crates/vike-secrets/Cargo.toml` declares NO `vike-*` dependency — the property that lets
//! `vike-bridge-core` (which owns the transport stack) and `vike-cli` (transport-free,
//! DataFusion-free) both link it — so that crate cannot name `vike-model` and cannot journal
//! anything. (⚠ This said *"declares a literally EMPTY `[dependencies]`"* until 2026-09-13, when
//! decision 0054 put `rusqlite` there. The forcing is unchanged and is if anything sharper: the
//! `vike-*` half is the half that forbids naming `vike-model`, and an EXTERNAL dependency arriving
//! does not relax it.) The CALLER writes the record. This crate is layer 75 and links `egui`, so the wrapper
...
pub fn save_credentials_journalled(
    ...
    // ⚠ **The store that ANSWERS, not the file.** `docs/decisions/0054`'s credential half lets
    // `<project>/settings/db/vike.db` shadow `secrets.env`, and a write that named the file there
    // succeeded, changed the file, appended a `credential_write` record and returned `Ok` — while
    // every reader kept answering from the database. The GUI's Connections editor would have shown
    // "Saved" over a key the venue never sees.
    //
    // The settings directory is `creds.store`'s parent — `CredentialHome::resolve` is what joined
    // the file name onto it — and `save_credentials_to_store` asks `vike_secrets::backend_in` about
    // it, the same question `vike_secrets::resolve_project` asks. The FILE branch is
    // `save_credentials` verbatim, so every byte-preservation property this wrapper's two call sites
    // depend on is the same property on an unmigrated box.
    let dir = vike_secrets::settings_dir_of_store(creds.store);
    vike_secrets::save_credentials_to_store(
        &dir,
        vike_secrets::Table::Credential,
        updates,
        // Schema 2 files a NEW credential name under the account its name names — see
        // `vike_bridge_core::credentials::classify_credential_name`, the one implementation. A key
        // the store already holds is replaced in place and never reaches it.
...
/// **One account-ROW lifecycle edit, plus its durable record** — `vike_secrets::edit_account_in`
/// and an `account_lifecycle` append, together.
///
/// The account-plane twin of [`save_credentials_journalled`], and it exists for that function's
/// reason rather than for symmetry: a store write and its ledger line must not be able to drift
/// apart at a CALL SITE. The GUI's account strip reaches this; nothing reaches the router directly.
///
/// # What it never carries
///
/// **No credential value, on any path.** `vike_secrets::edit_account` never selects a `value`
/// column — its one `credential` statement is `SELECT name` — and
/// `vike_model::change_journal::Change::account_lifecycle` takes no value parameter, which is the
/// enforcement rather than a convention. The key NAMES that DO reach the record are recorded for
/// [`save_credentials_journalled`]'s reason: they are not secret
...
/// # Ordering, and the two failure paths
///
/// The store write happens FIRST and its error is returned unchanged — an edit that did not land
/// must not leave a record saying it did. A JOURNAL failure, by contrast, cannot fail the call: the
/// row IS written, and reporting failure would send a caller down an error path for a write that
/// succeeded. It is logged at `error`, that being the one level a shipped
/// `VIKE_LOG_FILE_LEVEL=warn` still lets through.
///
/// # Errors
/// `vike_secrets::DbError` for every refusal `vike_secrets::edit_account` states — a row that still
/// owns credentials, a label another row carries, an unmigrated box. Each of them wrote nothing,
/// and each is safe to render: they name ids, venues, tiers and credential key NAMES.
pub fn edit_account_journalled(
    settings_dir: &Path,
    edit: vike_secrets::AccountEdit<'_>,
    actor: Actor,
    now_ms: i64,
) -> Result<vike_secrets::AccountWrite, vike_secrets::DbError> {
    let done = vike_secrets::edit_account_in(settings_dir, edit)?;

    // Nothing to record when nothing changed: a ledger line for a no-op reads as an edit that did
    // not happen, which is `vike-cli`'s `record_book_write` rule and for its reason.
    if !done.changed {
        return Ok(done);
    }
    // No state directory beside the settings one ⇒ still record: unlike the credential wrapper,
    // which takes an already-resolved `ChangeJournal`, this one derives it from the SAME settings
    // directory the write landed in — so the ledger and the store it describes can never resolve to
    // two different projects, and there is no "the caller found no project" case to honour.
    let journal = ChangeJournal::in_state_dir(
        &settings_dir.join(vike_secrets::STATE_DIR),
        Proc::current(env!("CARGO_PKG_VERSION")),
    );
    let Some(row) = done.after.as_ref().or(done.before.as_ref()) else { return Ok(done) };
    ...
    if let Err(e) = journal.append(now_ms, &change) {
        tracing::error!(
            ...
            id = row.id,
            verb = done.verb,
            "account row edit NOT recorded to the change journal (the row IS written)"
        );
    }
    Ok(done)
}
