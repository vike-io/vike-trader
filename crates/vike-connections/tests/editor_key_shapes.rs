...
use vike_connections::{VENUES, VenueCredStatus, credential_status_for_account};
...
mod baseline {
    ...
    pub fn edit_fields(venue: &str, env_label: &str) -> Vec<(&'static str, String)> {
        match venue {
            ...
            // ⚠ MOVED DELIBERATELY. This arm is the one exception to "[`baseline`] states what the
            // editor composed BEFORE, permanently": the `_SERVER` pair was added to the real table
            // by the PR that closed the reachability gap, and this file's own rule for that case is
            // the sentence above — *a PR that deliberately changes an untouched venue's arm must
            // edit the matching arm here in the same commit*. Doing so makes the equality below
            // vacuous for dukascopy ALONE and leaves it load-bearing for every other venue; what
            // replaces it for this one is `dukascopy_demo_reaches_both_accounts_whole`, an explicit
            // pin of the new list against the loader that reads it.
...
fn baseline_keys(venue: &str, tier: &str, account: &AccountLabel) -> Vec<String> {
    let mut out: Vec<String> = baseline::edit_fields(venue, tier)
        .into_iter()
        .map(|(_, k)| account_key(&k, account))
        .collect();
    if !out.is_empty() {
        out.extend(attribution_tail(venue, tier));
    }
    out
}

/// **The VENUE-WIDE attribution tail, recomputed here from the capability table.**
///
/// ⚠ [`baseline`] is a FROZEN copy of the pre-repair per-venue arms and must stay one, so the tail
/// cannot be pasted into it: doing that would make the equality in
/// [`untouched_venues_compose_the_same_keys`] vacuous for every mechanised venue at once, and for
/// deribit too (it shares the generic arm and has no mechanic, so a pasted tail would have to
/// re-derive the classification anyway). Recomputed instead, from `vike_model::attribution` and the
/// suffix constants — an expression written independently of `view.rs`'s, which is what keeps the
/// comparison a comparison.
///
/// ⚠ NOT account-scoped, matching `attribution_code_from`, which takes no `AccountLabel` at all.
///
/// ⚠ The CODE name comes from `vike_model::credential_keys::attribution_var_for` rather than being
/// composed here, and that is forced: `crates/vike-ops/tests/settings_registry.rs`'s
/// `generated_key_sites` reads a call to `attribution_key` as *this crate reads the whole grid* and
/// demands several hundred `SETTINGS` rows of whatever crate makes it. What this function still
/// recomputes independently is everything that decides WHERE the tail lands — the LIVE-only rule,
/// the has-a-mechanic rule, and the two fee knobs.
fn attribution_tail(venue: &str, tier: &str) -> Vec<String> {
    if tier != "LIVE" {
        return Vec::new();
    }
    let Some(code) = vike_model::credential_keys::attribution_var_for(venue) else {
        return Vec::new();
    };
    let mut out = vec![code];
    match venue {
        "hyperliquid" => out.push("HYPERLIQUID_BUILDER_FEE_TENTHS_BP".to_string()),
        "aster" => out.push("ASTER_BUILDER_FEE_RATE".to_string()),
        _ => {}
    }
    out
}
...
/// **A tier the grid can never light must offer no form at all.** `tier_row` renders the ✎ only
...
///
/// ⚠ **SEVEN names, not two.** The five after the funder address are the rest of what
/// `load_poly_tier` reads — the L2 CLOB trio (`_API_KEY`/`_SECRET`/`_PASSPHRASE`, derived at
/// connect when blank but honoured when set) and the EIP-1271 relayer pair
/// (`_RELAYER_API_KEY`/`_RELAYER_API_KEY_ADDRESS`, which `vike_polymarket::client`'s
/// `submit_order_relayer`/`cancel_order_relayer` sign with). All five were reachable from no form,
/// so an operator holding a CLOB key pair issued by Polymarket's own UI could set it only by
...
fn polymarket_composes_the_l1_private_key_on_the_live_cell_only() {
    ...
    // ⚠ Plus the VENUE-WIDE builder code, which this venue's `SignedBuilder` mechanic earns and
    // which carries NO account label in either spelling — `attribution_code_from` reads the bare
    // name.
    let mut want: Vec<String> = suffixes.iter().map(|s| format!("POLY_LIVE_{s}")).collect();
    want.push("POLYMARKET_BUILDER_CODE".to_string());
    assert_eq!(keys("polymarket", "LIVE", &AccountLabel::Default), want);
    let mut want_alt: Vec<String> =
        suffixes.iter().map(|s| format!("POLY_LIVE_{s}__ALT")).collect();
    want_alt.push("POLYMARKET_BUILDER_CODE".to_string());
    assert_eq!(
        keys("polymarket", "LIVE", &alt()),
        want_alt,
        "the tier-less fallback rung is a TIER fallback, never an ACCOUNT one — a labelled cell \
         must never write a name another account reads; the attribution code is the one name that \
         is venue-wide and therefore UNLABELLED in both"
    );
...
/// **Dukascopy's DEMO cell writes BOTH accounts WHOLE** — login, password and server per account,
/// which is exactly the triple `vike_dukascopy::config::dukascopy_env_var_names` composes and
/// `load_dukascopy_config_from` reads (`crates/bridges/dukascopy/src/config.rs`).
///
/// ⚠ This is the pin that REPLACES [`untouched_venues_compose_the_same_keys`]'s equality for this
/// venue — see [`baseline`]'s dukascopy arm for why that equality went vacuous here and nowhere
/// else. The `_SERVER` pair is the measured defect: both names were in the owner's store and in
/// `vike_ops::settings::SETTINGS`, and in no arm of the editor's table.
///
/// The accounts are numbered rather than tiered (`DEMO1` the Swiss demo, `DEMO2` the EU one), so
/// the order is load-bearing twice over: `render_edit_form`'s group divider is placed at the
/// LENGTH of the first group, and a mis-ordered list would put DEMO1's JNLP url under DEMO2's
/// heading.
#[test]
fn dukascopy_demo_reaches_both_accounts_whole() {
    for account in accounts() {
        let now = keys("dukascopy", "DEMO", &account);
        let mut want: Vec<String> = Vec::new();
        for n in ["DEMO1", "DEMO2"] {
            for s in ["LOGIN", "PASSWORD", "SERVER"] {
                want.push(account_key(&format!("DUKASCOPY_{n}_{s}"), &account));
            }
        }
        assert_eq!(now, want, "dukascopy/DEMO (account {account})");
    }
}
...
/// **The detail pane's `Key family` cell is DERIVED from [`edit_fields`], never written down** —
/// the prefix is the longest `_`-terminated common prefix of every key the venue's forms write,
/// and the shape word says whether those forms are the generic API-key trio.
///
/// Reddens on a hand-written per-venue table being introduced for it — the roster shape
/// `view.rs`' own module docs have twice recorded rotting — and on the prefix cut landing
/// mid-token.
///
/// ⚠ **Here rather than in `view.rs`'s `#[cfg(test)]` block, and it is the same forcing this
/// file's module doc opens with**: every assertion below spells a venue key PREFIX, which
/// `crates/vike-ops/tests/settings_registry.rs`'s `every_read_variable_is_declared` harvests out
/// of `src/` as an env-var-shaped literal and then demands a `vike_ops::settings::SETTINGS` row
/// for. MEASURED: five prefixes, five undeclared-variable failures, before this moved.
#[test]
fn the_key_family_cell_is_derived_from_the_write_table() {
    let binance = vike_connections::key_family("binance");
    assert!(binance.starts_with("BINANCE_"), "{binance}");
    assert!(binance.contains("generic"), "the API key/secret/passphrase trio: {binance}");

    // ⚠ The venue SLUG and the store PREFIX disagree for polymarket, and the cell must show the
    // store's spelling — naming `POLYMARKET_` would point an operator at keys nothing in this
    // tree writes or reads, which is the exact defect `edit_fields`' polymarket arm repaired.
    let poly = vike_connections::key_family("polymarket");
    assert!(poly.starts_with("POLY_"), "{poly}");
    assert!(!poly.starts_with("POLYMARKET_"), "{poly}");
    assert!(poly.contains("bespoke"), "{poly}");

    // cTrader's per-tier OAuth grant and its TIER-LESS Spotware app pair share only the venue
    // token, so the cut lands there rather than inside a tier.
    let ctrader = vike_connections::key_family("ctrader");
    assert!(ctrader.starts_with("CTRADER_"), "{ctrader}");
    assert!(ctrader.contains("bespoke"), "{ctrader}");

    // Aster's DEMO writes the bridge's own `TESTNET` token while LIVE maps 1:1, so the two tiers
    // share no tier token and the prefix stops at the venue.
    let aster = vike_connections::key_family("aster");
    assert!(aster.starts_with("ASTER_"), "{aster}");
    assert!(!aster.contains("TESTNET"), "the two tiers share no tier token: {aster}");
}
