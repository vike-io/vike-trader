...
//! `vike_tradehub::reconcile_config::health_from_feed_status`, which the HEADLESS daemon calls and which
...
//! the egui side paid for all of that to reach 142 dependency-free lines.) `vike-ops` re-exports the
//! whole module as `vike_ops::feed_status`; `vike-connections` carried a second pair of historical
//! names (`vike_connections::{parse_feed_status, ConnectionState}`) and no longer does — its own
//! consumers name this module.
