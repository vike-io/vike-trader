...
//!
//! There is no sibling slug list any more. `vike-catalog` carried one — `BRIDGE_VENUES`, which had
//! already been re-pointed at this const so the two could not drift — and the 2026-09-18 alias
//! ruling deleted it: a second NAME for one roster is an alias even when the VALUE cannot diverge,
//! and every catalog-side and credential-grid caller names this const directly now.
