...
//! choice.** `crates/vike-secrets/Cargo.toml` declares NO `vike-*` dependency — the property that
//! lets `vike-bridge-core` (the transport stack) and `vike-cli` (transport-free) both link it — so
//! that crate cannot name this module at all. (⚠ This said *"declares a literally EMPTY
//! `[dependencies]`"* until 2026-09-13, when decision 0054 put `rusqlite` there ahead of the
//! credential migration. The structural half is untouched: it is the `vike-*` clause that forbids
//! naming this module, and it still holds.) The CALLER writes the record, over a
...
impl AccountLifecycleTarget {
    ...
    /// The act — `create` / `rename` / `activate` / `deactivate` / `remove`.
