...
/// The venues where [`amend_semantics`] and `crate::venue_caps`'s `supports_modify` legitimately
/// DISAGREE, each paired with the reason — the declared-exception list for
/// `amend_semantics_agrees_with_venue_caps_supports_modify`.
///
/// ⚠ **A row here is a promise that the disagreement was REASONED, not that it is harmless.** Read
/// the reason before adding one, and prefer moving a value to adding a row.
///
/// ⚠ **This is a PUBLIC table rather than a test-local const, and the reason is the defect that put
/// it here.** The two tables it reconciles are flattened into one record per venue by
/// `vike_ops::docs_data`'s `venue_record`, which the released `venues.json` carries and the product
/// pages render. A page reading `supports_modify: false` beside `amend_semantics: "unknown"` has
/// every reason to conclude the export contradicts itself — the pair reads as a bug, and a
/// generated page that led with `amend_semantics` told a reader amending was SUPPORTED on a venue
/// whose exported caps row says it is not. The reason lives here, next to the values it reconciles,
/// so the renderer can carry it rather than restate it: one authority, consulted through
/// [`amend_caps_note`], and no second copy to rot.
pub const CAPS_DISAGREEMENTS: &[(&str, &str)] = &[(
    // `caps_for("ibkr")` returns the SOCKET row, whose backend has no native amend
    // (`crates/bridges/vike-ibkr/src/transport/mod.rs`'s `IbkrTransport::modify_order` is a default
    // no-op); `crate::venue_caps`'s `IBKR_CPAPI` sets `supports_modify: true` for the Client Portal
    // backend, whose `modify_order` override really does call the venue's amend endpoint. The
    // bridge picks between them at runtime (`vike_ibkr::caps_for_backend`). This table is keyed on
    // the bare venue string and has no backend axis, so it must answer for the AMENDING backend —
    // and `Unknown`, held conservative, is that answer: `InPlaceTotal` is the one arm that can
    // admit an order the gate should refuse, and no evidence establishes cpapi's convention.
    "ibkr",
    "caps_for returns the socket row; the cpapi backend (IBKR_CPAPI) does amend",
)];

/// The declared reason [`amend_semantics`] and `supports_modify` disagree for `venue`, or `None`
/// when the two tables agree — [`CAPS_DISAGREEMENTS`] as a lookup.
///
/// Consumers that flatten both facts into one view (the docs-data export, and the product pages
/// downstream of it) render this beside the pair so a reader is told WHY it looks contradictory,
/// instead of being left to guess which of the two is the bug.
#[must_use]
pub fn amend_caps_note(venue: &str) -> Option<&'static str> {
    CAPS_DISAGREEMENTS.iter().find(|(v, _)| *v == venue).map(|(_, why)| *why)
}
