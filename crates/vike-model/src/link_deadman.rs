    ...
    match venue {
        ...
        "ig" => LinkDeadMan::SessionBounded {
            why: "a CFD venue with session hours whose Lightstreamer session ends on an END/LOOP \
                  ...
                  its `on_session_error` and emits GapStart. Whether IG holds one session across a \
                  weekend close is unobserved here; flip this row once it has been watched through \
                  one",
        },
