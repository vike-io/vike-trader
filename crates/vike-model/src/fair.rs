...
/// The up/down window length in seconds — and the modulus a Polymarket slug's `sts` must be a
/// multiple of.
///
/// Hoisted here for the same reason [`p_up`] below was, and out of the same crate: it was
/// `vike_backtest::cheap_np`'s window constant (that module has since moved to vike-strategy too,
/// but it was the simulator's when this was hoisted). By the time anyone counted, the identical `300`
/// was spelled THREE times — once there, once in the cockpit tool view under a `POLY_`-prefixed
/// name, and a third time as a file-local `const` inside the `cheap_np_askgate` measurement bin.
/// Two crates, three spellings of one venue fact, none able to see the others. All three now read
/// this line.
pub const UPDOWN_WINDOW_SECS: i64 = 300;
...
/// 3. **Which beta wins.** `crates/vike-strategy/src/fair_value.rs`'s `wc_betas` folds this
///    across the beta set on a strict `<` minimum.
/// 4. **The trade SET.** `cheap_gate` ends `(e > theta).then_some(e)`, and that gate's 1-ulp
///    sensitivity is already a PINNED property — `crates/vike-strategy/src/cheap_np.rs`'s own
...
///
/// `crates/vike-strategy/tests/cheap_np_parity.rs` asserts EXACT integer counts against the
