...
///   arrives through archive readers instead (`vike_backfill`'s `pmxt` / `vike_archive` / the
///   raw-frame `poly_reparse`), which read files and an archive endpoint, not a metered venue API.
///   (A `clickhouse_poly` reader off a local database was in this list until 2026-09-20.)
///
