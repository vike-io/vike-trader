    ...
    pub fn last_instant_at_or_before(&self, now_ms: i64) -> Option<i64> {
        ...
        for _ in 0..7 {
            ...
            if self.weekdays & (1u8 << dow) != 0 {
                let fire_local = cand_day * MS_PER_DAY + target_in_day;
...
mod tests {
    ...
    fn daily_utc_instant_is_midnight() {
        ...
        assert_eq!(r.last_instant_at_or_before(DAY0 - 1), Some(DAY0 - MS_PER_DAY));
    ...
    fn daily_at_hhmm_utc() {
        ...
        assert_eq!(r.last_instant_at_or_before(eight - 1), Some(eight - MS_PER_DAY));
    }
    ...
    fn session_offset_is_before_close() {
        ...
        assert_eq!(got.rem_euclid(MS_PER_DAY), (15 * 60 + 55) * 60_000);
    }
