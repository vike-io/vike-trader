...
pub fn platform_key_service(key: &str) -> Option<&'static str> {
    match key {
        k if k == PLATFORM_KEYS[0] || k == PLATFORM_KEYS[1] => Some(TRADEHUB_SERVICE),
...
mod tests {
    ...
    /// **The two family predicates PARTITION [`PLATFORM_KEYS`]** — every name belongs to exactly
    /// one of them, neither claims a name outside the table, and neither is empty.
    ///
    /// That is the whole safety argument for narrowing `vike_secrets::resolve_node_keys`' probe
    /// from [`is_platform_key`] to a family: disjointness is what makes it impossible to stitch a
    /// pair across the two credential files, and exhaustiveness is what makes it impossible for a
    /// platform key to belong to no family and so be resolvable out of neither file.
    #[test]
    fn the_two_service_families_partition_the_platform_table() {
        let mut tradehub = 0;
        let mut datahub = 0;
        for key in PLATFORM_KEYS {
            let t = is_tradehub_node_key(key);
            let d = is_datahub_node_key(key);
            assert!(t ^ d, "{key} belongs to both families or to neither");
            tradehub += usize::from(t);
            datahub += usize::from(d);
        }
        assert_eq!(tradehub + datahub, PLATFORM_KEYS.len(), "a platform key belongs to no family");
        assert!(tradehub > 0 && datahub > 0, "a family predicate that matches nothing is not one");
        for key in lookup_keys() {
