...
pub const SETTINGS: &[Setting] = &[
    ...
    Setting {
        ...
        // nothing in this workspace consumes the `config.state_dir` this variable feeds. The ROW
        // stays because it is keyed to `vike-config`, whose loader genuinely still looks the name
        // up in the caller-supplied map; what changed is downstream of the read. Deliberately NO
        // repo-anchored citation of the deleted resolver: a `path`'s `SYMBOL` pointer to a function
        // being removed in the same change is a citation-gate failure waiting one commit, and the
        // sentence above is history rather than a place to send a reader. The declared-but-unread
        // KEY is `vike_config::CONSUMPTION`'s problem, not this registry's — its `config.state_dir`
        // row is the one that has to become a written admission.
        name: "VIKE_STATE_DIR",
        krate: "vike-config",
        scope: Scope::Vike,
        layer: Layer::Injected,
