...
pub const SETTINGS: &[Setting] = &[
    ...
    Setting {
        ...
        // an observer (ruling 1) and a market-data subscription is `VerbScope::Observe`
    ...
    Setting {
        ...
        // WRITE verb be `VerbScope::Observe` at all
    ...
    // The DEFAULT is a ladder, not a path, and the ladder is ONE implementation:
    // `vike_model::tick_store_path::resolve_tick_store_root`. The binary logs the resolved root and
    // the rung that chose it once at startup.
    //
    // ⚠ TOMBSTONE — this was a PAIR, and the `("VIKE_TICK_STORE", "vike-desktop")` row stood first.
    // The two binaries had each carried a byte-identical copy of a two-rung `env else <exe_dir>`
    // resolution, which is why the pair had to be kept in step by hand until the shared ladder
    // landed. The desktop cut then removed the GUI's local tick plane entirely — no tick recorder,
    // no properties recorder, no chain recorder, no materializer — and `tick_store_root` lost every
    // caller with them, so the desktop's READ is gone rather than relocated and its row is DELETED
    // rather than re-keyed. The daemon's row below is now the only one; the VARIABLE is unchanged.
    Setting {
