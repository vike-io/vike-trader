...
//! overrides outright — `VIKE_MAX_ORDER_NOTIONAL` (read by `vike-desktop`'s `main.rs` and by
...
//!   (`vike_model::state_path::project_settings_dir` and its no-`vike-*`-dependency twin in
...
//!    anything runs it, and for five of the names above nothing does.** `VIKE_PM_RESOLVE`,
//!    `VIKE_HL_OUTCOME`, `POLY_AUTO_REDEEM`, `POLY_REDEEM_HALT` and `POLY_HEARTBEAT` are each read
//!    inside a poller no composition root constructs, so exporting one changes nothing today —
//!    `vike_config::CONSUMPTION` carries the gated row per key (`Reader::Uncalled`, naming the
//!    entry point that must stay uncalled) and `vike-cli config show` prints the verdict. The rows
//!    below STAY, and stay in this family, because a row states where a variable is read and both
//!    halves of that are still true; what may not stand is a sentence in THIS doc that reads as
//!    "these are live toggles an operator may reach for". `VIKE_RECORD_CHAINS` is the same shape
//!    one family down (a recorder constructor with no call site at all).
...
//!    load/save/layout family, ~10 call sites in the CI-invisible `vike-desktop`); `VIKE_HALT_FILE`
...
pub const SETTINGS: &[Setting] = &[
    ...
    Setting {
        // The Connections editor's WRITE side for the same knob: `vike_connections::view`'s
        // `attribution_fields` spells this name as a literal so the LIVE form can offer the field
        // beside the builder CODE it pairs with. It writes and never reads — the same shape the
        // `DUKASCOPY_DEMO*` `vike-connections` rows carry.
        name: "ASTER_BUILDER_FEE_RATE",
    ...
    Setting {
        ...
        // `(name, krate)`, which is why this is its own row rather than an edit to either above.
        name: "CARGO",
    ...
    Setting {
        // The Connections editor's WRITE side, the twin of the `DUKASCOPY_DEMO1_LOGIN`/`_PASSWORD`
        // rows above it: `vike_connections::view::edit_fields` spells this name as a literal so the
        // form can offer the field. It WRITES the key and reads none — the same shape those two
        // rows have carried since that arm existed. It is here because the key was in the owner's
        // real store and in no arm of that table, which is the gap
        // `crates/vike-app-core/tests/credential_editor_completeness_gate.rs` now polices.
        name: "DUKASCOPY_DEMO1_SERVER",
        krate: "vike-connections",
        scope: Scope::Venue,
        layer: Layer::Injected,
    ...
    Setting {
        // The EU demo account's half of the pair above — same argument, same shape.
        name: "DUKASCOPY_DEMO2_SERVER",
        krate: "vike-connections",
        scope: Scope::Venue,
        layer: Layer::Injected,
    ...
    Setting {
        ...
        // `vike-buildinfo`'s otherwise-identical trio a few rows below, so a separate set of rows:
    ...
    Setting {
        // The env layer over `config.toml`'s `tradehub_advertise_addr`: WHICH BOX the
        ...
        name: "VIKE_TRADEHUB_ADVERTISE_ADDR",
        krate: "vike-config",
        scope: Scope::Vike,
        layer: Layer::Injected,
        naming: Naming::MapLookup,
    ...
    Setting {
        // The env layer over `config.toml`'s `backtest_addr` — the COMPUTE daemon's listen address
    ...
    Setting {
        ...
        // `crates/vike-ops/tests/settings_registry.rs`'s `LIBRARY_PIN`, which is a RATCHET that may
        // only shrink — so the correct shape was also the only one CI would accept.
        //
        // ⚠ `MapLookup` even though the key IS a named constant
        // (`vike_backtest::named_run::NAMED_RUN_ENV`): `layer_and_naming_agree_on_every_row` below
        // holds that an `Injected` row is a map lookup, because what that pairing describes is HOW
        // THE VALUE IS REACHED, not how the key is spelled. The constant stays anyway — it is the
        // doc anchor the arming message and the tests both name.
    ...
    Setting {
        // The DESKTOP's row for the same name, and it is a SECOND row because `SETTINGS` is keyed
        // on the PAIR `(name, krate)`: `crates/vike-app-core/src/backend_registry.rs` declares its
        ...
        //
        // ⚠ There is deliberately NO `VIKE_DATAHUB_CONTROL_KEY` row for this crate. The desktop is
        // an observer (ruling 1) and a market-data subscription is `VerbScope::Read`
        // (`docs/decisions/0052`); the datahub's control scope carries the verbs that COMPILE
        // CLIENT-SUPPLIED RHAI, and that key must never be resolved here.
        name: "VIKE_DATAHUB_OBSERVE_KEY",
        krate: "vike-app-core",
        scope: Scope::Vike,
        layer: Layer::Injected,
    ...
    Setting {
        ...
        name: "VIKE_DATAHUB_LIVE_RESIDENT",
        krate: "vike-datahub",
        scope: Scope::Vike,
        layer: Layer::Injected,
        naming: Naming::MapLookup,
    ...
    // `crates/vike-data/tests/manifest_delta.rs`'s `measure_v2_migration_on_a_real_manifest` runs
    ...
    // ⚠ TOMBSTONE — the `("VIKE_POLY_TICKS", "vike-desktop")` row stood here and is DELETED. Unlike
    // its `VIKE_POLY_COCKPIT_TOKEN` neighbour above (a QA seed the GUI still reads), this one armed
    ...
    Setting {
        ...
        // `vike_config::flags`' `VENUE_CATALOG_OFF_ENV` is the reader, and the FILE spelling
        // (`venue_catalog_off` in `flags.toml`) is the one `docs/decisions/0066` rules is primary —
        // the owner's ruling was that the switch moves into settings.
        name: "VIKE_DATAHUB_VENUE_CATALOG_OFF",
        krate: "vike-config",
        scope: Scope::Vike,
        layer: Layer::Injected,
        naming: Naming::MapLookup,
    ...
    Setting {
        ...
        // `crates/vike-ops/tests/settings_registry.rs` documents. When `reconcile_config` moved
    ...
    Setting {
        ...
        layer: Layer::Injected,
        naming: Naming::MapLookup,
        default: "DEAD — warned at startup, configures nothing",
    },
    Setting {
    ...
    Setting {
        // ⚠ NOT the state ROOT below, despite the name: this is the strategy-state SIDECAR
        // directory (the per-mount `strategy_state::write_json_atomic` blobs). It predates
        ...
        // commit.
        //
        // ⚠ `VIKE_STATE_ROOT` directly below is NOT the replacement and an operator must not be
        // sent there: it is the state ROOT, a different directory, and the refusal says so.
        name: "VIKE_STATE_DIR",
        krate: "vike-config",
        scope: Scope::Vike,
        layer: Layer::Injected,
    ...
    //   - ⚠ HISTORICAL, and the tombstone forty lines below is the live account: the four
    ...
    // licensed it — and the row below is the re-keyed `vike-config` twin that refusal owes the
    // registry, the same shape its sibling `VIKE_TRADEHUB_RECORD` takes.
    //
    // `vike_model::tick_store_path::resolve_tick_store_root` — the ladder the deleted row
    ...
    Setting {
        // The env layer over `config.toml`'s `tradehub_account_admin` — the THREE-VALUED barrier
        ...
        // caller-supplied map, like every other `config.toml` key.
        name: "VIKE_TRADEHUB_ACCOUNT_ADMIN",
        krate: "vike-config",
        scope: Scope::Vike,
        layer: Layer::Injected,
        naming: Naming::MapLookup,
    ...
    Setting {
        ...
        name: "VIKE_STRATEGY_BUILDER_KEY",
        krate: "vike-strategy-builder",
        scope: Scope::Vike,
        layer: Layer::Injected,
        naming: Naming::MapLookup,
    ...
    Setting {
        ...
        // directly off its one `std::env::vars()` sweep (`Layer::Binary`, the shape this file's
        // own doc names for `vike-backfill/src/bin/tardis_backfill.rs`'s `TARDIS_API_KEY`).
        ...
        name: "VIKE_STRATEGY_BUILDER_PORT",
        krate: "vike-strategy-builder",
        scope: Scope::Vike,
        layer: Layer::Injected,
        naming: Naming::MapLookup,
    ...
    Setting {
        name: "VIKE_STRATEGY_BUILDER_OUT_DIR",
        krate: "vike-strategy-builder",
        scope: Scope::Vike,
        layer: Layer::Injected,
        naming: Naming::MapLookup,
        default: "user_data/plugins",
    },
    Setting {
        name: "VIKE_STRATEGY_BUILDER_RETAIN",
        krate: "vike-strategy-builder",
        scope: Scope::Vike,
        layer: Layer::Injected,
        naming: Naming::MapLookup,
    ...
    Setting {
        ...
        // reason as `VIKE_STRATEGY_BUILDER_KEY` above: `crates/vike-ops/tests/compile_time_path_gate.rs`
        ...
        name: "VIKE_STRATEGY_BUILDER_WORKSPACE_ROOT",
        krate: "vike-strategy-builder",
        scope: Scope::Vike,
        layer: Layer::Injected,
        naming: Naming::MapLookup,
