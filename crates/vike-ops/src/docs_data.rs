...
//!   venue. ⚠ It is a SECOND table on the same axis as `caps.supports_modify` above, and the two
//!   are keyed differently — `caps_for` resolves a per-BACKEND row that table has no axis for — so
//!   a record can carry a pair that reads as a contradiction. `amend_semantics_note` is that pair's
//!   explanation and the reason this export renders both.
//! - `amend_semantics_note` — `vike_model::amend_caps_note`, the declared reason
//!   `amend_semantics` and `caps.supports_modify` disagree for this venue, or `null` when they
//!   agree (13 of the 14 roster venues today). NOT a fact of its own: it is
//!   `vike_model::venue_amend`'s `CAPS_DISAGREEMENTS` read through its accessor, so this renderer
//!   states no reason it could drift from. It exists because flattening two differently-keyed
//!   tables into one record is what MAKES the contradiction visible — `supports_modify: false`
//!   beside `amend_semantics: "unknown"` reads as a bug to a consumer that cannot see the backend
//!   axis, and a page rendering `amend_semantics` first told a reader amending was supported on a
//!   venue whose own caps row says it is not. `crates/vike-ops/tests/docs_data_gate.rs`'s
//!   `an_incoherent_amend_pair_is_explained_and_a_coherent_one_is_not` gates the pairing in BOTH
//!   directions over the rendered JSON, which is the artifact the pages actually read.
...
//!   `vike_model::venue_caps`'s `ORDER_KINDS`/`TRIGGER_KINDS`, `vike_model::AssetClass` (with
//!   each class's Symbol-picker tab, through `vike_catalog::tab_for`), `vike_data::store_kind::
...
use vike_model::{
    ...
    VenueMarginSupport, amend_caps_note, amend_semantics, caps_for, fee_schedule_for,
    venue_margin_support,
};
use vike_strategy::{
