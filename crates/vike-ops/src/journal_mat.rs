...
//!
//! It lives in `vike-ops` — which depends on BOTH `vike-journal` (the WAL itself) and
...
//! ([`vike_journal::MaterializeCheckpoint`]) + the store's `commit_key` idempotency, so a
...
use vike_journal::{JournalRecord, MaterializeCheckpoint, read_since};
...
fn materialize_once(
    ...
    let records = if MaterializeCheckpoint::exists(dir) {
        ...
        vike_journal::CommandJournal::read_all(dir)
    }
...
impl OrderTracker {
    ...
    fn fold_record(&mut self, rec: &JournalRecord, touched: &mut HashSet<String>) {
        match rec {
            ...
            // variant" contract on [`vike_journal::JournalRecord`]. This crate is a DIFFERENT
...
mod tests {
    ...
    fn materializes_wal_fills_into_tier2_idempotently() {
        ...
        {
            let mut j = vike_journal::CommandJournal::open(
                dir.path(),
                vike_journal::JournalFileConfig::default(),
            )
            .unwrap();
