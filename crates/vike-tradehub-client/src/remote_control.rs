...
use crate::proto::{
    FEATURE_ACCOUNT_ROUTING, FEATURE_ACCOUNT_SCOPED_REDUCE, FEATURE_MOUNT_ACCOUNT,
...
fn required_feature(cmd: &WireCommand) -> Option<&'static str> {
    match cmd {
        ...
        // the field here. The node then fans the verb out over EVERY account of the exchange and
        // answers `accepted`: `crates/vike-tradehub/src/server.rs`'s `lower_command` destructures
        // these variants as `{ venue, symbol, .. }`, and `vike_exec::OrderIntent`'s reducing
        // variants have no account field for it to land in. `FEATURE_ACCOUNT_SCOPED_REDUCE` is
        // deliberately advertised by NO node, so this arm refuses locally until that is fixed —
        // its doc names the three things that must be true first.
        //
        ...
        c if names_an_account(c) => Some(FEATURE_ACCOUNT_ROUTING),
        _ => None,
    }
}
...
impl RemoteControlHandle {
    ...
    /// `vike_app_core::tradehub_control::venue_routing_verdict` beside that list; do not branch on
...
mod tests {
    ...
    fn the_eviction_mark_never_overrides_a_recorded_answer() {
        ...
        assert_eq!(
            board.wait(evicted, NOW),
            None,
            "the entry that actually fell out of the window"
        );
        assert_eq!(
