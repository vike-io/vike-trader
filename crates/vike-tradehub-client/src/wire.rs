...
/// 2. **AUTHORIZATION.** Every verb here requires `Scope::Admin`, a THIRD key. The scope byte is
...
pub enum AccountVerb {
    ...
    /// NAMES each row owns.
    ///
    /// ⚠ **It is `Scope::Admin` like every other verb here, and that is the one place 0065 NARROWS
...
mod tests {
    ...
    fn every_venue_carrying_variant_answers_with_its_own_venue() {
        ...
        assert_eq!(
            WireCommand::Flatten { venue: "okx".into(), symbol: "SYM".into() }.addressed_venue(),
            Some("okx")
        );
        assert_eq!(
            WireCommand::MassCancel { venue: Some("bybit".into()), symbol: None }.addressed_venue(),
            Some("bybit")
        );
        assert_eq!(
            WireCommand::MarketExit { venue: Some("deribit".into()) }.addressed_venue(),
            Some("deribit")
        );
        assert_eq!(
            WireCommand::UpdateParams {
