...
pub enum WireCommand {
    ...
    ///
    /// ⚠ **`account` does NOT scope it yet, and a client may not send one.** The intent is that it
    /// scopes the cancel to ONE account of `venue`; the node drops it at
    /// `crates/vike-tradehub/src/server.rs`'s `lower_command` and cancels across every account of
    /// the venue instead. The field is therefore gated by
    /// `crate::proto::FEATURE_ACCOUNT_SCOPED_REDUCE`, which the node does not advertise, so the
    /// frame is refused client-side before it is sent. See [`WireOrderRequest::account`] for the
    /// three wire states and why absence cannot be read as a default — that part is unchanged, and
    /// an absent account still means every account of the venue, deliberately.
    MassCancel {
        venue: Option<String>,
    ...
    ///
    /// ⚠ **Naming an account does NOT narrow this verb yet, and a client may not send one.** The
    /// intent is that it narrows the exit back to that account; the node does not do it — the
    /// account is dropped at `crates/vike-tradehub/src/server.rs`'s `lower_command` and
    /// `vike_core`'s `CoreThread::exit_scope_engines` fans out over every account of the venue, so
    /// a frame naming `ALT` would flatten the unlabelled book too. Until that is built, this field
    /// is gated by `crate::proto::FEATURE_ACCOUNT_SCOPED_REDUCE`, which the node does not
    /// advertise, so `crate::remote_control::required_feature` refuses the frame client-side and
    /// nothing goes on the wire. That constant's doc is the work-list; delete this paragraph the
    /// day it comes true.
    MarketExit {
        venue: Option<String>,
        #[serde(default, skip_serializing_if = "Option::is_none")]
