...
///
/// Deliberately MINIMAL — the three facts the daemon's identity block already carries per mount.
/// The mount's addressing key (`venue`/`symbol`/`interval` — what a [`WireCommand::UpdateParams`]
/// targets) is still deferred: a multi-mount daemon's `params` string names the mount's own
/// venue/symbol (the daemon renders each row from its own profile row), and inventing a second
/// structured copy of that key before a client needs to ADDRESS a row would be a guess about the
/// runtime-mount work (B5).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
