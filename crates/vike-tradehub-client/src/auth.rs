...
//!
//! The crypto, [`NodeKeys`] and [`Scope`] live in [`vike_node_proto::auth`], generalized
...
//! crate already re-exports the FRAME CODEC rather than growing a second one (see [`crate::proto`]'s
//! "ONE framing across both localhost services"), and the two primitives belong in the same place —
//! so wherever the codec lives is where a primitive the two services must not disagree about
//! belongs. `vike_node_proto::auth`'s module doc carries the full argument.
//!
//! # The signed message
//!
...
//! [`vike_node_proto::auth::DATAHUB_DOMAIN`] — so a tag can never be cross-purposed
...
/// from the datahub's [`vike_node_proto::auth::DATAHUB_DOMAIN`] and from any venue
...
/// import them: that test asserts these names do NOT resolve on the datahub plane, and a literal is
/// the honest spelling for a name you are proving absent. ⚠ The reason used to be stated as a rank
/// gap — "it is layer 30 and cannot see this crate (layer 50)" — and on 2026-09-23 the two became
/// PEERS at 30 when the shared substrate moved below both. The edge is still refused, by the
/// same-rank rule rather than by a gap, and the literal is still the honest spelling; only the
/// arithmetic behind the sentence changed.
///
...
mod tests {
    ...
    /// replay) lives with the implementation in `vike_node_proto::auth` and is not
    /// duplicated here.
    #[test]
