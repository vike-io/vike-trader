...
///
/// ⚠ **NO SHIPPED NODE ADVERTISES THIS YET, and that is the state this constant exists to make
/// legible.** `vike-tradehub`'s `served_features` deliberately omits it: the daemon-side renderer
/// (open the journal `vike_report::tearsheet_cli::run` reads, render, reply) is a follow-up, and a
/// capability advertised before its arm works would turn a clean client-side refusal into an
/// opaque server error. So today every `vike-cli report` invocation takes the refusal path, which
/// is the honest answer while the server half is unwritten — and the day the arm lands, adding this
/// string to that list is what switches the verb on, with no client change and no version bump.
///
...
pub enum Request {
    ...
    /// [`Scope::Observe`] suffices, exactly like [`Request::StrategyStatus`]). Ask the node to
    ...
    /// version bump, and for why no shipped node advertises it yet.
    Tearsheet {
...
pub enum Response {
    ...
    /// the JSON verbatim is the shape [`vike_datahub_client::proto`]'s `Response::Report` and
    /// `Response::SweepReport` already use for the same reason: a machine consumer gets the ONE
