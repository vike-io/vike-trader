...
/// `vike_datahub_client::proto`'s `FEATURE_SEARCH_METHOD`, not a runtime one, because the field and
...
///
/// ⚠ Seeing it is NOT authorization. Every account verb additionally requires `Scope::Admin`, a
...
pub enum Request {
    ...
    /// declared.md`; post-auth, `Scope::Admin` ONLY). The settings database's `account` table, plus
