...
//! [`Scope::Read`], subscribes, and thereafter reads the latest pushed [`WireSnapshot`] off a
...
use crate::proto::{
    FEATURE_OBSERVE_HEARTBEAT, FEATURE_SETTINGS_SHOW, FEATURE_STRATEGY_PARAMS,
    FEATURE_STRATEGY_VERBS, FEATURE_TEARSHEET, Request, Response, Scope, Topic, read_frame,
    write_frame,
};
...
impl RemoteCoreHandle {
    /// Connect to a node's observe server at `addr`, run the full handshake under [`Scope::Read`]
    /// using `observe_key` (the raw bytes of `VIKE_TRADEHUB_OBSERVE_KEY`), subscribe to every block,
    /// and spawn the receive thread.
    ///
    /// The handshake is the shared `crate::handshake` sequence run under [`Scope::Read`]: send
...
/// Ask a node for ONE point-in-time [`WireSnapshot`] over a fresh short-lived [`Scope::Read`]
/// connection: handshake, [`Request::Snapshot`], read the [`Response::SnapshotFrame`], drop.
///
/// ⚠ **Named for the ROUND TRIP, because [`RemoteCoreHandle::snapshot`] is a different thing with
/// the same noun.** That method reads the last frame the subscription already pushed into a local
/// cell — no I/O, never blocks, possibly stale. This one goes to the node and back. A caller that
/// holds a subscription wants the method; a one-shot command that has no subscription and needs
/// one fact (the trading mode, say) wants this, and opening a whole `Subscribe` stream to read a
/// single field would spawn a receive thread and then poll for a first frame that may never come.
///
/// **Not feature-negotiated, deliberately**: [`Request::Snapshot`] is a BASE verb of the node
/// protocol — every node that completes the handshake serves it — so unlike [`strategy_status`]
/// there is no capability to check and no client-side refusal to write. A handshake failure
/// (wrong/absent observe key, version skew, transport fault) surfaces as its usual [`io::Error`];
/// a server-side [`Response::Error`] and an unexpected response kind both surface as
/// [`io::ErrorKind::InvalidData`], the shape [`strategy_status`] established so that a caller's
/// exit-code ladder can key on "the node ANSWERED" without a per-verb table.
pub fn snapshot_once<A: ToSocketAddrs>(addr: A, observe_key: &[u8]) -> io::Result<WireSnapshot> {
    ...
    match read_frame::<_, Response>(&mut stream)? {
        ...
        Response::Error(msg) => Err(io::Error::new(
            ...
            format!("tradehub snapshot error: {msg}"),
        )),
        other => Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "tradehub snapshot: expected SnapshotFrame, got {}",
...
/// [`Scope::Read`] connection (the same handshake [`RemoteCoreHandle::connect`] runs, minus the
...
/// [`strategy_status`], keeping the node's advertised `Welcome.features`.
///
/// ⚠ **The same call, not a second one** — `strategy_status` is a two-line wrapper over this, so
/// there is one implementation and the two cannot answer differently. It exists because a caller
/// that RENDERS a mount row needs a capability the row itself cannot express:
/// [`crate::proto::FEATURE_MOUNT_CLASS`] tells an old node (which cannot carry
/// `WireMountRow::asset_class` at all) from a current one whose mount is simply not migrated yet,
/// and `#[serde(default)]` makes both arrive as `None`. Every other caller wants the status alone
/// and keeps the shorter name.
///
/// ⚠ It returns the features VERBATIM rather than a set of decoded booleans. A decoded struct would
/// have to grow a field per capability and would be a second place the capability list is written
/// down; the strings are the wire's own vocabulary, and `iter().any(|f| f == FEATURE_…)` is the
/// idiom every other check in this file already uses.
...
/// Ask a node for its mounts' STRUCTURED LIVE PARAMS — the read half of the live-parameter plane.
/// Wire-identically [`strategy_status`]: same `Request::StrategyStatus`, same
/// `Response::StrategyStatus`, same per-call short-lived [`Scope::Read`] connection. What
/// differs is the CAPABILITY it negotiates on, and that is the entire point of it being its own
/// function.
///
/// **Feature-negotiated on [`FEATURE_STRATEGY_PARAMS`], refused CLIENT-SIDE**, the
/// [`settings_show`] shape verbatim: against a node that does not advertise it this fails with
/// [`io::ErrorKind::Unsupported`] naming the capability, and NOTHING is sent after the handshake.
///
/// ⚠ Why a second function rather than a field check on [`strategy_status`]'s answer: an older node
/// ANSWERS this request perfectly well, with rows whose `venue`/`symbol`/`interval`/`typed_params`
/// `#[serde(default)]` fills in as empty — indistinguishable from an honest "these mounts publish
/// no typed params". A caller that inspected the rows instead of the capability would read a stale
/// node as a params-less one, offer no tuning surface, and never say why. Callers that only want
/// the rendered human status keep calling [`strategy_status`] and keep working against every node.
///
/// The addressing key on each row is what makes the answer actionable: it is exactly what a
/// `WireCommand::UpdateParams` targets, so a caller patches `typed_params` and sends the whole
/// object back through the unchanged, already-gated write verb.
pub fn strategy_params<A: ToSocketAddrs>(
    addr: A,
    observe_key: &[u8],
) -> io::Result<WireStrategyStatus> {
    let (mut stream, features) = node_handshake(addr, observe_key, Scope::Read)?;
    if !features.iter().any(|f| f == FEATURE_STRATEGY_PARAMS) {
        return Err(io::Error::new(
            io::ErrorKind::Unsupported,
            format!(
                "this node does not advertise the \"{FEATURE_STRATEGY_PARAMS}\" capability \
                 (an older vike-tradehub) — the structured params read was refused client-side, \
                 nothing was sent"
            ),
        ));
    }
    ...
    match read_frame::<_, Response>(&mut stream)? {
        ...
        Response::Error(msg) => Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!("tradehub strategy params error: {msg}"),
        )),
        other => Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "tradehub strategy params: expected StrategyStatus, got {}",
                crate::handshake::resp_kind(&other)
            ),
        )),
    }
...
/// per-call shape: open a fresh short-lived [`Scope::Read`] connection (the same handshake
...
/// Ask a node to render a LIVE TEARSHEET from the journal IT is writing — ruling 16's `report`
/// verb, and [`settings_show`]'s exact per-call shape: one fresh short-lived [`Scope::Read`]
/// connection, one [`Request::Tearsheet`], one reply, connection dropped.
///
/// Returns the `vike_report::LiveTearsheet` JSON TEXT verbatim — see [`Response::Tearsheet`] for
/// why the payload is a string rather than a typed struct on this wire.
///
/// # Why the node renders instead of shipping the journal
///
/// The journal is an append-only fill stream that grows for the life of the deployment; the
/// tearsheet is a few dozen numbers. Rendering next to the data and returning the ANSWER is the
/// compute-to-data rule `crates/vike-datahub-client/src/client.rs`'s `run_backtest` is built on,
/// and it is what makes
/// this verb usable over a link an operator would not want to stream a journal across.
///
/// # Feature-negotiated, refused CLIENT-SIDE — against a daemon that PREDATES the arm
///
/// The request goes on the wire only when the node's `Welcome.features` advertises
/// [`FEATURE_TEARSHEET`]. ⚠ **This heading read *"and TODAY THAT IS EVERY NODE"*, and the
/// paragraph under it *"No shipped `vike-tradehub` build advertises it yet — the daemon-side
/// renderer is a follow-up"*.** Both were exact until the arm landed:
/// `crates/vike-tradehub/src/server.rs`'s `tearsheet_reply` serves the verb and that file's
/// `served_features` advertises the capability unconditionally, so a node built from this tree
/// passes the check and this function SENDS. The closing prediction is the half that came true —
/// *"the day the arm lands this function is already correct"* — and not one line of the body below
/// changed when it did.
///
/// What the check is FOR is the complement: an OLDER daemon, which still returns
/// [`io::ErrorKind::Unsupported`] having sent nothing after the handshake. That is the whole value
/// of negotiating — the caller gets a sentence naming the missing capability instead of an opaque
/// server error — and it is live code rather than a vestige; it is simply no longer the ordinary
/// outcome. ⚠ Do NOT read a [`Response::Error`] as that refusal: a node that DOES serve the verb
/// can still answer that it cannot resolve its own journal directory, and this function surfaces
/// that as [`io::ErrorKind::InvalidData`] carrying the node's own sentence. One is a claim about
/// the peer's BINARY, the other about how that process was started.
///
/// `seed` and `periods_per_year` are `None` for "use the renderer's own default" rather than a
/// number this side invents — the node owns those defaults (`vike_report::tearsheet_cli`).
pub fn tearsheet<A: ToSocketAddrs>(
    ...
    let (mut stream, features) = node_handshake(addr, observe_key, Scope::Read)?;
    if !features.iter().any(|f| f == FEATURE_TEARSHEET) {
        return Err(io::Error::new(
            io::ErrorKind::Unsupported,
            format!(
                "this node does not advertise the \"{FEATURE_TEARSHEET}\" capability \
                 — Tearsheet refused client-side, nothing was sent"
            ),
        ));
    }
    ...
    match read_frame::<_, Response>(&mut stream)? {
        ...
        Response::Error(msg) => Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!("tradehub tearsheet error: {msg}"),
        )),
        other => Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "tradehub tearsheet: expected Tearsheet, got {}",
                crate::handshake::resp_kind(&other)
            ),
        )),
    }
