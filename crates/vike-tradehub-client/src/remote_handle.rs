...
///
/// # Feature-negotiated, refused CLIENT-SIDE — and TODAY THAT IS EVERY NODE
///
/// The request goes on the wire only when the node's `Welcome.features` advertises
/// [`FEATURE_TEARSHEET`]. **No shipped `vike-tradehub` build advertises it yet** — the daemon-side
/// renderer is a follow-up, and that constant carries what it owes — so every call currently
/// returns [`io::ErrorKind::Unsupported`] having sent nothing after the handshake. That is
/// deliberate and is the whole value of negotiating: the caller gets a sentence naming the missing
/// capability instead of an opaque server error, and the day the arm lands this function is
/// already correct.
///
