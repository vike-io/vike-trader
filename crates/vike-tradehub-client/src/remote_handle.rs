...
/// Ask a node for ONE point-in-time [`WireSnapshot`] over a fresh short-lived [`Scope::Observe`]
...
pub fn snapshot_once<A: ToSocketAddrs>(addr: A, observe_key: &[u8]) -> io::Result<WireSnapshot> {
    let (mut stream, _features) = node_handshake(addr, observe_key, Scope::Observe)?;
...
/// `Response::StrategyStatus`, same per-call short-lived [`Scope::Observe`] connection. What
...
/// verb, and [`settings_show`]'s exact per-call shape: one fresh short-lived [`Scope::Observe`]
