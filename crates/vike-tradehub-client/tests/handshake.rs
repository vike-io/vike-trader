...
use vike_tradehub_client::{
    auth::{self, NodeKeys},
    proto::{NODE_PROTO_VERSION, Request, Response, Scope, read_frame, write_frame},
};
...
fn legitimate_control_client_authenticates() {
    ...
    let resp = run_handshake(&keys, server_nonce(1), keys.key_for(Scope::Write), Scope::Write);
    assert_eq!(resp, Response::AuthOk { scope: Scope::Write });
}

#[test]
fn legitimate_observe_client_authenticates() {
    let keys = NodeKeys::new(b"observe-key".to_vec(), b"control-key".to_vec());
    let resp = run_handshake(&keys, server_nonce(2), keys.key_for(Scope::Read), Scope::Read);
    assert_eq!(resp, Response::AuthOk { scope: Scope::Read });
}
...
fn observe_key_presented_for_control_is_denied() {
    ...
    let resp = run_handshake(&keys, server_nonce(3), keys.key_for(Scope::Read), Scope::Write);
...
fn wrong_key_is_denied() {
    let keys = NodeKeys::new(b"observe-key".to_vec(), b"control-key".to_vec());
    let resp = run_handshake(&keys, server_nonce(4), b"attacker-guess", Scope::Write);
    assert_eq!(resp, Response::AuthDenied { reason: "bad mac".into() });
}
...
fn a_replayed_mac_from_a_prior_connection_is_denied() {
    let keys = NodeKeys::new(b"observe-key".to_vec(), b"control-key".to_vec());
    let control_key = keys.key_for(Scope::Write);

    // Connection A (nonce seed 10): the client legitimately authenticates; capture its Auth mac.
    let nonce_a = server_nonce(10);
    let captured_mac = auth::sign(control_key, &nonce_a, NODE_PROTO_VERSION, Scope::Write);
    let valid_on_a =
        auth::verify(control_key, &nonce_a, NODE_PROTO_VERSION, Scope::Write, &captured_mac);
    ...
    let valid_on_b =
        auth::verify(control_key, &nonce_b, NODE_PROTO_VERSION, Scope::Write, &captured_mac);
