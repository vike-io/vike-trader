...
//! `poly_ch_backtest` bin uses: [`vike_backtest::harness::run_backtest`] over whichever
//! `vike_data::HistStore` the store flags select. There is NO external JSON/aggregation tooling —
//! every backtest, the metric extraction, and the group aggregation are Rust, and since 2026-09-20
//! this bin spawns NO external process at all: the one it used to spawn was `clickhouse-client`,
//! under the `--clickhouse` store that has since been deleted.
//!
...
//!
//! ```sh
//! poly_mm_batch --universe universe.tsv (--archive PATH | --store DIR) [--fee] [--floor N] [--latency-ms N] [--lane l1|l2|both] [--strategy spread|trailing|both]
//! ```
//!
//! Both store flags are parsed and opened by [`vike_data::backtest_store`], shared
//! verbatim with `poly_ch_backtest` — that module's doc is the authority on the precedence between
//! them, on why naming NONE is a refusal, and on what the deleted third flag (`--clickhouse`) took
//! with it. What is worth carrying here is which to reach for:
//!
//! - **`--archive PATH` is the preferred way to run against downloaded archive data.** It reads the
//!   `data.vike.io` Parquet day files IN PLACE — no import, no second copy. `PATH` may be a
//!   directory (every `*.parquet` under it) or a single day file.
...
//!
//! - `--store DIR` reads a local `vike_data::DataFusionHist` Parquet store — the fast path for a
//!   token universe already one-time-exported into `DIR`. ⚠ The exporter this line named —
//!   `clickhouse_poly_backfill --kind all --store DIR` — was RETIRED on 2026-09-19 for reaching
//!   ClickHouse directly, and its module was DELETED on 2026-09-20 with the read-side store beside
//!   it (`crates/vike-data/src/backtest_store.rs` carries what that cost). Fill `DIR` with
//!   `vike_archive_backfill --kind all --store DIR` instead: the same three lanes
//!   (`book_events`/`trades`/`l1_quotes`) over `data.vike.io/archive`, landing in the same
//!   `venue=polymarket`/`symbol=token_id` series this flag then reads.
//!
//! - ⚠ A THIRD flag, `--clickhouse`, read the live recorder's tables through a
//!   `clickhouse-client` shell per scan verb per market. It is DELETED, and with it the only route
//!   from this tree to a ClickHouse server. The per-market [`CachedHistStore`] memoizing wrapper
//!   (below) sits in front of both surviving backends unchanged.
//!
...
//! memoizes each scan verb's result — turning what was 26x redundant scans per market into one.
//! (It was built when the worst case was 26x redundant `clickhouse-client` shells; over a local
//! Parquet file the saving is smaller but the same shape, so the wrapper stays.)
...
use vike_data::backtest_store::{open_backtest_store, parse_backtest_store, window_clock_banner};
...
// `scan_book_updates` 26 times per market was 26x redundant reads and Arrow decodes of the exact
// same rows. (It was worse than that when it was written: the store then was the deleted
// `ClickHousePolyHistStore`, so each redundant scan was a `clickhouse-client` shell plus a Parquet
// export.) This wrapper memoizes each read verb's result the first time it is asked for a given
...
impl CachedHistStore {
    ...
    /// scan still gets a stable cache key. (It was written to mirror the deleted
    /// `backtest_bridge`'s own `range_bounds`, and the convention outlived it:
    /// `crates/vike-data/src/window_clock.rs`'s `other_clock_excludes` resolves the same way.)
...
// ⚠ No line below may name a box — same rule the sibling `poly_ch_backtest` bin's `USAGE` carries,
// for the same reason: `crates/vike-ops/tests/shipped_box_name_gate.rs` greps every shipped string
// literal and the release build greps the published binary's raw bytes. The deleted `--clickhouse`
// line is why the rule was written here; it described a database on a particular box and had to say
// "the recorder's own box" instead.
const USAGE: &str = "\
usage: poly_mm_batch --universe PATH (--archive PATH | --store DIR)
                     [--lane l1|l2|both] [--strategy spread|trailing|both]
...
BacktestReports in process, fanned across all cores. All-Rust: no external aggregation tooling.
  ...
  --archive PATH        downloaded data.vike.io archive Parquet, read in place (a day file or a
                        directory of them) — no import step
...
fn main() -> ExitCode {
    ...
    // Which of the two stores. ⚠ Naming NONE used to select ClickHouse; since 2026-09-19 it is a
    // refusal — `vike_data::backtest_store` owns the rule and the argument for it.
    let store_sel = match parse_backtest_store(
        arg(&args, "--store").as_deref(),
        arg(&args, "--archive").as_deref(),
    ) {
        Ok(s) => s,
        Err(e) => {
    ...
    // WHICH CLOCK every per-market window below will mean. `window_for` is a FIVE-minute slice —
    // one rolling market's whole life — which is precisely the window size at which the recorder's
    // two stamps disagree most (`vike_data::window_clock::WindowClock` measures it), so this
    // line prints before any result does rather than living only in a doc comment.
    eprintln!("poly_mm_batch: {}", window_clock_banner(&store_sel));
    // ⚠ A `<project>/tmp` scratch root was resolved HERE and threaded down for the ClickHouse
    // selection to stage its exports into. Both surviving stores read files the operator named, so
    // nothing is staged and there is no root to resolve.
    let store: Arc<dyn HistStore + Send + Sync> = match open_backtest_store(store_sel) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("poly_mm_batch: {e}");
            return ExitCode::FAILURE;
    ...
    // Sync>` cloned per run, and both surviving stores open their readers per scan, so concurrent
    // markets never race. ⚠ Capping with `RAYON_NUM_THREADS=N` used to be about not saturating a
    // SHARED ClickHouse server — the one store whose load landed off-box. With that store deleted
    // the only resource at stake is this box's own disk and memory. Aggregation is done
    // sequentially afterward (cheap).
    ...
    let per_market: Vec<(Vec<ConfigRow>, u64)> = lines
        ...
        .map(|line| {
            ...
            // wrapped store's `scan_quotes`/`scan_trades`/`scan_book_updates` each hit the backing
            // store once here instead of once per run (26x). Never shared ACROSS markets (a fresh
