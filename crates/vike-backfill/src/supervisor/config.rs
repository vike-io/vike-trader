...
//! collector    = "binance_klines"   # must name a `kline_source::KLINE_SOURCES` row
...
//! disagree. The venue always comes from [`crate::kline_source::KlineSource::venue`].
...
use crate::kline_source::{KLINE_KIND, collector_names, source_by_collector_name};
...
pub struct SourceConfig {
    ...
    pub name: String,
    /// A [`crate::kline_source::KLINE_SOURCES`] row name (e.g. `"binance_klines"`).
    pub collector: String,
    /// The symbols to keep fresh, in the collector's own symbol vocabulary (e.g. `"BTCUSDT"`).
    pub symbols: Vec<String>,
    /// Series kind; must equal [`crate::kline_source::KLINE_KIND`] — every kline source writes
    /// bars, which is a property of the shared ingest rather than a per-venue choice (that const's
    /// own doc carries the argument, and what narrowed when the per-row `kind` field went away).
...
pub fn validate(cfg: &SupervisorConfig) -> Result<(), SupervisorError> {
    ...
    for src in &cfg.sources {
        ...
        if source_by_collector_name(&src.collector).is_none() {
            let known = collector_names();
        ...
        if src.kind != KLINE_KIND {
            return Err(SupervisorError::Invalid(format!(
                "source {:?}: collector {:?} produces kind {:?}, not {:?}",
                src.name, src.collector, KLINE_KIND, src.kind
            )));
        }
        ...
        // The same question `crate::klines::ingest_klines` and `vike-datahub`'s `backfill_verb`
        // ask, spelled as the one predicate that carries the argument
        // (`vike_model::time::measures_bar_step`) rather than as a third `interval_ms(..).is_none()`.
        // Refused HERE as well as at the seam, deliberately: a roster is refused at PARSE time, so
        // a supervisor never starts holding a source it could only ever fail.
        if !vike_model::time::measures_bar_step(&src.interval) {
            return Err(SupervisorError::Invalid(format!(
