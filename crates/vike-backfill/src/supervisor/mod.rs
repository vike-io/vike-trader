...
//!   the matching one-shot bin calls ([`crate::kline_source`]), so idempotency (the `{venue}:{symbol}:
//!   {interval}:{start}-{end}` commit key), paging, and the still-forming-candle guard are inherited
//!   verbatim. Adding a collector is one [`crate::kline_source::KlineSource`] impl plus a config
//!   line.
...
//! - [`run`] — the only impure part: the `Worker` loop that glues the three together over a store.
//!
//! ⚠ **There is no `registry` submodule any more, and its absence is the point.** It held
//! `COLLECTORS`, a table of six rows naming the `backfill_*` entry points — and
//! `crates/vike-datahub/src/backfill.rs` held a second, independent table naming the same six, in a
//! crate that cannot see this one.
//! `docs/decisions/0059-bars-and-ticks-for-every-venue-are-two-asks-not-one.md`'s Phase 3
//! collapsed both into [`crate::kline_source::KLINE_SOURCES`], which this module now BINDS
//! against ([`config::validate`] at parse time, `run`'s `resolve_collectors` at startup) rather
//! than duplicating. The contract above is unchanged: the supervisor still reimplements no
//! collector, and a source's `collector = "..."` still names a registry row.
...
pub use config::{
    SourceConfig, SupervisorConfig, load_supervisor_config, parse_supervisor_config, validate,
};
pub use heal::{
    HEAL_CHUNK_MS, HealJob, JobReason, chunked_gaps, fresh_window, plan_heal_jobs, plan_pass,
};
pub use run::{CollectorSupervisor, run_once};
pub use schedule::{effective_interval_ms, is_due, is_parked, next_run_ms};
pub use status::{SourceStatus, SupervisorStatus, write_status};
