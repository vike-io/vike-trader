...
//! [`super::schedule`], [`super::heal`], [`crate::kline_source`] and [`super::status`] together
//! over a real `DataFusionHist`.
//!
...
use crate::kline_source::{KlineSource, backfill_kline_source, source_by_collector_name};
...
fn resolve_collectors(
    cfg: &SupervisorConfig,
) -> Result<Vec<&'static dyn KlineSource>, SupervisorError> {
    cfg.sources
        .iter()
        .map(|s| {
            source_by_collector_name(&s.collector).ok_or_else(|| {
...
fn run_loop(
    ...
    store: &DataFusionHist,
    cfg: &SupervisorConfig,
    collectors: &[&'static dyn KlineSource],
    status_path: Option<&Path>,
    tick: Duration,
) {
    let mut states: Vec<SourceState> = cfg.sources.iter().map(|_| SourceState::default()).collect();
...
/// [`KlineSource`]. Errors are collected onto the state, not propagated — this function cannot
/// fail.
///
...
fn run_source(
    ...
    for symbol in &src.symbols {
        ...
        for job in &jobs {
            // THE one ingest — `crate::kline_source::backfill_kline_source`, the same call the
            // one-shot `<venue>_backfill` bin and the datahub's wire verb make. It was
            // `collector.backfill`, a bare `fn` pointer off a row this module owned, until 0059
            // Phase 3 collapsed that table into the crate's one registry.
            match backfill_kline_source(
                store,
                collector,
                job.symbol.as_str(),
                job.interval.as_str(),
...
fn build_status(
    ...
    let sources = cfg
        ...
        .map(|((src, collector), st)| {
            ...
            SourceStatus {
                name: src.name.clone(),
                collector: collector.collector_name().to_string(),
...
"#;
    ...
    fn resolve_collectors_binds_each_declared_source() {
        ...
        assert_eq!(collectors[0].collector_name(), "binance_klines");
        assert_eq!(collectors[0].venue(), "binance");
    }

    /// **THE SUPERVISOR HALF OF 0059 PHASE 3'S BYTE-IDENTITY PROOF** — the twin of
    /// `crates/vike-datahub/tests/backfill_table_equivalence.rs`, on the mirror precedent
    /// (`crates/vike-config/tests/mirror.rs`'s `a_store_with_no_adoption_row_changes_no_effective_value`): resolve a
    /// roster the old way and the new way over the same fixture and demand equality.
    ///
    /// [`PRE_COLLAPSE_ROWS`] is the deleted `supervisor/registry.rs`'s `COLLECTORS` table, frozen —
    /// every `(name, venue, kind)` triple exactly as those six `Collector` rows declared them,
    /// including their order. The resolver now binds `&dyn KlineSource` out of
    /// `crate::kline_source::KLINE_SOURCES` instead, and this demands the binding be the same
    /// triple, source by source.
    ///
    /// ⚠ The `kind` column is the one field the collapse NARROWED: it was a per-row `&'static str`
    /// and is now the `KLINE_KIND` constant, because a kline source's kind is a property of the
    /// shared ingest (`append_bars`) rather than a per-venue choice. It is checked against the
    /// frozen `"bar"` LITERAL below and never against that constant — comparing the constant to
    /// itself would pass however it changed. The check runs through the REAL validator twice: the
    /// roster below omits `kind =` (so the config default must still be admitted), and the tail of
    /// this test hands the validator a non-bar kind and demands it be refused BY NAME.
    ///
    /// ⚠ Do not "simplify" the baseline into a fold over the registry: it is the independent
    /// second construction. A seventh source reddens this test, which is the intended bite — add
    /// its frozen triple then.
    #[test]
    fn the_registry_binds_the_rows_the_deleted_collectors_table_declared() {
        const PRE_COLLAPSE_ROWS: [(&str, &str, &str); 6] = [
            ("binance_klines", "binance", "bar"),
            ("bybit_klines", "bybit", "bar"),
            ("okx_klines", "okx", "bar"),
            ("aster_klines", "aster", "bar"),
            ("deribit_klines", "deribit", "bar"),
            ("hyperliquid_klines", "hyperliquid", "bar"),
        ];

        // One `[[source]]` per frozen row, named by the very `collector =` an operator's TOML
        // spells — so this resolves through `validate`'s own lookup, not through the static.
        let mut toml = String::new();
        for (collector, ..) in PRE_COLLAPSE_ROWS {
            toml.push_str(&format!(
                "\n[[source]]\nname = \"src-{collector}\"\ncollector = \"{collector}\"\n\
                 symbols = [\"SYM\"]\ninterval = \"1m\"\n"
            ));
        }
        let cfg = parse_supervisor_config(&toml).expect("every frozen collector name still parses");
        let bound = resolve_collectors(&cfg).expect("every frozen collector name still resolves");

        let now: Vec<(&str, &str, &str)> = bound
            .iter()
            .zip(cfg.sources.iter())
            .map(|(s, src)| (s.collector_name(), s.venue(), src.kind.as_str()))
            .collect();
        assert_eq!(
            now,
            PRE_COLLAPSE_ROWS.to_vec(),
            "the registry binds a different (name, venue, kind) than the deleted COLLECTORS table \
             did — 0059 Phase 3 was a refactor, so a difference here is a behaviour change that \
             needs its own argument"
        );

        // ...and the registry declares exactly those six and no seventh, in that order. The
        // resolver above can only report what a roster NAMED; this is the other half.
        assert_eq!(
            crate::kline_source::collector_names(),
            PRE_COLLAPSE_ROWS.iter().map(|(n, ..)| *n).collect::<Vec<_>>()
        );

        // The kind column, checked against the frozen literal through the real validator: a
        // roster asking a kline collector for anything but `"bar"` is refused, and the refusal
        // still names the kind the collector actually produces.
        let wrong_kind = parse_supervisor_config(
            "\n[[source]]\nname = \"s\"\ncollector = \"binance_klines\"\nsymbols = [\"SYM\"]\n\
             kind = \"quote\"\ninterval = \"1m\"\n",
        )
        .expect_err("a kline collector may not be asked for a non-bar kind");
        let rendered = wrong_kind.to_string();
        assert!(rendered.contains("produces kind \"bar\""), "{rendered}");
        assert!(rendered.contains("not \"quote\""), "{rendered}");
    }
