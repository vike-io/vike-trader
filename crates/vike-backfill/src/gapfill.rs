...
mod tests {
    ...
    ///
    /// ⚠ This asserted TWO plans (`archive + clickhouse`) until 2026-09-20, when `Source::ClickHouse`
    /// was deleted along with every producer that could serve it — see
    /// `crates/vike-backfill/src/caps.rs`'s `Source`. One plan is the honest answer now: the second
    /// one named a fetch nothing in this workspace could perform.
    #[test]
    fn plan_all_sources_ranks_by_how_much_each_can_do() {
        ...
        assert_eq!(plans.len(), 1, "archive alone; never the venue");
        assert_eq!(plans[0].source, Source::Archive);
        assert!(plans.iter().all(|p| p.source != Source::Venue));
        assert!(plans[0].fetch_count() > 0);
    }
}
