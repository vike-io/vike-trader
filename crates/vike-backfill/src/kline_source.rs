...
//! * The FOURTH roster — `FETCHABLE_VENUES` in the compute plane's own fetch — was a DIFFERENT
//!   PRODUCER behind a wall the layer gate enforces: `vike-backtest` is layer 50 and
//!   `vike-backfill` is 55, and the edge already runs 55 → 50 (`vike-backfill`'s optional
//!   `vike-backtest` dep, since deleted), so a normal edge the other way was both a layer
//!   inversion AND a cycle. It could not name this trait at any price.
//!
//!   ⚠ **It was retired on 2026-09-19 by DELETING THE PRODUCER**, not by reconciling it: the verb
//!   it served (`backtest data fetch`) called a venue directly from the compute plane, and
//!   `vike-cli data fetch` asks a datahub instead — which dispatches through THIS registry. So the
//!   fourth roster is gone and the wall no longer has anything behind it.
//!   `crates/vike-ops/tests/collector_dispatch_gate.rs`'s now-empty `ROSTERS_NOT_GATED` carries the
//!   argument, and `crates/vike-ops/tests/citation_gate.rs`'s `DEAD_PATH_EXCEPTIONS` carries the
//!   path.
//!
