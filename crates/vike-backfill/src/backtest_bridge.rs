    ...
    for i in 0..b.num_rows() {
        ...
        let (row_bids, row_asks) = match kind {
            ...
            BookUpdateKind::Delta => match side.value(i) {
                "buy" => {
                    (vec![BookLevel { price: price.value(i), qty: size.value(i) }], Vec::new())
                }
                "sell" => {
                    (Vec::new(), vec![BookLevel { price: price.value(i), qty: size.value(i) }])
                }
...
mod tests {
    ...
    fn delta_row_decodes_the_populated_side() {
        ...
        assert_eq!(out[0].bids, vec![BookLevel::new(0.42, 7.0)]);
        assert!(out[0].asks.is_empty());
        assert_eq!(out[1].kind, BookUpdateKind::Delta);
        assert!(out[1].bids.is_empty());
        assert_eq!(out[1].asks, vec![BookLevel::new(0.60, 0.0)]);
    }
    ...
    /// every grid point still travels the whole `run_paramscan` -> `run_backtest` -> `replay_ticks` ->
...
"#,
        ...
        assert!(profile.is_paramscan(), "the fixture must exercise the sweep path");

        let report = harness::run_paramscan(&profile, store, RankMetric::Sharpe)
