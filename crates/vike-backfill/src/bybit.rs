...
/// half, store-free. [`backfill_bybit_klines`] above, the supervisor's pass and the datahub's wire
/// verb all dispatch through it, so the three cannot name different fetchers.
pub struct BybitKlines;

impl crate::kline_source::KlineSource for BybitKlines {
    ...
    /// The bridge's own paged `fetch_klines_range` verbatim.
    fn fetch(
        &self,
        symbol: &str,
        interval: &str,
