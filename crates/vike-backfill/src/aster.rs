...
/// half, store-free. [`backfill_aster_klines`] above, the supervisor's pass and the datahub's wire
/// verb all dispatch through it, so the three cannot name different fetchers.
pub struct AsterKlines;

impl crate::kline_source::KlineSource for AsterKlines {
    ...
    /// The bridge's own paged `fetch_klines_range` verbatim, on `Environment::Live` — this venue is
    /// MAINNET-only for market data and the `.P` perp routing happens inside the bridge.
    fn fetch(
        &self,
        symbol: &str,
        interval: &str,
        ...
        fetch_klines_range(symbol, interval, start_ms, end_ms, Environment::Live)
