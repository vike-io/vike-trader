...
use crate::csvutil::{Header, f64_of, header_and_rows, i64_of, split_line, ts_ms as csv_ts_ms};
...
mod tests {
    ...
    fn mbp10_maps_depth_snapshot_skipping_empty_levels() {
        ...
        assert_eq!(
            b[0].bids,
            vec![BookLevel::new(4199.0, 3.0), BookLevel::new(4198.0, 4.0)],
            "only populated levels, price-scaled"
        );
        assert_eq!(b[0].asks, vec![BookLevel::new(4201.0, 5.0), BookLevel::new(4202.0, 6.0)]);
        assert_eq!(b[0].seq, 0);
        assert_eq!(b[0].ts, 1_700_000_000_000);
