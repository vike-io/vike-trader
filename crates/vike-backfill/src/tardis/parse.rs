...
pub fn parse_book_l2(csv: &str) -> Vec<BookUpdate> {
    ...
    for line in rows {
        ...
        let level = BookLevel { price, qty: amount };
        let (bids, asks) =
...
mod tests {
    ...
    fn book_l2_snapshot_delta_side_and_removal() {
        ...
        assert_eq!(b[0].bids, vec![BookLevel::new(34999.0, 10.0)]);
        assert_eq!(b[0].asks, vec![]);
        assert_eq!(b[0].seq, 0);
        assert_eq!(b[1].kind, BookUpdateKind::Delta);
        assert_eq!(
            b[1].asks,
            vec![BookLevel::new(35001.0, 0.0)],
            "amount 0 = level removed (kept as size 0)"
        );
