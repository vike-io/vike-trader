...
/// half, store-free. [`backfill_deribit_klines`] above, the supervisor's pass and the datahub's wire
/// verb all dispatch through it, so the three cannot name different fetchers.
pub struct DeribitKlines;

impl crate::kline_source::KlineSource for DeribitKlines {
    ...
    /// The bridge's own paged `fetch_klines_range` verbatim — `symbol` is a Deribit instrument name
    /// (`BTC-PERPETUAL`) and is passed through untouched, dated futures and option names included.
    fn fetch(
        &self,
        symbol: &str,
        interval: &str,
