...
//! account's keyless `userFunding` history and persist it into the `kind=exec_funding` Tier-2 series
...
// --- realized perp funding backfill (kind=exec_funding) ----------------------------------------
//
...
// `kind=exec_funding` Tier-2 store series (`append_funding`), split by coin, idempotent per window.
...
///
/// ⚠ `account` is STAMPED ON EVERY ROW, and it is the same value the caller was already going to
/// spend on [`funding_commit_key`] — this function takes it rather than inventing it. Per
/// `docs/decisions/0080-the-account-funding-kind-takes-the-qualified-name.md` verdict 5: the series partition is `(venue, coin)` and carries no account,
/// so without this stamp two accounts funding one coin produce rows nothing can attribute (`hash` is
/// the venue TRANSACTION hash and names no account). On this venue the value is the wallet address.
pub fn funding_rows_by_coin(
    account: &str,
    payments: Vec<FundingPayment>,
) -> Vec<(String, Vec<FundingRow>)> {
...
pub fn funding_commit_key(account: &str, coin: &str, start_ms: i64, end_ms: i64) -> String {
    format!("{VENUE}:exec_funding:{account}:{coin}:{start_ms}-{end_ms}")
}
...
pub fn backfill_hyperliquid_funding(
    ...
    for (coin, rows) in funding_rows_by_coin(account, payments) {
...
mod tests {
    ...
    fn groups_by_coin_dedups_hash_and_sorts_ts() {
        ...
        let out = funding_rows_by_coin("0xMAIN", payments);
    ...
    fn empty_fetch_groups_to_nothing() {
        assert!(funding_rows_by_coin("0xMAIN", Vec::new()).is_empty());
    }
    ...
    fn commit_key_has_account_coin_and_window() {
        assert_eq!(
            ...
            "hyperliquid:exec_funding:0xabc:BTC:1000-2000"
        );
    }
