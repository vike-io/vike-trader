...
use vike_data::archive_store::{VENUE, select_row_groups};
...
// ⚠ ROW-GROUP PRUNING stood here and moved DOWN with the reader on 2026-09-20:
// `vike_data::archive_store::select_row_groups` is its home, and this module imports it from there
// (see the `use` at the top). It is a read-side fact about the published files — which groups a
// token set can possibly be in — not a fact about fetching, and holding it HERE is what made the
// reader depend upward.
...
/// was the resting maker (`is_buyer_maker = true`), the same taker-side convention the deleted
/// `backtest_bridge::trades_from_batch` inverted for the live ClickHouse tape — where it was first
/// pinned, and which `crate::events_api::trade_from_row` also matches.
