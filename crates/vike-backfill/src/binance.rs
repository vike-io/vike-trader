...
/// half, store-free. [`backfill_binance_klines`] above, the supervisor's pass and the datahub's
/// wire verb all dispatch through it, so the three cannot name different fetchers.
pub struct BinanceKlines;

impl crate::kline_source::KlineSource for BinanceKlines {
    ...
    /// The bridge's own paged `fetch_klines_range` verbatim — spot/perp routing (and the `.P`
    /// suffix strip) happens inside it, never here.
    fn fetch(
        &self,
        symbol: &str,
        interval: &str,
