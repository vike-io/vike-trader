...
pub fn backfill_okx_klines(
    ...
    crate::kline_source::backfill_kline_source(hist, &OkxKlines, symbol, interval, start_ms, end_ms)
}

/// This venue's row in the ONE kline registry (`crate::kline_source::KLINE_SOURCES`) — the fetch
/// half, store-free. [`backfill_okx_klines`] above, the supervisor's pass and the datahub's wire
/// verb all dispatch through it, so the three cannot name different fetchers.
pub struct OkxKlines;

impl crate::kline_source::KlineSource for OkxKlines {
    ...
    /// The bridge's own paged `fetch_klines_range` verbatim — `history-candles`, closed-candle-only,
    /// which is why the shared ingest's still-forming guard is a harmless no-op on this venue.
    fn fetch(
        &self,
        symbol: &str,
        interval: &str,
        ...
        fetch_klines_range(symbol, interval, start_ms, end_ms).map_err(CollectError::Fetch)
    }
}
