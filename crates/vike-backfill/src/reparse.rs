...
use vike_polymarket::MarketStream;
    ...
    fn read_frame(&mut self) -> Result<String, StreamError> {
        loop {
            if self.cur.is_none() && !self.open_next() {
                return Err(StreamError::Closed("eof".into()));
            }
...
mod tests {
    ...
    fn rewrites_local_ts_from_cell_on_quote_trade_book() {
        ...
        sink.book_update(
            ...
            BookUpdate {
                ...
                asks: vec![BookLevel::new(0.5, 1.0)],
                symbol: String::new(),
            },
        );
    ...
    fn reads_lines_in_order_strips_prefix_updates_cell_then_closed() {
        ...
        assert!(matches!(s.read_frame(), Err(StreamError::Closed(_))), "Closed at EOF");
    ...
    fn spans_multiple_day_files_in_order() {
        ...
        assert!(matches!(s.read_frame(), Err(StreamError::Closed(_))));
    }
}
