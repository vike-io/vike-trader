...
pub fn parse_levels(json: &str) -> Vec<BookLevel> {
...
pub fn map_row(state: &mut MapState, row: &PmxtRow) -> Mapped {
    ...
    match row.event_type.as_str() {
        ...
        "price_change" => {
            ...
            let level = BookLevel { price, qty: size };
            let (bids, asks) =
...
mod tests {
    ...
    fn parse_levels_decodes_json_pairs() {
        assert_eq!(
            ...
            vec![BookLevel::new(0.51, 10.0), BookLevel::new(0.50, 5.0)]
        );
    ...
    fn book_row_maps_to_snapshot() {
        ...
        assert_eq!(b.bids, vec![BookLevel::new(0.5, 10.0)]);
        assert_eq!(b.asks, vec![BookLevel::new(0.51, 8.0)]);
        assert_eq!(b.ts, 1000);
    ...
    fn price_change_maps_to_side_correct_delta() {
        ...
        assert_eq!(b.bids, vec![BookLevel::new(0.42, 7.0)]);
        ...
        assert_eq!(s.asks, vec![BookLevel::new(0.60, 0.0)], "SELL → asks; size 0 = level removed");
