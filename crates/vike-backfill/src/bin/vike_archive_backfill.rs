...
use vike_backfill::vike_archive::{
    ArchiveClient, DEFAULT_BASE, FamilyTarget, Stream, default_discovery_base,
...
// ⚠ `VENUE` moved DOWN to the reader on 2026-09-20 and is named at its canonical path, not through
// a re-export in `vike_archive` — a moved symbol has ONE name.
use vike_data::archive_store::VENUE;
...
///
/// One name. A `vike_archive_backfill` run used to resolve the WHOLE credential store into memory
/// (67 rows on the live box) to read this single `X-API-Key`; it now holds this row and no other.
/// A name outside this array answers `vike_secrets::Lookup::NotDeclared`, which is NOT the live
/// gate — see the refusal arm in `main`.
...
///
/// `Ok(None)` is the credential genuinely not being in the store — the keyless manifest/sample
/// path still works and `main` warns. `Err` is the name having fallen out of [`SCOPE`], which is a
/// defect in this binary rather than a fact about the box; an `Option` return would have made the
/// two indistinguishable, which is the failure `vike_secrets::Lookup` exists to prevent.
///
/// # Errors
