...
use vike_backfill::hyperliquid::{VENUE, backfill_hyperliquid_klines};
...
mod tests {
    ...
    /// **PINNED DISAGREEMENT — this bin's interval vocabulary is NARROWER than every dispatch
    /// path's, in both directions, and neither side is wrong.**
    ///
    /// * `4h`, `15m` and `30m` MEASURE (`vike_model::time::measures_bar_step`), so the hist store
    ///   can hold them, `vike_backfill::supervisor::config::validate` accepts a roster naming them
    ///   and `vike-datahub`'s `backfill_verb` serves them — and `HyperliquidKlines::fetch` passes
    ///   the interval straight to the venue. This bin refuses all three, because [`INTERVALS`] is
    ///   an allow-list of the HL `candleSnapshot` codes its author verified, and HL answers an
    ///   unknown code with an EMPTY page that reads downstream as "no history" rather than as a bad
    ///   flag.
    /// * `1w`, `1M` and `1mo` do NOT measure, so the store refuses them at the collector seam
    ///   (`docs/decisions/0059-bars-and-ticks-for-every-venue-are-two-asks-not-one.md`'s bug B).
    ///   This bin refuses them too — for the unrelated reason above, and that coincidence is
    ///   exactly why the narrower list must not be read as this bin's bug-B guard.
    ///
    /// Nothing here is resolved. Widening [`INTERVALS`] to every measurable step trades a named
    /// refusal for a silent empty page; narrowing the dispatch paths to this list would refuse
    /// steps other venues genuinely serve. Both are changes with their own argument.
    #[test]
    fn the_bins_interval_vocabulary_is_narrower_than_the_stores() {
        for iv in INTERVALS {
            assert!(
                vike_model::time::measures_bar_step(iv),
                "{iv} is on this bin's allow-list but the store cannot measure it — the two lists \
                 have drifted into a step that would be refused one layer down"
            );
        }
        for iv in ["4h", "15m", "30m"] {
            assert!(
                vike_model::time::measures_bar_step(iv),
                "{iv} no longer measures — re-pick the example, the pin is about a step the store \
                 accepts"
            );
            ...
            .expect_err("the bin's allow-list refuses a step the dispatch paths accept");
            assert!(e.contains(iv), "the refusal names the value: {e}");
        }
        for iv in ["1w", "1M", "1mo"] {
            assert!(!vike_model::time::measures_bar_step(iv), "{iv} must not measure");
            assert!(
                ...
                .is_err(),
                "{iv} reaches the bin's own refusal first — but it is the allow-list refusing, not \
                 a forming-bar guard"
            );
        }
    }
