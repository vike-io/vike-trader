...
//! ingests it as `kind=exec_funding`/`venue=hyperliquid`/`symbol=<coin>` into the vike-data DataFusion
...
use vike_backfill::cli::{CliSpec, flag_value, log_config, store_root};
use vike_backfill::hyperliquid::{VENUE, backfill_hyperliquid_funding};
