...
fn main() -> ExitCode {
    ...
    if let (Some(min), Some(max)) = (min_local_ts, max_local_ts)
        && min > max
    {
        tracing::error!("--min-local-ts {min} is after --max-local-ts {max} — nothing would match");
        return ExitCode::FAILURE;
    }
    ...
    let target = if clickhouse {
        format!(
            "clickhouse via {ch_bin} (polymarket.book_events / polymarket.l1_quotes, NOT idempotent — see module doc)"
        )
    } else {
