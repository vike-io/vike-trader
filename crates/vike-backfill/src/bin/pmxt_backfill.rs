...
//!
//! # ⚠⚠ OBSOLETE — RUNNING THIS FETCHES 404s AND EXITS 0
//!
//! **The pmxt archive stopped publishing on 2026-08-10; the owner ruled the source obsolete on
//! 2026-09-21.** `crates/vike-backfill/src/pmxt/mod.rs`'s module doc carries the measurement — do
//! not re-derive it here.
//!
//! What that means for THIS binary specifically: the 404-is-not-fatal rule three paragraphs down
//! was written for an archive that publishes on a lag, and it is now the reason a run over any
//! range after 2026-08-10T00 **logs a skip per hour and exits successfully having ingested
//! nothing**. A `--from`/`--to` inside the published range still works and still backfills real
//! history; anything past it is a no-op that looks like a success.
//!
