...
use vike_backfill::vikedata::{
    ...
    panel_funding_commit_key, scheduled_windows, Axis, CohortWindow, Grading, Ladder,
    DEFAULT_CATCH_UP, MAX_CATCH_UP, POLLED, VIKEDATA_BASE,
};
...
mod tests {
    ...
    fn the_two_run_shapes_are_never_mixed_or_guessed_at() {
        ...
        assert!(resolve_window(None, Some("2025-08-01"), None, ANCHOR)
            .unwrap_err()
            .contains("--end"));
        assert!(resolve_window(None, None, Some("2025-08-08"), ANCHOR)
            .unwrap_err()
            .contains("--start"));
    ...
    fn the_look_back_belongs_to_the_scheduled_shape_and_is_bounded() {
        ...
        assert!(resolve_plan(&scheduled(Some("many")), ANCHOR)
            .unwrap_err()
            .contains("not a whole number"));
        let over = MAX_CATCH_UP + 1;
        assert!(resolve_plan(&scheduled(Some(&over.to_string())), ANCHOR)
            .unwrap_err()
