...
use vike_backfill::vikedata::{
    ...
    backfill_cohort, fetch_panel, panel_commit_key, scheduled_windows,
};
...
///
/// One name, and it is [`API_KEY_ENV`] itself rather than a second spelling of it: the scope and
/// the lookup then cannot drift into disagreement by an edit to either one alone.
const SCOPE: [&str; 1] = [API_KEY_ENV];

/// The cohort API key from the SCOPED credential read — one name materialised, not the store.
///
/// # ⚠ `Ok(None)` and `Err` are different events and this bin acts differently on them
///
/// `Ok(None)` is the credential genuinely not being in the store — the live gate, and this bin's
/// documented refusal. `Err` is [`API_KEY_ENV`] having fallen out of [`SCOPE`], which is a defect
/// in this binary rather than a fact about the box; an `Option` return would have made the two
/// indistinguishable, which is the failure `vike_secrets::Lookup` exists to prevent.
///
/// `env` is the one `std::env::vars()` sweep each caller already owns, so this performs no sweep
/// of its own.
///
/// # Errors
...
fn resolve_api_key(
    ...
    api_key(&load_workspace_secrets_scoped_from_env(env, &KeyScope::of(SCOPE)))
}

/// The pure half of [`resolve_api_key`] — the lookup, with the read already performed. Split out so
/// the SCOPE can be exercised against the real lookup without a store on disk.
///
/// # Errors
/// [`vike_bridge_core::credentials::UndeclaredKey`] when the name asked for is outside [`SCOPE`].
fn api_key(
    ...
    Ok(store.get(API_KEY_ENV).declared()?.map(str::to_string))
}
...
fn run_panel(
    ...
    let api_key = match resolve_api_key(&env) {
        Ok(Some(key)) => key,
        Ok(None) => {
            eprintln!(
                "{API_KEY_ENV} is not in the credential store (<project>/settings/secrets.env), \
                 so this run is REFUSED. An unauthenticated fetch 403s part-way through the \
                 window and would store a partial batch under a commit key claiming the whole one."
            );
