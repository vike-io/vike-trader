...
//! ```
//!
//! SWEEP profiles run here too: a `[sweep]` table dispatches to [`harness::run_paramscan`] (ranked by
//! `--rank-by`, default `sharpe`) instead of one `run_backtest`, printing the same ranked
//! `ParamscanReport` table/JSON the sibling `backtest` bin prints. This used to be an explicit error,
//! on the premise that the sweep path was keyed to the concrete `DataFusionHist` and had not been
//! relaxed to the trait object. That premise is stale: `run_paramscan`/`run_paramscan_exec`/`run_paramscan_with`
...
//! `RankMetric` names are wired; the `multi` objective, `--optimizer euler` and `--optimizer tpe` stay
...
fn main() -> ExitCode {
    ...
    // A `[sweep]` table runs the whole cartesian grid through `harness::run_paramscan` over this SAME
    ...
    if profile.is_paramscan() {
        ...
        let sweep_report = match harness::run_paramscan(&profile, store, rank_by) {
            Ok(r) => r,
            Err(e) => {
