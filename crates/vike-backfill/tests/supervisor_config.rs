...
use vike_backfill::kline_source::{KLINE_KIND, source_by_collector_name};
use vike_backfill::supervisor::{
    JobReason, SupervisorConfig, effective_interval_ms, is_due, is_parked, load_supervisor_config,
    next_run_ms, plan_pass,
};
...
fn the_shipped_example_roster_parses_and_names_only_registered_collectors() {
    ...
    for src in &cfg.sources {
        let c = source_by_collector_name(&src.collector).unwrap_or_else(|| {
            panic!("example names an unregistered collector {:?}", src.collector)
        });
        assert_eq!(src.kind, KLINE_KIND);
        assert_eq!(c.collector_name(), src.collector);
