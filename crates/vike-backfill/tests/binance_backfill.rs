...
//! Offline gate for the Binance klines → hist-store backfill (slice 6, crypto half). No network: a
//! SMALL committed klines JSON fixture (`fixtures/binance_klines_btcusdt_1m.json`) is mapped by the
//! extracted `vike_binance::data::parse_klines` and asserted bit-exact (`to_bits` on o/h/l/c/v,
//! `ts` exact). Then the mapped bars are `append_bars`'d into a `tempdir` `DataFusionHist` and
//! `load_bars` must round-trip them bit-for-bit, plus commit-key idempotency (same key twice → 0).
//! The one REAL network fetch is `#[ignore]`d — this file is deterministic + offline in CI.

use vike_backfill::binance::{VENUE, backfill_binance_klines, klines_commit_key};
...
fn commit_key_is_stable_and_windowed() {
    assert_eq!(klines_commit_key("BTCUSDT", "1m", 0, 179_999), "binance:BTCUSDT:1m:0-179999");
}

/// LIVE: hits Binance's public REST endpoint. Run explicitly:
/// `cargo test -p vike-backfill --test binance_backfill -- --ignored --nocapture live_`
#[test]
#[ignore = "live: hits Binance's public REST klines endpoint"]
fn live_backfill_btcusdt_1m() {
    let dir = tempfile::tempdir().unwrap();
    let hist = DataFusionHist::open(dir.path()).unwrap();
    // 2024-01-02 00:00–02:00 UTC — >60 one-minute bars, so paging past the 1000-cap is exercised
    // only lightly here; the assertion is on a contiguous, non-empty window.
    let start = 1_704_153_600_000i64; // 2024-01-02T00:00:00Z
    let end = start + 2 * 3_600_000 - 60_000; // last 1m bar opens at 01:59

    let n = backfill_binance_klines(&hist, "BTCUSDT", "1m", start, end).expect("backfill");
    eprintln!("backfilled {n} BTCUSDT 1m bars");
    assert!(n > 0, "a liquid window should yield bars");

    // idempotent second run
    let again = backfill_binance_klines(&hist, "BTCUSDT", "1m", start, end).expect("backfill#2");
    assert_eq!(again, 0, "same window is a no-op");

    let got = hist.load_bars("binance", "BTCUSDT", "1m", TsRange::all()).unwrap();
    assert_eq!(got.len(), n, "loaded count matches appended count");
    assert!(got.windows(2).all(|w| w[0].ts < w[1].ts), "strictly ascending, no dup openTimes");
}
