...
//! same failure is available here: several of these bins open a store or start paging an HTTP
//! archive. A regression must fail in 30 s with a diagnosis, not park a CI job. (It read "spawn
//! `clickhouse-client`" until 2026-09-20; no bin in this crate spawns any subprocess now, which
//! narrows what this deadline can catch without removing the reason for it.)
//!
...
// ⚠ `POLY_CH_BINS` STOOD HERE and is deleted with both bins it named (2026-09-20):
// `poly_ch_backtest` went with its cause — the engine opens an archive day itself now — and
// `poly_mm_batch` moved to the `vike-poly-research` crate.
