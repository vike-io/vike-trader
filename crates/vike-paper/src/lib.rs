...
use vike_model::{
    combo_net_cross, order_request_to_working, Bar, ComboLeg, OrderKind, OrderRequest, TimeInForce,
    WorkingOrder,
};
        ...
        match &self.fee_schedule {
            Some(sched) => {
                ...
                if matches!(sched, FeeSchedule::PercentOfUnderlying { .. }) {
                    if let Some(src) = &self.underlying_source {
                        if let Some(underlying) = src(&self.venue, symbol, ts) {
    ...
    fn fill_combos(&mut self, ts: i64) {
        ...
        for (coid, combo) in std::mem::take(&mut self.combos) {
            ...
            if let Some(limit) = combo.net_limit {
                if combo_net_cross(combo.side, limit, &quotes).is_none() {
...
impl ExecutionClient for PaperExecutionClient {
    ...
    /// the copy this mirrors, down to the silence: non-terminal, nothing synthesized, the resting
    /// order keeps its terms — the `ExecutionClient::modify` contract for a command that could not
    /// be delivered. That silence is a KNOWN gap on all three clients
    /// (`docs/ops/kill-switches.md` section 5), and this crate is the wrong place to fix it alone:
    /// the whole point of the arm is that paper and live agree.
...
mod tif_expiry_tests {
    ...
    use vike_model::{utc_day, MS_PER_DAY};
...
mod bracket_tests {
    use super::*;
    use vike_model::{build_bracket, BracketSpec};
...
mod combo_tests {
    use super::*;
    use vike_model::{build_combo, ComboSpec};
