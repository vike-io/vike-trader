...
fn kinds(events: &[Event]) -> Vec<&'static str> {
    events
        ...
        .map(|e| match e {
            ...
            Event::OrderModifyRejected(_) => "modify-rejected",
            _ => "other",
        })
        .collect()
}
...
/// mirrors. `crates/vike-mount/tests/halt_paper_live_agree.rs` is the cross-client half; this is
/// the local one.
///
/// ⚠ **The refusal must now be AUDIBLE, and this test asserts both halves.** It used to require
/// `drain(...).is_empty()` — the SILENCE that made a halted amend byte-indistinguishable from one
/// the adapter had lost (`docs/ops/kill-switches.md` gap 5). Exactly one NON-terminal
/// `OrderModifyRejected` is now owed, and its non-terminality is what keeps the verdict unchanged:
/// the resting order stays MODIFIABLE and at its old terms.
///
/// ⚠ Mutation proof: delete the `halt_engaged()` arm from `PaperExecutionClient::modify` and this
/// goes red on the `OrderModified` it then books; leave the arm and delete only its `push_back` and
/// it goes red on the missing advisory.
#[test]
fn an_armed_paper_mount_refuses_a_modify_out_loud_while_the_sentinel_exists() {
    ...
    let refused = drain(&mut client);
    assert_eq!(
        kinds(&refused),
        vec!["modify-rejected"],
        "a modify under HALT must be blocked AND said out loud — it can add size or chase price, \
         the exit path under a halt is CANCEL, and a blocked amend that emits nothing is \
         indistinguishable from a lost one. Got: {refused:?}"
    );
    match refused.as_slice() {
        [Event::OrderModifyRejected(m)] => {
            assert_eq!(m.client_order_id, "resting");
            assert_eq!(
                m.reason,
                vike_exec::halt::HALT_REJECT_REASON,
                "the SHARED wording — one recogniser must match all four enforcing clients"
            );
        }
        other => panic!("expected exactly one OrderModifyRejected, got {other:?}"),
    }
...
fn a_modify_is_blocked_under_halt_even_for_an_order_that_was_reduce_only() {
    ...
    assert_eq!(
        ...
        "`reduce_only` must NOT buy an amend past the halt: the flag describes the original order, \
         and this amend raises the qty. The REFUSAL is the assertion; the advisory only says so. \
         Got: {refused:?}"
    );
}
