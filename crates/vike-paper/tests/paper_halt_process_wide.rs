...
//!
//! ⚠ **The sentinel is supplied to a CHILD process, never set in this one.** `std::env::set_var` is
//! an `unsafe fn` since edition 2024 and this workspace forbids unsafe code outright, so the shape
//! that used to justify "a separate binary containing exactly ONE test" — mutate the environment
//! where no other thread can read it — is no longer available at all. The test re-executes ITS OWN
//! binary with `Command::env(VIKE_HALT_FILE, …)` filtered to itself (the idiom
//! `crates/bridges/ctrader/tests/common/mod.rs` uses for the same reason) and asserts the child's
//! exit status; the child branch, which finds the variable already set by its parent, runs the real
//! assertions. The file stays separate for its documentary value and so the re-exec filter is
//! unambiguous; nothing about it needs to be one test per binary any more.
//!
...
fn an_unarmed_book_ignores_the_process_wide_sentinel_even_when_it_exists() {
    let Some(path) = std::env::var_os(HALT_FILE_ENV).map(PathBuf::from) else {
        // PARENT: no sentinel in OUR environment. Create one and re-run this exact test in a
        // CHILD whose environment names it — `Command::env` on the child, never
        // `std::env::set_var` here: that is an `unsafe fn` since edition 2024, and this workspace
        // forbids unsafe code outright.
        //
        // ⚠ `dir` is BOUND for the whole of this arm: the `TempDir` guard deletes the directory
        // when it drops, and the sentinel the child is pointed at lives inside it. `tempfile`
        // rather than a pid-named `temp_dir()` child, measured on the CI box 2026-08-25: nothing ever
        // deleted the pid-named parent, and a REUSED pid colliding with a directory the box's
        // OTHER test user made (`the CI user` for CI, `the operator` for the verification lanes) let
        // `create_dir_all` succeed while the write failed PermissionDenied — a live flake.
    ...
    // CHILD: the parent pointed `VIKE_HALT_FILE` at a sentinel it created. Prove the wiring
