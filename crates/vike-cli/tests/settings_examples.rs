...
/// A throwaway PROJECT holding `contents` in its `settings/` child, written under the REAL file
/// names. Returns the project root (bound, so its `Drop` removes the tree) and the settings
/// directory itself.
///
/// ⚠ **Two levels, and the second one is the whole point.** `vike_config::load` runs
/// `refuse_removed_project_file` as layer 0, which stats `<settings-dir>/../vike.toml` and returns
/// a HARD error on anything but `NotFound` — so the directory handed to the loader decides which
/// `vike.toml` this test's verdict depends on. A BARE `tempfile::tempdir()` aims that probe at the
/// SYSTEM TEMP ROOT: one `vike.toml` dropped there by any process on the box fails these tests for
/// every user, on every run, until somebody deletes a file no line of this test mentions — and
/// `/tmp` is sticky, so the user who did not create it cannot. Nesting the settings directory one
/// level down puts the probe inside this test's own `TempDir`, which it created and will remove.
///
/// This is the same repair `crates/vike-cli/tests/backtest_cli.rs`'s `project_with_settings` makes
/// for the same reason, and the shape `crates/vike-cli/src/cmd/config_check.rs`'s `project` helper
/// already documents for the grandparent probe one crate over.
fn settings_dir_with(files: &[(&str, String)]) -> (tempfile::TempDir, PathBuf) {
    let root = tempfile::tempdir().expect("tempdir");
    let dir = root.path().join("settings");
    std::fs::create_dir_all(&dir).expect("create the settings directory");
    for (name, body) in files {
        std::fs::write(dir.join(name), body).expect("write the settings file");
    }
    (root, dir)
}

/// **The fixture's hermeticity, proven POSITIVELY rather than by a green run.**
///
/// A green run of the tests below is NOT evidence that [`settings_dir_with`] nests: they pass with
/// a bare temp directory too, on every box where nobody has yet dropped a `/tmp/vike.toml`. So the
/// property is asserted directly, and BEHAVIOURALLY — layer 0's probe is aimed at the parent of the
/// directory it is handed, so planting the refused file inside THIS test's own root must be what
/// the loader refuses. If the fixture ever went back to one level, the plant would land beside a
/// sibling instead of on the probe's path and the load would succeed.
#[test]
fn the_fixture_aims_the_removed_file_probe_inside_this_tests_own_tempdir() {
    let (root, dir) = settings_dir_with(&[]);
    assert_eq!(
        dir.parent(),
        Some(root.path()),
        "the project the loader infers must be this test's own TempDir, not the shared system \
         temp root — see `settings_dir_with`"
    );

    // Nothing planted: the probe finds no file, and the load succeeds.
    vike_config::load(Some(dir.as_path()), &std::collections::HashMap::new())
        .expect("an empty settings directory loads");

    // Planted INSIDE this test's root: the same load must now refuse, naming the planted path.
    let planted = root.path().join("vike.toml");
    std::fs::write(&planted, "").expect("plant the refused file");
    let err = vike_config::load(Some(dir.as_path()), &std::collections::HashMap::new())
        .expect_err("a `vike.toml` beside the settings directory is refused");
    let text = err.to_string();
    assert!(
        text.contains(&planted.display().to_string()),
        "the refusal must name the file this test planted, which is how we know the probe resolved \
         inside this test's own TempDir: {text}"
    );
}
...
fn every_template_line_is_one_the_loader_accepts() {
    let files: Vec<(&str, String)> = template_files()
        ...
        .collect();
    // `_project` is BOUND: it is the TempDir whose `Drop` removes the tree, and letting it fall
    // would delete the settings directory before the loader opened it.
    let (_project, dir) = settings_dir_with(&files);
    ...
    let settings = vike_config::load(Some(dir.as_path()), &std::collections::HashMap::new());
...
fn the_templates_as_shipped_configure_nothing() {
    let files: Vec<(&str, String)> = template_files()
        ...
        .collect();
    // `_project` is BOUND — see the sibling test above.
    let (_project, dir) = settings_dir_with(&files);
    let env = std::collections::HashMap::new();

    let shipped = vike_config::load(Some(dir.as_path()), &env)
