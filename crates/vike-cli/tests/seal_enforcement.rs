...
/// **The residual the refusal above does NOT cover: a value that CHANGED at an unchanged row
/// count.** The guard compares row COUNTS against the seal, so this passes it and is written — and
/// until 2026-09-21 the command then printed *"The files still win — nothing this box resolves has
/// changed"*, which on an adopted box is false in both halves. It is the same sentence the incident
/// at `650907a37` printed; the guard that stops the LOSS landed, the sentence that misreports it
/// did not.
///
/// ⚠ The two assertions are a PAIR and neither works alone: the message must name the adoption, and
/// the value must actually have moved. Without the second, a command that silently did nothing
/// would satisfy the first.
#[test]
fn mirroring_a_changed_value_onto_an_adopted_box_says_the_rows_are_what_it_resolves() {
    let c = Case::new("changed");
    c.write("policy.toml", "max_notional_per_order = 250\nmax_leverage = 3\n");
    c.adopt();

    // SAME number of keys, one different value — so the row-count guard cannot see it.
    c.write("policy.toml", "max_notional_per_order = 100\nmax_leverage = 3\n");

    let o = c.run(&["config", "mirror"]);
    assert!(o.status.success(), "an unchanged row COUNT passes the guard: {}", both(&o));
    let text = both(&o);
    assert!(
        !text.contains("nothing this box resolves has changed"),
        "the box is ADOPTED and a value moved — this sentence is the false one: {text}"
    );
    assert!(text.contains("ADOPTED"), "the message must name why it matters here: {text}");
    assert!(
        text.contains("config compare"),
        "...and the verb that shows what moved, since the guard cannot: {text}"
    );

    // THE OTHER HALF. The value really did change, so the assertion above is about a real event.
    let o = c.run(&["config", "show"]);
    assert!(both(&o).contains("100"), "the mirrored value must be what resolves now: {}", both(&o));
}

/// ...and the control: on an UNADOPTED box the files really do keep winning, so the original
/// sentence is correct there and must survive. Without this the fix above could have been "delete
/// the reassuring sentence", which would lose a true statement to fix a false one.
#[test]
fn mirroring_an_unadopted_box_still_says_the_files_keep_winning() {
    let c = Case::new("unadopted");
    c.write("policy.toml", "max_notional_per_order = 250\n");
    // ⚠ The database has to exist before `mirror` will write, and `mirror` deliberately will not
    // create it — `Case::adopt` carries the whole argument. So this fixture takes adopt's first two
    // steps and STOPS before `config adopt`, which is exactly the state under test: rows present,
    // no seal, files still winning.
    c.write("secrets.env", "BINANCE_DEMO_API_KEY=fixture-not-a-real-key\n");
    let g = c.run(&["secrets", "migrate"]);
    assert!(g.status.success(), "the fixture's migrate must succeed: {}", both(&g));
    ...
    assert!(
        text.contains("nothing this box resolves has changed"),
        "an unadopted box resolves from the FILES; the sentence is true here: {text}"
    );
    assert!(!text.contains("ADOPTED"), "...and the adopted warning must not fire: {text}");
}
