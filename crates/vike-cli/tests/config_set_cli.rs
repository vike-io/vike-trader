...
fn the_live_arm_flag_needs_the_retyped_key_and_an_ordinary_flag_does_not() {
    ...
    assert!(out.status.success(), "{} / {}", stdout(&out), stderr(&out));
    assert!(c.read("flags.toml").contains("cancel_orders_on_shutdown"), "{}", c.read("flags.toml"));
}

/// **A `venue.*` key ROUTES TO THE TABLE, not to a settings file** — proven over the shipped binary
/// by the refusal it gives, which no file-shaped key can produce.
///
/// ⚠ The routing is the claim worth testing here. `execute` resolves a key to its settings FILE
/// before anything else, and no file can spell one of these: `vike_config::Config` carries
...
/// So a `venue.*` key that reached `file_for` would be refused for the WRONG reason, and the two
/// refusals are what tells the branches apart.
///
...
fn a_venue_key_is_routed_to_the_venue_setting_table() {
    ...
    // The VENUE branch's own words. A `file_for` refusal names a settings FILE and says nothing
    // about a database, so this string is what proves which branch ran.
    assert!(err.contains("no settings database"), "not the venue branch: {err}");
    assert!(err.contains("secrets migrate"), "the refusal must name the way through: {err}");
    assert!(
        err.contains("move-venue-config"),
        "…and how these values become rows in the first place: {err}"
    );

    // ⚠ THE CONTROL: a file-shaped key on the SAME empty directory is refused differently. Without
    // it this test would pass for a binary that refused every key with one message.
    let other = c.run(&["config.tradehub_addr", "127.0.0.1:7999"]);
    assert!(
        !stderr(&other).contains("no settings database"),
        "a file key took the venue branch: {}",
        stderr(&other)
    );
}
