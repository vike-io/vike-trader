...
/// ⚠ **The rosters' own completeness — the hole that let a shipped example be executed by nothing.**
///
/// [`SHIPPED_SCRIPTS`] and [`SHIPPED_INDICATORS`] are hand-written second copies, like [`EXPECTED`]
/// and for the same reason. Nothing held them TO it, and the gap is not theoretical:
/// [`every_shipped_rhai_strategy_reaches_the_broker`] and
/// [`every_shipped_indicator_computes_a_finite_value_by_its_own_warmup`] iterate the ROSTERS, while
/// every scaffold test iterates [`EXPECTED`]. So a sixth `.rhai` added to the scaffold joins
/// `EXPECTED`, passes every scaffold assertion, and is RUN by no gate at all — which is exactly the
/// "looks mounted and silently never trades" example this file's header calls worse than no example.
///
/// The direction is [`EXPECTED`] -> rosters and never the reverse. `EXPECTED` is the promise a user
/// was made, and it is itself pinned to what `init` really writes by the `--json` report tests'
/// `paths.len() == EXPECTED.len()`. With this test the chain closes end to end: what `init` ships
/// == `EXPECTED` == what a gate executes.
///
/// Both directions, because a stale row is the other half of the same defect: a roster naming a
/// file the scaffold no longer carries keeps passing while driving a script nobody receives.
///
/// It pins the LAYOUT as a side effect, and that is wanted: a strategy is promised at
/// `strategies/rhai/<name>/<name>.rhai` and an indicator at `indicators/<name>.rhai`, so an example
/// shipped anywhere else reads here as unexecuted rather than passing unnoticed.
#[test]
fn every_shipped_rhai_file_is_executed_by_a_gate() {
    let executed: Vec<(&str, String)> = SHIPPED_SCRIPTS
        .iter()
        .map(|(n, _)| (*n, format!("strategies/rhai/{n}/{n}.rhai")))
        .chain(SHIPPED_INDICATORS.iter().map(|(n, _)| (*n, format!("indicators/{n}.rhai"))))
        .collect();
    let shipped: Vec<&str> = EXPECTED.iter().copied().filter(|r| r.ends_with(".rhai")).collect();

    let unexecuted: Vec<&str> =
        shipped.iter().copied().filter(|r| !executed.iter().any(|(_, p)| p == r)).collect();
    assert!(
        unexecuted.is_empty(),
        "these shipped .rhai files are executed by NO gate. Add a row to SHIPPED_SCRIPTS or \
         SHIPPED_INDICATORS so the execution tests drive them — a shipped example that nothing \
         runs is the one this file exists to prevent: {unexecuted:?}"
    );

    let stale: Vec<&str> = executed
        .iter()
        .filter(|(_, path)| !shipped.contains(&path.as_str()))
        .map(|(name, _)| *name)
        .collect();
    assert!(
        stale.is_empty(),
        "these roster rows name a file the scaffold no longer ships — delete the row: {stale:?}"
    );
}
...
/// where `vike-cli backtest params` and a profile's `[sweep]` grid look for it. A `param()`
...
fn the_shipped_profiles_are_valid_toml_with_the_sections_a_run_needs() {
    ...
    // These two profiles carry the script INLINE, byte-identical to the strategy folder's copy —
    // they are written from the same constant, and this is what keeps that true if somebody edits
    // one of them. ⚠ The reason changed with ruling 13: `sweep` had no `--script` flag, and now
    // there is no `sweep` verb at all (`vike-cli backtest` runs the grid, and it DOES take
    // `--script`). What keeps the inlining right is that a search profile is handed to a remote
    // daemon as ONE document, so a script it referenced by path would not be there.
    ...
    // ...and the scaffolded search must actually grid something, or it is a backtest with extra
    // steps. ⚠ The SECTION is `[paramscan]` since stage 7 (ruling R2); the FILE keeps its
    // `sweep.toml` name, which is a separate surface the plane stage owns.
    let scan: toml::Value = toml::from_str(&read(&root, "profiles/sweep.toml")).unwrap();
    assert!(scan.get("paramscan").is_some(), "sweep.toml has no [paramscan] grid");
...
// ── 3. the machine rendering ─────────────────────────────────────────────────────────────────

/// `init --dry-run --json` answers with ONE JSON document naming every shipped file and what would
/// happen to it — and writes nothing, which is what `--dry-run` promises and what a caller running
/// this to plan an install depends on.
///
/// ⚠ It asserts the document is the WHOLE of stdout. The human rendering prints the folder map and
/// four blocks of next-step prose; leaving any of that beside the JSON would hand a caller a stream
/// that does not parse, which is the one way a machine-readable flag can be worse than no flag.
#[test]
fn dry_run_json_lists_every_file_and_writes_nothing() {
    let scratch = scratch("json_dry");
    let root = scratch.path().join("user_data");

    let text = init(&root, &["--dry-run", "--json"]);
    let doc: serde_json::Value = serde_json::from_str(&text)
        .unwrap_or_else(|e| panic!("`init --json` must print ONE JSON document: {e}\n{text}"));

    assert_eq!(doc["dry_run"], serde_json::json!(true));
    assert_eq!(doc["reset"], serde_json::json!(false));
    assert_eq!(doc["user_data"].as_str(), root.to_str());

    let files = doc["files"].as_array().expect("files is an array");
    let paths: Vec<&str> = files.iter().map(|f| f["path"].as_str().expect("a string")).collect();
    for rel in EXPECTED {
        assert!(paths.contains(rel), "the report must name every shipped file; missing {rel}");
    }
    assert_eq!(paths.len(), EXPECTED.len(), "…and nothing else: {paths:?}");
    // A fresh directory: every one of them WOULD be created.
    for f in files {
        assert_eq!(f["action"], serde_json::json!("created"), "{f}");
    }

    assert!(!root.exists(), "--dry-run must write nothing, not even the directory");
}

/// The second run reports `kept` for every file — and reports them at all. The human rendering
/// COUNTS the unchanged ones (a wall of `kept` lines buries the one line that matters); a machine
/// has the opposite need, because a filtered array cannot distinguish "unchanged" from "not
/// shipped".
#[test]
fn a_json_report_names_the_unchanged_files_the_human_one_only_counts() {
    let scratch = scratch("json_second");
    let root = scratch.path().join("user_data");

    init(&root, &[]); // the real scaffold
    let text = init(&root, &["--json"]);
    let doc: serde_json::Value = serde_json::from_str(&text).expect("valid JSON");

    assert_eq!(doc["dry_run"], serde_json::json!(false));
    let files = doc["files"].as_array().expect("files is an array");
    assert_eq!(files.len(), EXPECTED.len(), "every shipped file has a row, kept included");
    for f in files {
        assert_eq!(f["action"], serde_json::json!("kept"), "{f}");
    }

    // …and the human rendering of the same run counts them instead of listing them.
    let human = init(&root, &[]);
    assert!(human.contains("nothing to do"), "{human}");
}

/// `--reset` over an edited sample reports `restored`, and the flag is echoed — the two together
/// are what let a caller read an `action` without knowing which flags produced it.
#[test]
fn a_restored_sample_is_reported_as_restored() {
    let scratch = scratch("json_reset");
    let root = scratch.path().join("user_data");

    init(&root, &[]);
    std::fs::write(at(&root, "profiles/backtest.toml"), "# edited by hand\n").expect("edit");

    let text = init(&root, &["--reset", "--json"]);
    let doc: serde_json::Value = serde_json::from_str(&text).expect("valid JSON");
    assert_eq!(doc["reset"], serde_json::json!(true));

    let restored: Vec<&str> = doc["files"]
        .as_array()
        .expect("files is an array")
        .iter()
        .filter(|f| f["action"] == serde_json::json!("restored"))
        .map(|f| f["path"].as_str().expect("a string"))
        .collect();
    assert_eq!(restored, vec!["profiles/backtest.toml"], "only the edited sample: {restored:?}");
}
