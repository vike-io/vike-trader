...
fn a_usage_error_is_two() {
    ...
    // ⚠ **A RETIRED verb rides the SAME rung as an unknown one, so this row cannot tell them
    // apart** — which is why it says what it is asserting rather than naming a flag `sweep` no
    // longer has. The message it carries is pinned in
    // `crates/vike-cli/tests/help_cli.rs`'s `the_retired_sweep_verb_fails_naming_its_replacement`;
    // if that test ever goes, this row silently stops testing anything.
