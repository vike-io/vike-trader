...
fn help_names_every_subcommand_and_exits_zero() {
    ...
    let text = stdout(&out);
    for sub in ["fetch", "seed-demo", "list", "coverage", "rm"] {
...
fn json_is_the_whole_of_stdout_and_the_engines_report_moves_to_stderr() {
    ...
    assert_eq!(
        doc["engine_argv"],
        serde_json::json!(["--fetch", "binance:BTCUSDT:1h", "--days", "180", "--store", "/s"]),
...
fn seed_demo_json_reports_a_run_with_no_series_and_no_window() {
    ...
    assert_eq!(doc["engine_argv"], serde_json::json!(["--seed-demo"]));
...
fn without_json_the_output_is_the_engines_own_and_nothing_else() {
    ...
    assert!(out.status.success(), "{}", stderr(&out));
    assert_eq!(stdout(&out), "argv: --seed-demo\n", "the human path is unchanged");
}
...
fn rm_reaches_the_engine_as_its_own_flags() {
    ...
    assert_eq!(
        stdout(&out).trim(),
        "argv: --rm-series --kind bar --venue hyperliquid --symbol BTC --interval 1h --yes --store /s",
        "the verb's product is the engine's argv"
    );
    ...
    assert_eq!(
        stdout(&out).trim(),
        "argv: --rm-series --kind cohort --venue hyperliquid --produced-by cohort: --dry-run",
