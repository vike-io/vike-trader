...
fn a_bad_command_line_is_the_usage_rung() {
    ...
    for (args, needle) in [
        ...
        (vec!["data", "fetch", "b:S:1h", "--days", "1", "--store", "/tmp/s"], "--store"),
        (vec!["data", "fetch", "b:S:1h", "--days", "1", "--engine", "/tmp/e"], "--engine"),
    ] {
...
fn a_missing_engine_is_the_connect_rung() {
    ...
    let out = run(
        scratch.path(),
        &["data", "seed-demo", "--engine", absent.to_str().expect("utf-8 temp path")],
    );
    let err = stderr(&out);
...
fn an_overloaded_engine_code_lands_on_the_unclassified_rung() {
    ...
    let out = run(scratch.path(), &["data", "seed-demo", "--engine", planted.arg()]);
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn seed_demo_json_reports_a_run_with_no_series_and_no_window() {
    ...
    let out = run(scratch.path(), &["data", "seed-demo", "--json", "--engine", planted.arg()]);
    assert!(out.status.success(), "{}", stderr(&out));
    let doc: serde_json::Value = serde_json::from_str(&stdout(&out))
        .unwrap_or_else(|e| panic!("stdout is not one JSON document ({e}): {}", stdout(&out)));
...
fn a_failing_engine_under_json_writes_no_document_and_keeps_the_rung() {
    ...
    let out = run(scratch.path(), &["data", "seed-demo", "--json", "--engine", engine]);
    assert_eq!(
        out.status.code(),
        Some(1),
...
fn the_recorded_asset_class_reaches_the_operator_and_its_absences_are_two_words() {
    ...
    let out = run(scratch.path(), &["data", "list", "--addr", &addr, "--class", "--json"]);
    assert!(out.status.success(), "{}", stderr(&out));
    let doc: serde_json::Value = serde_json::from_str(&stdout(&out))
        .unwrap_or_else(|e| panic!("stdout is not one JSON document ({e}): {}", stdout(&out)));
    ...
    let out = run(scratch.path(), &["data", "list", "--addr", &addr, "--class"]);
    assert!(out.status.success(), "{}", stderr(&out));
    let text = stdout(&out);
...
fn the_class_flag_is_refused_on_the_sibling_read_verbs() {
    ...
    for sub in ["coverage", "tape-health", "universe"] {
        let out = run(scratch.path(), &["data", sub, "--class"]);
        ...
        assert!(err.contains("data list --class"), "…and where the class is: {err}");
    }
}
...
fn an_unreachable_datahub_is_the_connect_rung() {
    ...
    for _ in 0..PREMISE_ATTEMPTS {
        ...
        for sub in ["list", "coverage"] {
            ...
            let out = run(scratch.path(), &["data", sub, "--addr", &text]);
            let err = stderr(&out);
            if is_listening(&addr) {
...
fn repair_rehearses_by_default_and_has_no_remote_route() {
    ...
    let out = run(
        scratch.path(),
        &[
            "data", "repair", "--kind", "trade", "--venue", "d", "--symbol", "S", "--yes",
            "--engine", engine,
        ],
    );
    ...
    let out = run(
        scratch.path(),
        &["data", "repair", "--kind", "bar", "--venue", "b", "--symbol", "S", "--addr", "h:1"],
    );
