...
fn the_local_and_remote_flag_sets_refuse_each_other() {
    ...
    for (args, needle) in [
        ...
        // The SEARCH flags `backtest` absorbed from the deleted `sweep` verb obey the same rule,
        // and the wire-shaped half is new: `--optimizer euler` has no remote route at all, so it
        // is refused BY NAME rather than downgraded to the grid.
        (vec!["backtest", "--profile", p, "--optimizer", "tpe"], "--local only"),
        (vec!["backtest", "--profile", p, "--rank-by", "multi"], "--local only"),
        (vec!["backtest", "--profile", p, "--trials", "8"], "--local only"),
        (vec!["backtest", "--local", "--profile", p, "--rank-by", "bogus"], "--rank-by"),
        (vec!["backtest", "--local", "--profile", p, "--optimizer", "bogus"], "--optimizer"),
    ] {
