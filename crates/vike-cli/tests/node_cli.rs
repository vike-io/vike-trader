//! End-to-end tests for `vike-cli node`, driving the SHIPPED binary (`CARGO_BIN_EXE_vike-cli`).
//!
...
impl Case {
    ...
    /// The file `node` verbs WRITE — `node.env` since 2026-09-08.
    ///
    ...
    /// the ones proving the split: a node verb must not write here, and a box whose keys have not
    /// migrated must still be READ from here.
...
/// `tests/help_cli.rs`.
#[test]
fn node_is_a_registered_verb_whose_help_is_a_success_on_stdout() {
    let c = Case::new("help");
    let out = c.run(&["--help"]);
    ...
    assert!(String::from_utf8_lossy(&top.stdout).contains("node"), "the command list omits `node`");
}
...
fn setup_refuses_an_absent_store_and_names_how_to_make_one() {
    ...
    assert_eq!(
        ...
        "a node verb wrote into the VENUE credential store"
    );
}
...
fn status_on_an_unattached_box_names_what_is_missing() {
    ...
    assert!(stderr(&out).contains("node connect"), "{}", stderr(&out));
}
