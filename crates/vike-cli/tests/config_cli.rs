...
fn an_unread_key_gets_the_env_verdict_above_its_reason() {
    ...
    // `POLY_HEARTBEAT`, so NEITHER spelling does anything. `sweep_threads` is the counterexample —
    // the table's one `Reader::Live`, where exporting the variable genuinely still works — and
    // both are configured here so this test proves a DISTINCTION rather than the presence of a
    // string.
    c.write("flags.toml", "poly_heartbeat = true\n");
    ...
    assert!(
        warned.contains("The ENVIRONMENT VARIABLE still works"),
        "…and the live row must still say so, or the verdict is a constant rather than a \
         verdict: {out}"
    );
...
fn the_env_table_names_the_variables_that_do_nothing() {
    ...
    assert!(
        ...
        "a `Reader::Live` variable must NOT be listed — exporting it genuinely works, and listing \
         it would make this block the new false claim: {out}"
    );
}
