...
fn spawn_node(
    ...
    thread::spawn(move || {
        let _ = server::serve(
            ...
            None,
            // No `AccountAdminSource`: the account capability is an ABSENCE on every box that has
            // not DECLARED a barrier, which is every fixture here and every shipped box today.
            None,
...
/// and once.
fn settings_dir_with_store(keys: &[(&str, &str)]) -> SettingsFixture {
    settings_dir_with_store_in(vike_secrets::NODE_FILE, keys)
}

/// A throwaway PROJECT and its `settings/` child — the two halves kept together so the `TempDir`
/// outlives the path handed to the child process.
///
/// ⚠ **The settings directory is a CHILD, never the bare `TempDir`, and that is not tidiness.**
/// `$VIKE_SETTINGS_DIR` is returned VERBATIM by the resolver, and `crates/vike-cli/src/lib.rs`'s
/// `resolve_policy` then takes its PARENT as `<project>` — so a bare temp directory makes the
/// child's `<project>` the SHARED SYSTEM TEMP ROOT, and every project-relative path the CLI touches
/// lands in a directory every user and process on the box shares. For this suite the live one is
/// `<project>/user_data/indicators`: `install_user_indicators` reads it BEFORE any verb runs and
/// prints one `vike-cli: indicator not loaded — …` line per diagnostic to stderr, which is exactly
/// what [`trade_status_renders_the_mode_the_node_identity_and_mount_rows`]'s "nothing lands on
/// stderr" assertion measures. `/tmp/user_data` does not exist today; `vike-cli init` with no
/// `--dir` scaffolds `<project>/user_data`, so the CLI's own verb can create it, and once one bad
/// `.rhai` is in it that assertion is red for whichever user did not put it there — `/tmp` is
/// sticky, so they cannot remove it either. `<project>/bin` and `<project>/tmp` are the same
/// hazard wearing different names, which is why the repair is the PROJECT and not a pin on one
/// variable.
///
/// `crates/vike-cli/tests/backtest_cli.rs`'s `project_with_settings` is the same repair.
struct SettingsFixture {
    /// The project — BOUND, so its `Drop` removes the tree. Also the directory whose
    /// `user_data/` the child resolves.
    root: tempfile::TempDir,
    settings: std::path::PathBuf,
}

impl SettingsFixture {
    /// `<project>/settings` — what `$VIKE_SETTINGS_DIR` must name.
    fn path(&self) -> &std::path::Path {
        &self.settings
    }

    /// `<project>` — the directory the child derives `user_data/`, `bin/` and `tmp/` from.
...
fn settings_dir_with_store_in(file: &str, keys: &[(&str, &str)]) -> SettingsFixture {
    let root = tempfile::tempdir().expect("tempdir");
    let settings = root.path().join("settings");
    std::fs::create_dir_all(&settings).expect("create the settings directory");
...
fn run_cli(addr: &str, settings_dir: &std::path::Path, extra: &[&str]) -> Run {
    let out = Command::new(env!("CARGO_BIN_EXE_vike-cli"))
        ...
        // ⚠ The user-content directory is left to be DERIVED from the settings directory — this
        // clears an operator box's export so it is, because that derivation (`<project>/user_data`)
        // is half of what [`SettingsFixture`] exists to keep inside the fixture, and
        // [`a_broken_user_indicator_is_reported_from_this_tests_own_project`] asserts on it.
...
fn trade_status_renders_the_mode_the_node_identity_and_mount_rows() {
    ...
    //
    // ⚠ This assertion is the one in this file whose verdict can be decided from OUTSIDE the
    // fixture, which is why [`SettingsFixture`] nests and why
    // [`a_broken_user_indicator_is_reported_from_this_tests_own_project`] exists: a green here is
    // not evidence that `<project>` resolved inside this test's TempDir, because it is also what a
    // box whose shared `/tmp/user_data/indicators` happens to be empty prints.
    assert!(run.stderr.trim().is_empty(), "{transcript}");
}

/// **The POSITIVE proof that `<project>` is this test's own TempDir**, and the reason the fixture
/// above nests a `settings/` child instead of handing the child a bare temp directory.
///
/// `install_user_indicators` reads `<project>/user_data/indicators` before ANY verb runs and prints
/// one `vike-cli: indicator not loaded — …` line per diagnostic, naming the file's full path. So
/// planting one deliberately-broken `.rhai` under THIS test's project and finding its path on
/// stderr proves the directory the CLI read is the one this test created — a claim no green run of
/// the happy path can make, since that passes equally on a box where the shared system temp root
/// simply has no `user_data/` yet.
///
/// Read the failure the other way round too: if the fixture regressed to a bare temp directory,
/// this plant would land somewhere the child never looks and the assertion below would find no
/// diagnostic at all.
#[test]
fn a_broken_user_indicator_is_reported_from_this_tests_own_project() {
    use vike_model::state_path::{INDICATORS_SUBDIR, PROJECT_USER_DATA_DIR};

    let (_mount, addr) = spawn_node(Some(identity()), mounts());
    let settings = settings_dir_with_store(&[("VIKE_TRADEHUB_OBSERVE_KEY", OBSERVE_KEY)]);

    let indicators = settings.project().join(PROJECT_USER_DATA_DIR).join(INDICATORS_SUBDIR);
    std::fs::create_dir_all(&indicators).expect("create the indicators directory");
    // Not Rhai by any reading — the point is a diagnostic, not a particular compiler message.
    let planted = indicators.join("deliberately_broken.rhai");
    std::fs::write(&planted, "fn (((\n").expect("plant a broken indicator");

    let run = run_cli(&addr.to_string(), settings.path(), &[]);
    let transcript = format!("--- stdout ---\n{}\n--- stderr ---\n{}", run.stdout, run.stderr);
    // A bad indicator never fails the command — that contract is `install_user_indicators`'s, and
    // it is restated here because this test would otherwise look like it is asserting a failure.
    assert!(run.ok, "a broken indicator must not fail the verb:\n{transcript}");
    assert!(
        run.stderr.contains("indicator not loaded"),
        "the diagnostic must reach stderr:\n{transcript}"
    );
    assert!(
        run.stderr.contains(&planted.display().to_string()),
        "the diagnostic must name the file THIS test planted — that is what proves `<project>` \
         resolved inside this test's own TempDir rather than at the shared system temp root:\n\
         {transcript}"
    );
}
    ...
    thread::spawn(move || {
        let verdict = (|| -> Result<Vec<ConnLog>, String> {
            ...
            for _ in 0..connections {
                ...
                write_frame(&mut stream, &Response::AuthOk { scope: Scope::Read })
