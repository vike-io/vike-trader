...
/// configuration fact. `crates/vike-cli/src/cmd/strategy_status.rs`'s `failure_exit` carries the
...
/// write a key into the file the very next run tells them to move it out of — and `vike-cli node
/// setup`, this binary's own writer, writes `node.env`. A refusal that names a remedy the writer
/// disagrees with is the defect class this whole split exists to close.
#[test]
