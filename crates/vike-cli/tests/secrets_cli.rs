...
/// message named `vike-cli secrets path` and did NOT name `vike-cli node setup`, with the reason
/// written beside it: *"a node-key GENERATOR is designed and NOT BUILT, and naming one in a refusal
...
fn a_key_the_workspace_reads_is_refused_without_being_called_unloadable() {
    ...
    assert!(err.contains("node setup"), "the operator must be given a route: {err}");
    assert!(err.contains("node connect"), "…including the client's half: {err}");
    ...
    // step `node setup` exists to delete.
    ...
    assert!(!err.contains("node setup"), "it must not route a datahub key at the tradehub: {err}");
    assert!(
