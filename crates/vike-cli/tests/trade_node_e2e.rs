...
//!   binary.** The node keys exist ONLY in the credential store (the process environment is
...
/// A throwaway `<project>/settings` directory holding a credential store with the node keys in it —
...
///
/// Keys only in the credential store; an order typed at the REPL; the order read back off the
...
fn repl_submit_places_an_order_end_to_end() {
    ...
    assert!(
        ...
        "the control key must resolve from the credential store\n{transcript}"
    );
    assert!(
        run.stdout.contains("credential store"),
...
fn repl_with_no_key_anywhere_names_the_store_it_looked_in() {
    ...
    assert!(
        run.stderr.contains("credential store"),
...
/// `VIKE_TRADEHUB_CONTROL_KEY` in the credential store. Every write printed
