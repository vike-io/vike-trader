...
//!
//! `vike-cli init` writes `user_data/profiles/backtest.toml`, and `vike-cli data seed-demo` writes
...
fn the_shipped_profile_names_a_slice_the_demo_tape_holds() {
    ...
    assert_eq!(
        ...
        "the shipped profile's venue is {:?} but the demo tape is written under {DEMO_VENUE:?}. \
         `vike-cli data seed-demo` then leaves that profile with an empty slice, which runs to \
...
fn the_shipped_profile_says_the_demo_tape_is_not_market_data() {
    ...
    assert!(
        text.contains("vike-cli data seed-demo"),
        "the shipped profile no longer names the command that fills the slice it asks for, so a \
         reader who hits an empty run has nothing to follow. ⚠ The needle is the WHOLE command \
         now rather than the bare `--seed-demo` flag: ruling 12 retired that flag off the engine, \
         so a profile still naming it would send a fresh install to a spelling the binary refuses."
    );
}
