...
/// reason this helper exists: `--help` also prints every SUMMARY as well as every name, so a
/// `contains` hit can come from any row's prose. Today `data`'s summary ends *"list what it holds
/// and report coverage"* — so `stdout.contains("report")` is TRUE with the `report` verb deleted,
/// and was true before that verb existed.
///
/// ⚠ This used to make the same point by quoting `backtest`'s summary as *"run a backtest on a
/// remote vike-datahub server and print the report"*, and that example rotted TWICE: the row had
/// moved on from the wording, and the daemon it named stopped serving the verb at ruling 7. The
/// example above is one a reader can check against the rows in `crate::COMMANDS` right now. `crate::COMMANDS`
...
fn data_help_exits_zero_on_stdout_at_both_levels() {
    ...
    assert_help_is_clean(&["data", "hist", "fetch", "-h"]);
}
