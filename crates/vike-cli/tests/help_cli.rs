...
fn the_top_level_help_lists_every_command() {
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    for command in
        ["backtest", "config", "data", "secrets", "trade", "strategy-status", "init", "indicators"]
    {
...
/// the two subcommands are named — the verb itself takes no default action. A `--help` that failed
/// there would leave "get some market data" with no discoverable answer at all.
#[test]
