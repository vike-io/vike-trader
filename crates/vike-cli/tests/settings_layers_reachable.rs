...
//! [`vike_config::PRECEDENCE_FILES`] against the resolver's own variants and order, and walks the
...
use vike_config::{PRECEDENCE_FILES, REMOVED_PROJECT_FILE};
...
impl Case {
    ...
    ///
    /// The catch-all PANICS rather than skipping: a layer added to [`PRECEDENCE_FILES`] with no scenario
        ...
        match kind {
            ...
            other => panic!(
                "no scenario for the advertised layer {other:?}. Every layer in \
                 vike_config::PRECEDENCE_FILES must be proven to take effect end to end — add its arm \
    ...
    /// `crates/vike-config/src/provenance.rs`'s `describe_with_source` made unable to report a `db`
...
///
/// Driven by [`PRECEDENCE_FILES`] itself rather than by a written-out list, so a layer added to the
...
fn every_advertised_layer_takes_effect_through_the_shipped_binary() {
    for (i, target) in PRECEDENCE_FILES.iter().enumerate() {
        ...
        for lower in &PRECEDENCE_FILES[i..] {
            case.arm(lower.kind);
        }
...
fn the_run_profile_ceilings_are_readable_with_the_daemon_down() {
    ...
    assert!(
        report.contains("run `run-live`"),
        "the mirror must report the body it stored, under its STEM: {report}"
    );
    assert!(
        report.contains("risk.max_notional_per_order = 5000.0"),
        "…and the rows it carried: {report}"
    );
    // ⚠ The sentence that replaced *"the `[risk]` rows are a DISCLOSURE copy"*. These rows CAN
    // bind now — that is the whole migration — so what the report must say every time is that
    // storing a body is not selecting one, and which verb does the selecting.
    assert!(
        report.contains("No active run row was written"),
        "the mirror must say what it did NOT do, every time: {report}"
    );
    assert!(
        report.contains("config activate run"),
        "…and name the deliberate act that would: {report}"
    );
    ...
    assert!(
        ...
        "with NO row active, the human block must still say what a stored ceiling is not:\n{text}"
    );
    assert!(
        text.contains("VIKE_RUN_PROFILE"),
        "it must say which profile is live is still the file rungs' answer:\n{text}"
    );
    assert!(
    ...
    // ── THE CROSSING, end to end through the real binary ─────────────────────────────────────
    //
    // ⚠ This is the half the old sentence made impossible. `--proves` is the fence: the stored rows
    // must render back to a document that parses EQUAL to the file the operator names.
    ...
    let crossed = case.run_args(&["config", "activate", "run", "run-live", "--proves", &profile]);
    ...
    assert_eq!(
        activated["profile_risk"]["enforced"], true,
        "…and stop calling it unenforced, which is the sentence that would now be false"
    );

    // …and the ROLLBACK, which is one column and no data.
    ...
    assert!(rolled["profile_risk"]["active"].is_null(), "the file rungs decide again");
    assert_eq!(rolled["profile_risk"]["enforced"], false);
    assert_eq!(
        rolled["profile_risk"]["profiles"][0]["rows"],
        p["rows"].clone(),
        "the BODY is untouched by the rollback — that is why re-crossing needs no re-mirror"
    );
}
...
/// **THE crossing gate, end to end, through the SHIPPED BINARY.** The one test I would not merge
/// this change without.
///
/// Its sibling above plants a key NO FILE SETS, which proves the `db` layer is reached. That is not
/// enough for the crossing: it would pass against a loader in which the file layer still wins,
/// because there is no file to win. This one plants a **DISAGREEING** file, adopts through
/// `vike-cli config adopt`, and demands the ROW win — which can only happen if the file layer
/// stopped being read.
///
/// Every step is a shipped verb: `secrets migrate` creates the store, `config mirror` writes the
/// rows, `config compare` proves the two resolutions identical, `config adopt` seals it, and
/// `config show --json` reports the result. Nothing here plants a row or a seal with a library
/// call, because a green that only proves a struct round-trips is the shape of green this file
/// exists to replace.
///
/// It also pins the two operator-facing facts the crossing turns on, because both are things
/// somebody reads and acts on:
///
/// * `config compare` **exits 1** on a disagreement and `config adopt` **refuses** on it — the
///   precondition that makes adopting a change of ARTIFACT rather than a change of VALUE;
/// * `config adopt --undo` hands the box back to the files **immediately**, with the rows left in
///   place — the rollback that is one row rather than a redeploy of a trading daemon.
#[test]
fn adopting_makes_the_row_win_over_a_disagreeing_file_through_the_shipped_binary() {
    let case = Case::new("adopt");

    // A store, created the one sanctioned way.
    case.write("settings/secrets.env", "BINANCE_DEMO_API_KEY=not-a-real-key\n");
    let migrated = case.run_args(&["secrets", "migrate"]);
    assert!(
        migrated.status.success(),
        "`vike-cli secrets migrate` must create the store: {}",
        String::from_utf8_lossy(&migrated.stderr)
    );

    // The FILE's answer, mirrored into the rows. At this point the two agree.
    case.write("settings/config.toml", "log_dir = \"/from/file\"\n");
    let mirrored = case.run_args(&["config", "mirror"]);
    assert!(
        mirrored.status.success(),
        "`vike-cli config mirror`: {}",
        String::from_utf8_lossy(&mirrored.stderr)
    );
    let compared = case.run_args(&["config", "compare"]);
    assert!(
        compared.status.success(),
        "a freshly mirrored box must compare IDENTICAL — that is what makes adopting safe: {}",
        String::from_utf8_lossy(&compared.stdout)
    );

    // Now make the two DISAGREE, and prove both preconditions bite.
    case.write(
        "settings/config.toml",
        "log_dir = \"/from/file\"\n# and a change nobody mirrored\n",
    );
    case.write("settings/config.toml", "log_dir = \"/changed/after/the/mirror\"\n");
    let diverged = case.run_args(&["config", "compare"]);
    assert!(
        !diverged.status.success(),
        "`config compare` must EXIT 1 on a disagreement: {}",
        String::from_utf8_lossy(&diverged.stdout)
    );
    let refused = case.run_args(&["config", "adopt"]);
    assert!(
        !refused.status.success(),
        "`config adopt` must REFUSE while the two resolutions differ — adopting there would change \
         what the box RUNS ON rather than only where it reads it from"
    );
    assert!(
        String::from_utf8_lossy(&refused.stderr).contains("NOTHING WAS WRITTEN"),
        "…and say so: {}",
        String::from_utf8_lossy(&refused.stderr)
    );

    // Put the file back to what the rows say, adopt, and then make them disagree AGAIN — this time
    // with the seal in place, so the ROW is the one that must win.
    case.write("settings/config.toml", "log_dir = \"/from/file\"\n");
    let adopted = case.run_args(&["config", "adopt"]);
    assert!(
        adopted.status.success(),
        "`vike-cli config adopt`: {}",
        String::from_utf8_lossy(&adopted.stderr)
    );
    case.write("settings/config.toml", "log_dir = \"/an/inert/draft\"\n");

    let shown = case.show_json();
    assert_eq!(
        reported_origin(&shown),
        "db",
        "⚠ THE CROSSING: with the box ADOPTED, a settings file that disagrees is an INERT draft. \
         A `file` here means the loader is still reading layer 2, and the whole change is a no-op."
    );
    let value =
        shown["settings"].as_array().unwrap().iter().find(|r| r["key"] == KEY).unwrap()["value"]
            .as_str()
            .unwrap()
            .to_string();
    assert_eq!(
        value, "/from/file",
        "…and the VALUE is the row's, not the draft's. An origin column that inverted while the \
         value did not would be the worse half of a disagreement."
    );

    // ...and the ROLLBACK: one row, immediately, with the rows left in place.
    let undone = case.run_args(&["config", "adopt", "--undo"]);
    assert!(
    ...
    assert_eq!(
        reported_origin(&case.show_json()),
        "file",
        "un-adopting hands the box back to the files immediately — that is the rollback, and it is \
         one row rather than a redeploy of a trading daemon"
    );
}
