...
//! and both halves resolve it through the one call — `vike_secrets::resolve_node_keys`, which reads
//! that file and falls back to `secrets.env` for a box that has not migrated. What did NOT change
...
mod tests {
    ...
    /// key nobody sets, and the symptom is an endless `bad mac` at the node on `trade`, `mcp` and
    /// `strategy-status` — with every test in this crate green.
    #[test]
