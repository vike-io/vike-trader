...
/// (`vike_secrets::HAND_MAPPED_ACCOUNTS`), so this verb and the read-side fold cannot disagree
/// about which rows these are or about what name a row answers for. The grammar half moved down on
/// 2026-09-22 so the fold could run inside the store; `crates/vike-secrets/src/venue_setting.rs`
/// carries the two blindnesses that forced it.
fn moves_to(name: &str) -> Option<String> {
    ...
    Some(vike_secrets::venue_setting_key(&venue, tier.as_deref(), &class.field))
}
...
pub fn run_move(dry_run: bool, settings_dir: Option<&Path>) -> CmdResult<()> {
    ...
    let report = vike_secrets::move_pending_rows(
        &db,
        &moves_to,
        &vike_secrets::parse_venue_setting_key,
        collisions,
        dry_run,
    )
    .map_err(|e| CliError::from(e.to_string()))?;
