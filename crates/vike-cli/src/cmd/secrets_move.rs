...
pub fn run_move(dry_run: bool, settings_dir: Option<&Path>) -> CmdResult<()> {
    ...
    // ⚠ The ENCODER, not a raw write. A `setting` row holds ONE JSON SCALAR, and storing the
    // credential's plain string broke the CI box's whole `config` section —
    // `vike_bridge_core::credentials::encode_setting_scalar` carries the measurement.
    let report = vike_secrets::move_pending_rows(
        &db,
        &moves_to,
        &vike_bridge_core::credentials::encode_setting_scalar,
        collisions,
        dry_run,
    )
    .map_err(|e| CliError::from(e.to_string()))?;
