...
//! with the same sentence whichever route ran. The knobs are forwarded as the TOKENS you typed —
//! this side parses none of them, which is what keeps that sentence single.
//!
...
use vike_datahub_client::{DatahubClient, NodeKeys, Scope, WireSearch};
...
///
/// ⚠ **`multi` reaches BOTH routes since stage 7, and this doc said the opposite.** It used to be
/// LOCAL-ONLY as a WIRE fact: the server resolved `rank_by` through
/// `harness::RankMetric::from_str_ci`, whose four arms have no `multi`. It resolves through
/// `vike_backtest::harness::search_select`'s `resolve_rank` now, which has FIVE, and a daemon too
/// old to do that is refused BY NAME through `vike_datahub_client::FEATURE_SEARCH_METHOD` with
/// nothing sent.
const RANK_METRICS: [&str; 5] = ["sharpe", "return", "max_dd", "equity", "multi"];

/// The `--optimizer` METHOD names — `vike_datahub_client::SEARCH_METHODS`, not a local literal.
///
/// ⚠ It was a THREE-element array refusing `genetic` during arg parsing, and `grid` was the only
/// name with a remote route. Stage 7 of
/// `docs/superpowers/specs/2026-09-12-backtest-cli-surface-design.md` collapsed three rosters into
/// one: the wire carries a method (`Request::RunParamscanProfile`'s `search`), and the roster lives in
/// the protocol crate because that is the only crate this one and `vike-backtest` both depend on.
/// This stays a SPELLING check — what a method MEANS is
/// `vike_backtest::harness::search_select`.s, on whichever side runs — and it is what makes a typo
/// a local usage error instead of a wasted round trip.
const OPTIMIZERS: [&str; 4] = vike_datahub_client::SEARCH_METHODS;
...
///
/// ⚠ **It advertises NINE sub-verbs, not two**, because two stages landed on this plane at once:
/// stage 3 made the sub-verb MANDATORY and gave the computing half its own token (`run`), while
/// stages 4–6 added the eight verbs that READ and judge what a run left behind. ONE usage and one
/// roster — [`SUBCOMMANDS`] — so no refusal can name the set short.
...
\n\
       vike-cli backtest run [--profile <run.toml>] [--venue NAME] [--symbol SYM[,SYM]] [--interval 1h] [--from DATE] [--to DATE] [--kind bar|tick] [--strategy NAME] [--cash N] [--fee RATE] [--slippage RATE] [--param k=v] [--set key=value] [--preset <p.toml>] [--script <s.rhai>] [--write-profile <out.toml>] [--show-effective] [--addr 127.0.0.1:7880] [--json]\n\
...
///
/// ⚠ The eight reading rows are spelled out rather than folded in from [`READ_SUBCOMMANDS`],
...
struct Args {
    ...
    /// `--optimizer`: the search METHOD, spelling-checked against [`OPTIMIZERS`] and otherwise
    /// forwarded verbatim. `None` is forwarded as `None` on BOTH routes — this side substitutes no
    /// default, because the side that RUNS resolves it against the protocol's own
    /// `DEFAULT_SEARCH_METHOD` and a second answer here could disagree with it.
    ///
...
impl Args {
    ...
    /// The selection this command line asks the SERVER for, as the TOKENS the operator typed.
    ///
...
///
/// ⚠ **The name says READ and one member WRITES.** [`ReadSub::Tag`] appends to a run's optional
...
/// `crate::cmd::runs`'s module doc is where that exception is argued.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// The parsed reading command line. Every field is shared across the five sub-verbs and refused by
/// name where it does not apply — [`refuse_foreign_read_flags`], and the reason each carries.
#[derive(Debug)]
...
fn parse_read(argv: &[&str]) -> Result<ReadArgs, String> {
    ...
    match sub {
        ...
        ReadSub::Params | ReadSub::Strategies => {}
    }
...
fn refuse_foreign_read_flags(a: &ReadArgs) -> Result<(), String> {
    ...
    let params_only = [("--script", a.script.is_some()), ("--strategy", a.strategy.is_some())];
    ...
    // ⚠ `--trades` is the one flag TWO sub-verbs own, so it is refused on the other six rather than
    // living in either roster: `show --trades` renders the ledger and `diff --trades` compares two
    // of them, which is the same document answering the same question from two sides.
    ...
    if a.sub != ReadSub::Params {
        refuse(&params_only, "that flag names the strategy or script whose knobs `params` lists")?;
    }
    ...
    if !matches!(a.sub, ReadSub::Ls | ReadSub::Show) && a.out.is_some() {
        return Err(format!(
            "--out names a FILE to write a rendered document to, and `backtest {}` prints a line \
             the shell can already redirect",
            a.sub.as_str()
        ));
    }
    Ok(())
...
/// ⚠ **The return is a RUNG, not `()`, and exactly one sub-verb needs that.** `gate`'s product IS
/// its exit code: it evaluates every criterion, prints the whole verdict to stdout, and the number
/// beside it is that document's summary (`crate::exit::Exit::Breach`). Routing a breach through
/// `CliError` instead would print one stderr line and throw the document away — §7.1 requires the
/// opposite. Every other arm answers `Exit::Ok` and is unchanged.
fn execute_read(
    a: &ReadArgs,
...
/// The two spellings `DataKind` accepts, for the local `--kind` roster — the same
/// spelling-check-against-a-roster shape [`one_of`] already applies to `--rank-by` and
/// `--optimizer`, never a second implementation of what the value MEANS.
...
mod tests {
    ...
    ///
    /// ⚠ The ROUTER is all this drives, so the bare NAME is enough for the eight reading verbs:
    ...
    /// `parse_params_args` did not survive the merge with the reading plane: `params` is one of
    /// eight sub-verbs sharing ONE flag loop now. What survived is the roster and the sentence.
    #[test]
    ...
    fn every_reading_subcommand_is_reachable_by_the_name_it_advertises() {
        for sub in READ_SUBCOMMANDS {
            // ⚠ The MINIMUM line each sub-verb accepts, not a bare name: three of them REFUSE a
            // command line that would do nothing, so a bare token would prove only that the refusal
            // fires. What this test holds is that the NAME routes to the sub-verb it advertises.
            let argv: Vec<&str> = match sub {
    ...
    /// ⚠ `--trades` is the one flag TWO sub-verbs own — `show` renders the ledger and `diff`
    /// compares two of them — so it is refused on the other six rather than living in either
    /// roster.
    #[test]
