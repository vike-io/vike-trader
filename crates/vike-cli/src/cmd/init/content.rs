...
vike-cli sweep       --profile sweep.toml       --rank-by sharpe
...
# market. Seed it with `backtest --seed-demo`, which writes exactly this slice.
#
...
#   vike-cli sweep --profile sweep.toml --rank-by sharpe
#
# ⚠ The script is INLINE below, because `sweep` has no --script flag. `vike-cli init` writes it
# from the same source as ../strategies/rhai/sma_cross/sma_cross.rhai; if you edit one, edit both.
