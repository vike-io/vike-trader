...
options:
  ...
  --json          the same report as one JSON object — the directory, every
                  shipped file and what happened (or would happen) to it —
                  instead of the map and the next-step notes
  -h, --help      this message
...
struct Args {
    ...
    reset: bool,
    dry_run: bool,
    /// `--json`: the same report as ONE machine-readable object instead of the map + the change
    /// list + the next-step prose. See [`report_json`].
    json: bool,
}
...
fn parse(args: impl Iterator<Item = String>) -> Result<Args, String> {
    ...
    Ok(Args { dir, reset, dry_run, json })
}
...
impl Action {
    /// The name a MACHINE reads, in [`report_json`]. Deliberately not the human column label:
    /// `print_report` prints imperatives (`create`, `restore`) beside a `would be` when nothing was
    /// written, and a caller parsing JSON reads the `dry_run` field for that instead of a tense.
...
pub fn run(args: impl Iterator<Item = String>, user_data_dir: Option<&Path>) -> ExitCode {
    ...
    match scaffold(&root, args.reset, args.dry_run) {
        Ok(report) => {
            // Same report, two renderings — and under `--json` the JSON is the WHOLE of stdout.
            // The map, the change list and the next-step lines are prose for a person; emitting
            // them alongside the document would leave a caller parsing a stream that is not JSON.
            if args.json {
                println!("{}", report_json(&root, &report, &args));
            } else {
...
/// The `--json` report: the directory, whether anything was actually written, and one row per
/// shipped file with what happened to it.
///
/// ⚠ **Every file is a row, `kept` included** — which is exactly where this differs from the human
/// rendering, and deliberately. [`print_report`] COUNTS the unchanged ones because a wall of `kept`
/// lines buries the one line a person needs; a machine has the opposite problem, because "the file
/// is not in the list" and "the file is not shipped at all" are indistinguishable from a filtered
/// array. The `dry_run` field is what a caller reads to know whether the actions describe what
/// happened or what would have.
fn report_json(root: &Path, report: &[(&str, Action)], args: &Args) -> String {
    let doc = serde_json::json!({
        "user_data": root.display().to_string(),
        // The two flags are echoed because they change what every `action` below MEANS: under
        // `--dry-run` nothing was written, and without `--reset` an edited sample reads `kept`
        // because it was protected rather than because it matched.
        "dry_run": args.dry_run,
        "reset": args.reset,
        "files": report
            .iter()
            .map(|(rel, action)| serde_json::json!({
                // RELATIVE to `user_data`, the same string the shipped-file table is keyed on, so
                // a caller can join it onto `user_data` itself rather than parsing a rendered path.
                "path": rel,
                "action": action.name(),
            }))
            .collect::<Vec<_>>(),
    });
    serde_json::to_string_pretty(&doc).expect("a tree of strings and bools; serialization is total")
}
...
fn print_report(root: &Path, report: &[(&str, Action)], args: &Args) {
    ...
    println!("  vike-cli data seed-demo                    # write it (safe to re-run)");
    ...
    // routes, in the order a user should try them: synthetic (always works), published (needs only
    // HTTPS), datahub (needs a server).
    //
    // ⚠ **The third rung's PRECONDITION changed and the hint has to say so.** It read
    // "# straight from the venue", which was true while `data fetch` called an exchange itself. It
    // asks a DATAHUB now, so on a fresh install — which is exactly who reads this — it fails at the
    // connection unless one is running. Printing it without the caveat would send every new user
    // to a connect error as their first "real data" command, which is worse than not offering it.
    println!();
    println!("...or real market data:");
    println!("  vike-cli data fetch-starter                # published dataset, no venue needed");
    println!("  vike-cli data fetch binance:BTCUSDT:1h --days 180   # needs a datahub (--addr)");
    println!(
        "  vike-cli backtest run --profile {} \\",
        resolve(root, "profiles/backtest.toml").display()
    );
    println!(
        "      --script {}",
