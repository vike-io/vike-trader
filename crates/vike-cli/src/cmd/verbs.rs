...
impl Verb {
    ...
    pub fn to_wire_command(&self) -> Option<WireCommand> {
        match self {
            ...
            Verb::Flatten { venue, symbol } => Some(WireCommand::Flatten {
                venue: venue.clone(),
                symbol: symbol.clone(),
                account: None,
            }),
            Verb::MarketExit { venue } => {
                Some(WireCommand::MarketExit { venue: venue.clone(), account: None })
            }
            Verb::SetState(s) => Some(WireCommand::SetTradingState(*s)),
            Verb::MassCancel { venue, symbol } => Some(WireCommand::MassCancel {
                venue: venue.clone(),
                symbol: symbol.clone(),
                account: None,
            }),
...
mod tests {
    ...
    fn a_conforming_coid_and_every_submit_are_silent() {
        ...
        assert_eq!(
            coid_charset_warning(&WireCommand::MassCancel {
                venue: None,
                symbol: None,
                account: None
            }),
