...
//!
//! # ⚠ There is deliberately NO `--local`, unlike the `backtest` and `sweep` siblings
//!
//! Those two grew one because the standalone engine can do what they ask: `backtest` runs a
//! profile, and `sweep` is the same run with a `[sweep]` table expanded. **The engine has no
//! walk-forward mode at all** — `crates/vike-backtest/src/backtest_cli.rs` has one profile path,
//! which branches on `BacktestProfile::is_sweep` and nothing else, so `vike_backtest::harness::
//! run_walkforward` is reachable from the datahub server and from no binary. A `--local` here
...
fn execute(args: &Args) -> CmdResult<()> {
    let profile_toml = std::fs::read_to_string(&args.profile_path)
        .map_err(|e| format!("cannot read profile {}: {e}", args.profile_path))?;

    // The CONNECT rung — the same classification the `backtest`/`sweep` siblings make.
    let mut client = DatahubClient::connect(&args.addr).map_err(|e| {
        CliError::connect(format!("cannot connect to datahub at {}: {e}", args.addr))
    })?;
