...
verbs:
  ...
  mirror  copy this box's settings files into the settings database (the files keep winning)
...
fn execute(args: &Args, settings_dir: Option<&Path>) -> Result<(), String> {
    ...
    //
    // A read FAILURE is a warning rather than a refusal, and the asymmetry with the file load below
    // is deliberate: during the mirror period the files win, so a store that will not open costs
    // this command nothing but the `db` origin column — where a broken `policy.toml` reported as
    // "defaults" would be the single most misleading thing it could print.
    let store = match settings_dir.map(vike_secrets::read_settings_in) {
        Some(Ok(found)) => found,
        Some(Err(e)) => {
            eprintln!(
                "warning: the settings database could not be read ({e}). Every value below comes \
                 from the files, the environment and the compiled-in defaults, exactly as it would \
                 on a box that has never been mirrored."
            );
            vike_secrets::SettingsSource::NoDatabase { path: e.path }
        }
        None => vike_secrets::SettingsSource::NoDatabase { path: PathBuf::new() },
    };
    ...
    let described = vike_config::describe_with_store(settings_dir, store.rows(), &env)
