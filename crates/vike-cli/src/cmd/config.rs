        ...
        other => {
            eprintln!(
                "vike-cli config: unknown verb '{other}' (expected `show`, `check`, `set`, \
                 `mirror`, `compare`, `adopt`, `recorder`, `activate` or \
                 `deactivate`)\n{USAGE}"
            );
            ExitCode::FAILURE
        }
...
mod tests {
    ...
    fn the_profile_risk_block_renders_in_every_state() {
        ...
        print_profile_risk(
            None,
            &RunProfileRows {
                source: vike_secrets::ProfileRiskSource::Rows(vec![
                    vike_secrets::StoredProfileRisk {
    ...
    fn the_json_document_carries_the_mirrored_ceilings_and_calls_them_unenforced() {
        ...
        let live =
            settings_json(&d, &empty_store(), &[], &[], &UnknownKeys::default(), &active_profile());
