...
pub fn run(mut args: impl Iterator<Item = String>, ctx: Ctx<'_>) -> ExitCode {
    ...
    match verb.as_str() {
        ...
        other => {
            eprintln!(
                "vike-cli config: unknown verb '{other}' (expected `show`, `check` or \
                 `set`)\n{USAGE}"
            );
            ExitCode::FAILURE
        }
...
///
/// VALUES for the run-profile rows. Resolving them means parsing a `RunProfile`, and `vike-cli`
/// links neither `vike-core` nor `vike-exec` on purpose — the `light-consumers` CI lane exists to
/// hold it out of that closure. Inventing a second `[risk]` parser here is exactly the drift this
/// workspace forbids. Naming the ceiling, its act, its absence semantics and the file that holds it
/// is the honest shape; the ORIGIN line says which rows this command read and which it did not, so
/// a blank is never mistaken for an unset ceiling.
fn print_ceilings(filter: Option<&str>) {
    ...
    println!("{:<wn$}  {:<wh$}  VALUE SHOWN ABOVE", "CEILING", "LIVES IN");
    for c in &rows {
        let shown = if c.home.value_shown_by_config_show() {
            "yes"
        } else {
            "no — this command does not read that file"
        };
...
mod tests {
    ...
    fn both_printers_render_everything_without_panicking() {
        ...
        print_human(Section::All, None, &d, &empty_store(), &files, &envs, &unknown);
        print_json(&d, &empty_store(), &files, &envs, &unknown).unwrap();
        // and the empty views
        let none = UnknownKeys::default();
        print_human(Section::Files, None, &d, &empty_store(), &[], &[], &none);
        print_human(Section::Env, None, &d, &empty_store(), &[], &[], &none);
        // …and the filtered-to-nothing env table WITH a complement, the one path that would
        // otherwise return before printing it.
        print_human(Section::Env, None, &d, &empty_store(), &[], &[], &unknown);
        ...
        print_human(Section::All, None, &d, &migrated_store(), &files, &envs, &unknown);
        print_json(&d, &migrated_store(), &files, &envs, &unknown).unwrap();
    }
    ...
    fn the_json_store_object_names_the_database_when_it_answers() {
        ...
        let doc = settings_json(&d, &migrated_store(), &[], &envs, &UnknownKeys::default());
        ...
        let plain = settings_json(&d, &empty_store(), &[], &envs, &UnknownKeys::default());
    ...
    fn the_json_document_discloses_every_pre_trade_ceiling() {
        ...
        let doc = settings_json(&d, &empty_store(), &files, &[], &UnknownKeys::default());
    ...
    fn the_ceilings_block_renders_filtered_and_unfiltered() {
        ...
        print_ceilings(Some("run profile"));
