...
use crate::cmd::runs::failif::{Judgement, Outcome, judge, parse_fail_if};
...
/// The RUNG the same judgement set exits on — the twin of [`render_human`], split out so the words
/// and the number are decided over the same `Vec<Judgement>` and cannot come to disagree about which
/// criterion is which.
///
/// ⚠ Computed with `.max()` over `Outcome`'s DERIVED ordering rather than with a hand-written
/// if-ladder, so the "a breach outranks everything" property lives in exactly one place — the order
/// the variants are declared in.
fn rung(judgements: &[Judgement]) -> Exit {
    match judgements.iter().map(|j| &j.outcome).max() {
        Some(Outcome::Breach) => Exit::Breach,
        Some(Outcome::Unevaluated(_)) => Exit::Empty,
        Some(Outcome::Pass) => Exit::Ok,
        // Unreachable through the verb — `parse_fail_if` refuses an empty expression and `--fail-if`
        // is required — but "no criteria" is emphatically not a pass if it ever became reachable.
        None => Exit::Empty,
    }
}

/// The one-word verdict, off the same rung the process exits on.
fn verdict_word(exit: Exit) -> &'static str {
    match exit {
        Exit::Breach => "breach",
