...
///
/// ⚠ Two spellings of one number already ship: `crates/vike-cli/src/cmd/backtest.rs`'s
/// `RANK_METRICS` teaches `max_dd`, `return` and `equity` while the report's own keys are
