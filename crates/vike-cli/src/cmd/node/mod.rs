//! `vike-cli node` — stand a **vike-tradehub node** up, and attach this machine to one.
//!
...
//! Node.js — the word is [`vike_tradehub_client`]'s own (`WireNodeIdentity`, `--node`), and it is
//! spelled out on first use in every usage line below for exactly that reason.
//!
//! ```text
//! ── on the DAEMON's box ─────────────────────────────────────────────────────────────────────
//! vike-cli node setup       mint both node keys into the store, set the bind address, print the
//!                           two key ids and the restart line. Prints NO key
//! ── on the CLIENT's box (a laptop) ──────────────────────────────────────────────────────────
//! vike-cli node connect     raise the ssh tunnel, take the keys, record the dial address, and
//!                           VERIFY with a real round trip against the node
//! vike-cli node status      what this box is configured to reach, and whether it answers
//! vike-cli node disconnect  close the tunnel `connect` raised
//! ```
//!
...
usage: vike-cli node <subcommand> [options]
...
/// The parsed `node` command line. PURE — the whole grammar is unit-tested below.
...
/// Parse `node`'s own argv tail (everything after the subcommand name). PURE — no I/O, so the whole
...
fn check_flag_scope(a: &Args) -> Result<(), String> {
    ...
    if a.sub == Sub::Connect && a.host.is_none() {
        ...
        return Err("`connect` needs the node's HOST — the ssh destination, e.g. \
                    `vike-cli node connect my-node`. Pass --no-tunnel as well if this client is \
...
pub fn run(args: impl Iterator<Item = String>, ctx: Ctx<'_>) -> ExitCode {
    let args = match parse(args) {
        Ok(a) => a,
        Err(msg) => return exit_for_parse_error("node", USAGE, &msg),
    };
...
/// legacy file when it existed would migrate nothing, ever: every `node setup` would keep the keys
/// where they are and the fallback would never become dead code. Writing forward is what makes the
/// migration finish.
...
pub(crate) fn record_credential_write(ctx: &Ctx<'_>, store: &Path, keys: &[&str]) {
    ...
    if let Err(e) = journal.append(ctx.now_ms, &change) {
        ...
        eprintln!(
            "vike-cli node: ⚠ the keys were saved, but the change journal in {} could not record \
             the write: {e}",
            journal.dir().display()
        );
    }
...
fn set_setting_journalled(
    ...
    let record = |change: Change| {
        ...
        {
            eprintln!("vike-cli node: ⚠ the change journal could not record {key}: {e}");
        }
    };
...
mod tests {
    ...
    fn usage_errors_are_clean_and_help_short_circuits_at_both_levels() {
        ...
        assert!(parsed(&["pair"]).unwrap_err().contains("unknown `node` subcommand 'pair'"));
    ...
    /// the residual the design declared rather than fixed (a reader arriving from QuantConnect reads
    /// "node" as rented capacity; one arriving from nowhere reads it as Node.js), mitigated by prose
    /// on every line rather than by a rename.
    #[test]
