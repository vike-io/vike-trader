//! `vike-cli node connect|status|disconnect` — the CLIENT's side: raise the tunnel, take the keys,
//! record where the node is, and prove the whole chain with a real round trip.
//!
...
//! with the two `vike-cli node setup` printed on the daemon's box is this design's whole answer to
//! *did I reach the right node*, and it costs nothing: the fingerprint is already built, already
...
/// `config.toml`'s CLIENT-side dial key — the fourth of the four addresses that file documents, and
/// the one that makes `--node` optional. `crates/vike-config/src/config.rs`'s `Config::node_addr`
/// sorts all four by whose box holds the value and whether that box listens or dials.
...
/// Run `node connect`. The order is the module doc's: preconditions, tunnel, keys, write, record,
/// verify.
    ...
    let (observe, control) = if args.manual {
        ...
        let Some((key, origin)) = ctx.keys.observe() else {
            return Err(CliError::failed(format!(
                ...
                ctx.keys.observe_absent_message("node connect")
            )));
        };
...
/// Run `node status` — what this box is configured to reach, and whether that answers right now.
///
...
pub(super) fn run_status(ctx: &Ctx<'_>) -> CmdResult<()> {
    ...
    let Some(dial) = ctx.node_addr else {
        return Err(CliError::failed(format!(
            "no {DIAL_ADDR_KEY} on this box, so there is no node to ask. `vike-cli node connect \
             <host>` records one; `--node HOST:PORT` still names one per command."
        )));
    };
    let Some((key, _)) = ctx.keys.observe() else {
        return Err(CliError::failed(ctx.keys.observe_absent_message("node status")));
    };
    verify(dial, key, ctx.keys.control().map(|(k, _)| k))
}

/// Run `node disconnect` — close the tunnel [`run_connect`] raised.
///
...
pub(super) fn run_disconnect(args: &Args, ctx: &Ctx<'_>) -> CmdResult<()> {
    ...
    let Some((host, port)) = recorded.or_else(|| args.port.map(|p| (String::new(), p))) else {
        println!(
            "no tunnel recorded by `vike-cli node connect` on this box — nothing to close. (A \
    ...
    if observe.is_empty() {
        return Err(CliError::usage(
            "no observe key on stdin. --manual reads BOTH keys from stdin, one per line, observe \
             first:\n  printf '%s\\n%s\\n' \"$OBSERVE\" \"$CONTROL\" | vike-cli node connect HOST \
             --manual\nThe values never go on the command line — argv lands in shell history and \
    ...
    if !status.success() {
        return Err(CliError::connect(format!(
            "ssh refused to forward {port} to {host} (exit {}). The usual causes, in order: the \
             local port is already taken by an earlier tunnel (`vike-cli node disconnect` closes \
             the one this command raised), the host is unreachable, or your ssh key is not \
...
fn record_tunnel(ctx: &Ctx<'_>, host: &str, port: u16) {
    ...
    if let Err(e) = wrote {
        eprintln!(
            "vike-cli node: ⚠ the tunnel is up, but it could not be recorded in {} ({e}) — \
             `node disconnect` will not find it; close it by hand.",
            path.display()
        );
    }
}
...
///
/// ⚠ The failure mapping is `crate::cmd::strategy_status`' rule, applied here for the same reason it
...
fn verify(dial: &str, observe: &str, control: Option<&str>) -> CmdResult<()> {
    match vike_tradehub_client::strategy_status(dial, observe.as_bytes()) {
        Ok(status) => {
            ...
            println!(
                "⚠ compare those ids with the ones `vike-cli node setup` printed on the daemon's \
                 box. They match, or you are talking to a different node."
            );
            Ok(())
        }
        Err(e) => Err(match e.kind() {
            io::ErrorKind::PermissionDenied => CliError::failed(format!(
                "the node at {dial} refused the observe handshake: {e}\nThe key this box presented \
                 has id {} — if that is not one of the ids `vike-cli node setup` printed on the \
