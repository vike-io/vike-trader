...
//! written through `vike_secrets::save_credentials_to_store`, the same upsert `super::setup` uses,
...
pub(super) fn run_connect(args: &Args, ctx: &Ctx<'_>) -> CmdResult<()> {
    ...
    // settings database's `node_key` table shadows `node.env`, so the local-key comparison below —
    // which is what refuses to detach a box already attached to some node — must be computed
    // against the rows a reader will actually see, and the write must land in the same store.
    // `super::landed` over `backend_in` names it once; nothing here opens anything to decide.
    ...
    let (observe, control) = if args.manual {
        ...
        // Routed to the store that answers — the same `vike_secrets::backend_in` question `path`
        // above was resolved from, so this command cannot compare against one store and write to
        // another.
...
mod tests {
    ...
    fn status_lines_report_both_planes_without_a_key() {
        ...
        let ctx = Ctx {
            ...
            keys: &keys,
            // `status` reaches neither — they are `ping`'s, and a second context type to omit two
            // unread fields would be more to keep in step than it saves (the `datahub` dispatch
            // arm already argues that for `keys`).
            backtest_addr: None,
            datahub_keys: None,
            now_ms: 0,
        };
