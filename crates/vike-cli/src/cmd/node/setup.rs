//! `vike-cli backend setup` — the DAEMON's side: mint both node keys, set the bind address, and say
//! what to restart.
//!
...
//! 5. **Mint and write the keys**, through `vike_secrets::save_credentials_to_store` — the same
//!    in-place upsert, routed to whichever store answers, which replaces exactly the two rows it
//!    was handed and leaves every other byte, comment and ordering intact.
...
/// Run `backend setup`. See the module doc for why the steps are in this order.
pub(super) fn run_setup(args: &Args, ctx: &Ctx<'_>) -> CmdResult<()> {
    ...
    //
    // ⚠ The DIRECTORY, not the file: since `docs/decisions/0054`'s credential half the settings
    // database's `node_key` table shadows `node.env`, and `open_store` asks
    // `vike_secrets::backend_in` about this directory — the same question the write below asks — so
    // the overwrite refusal cannot be computed against rows nothing reads.
    let dir = super::settings_dir(ctx);
    // The store that answers, named ONCE so every sentence below — the refusal, the write's error,
    // the ledger's cell and the report — says the same thing. `backend_in` is one `is_file` on one
    // path and opens nothing.
    ...
    // ⚠ Routed to the store that ANSWERS, and it is the same `vike_secrets::backend_in` question
    // `open_store` asked above. Naming the FILE here on a migrated box wrote a real change to a real
    // file, journalled it, printed two key ids — and the daemon that has to verify those keys reads
    // the `node_key` table, which still held the old pair.
...
fn rotation_refusal(present: &[&str], store: &str) -> String {
    ...
    format!(
        "{names} already in {store}, and `setup` will not silently replace a key some client is \
         ...
         credential rather than like a rotation. Re-run `vike-cli backend connect` on each of \
         them afterwards.\n\
         Nothing was written."
    )
}
...
fn report(
    ...
    if !rotated.is_empty() {
        lines.extend([
            ...
            "  old pair — a laptop's `vike-cli`, a `vike-app --observe`, a thin container — stops"
                .to_string(),
            "  working at the restart below. Re-run `vike-cli backend connect` on each of them."
                .to_string(),
            String::new(),
        ]);
    }
    ...
    lines.extend([
        ...
        "    vike-cli backend connect <this host> --manual".to_string(),
...
mod tests {
    ...
    fn a_rotation_is_reported_as_one_and_repeats_its_blast_radius() {
        ...
        assert!(rotated.contains("backend connect"), "…with the fix for each client: {rotated}");
