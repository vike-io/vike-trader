...
//!    `VerbScope::Observe` rather than `Handshake` for a strictly weaker disclosure ("a liveness
...
//!    `VerbScope::Observe`, negotiated through a new `FEATURE_*` capability string and **NOT** by a
...
//!    (`vike_datahub_client::node_auth::sign`), so a bump breaks the handshake against every keyed
...
pub(super) fn run_ping(args: &Args, ctx: &Ctx<'_>) -> CmdResult<()> {
    ...
    let mut client = match ctx.datahub_keys {
        // ⚠ `Scope::Observe`, the weakest ceiling that can send anything at all. This verb's
        // heaviest frame is `Request::Ping`, which `vike_datahub_client::required_scope` puts at
        // `VerbScope::Observe` — deliberately, rather than at `Handshake`, so an unauthenticated
        ...
        // nothing at all.
        Some(keys) => DatahubClient::connect_authed(&addr, keys, Scope::Observe),
...
mod tests {
    ...
    fn the_auth_line_separates_what_the_server_demands_from_what_this_connection_got() {
        ...
        keyed.authenticated_as = Some(Scope::Observe);
        ...
        odd.authenticated_as = Some(Scope::Control);
    ...
    fn the_json_document_names_the_address_the_version_and_the_plane() {
        ...
        p.authenticated_as = Some(Scope::Control);
        let doc = json_of(&p);
