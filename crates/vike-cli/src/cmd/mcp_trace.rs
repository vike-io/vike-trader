...
//! `docs/decisions/0054-settings-move-into-one-database.md` now carries the verdict that JSONL beats
//! an embedded database for this workspace's ledgers (0028 settled it first and reads `superseded`).
//! ⚠ The deciding property named here until 2026-09-13 — "an embedded store's connection-lifetime
//! write lock turns a second writer into a failure rather than a wait" — was measured FALSE for
//! SQLite, which locks for a write TRANSACTION and serves WAL readers alongside a writer. What
//! survives and does apply here is cheaper: multi-process APPEND with no schema, no migration and no
//! query to run — two agent sessions on one project
