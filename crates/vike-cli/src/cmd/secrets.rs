...
options:
  ...
  --dry-run       migrate: print what the migration WOULD do and write nothing.
                  set-book: name the STORE, print the row you are about to
                  change and stop.
...
fn parse(mut it: impl Iterator<Item = String>) -> Result<Args, String> {
    ...
    if dry_run && sub != Sub::Migrate && sub != Sub::SetBook && sub != Sub::Confirm {
        return Err("--dry-run applies to `migrate`, `set-book` and `confirm` only".to_string());
    }
    ...
    // for.
    if account_id.is_some() && sub != Sub::SetBook {
        return Err("--id applies to `set-book` only".to_string());
    }
...
mod book_tests {
    ...
    fn the_book_flags_are_refused_off_set_book() {
        for argv in [
            ...
            assert!(err.contains("`set-book` only"), "{argv:?} must be refused: {err}");
        }
    }
