...
//!   otherwise, so a typo cannot write a key nothing will ever read;
...
subcommands:
  ...
  set KEY   upsert ONE key into an EXISTING store, preserving every other byte.
            ...
            KEY must be one this workspace can read; an unknown name is refused
            by name. An absent store is refused too — create it with `template`
...
/// The refusal for a key name this workspace cannot read, with the nearest real names.
///
...
fn unknown_key_message(key: &str) -> String {
    ...
    format!(
        "'{key}' is not a credential key this workspace reads, so setting it would write a line \
         nothing would ever load — {tail}.\n⚠ The BESPOKE key shapes are outside this grid on \
...
mod set_tests {
    ...
    /// The refusal also NAMES the bespoke families it cannot set, rather than leaving an operator
    /// to conclude the command is broken. They are a real gap — `vike_model::credential_keys`'s own
    /// module doc calls it structural — and a gap stated is not a gap hidden.
    #[test]
    fn the_refusal_names_the_shapes_that_are_outside_the_grid() {
        // Composed, not spelled — see the sibling test above for the harvest this avoids.
        let msg = unknown_key_message(&format!("FXCM_{}", "DEMO_USER"));
