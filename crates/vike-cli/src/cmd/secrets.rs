...
subcommands:
  ...
  set KEY   upsert ONE key into an EXISTING store, preserving every other byte.
            ...
            exception and have a command of their own, `vike-cli node setup`,
...
fn run_list(
    ...
    // Since 2026-09-08 they live in `node.env` beside this file, and `vike-cli node status` already
    // owns the question "which node keys resolved, and from where" — it reports the pair, each key's
    // id, and whether the node answers. Listing them here too would be a SECOND answer to one
    // question, which is the failure this workspace gates against elsewhere.
    //
    ...
    if !args.json {
        eprintln!(
            "note: node keys are not in this listing — they live in `{}` beside this store and are \
             reported by `vike-cli node status`",
...
/// route while nothing in this tree generated a key. `vike-cli node setup` generates the two
...
fn unknown_key_message(key: &str) -> String {
    ...
    // — since `vike-cli node setup` landed — are no longer something an operator writes by hand at
    // all. They are the one outside-the-grid family with a real route, so they get their own
    ...
    if let Some(service) = vike_model::credential_keys::platform_key_service(key) {
        // ⚠ The VERB is chosen from the service, never assumed. This arm named `node setup`
        ...
        // ⚠ THE CLIENT LINE IS PER-SERVICE BECAUSE THE COMMAND IS. `vike-cli node connect` exists;
        // there is no `datahub connect` — the datahub's client half is not built. Naming one anyway
        // would be this arm's own defect wearing the other service's clothes: a refusal that is
        // right about refusing and wrong about the route. Each service names only what it has.
        let (verb, fallback, client) = match service {
            ...
            _ => (
                "node",
                "the tradehub daemon",
                "\n→ On a CLIENT box, `vike-cli node connect <host> --manual` writes the pair it \
                 reads from stdin.",
            ),
        };
        return format!(
    ...
    if let Some(readers) = registry_readers(key) {
        return format!(
            "'{key}' IS read by this workspace — `vike_ops::settings` records {readers} reading \
             ...
             own: `vike-cli node setup`.) WHICH store a \
...
mod set_tests {
    ...
    /// `vike-cli node setup` existed the honest advice really was "open the file" — there was no
    ...
    fn the_node_keys_refusal_names_the_command_that_mints_them() {
        ...
        // could name `node setup` unconditionally; with two, a constant would send an operator to
        // the command for a different service — the same defect the arm exists to end, which is why
        // the datahub pair is exercised here rather than trusted.
        for (key, verb, service) in [
            (format!("VIKE_{}", "TRADEHUB_OBSERVE_KEY"), "node", "vike-tradehub"),
            (format!("VIKE_{}", "TRADEHUB_CONTROL_KEY"), "node", "vike-tradehub"),
            ...
            // ⚠ AND IT MAY NOT NAME A COMMAND THAT DOES NOT EXIST. `node connect` is real;
        ...
        assert!(!unknown_key_message(&near).contains("node setup"), "{near}");
    }
