...
//!   [`settable_outside_the_grid`] and [`rotation_owner`] are the two rules that close it, and each
...
//!   saying one when another is true is a lie an operator acts on — [`unknown_key_message`] carries
//!   the measurements;
...
options:
  ...
  --tier TIER     account add/set-tier: paper | demo | live — what this
                  ...
                  policy.toml CEILING of the same name: it is the account a
...
pub fn run(args: impl Iterator<Item = String>, ctx: Ctx<'_>) -> ExitCode {
    ...
    let outcome: CmdResult<()> = refuse_a_database_path(args.file.as_deref())
        ...
        .and_then(|()| match args.sub {
            ...
            Sub::MoveVenueConfig => {
                crate::cmd::secrets_move::run_move(args.dry_run, ctx.settings_dir)
            }
...
fn run_set(args: &Args, ctx: &Ctx<'_>) -> CmdResult<()> {
    ...
    //    why the editor route it replaces is dead on a migrated box.
    //
...
fn unknown_key_message(key: &str) -> String {
    ...
    format!(
        "'{key}' is not a credential key this workspace reads — no `vike_ops::settings` row names \
         ...
         this command CAN write is wider than the template prints: the fixed grid, plus every \
...
const DEPLOYMENT_OWNER: &str = "deployment";
...
/// `docs/decisions/0054`'s credential half: once `settings/db/vike.db` exists the credential FILES
/// are not read at all, so the editor edits a draft nobody loads. Half the store became
/// unwritable by anything, and the refusal still advised the dead route.
///
...
fn settable_outside_the_grid(key: &str) -> Option<(String, Option<String>)> {
...
fn rotation_owner(key: &str, live: &vike_secrets::SecretMap) -> Option<(String, Option<String>)> {
    if vike_model::credential_keys::platform_key_service(key).is_some() {
        return None;
    }
...
mod set_tests {
    ...
    fn a_key_something_reads_is_settable_rather_than_sent_to_an_editor() {
        ...
        let (venue, tier) =
            settable_outside_the_grid(&fx).expect("a bespoke venue login is settable");
        ...
        assert!(
            registry_readers(store_root).is_some(),
            "the control is not a control: it has no row"
        );
        assert!(
    ...
    let resolve = |rec: &ConfirmationRecord| -> Vec<&vike_secrets::Account> {
        rows.iter()
            ...
            .filter(|a| match (rec.label.as_deref(), rec.tier.as_deref()) {
                ...
                // nothing rewrites it, and `vike_model::account_confirmation::ConfirmationRecord`
...
/// (`vike_model::account_confirmation::ConfirmationRecord::key_prefix` says why), and a line built
...
fn confirmation_address_line(
    rec: &vike_model::account_confirmation::ConfirmationRecord,
) -> (String, String) {
    match (rec.label.as_deref(), rec.tier.as_deref()) {
        ...
        // secrets_cli.rs`'s `confirm_keeps_a_record_whose_address_names_no_row` pins it.
...
fn account_action_missing() -> String {
    format!(
...
/// `{VENUE}_SIM_*` credential mints, and it is unrelated to `policy.toml`'s own `paper` CEILING,
/// which is a separate concept that happens to share the word.
///
...
fn tier_missing_message(action: &str) -> String {
    format!(
        "`account {action}` needs --tier, one of: {}. `paper` is a tier, not the policy.toml \
         CEILING of the same name: it is the account a {{VENUE}}_SIM_* credential mints, and \
         it is NOT what arms a venue. A venue with no credential at all still has no row.",
...
fn tier_unknown_message(action: &str, tier: &str) -> String {
    format!(
        "unknown tier '{tier}'. `account {action} --tier` takes one of: {}. `paper` IS one of \
         them — it is the account a {{VENUE}}_SIM_* credential mints, not the policy.toml \
         CEILING of the same name.",
        vike_secrets::ACCOUNT_TIERS.join(" | ")
    )
}
...
mod account_tier_message_tests {
    ...
    fn the_tier_refusals_do_not_deny_a_tier_the_roster_contains() {
        ...
        for action in ["add", "set-tier"] {
            for msg in
                [tier_missing_message(action), tier_unknown_message(action, "not-a-real-tier")]
            {
    ...
    fn the_account_action_list_is_spelled_once() {
        ...
        assert!(
            missing.contains(&rendered),
            "the no-action refusal must render the list: {missing}"
        );
        ...
        assert!(
            second.contains(&rendered),
            "the parse refusal must render the SAME list: {second}"
        );
