...
//! environment first and then the node-key store the daemon itself reads
...
//!
//! The grammar ([`parse`]), both renderers ([`human_lines`], [`json_body`]) and BOTH halves of the
//! failure mapping ([`failure_lines`] for the words, [`failure_exit`] for the rung) are PURE and
//! unit-tested below. The connection itself is
