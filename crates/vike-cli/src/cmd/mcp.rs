...
//! not a mount, and the node has no opinion about one. It reports [`VENUE_CHECK_NONE`] rather than
...
/// made false: that tool and its three siblings dial [`DEFAULT_BACKTEST_ADDR`], and the doc on
/// [`Server::backtest_addr`] is where the split is argued.
///
/// ⚠ It is also a hand copy: `vike_config::DEFAULT_DATAHUB_ADDR` holds the same literal, while
/// the sibling below reaches its default THROUGH that module so the client and the daemon cannot
/// answer differently. Named rather than swept — a const swap is behaviour-adjacent work, not a
/// doc fix.
...
- backtest with no server: `vike-cli backtest run --local --profile run.toml`.
...
fn parse_config(
    ...
    // every other compute dialer has — `--backtest-addr` → the setting → `DEFAULT_BACKTEST_ADDR`.
    let mut backtest_addr = configured_backtest_addr
        .filter(|s| !s.trim().is_empty())
        .map_or_else(|| DEFAULT_BACKTEST_ADDR.to_string(), str::to_string);
...
/// `vike-script`, which carries no DataFusion, sits at `layer = 30`, and was ALREADY a normal
...
fn search_from_args(args: &Value) -> Result<Option<WireSearch>, String> {
...
fn tool_run_paramscan(backtest_addr: &str, args: &Value) -> Result<Value, String> {
...
mod tests {
    ...
    fn an_unknown_profile_is_refused_and_never_widens_to_full() {
        ...
        let refused =
            parse_config(["--profile", "readonly"].map(String::from).into_iter(), None, None)
    ...
    fn the_mcp_surface_advertises_no_credential_writer() {
        ...
        assert!(
            include_str!("secrets.rs").contains(ROUTED),
        ...
        // SCOPE (those verbs require `Scope::Admin`, a third key, and `vike-cli` holds no verb
        ...
        for needle in [ACCOUNT_REQ, ACCOUNT_VERB, ACCOUNT_LIST, ACCOUNT_WRITTEN] {
            assert!(
                ...
                "cmd/mcp.rs names `{needle}` — the MCP surface may not reach the account-\
    ...
    fn rs_sources_under(roots: &[&str]) -> Vec<(String, String)> {
        ...
        while let Some(dir) = stack.pop() {
            let entries = std::fs::read_dir(&dir).unwrap_or_else(|e| {
                panic!("this scan must be able to read {}: {e}", dir.display())
            });
    ...
    fn the_account_verbs_are_unreachable_through_a_helper_that_names_no_wire_type() {
        ...
        const DIALS: [&str; 2] =
            [concat!("node_", "handshake("), concat!("TcpStream", "::connect")];
    ...
    /// property of THIS file": the SCOPE — those verbs want `Scope::Admin`, and `vike-cli` holds
    ...
    ///
    /// ⚠ **Both are exhaustive `match` arms turning a scope into a WORD**, and that is the whole
    /// argument: a label authorises nothing, and neither arm is reachable from a handshake.
    ///
    /// ⚠ **This said "the ONE site" and named one of the two.** The adversarial review that produced
    /// this test had already reported both; the first draft pinned `handshake.rs` and the lane found
    /// `ping.rs` immediately. Recorded because it is the failure this whole family is about — a
    /// list written from memory when the measurement was in hand.
    ///
    /// A THIRD site is not absorbed by adding a row. The refusal below says what to do instead, and
    /// says it because an exception list that grows on contact is the shape of gate this tree has
    /// watched rot elsewhere.
    const SCOPE_LABEL_SITES: [&str; 2] = [
        // `Scope -> &str` for the ping report's table. It RENDERS a scope it was handed; nothing
        // here presents one.
        "vike-cli/src/cmd/node/ping.rs",
        // The client crate's twin of that rendering, beside the handshake rather than inside it.
        "vike-tradehub-client/src/handshake.rs",
    ];
    ...
    /// by `crates/vike-tradehub/src/server.rs`'s `account_admission` under anything but
    /// `Scope::Admin`, so a reach that works has `Scope::Admin` in the file that authenticates. That
    /// is a property of the authorization rather than of a spelling, which is why this evidence
    /// keys on it.
    ///
    /// ⚠ **It still does not hold the CLASS, and saying so is the point.** `Scope::Admin` is itself
    ...
    fn a_dialing_file_does_not_present_the_admin_scope() {
        const ADMIN_SCOPE: &str = concat!("Scope", "::Admin");
