...
- record a live tape: `vike-backend datahub --record <profile>` — part of the data daemon now.
...
impl Server {
    ...
    fn datahub_for_delete(&self) -> Result<DatahubClient, ToolError> {
        ...
        match &self.datahub_keys {
            Some(keys) => {
                DatahubClient::connect_authed(addr, keys, vike_datahub_client::Scope::Write)
                    .map_err(|e| {
