...
daemon now.
- get bars in: `vike-cli data fetch binance:BTCUSDT:1h --days 180` (needs a datahub), or \
...
impl Server {
    ...
    fn node_accounts_epoch(&mut self) -> Option<u64> {
        let snapshot = self.tool_node_snapshot().ok()?;
...
fn venue_verdict(
    ...
    if let Some(account) = account {
        let label = vike_model::account_keys::parse_wire_account(account).map_err(|e| {
            format!(
    ...
    let carriers: Vec<&String> = mounted
        .iter()
        .filter(|m| vike_model::account_keys::label_of_route_key(venue, m).is_some())
        .collect();
    if carriers.len() > 1 {
        return Err(format!(
