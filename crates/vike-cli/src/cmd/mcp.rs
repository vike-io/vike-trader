...
pub fn run(
    ...
    let config = match parse_config(args, configured_backtest_addr) {
        Ok(c) => c,
...
mod tests {
    ...
    fn an_unknown_profile_is_refused_and_never_widens_to_full() {
        ...
        let refused = parse_config(["--profile", "readonly"].map(String::from).into_iter(), None)
        ...
        let ok = parse_config(["--profile", "read-only"].map(String::from).into_iter(), None)
    ...
    fn an_unknown_deny_tool_is_a_usage_error_naming_the_roster() {
        ...
        let err = parse_config(["--deny-tool", "submit-order"].map(String::from).into_iter(), None)
        ...
        let ok = parse_config(["--deny-tool", "submit_order"].map(String::from).into_iter(), None)
