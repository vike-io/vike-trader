...
//! `vike-recorder`" is a fact about the ABSENCE of a tool, so there is no description it could be
...
- record a live tape: `vike-recorder`, a separate binary.
...
/// What [`parse_config`] resolved: the two addresses, the tool scope, and whether a transcript was
...
impl Server {
    ...
    fn call_tool(&mut self, name: &str, args: &Value) -> Result<Value, ToolError> {
        ...
        match name {
            ...
            "run_sweep" => Ok(tool_run_sweep(&self.datahub_addr, args)?),
            "run_walk_forward" => Ok(tool_run_walk_forward(&self.datahub_addr, args)?),
            "list_strategies" => Ok(tool_list_strategies(&self.datahub_addr)?),
    ...
    fn tool_strategy_status(&self) -> Result<Value, String> {
        ...
        match vike_tradehub_client::strategy_status(addr.as_str(), key.as_bytes()) {
            // Serialized VERBATIM, the same convention `vike-cli strategy-status --json` follows:
            // an agent gets the wire shape rather than a second hand-maintained schema that could
    ...
    ///
    /// ⚠ The second command it names is `node status`, NOT `secrets list`. A node key is not in the
    /// credential store any more (`docs/decisions/0051-node-keys-live-in-their-own-store.md`), and
    /// `secrets list` deliberately does not list one — an agent told to run it would read an
    ...
    fn missing_key(&self, name: &str) -> String {
        format!(
            "{name} is set neither in the process environment nor in the node-key store — \
             ...
             `vike-cli node status` to see whether this box holds a node key"
        )
    }
...
/// Run a remote out-of-sample walk-forward on the vike-datahub server — the absorbed `vike-mcp`
...
/// therefore gains no argument and needs none: `crates/vike-datahub/src/server.rs`'s
    ...
    let produced_by = args
        ...
        .ok_or(
            "`produced_by` is REQUIRED on this surface, for every call — a sweep and a fully-named \
             series alike. It is the commit-key PREFIX the rows carry (or the repo-relative path of \
             a declared producer, which resolves to its prefix), and every key of every matched \
             series must carry it or the whole run is refused. A model composes an identity from \
             context that may be stale; a provenance assertion is the one check that fails on a \
             plausible-but-wrong one. There is no override. Read the prefix off `list_series`, or \
             ask the operator.",
        )?
        .to_string();
...
/// `crate::cmd::strategy_status`'s `failure_lines`, which writes the same three diagnoses for a
/// human at a terminal.
///
        ...
        },
        {
            ...
            "inputSchema": {
                ...
                "properties": {
                    ...
                    "produced_by": { "type": "string", "description": "REQUIRED. The commit-key PREFIX every key of every matched series must carry (`panel_bars:`, `pmxt:quote:`), or the repo-relative path of a declared producer, which resolves to its prefix. One key that does not carry it refuses the whole run. There is no override and no --force." },
                    "reason": reason_property(),
                    "confirm": confirm_property(), "preview_token": preview_token_property()
                },
...
mod tests {
    ...
    fn initialize_carries_instructions_that_name_the_surface_beyond_this_one() {
        ...
        for needle in ["vike-recorder", "vike-cli data fetch", "vike-cli backtest --local"] {
    ...
    fn the_instructions_are_scoped_by_the_same_access_the_roster_is() {
        ...
        for (name, text) in [("full", &full), ("read-only", &read_only), ("offline", &offline)] {
            assert!(
                text.contains("vike-recorder") && text.contains("settings/secrets.env"),
    ...
    fn the_instructions_name_only_real_commands() {
        ...
        // `vike-recorder` to a real `[[bin]]` from outside.
