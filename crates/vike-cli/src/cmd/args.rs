...
/// * anything else — a usage **error**: `vike-cli <command>: <msg>` and the usage, on stderr, and
///   the process exits on [`Exit::Usage`]. Because this is the ONE place, converting it moved
///   every subcommand's parse errors onto that rung at once — the parsers themselves are untouched
///   and still return a plain `String`.
///
...
mod tests {
    ...
    /// spawn tests in `tests/help_cli.rs`; the RUNG likewise, in `tests/exit_codes.rs`.
    #[test]
    fn the_help_short_circuit_is_a_success_and_a_real_error_is_the_usage_rung() {
        assert_eq!(
        ...
        assert_eq!(
            ...
            format!("{:?}", ExitCode::from(Exit::Usage)),
            "…and a genuine usage error exits on the USAGE rung, not the run-failure one"
        );
    }
}
