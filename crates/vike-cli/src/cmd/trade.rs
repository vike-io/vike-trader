...
//! verbs (`submit`, `cancel`, `flatten`, `orders`, `positions`, `status`, …) and this dispatches them
...
//! REPL. `status` is the READ ([`crate::cmd::trade_status`] owns it, and its module doc carries the
...
//! wire (`Scope::Observe` cannot send `SetState`); it was at the CONTROL prompt, where the operator
...
const USAGE: &str = "usage: vike-cli trade --node <host:port> [--yes]          (the REPL)\n\
                     \x20      vike-cli trade status --node <host:port> [--json]   (read the mode \
                     + what is mounted)\n\
                     \x20      vike-cli trade halt   --node <host:port> [--yes]    (mode -> \
                     Halted)\n\
                     \x20      vike-cli trade resume --node <host:port> [--yes]    (mode -> \
                     Active)\n\
                     \n\
                     no --json on the REPL: it is interactive, for a human. The MACHINE surface \
                     over the same node\nis `vike-cli mcp`, which speaks stdio JSON-RPC and \
                     exposes the same verbs as tools.";
...
pub fn run(
    ...
    // construction and could never have become a subcommand there. An unrecognised leading word is
    // handed BACK to the REPL parser, which names it in a usage error, rather than being guessed at.
    let mut rest = args;
    let first = rest.next();
    match first.as_deref() {
        Some("status") => return crate::cmd::trade_status::run(rest, keys),
...
/// registry failure the node could still answer ([`crate::cmd::trade_status`]'s
...
fn run_status(session: &Session<'_>) {
    ...
    match vike_tradehub_client::strategy_status(session.node.as_str(), key.as_bytes()) {
        Ok(status) => {
            for line in crate::cmd::trade_status::registry_lines(&status) {
                println!("{line}");
            }
        }
        // Printed, not returned: a REPL has no exit code, and the mode above already landed.
        Err(e) => {
            for line in crate::cmd::trade_status::failure_lines(&session.node, &e) {
                println!("{line}");
            }
        }
    }
...
fn run_set_mode_once(
    ...
    let control = match RemoteControlHandle::connect(node.as_str(), key.as_bytes()) {
        ...
        Err(e) => {
            ...
            // on), not a socket to retry — the `trade_status` ladder's rule, applied to the write
            // side.
            return match cause {
...
fn run_set_setting(
    ...
    // THE TYPED CONFIRM (`WireCommand::SetSetting`'s contract). It is asked for the POLICY plane
    // and nothing else, and it is asked EVEN UNDER `--yes` — see this module's doc.
    //
    // ⚠ Keyed on the KEY's section word, not on the `file` argument beside it. The node decides the
    // same way (`crates/vike-tradehub/src/server.rs`'s `apply_set_setting`), through the same
    // `vike_config::is_policy_plane_key`, so the two cannot answer differently — and neither
    // answer depends on a settings FILE NAME surviving the vocabulary move
    // (`docs/decisions/0057-the-seven-settings-files-answered-one-at-a-time.md`).
    let confirm = if vike_config::is_policy_plane_key(key) {
...
// ⚠ `fn is_policy_file(file: &str)` stood here — a case-insensitive match on `"policy"` /
// `"policy.toml"`, deciding the retype prompt from the FILE NAME an operator typed. It is deleted
// and the decision moved onto the key's SECTION WORD (`vike_config::is_policy_plane_key`), for the
// reason that predicate's doc gives: a file-name rule is a safety check a vocabulary move disarms
// in silence, and this one accepted two spellings of a word that will not be on the wire forever.
//
// The old doc's "errs SAFE in both directions" argument does not have to be made any more, and
// that is the improvement: the CLI and the NODE now reach the same answer through the same
// function over the same input, so there is no approximation to be safe about.
...
///
/// The WORDING is [`crate::cmd::trade_status::mode_lines`]', not this file's — three surfaces render
...
fn print_state(snap: &WireSnapshot) {
    let mut lines =
        crate::cmd::trade_status::mode_lines(snap.trading_state, snap.fault.as_deref()).into_iter();
...
mod tests {
    ...
    fn parse_status_reads_and_halt_resume_write() {
        ...
        for (line, want) in
            [("halt", WireTradingState::Halted), ("resume", WireTradingState::Active)]
        {
    ...
    /// **Which KEY gets the retype prompt**, and — the half that matters — that the prompt still
    /// ARMS after the re-key.
    ///
    /// ⚠ This was `only_the_policy_file_asks_for_a_typed_confirm`, over
    /// `is_policy_file("policy.toml" | "policy" | "POLICY.TOML" | "Policy")`. Those spellings are
    /// gone with the predicate: the decision is the key's SECTION WORD now, which is the one thing
    /// a dotted key has always carried and the one thing the vocabulary move cannot take away.
    /// Keeping the old test would have pinned a function that no longer decides anything.
    #[test]
    fn only_a_policy_plane_key_asks_for_a_typed_confirm() {
        // ⚠ The leverage key is ASSEMBLED rather than written out, and the test demonstrates its
        // own reason: `crates/vike-config/tests/policy_is_consumed.rs` scans the tree for
        // `policy.<field>` mentions to find a claimed-unconsumed ceiling's readers, and a literal
        // here made THIS test read as `Policy::max_leverage`'s consumer — a gate in another crate
        // red with a message about something else. Measured, once.
        let leverage = format!("policy.{}", "max_leverage");
        for key in [
            "policy.max_notional_per_order",
            leverage.as_str(),
            // ⚠ The ARMING tables too. `vike_config::typed_confirm_reason` EXEMPTS these (0055's
            // ruling), and this surface does not adopt that predicate — adopting it would relax a
            // live contract the node enforces server-side. Pinned so the difference is deliberate
            // rather than discovered.
            "policy.venues.binance",
            "policy.accounts.binance.ALT",
            // A bare section with no key under it answers YES rather than being a special case —
            // `set_setting` refuses it by name long before a confirm matters, so a needless
            // prompt and a silent skip end in the same refusal, and the simpler rule is the one
            // with no arm to get wrong. `vike_config::typed_confirm_reason` says the same.
            "policy",
        ] {
            assert!(vike_config::is_policy_plane_key(key), "{key} is in the policy plane");
        }
        for key in [
            "config.tradehub_addr",
            "flags.tradehub_live",
            "preferences.log_level",
            // Not a section word this loader knows — no confirm, and the node refuses the write.
            // ⚠ The near-miss is the point: the deleted file-name rule compared whole strings, so
            // it too rejected this — but a PREFIX rule would accept it, and a prefix rule is the
            // obvious way to write this function.
            "policies.max_leverage",
        ] {
            assert!(!vike_config::is_policy_plane_key(key), "{key} is not in the policy plane");
        }
    }
