...
//! vike-cli data hist fetch-starter [--store DIR] [--json]
//! vike-cli data hist seed-demo [--store DIR] [--json]
//! vike-cli data hist export <SPEC> --out FILE [--from LABEL] [--to LABEL] [--store DIR] [--json]
//! vike-cli data hist ls     [--addr HOST:PORT] [--kind K] [--venue V] [--name N] [--gaps]
//!                        [--class] [--json]
...
const RETIRED_SPELLINGS: &[(&str, &str)] = &[
    ...
    ("fetch-starter", "data hist fetch-starter"),
    ("seed-demo", "data hist seed-demo"),
...
fn parse(
    ...
    // `hist` is the only group this phase builds; `realtime`, `catalog` and `source` are named in
    // the refusal below so that an operator who types one learns it is coming rather than that it
    // is wrong.
    //
    ...
    let verb = match first.as_str() {
        ...
        "realtime" | "catalog" | "source" => {
            return Err(format!(
                "`data {first}` is designed but not built yet — this phase ships `data hist` only.                  See docs/superpowers/specs/2026-09-20-cli-data-surface-design.md §7."
            ));
        }
        ...
        other => {
            if let Some((_, now)) = RETIRED_SPELLINGS.iter().find(|(was, _)| *was == other) {
                return Err(format!(
                    "`data {other}` moved: it is `{now}` now. Every `data` verb lives in a GROUP                      (hist | realtime | catalog | source) — the plane got too wide to be flat."
                ));
            }
            return Err(format!(
    ...
    let sub = match verb.as_str() {
        ...
        // The two verbs the group split RENAMED, caught here so the message can name the new
    ...
    let (spec, window) = match sub {
        Sub::Fetch => {
            let spec = spec.ok_or(
                "fetch needs a spec: VENUE:SYMBOL:INTERVAL, e.g. `vike-cli data hist fetch \
        ...
        Sub::Coverage => {
            ...
            refuse_foreign_flags(
                ...
                "that flag fetches one SERIES' gap ranges in epoch-ms, while a `coverage` row is \
                 one INSTRUMENT's kinds lined up in UTC-day indices — `vike-cli data hist ls --gaps` \
        ...
        Sub::TapeHealth => {
            ...
            refuse_foreign_flags(
                ...
                "that flag reports what is MISSING, and this verb reports what is PRESENT and \
                 self-contradictory — a gap is absence, which `vike-cli data hist ls --gaps` and \
        ...
        Sub::Universe => {
            refuse_foreign_flags(
                ...
                "that flag annotates one series' recorded span, while a `universe` row is one \
                 ...
                 `vike-cli data hist ls --gaps`; this verb answers whether it was there at all",
            )?;
...
fn refuse_an_account_kind(kind: &str) -> Result<(), String> {
    ...
    let extra = if kind == "exec_funding" {
        " — and for this kind a scoped delete is not merely unimplemented but UNEXPRESSIBLE: the          series partitions on (venue, coin) while the account rides in the commit key, so one          series holds every account's payments for that coin and `rm` deletes by series"
    } else {
        ""
    };
    Err(format!(
        "`{kind}` is ACCOUNT data, not market data, and `data hist rm` refuses it{extra}.
         Your fills, orders, funding payments and equity belong to `vike-cli account`, which is          not built yet. The market funding RATE is unaffected and is not an account kind — it is          `--kind bar --interval funding`."
    ))
}
...
/// `data hist list` — one `inventory()` round trip, filtered client-side, plus one `series_gaps` probe
...
fn refuse_an_account_kind_filter(args: &Args) -> Result<(), String> {
    ...
    Err(format!(
        "`{kind}` is ACCOUNT data, not market data. This plane does not serve it.\n\
         Your fills, orders, funding payments and equity belong to `vike-cli account`, which is \
         not built yet. The market funding RATE is not an account kind — it is \
         `--kind bar --interval funding`."
    ))
}

/// The `§9.3.2` disclosure: what this listing WITHHELD, and why, in the operator's own terms.
///
...
fn account_exclusion_note(
    ...
    Some(format!(
        "note: {count} series of kind{} {} {is_are} in this store and {is_are} not listed here — \
         they are account data (`vike-cli account`, not built yet).",
...
/// `vike-cli data hist ls --gaps` is the per-series view.
...
mod tests {
    ...
    fn every_named_replacement_is_a_spelling_that_parses() {
        for (_, now) in RETIRED_SPELLINGS {
            ...
            parse_of(&argv)
                .unwrap_or_else(|e| panic!("the replacement named for a retired verb, {now:?},                                             does not parse: {e}"));
        }
    }
    ...
    /// The groups that are DESIGNED but not built say so, rather than reading as typos. An
    /// operator who types `data realtime` has read the spec; telling them "unknown" would be false.
    #[test]
    fn a_designed_but_unbuilt_group_says_so_rather_than_unknown() {
        for group in ["realtime", "catalog", "source"] {
            let err = parse_of(&[group]).unwrap_err();
            assert!(
                err.contains("not built yet"),
                "`data {group}` is designed — the refusal must say so: {err}"
            );
    ...
    fn each_subcommand_parses() {
        ...
        assert_eq!(parse_of(&["hist", "seed-demo"]).unwrap().sub, Sub::SeedDemo);
    ...
    fn seed_demo_refuses_a_window_and_a_spec() {
        assert!(parse_of(&["hist", "seed-demo", "--days", "30"]).unwrap_err().contains("--days"));
        assert!(parse_of(&["hist", "seed-demo", "--from", "0"]).unwrap_err().contains("--from"));
        assert!(
            parse_of(&["hist", "seed-demo", "binance:BTCUSDT:1h"]).unwrap_err().contains("no spec")
        );
        // …and the one flag that DOES apply to it still parses.
        assert_eq!(
            parse_of(&["hist", "seed-demo", "--store", "/s"]).unwrap().store.as_deref(),
            Some("/s")
        );
    }
    ...
    fn the_engine_argv_is_the_translation() {
        ...
        assert_eq!(engine_argv(&parse_of(&["hist", "seed-demo"]).unwrap()), ["data", "seed-demo"]);
        assert_eq!(
            engine_argv(&parse_of(&["hist", "seed-demo", "--store", "/s"]).unwrap()),
            ["data", "seed-demo", "--store", "/s"]
        );
        assert_eq!(
            engine_argv(&parse_of(&["hist", "fetch-starter", "--store", "/s"]).unwrap()),
    ...
    fn export_and_fetch_starter_carry_their_own_grammar() {
        ...
        assert!(parse_of(&["hist", "fetch-starter", "d:S:1h"]).unwrap_err().contains("no spec"));
        assert!(
            parse_of(&["hist", "fetch-starter", "--days", "7"]).unwrap_err().contains("--days")
        );
        assert_eq!(
            parse_of(&["hist", "fetch-starter", "--store", "/s"]).unwrap().store.as_deref(),
            Some("/s")
        );

        // `--out` belongs to `export` alone, and is refused elsewhere by name rather than ignored.
        for sub in ["fetch-starter", "seed-demo", "ls", "rm"] {
            let err = parse_of(&["hist", sub, "--out", "s.parquet"]).unwrap_err();
    ...
    fn json_parses_on_every_write_subcommand_and_is_not_forwarded() {
        for argv in [
            ...
            vec!["hist", "seed-demo", "--json"],
            vec!["hist", "fetch-starter", "--json"],
        ...
        assert!(!parse_of(&["hist", "seed-demo"]).unwrap().json, "absent means absent");
        // A value is refused rather than swallowed — the same `no_value` rung every other
        // valueless flag in this crate uses.
        assert!(parse_of(&["hist", "seed-demo", "--json=1"]).unwrap_err().contains("--json"));
    }
    ...
    fn the_json_document_carries_the_request_the_engine_and_its_report() {
        ...
        let seed = parse_of(&["hist", "seed-demo", "--json"]).unwrap();
        let doc: serde_json::Value =
    ...
    fn the_engine_flag_is_consumed_here_and_not_forwarded() {
        let a = parse_of(&["hist", "seed-demo", "--engine", "/opt/backtest"]).unwrap();
    ...
    fn the_selector_flags_name_both_verbs_that_take_them() {
        for sub in ["fetch", "seed-demo", "ls", "coverage"] {
            let err = parse_of(&["hist", sub, "--symbol", "S"]).unwrap_err();
    ...
    fn the_read_booleans_take_no_value() {
        assert!(parse_of(&["hist", "ls", "--gaps=1"]).unwrap_err().contains("--gaps"));
    ...
    fn the_class_flag_belongs_to_list_alone_and_every_refusal_says_why() {
        ...
        for args in [
            ...
            &["hist", "seed-demo", "--class"][..],
            &["hist", "fetch-starter", "--class"][..],
    ...
    fn the_new_read_verbs_refuse_the_absence_flags_and_name_the_verbs_that_answer_absence() {
        for sub in ["health", "universe"] {
            for flag in ["--gaps", "--partial-only"] {
                let err = parse_of(&["hist", sub, flag]).unwrap_err();
                assert!(err.contains(flag), "{sub} {flag}: {err}");
                assert!(err.contains("hist ls --gaps"), "{sub} {flag} must name it: {err}");
            }
        }
    }
    ...
    fn a_datahub_flag_on_a_write_verb_is_refused_and_names_the_read_half() {
        for (argv, flag) in [
            (vec!["hist", "seed-demo", "--gaps"], "--gaps"),
            (vec!["hist", "seed-demo", "--venue", "binance"], "--venue"),
    ...
    fn each_read_verb_refuses_the_others_flag_with_the_reason() {
        let err = parse_of(&["hist", "ls", "--partial-only"]).unwrap_err();
        assert!(err.contains("--partial-only") && err.contains("coverage"), "{err}");

        let err = parse_of(&["hist", "coverage", "--gaps"]).unwrap_err();
    ...
    fn list_json_carries_the_raw_identity_beside_the_resolved_name() {
        ...
        let args = parse_of(&["hist", "ls", "--venue", "poly", "--gaps", "--json"]).unwrap();
        let doc: serde_json::Value =
