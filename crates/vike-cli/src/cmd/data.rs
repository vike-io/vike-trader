...
options:
  ...
  --from LABEL    fetch/export: window start — epoch-ms, or YYYY-MM-DDTHH. On `fetch` it
                  needs a matching --to; on `export` it stands alone
  --to LABEL      fetch/export: window end, same spellings and the same asymmetry
  --out FILE      export: the Parquet file to write. REQUIRED there, refused elsewhere
  --store DIR     the WRITE half and rm: the hist-store root to act on
  --engine PATH   the WRITE half and rm: the standalone engine to run, instead of
                  searching <project>/bin, this executable's directory, and PATH
  ...
  --json          one JSON object on stdout describing the run. For the WRITE half
                  ...
                  rather than this side's rendering of them. For rm: the plan, the
                  outcome, and the provenance refusal when there is one
  -h, --help      this message";
...
const SUBCOMMANDS: &[Sub] =
    &[Sub::Fetch, Sub::FetchStarter, Sub::SeedDemo, Sub::Export, Sub::List, Sub::Coverage, Sub::Rm];
...
fn parse(mut it: impl Iterator<Item = String>) -> Result<Args, String> {
    ...
    // subcommand, in one place, for the same reason.
    if sub != Sub::Rm {
        refuse_foreign_flags(
            sub,
            &[
...
fn execute(args: &Args, project_root: Option<&Path>, keys: Option<&NodeKeys>) -> CmdResult<()> {
    match args.sub {
        ...
        Sub::Fetch | Sub::FetchStarter | Sub::SeedDemo | Sub::Export => {
            execute_engine(args, project_root)
        }
...
mod tests {
    ...
    fn the_engine_argv_is_the_translation() {
        ...
        assert_eq!(
            engine_argv(&days),
            ["data", "fetch", "binance:BTCUSDT:1h", "--days", "180", "--store", "/s"]
        );

        let range = parse_of(&["fetch", "okx:BTC-USDT:1h", "--from", "0", "--to", "100"]).unwrap();
        assert_eq!(
            engine_argv(&range),
            ["data", "fetch", "okx:BTC-USDT:1h", "--from", "0", "--to", "100"]
        );
    ...
    fn the_json_document_carries_the_request_the_engine_and_its_report() {
        ...
        assert_eq!(doc["engine_argv"][1], "fetch");
