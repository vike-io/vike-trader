...
options:
  ...
  --addr H:P      every READ verb and rm: the datahub to ask (default 127.0.0.1:7878).
  ...
  --json          one JSON object on stdout describing the run. For the WRITE half
                  ...
                  rather than this side's rendering of them. For
...
fn engine_argv(args: &Args) -> Vec<String> {
    ...
    match args.sub {
        ...
        Sub::List | Sub::Coverage | Sub::TapeHealth | Sub::Universe | Sub::Rm | Sub::Repair => {}
    }
    if let Some(store) = &args.store {
        argv.push("--store".to_string());
        argv.push(store.clone());
