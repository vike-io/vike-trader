...
fn execute(args: &Args, settings_dir: Option<&Path>) -> Result<String, String> {
    ...
    let seal = vike_secrets::read_settings_in(dir)
        .ok()
        .and_then(|s| s.adoption().map(|a| (a.setting_rows, a.arming_rows, a.adopted_at.clone())));
    ...
    if args.dry_run {
        return Ok(format!(
            "would mirror {counts}{recorder_counts} into {} — NOTHING WAS WRITTEN.{}{}",
