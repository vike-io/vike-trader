...
/// # Errors
///
/// A `String` naming the file and what was wrong with it — including the CROSS-KIND refusal, for
/// the reason `crate::cmd::config_mirror_profile::plan`'s own doc gives: every half is planned
/// before any half is written, so a `--dry-run` may not report *would mirror* for a name
/// `store_profile` is going to refuse. The words are the store's own
/// (`vike_secrets::profile_store::ProfileError::NameHeldByAnotherKind`), constructed rather than
/// re-spelled.
pub fn plan(settings_dir: &Path, file: &Path, name: &str) -> Result<Mirrored, String> {
    ...
    // ⚠ THE CROSS-KIND REFUSAL, one step before the store's own — `profile.name` is ONE namespace
    // across all three kinds, so a recorder body stored under a name a `run` or `daemon` row holds
    // would delete that body and inherit its `active` bit.
    if let Some(p) = &stored_now
        && let Some(held) = p.by_name(name)
        && held.row.kind != ProfileKind::Recorder
    {
        return Err(vike_secrets::profile_store::ProfileError::NameHeldByAnotherKind {
            name: name.to_string(),
            held: held.row.kind.sql_word().to_string(),
            wanted: ProfileKind::Recorder,
        }
        .to_string());
    }
    let already_active =
        stored_now.and_then(|p| p.active(ProfileKind::Recorder).map(|s| s.row.name.clone()));
