...
fn inspect(
    ...
    //
    // A store that will not OPEN is a WARN rather than a FAIL, matching the boot's own disposition:
    // during the mirror period the files win, so an unreadable store costs this box nothing it
    // resolves — but it is exactly the kind of silence this command exists to break.
    let store = match settings_dir.map(vike_secrets::read_settings_in) {
        Some(Ok(found)) => {
        ...
        Some(Err(e)) => {
            findings.push(finding(
                ...
                Level::Warn,
                format!(
                    "{e} — every setting below comes from the files, the environment and the \
                     compiled-in defaults, exactly as it would on a box that has never been \
                     mirrored"
                ),
            ));
            vike_secrets::SettingsSource::NoDatabase { path: e.path }
        }
        None => vike_secrets::SettingsSource::NoDatabase { path: std::path::PathBuf::new() },
    };
    let mut resolved_flags: Option<vike_config::Flags> = None;
    match vike_config::describe_with_store(settings_dir, store.rows(), env) {
        Ok(described) => {
