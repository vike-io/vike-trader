...
fn inspect(
    ...
    match &store {
        ...
        Some(Err(e)) => {
            findings.push(finding(
                ...
                Level::Fail,
                format!(
                    "{e} — this box's settings SOURCE could not be opened, so nothing here can \
                     tell whether its ceilings come from the database or from the four files. \
                     Repair with `vike-cli config mirror`, or step back to the files with \
                     `vike-cli config adopt --undo`; both still run in this state"
                ),
            ));
        }
        None => {}
    ...
    let adopted = source.adoption().is_some();
    ...
    // is what licenses `apply_adopted_rows` marking rather than refusing. The `Some(Ok(found))` arm
        ...
        && let (Some(found), Some(seal)) = (src.rows(), src.adoption())
    {
        ...
        if found.settings.is_empty() && found.arming.is_empty() {
            findings.push(finding(
                "settings seal",
                Level::Fail,
                "this box is ADOPTED and carries NO settings rows at all, so every value it \
                 resolves is a compiled-in default — no ceiling, and every venue capped `paper` \
                 with nothing recording that an arming was ever stated. `vike-cli config adopt` \
                 refuses to create this state; reaching it means the rows were erased after the \
                 seal. Repair with `vike-cli config mirror`, or step back to the files with \
    ...
    // `Fail` rather than `Warn` on an ADOPTED box, because there the row is the only copy of
    // whatever it holds; `Warn` otherwise, where the files still answer.
        ...
        && let Some(rows) = found.rows()
    {
        ...
        if !unknown.is_empty() {
            findings.push(finding(
                "settings rows",
                if adopted { Level::Fail } else { Level::Warn },
                format!(
                    "{} row(s) carry a section outside `{:?}` ({unknown:?}) and are read as \
                     ...
                     altered schema produced these. Re-run `vike-cli config mirror` to re-derive \
                     every row from the files.",
                    unknown.len(),
    ...
    // ⚠ **DRIFT — a `Level::Warn`, deliberately, and this is where the choice is paid for.** On an
    // adopted box a present settings file is INERT, which reduces no authority
    // (`docs/decisions/0013-degrade-vs-refuse.md`'s question 2) and can only mislead a human. A
    // `Fail` here would be worse than the silence it removes for a reason this command is uniquely
    // exposed to: it is `vike-datahub`'s `ExecStartPre=`, so it would convert an inert hand edit
    // into a start-blocker at the 06:40 unattended-upgrade restart — and the remedy such a refusal
    // teaches is `rm policy.toml`, which on an adopted box removes NOTHING while the operator
    // believes it took a ceiling off.
    if adopted
        && let Some(dir) = settings_dir
        && let Some(rows) = source.rows()
    {
        let drift = vike_config::compare_sources(dir, rows);
        if !drift.files_present.is_empty() {
            let detail = if drift.keys.is_empty() && drift.unreadable.is_empty() {
                format!(
                    "{} still on disk and INERT — this box resolves from the database, so editing \
                     one changes nothing. They agree with the rows today; `vike-cli config mirror` \
                     files a change, `vike-cli config set` writes both",
                ...
                let keys: Vec<String> = drift
                    ...
                    .collect();
                format!(
                    "{} still on disk and INERT, and they DISAGREE with what this box is running \
                     on: {}{}. Run `vike-cli config compare`",
                    drift.files_present.join(", "),
                    keys.join("; "),
                    if drift.unreadable.is_empty() {
                        String::new()
                    } else {
                        format!(" [{}]", drift.unreadable.join("; "))
                    }
                )
            };
            findings.push(finding("settings drift", Level::Warn, detail));
        }
    }
...
/// which is the shape `crates/vike-ops/tests/settings_registry.rs`'s `CREDENTIAL_STORE_PIN`
...
/// this verb exists.
///
/// ⚠ **It reports the store that ANSWERS, which since `docs/decisions/0054`'s credential half is
/// not necessarily a file.** Naming the file unconditionally was a confident wrong answer on a
/// migrated box: the path was the shadowed store's and the key count was read out of it.
///
...
fn store_findings(
    ...
    // ⚠ **The STORE THAT ANSWERS, not the file this command can name.** It used to hand
    // `vike_secrets::resolve` a path it had built itself, which is the FILE arm by definition — so
    // on a migrated box this row printed the `secrets.env` path beside a key count read out of a
    // store nothing reads any more. Two wrong halves that read as one confident answer: the path was
    // the dead store's and the count was the dead store's.
    //
    ...
    // `crates/vike-ops/tests/settings_registry.rs`'s `CREDENTIAL_STORE_PIN` ratchets down, on that
    ...
    out.push(match &resolved.source {
        vike_secrets::Source::File(p) => finding(
            "credential store",
            Level::Ok,
            format!("{} — present, {} key(s)", p.display(), resolved.secrets.len()),
        ),
    ...
    // ⚠ The file the database now shadows. It was returned as data by `vike-secrets` and printed by
    // NOTHING, which made the whole operator-facing mitigation for the move unreachable: every
    // runbook in this tree says *edit `<project>/settings/secrets.env`*, and after a migration that
    // edit changes nothing while looking exactly like it worked. `config check` is the verb an
    // operator runs when something is already wrong, so it is one of the two places this must land.
    if let Some(s) = &resolved.shadowed {
        out.push(finding("credential store", Level::Warn, s.to_string()));
    }
    out
}
