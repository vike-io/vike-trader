...
// Epoch-ms per UTC day — the `GONE` column's unit. ⚠ IMPORTED, where this file used to declare
// `const DAY_MS: i64 = 86_400_000;` "for the reason `super::tape_health`'s own copy is". That
// reason only ever covered half the question: `vike-data`, where the value lives twice, IS a
// DEV-dependency here and cannot be imported — but `vike-model` is a normal one and has exported
// `MS_PER_DAY` all along. All three copies in this module tree are gone together, because the one
// that survived would be the one the next submodule imitated.
...
pub(super) fn classify(
    ...
    let gone_days = if left_inside {
        store_last_ts.map(|store_last| store_last.saturating_sub(last) / MS_PER_DAY)
    } else {
        None
    };
