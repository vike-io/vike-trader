...
///
/// A local copy of the constant `crates/vike-data/src/coverage.rs`'s `DAY_MS` and
/// `crates/vike-data/src/quality.rs`'s `DAY_MS` each declare for themselves, and it has to be:
/// that crate is a DEV-dependency here (see the module doc's type wall), so the value cannot be
/// imported, and `vike-model` — which this crate does link — exports no day constant. The two
/// consumers below are the UTC-day floor for the partition-count check and the divides-a-day test
/// that bounds the off-grid check; both would be wrong with any other number, so it is spelled
/// once here rather than at either site.
...
fn span_days(facts: &SeriesFacts) -> Option<i64> {
    let first_day = facts.first_ts.div_euclid(DAY_MS);
    let last_day = facts.last_ts.div_euclid(DAY_MS);
