...
pub enum Exit {
    ...
    /// a document naming every criterion that passed and failed, never a bare number"). Its
    /// producers are the two `rung` functions — `crates/vike-cli/src/cmd/runs/gate.rs`'s for the
    ...
    /// `crates/vike-cli/src/cmd/config_check.rs`'s `Report::failed` collapses findings. (This said
    /// "its ONE producer" while there was one; the SET is the claim, and it is stated here rather
    /// than counted.)
    ///
