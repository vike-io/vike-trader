...
//! either today — their constructors are called from nowhere outside this module's own tests, so
//! `crates/vike-cli/tests/exit_codes.rs` has no invocation to write a case against. (It DOES assert
//! the two rungs below, which are live.)
//!
...
//!
//! [`Exit::Breach`] and [`Exit::Empty`] belong to the backtest plane's judging verbs
...
pub enum Exit {
    ...
    Venue = 5,
    /// A DECLARED THRESHOLD WAS BREACHED — the product of `vike-cli backtest gate`, and the one
    /// rung on this ladder that is a RESULT rather than a failure. The verdict document goes to
    /// stdout in full; this number is its summary, so a CI step can branch without parsing it.
    ///
    ...
    /// a document naming every criterion that passed and failed, never a bare number"). Its ONE
    /// producer is `crates/vike-cli/src/cmd/runs/gate.rs`'s `rung`, which collapses a rendered
    /// verdict onto this rung the way `crates/vike-cli/src/cmd/config_check.rs`'s `Report::failed`
    /// collapses findings.
    ///
    ...
    /// through `crates/vike-cli/src/cmd/runs/gate.rs`'s `rung` when every criterion was
    /// unevaluated, and through [`CliError::empty`] when there was no document to judge at all.
    Empty = 7,
}
