...
const COMMANDS: &[(&str, &str)] = &[
    (
        ...
        "compute a strategy over history and judge what came back: `backtest run` runs one on a \
         remote vike-datahub server (or --local), and the PROFILE says what — a [paramscan] grid \
    ...
    //
    // `report`'s half is still open-ended: its wire verb shipped, its RENDERER did not.
    //
...
fn dispatch(command: &str, args: impl Iterator<Item = String>, resolved: &Resolved) -> ExitCode {
    match command {
        ...
        // `Scope::Control` — they compile client-supplied Rhai on the server — so it carries the
