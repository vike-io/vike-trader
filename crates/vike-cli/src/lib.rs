...
const COMMANDS: &[(&str, &str)] = &[
    ...
    ("data", "get market data into the hist store: fetch real bars, or seed the demo tape"),
...
struct Resolved {
    ...
    ///
    /// Reaches `secrets` alone, and only its `set` arm, which appends one
    /// `vike_model::change_journal` `credential_write` record beside the store write. `None` (no
    /// project above the working directory) means the write still happens and NOTHING is recorded
    /// — an append-only ledger in a guessed directory is worse than a counted absence, the same
    /// rule `vike_boot::journal_boot_settings` follows.
...
fn dispatch(command: &str, args: impl Iterator<Item = String>, resolved: &Resolved) -> ExitCode {
    match command {
        ...
        "backtest" => cmd::backtest::run(args, resolved.project_root.as_deref()),
        "sweep" => cmd::sweep::run(args, resolved.project_root.as_deref()),
        ...
        "data" => cmd::data::run(args, resolved.project_root.as_deref()),
        "config" => {
