...
//! performs it for the node-facing arms (`trade`/`mcp`/`strategy-status` — the last a READ verb
//! that uses just the observe half).
//!
...
//! first half is simply no longer true — `backtest`, `sweep`, `walkforward`, `data` and `mcp` all
//! dial a datahub that may require authentication. The second half was written when nothing in
//! this binary could authenticate to one, and a `backtest --addr` is a NODE-FACING invocation, the
//! category that sentence already excepted. So the rule now reads: **an arm that can dial a node
//! may open the store; one that cannot, may not.** `config`, `secrets`, `init` and `indicators`
...
struct Resolved {
    ...
    /// `crates/vike-config/src/config.rs`'s `Config::node_addr` sorts all four of that file's
    /// addresses by whose box holds the value and whether that box listens or dials.
    ///
    /// Reaches the `node` arm alone today. The three verbs that still REQUIRE `--node`
    /// (`strategy-status`, `trade`, `mcp`) are a deliberately separate change: each parses the flag
    /// as mandatory, and making it optional is a per-verb migration rather than a dispatcher edit.
...
fn resolve_policy() -> Result<Resolved, String> {
    ...
    Ok(Resolved {
        ...
        // ceiling above comes from, so `vike-cli config show` and the `node` verbs can never
...
fn node_keyring(resolved: &Resolved) -> NodeKeyring {
    ...
    // by the notice below to move it. `node setup` writes `node.env`; the refusal names `node.env`;
    // the two mouths of this binary say one thing. A box that HAS a legacy pair never sees this
    // message at all — its keys resolve, and the migration notice is what it gets instead.
...
/// venue key on the box, and only the five arms that can dial a datahub call it. That is a
...
fn dispatch(command: &str, args: impl Iterator<Item = String>, resolved: &Resolved) -> ExitCode {
    match command {
        // ⚠ The three REMOTE run verbs, two of which also have a `--local` arm. That arm SPAWNS the
        ...
        // The three profile-RUNNING verbs. Each dials a datahub, and `required_scope` puts every
        // one of them under `Scope::Control` — they compile client-supplied Rhai on the server —
        // so they carry the same keys `data` does. See [`datahub_keyring`] for where they come from.
        ...
        //
        // This is the FOURTH arm to resolve the keyring, and the only one that also WRITES it.
        "node" => cmd::node::run(
            args,
            cmd::node::Ctx {
                settings_dir: resolved.settings_dir.as_deref(),
        ...
        // separate VERB, because `node` means a vike-tradehub node and a datahub is not one.
