...
use vike_marketdata::{BookLevel, L2Book};
...
pub struct OtrTracker {
    ...
    prev: Option<(Vec<BookLevel>, Vec<BookLevel>)>,
...
fn diff_counts(old: &[BookLevel], new: &[BookLevel]) -> (u64, u64, u64) {
    let old_map: BTreeMap<u64, f64> =
        old.iter().map(|&BookLevel { price: p, qty: q }| (p.to_bits(), q)).collect();
    let (mut adds, mut updates) = (0u64, 0u64);
    for &BookLevel { price: p, qty: q } in new {
    ...
    for k in old_map.keys() {
        if !new.iter().any(|&BookLevel { price: p, .. }| p.to_bits() == *k) {
            cancels += 1;
        }
    }
...
mod tests {
    ...
    fn book(seq: u64, bids: &[BookLevel], asks: &[BookLevel]) -> L2Book {
    ...
    fn diff_counts_add_update_cancel() {
        let old = [BookLevel::new(99.0, 10.0), BookLevel::new(98.0, 5.0)];
        let new = [BookLevel::new(99.5, 2.0), BookLevel::new(99.0, 7.0)]; // 99.5 add, 99.0 qty change, 98.0 gone
    ...
    fn otr_window_expected_values() {
        ...
        let b0 = book(1, &[BookLevel::new(99.0, 10.0)], &[BookLevel::new(101.0, 10.0)]);
        assert_eq!(t.push(0, &b0, 0), None); // baseline: no prev to diff
        let b1 = book(
            2,
            &[BookLevel::new(99.0, 10.0), BookLevel::new(98.5, 4.0)],
            &[BookLevel::new(101.0, 6.0)],
        );
        assert_eq!(t.push(50, &b1, 3), None);
        let f = t.forming().unwrap();
        assert_eq!((f.adds, f.updates, f.cancels, f.trades), (1, 1, 0, 3));
        assert_eq!(f.otr, 0.0);
        let b2 = book(3, &[BookLevel::new(99.0, 10.0)], &[BookLevel::new(101.0, 6.0)]); // 98.5 bid pulled
    ...
    fn otr_boundary_is_half_open() {
        ...
        let b = book(1, &[BookLevel::new(99.0, 1.0)], &[BookLevel::new(101.0, 1.0)]);
        t.push(0, &b, 0);
    ...
    fn scroll_out_of_depth_counts_as_cancel() {
        ...
        let b0 = book(
            1,
            &[BookLevel::new(99.0, 1.0), BookLevel::new(98.5, 1.0)],
        ...
        let b1 = book(
            2,
            &[BookLevel::new(99.5, 2.0), BookLevel::new(99.0, 1.0), BookLevel::new(98.5, 1.0)],
            &[BookLevel::new(101.0, 1.0)],
        );
        t.push(10, &b1, 0);
        let f = t.forming().unwrap();
    ...
    fn otr_trade_floor() {
        let mut t = OtrTracker::new(OtrConfig { depth: 5, window_ms: 100 });
        t.push(0, &book(1, &[BookLevel::new(99.0, 1.0)], &[BookLevel::new(101.0, 1.0)]), 0);
        // bids: add 98.5, update 99; asks: 101 → 101.5 (add + cancel)
        let b1 = book(
            2,
            &[BookLevel::new(99.0, 3.0), BookLevel::new(98.5, 1.0)],
            &[BookLevel::new(101.5, 1.0)],
        );
        t.push(10, &b1, 0);
