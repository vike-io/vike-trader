...
/// The set of interval strings the live venue kline feeds serve natively — anything
...
///
/// Mirrors `crates/vike-desktop/src/main.rs`'s `IVLS` (the menu that offers them). ⚠ Membership
/// changes NOTHING for `"4h"`/`"1d"`: neither ends in `t` or `v`, so both already fell through to
/// the `Kline` passthrough at the bottom of `parse`. They are named here because this list is the
/// written-down answer to "which strings are klines", and a reader checking it should not have to
/// re-derive the fallback to find out that two of the menu's rows are missing from it.
const KLINE_SET: [&str; 7] = ["1s", "1m", "5m", "15m", "1h", "4h", "1d"];
