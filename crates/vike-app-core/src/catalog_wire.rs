...
//! `VerbScope::Observe` (0062's decision 1, asserted in
//! `crates/vike-datahub/tests/auth_roundtrip.rs`), so the datahub OBSERVE pair the desktop already
//! resolves for its store reads reaches it. `Request::Backfill` is `VerbScope::Control`, which is
...
use vike_datahub_client::{DatahubClient, FEATURE_VENUE_CATALOG, NodeKeys, Scope};
...
pub fn fetch_venue_catalog(dial: Option<&CatalogDial>, venue: &str) -> CatalogFetchReport {
    ...
    let dialled = match &dial.keys {
        Some(k) => DatahubClient::connect_authed(&dial.addr, k, Scope::Observe),
