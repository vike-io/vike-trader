...
//! [`vike_config::set_setting_within`] — comment-preserving, validated through the loader BEFORE a
//! byte lands, atomic, one key, and with a lock budget this surface chooses itself
//! ([`VENUE_ARM_LOCK_BUDGET`], which is where the argument for it lives).
//!
...
pub const ARM_RESTART_NOTE: &str = "saved to policy.toml — restart vike-app for it to take effect (a venue's ceiling is read once, \
     at mount)";

/// **The budget this surface gives the settings-directory lock: none.**
///
/// ⚠ [`apply_arming`] is called from the DEFERRED-MUTATIONS block of [`venues_tab_content`], which
/// runs inside an egui frame — so whatever this value is, the UI thread spends it. The shared
/// writer's default is ~3 s, sized (correctly) for a CLI that is the whole process and has no
/// supervisor above it; spent HERE it is a frozen window with no repaint, no spinner and no cancel,
/// which an operator reads as "the app hung" rather than as "another writer holds the directory".
/// A frame owes 16 ms and cannot lend one of them to another process's critical section.
///
/// So: ONE `try_lock` and no sleep at all ([`vike_config::LockBudget::NON_BLOCKING`]). The
/// contention this gives up on is not exotic — it is exactly the GUI-Save-versus-daemon-control-
/// write pair the writer's lock exists for — and giving up on it costs the operator one click,
/// because a refusal here lands in the [`ArmEdit::Failed`] row the surface already renders and
/// already invites them to start again from.
///
...
pub const VENUE_ARM_LOCK_BUDGET: vike_config::LockBudget = vike_config::LockBudget::NON_BLOCKING;

/// What the row says when the write found the settings directory locked by another writer.
///
/// ⚠ It REPLACES `SettingsWriteError::Busy`'s own text rather than rendering it verbatim, which is
/// the one place this surface does that. That message is written for a process: it reports how many
/// milliseconds it waited (zero, here — see [`VENUE_ARM_LOCK_BUDGET`]) and ends "Re-run the
/// command", and there is no command. The DISPOSITION is identical and is what matters: nothing was
/// written, and trying again is the whole remedy.
pub const ARM_BUSY_NOTE: &str = "not saved: another writer holds this settings directory right now (this box's \
...
pub const OBSERVING_NOTE: &str = "observing a remote backend — this file governs THIS process (which mounts no venues while \
...
/// `docs/decisions/0057`'s crossing, a box that has run `vike-cli config adopt` resolves
/// `policy.venues` from the settings DATABASE and does not open `policy.toml` for resolution at
/// all. A store-blind `vike_config::load` on such a box answers `Policy::default()` — **every roster
/// venue `paper` with `is_declared()` false — and this function feeds the ARMING TAB**, so it would
/// have painted a full unmount every frame that tab was visible, on a box whose arming was correct.
///
...
pub fn can_apply(state: &ArmEdit) -> bool {
    match state {
        ArmEdit::Confirming { key, confirm, .. } => {
            super::backend_settings::can_save_fields(key, confirm)
        }
        _ => false,
    }
}
...
pub fn apply_arming(
    ...
    if arm_direction(current, target) == ArmDirection::Loosen {
        match confirm {
            ...
            Some(c) if !super::backend_settings::can_save_fields(key, c) => {
                return Err(format!(
    ...
    // ⚠ `set_setting_within`, never the no-budget `set_setting`: this call is on the RENDER thread.
    // [`VENUE_ARM_LOCK_BUDGET`] carries the whole argument.
    // ⚠ `RowSync::Mirror` — the GUI mounts no venue, signs no order and runs no core (rulings 1 and
    // 2 of the desktop/daemon rename), so it is not the daemon-side writer `docs/decisions/0068` names as a reopener,
    // and it may keep the rows in step with the file it just wrote. Without this an arming change
    // made here on an ADOPTED box would report success and arm nothing.
    match vike_config::set_setting_within(
        dir,
        SettingsFile::Policy,
        key,
        target.as_str(),
        VENUE_ARM_LOCK_BUDGET,
        ...
        Err(e) => {
            // The LEDGER keeps the writer's own words (a `Busy` row should say which sentinel and
            // how long), while the ROW gets the one an operator with a mouse can act on. They are
            // the same disposition; only the audience differs.
            let error = e.to_string();
            let shown = match &e {
                vike_config::SettingsWriteError::Busy { .. } => ARM_BUSY_NOTE.to_string(),
                _ => error.clone(),
            };
            // ⚠ `old: None` is "no previous value was established", NOT "the key was unset in the
            // file": this writer refused, so it either never read one or (on `Stranded`) had the
            // whole previous file moved aside underneath it, and `target` is the ceiling that did
            // NOT land. `vike_model::change_journal::SettingTarget`'s own doc is the authority for
            // how a reader is meant to take a non-applied row's cells — outcome first.
            record(
                journal,
                now_ms,
                Change::set_setting(
                    vike_config::journal_outcome(&e),
                    Actor::Gui,
                ...
                for row in &inputs.rows {
                    ...
                    match state {
                        ArmEdit::Confirming { key, target, confirm, .. } if *key == row_key => {
                            ...
                            let allowed = super::backend_settings::can_save_fields(&key, confirm);
                            ui.horizontal(|ui| {
                                ui.add(
