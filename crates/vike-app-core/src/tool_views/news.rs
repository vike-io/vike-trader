...
pub fn news_tool_content(ui: &mut egui::Ui, ctx: &ToolCtx<'_>, tv: &mut tools::ToolView) {
    ...
    let avatar = |ui: &mut egui::Ui, sz: f32, src: &str| {
        ...
        if let Some(tex) = logos.get(src) {
            ...
            ui.painter().text(
                ...
                palette::BG, // BG, matching Py avatar fg
            );
        }
    };
        ...
        let follow = ui.add(
            egui::Button::new(
                RichText::new("⌖ Follow chart")
                    .color(if tv.news_follow { palette::ACCENT } else { palette::TEXT2 })
                    .size(13.0),
            )
            .fill(palette::SURFACE)
            .stroke(egui::Stroke::new(
                1.0,
                if tv.news_follow { palette::ACCENT } else { palette::BORDER },
            ))
    ...
    ui.horizontal_top(|ui| {
            ...
            egui::ScrollArea::vertical().id_salt("news_list").show(ui, |ui| {
                ...
                for (vi, n) in visible.iter().enumerate() {
                    ...
                    let inner = egui::Frame::new()
                        .fill(if selected { palette::SURFACE } else { Color32::TRANSPARENT })
                        .corner_radius(4.0)
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.horizontal_top(|ui| {
                                ...
                                ui.vertical(|ui| {
                                    ...
                                    ui.add(
                                        egui::Label::new(
                                            RichText::new(&n.title).size(15.0).color(palette::TEXT),
                                        )
                                        .wrap(),
                                    );
                                });
        ...
        if reader_on {
            ...
            ui.painter().rect_filled(sr, 0.0, palette::BORDER);
            ui.add_space(9.0);
            // ----- reader pane -----
                ...
                |ui| {
                    if let Some(n) = visible.get(tv.news_sel) {
                        ui.horizontal(|ui| {
                            avatar(ui, 30.0, &n.source);
                            ui.add_space(8.0);
                            ui.label(font::semibold(&n.source).size(13.0).color(palette::TEXT));
                            ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        ...
                        ui.add_space(8.0);
                        ui.add(
                            egui::Label::new(font::bold(&n.title).size(26.0).color(palette::TEXT))
                                .wrap(),
                        );
                        ui.add_space(5.0);
                        ...
                        ui.add(
                            egui::Label::new(
                                RichText::new(&n.summary).size(16.0).color(palette::TEXT2),
                            )
                            .wrap(),
                        );
                        ui.add_space(12.0);
                            ...
                            ui.horizontal_wrapped(|ui| {
                                ...
                                for c in &chips {
                                    egui::Frame::new()
                                        ...
                                        .show(ui, |ui| {
                                            ui.label(
                                                RichText::new(c).size(12.0).color(palette::TEXT),
                                            );
                                        });
                                }
                            });
                        ...
                        let open = ui.add_enabled(
                            has_url,
                            egui::Button::new(
                                font::semibold("↗  Open original").size(14.0).color(palette::TEXT),
                            ),
                        );
                        if open.clicked() {
    ...
    ui.label(RichText::new(status).size(11.0).color(palette::TEXT3));
}
...
fn news_filter_pill(
    ...
    let resp = ui.add(
        egui::Button::new(RichText::new(label).color(palette::TEXT2).size(13.0))
            .fill(palette::SURFACE)
