...
pub fn bucket_prob_levels(
    bids: &[vike_model::BookLevel],
    asks: &[vike_model::BookLevel],
...
pub fn cockpit_tool_content(ui: &mut egui::Ui, ctx: &ToolCtx<'_>, tv: &mut tools::ToolView) {
    ...
    let up_price = real.and_then(|b| b.best_ask()).map(|BookLevel { price: p, .. }| p);
    let dn_price = real.and_then(|b| b.best_bid()).map(|BookLevel { price: p, .. }| 1.0 - p);
    ...
    let chain = vike_cockpit::chain_windows(now_ms, UPDOWN_WINDOW_SECS, POLY_CHAIN_LOOKAHEAD);
    ...
    let rail_inputs = vike_cockpit::ChainRailInputs {
        asset: &asset_label,
        cards: &cards,
        window_secs: UPDOWN_WINDOW_SECS,
        now_ms,
    };
    ...
    let resolution_ts = vike_cockpit::window_close_ms(
        vike_cockpit::window_open_ms(now_ms, UPDOWN_WINDOW_SECS),
        UPDOWN_WINDOW_SECS,
    );
    ...
    let inside_bid =
        real.and_then(|b| b.best_bid()).map(|BookLevel { price: p, .. }| price_to_cents(p));
    let inside_ask =
        real.and_then(|b| b.best_ask()).map(|BookLevel { price: p, .. }| price_to_cents(p));
...
mod tests {
    ...
    fn bucket_prob_levels_sums_levels_that_share_a_cent() {
        ...
        let bids =
            [BookLevel::new(0.501, 10.0), BookLevel::new(0.5049, 5.0), BookLevel::new(0.49, 3.0)];
    ...
    fn bucket_prob_levels_merges_both_sides_onto_one_rung() {
        let levels = bucket_prob_levels(
            &[BookLevel::new(0.42, 7.0)],
            &[BookLevel::new(0.42, 2.0), BookLevel::new(0.43, 4.0)],
        );
        assert_eq!(levels.len(), 2);
    ...
    fn bucket_prob_levels_drops_the_resolved_ends() {
        ...
        let levels =
            bucket_prob_levels(&[BookLevel::new(0.001, 9.0)], &[BookLevel::new(0.9999, 9.0)]);
    ...
    fn levels_come_back_ascending_by_cent() {
        let levels = bucket_prob_levels(
            &[BookLevel::new(0.90, 1.0), BookLevel::new(0.10, 1.0), BookLevel::new(0.50, 1.0)],
            &[],
        );
