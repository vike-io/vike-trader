...
// ⚠ `const POLICY_FILE: &str = "policy.toml"` stood here — compared against
// `WireSettingsRow::section`, which the read half renders as a file name, to decide which rows
// demand the typed confirm. That comparison is the safety check a vocabulary move disarms in
// silence, so the decision moved onto the KEY's section word ([`demands_typed_confirm`]) and the
// constant went with it. Nothing in this file needs to know what the policy file is CALLED.
...
/// Whether a row demands the typed confirm (the POLICY plane — the risk ceilings), decided from
/// the row's KEY.
///
/// ⚠ **This was `is_policy_file(file: &str) -> file == POLICY_FILE`, and the re-key is a SAFETY
/// repair rather than tidying.** `file` here is a [`vike_tradehub_client::WireSettingsRow`]'s
/// `section` field, which carries the literal `"policy.toml"` today — so an exact match against
/// the file name is a check that answers `false` the moment the wire stops carrying file names,
/// at which point the GUI stops demanding the retype on every risk ceiling and NOTHING FAILS.
/// `docs/decisions/0057-the-seven-settings-files-answered-one-at-a-time.md` names that as the
/// half-disarmed confirm a vocabulary move ships unless the re-key lands with it.
///
/// `vike_config::is_policy_plane_key` is the ONE predicate, in the crate the daemon's own
/// enforcement already shares, and it decides from the key's SECTION WORD (`policy.…`). Today's
/// membership is unchanged — `set_setting` refuses a write whose key section disagrees with its
/// file, so for every write that can land the two answers are the same — and it survives the
/// vocabulary move by construction, because a dotted key has always been spelled with the section.
pub fn demands_typed_confirm(key: &str) -> bool {
    vike_config::is_policy_plane_key(key)
}
...
pub fn can_save(state: &SettingsEditState) -> bool {
    match state {
        SettingsEditState::Editing { key, confirm, .. } => can_save_fields(key, confirm),
        _ => false,
    }
}
...
///
/// ⚠ It no longer takes the FILE at all, and losing that parameter is the repair rather than a
/// simplification — see [`demands_typed_confirm`]. The `file`/`section` string still rides the
/// wire and still names the file a write edits; what it stopped doing is DECIDE a safety check.
pub(crate) fn can_save_fields(key: &str, confirm: &str) -> bool {
    !demands_typed_confirm(key) || confirm == key
}
...
pub fn take_save(state: &mut SettingsEditState) -> Option<SettingsWriteRequest> {
    ...
    *state = SettingsEditState::Saving { key: key.clone() };
    let confirm = demands_typed_confirm(&key).then_some(confirm);
...
///
/// The shipped unit (`deploy/vike-tradehub.service`, which the CI box installs under that name with
/// its own root substituted in) runs `ProtectSystem=strict` with a single
...
fn settings_edit_panel(
    ...
    egui::Frame::new()
        ...
        .show(ui, |ui| match edit {
            ...
            SettingsEditState::Editing { file: _, key, value, confirm } => {
                let policy = demands_typed_confirm(key);
                ...
                let allowed = can_save_fields(key, confirm);
                ui.add_space(3.0);
                ui.horizontal(|ui| {
...
mod tests {
    ...
    /// **THE RE-KEY'S OWN PROOF: the confirm still ARMS when the row's `section` stops being a
    /// file name.**
    ///
    /// This is the failure `docs/decisions/0057-the-seven-settings-files-answered-one-at-a-time.md`
    /// names and the reason [`demands_typed_confirm`] was re-keyed: the old gate compared
    /// `section` against the literal `"policy.toml"`, so a wire that started sending the SECTION
    /// WORD — or anything else — would have let every risk-ceiling edit save with an empty
    /// confirm, with no test anywhere going red. A safety check that silently stops being required
    /// is exactly what an ungated vocabulary move ships.
    ///
    /// Four spellings, one of which is the one a migration actually produces (`"policy"`), one of
    /// which is deliberately meaningless (`""`) to show the decision does not consult the field at
    /// all. In every case the walk must still refuse an empty confirm and still demand the exact
    /// key.
    ///
    /// ⚠ **Mutation proof:** change [`demands_typed_confirm`] back to `section == "policy.toml"`
    /// and three of the four rounds go red at the first assertion — while
    /// [`a_policy_edit_walks_edit_confirm_saved_restart_required`] above, which uses the file-name
    /// spelling, stays green. That asymmetry IS the finding.
    #[test]
    fn the_typed_confirm_still_arms_when_the_row_stops_naming_a_file() {
        for section in ["policy.toml", "policy", "settings.policy", ""] {
            let row = WireSettingsRow { section: section.into(), ..policy_row() };
            let mut edit = start_edit(&row);
            assert!(
                !can_save(&edit),
                "an unconfirmed POLICY edit must stay blocked with section {section:?} — the \
                 ceremony is decided by the KEY, and a vocabulary change to this field must not \
                 disarm it"
            );
            ...
            assert!(!can_save(&edit), "section {section:?}: a near-miss confirm must not save");

            // …and the EXACT key still unlocks it, with the confirm attached to the request.
            if let SettingsEditState::Editing { confirm, .. } = &mut edit {
                *confirm = "policy.max_notional_per_order".to_string();
            }
            assert!(can_save(&edit), "section {section:?}: the exact key must still unlock");
            let req = take_save(&mut edit).expect("the confirmed edit saves");
            assert_eq!(
                req.confirm.as_deref(),
                Some("policy.max_notional_per_order"),
                "section {section:?}: the confirm must still reach the wire"
            );
        }
    }

    /// …and the converse, so the test above is not asserting that EVERYTHING demands a confirm: a
    /// non-policy key stays confirm-free even when its `section` string is made to look like the
    /// policy file's. The old gate would have DEMANDED one here — the same defect facing the other
    /// way, a needless ceremony on a chart colour.
    #[test]
    fn a_non_policy_key_needs_no_confirm_even_when_its_section_string_says_policy() {
        let row = WireSettingsRow { section: "policy.toml".into(), ..config_row() };
        let mut edit = start_edit(&row);
        assert!(can_save(&edit), "the KEY is `config.*`; no ceremony is owed");
        let req = take_save(&mut edit).expect("saves without a confirm");
        assert_eq!(req.confirm, None, "and no confirm is invented for the wire");
    }
    ...
    fn a_non_policy_row_saves_without_any_confirm() {
        ...
        assert!(can_save(&edit), "no typed confirm demanded outside the policy plane");
