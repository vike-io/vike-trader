...
//!    is argued.
//!
//!    ⚠ **The rail no longer claims that height, and the `RAIL_W` named above no longer exists** —
//!    `crates/vike-connections/src/view.rs`'s `connections_ui` allocates its rail at its NATURAL
//!    height and paints the divider beside it rather than allocating a full-height
//!    `ui.separator()`. That is a fix to the OTHER end of the same mechanism: the greedy rail was
//!    what made an unlatched window grow to the arena, which left this strip correctly pinned to a
//!    floor ~600pt below the last thing it described. The reservation here is UNCHANGED and still
//!    load-bearing — it is what keeps the strip's row unavailable to a body that overflows — so
//!    every gate below still measures what it measured. The paragraph above is kept as the account
//!    of how the strip went missing; read `RAIL_W` as history.
//!
...
//!
//!   ⚠ **That last clause was a CLAIM this file did not keep until now.** [`backend_tab`] rendered
//!   `crate::backend_conn::picker_rows`, whose unlisted head row restated the strip's dot, name,
//!   ADDRESS, `control armed`, `(not in registry)` and `Disconnect` — on a tab the strip is drawn
//!   on. It reads `registry_rows` instead, and
//!   `crates/vike-app-core/tests/connections_tabs.rs`'s
//!   `the_live_connection_is_rendered_once_on_the_tab_that_used_to_duplicate_it` counts the
//!   DISTINCT POSITIONS the address is laid out at, over the real tab body rather than a stub.
//!
...
use crate::backend_conn::{BackendAction, active_is_unlisted, click_action, registry_rows};
use crate::backend_editor::{self, BackendForm, EditorState, FormError, KeyPresence};
use crate::backend_identity;
...
/// `data_dest`) rather than in egui temp memory, because the ambient strip's **Add backend**
...
pub struct BackendPicker<'a> {
    ...
    /// [`crate::backend_identity::SelfReport`].
...
/// The rounding margin [`strip_row`] leaves when it right-aligns its action buttons. The run is
/// measured by the same painter that then lays it out, and a fraction of a point of disagreement
/// would wrap the LAST button alone onto its own line — the twin of
/// `vike_connections::view`'s `LEGEND_FIT_SLACK`, for the same reason and with the same value.
const STRIP_ACTIONS_FIT_SLACK: f32 = 2.0;
...
/// side, and the shipped app sets its OWN `button_padding` (`vike-desktop`'s `main.rs` uses
/// `(8, 4)`, not egui's `(4, 1)`), so a constant here would be wrong in the one build that matters.
/// This is the horizontal twin of [`strip_reservation`], which measures the same row's HEIGHT from
/// the live `Ui` for the same reason.
...
/// ⚠⚠ **THE DEADBAND — the one edge that CUTS the feedback loop rather than shrinking it.**
///
/// [`ambient_strip`] removed the big dependence (the strip growing into the vertical ROOM around
/// it, worth ~4.5pt), and its doc carries that measurement. What it cannot remove is the small
/// one: the strip is drawn at `bottom - reserved`, so the reservation MOVES it, and a wrapped row
/// laid out at a different y lands on egui's rounding grid differently. MEASURED at 400x700, a
/// clean 2-cycle that never settles:
///
/// ```text
///   reserved=61.0  strip_h=59.5   (strip drawn at y=631.0)
///   reserved=59.5  strip_h=61.0   (strip drawn at y=632.5)
/// ```
///
/// Chasing that through egui's layout arithmetic is not a fix, it is the next place to be wrong:
/// the position dependence is inherent to laying content out at a variable y. So this cuts the
/// EDGE instead.
///
/// ⚠⚠ **THE BAND IS ASYMMETRIC, and the asymmetry is the whole of it — a symmetric one was tried
/// first and was WRONG in a way worth recording.** With the band applied in both directions the
/// remembered value LATCHED onto a first-frame transient and then refused a real 2.78pt growth,
/// so the reservation sat permanently BELOW the strip and the repaint fired every frame — the
/// same symptom, reached from the opposite side. MEASURED, every frame, never moving:
/// `reserved=32.53  strip_h=35.31`.
///
/// So:
///
/// * **GROWTH is adopted immediately, always.** Under-reserving is the failure this whole
///   mechanism exists to prevent — it is what puts the strip through the window floor — so a
///   bigger measurement is never second-guessed. It also cannot loop: raising the reservation only
///   moves the strip UP, and the strip's height is bounded by its content.
/// * **SHRINKAGE must clear [`STRIP_SETTLE_BAND`] to be believed**, because shrinkage is the
///   direction the position jitter exploits (measured 59.5 against a remembered 61.0). The cost of
///   disbelieving a real one is at most a band's worth of dead space above the strip, which
///   `the_foot_strip_fits_the_row_that_was_reserved_for_it` already allows for.
///
/// That converges in at most two frames by construction: once the remembered value stops moving,
/// the reservation stops moving, so the strip stops moving, so the measurement stops moving.
///
/// ⚠ The band is chosen from the MEASUREMENTS, not from taste: the jitter is ≤1.5pt (above) and
/// the smallest REAL shrink this must still track is a wrapped line disappearing, which is a whole
/// `interact_size` row — ~20pt at this text size. A band between those two separates them with
/// room to spare either way. It is deliberately NOT the `measured > reserved + 0.5` threshold
/// [`connections_body`] repaints on: that one must stay tight, because it is what would tell us a
/// dependence had come back at a scale that matters.
const STRIP_SETTLE_BAND: f32 = 4.0;

/// Fold this frame's `measured` strip height into what is remembered, applying
/// [`STRIP_SETTLE_BAND`] to SHRINKAGE only. Returns the value to remember.
fn settled_strip_height(ui: &egui::Ui, measured: f32) -> f32 {
    let stored = remembered_strip_height(ui);
    if stored <= 0.0 || measured > stored || (stored - measured) >= STRIP_SETTLE_BAND {
        measured
    } else {
        stored
    }
...
pub fn connections_body(
    ...
    // Pad down to the reserved row, then draw the strip. ⚠ Computed against `bottom` rather than
    // against `available_height`, because the shrink above is what `available_height` now reports.
    //
    // ⚠⚠ **THE OVERFLOW ARM IS NOT A PAD, AND THAT IS THE WHOLE OF IT.** This used to be a single
    // `if left > 0.0 { add_space(left) }` followed by an unconditional in-the-flow
    // `ambient_strip(ui, …)`, on the reasoning that a body which overflowed its bound leaves a
    // NEGATIVE pad, the strip lands immediately after it, and the window auto-sizes to include it.
    // That reasoning holds for a window free to grow. It is FALSE for a window that cannot — a
    // maximized one, a user-sized one, an arena-bounded one — and there the strip was laid out
    // BELOW the window's own floor, where `egui-0.36.1/src/hit_test.rs` drops a widget whose
    // `interact_rect` (`clip_rect ∩ rect`) is empty.
    //
    // MEASURED on `origin/main` at 900x420, a 420pt window holding a ~480pt panel:
    //
    // ```text
    //   window floor          412
    //   Disconnect            [[664.2 428.0] - [726.5 446.0]]   ← 16-34pt BELOW the floor
    //   click reaches it?     NO
    // ```
    //
    // The button was in the accessibility tree with its label and dead to every click, which is
    // #1857's defect wearing the strip instead of the venue rail. **Disconnect and Add backend are
    // the only door out of a wrong backend, and a window shorter than its panel is exactly when
    // somebody is trying to use it.**
    //
    // ⚠ It was invisible to every gate because of the SECOND half of the same defect: a strip
    // drawn past the floor came back with a ZERO-HEIGHT rect (`[[8 548] - [804 548]]`), so
    // `measured` was `0.0`, the `measured > reserved + 0.5` arm below could never fire, and
    // `the_foot_strip_fits_the_row_that_was_reserved_for_it` — which asserts the strip is inside
    // the window — never saw the case at all, because its stub body fills its bound exactly rather
    // than overflowing it. `the_foot_strip_is_a_hittable_row_when_the_body_overflows_the_window`
    // is the gate that does.
    //
    // So when there is no room left, the strip is placed EXPLICITLY on the floor instead of being
    // appended to the flow. It then overlaps the tail of a body that is already running off the
    // bottom of the window, and it WINS that overlap: a widget registered later takes the hit,
    // which is the same rule that already lets the title bar's `─ □ ✕` beat its drag area.
    let left = bottom - reserved - ui.cursor().top();
    let strip = if left > 0.0 {
        ui.add_space(left);
        ambient_strip(ui, picker, action, tab, editor)
    } else {
        let floor_row = egui::Rect::from_min_max(
            egui::pos2(ui.max_rect().left(), bottom - reserved),
        ...
        ui.scope_builder(
            ...
            |ui| ambient_strip(ui, picker, action, tab, editor),
        )
        .inner
    };

    // ⚠⚠ **Close the loop: remember what the strip COST, so next frame's reservation is the
    // strip's own measurement rather than an estimate of it.** The repaint is asked for only when
    // the strip OVERFLOWED the row.
    //
    // ⚠ **This used to claim the loop converges because "the raised reservation does not change
    // the strip's height (its width is the window's either way)", and that claim was FALSE — twice,
    // in two different shapes, and it is the defect rather than the reasoning around it.** The
    // strip's height was a function of the vertical room it was drawn into, and this line is what
    // decides that room, so the measurement was a measurement of its own output. Both shapes and
    // both measurement traces are at [`ambient_strip`], which is where the cure lives.
    //
    // What is true now, and what a future edit here has to preserve: **`ambient_strip` draws its
    // whole body inside a child allocated at zero height, so `strip.height()` is a function of the
    // strip's CONTENT and WIDTH only.** The reservation can therefore move without moving the
    // measurement, which is what makes this arm settle on the frame after a change rather than
    // chase it.
    //
    // ⚠ One residual REMAINS after that cure and is cut here rather than assumed away: the strip
    // is drawn at `bottom - reserved`, so this line MOVES it, and content laid out at a different
    // y lands on egui's rounding grid differently. MEASURED at 400x700, where the strip wraps to
    // two rows — a clean 2-cycle, amplitude 1.5pt, that never settles:
    //
    // ```text
    //   reserved=61.0  strip_h=59.5   (drawn at y=631.0)
    //   reserved=59.5  strip_h=61.0   (drawn at y=632.5)
    // ```
    //
    // [`settled_strip_height`] is what cuts that edge — a deadband, so a measurement that differs
    // from the remembered one only by position jitter is not adopted, while a real change (a
    // wrapped line, a resize, a different button set) is. Its doc carries the numbers the band
    // sits between. ⚠ The `+ 0.5` below is NOT part of that and must stay tight: it is what would
    // tell us a dependence had come back at a scale that matters.
    let measured = strip.height();
    let settled = settled_strip_height(ui, measured);
    ui.ctx().data_mut(|d| d.insert_temp(strip_height_id(ui), settled));
...
///
/// [`crate::backend_identity`] decides WHAT each piece says and this decides how it looks, once,
/// so the picker rows and the foot strip cannot drift apart. An unnamed backend's headline is
/// italic and dimmed: it is this module's stand-in for a name rather than a name, and it must not
/// read as one the operator chose.
fn identity_labels(ui: &mut egui::Ui, id: &backend_identity::BackendIdentity<'_>) {
    let headline = egui::RichText::new(id.headline).monospace().size(11.0).strong();
    let headline = if id.named { headline } else { headline.italics().color(palette::TEXT2) };
    ...
    let addr = egui::RichText::new(id.box_address()).monospace().size(11.0);
    let addr = if id.box_address_is_reported() { addr } else { addr.color(palette::TEXT2) };
...
/// are attached to; the accent colour is the difference between a fault report and an action.
fn identity_note(ui: &mut egui::Ui, id: &backend_identity::BackendIdentity<'_>) {
    if let Some(note) = id.note {
        let text = egui::RichText::new(note).monospace().size(10.0);
        ui.label(if id.invitation { text.color(palette::ACCENT) } else { text.weak() });
    }
}
...
///
/// ⚠⚠ **THAT PARAGRAPH WAS A CLAIM THE CODE DID NOT KEEP, and this function is where it was
/// broken.** It called `crate::backend_conn::picker_rows`, whose FIRST arm pushes the live
...
/// `(not in registry)` and a `Disconnect` on this tab AND every one of them again in
/// [`ambient_strip`] at the foot of the same window. Two renderings of one fact is precisely what
/// the strip exists to end.
///
/// Two edits close it and both are visible here:
///
/// * `crate::backend_conn::registry_rows` — the registry's records ALONE. The head row was never
///   editable anyway (`if row.listed` already suppressed its Edit/Delete), which is the tell that
///   it was not a record. When the live connection is unlisted this tab says so in a SENTENCE that
///   names the strip, which is information the strip does not carry and a row could not add.
/// * **The active row keeps no `Disconnect`.** Disconnecting is one act on process-level state and
///   the strip owns it. `Connect` STAYS on every non-active row, because that is how switching
///   happens and the strip offers no switch — so the tab keeps the verb the strip lacks and gives
///   up the one it duplicates.
///
/// ⚠ `pub` for the same reason [`ambient_strip`] and [`status_line`] are: a headless harness has to
/// be able to draw THIS, inside [`connections_body`], or the de-duplication gate is a test of a
/// stub. It was — the first cut of `crates/vike-app-core/tests/connections_tabs.rs`'s
/// `the_live_connection_is_rendered_once_on_the_tab_that_used_to_duplicate_it` passed
/// `ui.allocate_space(…)` as the body and could therefore only ever have counted the strip's own
/// rendering, which is an assertion that cannot fail for its stated reason.
...
pub fn backend_tab(
        ...
        ui.label(
            egui::RichText::new(
                "the live connection is not a backends.json record, so it has no row here — it is \
                 rendered once, in the strip at the foot of this window, where Disconnect and the \
                 invitation to give this box a name both live.",
            )
            .monospace()
            .size(10.0)
            .weak(),
    ...
    if rows.is_empty() {
        ...
        for row in rows {
            ui.horizontal(|ui| {
                ...
                // NAME first, address second — `crate::backend_identity` is the one authority for
                // both, so this row and the foot strip cannot describe the same backend two ways.
    ...
    // authority, so it cannot say `(--observe)` while the strip two rows down says something else.
...
pub fn ambient_strip(
    ...
    // ⚠⚠ **THE WHOLE STRIP IS DRAWN INSIDE A CHILD ALLOCATED AT ZERO HEIGHT, and that is a
    // MEASUREMENT fact rather than a styling one: it is what makes `strip.height()` a pure
    // function of this strip's CONTENT and WIDTH, and independent of where it happens to be
    // drawn.**
    //
    // [`connections_body`] feeds this height back into [`strip_reservation`], the row it holds
    // back for the strip — and that row is what decides how much vertical space is left where the
    // strip lands. So a height that varies with the surrounding room is a measurement of its own
    // output, and the loop does not close. It has been wrong TWICE here, in two different shapes,
    // and only the second fix is unconditional:
    //
    // 1. **A fixed 2-cycle.** The row was `ui.horizontal_wrapped`, i.e.
    //    `Layout::left_to_right(Align::Center)` with `main_wrap`, and `egui-0.36.1`'s
    //    `Layout::next_frame_ignore_wrap` carries
    //    `if (is_horizontal() && vertical_align() == Align::Center) || vertical_justify() { …
    //    frame_size.y = frame_size.y.max(region.cursor.height()) }` — so the row INFLATED to the
    //    leftover room. MEASURED at 560x520, alternating for ever:
    //    `reserved=40.50 strip_h=36.47` then `reserved=36.47 strip_h=40.50`.
    //    Giving the ROW an explicit zero-height layout fixed that one.
    // 2. **A DAMPED oscillation, ~0.09pt of wobble decaying toward a fixed point, and a strip
    //    measured 5pt SHORT.** The row was no longer the only thing reading the room. MEASURED by
    //    drawing this strip at a controlled `set_max_height`:
    //
    //    ```text
    ...
    //    ```
    //
    //    …so the strip still grew into whatever it was given, up to a ceiling. Fed back, that
    //    converges only slowly and never settles inside the half-point `connections_body` allows,
    //    so the window repaints for ever. ⚠ **That residual was DETECTED and then DISMISSED**: the
    //    first regression gate for defect 1 drew this strip with 400pt of room and with none, got
    //    40 versus 36, and was rewritten as a composition test on the grounds that the isolated
    //    number "was not the property". It was the property. The gate is back, at
    //    `the_strips_measured_height_does_not_depend_on_the_room_below_it`.
    //
    // Bounding the WHOLE child — separator included — at height `0.0` is what makes this
    // unconditional: every `available_size_before_wrap()` inside resolves against a zero-height
    // `max_rect`, so there is no leftover room for anything in here to grow into, wherever
    // [`connections_body`] places it. Nothing is clipped by that (egui clips by the painter's clip
    // rect, never by `max_rect`) and the parent still advances by the child's own `min_rect`.
    //
    // ⚠ It went unseen for a release because the two arms only differ once the body is close
    // enough to the window floor for there to BE leftover room in one frame and not the other. The
    // credential panel gaining a rule was enough to cross that line; the credential EDITOR branch
    // gaining fields crossed it again, harder.
    let strip_body = ui
        .allocate_ui_with_layout(
            ...
            |ui| {
                ...
                strip_row(ui, picker, action, tab, editor)
            },
        )
        .response
        .rect;
    // The strip's real extent: from the flow position the separator started at, down to the
    // wrapped row's own floor. ⚠ Read off the drawn rects rather than computed from the pieces,
    // because the WRAP is the one part of this that cannot be predicted — on a narrow window the
    // Disconnect/Add buttons go to a second line and the strip is a row taller than any
    // single-row arithmetic would say.
    egui::Rect::from_min_max(
        egui::pos2(left, top),
        egui::pos2(strip_body.right().max(left), strip_body.bottom().max(top)),
    )
}

/// The strip's own wrapped row, drawn into the zero-height child [`ambient_strip`] opens.
///
/// ⚠ **The layout is spelled out and never inherited**, for the reason
/// `crates/vike-connections/src/view.rs`'s `rail_chips` documents at length: `Ui::scope` would
...
/// a narrow window, which is the behaviour `the_foot_strip_fits_the_row_that_was_reserved_for_it`
/// drives at 300pt.
fn strip_row(
    ui: &mut egui::Ui,
    picker: &BackendPicker<'_>,
    ...
    ui.allocate_ui_with_layout(
            ...
            match picker.active {
                Some(record) => {
                    ...
                    // `crate::backend_identity` carries the measurement.
                    let id = backend_identity::identify_reported(
                        record,
            ...
            //     wrap onto a second line (`the_foot_strip_fits_the_row_that_was_reserved_for_it`
            //     drives exactly that at 300pt), and padding a row too narrow for them would push
            //     the tail off the right edge instead.
            ...
            if room >= run + STRIP_ACTIONS_FIT_SLACK {
                ui.add_space(room - run - STRIP_ACTIONS_FIT_SLACK);
            }
            if show_disconnect
                && ui
                    .add_enabled(
