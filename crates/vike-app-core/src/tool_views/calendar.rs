...
use super::ToolCtx;
use crate::tools;
use vike_ui_theme::{font, palette};
...
pub fn calendar_tool_content(ui: &mut egui::Ui, ctx: &ToolCtx<'_>, tv: &mut tools::ToolView) {
    ...
    ui.horizontal(|ui| {
        ...
        let _ = ui.button("›");
        ui.add_space(6.0);
        ui.label(font::semibold(&td.cal_range).size(16.0).color(palette::TEXT)); // Py 16px/600
        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
            ...
            ui.add_space(4.0);
            ui.checkbox(&mut tv.cal_high_only, RichText::new("High only").color(palette::TEXT2));
        });
    });
    ui.add_space(6.0);
                ...
                let inner = egui::Frame::new()
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.vertical(|ui| {
                            ...
                            let tc = if on { palette::ACCENT } else { palette::TEXT };
                            ...
                            for (ci, cnt) in counts.iter().enumerate() {
                                ...
                                ui.horizontal(|ui| {
                                    ui.label(
                                        RichText::new(CATS[ci]).size(12.0).color(palette::TEXT2),
                                    ); // 12/400
                                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                                        ui.label(
    ...
    ui.horizontal(|ui| {
        for (i, name) in PAGES.iter().enumerate() {
            ...
            if ui
                .selectable_label(
                    active,
                    RichText::new(*name).size(12.0).color(if active {
                        palette::TEXT
                    } else {
                        palette::TEXT3
                    }),
    ...
    if page == 0 {
        ...
        let hcell = |ui: &mut egui::Ui, w: f32, t: &str, right: bool| {
            ...
            ui.allocate_ui_with_layout(vec2(w, 20.0), lay, |ui| {
                ...
                ui.add(
                    egui::Label::new(RichText::new(t).size(12.0).color(palette::TEXT3)).truncate(),
                );
        ...
        egui::ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
            ...
            for e in td.calendar.iter().filter(|e| {
                ...
                if e.day != cur_day {
                    ...
                    prev_ctry.clear();
                    ui.add_space(5.0);
                    ui.label(font::bold(&cur_day).size(13.0).color(palette::TEXT));
                ...
                if !marker_done && e.ts_ms > now_ms {
                    ...
                    let hhmm = chrono::DateTime::from_timestamp_millis(now_ms)
                        ...
                        .unwrap_or_default();
                    ui.horizontal(|ui| {
                        ui.colored_label(palette::DOWN, RichText::new("●").size(12.0));
                        ui.label(font::bold(format!("now  {hhmm}")).color(palette::DOWN));
                    });
                }
                ...
                let (act_txt, act_col) = if future {
                    ...
                    ("—".to_string(), palette::TEXT)
                } else {
                    let col = match (num(&e.actual), num(&e.forecast)) {
                        (Some(a), Some(f)) if a > f => palette::UP,
                        (Some(a), Some(f)) if a < f => palette::DOWN,
                        _ => palette::TEXT,
                    };
                    (e.actual.clone(), col)
                };
                ui.horizontal(|ui| {
                    ...
                    ui.allocate_ui_with_layout(
                        ...
                        |ui| {
                            ...
                            if show_ctry {
                                ...
                                ui.add(
                                    egui::Label::new(RichText::new(nm).color(palette::TEXT))
                                        .truncate(),
                                );
                            }
                        },
                    ...
                    ui.allocate_ui_with_layout(
                        ...
                        |ui| {
                            ...
                            ui.add(
                                egui::Label::new(RichText::new(&e.title).color(palette::TEXT))
                                    .truncate(),
                            );
                        },
                    );
                    ...
                    rcell(ui, w_fc, dash(&e.forecast), palette::TEXT); // Py: Forecast/Prior = default palette::TEXT
                    rcell(ui, w_pr, dash(&e.previous), palette::TEXT);
                });
            }
        });
    } else {
    ...
    if !td.cal_status.is_empty() {
        ui.label(RichText::new(&td.cal_status).size(11.0).color(palette::TEXT3));
    }
}
...
/// are lit (TEXT3/WARN/palette::DOWN by level); the rest dim BORDER. Ported from calendar_delegate.py.
fn draw_impact(p: &egui::Painter, r: egui::Rect, importance: u8) {
    use egui::{Rect, pos2};
    let lit = match importance {
        2 => palette::DOWN,  // red
        1 => palette::WARN,  // amber
        _ => palette::TEXT3, // grey
    };
            ...
            let rows = td
                ...
                .map(|r| {
                    ...
                    (
                        r.date.clone(),
                        vec![
                            (r.hour.clone(), palette::TEXT2),
                            (r.symbol.clone(), palette::TEXT),
                            (dash(&r.eps_est), palette::TEXT),
                            (dash(&r.eps_act), palette::TEXT),
                            (dash(&r.surprise), sc),
                        ],
                    )
                })
            ...
            let rows = td
                ...
                .map(|r| {
                    (
                        ...
                        vec![
                            (r.symbol.clone(), palette::TEXT),
                            (dash(&r.date), palette::TEXT),
                            (dash(&r.pay_date), palette::TEXT),
                            (dash(&r.amount), palette::TEXT),
                            (dash(&r.yield_pct), palette::TEXT),
                            (dash(&r.freq), palette::TEXT2),
                        ],
                    )
                })
                .collect();
            ...
            let rows = td
                ...
                .map(|r| {
                    (
                        ...
                        vec![
                            (r.symbol.clone(), palette::TEXT),
                            (dash(&r.company), palette::TEXT),
                            (dash(&r.exchange), palette::TEXT2),
                            (dash(&r.price), palette::TEXT),
                            (dash(&r.shares), palette::TEXT),
                            (dash(&r.status), palette::TEXT2),
                        ],
                    )
                })
                .collect();
    ...
    ui.horizontal(|ui| {
        ...
        for (i, h) in headers.iter().enumerate() {
            ...
            ui.allocate_ui_with_layout(vec2(widths[i], 20.0), lay, |ui| {
                ...
                ui.add(
                    egui::Label::new(RichText::new(*h).size(12.0).color(palette::TEXT3)).truncate(),
                );
            });
        }
    });
    ...
    egui::ScrollArea::vertical().auto_shrink([false, false]).id_salt("cal_equity").show(ui, |ui| {
        ...
        for (date, cells) in &rows {
            if *date != cur {
                cur = date.clone();
                ui.add_space(5.0);
                ui.label(font::bold(cal_day_label(date)).size(13.0).color(palette::TEXT));
            }
            ui.horizontal(|ui| {
                ui.spacing_mut().item_spacing.x = 0.0;
