...
pub use backend_settings::{
    ...
    SANDBOX_WRITE_NOTE, SAVED_RESTART_NOTE, SECTION_TITLE, SettingsEditState, SettingsFilter,
    SettingsWriteRequest, UNREAD_VERDICT_POINTER, backend_settings_section, can_save, env_shadow,
    origin_kind, row_is_finding, row_is_set, row_read_by_nothing, settings_fetch_state,
    settings_write_state, should_fetch_settings, should_refetch_after_write, start_edit, take_save,
    write_target,
};
...
pub use venues::{
    ...
    arm_direction, begin_confirm, can_apply, credentials_cell, effective_cell,
    reload_venue_ceilings, take_apply, venues_tab_content,
};
...
pub struct ToolCtx<'a> {
    ...
    /// it** — `crate::catalog_refresh::CatalogRefresh`, owned by the binary because it holds the
    ...
    pub catalog: &'a crate::catalog_refresh::CatalogRefresh,
...
pub struct StoredCtx<'a> {
    ...
    /// [`crate::stored_load::RPC_DEADLINE`]. `None` → [`Self::tree`] is the store's real answer,
    ...
    /// `crates/vike-app-core/src/tool_views/stored.rs`'s `stored_tool_content`'s job, beside the
