...
pub fn greeks_tool_content(ui: &mut egui::Ui, ctx: &ToolCtx<'_>) {
    use crate::options_greeks::{PositionViewLite, position_greeks};
    ...
    ui.horizontal(|ui| {
        ui.label(RichText::new("Portfolio Greeks").strong().color(palette::TEXT));
        ui.label(RichText::new("· Deribit options").small().color(palette::TEXT3));
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ui.label(RichText::new(&td.opt_status).small().color(palette::TEXT3));
        });
    });
    ui.add_space(4.0);
    ...
    if positions.is_empty() {
        ...
        ui.vertical_centered(|ui| {
            ui.label(RichText::new("No open option positions").color(palette::TEXT2));
            ui.label(
    ...
    let greek_cell = |ui: &mut egui::Ui, v: Option<f64>| {
        match v {
            ...
            None => {
                ui.label(RichText::new("—").monospace().color(palette::TEXT3));
            }
        };
    };
    let num_cell = |ui: &mut egui::Ui, v: Option<f64>, dp: usize| match v {
        Some(x) => {
            ui.label(RichText::new(format!("{x:.*}", dp)).monospace().color(palette::TEXT2));
        }
        None => {
            ui.label(RichText::new("—").monospace().color(palette::TEXT3));
        }
    ...
    egui::ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
        egui::Grid::new("greeks_positions")
            ...
            .show(ui, |ui| {
                for h in ["Instrument", "Qty", "Avg Px", "Delta", "Gamma", "Vega", "Theta"] {
                    ui.label(RichText::new(h).small().strong().color(palette::TEXT3));
                }
                ui.end_row();
                for row in &report.rows {
                    ui.label(RichText::new(&row.instrument).monospace().color(palette::TEXT));
                ...
                ui.label(RichText::new("NET").strong().color(palette::ACCENT));
                ui.label("");
                ui.label("");
        ...
        ui.horizontal(|ui| {
            ui.label(RichText::new("NET Δ · USD").strong().color(palette::ACCENT));
