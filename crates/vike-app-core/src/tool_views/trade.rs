...
fn trade_equity_hero(
    ...
    ui.horizontal(|ui| {
        ui.label(
            font::extralight(format!("{:.2}", model.equity_total)).size(28.0).color(palette::TEXT),
        );
        let (pc, ptxt) = if realized >= 0.0 {
            (palette::UP, format!("P&L +{realized:.2}"))
        } else {
            (palette::DOWN, format!("P&L {realized:.2}"))
        };
    ...
    ui.horizontal(|ui| {
        ...
        stat(
            ...
            "Realized",
            format!("{realized:+.4}"),
            if realized >= 0.0 { palette::UP } else { palette::DOWN },
        );
        ...
        stat(
            ...
            "Funding",
            format!("{funding:+.4}"),
            if funding >= 0.0 { palette::UP } else { palette::DOWN },
        );
    ...
    for row in &model.rows {
        ui.horizontal(|ui| {
            ...
            trade_num_cell(
                ...
                if row.unrealized >= 0.0 { palette::UP } else { palette::DOWN },
            );
            ...
            trade_num_cell(
                ...
                if row.funding >= 0.0 { palette::UP } else { palette::DOWN },
            );
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if let Some(fault) = &snap.fault {
                    ui.label(RichText::new(format!("⚠ {fault}")).size(11.0).color(palette::DOWN));
                }
                if let Some(m) = mark {
            ...
            egui::Frame::new()
                ...
                .show(ui, |ui| {
                    if let Some(p) = pos.filter(|p| p.size.abs() > 1e-12) {
                        ...
                        let col = if p.size > 0.0 { palette::UP } else { palette::DOWN };
                        ui.horizontal(|ui| {
                            let (rect, painter) =
            ...
            ui.horizontal(|ui| {
                ...
                if ui
                    .add(
                        egui::Button::new(
                            RichText::new(format!("{lev:.0}× ▾")).size(11.0).color(palette::ACCENT),
                        )
                        .fill(palette::SURFACE)
                        .stroke(egui::Stroke::new(1.0, palette::BORDER)),
                    )
    ...
    egui::ScrollArea::vertical().max_height(160.0).auto_shrink([false, false]).show(ui, |ui| {
        for o in &snap.orders {
            ui.horizontal(|ui| {
                ...
                let sc = if o.side > 0 { palette::UP } else { palette::DOWN };
                trade_text_cell(
                    ui,
                    36.0,
