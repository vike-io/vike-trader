...
pub fn rail(ui: &mut egui::Ui, dest: &mut DataDest, counts: &RailCounts) -> bool {
    ...
    for d in DataDest::ALL {
        ...
        let p = ui.painter();
        if on {
            p.rect_filled(rect, 2.0, theme::SURFACE);
        ...
        let ink = if on { theme::TEXT } else { theme::TEXT2 };
        p.text(
            egui::pos2(rect.left() + 17.0, rect.center().y),
            Align2::CENTER_CENTER,
            d.icon(),
            FontId::new(11.0, FontFamily::Proportional),
            if on { theme::ACCENT } else { theme::TEXT3 },
        );
        p.text(
        ...
        if let Some(n) = counts.badge(d) {
            p.text(
                ...
                if on { theme::TEXT2 } else { theme::TEXT3 },
            );
        }
        if resp.clicked() {
            *dest = d;
    ...
    ui.allocate_ui_with_layout(
        ...
        |ui| {
            ...
            ui.horizontal(|ui| {
                ui.add_space(10.0);
                ui.vertical(|ui| {
                    ui.label(egui::RichText::new("NO CEILING").size(9.0).color(theme::TEXT3));
                    ui.add_space(2.0);
                    ui.label(
                        egui::RichText::new(
...
pub fn crumb(ui: &mut egui::Ui, leaf: &str, summary: &str) {
    ...
    ui.horizontal(|ui| {
        ui.add_space(2.0);
        ui.label(RichText::new("Data Manager").size(11.0).color(theme::TEXT3));
        ui.label(RichText::new("\u{203A}").size(11.0).color(theme::BORDER));
        ui.label(RichText::new(leaf).size(11.0).color(theme::TEXT));
        if !summary.is_empty() {
            ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                ui.add_space(2.0);
                ui.label(RichText::new(summary).size(11.0).color(theme::TEXT3));
            });
        }
    });
    ui.add_space(5.0);
...
pub fn footline(ui: &mut egui::Ui, ctx: &ToolCtx<'_>, counts: &RailCounts) {
    ...
    ui.horizontal_wrapped(|ui| {
        ...
        if let Some(note) = ctx.stored.partials_note {
            ui.label(
                RichText::new("\u{2022} partial column unavailable").size(10.5).color(theme::TEXT3),
            )
            .on_hover_text(note);
        }
        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                let _ = ui
                    .add(
                        ...
                        .fill(theme::BG)
                        .stroke(egui::Stroke::new(1.0, theme::TEXT3))
                        .corner_radius(2.0)
                        .min_size(egui::vec2(0.0, 24.0)),
                    )
                    .on_hover_text("Which store every destination below is reading.");
                ...
                if ctx.stored.loading {
                    ui.label(egui::RichText::new("loading\u{2026}").size(10.5).color(theme::TEXT3));
                }
