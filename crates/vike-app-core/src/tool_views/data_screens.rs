        ...
        if let Some((label, dest)) = action {
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if ui
                    .add(
                        egui::Button::new(
                            egui::RichText::new(label).size(10.5).color(palette::TEXT),
                        )
                        .fill(palette::CARD)
                        .stroke(egui::Stroke::new(1.0, palette::BORDER)),
                    )
    ...
    ui.horizontal_wrapped(|ui| {
            ...
            [DataDest::AllSeries, DataDest::VenueArming, DataDest::DataSets, DataDest::ActivityLog]
        {
            if ui
                .add(
                    egui::Button::new(
                        egui::RichText::new(d.label()).size(11.0).color(palette::TEXT),
                    )
                    .fill(palette::CARD)
                    .stroke(egui::Stroke::new(1.0, palette::BORDER)),
                )
...
pub fn by_venue(ui: &mut egui::Ui, ctx: &ToolCtx<'_>) {
    ...
    if tree.is_empty() {
        ui.label(egui::RichText::new("No stored data.").size(12.0).color(palette::TEXT3));
        return;
    }
            ...
            egui::Grid::new("dm_by_venue_grid")
                ...
                .show(ui, |ui| {
                    ...
                    for v in tree {
                        ...
                        ui.label(egui::RichText::new(&v.venue).size(12.0).color(palette::TEXT));
                        ui.label(
                        ...
                        ui.label(egui::RichText::new(gaps.to_string()).size(11.5).color(
                            if gaps > 0 { ui.visuals().warn_fg_color } else { palette::TEXT3 },
                        ));
                        ui.label(egui::RichText::new(stale.to_string()).size(11.5).color(
                            if stale > 0 { ui.visuals().warn_fg_color } else { palette::TEXT3 },
                        ));
                        ui.end_row();
