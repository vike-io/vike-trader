    ...
    R: Fn() + Send + 'static,
{
    ...
    // This is the client's half of the same sentence, and it now NAMES THE SAME FILE: it said
    // `secrets.env` until the desktop was migrated onto the node store, which sent every operator
    // who read it to write the pair into the file decision 0051 had just moved it out of.
    //
    ...
    if observe_key.is_none_or(str::is_empty) {
        tracing::error!(
            ...
            "no observe key: {} is absent from the credentials map, so this client will sign with \
             ...
             read from the NODE store (<project>/settings/node.env), falling back to \
             <project>/settings/secrets.env only while a box has not migrated (decision 0051), or \
    ...
    let bridge = observe_bridge::spawn_bridge(
        ...
        // `127.0.0.1`. `crate::backend_identity` is the authority for both.
        crate::backend_identity::status_label(record).map(str::to_string),
...
pub struct SwitchSlots<'a> {
    ...
    /// [`series_follow::follow_backend`](crate::series_follow::follow_backend), which RETARGETS
...
mod tests {
    ...
    struct Fixture {
        ...
        published: Vec<crate::series_follow::PublishedSeries>,
        last_seq: u64,
        status_line: String,
        feeds: FeedMap,
    ...
    fn populated() -> Fixture {
        ...
        Fixture {
            ...
            published: vec![crate::series_follow::PublishedSeries::new(
                "deribit",
                "BTC-PERPETUAL",
                "1m",
            )],
    ...
    fn the_backend_store_plane_clears_its_own_store_on_a_switch() {
        let mut fx = populated();
        assert!(
            fx.direct_bars.series("binance", "BTCUSDT", "1m").is_some(),
            "seeded by `populated`"
        );
