...
/// Credential-store key name the `--observe ADDR` CLI path reads its observe key from — the same
/// name `App::new`'s observe arm always looked up, now carried by [`cli_observe_record`].
pub const CLI_OBSERVE_KEY: &str = "VIKE_TRADEHUB_OBSERVE_KEY";
/// Credential-store key name the `--observe ADDR` CLI path reads its control key from.
pub const CLI_CONTROL_KEY: &str = "VIKE_TRADEHUB_CONTROL_KEY";
...
///
/// - `observe_key` is [`CLI_OBSERVE_KEY`], the exact name the old arm looked up in the workspace
///   credentials map (absent ⇒ empty key, and the daemon refuses the handshake — same as today).
/// - `control_key` is [`CLI_CONTROL_KEY`] and the record is ARMED (`control = true`), so the
...
pub fn cli_observe_record(addr: &str) -> BackendRecord {
    BackendRecord {
        name: String::new(),
        addr: addr.to_string(),
        observe_key: CLI_OBSERVE_KEY.to_string(),
        control_key: Some(CLI_CONTROL_KEY.to_string()),
        control: true,
    }
}
...
/// THE STARTUP LADDER: which backend a launch connects to, as a whole RECORD.
///
/// `flag` is the `--observe` argument the binary scanned out of its own argv (the shell's job stays
/// in the shell; this function reads no environment and no argv). The rungs, in order:
///
...
///    [`CLI_OBSERVE_KEY`] and failed `bad mac` forever (that function's doc carries the
///    measurement).
...
#[must_use]
pub fn startup_backend(flag: Option<&str>) -> BackendRecord {
    startup_backend_from(flag, crate::backend_registry::active_record())
}

/// [`startup_backend`]'s PURE half — the ladder with the registry answer supplied, so every rung is
/// testable without a file on disk and the tests drive the shipped decision rather than a copy.
#[must_use]
...
mod tests {
    ...
    fn the_synthetic_observe_record_reproduces_todays_env_key_behavior() {
        ...
        assert_eq!(r.observe_key, CLI_OBSERVE_KEY);
        assert_eq!(r.control_key.as_deref(), Some(CLI_CONTROL_KEY));
        assert!(r.control, "CLI record must be ARMED: pre-B1, VIKE_TRADEHUB_CONTROL=1 + the key's presence decided alone");
        ...
        let both = vars(&[(CLI_OBSERVE_KEY, "obs"), (CLI_CONTROL_KEY, "ctl")]);
        assert_eq!(backend_registry::resolve_keys(&r, &both), (Some("obs"), Some("ctl")));
        let observe_only = vars(&[(CLI_OBSERVE_KEY, "obs")]);
    ...
    struct Fixture {
        ...
        /// Held so the backfill channel stays CONNECTED — the drain must see Empty, not
        /// Disconnected, exactly like the live App (which owns its sender for the process life).
    ...
    fn the_registry_rung_answers_with_the_record_itself() {
        ...
        assert_ne!(r.observe_key, CLI_OBSERVE_KEY, "NOT the synthetic key name");
    }
