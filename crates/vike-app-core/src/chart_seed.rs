...
//! it dials UNAUTHENTICATED on purpose — `Request::Backfill` is `VerbScope::Write`, and
...
pub fn run_chart_seed(
    ...
    for r in plan {
        // ⚠ The CLASSED call, and a `None` class is byte-identical to the frame this line sent
        // before `docs/decisions/0061` Phase 3 existed. The capability check for the class rides
        // INSIDE this call rather than up front beside the lane check above, and deliberately: a
        // class-less request must still be sent to a daemon that does not advertise
        // `FEATURE_SEED_CLASS`, so the refusal is per-request rather than per-batch. A refused one
        // lands in `failed` with its message verbatim in `first_error`, which
        // `render_chart_seed_status` then shows the operator.
        match client.seed_series_classed(&r.venue, &r.symbol, &r.interval, r.class) {
...
mod tests {
    ...
    /// 0061 Phase 2's class survives the plan and reaches the request the wire is built from. The
    /// plan's dedup is keyed on [`StoreBarRequest::key`], which does NOT carry the class — see that
    /// method's doc for why, and `store_bars`'
    /// `two_windows_on_one_series_with_different_classes_ask_once_and_the_first_wins` for the
    /// consequence this test is the downstream half of.
    #[test]
    fn a_planned_seed_carries_the_windows_class_through_to_the_wire() {
        let mut classed = req("bybit", "BTCUSD.P", "5m");
        classed.class = Some(vike_model::AssetClass::CryptoPerp);
        let plan = plan_chart_seeds(&[classed, req("binance", "BTCUSDT", "5m")]);
        assert_eq!(plan.len(), 2);
        assert_eq!(plan[0].class, Some(vike_model::AssetClass::CryptoPerp));
        assert_eq!(plan[1].class, None);
    }
