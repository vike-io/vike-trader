...
///
/// The cure may NOT be a second environment read here — `crates/vike-ops/tests/settings_registry.rs`'s
...
const FAMILY_MEMBERS: [&str; 4] =
    [WORKSPACE_FILE, LAYOUTS_SUBDIR, LAST_LAYOUT_FILE, crate::backend_registry::BACKENDS_FILE];
...
///    over everything below. The env-reading half of `vike_model::state_path` (which is pure), and
...
mod tests {
    ...
    fn a_relocation_that_strands_a_family_member_says_so_and_names_both_directories() {
        ...
        std::fs::write(old.join(crate::backend_registry::BACKENDS_FILE), "{}")
            .expect("plant a backends.json");
        let notice = relocation_notice(Some(&old), Some(&new))
            .expect("a stranded family member is reported");
        ...
        assert!(
            notice.contains(crate::backend_registry::BACKENDS_FILE),
            "…and names the member that was left behind: {notice}"
        );
        assert!(
            notice.contains("VIKE_STATE_ROOT"),
            "…and the one setting that keeps the old answer: {notice}"
        );
        ...
        assert!(
            old.join(crate::backend_registry::BACKENDS_FILE).exists(),
    ...
    fn the_family_member_list_holds_every_file_this_module_resolves() {
        ...
        assert!(
            FAMILY_MEMBERS.contains(&crate::backend_registry::BACKENDS_FILE),
    ...
    fn the_user_sized_latch_round_trips_and_an_old_file_loads_unlatched() {
        ...
        let ws = capture(&wins, DisplayTz::Local, 2.0, false);
