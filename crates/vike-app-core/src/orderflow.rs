...
pub struct OrderflowAgg {
    ...
    drop_warn_at: u64,
    /// The tape-gap EPOCH this aggregator has already repaired to — see [`OrderflowAgg::mark_gap`].
    gap_epoch: u64,
}
...
impl OrderflowAgg {
    ...
    /// **A hole opened in this key's tape at `epoch`** — DROP AND REBUILD, and answer whether this
    /// call was the one that did it.
    ///
    /// `epoch` is the same monotonic per-`(venue, symbol)` counter
    /// [`TickVolAgg::mark_gap`](crate::tickvol::TickVolAgg::mark_gap) takes, and that method's doc
    /// carries the argument for a counter (the producer is another thread and cannot reach these
    /// aggregators) and for the `bool` (the caller must DISCARD the batch it drained, because the
    /// drain and the epoch read are two instants).
    ///
    /// # Why the whole aggregator, and why that is the interim rather than the end state
    ///
    /// [`OrderflowAgg::ingest`] has no per-trade dedup and no gap concept, and CVD is CUMULATIVE:
    /// a hole understates one bar's cells and EVERY CVD value from that bar onward, with nothing
    /// anywhere able to say which cells are short. `cells` is index-aligned to the chart's bars, so
    /// there is no narrower repair available — unlike `TickVolAgg`, whose closed bars all closed
    /// before the hole. **Silently continuing is the one option that must not be taken: it shows a
    /// WRONG CVD rather than a missing one.** So this drops the history and rebuilds empty.
    ///
    /// ⚠ The design's end state is a RENDERED BADGE — a break painted at the gap, history kept —
    /// and this is explicitly the budgeted interim until that exists
    /// (`docs/superpowers/specs/2026-09-09-datahub-market-data-wire-design.md` §9 item 4).
    ///
    /// ⚠ **`generation` is BUMPED, never reset**, and that is not tidiness. `cache`'s own doc
    /// records the ABA hazard it exists to close: `vike_chart`'s CVD cache keys on
    /// `(generation, len)`, and a generation that went back to 0 while `cells` changed shape is
    /// exactly the stale-served-frame this counter was introduced to make impossible. The
    /// held-tick and discard COUNTERS are cumulative process totals feeding a doubling log
    /// schedule, so they are kept too — a gap is not a reason to forget what was already lost.
