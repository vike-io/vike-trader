...
//! - [`dom_math`] — DOM pure math: the signed-position resolver + the no-book classifier that
...
//!   seam itself is unchanged: `reconcile_enabled` + `build_recon_config` +
//!   `health_from_feed_status`, all over a caller-supplied map.
...
//! - [`tradehub_control`] — the thin-client Scope::Write WRITE-path lowering: `wire_from_command`
...
// ⚠ `reconcile_config` left this list on 2026-09-23 — it lives in `vike-tradehub` now, with its
// only caller. Nothing on this side ever used it: `crates/vike-desktop/src/main.rs` names it zero
// times, which is what made the "both live roots" argument for sharing it stale rather than
// merely unused.
pub use vike_ops::{alerting, journal_mat, scan, settings, shutdown};
