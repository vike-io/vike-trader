...
pub struct BackendRecord {
    ...
    ///
    /// ⚠ **A custom name only resolves ALONGSIDE a platform key in the same file.**
    /// `vike_secrets::resolve_node_keys` decides WHICH file answers by probing it for
    /// `vike_model::credential_keys::is_platform_key`, a fixed four-name table — so a `node.env`
    /// carrying ONLY a custom name fails that probe and the whole resolution falls through to
    /// `secrets.env`. Leave this blank unless you have a reason not to.
    ///
