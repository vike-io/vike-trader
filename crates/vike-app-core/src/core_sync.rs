...
pub struct CoreSyncInputs<'a> {
    ...
    /// ([`split_plane::direct_bars_mount`](crate::split_plane::direct_bars_mount) is the arm-table
    ...
    /// [`split_plane::BarPlane`](crate::split_plane::BarPlane) for what the conflation cost.
    ...
    pub bar_plane: crate::split_plane::BarPlane,
    ...
    /// ([`md_session::MdSession::tape_gap_epoch`](crate::md_session::MdSession::tape_gap_epoch)),
    ...
    /// [`TickVolAgg::mark_gap`](crate::tickvol::TickVolAgg::mark_gap) were therefore all nominal:
...
pub struct CoreSyncState<'a> {
    ...
    /// the one list [`crate::series_follow`]'s three surfaces (adoption, the symbol picker's
    ...
    pub published: &'a mut Vec<crate::series_follow::PublishedSeries>,
...
///
/// ⚠ **KEEP IT.** [`crate::series_follow`] makes the `not subscribed GUI-side` reason much rarer —
...
pub fn sync_from_core(inp: CoreSyncInputs<'_>, st: CoreSyncState<'_>) {
    ...
    debug_assert_eq!(
        direct_bars.is_some(),
        bar_plane != crate::split_plane::BarPlane::None,
    ...
    if snap.seq != *last_seq {
        ...
        *published = crate::series_follow::published_series(snap);
        for ((venue, symbol, interval), series) in &snap.bars {
            ...
            let source = crate::split_plane::series_render_source(
                bar_plane,
                venue,
                interval,
                published_live,
        ...
        if generation != *last_direct_gen {
            *last_direct_gen = generation;
            for (venue, symbol, interval) in store.keys() {
                ...
                if crate::split_plane::series_render_source(
                    bar_plane,
                    &venue,
                    &interval,
                    published_live,
...
mod test_support {
    ...
    pub struct State {
        ...
        pub published: Vec<crate::series_follow::PublishedSeries>,
        pub last_seq: u64,
    ...
    pub fn sync(
        ...
        sync_with(
            ...
            feed_status,
            None,
            crate::split_plane::BarPlane::None,
            None,
        );
    }
    ...
    pub fn sync_direct(
        ...
        sync_with(
            ...
            feed_status,
            Some(direct_bars),
            crate::split_plane::BarPlane::VenueFeeds,
            None,
        );
    }
    ...
    pub fn sync_backend_store(
        ...
        sync_with(
            ...
            feed_status,
            Some(direct_bars),
            crate::split_plane::BarPlane::BackendStore,
            None,
        );
    }
    ...
    fn sync_with(
        ...
        plane: crate::split_plane::BarPlane,
        ...
        let from_state = move |v: &str, s: &str| {
            snapshot.get(&(v.to_string(), s.to_string())).copied().unwrap_or(0)
        };
...
mod backfill_hold_tests {
    use super::test_support::*;
    use super::*;
    use crate::feed_lifecycle::{BackfillRetries, should_spawn_backfill};
    use vike_core::CoreSnapshot;
...
mod direct_bar_tests {
    ...
    fn under_the_backend_store_plane_the_published_series_folds_live_and_the_rest_from_the_store() {
        ...
        store.seed(
            ...
            "BTCUSDT",
            "5m",
            vec![core_bar(300_000, 42.0), core_bar(600_000, 43.0)],
        );
