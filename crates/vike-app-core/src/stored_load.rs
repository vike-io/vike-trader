    ...
    for (id, _cov) in &inv {
        match store.series_gaps(id) {
            Ok(ranges) if !ranges.is_empty() => {
                ...
                // tree is its GROUP NAME. `crates/vike-data-manager/src/model.rs`'s `build_tree`
                // keys the tree on `id.label()` for exactly that reason, and the grid's own
                // `series_key` looks up this map with the node label it got from there.
                //
                // Keyed on `symbol`, every grouped series produced a key that matched no row, so
                // the Stored grid painted every polymarket group as gap-FREE and the "Has gaps"
                // smart view could never list one. No test caught it: every gap test builds its
                // fixture with `SeriesId::per_symbol`, where `symbol == label` and the two spellings
