...
pub fn run_wire_backfill(addr: &str, jobs: &[BackfillJob], skipped: usize) -> WireBackfillReport {
    // ⚠ **UNAUTHENTICATED ON PURPOSE — do NOT "fix" this the way the store plane was fixed.**
    // `Backfill` is a CONTROL-scope verb (`vike_datahub::server`'s `required_scope`), and
    // `crate::backend_registry::BackendRecord::datahub_observe_key` states why this binary may not
    // hold a datahub control key: control scope also carries the verbs that COMPILE
    // CLIENT-SUPPLIED RHAI, "so a desktop that only wants a DOM ladder must never hold that key".
    //
    // The consequence is real and is declared rather than hidden: against a datahub that requires
    // authentication — which, by `vike_datahub_client::bind::bind_decision`, is EVERY datahub bound
    // off-box — this dial fails and the run returns [`WireBackfillReport::ConnectFailed`] carrying
    // the connect error, which names the missing keys. That is the closed-gate direction and the
    // operator gets a sentence, not a silent no-op. Giving this call an observe key would not help
    // either: the handshake would succeed and the server would then refuse the verb on scope.
    //
    // The fix, when someone wants GUI-initiated backfill against a keyed server, is a SERVER-side
    // or CLI-side path that already holds control scope — not a control key in this process.
...
mod tests {
    ...
    fn the_ran_line_counts_like_the_local_arm_and_carries_the_first_error_verbatim() {
        let refusal = "backfill: venue `polymarket` has no collector in this build. \
