...
//! daemon's observe server under `Scope::Read` and republishes each pushed
...
pub fn observing_status(
    ...
    let addr =
        crate::backend_identity::shown_address(addr, identity.map(|id| id.advertise_addr.as_str()));
...
/// `backend` is the record's NAME (`crate::backend_identity::status_label`) — the one thing this
...
mod tests {
    ...
    /// (`crates/vike-app-core/src/backend_identity.rs` carries the measurement).
    #[test]
