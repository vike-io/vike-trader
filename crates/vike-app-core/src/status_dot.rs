...
//! near-copies into it. `AMBER` below is a local NAME for one of those shared constants, not a
//! value; `MUTED` is imported under its own name.

use vike_model::feed_status::{ConnectionState, parse_feed_status};
use vike_ui_theme::palette::status::MUTED;
...
mod tests {
    ...
    fn a_healthy_observe_link_now_reads_connected() {
        let line = crate::observe_bridge::observing_status(None, "127.0.0.1:9301", None);
