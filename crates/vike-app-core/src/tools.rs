...
fn fetch_finnhub_earnings(agent: &ureq::Agent, key: &str, frm: &str, to: &str) -> Vec<EarningsRow> {
    ...
    if let Some(v) = nasdaq_json(agent, &url)
        && let Some(arr) = v["earningsCalendar"].as_array()
    {
        for r in arr {
...
fn fetch_fmp_dividends(agent: &ureq::Agent, key: &str, frm: &str, to: &str) -> Vec<DividendRow> {
    let mut out = Vec::new();
    let url = format!(
        "https://financialmodelingprep.com/stable/dividends-calendar?from={frm}&to={to}&apikey={key}"
    );
    if let Some(v) = nasdaq_json(agent, &url)
        && let Some(arr) = v.as_array()
    {
        for r in arr {
            ...
            let amount = r["dividend"].as_f64().map(|x| format!("{x:.2} $")).unwrap_or_default();
...
pub fn resolve_options_selection<'a>(
    ...
    if let Some(sel) = expiry_sel.as_ref()
        && !bundle.chains.contains_key(sel)
    {
        *expiry_sel = None;
    }
    Some(bundle)
...
///
/// ⚠ `chain_rec` is **`None` at every call site in the tree**, and this line used to read "`None`
/// unless `vike-app` resolved one from `VIKE_RECORD_CHAINS=1`". That root is gone —
/// `crates/vike-desktop/src/main.rs` binds it to `None` outright, and
/// [`vike_data::ChainRecorder::open_from_env`] is called from nowhere — so
/// `VIKE_RECORD_CHAINS=1` arms nothing here. `vike_config::CONSUMPTION`'s `flags.record_chains`
/// row is the gated authority; a doc comment still promising the old wiring is how that key went
/// on being advertised as settable. `None` is byte-identical to pre-recording behavior:
...
/// fires only when recording is actually live, which today is never.
///
...
pub fn apply_ticker_to_chains(
    ...
    for row in rows {
        ...
        let usd = |v: Option<f64>| -> Option<f64> {
            if scale == 0.0 { None } else { v.map(|x| x * scale) }
        };
        let t = vike_options::years_to_expiry(&expiry_iso, now);
        let Some(slot) = chain.rows.iter_mut().find(|sr| sr.strike == strike) else { continue };
        let q = match kind {
...
///
/// `chain_rec` is the opt-in option-chain snapshot recorder, threaded into the options-refresh
/// thread's provider by [`options_provider`] so every polled chain would land in the `kind=chain`
/// PIT series. `None` leaves the options path byte-identical to pre-recording behavior.
///
/// ⚠ **It is `None` at EVERY call site in the tree, and this line used to say "the app root's …
/// (`VIKE_RECORD_CHAINS=1`, `None` by default)" as though a root still resolved one.** It does not:
/// `crates/vike-desktop/src/main.rs` binds it to `None` outright, and
/// [`vike_data::ChainRecorder::open_from_env`] has no call site anywhere. The correction belongs
/// HERE and not only on [`options_provider`] — this is the function that doc names as its caller,
/// so leaving the old sentence standing here re-asserted the wiring the other one had just
/// retracted. `vike_config::CONSUMPTION`'s `flags.record_chains` row is the gated authority.
///
/// ⚠ **What is missing is the STORE, not the fetch.** The options-refresh thread below still polls
/// chains on its 30 s cadence in the desktop, exactly as before; the desktop simply links vike-data
/// with DEFAULT features now (the trait-only `HistStore` seam), and `open_from_env` is a
/// `hist-datafusion` constructor, so there is no engine for it to open. The tombstone at the call
/// site in `crates/vike-desktop/src/main.rs` says the same thing.
///
...
pub struct ToolView {
    ...
    // Data
    /// Which Data-Manager rail destination this window is showing.
    ///
    /// ⚠ Was `data_subtab: usize` with an index-map comment and an append-only rule, both of which
    /// rested on the claim that the workspace file persisted it. It does not — see
    /// `crates/vike-app-core/src/tool_views/data_rail.rs`'s module doc for the measurement.
    pub data_dest: crate::tool_views::DataDest,
    ...
    /// Which of the Connections window's two tabs is showing.
    ///
    /// ⚠ Here rather than in the widget's own egui temp memory because it is written from OUTSIDE
    /// the tab body: the ambient strip at the foot of the window is identical on both tabs, and
    /// its **Add backend** has to switch to the Backend tab (where the form renders) as well as
    /// opening the form. Per window, like `data_dest`.
    pub connections_tab: crate::tool_views::ConnectionsTab,
    /// The Backend tab's settings-table row filter (`set · N` / `all · N`). Per window, so two
    /// Connections windows can watch different slices of one node's settings.
    pub connections_settings_filter: crate::tool_views::SettingsFilter,
    ...
    /// `vike_data_manager::stored_catalog_grid` — query, sort,
...
impl Default for ToolView {
    fn default() -> Self {
        Self {
            ...
            cal_selected_day: -1,
            cal_high_only: false,
            // ⚠ The window opens on BROWSE now, not on an editor. It used to open on index 0, the
            // DataSet editor, because that was the leftmost tab — a position, not a decision.
            // `the_default_destination_is_browse` in `tool_views::data_rail` holds the new one.
            data_dest: crate::tool_views::DataDest::default(),
            data_sel: None,
            data_delete: None,
            data_log: Vec::new(),
            arm_edit: crate::tool_views::ArmEdit::Idle,
            connections_account: None,
            connections_tab: crate::tool_views::ConnectionsTab::default(),
            connections_settings_filter: crate::tool_views::SettingsFilter::default(),
