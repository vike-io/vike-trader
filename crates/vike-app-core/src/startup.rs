...
pub struct StartupLayout {
    ...
    /// env read.
    pub data_dest: Option<(egui::Id, crate::tool_views::DataDest)>,
...
pub const DATA_SHOT_DEST: crate::tool_views::DataDest = crate::tool_views::DataDest::AllSeries;
...
pub fn plan(
    ...
    if let Some(ws) = restored {
        ...
        out.data_dest = Some((w.id, DATA_SHOT_DEST));
        out.wins.push(w);
        ...
        //
        // ⚠ It needs its OWN arm rather than riding `data`, which hardcodes `DATA_SHOT_DEST`.
        ...
        out.data_dest = Some((w.id, crate::tool_views::DataDest::VenueArming));
        out.wins.push(w);
    } else if shot_win == Some("instruments") {
        // QA: VIKE_SHOT_WIN=instruments opens the Data Manager on its INSTRUMENTS destination —
        // the per-venue instrument-catalog table: cached count, refresh stamp, status, button.
        //
        // ⚠ Same argument as the `venues` arm above: `data` hardcodes [`DATA_SHOT_DEST`], so a
        // destination with no arm of its own is reachable by no capture at all. What a human has
        // to check here is specifically a NEGATIVE — that a venue this binary links no provider
        // for renders its reason instead of a button — and a negative is exactly the kind of claim
        // an accessibility-tree test states and a rendered frame proves.
        //
        // Same geometry as `venues`: both are one-row-per-roster-venue grids.
        let r =
            egui::Rect::from_min_size(area.min + egui::vec2(8.0, 8.0), egui::vec2(1200.0, 700.0));
        let w = WinState::tool("tool-instruments", WinKind::Data, r);
        out.data_dest = Some((w.id, crate::tool_views::DataDest::Instruments));
        out.wins.push(w);
    } else if shot_win == Some("overview") {
        // QA: VIKE_SHOT_WIN=overview opens the Data Manager on its LANDING screen — the attention
        // tiles and the outstanding-work rows.
        //
        // ⚠ Same argument as the `venues` arm above, and it applies twice over here. `data`
        // hardcodes [`DATA_SHOT_DEST`] (the grid), so without this arm the screen the window OPENS
        // ON was reachable by no capture at all — the one surface every operator sees first would
        // have been the one surface never reviewed. The contact sheet is the only place a human
        // looks at a rendered surface in this repo; no CI runner has a GPU.
        //
        // Same geometry as `data`: the tiles wrap against the body width, so a narrower window
        // would review a layout nobody runs.
        let r =
            egui::Rect::from_min_size(area.min + egui::vec2(8.0, 8.0), egui::vec2(1200.0, 760.0));
        let w = WinState::tool("tool-overview", WinKind::Data, r);
        out.data_dest = Some((w.id, crate::tool_views::DataDest::Overview));
        out.wins.push(w);
...
mod tests {
    ...
    /// The two Data-Manager arms open it on the destination they NAME.
    ///
    ...
    fn the_data_manager_arms_select_the_destination_they_name() {
        for (val, want) in [
            ("data", DATA_SHOT_DEST),
            ("venues", crate::tool_views::DataDest::VenueArming),
            ("overview", crate::tool_views::DataDest::Overview),
            ("instruments", crate::tool_views::DataDest::Instruments),
        ] {
            let l = plan(area(), &env_with_shot_win(val), None);
            let (id, dest) =
