...
//! `VIKE_STYLE`, `VIKE_SCALE`, `VIKE_POLY_COCKPIT_TOKEN` — plus a saved-workspace restore, and every
...
use crate::workspace::{self, persist, WinKind, WinState};
use vike_chart::{chart::ChartStyle, DisplayTz, ScaleMode};
...
pub fn plan(
    ...
    if let Some(s) = env.style.as_deref() {
        if let Ok(idx) = s.parse::<usize>() {
            if let Some(&st) = ChartStyle::ALL.get(idx) {
