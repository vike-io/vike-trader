...
//! compile the crate runs its unit tests over `crates/vike-app/src/chart_gpu.rs`'s GPU byte-layout
...
//! `crates/vike-app/src/main.rs`'s `apply_spawn` applies the plan in the order documented on
...
use crate::workspace::{WinKind, WinState, DEFAULT_VENUE};
use vike_panels::dom;
...
///
/// One variant per call site in `crates/vike-app/src/main.rs`'s frame loop, deliberately NOT
...
pub enum SpawnRequest {
    ...
    /// `crates/vike-app/src/chart_window.rs`'s picker fallback, so the list belongs to `vike-app`,
...
///
/// The order below is the order `crates/vike-app/src/main.rs` performed these steps at every site
