...
pub struct ChartTarget {
    ...
    /// (`crates/vike-desktop/src/app_ui.rs`'s `draw_windows`), `crate::workspace::persist` round-trips
    /// it across restarts, and `crate::startup::plan` already carries it into a `FeedSpec` — so the
    ...
    /// [`crate::store_bars::StoreBarRequest`] and, beyond it, the seed request's optional class.
...
/// backend-STORE read ([`crate::store_bars`]) exists to serve, and why the two changes only make
...
/// ([`crate::chart_seed::render_chart_seed_status`]). They are different questions and an operator
