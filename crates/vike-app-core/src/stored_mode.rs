...
//!   trait verb. A remote-mode Delete that "worked" would delete from the LOCAL store the grid is
//!   not even showing.
//!
//!   ⚠ This row's REASON changed on 2026-09-07 and its verdict did not. It read "the datahub wire
//!   serves no delete", which PR #1688 made false. What keeps Delete grayed here is now the seat
//!   rather than the wire: the verb is served only by a KEYED datahub, and this crate resolves no
//!   datahub keys. [`DELETE_LOCAL_ONLY`] carries the whole argument, including why that constant
//!   was NOT renamed the way `PARTIALS_LOCAL_ONLY` was.
//!
...
///
/// ⚠ **The REASON changed on 2026-09-07 and the NAME deliberately did not**, which is the opposite
/// call from [`PARTIALS_UNSERVED`] one constant below — so the difference is worth stating rather
/// than leaving as an apparent inconsistency.
///
/// The old text was *"Delete is a local-store operation — the datahub wire serves no delete verb"*.
/// Its second clause is now FALSE: PR #1688 built `Request::DeleteSeries`, and
/// `crates/vike-cli/src/cmd/data.rs` reaches it over `--addr`. What did NOT change is the
/// CONCLUSION, and it is still the right one for this seat: the verb is served only by a KEYED
/// datahub (`docs/decisions/0050-a-key-less-datahub-serves-no-delete-verb.md`), and nothing in
/// `vike-app` or `vike-app-core` resolves `VIKE_DATAHUB_OBSERVE_KEY` / `VIKE_DATAHUB_CONTROL_KEY` —
/// `vike-cli` learned to in #1691 and this GUI has not. So a remote Delete from here would be
/// refused at the far end whichever kind of server answered: unadvertised on a key-less one,
/// unauthenticated on a keyed one.
///
/// `PARTIALS_LOCAL_ONLY` was RENAMED because the falsehood sat in its name — the coverage report
/// had become a wire verb the GUI could actually use. Here "local only" is still true FROM THIS
/// SEAT, so only the reason moved. The day this crate resolves datahub keys, the name goes too.
pub const DELETE_LOCAL_ONLY: &str = "Delete is local-store only here — the wire's delete verb needs a keyed datahub, and this app \
     holds no datahub keys";
...
mod tests {
    ...
    fn remote_mode_grays_delete_with_a_reason() {
        ...
        assert!(
            ...
            "the reason should say Delete is local-store only from here: {reason}"
        );
        // ⚠ And that it says WHY, which is the half that went stale once the wire grew the verb:
        // `contains("local")` alone stayed green through a reason that had become false.
        assert!(
            reason.contains("key"),
            "the reason must name the key requirement — the wire DOES serve a delete verb since \
             #1688, and what stops this app is holding no datahub keys: {reason}"
        );
    }
