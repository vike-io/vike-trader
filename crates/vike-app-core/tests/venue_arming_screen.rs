...
/// `venues_tab_content`). It inherited a ~3 s spin budget argued entirely for a CLI — three seconds
/// of frozen window with no repaint, no spinner and no cancel, which an operator reads as "the app
/// hung". Two assertions, neither of them a duration:
///
/// 1. the budget this surface passes IS `LockBudget::NON_BLOCKING` — one `try_lock`, and
///    `max_wait_ms() == 0`, which `vike-config`'s own spin test proves means no sleep is executed;
/// 2. with the directory genuinely HELD by another writer, the call RETURNS — taking the refusal
///    arm rather than waiting for the holder — the row's own `ARM_BUSY_NOTE` reaches the operator,
///    and no byte of `policy.toml` moves.
///
/// The lock is taken here the way another PROCESS would take it (by path, with the bare `std`
/// primitive) — `flock`/`LockFileEx` key on the open file description rather than the process,
/// which is what makes a single-process test of a cross-process interlock meaningful.
#[test]
fn the_arming_write_cannot_block_the_frame_it_runs_in() {
    assert_eq!(
        vike_app_core::tool_views::VENUE_ARM_LOCK_BUDGET,
        vike_config::LockBudget::NON_BLOCKING,
        "a render-thread write may not be given a budget it can spend"
    );
    assert_eq!(
        vike_app_core::tool_views::VENUE_ARM_LOCK_BUDGET.max_wait_ms(),
        0,
        "…and NON_BLOCKING must mean zero, not 'not much'"
    );
    ...
    let held = std::fs::OpenOptions::new()
        ...
        .create(true)
        .truncate(false)
        .open(dir.path().join(vike_config::SETTINGS_LOCK_FILE))
        .expect("the sentinel opens");
    held.try_lock().expect("this test must be the holder for anything below to prove anything");
    ...
    let err = apply_arming(
        ...
        "binance",
        VenueMode::Demo,
        VenueMode::Paper, // a TIGHTENING: no ceremony, so the lock is the only thing in the way
        None,
        None,
        T,
    )
    .expect_err("a held settings directory must refuse rather than spin the frame out");
    assert_eq!(
        err,
        vike_app_core::tool_views::ARM_BUSY_NOTE,
    ...
    assert_eq!(policy_text(dir.path()), before, "a busy refusal changes NO byte");
    drop(held);
    ...
    .expect("the same write lands once the directory is free");
}
