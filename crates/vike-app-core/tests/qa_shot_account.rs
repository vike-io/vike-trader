...
fn fixture_store(script: &str) -> HashMap<String, String> {
    ...
    assert!(lines.next().is_some(), "scripts/qa_shots.sh no longer writes a fixture store: the heredoc opening with {FIXTURE_HEREDOC} is gone, so the labelled-account capture has no account to select");
