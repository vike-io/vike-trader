...
fn a_datahub_only_node_env_does_not_strip_the_tradehub_pair() {
    ...
    plant(
        ...
        "node.env",
        &[
            (vike_node_proto::auth::DATAHUB_OBSERVE_KEY_ENV, "dh-obs"),
            (vike_node_proto::auth::DATAHUB_CONTROL_KEY_ENV, "dh-ctl"),
        ],
    );
...
fn a_tradehub_only_node_env_does_not_hide_the_datahub_key() {
    ...
    plant(
        split.path(),
        "secrets.env",
        &[(vike_node_proto::auth::DATAHUB_OBSERVE_KEY_ENV, "dh-legacy")],
    );
    plant(
        split.path(),
        "node.env",
    ...
    assert_eq!(keys.key_for(vike_node_proto::auth::Scope::Read), b"dh-legacy");
    assert!(
...
fn a_custom_datahub_key_name_resolves_out_of_the_file_that_carries_it() {
    ...
    let observe = |dir: &Path| -> (Option<Vec<u8>>, Vec<String>) {
        ...
        (keys.map(|k| k.key_for(vike_node_proto::auth::Scope::Read).to_vec()), notices)
    };
