...
fn signing_pair(vars: &HashMap<String, String>) -> (Option<Vec<u8>>, Option<Vec<u8>>) {
    ...
    let observe = keys.has(Scope::Observe).then(|| keys.key_for(Scope::Observe).to_vec());
    let control = keys.has(Scope::Control).then(|| keys.key_for(Scope::Control).to_vec());
    (observe, control)
}
...
fn a_tradehub_only_node_env_does_not_hide_the_datahub_key() {
    ...
    assert_eq!(keys.key_for(vike_datahub_client::Scope::Observe), b"dh-legacy");
    assert!(
...
fn a_custom_datahub_key_name_resolves_out_of_the_file_that_carries_it() {
    ...
    let observe = |dir: &Path| -> (Option<Vec<u8>>, Vec<String>) {
        ...
        (keys.map(|k| k.key_for(vike_datahub_client::Scope::Observe).to_vec()), notices)
    };
