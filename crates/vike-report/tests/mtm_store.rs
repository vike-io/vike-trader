...
//!
//! ⚠ `#![cfg(feature = "journal")]` on the whole file: the function under test
//! (`vike_report::mtm_curve_from_store`) is the one item in `src/mtm.rs` that names a `HistStore`,
//! so it is gated and so is its gate. `RuntimeStats` and `mtm_equity_curve` are NOT gated — they
//! are folds over `MtmPoint` — but they are only interesting here beside the store, so the file
//! travels as one.
...
use vike_report::{RuntimeStats, mtm_curve_from_store, mtm_equity_curve};
