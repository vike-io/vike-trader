...
//!
//! ⚠ `#![cfg(feature = "journal")]` on the WHOLE file rather than per test: `spawn_core` is
//! vike-core and `fills_from_journal` is the gated reader, so without that feature there is
//! nothing here to compile — and the inner attribute strips the file before `use` resolution, which
//! is why the imports below need no gate of their own.
...
fn journal_roundtrip_reconstructs_one_trade() {
    ...
    // (`crates/vike-journal/src/segment.rs`'s `reserve_blocks`), which is the exact shape that
    ...
    // callee does, inside `crates/vike-journal/src/writer.rs`'s `CommandJournal::open`. So this
    ...
    // The full from_journal path yields a finite, one-trade tearsheet. Read by CATALOG ID rather
    // than by field: `LiveTearsheet` is keyed on `vike_analytics::metric_catalog::METRICS` now, so
    // `metric(id)` is the accessor and `None` means the document does not carry that number.
    let sheet = LiveTearsheet::from_journal(&dir, 10_000.0, 252.0).expect("tearsheet");
    assert_eq!(sheet.metric("n_trades"), Some(1.0));
    assert_eq!(sheet.metric("net_profit"), Some(10.0));
    assert!(sheet.metric("final_equity").expect("a compact scalar always answers").is_finite());
    // The document declares its shape, which is what lets a reader branch before it reads a value.
    assert_eq!(sheet.schema, vike_report::TEARSHEET_SCHEMA);
