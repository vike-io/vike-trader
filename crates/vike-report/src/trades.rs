...
mod tests {
    ...
    ///
    /// ⚠ Behind the `journal` feature: `vike_exec` is an OPTIONAL dependency enabled by it, so
    /// this test does not COMPILE in a feature-less build — and the `report-journal-off` lane
    /// found it the first time that configuration was ever built. Gating it costs the anchor
    /// nothing: the feature is `default`, so every ordinary build, `just test` and every CI roster
    /// lane still run it, and only the compile-only lane skips it.
    #[cfg(feature = "journal")]
    #[test]
