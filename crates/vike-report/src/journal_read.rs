...
//!
//! ⚠ **This module is the whole reason the crate's `journal` feature exists**, and it is the one
//! file here that names all three of vike-core (the journal), vike-exec (the `Ingest` the records
//! wrap) and vike-data (the Tier-2 `kind=exec_fill` series [`fills_from_store`] reads). That is the
//! dependency closure `crates/vike-tradehub-client/src/proto.rs`'s `Response::Tearsheet` and
//! `crates/vike-cli/src/cmd/report.rs` both name as the reason they would not link this crate — so
//! the three go behind one gate together rather than three, because reading a fill stream needs all
//! three and nothing else here needs any of them.
