...
//!
//! ⚠ Behind the crate's `journal` feature, and so is the `tearsheet` BIN (`required-features` in
//! the manifest): this tool's whole input is a journal directory, so it cannot exist in a build
//! that links no journal reader. `crates/vike/Cargo.toml`'s `tearsheet = ["dep:vike-report"]` takes
//! this crate with DEFAULT features, where `journal` is on, so the multicall arm is unchanged.
//!
...
//!             [--html PATH] [--store DIR | --datahub] [--mtm] [--venue V] [--interval STR]
//!
...
//! rollup, not per-venue). Built WITHOUT `--features hist`, both `--store` and `--datahub` error
//! (exit 2) — neither store backend is compiled in.
//!
//! ## `--datahub` (the same enrichment, read over the WIRE)
//!
//! `docs/decisions/0084-only-the-datahub-touches-the-store.md` gave the hist store ONE reader and
//! asked every other one to ask over the wire. `--datahub` is this tool's side of that: the SAME
//! two enrichments, with the series fetched from the data daemon instead of opened from local
//! Parquet. The address is `$VIKE_DATAHUB_ADDR` or the compiled default — the short ladder a bare
//! research bin gets, because this tool loads no settings file and inventing one for it would give
//! a box two answers to "where is my datahub". Node keys come from the credential STORE, and an
//! unreadable store warns rather than refusing; `vike_datahub_client::route`'s own doc argues both.
//!
//! ⚠ **The WIRE is not the default here, and that is a difference from every other reader 0084
//! routed.** For the `cheap_np` bins and the compute daemon, no flag means the wire and `--store`
//! is the opt-out — those tools always read history. This one does not: with neither flag it
//! computes the realized-only tearsheet from the journal alone and touches no store at all, so
//! "no flag" was already spoken for. Making the wire silent would have turned every existing
//! journal-only invocation into a network read. The two flags are mutually exclusive (both is
//! exit 2), and every command line that worked before this change still does exactly what it did.
//!
//! ⚠ **It still needs `--features hist`, and so still links DataFusion.** 0084 is about who
//! TOUCHES the store, not about who links the engine: `--store DIR` remains the documented way
//! back, so the local backend has to stay compiled in. A wire-only build would need a second
//! spelling of the route decision, which is what moving that decision below both its readers
//! existed to prevent.
//!
//! `--mtm` (requires `--store` or `--datahub`) swaps the equity source: instead of the sampler's `kind=equity`
...
pub fn run(env: &std::collections::HashMap<String, String>, args: &[String]) -> ExitCode {
    ...
    let Some(dir) =
        ...
        eprintln!(
            "tearsheet: --journal <dir> is required (or set $VIKE_JOURNAL_DIR)\n\
             ...
             [--json] [--html PATH] [--store DIR | --datahub] [--venue V] [--interval STR]"
        );
    ...
    // ⚠ A BLANK `--store` is REFUSED, not treated as absent and not passed on.
    //
    // Newly reachable since `vike_analytics::binutil::arg` learned the inline `=` spelling:
    // `--store=` used to match nothing and this fell through to the journal path. Both silent
    // answers are wrong, and the new one is the worse of the two — an empty root reaches
    // `DataFusionHist::open`, which `create_dir_all`s what it is given and then resolves queries
    // relative to the WORKING DIRECTORY. `crates/vike-backtest/src/binutil.rs`'s
    // `store_root_resolved` can fall THROUGH a blank because it has lower rungs to fall to; this
    // flag is the whole selector between two paths, so the only honest answer is to say so.
    if arg(args, "--store").is_some_and(|s| s.trim().is_empty()) {
        eprintln!(
            "tearsheet: --store was given an empty value — name a hist-store root, or drop the flag to read the journal"
        );
        return ExitCode::from(2);
    }
    let store_flag = arg(args, "--store");
    let wire = has_flag(args, "--datahub");
    // ⚠ Two flags naming two different histories is a REFUSAL, not a precedence rule. Picking one
    // silently would publish a tearsheet whose numbers came from a source the operator did not
    // choose, and a report nobody can attribute is worse than one that did not run.
    if wire && store_flag.is_some() {
        eprintln!(
            "tearsheet: --store and --datahub name two different histories — pass one. \
             --datahub reads the series from the data daemon, which \
             docs/decisions/0084-only-the-datahub-touches-the-store.md made every other reader's \
             default; --store DIR opens local Parquet and is the documented way back"
        );
        return ExitCode::from(2);
    }
    // Store-backed path if EITHER flag selects a history; otherwise the unchanged realized-only
...
/// The store-backed path, WITHOUT the `hist` feature: neither backend is compiled in — no concrete
/// `DataFusionHist` for the local arm and no `vike_datahub_client::route` for the wire one — so
/// there is nothing to open either way. Fail clearly (exit 2) rather than silently ignoring.
// ⚠ This function sat at EXACTLY clippy's ceiling (7) before the route arrived, and `env` — the
// caller-owned sweep `history_route` resolves the datahub address and the credential-store
// override from — is the argument that pushed it to 8. It is threaded rather than read here
// BECAUSE `crates/vike-ops/tests/settings_registry.rs` asks libraries to take configuration as a
// parameter; an `env::var` at this site would clear the lint and add a `Layer::Library` row to a
// ratchet that may only shrink. The allow is the cheaper of the two, and it is this tree's
// established shape for an argv-derived flat list — `crates/bridges/binance/src/exec.rs` carries
// the same attribute with the same kind of note about what pushed it over.
#[allow(clippy::too_many_arguments)]
#[cfg(not(feature = "hist"))]
fn build_from_store(
    _journal: &std::path::Path,
    _store_dir: Option<&str>,
    ...
    eprintln!("tearsheet: --store and --datahub both require building with --features hist");
    Err(ExitCode::from(2))
}

/// The store-backed path, WITH the `hist` feature: ROUTE the history (local files on `--store DIR`,
/// the data daemon otherwise), read the equity curve from its `kind=equity` series (falling back
...
/// with, stated the same way the `cheap_np` bins state it; `--store DIR` is the way back.
// ⚠ See the refusing arm above for why this list is 8 long and why the allow rather than a read at
// this site. Both arms must carry it: a default build compiles that one and the `hist` lanes
// compile this one, so an attribute on either alone is red in whichever lane sees the other.
#[allow(clippy::too_many_arguments)]
#[cfg(feature = "hist")]
fn build_from_store(
    journal: &std::path::Path,
    ...
    // WHERE the history comes from. `--store DIR` is the LOCAL opt-out and anything else is the
    // wire — `history_route`'s rule for every reader 0084 routed. The difference here is only
    // that a caller who passed NEITHER flag never reaches this function at all, which is why
    // `run` refuses the two together rather than letting one win.
    //
    // The root handed to the opener is the flag's own value, and the EMPTY path when there is no
    // flag: the wire arm reads neither it nor the label's copy of it, and a blank `--store` was
    // already refused in `run` — so no branch here can resolve a directory nobody named.
    let local_root = std::path::Path::new(store_dir.unwrap_or(""));
    let route = history_route(store_dir.map(std::path::Path::new), datahub_addr_for_bin(env));
    // ONE disclosure covering both arms, which is what `HistoryRoute::label` is for: on the wire
    // it names the ADDRESS and never a directory, because the resolved root is the SERVER's and a
    // path printed here would be a guess about another box's filesystem.
    eprintln!("tearsheet: history from {}", route.label(local_root));
    let store = match open_routed_history(&route, local_root, env) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("tearsheet: {e}");
            return Err(ExitCode::FAILURE);
