...
//!      `mtm_curve_from_store` (behind the `journal` feature, because a store is the only thing in
//!      this module that needs one) sources that closure from a `vike_data::HistStore`'s bars.
//!      [`reconstruct_mtm`] itself takes the CLOSURE, so the fold is pure and needs no data layer.
//!      Equity is
...
///
/// ⚠ Behind the `journal` feature because [`mtm_curve_from_store`] is its ONLY reader, so a
/// `journal`-off build otherwise warns `constant is never used` — an ERROR under the
/// `-D warnings` clippy gate. It is the only conditional item in this module besides that
/// function; nothing else here reads it.
#[cfg(feature = "journal")]
...
/// [`reconstruct_mtm`]'s price closure. DataFusion-free: takes the venue-agnostic `HistStore` TRAIT
/// object, never a concrete backend, exactly like `crate::excursions::backfill_excursions`.
///
...
///
/// ⚠ Behind the `journal` feature — it is the ONLY item in this module that names vike-data, which
/// is why the fold above takes a closure instead of a store. Its imports live in the body rather
/// than at module scope, so the only OTHER conditional item in this module is the private
/// `MULTIPLIER` const it reads — which has to be, or a `journal`-off build warns on dead code.
#[cfg(feature = "journal")]
pub fn mtm_curve_from_store(
    ...
    venue: &str,
    interval: &str,
) -> Result<Vec<MtmPoint>, vike_data::DataError> {
