...
mod tests {
    use super::*;
    use crate::venues::BRIDGE_VENUES;
    ...
    fn the_desktops_own_routing_is_one_direct_venue_and_eight_server_backed() {
        ...
        let routed =
            BRIDGE_VENUES.iter().filter(|v| catalog_source_for(v, DESKTOP_LINKS).is_some());
    ...
    fn nine_roster_venues_are_publicly_enumerable_and_the_count_is_derived() {
        ...
        let public: Vec<&&str> =
            BRIDGE_VENUES.iter().filter(|v| catalog_availability(v).is_public()).collect();
        let not_public: Vec<&&str> =
            BRIDGE_VENUES.iter().filter(|v| !catalog_availability(v).is_public()).collect();
        assert_eq!(public.len() + not_public.len(), BRIDGE_VENUES.len());
