...
//! the whole reason a `linear`/`inverse`/`SWAP` word must not appear in this crate. The one
//! production consumer today is `crates/bridges/bybit/src/data.rs`'s `route_target`, which reads
//! [`VenueAddressing::must_claim`] to decide whether a bare symbol needs proving at all.
...
mod tests {
    ...
    fn every_roster_venue_has_a_named_row() {
        let src = own_source();
        for &venue in crate::BRIDGE_VENUES {
            assert!(
