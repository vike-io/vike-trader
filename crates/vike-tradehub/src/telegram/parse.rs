...
pub enum Instruction {
    ...
    /// largest order frame. `clippy::large_enum_variant` says so under `-D warnings`, and this
    /// workspace's only `#[allow]`s for it are in the VENDORED `ibapi` copy, i.e. not a precedent
    /// for first-party code. Construct through [`Instruction::write`] rather than spelling the
    /// `Box` at each site.
...
mod tests {
    ...
    fn parses_the_write_grammar() {
        ...
        assert_eq!(
            ...
            Instruction::write(WireCommand::MassCancel {
                venue: None,
                symbol: None,
                account: None
            })
