...
impl TelegramDeps for ProdTelegramDeps {
    ...
    fn preview(&self, cmd: &WireCommand) -> Option<String> {
        // The same two-step verdict the TCP preview arm gives, in the same order: the edge gate,
        // then the ROUTING gate. A chat surface previewing a command whose venue this node does
        // not run must say so here — this channel's whole affordance is that a human types a venue
        // name into a phone.
        self.limits
            .lock()
    ...
    fn accept(&self, cmd: WireCommand, reason: &str) -> Result<String, String> {
        ...
        //
        // ⚠ The ENGINE ROSTER is NOT `None`-shaped here, unlike the two above: this surface holds
        ...
        // refused with the node's own roster rather than applied to its primary engine.
        accept_command(cmd, Some(reason), &mut limits, &self.sink, None, &engines, None, None)
