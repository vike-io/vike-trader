...
//!   `vike_core::CommandSink` in (gated by `VIKE_TRADEHUB_CONTROL=1`) — a `Scope::Write` peer's
...
// The reconcile ENGINE's env-driven configuration and its arming gate. ⚠ This was
// `vike_tradehub::reconcile_config` until 2026-09-23, on the stated ground that the gate is "called
// once by each live root so the GUI and the daemon cannot answer differently". There is ONE live
// root now — the desktop lost its local core (#1610) and names this module zero times — so the
// module that was shared is shared with nobody, and it lives with its only caller.
pub mod publish;
...
// ⚠ A COMMAND, in a daemon's crate, that starts no daemon — `vike-backend catalog`, the
// locally-credentialed instrument listing `docs/decisions/0066`'s decision 9 armed. It lives here
// because this crate is the only operator-facing one that ALREADY links alpaca/oanda/ctrader and
// already opens the credential store; the obvious alternative, `vike-cli`, is refused by
// `crates/vike-boot/tests/dependency_floor.rs`, which holds that binary transport-free. It mounts
// nothing, spawns nothing and can place no order — its own module doc states that at the top.
pub mod catalog_cli;
// `vike-backend venues` — the read-only report of WHICH venues and accounts this box would mount
// and WHY the rest stay on paper. Sits here for `catalog_cli`'s reason above, one rung sharper: its
// load-bearing column is `vike_mount::venue_account_arming`'s answer, which reaches THIRTEEN venue
// crates because each exchange's tier rule is its own. `vike-cli` cannot host it — the same
// `dependency_floor` gate that refuses `catalog` there holds that binary transport-free — so a
// screen built on those crates has exactly one home, and this is it. Like `catalog` it mounts
// nothing, opens no socket and can place no order.
pub mod venues_cli;
