...
fn spawn_node(policy: LinkPolicy) -> (MakerMount, SocketAddr) {
    ...
    thread::spawn(move || {
        let _ = serve_with_link_policy(
            ...
            None,
            // No `AccountAdminSource`: these fixtures exercise the LINK policy, and the account
            // capability is an absence on every box that has not declared a barrier.
            None,
            None,
            policy,
        );
...
fn an_authed_control_connection_survives_the_old_idle_timeout() {
    ...
    let mut ctl = authed_stream(addr, CONTROL_KEY, Scope::Write);
...
fn an_idle_observe_stream_carries_a_heartbeat() {
    vike_log::test_init();
    let (_mount, addr) = spawn_node(scaled_policy());
    let mut obs = authed_stream(addr, OBSERVE_KEY, Scope::Read);
...
fn a_subscriber_that_stops_reading_is_closed_on_the_push_write_bound() {
    ...
    let mut stalled = authed_stream(addr, OBSERVE_KEY, Scope::Read);
