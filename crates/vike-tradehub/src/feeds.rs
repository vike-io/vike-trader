...
pub struct LiveTeardown {
    ...
    /// The reconciliation driver, `Some` ONLY on the `VIKE_RECONCILE=1` path with ≥1 live venue
    /// ReconClient (`None` otherwise — the byte-identical default). Shut down in the SEQUENTIAL tail
...
pub fn check_poly_token_intervals(mounts: &[ResolvedMount]) -> Result<(), String> {
    for (j, m) in mounts.iter().enumerate() {
        ...
        if let Some((i, first)) = earlier {
            if first.cfg.interval != m.cfg.interval || first.cfg.interval_ms != m.cfg.interval_ms {
                return Err(format!(
