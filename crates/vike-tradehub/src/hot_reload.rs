...
pub const CLASSIFICATION: &[(SettingsFile, &str, HotClass)] = &[
    ...
    (SettingsFile::Config, "tradehub_advertise_addr", HotClass::Restart),
    ...
    // ONCE, in `crate::tradehub_cli`'s `account_admin_source`, before the listener exists — the
    ...
    //
    // ⚠ And that is the property 0065 §3c leans on. A `SetSetting` frame carries any key in
    // `config.toml` with no server-side typed confirm, so a Control peer can in principle WRITE
    ...
    // write cannot mint.
    (SettingsFile::Config, "tradehub_account_admin", HotClass::Restart),
    ...
    (SettingsFile::Flags, "venue_catalog_off", HotClass::Restart),
...
impl LogLevelApplier {
    ...
    pub fn apply(&self, key: &str) -> bool {
        // ⚠ THROUGH THE SETTINGS SOURCE. This was a store-blind `vike_config::load` until the
        // crossing landed, and on a box that has run `vike-cli config adopt` that reads
        // `preferences.log_level` out of a `preferences.toml` nothing resolves from — so a hot
