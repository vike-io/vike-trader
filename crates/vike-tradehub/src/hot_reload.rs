...
pub const CLASSIFICATION: &[(SettingsFile, &str, HotClass)] = &[
    // -- config.toml: every key is a PATH or an ADDRESS resolved at boot into something that is
    //    then held open or passed on — a store root the recorder opened, a log directory the
    //    appender is writing into, an address a listener is bound to. Re-pointing any of them
    //    mid-flight would change where a HELD resource claims to live without moving the
    //    resource, so all seven are restart-required — `datahub_advertise_addr` included: `serve`
    //    Arc<str>s it ONCE for the listener's lifetime and hands the same value into every
