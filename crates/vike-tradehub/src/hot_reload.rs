...
pub const CLASSIFICATION: &[(SettingsFile, &str, HotClass)] = &[
    ...
    // ⚠ `node_addr` is the one config key THIS DAEMON NEVER READS — it is a CLIENT's dial address,
    // the default for `vike-cli`'s `--node`, written on a laptop by `vike-cli node connect` and
