...
use vike_tradehub_client::proto::{
    ...
    FEATURE_STRATEGY_PARAMS, FEATURE_STRATEGY_VERBS, NODE_PROTO_VERSION, Request, Response, Scope,
    datahub_feature, read_frame_raw, read_frame_raw_capped, write_frame,
};
...
/// spent.** `deploy/vike-tradehub.service` and the installed `deploy/vike-tradehub-project.service`
/// carry `ProtectSystem=strict` with a `ReadWritePaths=` grant naming `<project>/settings/state`
...
fn handle_connection(
    ...
    loop {
        ...
        match request {
            ...
            // LIVE-JOURNAL REPORT read (ruling 16 of the datahub market-data wire design): the
            // CLIENT half shipped, the SERVER half has not, and this arm is the honest statement
            // of that rather than a hole.
            //
            // ⚠ It exists because the match is EXHAUSTIVE — a new `Request` variant is a compile
            // error here until it is classified — and `served_features` deliberately does NOT
            // advertise `FEATURE_TEARSHEET`, so a conforming client refuses this verb
            // client-side and NOTHING reaches this arm
            // (`vike_tradehub_client::remote_handle`'s `tearsheet` is the enforcing side). What
            // does reach it is a client that skipped the negotiation, and the right answer to
            // that is a named refusal, not a fabricated empty tearsheet — the same rule the
            // identity-less `StrategyStatus` and the source-less `SettingsShow` arms above follow.
            //
            // What the follow-up owes: the journal directory this daemon writes to, threaded in
            // the way `SettingsShowSource` already is, and one `vike_report` fold over it. The
            // reply is `Response::Tearsheet(json)` and the capability then joins
            // `served_features` — no client change, no `NODE_PROTO_VERSION` bump.
            Request::Tearsheet { .. } => {
                let _ = write_frame(
                    &mut stream,
                    &Response::Error(
                        "tearsheet unavailable: this vike-tradehub build does not serve the \
                         report verb (it advertises no \"tearsheet\" capability)"
                            .into(),
                    ),
                );
            }
