...
use vike_tradehub_client::proto::{
    FEATURE_ACCOUNT_VERBS, FEATURE_MOUNT_VERBS, FEATURE_OBSERVE_HEARTBEAT, FEATURE_SETTINGS_SHOW,
...
impl SettingsShowSource {
    ...
    fn response(&self) -> Response {
        ...
        //
        // A store that will not OPEN degrades rather than failing the reply, matching the boot's
        // own disposition: during the mirror period the files win, so the rows below are still the
        // effective values — only the `db` origin column is lost.
        let store = match self.settings_dir.as_deref().map(vike_secrets::read_settings_in) {
            Some(Ok(found)) => found,
            Some(Err(e)) => {
                tracing::warn!(error = %e, "the settings database could not be read; describing the files alone");
                vike_secrets::SettingsSource::NoDatabase { path: e.path }
            }
            None => vike_secrets::SettingsSource::NoDatabase { path: std::path::PathBuf::new() },
        };
        match vike_config::describe_with_store(
    ...
    fn apply_set_setting(
        ...
        let write =
            vike_config::set_setting_within(dir, settings_file, key, value, SETTINGS_LOCK_BUDGET)
...
fn run_handshake(
    ...
    if scope == Scope::Admin && !keys.has(Scope::Admin) {
        ...
        let _ = write_frame(
            stream,
            &Response::AuthDenied {
                reason: "account administration is not armed on this node".into(),
            },
        );
        return HandshakeOutcome::Closed;
    }
...
mod venue_refusal_tests {
    ...
    fn every_scoped_verb_is_checked_not_just_submit() {
        let roster = roster();
        for cmd in [
            WireCommand::Flatten { venue: "okx".into(), symbol: "SYM".into() },
            WireCommand::MassCancel { venue: Some("okx".into()), symbol: None },
            WireCommand::MarketExit { venue: Some("okx".into()) },
