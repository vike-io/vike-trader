...
//! [`venue_refusal`] (the ROUTING gate) -> [`audit::sanitize_reason`] -> [`lower_command`] ->
                ...
                if !vike_secrets::ACCOUNT_TIERS.contains(&tier.as_str()) {
                    return Err(format!(
                        "unknown tier '{tier}' — nothing was written. It must be one of: {}. ⚠ \
                         `paper` is not one of them: a paper venue loads no credential and so has \
                         no account row, and `paper` is a policy.toml CEILING rather than a row.",
...
/// `run_handshake` or `Scope::Admin` at all — so the ONE check standing between a peer's CLAIM of
...
fn scope_admission(scope: Scope, keys: &NodeKeys) -> Result<(), &'static str> {
    match scope {
        Scope::Admin if !keys.has(Scope::Admin) => {
            Err("account administration is not armed on this node")
        }
        Scope::Control if !keys.has(Scope::Control) => Err("control disabled on this node"),
        _ => Ok(()),
    }
}
    ...
    loop {
        ...
        match request {
            ...
            Request::Preview(wire_cmd) => {
                ...
                // first, then the address), so the two cannot report different reasons for one
                // frame. Read-only like the rest of this arm — `venue_refusal` is pure, and the
                // roster read is a snapshot-cell load.
                let reason = limits
            ...
            //   2. AUTHORIZATION — `Scope::Admin`, a THIRD key. Checked here rather than at the
...
fn served_features(datahub_advertise: Option<&str>, account_admin: bool) -> Vec<String> {
    let mut features = vec![
        ...
        // The ACCOUNT field on order-carrying commands. UNCONDITIONAL, and for a sharper reason
        // than the entries around it: the field decodes on an old node too — `#[serde(default)]`
        // turns a dropped `account` into `None` — so a client CANNOT tell from a reply whether it
        // was read or silently discarded. This string is the only evidence, which is why a client
        // that names an account refuses locally against a node that omits it.
        // `FEATURE_ACCOUNT_ROUTING`'s own doc carries the argument.
    ...
    // `Scope::Admin`, a THIRD key whose tag is inside the signed preimage — so a Control peer that
...
#[allow(clippy::too_many_arguments)] // see `serve_with_link_policy`'s note — the engine roster is
...
mod account_admission_tests {
    ...
    const EVERY_SCOPE: [Scope; 3] = [Scope::Observe, Scope::Control, Scope::Admin];
    ...
    fn the_absence_refusal_and_the_authorization_refusal_are_different_facts() {
        let unarmed = account_admission(false, Scope::Control).expect_err("unarmed refuses");
        let unauthorized =
            account_admission(true, Scope::Control).expect_err("wrong scope refuses");
...
/// `crates/vike-tradehub/tests/` named `run_handshake` or `Scope::Admin` at all, so the one check
...
mod scope_admission_tests {
    ...
    fn a_node_with_no_admin_key_refuses_the_admin_claim() {
        let reason = scope_admission(Scope::Admin, &keys(true, false))
    ...
    fn a_node_with_no_control_key_refuses_the_control_claim() {
        let reason = scope_admission(Scope::Control, &keys(false, false))
    ...
    fn the_observe_scope_is_admitted_even_by_a_node_armed_for_nothing_else() {
        assert!(scope_admission(Scope::Observe, &keys(false, false)).is_ok());
    }
    ...
    fn the_admin_and_control_refusals_are_different_facts() {
        let admin = scope_admission(Scope::Admin, &keys(true, false)).expect_err("unarmed admin");
        let control =
            scope_admission(Scope::Control, &keys(false, true)).expect_err("unarmed control");
