...
use vike_tradehub_client::proto::{
    datahub_feature, read_frame_raw, read_frame_raw_capped, write_frame, Request, Response, Scope,
    FEATURE_MOUNT_VERBS, FEATURE_SETTINGS_SHOW, FEATURE_SETTINGS_WRITE, FEATURE_STRATEGY_VERBS,
    NODE_PROTO_VERSION,
};
    ...
    loop {
        ...
        match request {
            ...
            Request::Subscribe { topics } => {
                ...
                run_push_writer(&mut stream, &publisher, peer);
                return;
            }
...
fn run_handshake(
    ...
    if mac_ok {
        ...
        tracing::info!(?peer, ?scope, "vike-tradehub node: auth denied (bad mac / no key)");
...
fn run_push_writer(
    ...
    loop {
        match subscription.recv_timeout(WRITER_RECV_TIMEOUT) {
            ...
            Recv::Timeout => {} // no new frame; loop to re-check the mailbox's closed state
            Recv::Closed => {
...
mod served_features_tests {
    ...
    fn a_configured_daemon_advertises_its_datahub_beside_the_named_capabilities() {
        ...
        for named in ["observe", "preview", FEATURE_STRATEGY_VERBS, FEATURE_SETTINGS_SHOW] {
...
mod control_limits_tests {
    ...
    fn a_priceless_qty_raising_modify_is_refused_not_waved_through() {
        ...
        assert!(l
            .preview_vet(&WireCommand::Modify {
                client_order_id: "c1".into(),
                new_qty: Some(1e9),
                new_price: None,
            })
    ...
    fn risk_reducing_verbs_are_never_size_capped() {
        ...
        assert!(l
            .vet(&WireCommand::Flatten { venue: "hyperliquid".into(), symbol: "BTC".into() })
            .is_none());
