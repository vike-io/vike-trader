...
fn a_data_only_oanda_mount_keeps_exec_paper_and_scripted_frames_reach_the_strategy() {
    ...
    let (handle, mut teardown, live_venues, live_locks) = super::live_mount_with(
        ...
        vars,
        lock_dir.path(),
        // No origin: these seams assert the mount path, and an untagged instance is the
        // pre-feature behaviour every one of them was written against.
        None,
        // No WAL: these seams assert the mount path, and an empty map resolves the same
        // `None` journal the process env would on a box that sets neither variable.
        &std::collections::HashMap::new(),
        // The FILES authority: these seams assert the mount path, and an unadopted box is what
        // every one of them was written against. The authority steers only the operator-facing
        // dead-man remedy (`vike_config::remedy`), never a mount decision.
