...
//!
//! This is a SECURITY-SENSITIVE network write path: a remote `Scope::Write` peer can place and
...
//! surfaced by the server's `Response::Error` / `Response::AuthDenied` reply and by the connection
//! log; this records only what the core actually accepted, keyed by peer + verb + client-order-id.
//!
//! ⚠ **That "and by the connection log" used to be a stronger claim than the code supported, and
//! this lane is what exposed it.** The refusals were `tracing::info!` lines on
//! `vike_tradehub::server`, and the same `VIKE_LOG_FILE_LEVEL=warn` that swallowed THIS module's
//! records swallowed them too — so pinning only the accepted half would have left a log file
//! holding a complete record of every command that was taken and no evidence whatever that anyone
//! had ever been turned away, which is WORSE than the old shape because the file now looks
//! authoritative. `crates/vike-tradehub/src/server.rs`'s two auth refusals (the bad-mac arm and
//! the control-disabled arm) are therefore emitted at `warn`, which is a level statement rather
//! than a pin: a refused handshake IS a fault, where an accepted command is not — that asymmetry
//! is the whole reason [`FILE_PIN`] exists for this module and not for that one.
//!
//! ⚠ The residual, declared rather than closed: the PRE-AUTH protocol refusals in the same
//! function (an undecodable opener, a verb before `Hello`, a frame that is not `Auth`) log NOTHING
//! at any level — they answer on the wire and drop the socket. A malformed-frame flood is visible
//! to a peer and to a packet capture, and to nothing on disk.
//!
...
//!   ⚠ **That second bullet is now CLOSED, and the first is not.** [`FILE_PIN`] raises this
//!   module's own target above the global file level, so an accepted command reaches the file
//!   again at the `warn` the shipped units set — `crates/vike-tradehub/tests/daemon/audit_reaches_disk.rs`
//!   is the proof, and it drives a real daemon at that exact level rather than an in-memory
//!   subscriber. The RETENTION half stands untouched: three daily files is still three daily files,
//!   which is why the change journal below remains the durable channel and the pin is not offered
...
/// The file-layer pin that keeps this module's records ON DISK, whatever the global file level is.
///
/// `(target, level)`, handed to `vike_log::LogConfig::file_target_pins` by
/// [`crate::tradehub_cli`]'s `run` — the composition root, which is where a binary's logging
/// decisions belong. The target is this module's own `tracing` target (a `tracing` event's default
/// target is the emitting module path), so the pin and the emit cannot drift apart without moving
/// this file.
///
/// **Why it has to exist**, and it is the module doc's own measurement: [`record`] emits at `info`,
/// every shipped DAEMON unit sets `Environment=VIKE_LOG_FILE_LEVEL=warn` to bound the write rate (a
/// backfill once wrote 341 GB and nearly filled the disk under a live trading node), and the
/// environment beats `LogConfig::file_level` inside `vike_log::file_level_directive`. So on the
/// live the CI box box an accepted control command left NO durable trace at all: a 23 MB log file held
/// 53,160 ERROR lines, 1,928 WARN lines and **zero INFO**. A gated surface with no record of what
/// passed the gate is half a gate.
///
/// ⚠ Three fixes were available and two are wrong. Raising [`record`] to `warn` makes an ORDINARY
/// accepted command look like a fault and pollutes every alerting rule that keys on the level.
/// Lowering the unit to `info` re-opens the firehose the knob exists to close, and leaves the audit
/// trail at the mercy of the next operator who turns it down again. A pin is the only one of the
/// three that raises exactly the record that must survive and nothing else — and the file layer's
/// `off` does not exempt it either (`vike_log::file_level_directive_with_pins` carries that rule
/// and the one way to override it, which is naming this target outright).
pub const FILE_PIN: (&str, &str) = ("vike_tradehub::audit", "info");
...
pub fn record_settings_write(audit: SettingsWriteAudit<'_>) {
    ...
    let change = Change::set_setting(
        ...
        // server's `Request::Command` arm refuses anything but `Scope::Write` before
