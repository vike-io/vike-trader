...
fn project_venue(v: &VenueBlock) -> WireVenueBlock {
    WireVenueBlock {
        ...
        // (`vike_model::account_keys::parse_wire_account`) and precisely the one `policy.toml`
        // refuses. Here the block already says which account it is by carrying `None`, so the
...
impl PublisherHandle {
    ...
    /// one thing: the field it reads. `vike_core::snapshot::VenueBlock::route_key` is
    ...
    /// pose, and the one `crate::server::venue_refusal`'s exactness argument is written against.
    ...
    /// construction — see `vike_model::account_keys::AccountRef::route_key`). Re-pointing the first
    /// at these keys would change what an existing refusal MEANS, so the gates are added beside
    /// each other: `crate::server::account_refusal` reads this one.
    ///
