...
///
/// ⚠ The seven keys the owner DEFERRED (`pm_resolve`, `hl_outcome`, `poly_auto_redeem`,
/// `poly_redeem_halt`, `poly_heartbeat`, `record_chains`, `record_dvol`) are deliberately absent:
/// their consumers exist but are mounted by no running binary, so folding them in would move a
/// value nothing would read and turn an honest admission into a false claim.
...
/// can legitimately grant (`deploy/vike-datahub-record.service` already grants `<project>/market_data`
...
fn start_observe_server(
    ...
    let keys = match vike_tradehub_client::auth::from_vars(&node_store.secrets.into_map()) {
