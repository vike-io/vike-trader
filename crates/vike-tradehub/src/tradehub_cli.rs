...
fn daemon_recon_env() -> HashMap<String, String> {
    reconcile_config::quarantine_first_default(process_env().clone())
}
...
fn live_mount_with(
    ...
    // all built from this one map by `reconcile_config::build_recon_config` below. Those two flags
    // are recorded as unconsumed in `vike_config::CONSUMPTION` rather than half-wired here: a family
    // whose parts disagree about a single reconcile pass would be worse than one that is uniformly
    // environment-driven. `daemon_recon_env` folds in the QUARANTINE-FIRST policy default.
    //
...
/// can legitimately grant (`deploy/vike-datahub-project.service` already grants its own `market_data/`), while the
