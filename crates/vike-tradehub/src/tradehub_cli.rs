...
//! `vike-app` does — same [`crate::reconcile_config`] env parser, same
//! `node.recon_clients`/`node.recon_trigger`, same per-venue `ReconConfig`.
//!
...
//! [`crate::reconcile_config::reconcile_gate`], called once here and once in `vike-app` so the two
//! binaries cannot answer differently, and it takes three inputs: the resolved `flags.reconcile`
//! (force-on), the resolved `flags.reconcile_off` (the refusal, which wins), and the LENGTH of
//! [`vike_run::armed_live_venues`] — this mount's own pre-mount probe of how many venue ACCOUNTS it
...
use crate::config::{DaemonProfile, resolve_paper_risk_limits};
...
pub fn run(env: &HashMap<String, String>, cwd: Option<&Path>, argv: &[String]) -> ExitCode {
    ...
    // ⚠ WHICH PROFILE IS LIVE — and since the owner's ruling on 0057's Question 3, **the ROW
    // WINS**. The read half needs NO unit change and never did: 0057's EROFS section measures that
    // `ProtectSystem=strict` leaves reads untouched, and this daemon already opens this same store
    // read-only at mount for the account table.
    //
    // ⚠ Three states answer `None` here and all three are today's behaviour, byte for byte: no
    // database, a database predating the profile tables, and profile tables holding bodies with no
    // `active` row. `vike_secrets::profile_store::plan_active_row` is the only thing in the tree
    // that may write the fourth, and it refuses unless the row reproduces what is already in force.
    let store_profiles = match booted.settings_dir.as_deref() {
        Some(dir) => {
            match vike_secrets::profile_store::read_profiles(&vike_secrets::db_path_in(dir)) {
                Ok(p) => p,
                // A store that EXISTS and cannot be read is an ERROR, never "no profiles" — the same
                // distinction the credential store draws. It is NOT fatal here: the `--config` rung is
                // still there and is what every box runs on today, so a degraded read costs the ROW and
                // nothing else. Refusing to start would make an unreadable store worse than an absent
                // one on a box that has never used a row.
                Err(e) => {
    ...
    let daemon_selection =
        crate::profile_rows::select_daemon_profile(active_daemon.as_deref(), &args.config_path);
    // The disclosure §7 of the schema design calls for: name the winner AND everything it shadowed.
    // An operator whose `ExecStart --config` has stopped deciding anything has to be told, or the
    // ruling ships the very defect it ruled against — positive confirmation of something false.
    if daemon_selection.row_shadowed_something() {
        tracing::warn!(
            "{}",
            crate::profile_rows::selection_line("daemon profile", &daemon_selection)
        );
    } else {
        tracing::info!(
            "{}",
    ...
    let profile = match active_daemon.as_deref().and_then(|n| store_profiles.by_name(n)) {
        ...
        Some(stored) => match crate::profile_rows::rows_to_daemon_profile(stored) {
            Ok(p) => p,
            Err(e) => {
        ...
        None => match DaemonProfile::load(&args.config_path) {
            Ok(p) => p,
            Err(e) => {
    ...
    // process: the live core's `CoreConfig::journal` and the off-path materializer below both read
    // it, and two resolutions could name two directories. See [`journal_vars`].
    ...
    let (handle, live_extras, live_venues): (
        ...
        match live_mount(
            ...
            &journal_vars,
            // WHICH SOURCE answered for every key above — the ONE resolution this binary already
            // performed, so the live mount's own operator-facing warnings name a remedy this box
            // can follow. `crates/vike-config/src/remedy.rs` carries what happened without it.
            settings.authority,
        ) {
    ...
    if identity.advertise_addr.is_empty() {
        tracing::warn!(
            "this daemon could not determine its own address (no route to name a source address \
             ...
             about which box this is. Set config.toml's `tradehub_advertise_addr` (or \
             VIKE_TRADEHUB_ADVERTISE_ADDR) to name it."
        );
    } else {
        tracing::info!(
    ...
    let observe = start_observe_server(
        ...
        server::SettingsShowSource {
            ...
            hot: Some(hot_handle),
            // ...and WHICH SOURCE this process actually booted on, so a re-resolution taken during
            // an adoption window cannot be reported as though the daemon had applied it. See the
            // field's own doc.
            booted_authority: settings.authority,
        },
...
/// off this one map, exactly as `vike-app`'s `App::new` builds its own.
///
...
/// applies to `vike-app`'s live mount too, and that root was building its `ReconConfig` off a raw
...
/// `the_environment_overrides_the_file_for_every_flag`), so the two sources cannot disagree even in
...
/// Which map wins for ONE folded key when `vars` ALREADY carries it — the credential store's own
/// `.env` being the only thing that can have put it there.
///
/// ⚠ This is a two-row enum because the two answers are not a preference: the question "may the
/// credential store outrank the environment for this key" has a different answer for a Polymarket
/// arming gate (whose reader has documented that store as its second tier since before this
/// wiring, and whose store spelling is REFUSED AT BOOT by
/// [`vike_config::arming::CREDENTIAL_FILE_ARMING_REFUSED`]) than it has for a `VIKE_*` operator
/// toggle (whose readers had never looked at the store at all).
/// `the_credential_store_tier_is_exactly_the_refused_arming_set` is the machine check that keeps
/// the two columns from being decided by whoever adds the next row.
...
enum FoldTier {
    ...
    ///
    /// It is the tier for every key whose reader took no credential-store tier before the wiring:
    ...
    Resolved,
    /// **LEAVE WHAT IS THERE.** The reader already consults the credential store as a documented
    /// second tier, so the fold fills that tier only where nothing filled it — and an arming line
    /// in the store cannot silently outrank the environment here either, because
    /// [`vike_config::refuse_credential_file_arming`] refuses to START on one (`vike-boot` step 3,
    /// long before this fold runs).
    CredentialStoreFirst,
}
...
fn folded_flag_rows(flags: vike_config::Flags) -> [(&'static str, bool, FoldTier); 6] {
    ...
    [
        // Polymarket's two mount gates. Both readers are `env-then-map` hybrids already
        // (`vike_polymarket::mount::poly_exec_enabled`,
        // `vike_polymarket::recon_client::poly_reconcile_enabled`), so this fills the tier they
        // already consult and the process-env tier keeps its precedence inside the reader itself.
        (f::POLY_EXEC_ENV, flags.poly_exec, FoldTier::CredentialStoreFirst),
        (f::POLY_RECONCILE_ENV, flags.poly_reconcile, FoldTier::CredentialStoreFirst),
        ...
        // The PIT `SymbolProperties` recorder's gate, read by `vike_data::PropertiesRecorder`'s
        // `open_from_vars` at the construction site below. ⚠ `Resolved` is what makes that call
        // site correct: `open_from_env` — the constructor it replaced — is a one-line wrapper that
        // calls `open_from_vars` over a snapshot of the PROCESS ENV, so the map handed to the map
        // half has to carry the environment's answer or the swap loses it in both directions.
        ...
        // the live gates — and what makes that safe is the tier, not the ruling: the value the
        // reader ORs in is the RESOLVED flag and nothing else, so an exported `=0` disarms both
        // sides of that OR and no file and no credential line can re-arm it.
...
/// flag through `vike_run::NodeConfig` and `vike_mount::make_engine`: those already take a
...
/// `the_environment_overrides_the_file_for_every_flag` proves it for every flag, registry-wide),
...
///
/// ⚠ **The write is per-key: [`FoldTier::Resolved`] OVERWRITES, [`FoldTier::CredentialStoreFirst`]
/// does not.** An earlier spelling of this function used `or_insert` for all six, which left a
/// `VIKE_ALLOW_WITHDRAW_KEYS=1` line in `<project>/settings/secrets.env` standing and let it be
/// OR-ed in by `vike_mount::arming`'s withdraw gate — a third source, outranking the environment,
/// in no precedence model and invisible to `vike-cli config show`. The two tiers and the reason
/// each key sits where it does are on [`FoldTier`]; the property is
/// `a_process_env_value_beats_a_file_value_for_every_wired_key`.
///
...
fn fold_flags_into_vars(flags: vike_config::Flags, vars: &mut HashMap<String, String>) {
    for (name, resolved, tier) in folded_flag_rows(flags) {
        match tier {
            FoldTier::Resolved => {
                vars.insert(name.to_string(), flag_wire_value(resolved));
            }
            FoldTier::CredentialStoreFirst => {
                vars.entry(name.to_string()).or_insert_with(|| flag_wire_value(resolved));
            }
        }
    }
}
...
/// process environment), and it is resolved ONCE so the live core's `CoreConfig::journal` and the
/// materializer's tail-follow cannot answer differently about where the WAL is.
///
...
fn live_mount(
    ...
    // `main` resolves it ONCE so the core's `journal` and the materializer's tail-follow name the
    // same directory. AFTER `state_dir`, like every other addition — see the note above.
    journal_vars: &HashMap<String, String>,
    // **WHICH SOURCE answered for every settings key on this process** —
    // `vike_config::Settings::authority`, taken from the ONE resolution `main` already performed
    // rather than probed again here. See [`live_mount_with`]'s own parameter, and
    // `crates/vike-config/src/remedy.rs` for the defect class it repairs.
...
/// alternative (keying on `vike_run::armed_live_venues`) as an owner decision not taken here.
/// `vike-app`'s live mount and `crates/vike-run/src/bin/ibkr_mount.rs` do not construct it
/// either; `docs/ops/kill-switches.md` carries those residuals.
...
fn mount_link_disclosure(plan: &VenuePlan) -> MountLinkDisclosure {
    match plan {
        ...
        // The one arm that DOES: `subscribe_book` on every seated token, onto a `vike_run::
...
fn link_deadman_arming_report(
    ...
    // `(None, _, _)` arm told an operator to DELETE A LINE from a file that, on an adopted box,
    // holds no line anybody reads. `vike_config::remedy` renders both halves from the source that
    // answered; `vike_config::DEFAULT_LINK_DEADMAN_GRACE_MS` is the value the store arm has to
    // name outright, because writing the default back is how an adopted box "deletes" a key.
    let grace_remedy =
        authority.write_remedy(vike_config::SettingsFile::Policy, "link_deadman_grace_ms");
    mounted_venues
        .iter()
...
pub fn deadman_absent_warning(
    ...
    // ⚠ **THE REMEDY FOLLOWS THE STORE THAT ANSWERED, and this parameter is the whole of the
    // fix.** MEASURED in the live journal, the CI box, v0.1.33, 2026-09-23T02:41:43Z, on the boot of a
    // daemon that logged `binance: DEMO credentials present → LIVE exec client (real demo orders)`
    // a few lines later: this message told the operator to add `deadman_timeout_ms` to
    // `<project>/settings/policy.toml`, while the SAME boot, 23 lines earlier, said `policy.toml
    // ABSENT` and `the settings DATABASE answers for every key on this box`. Following the
    // instruction creates a file nothing reads, and the next restart prints the identical warning.
    //
    // The parameter is `Authority` and not a re-resolution, deliberately: `vike_config::Settings`
    // already carries which source answered, `vike-tradehub`'s `main` threads it into
    // [`live_mount`], and probing the settings directory a second time here would be its own
    // defect class (`crates/vike-boot/tests/one_owner.rs`). `vike_config::remedy` is where both
    // arms are rendered, and where the class this belongs to is argued.
    let remedy = authority.write_remedy(vike_config::SettingsFile::Policy, "deadman_timeout_ms");
    ...
    let sibling: String = match policy.link_deadman_grace_ms_effective() {
        Some(_) => "\
         ...
         detector."
            .to_string(),
        None => {
            // ⚠ The SIBLING's way back is the same class as this message's own remedy: "deleting
            // that line" is a files-box act, and on an adopted box there is no line. Rendered
            // through the same `vike_config::remedy` so the two halves of one warning can never
            // disagree about which store this box reads.
            let grace =
                authority.write_remedy(vike_config::SettingsFile::Policy, "link_deadman_grace_ms");
            format!(
                "\
...
pub fn warn_deadman_absent(policy: &vike_config::Policy, authority: vike_config::Authority) {
    static ONCE: std::sync::Once = std::sync::Once::new();
    if let Some(message) = deadman_absent_warning(policy, authority) {
...
fn live_mount_with(
    ...
    journal_vars: &HashMap<String, String>,
    // **WHICH SOURCE answered for every settings key on this process** — `vike_config::Settings::
    // authority`, threaded from `main` rather than re-resolved, so the dead-man warning below can
    // name a remedy this box can actually follow. A PARAMETER for the same reason every other
    // impurity here is one, and AFTER `state_dir` for the index reason above.
    // `crates/vike-config/src/remedy.rs` carries the defect class it exists for.
    authority: vike_config::Authority,
    make: &dyn FeedCtors,
) -> Result<LiveMount, String> {
    ...
    // until `vars` has been through the `data_only` withhold below and `vike_run::armed_live_venues`
    ...
    let mount_policy = vike_run::MountPolicy {
        ...
        // row's credential-key owner prefix (`vike_mount`'s `dukascopy` module) — and so does the
        // arming projection, which must describe exactly what the mount will do. Until 2026-09-15
        ...
        ..vike_run::MountPolicy::from(policy)
    };
    ...
    // paired with `quarantine` so it folds nothing. One decision, in `vike-ops`, called identically
    // by `vike-app`'s `App::new`, so the GUI and the daemon cannot differ about what a restart does.
    //
    ...
    // find in a journal without knowing which flag to grep for, and `vike-app` must say it the same
    // way. See `reconcile_config::log_reconcile_gate`.
    ...
    //
    // ⚠ `open_from_vars`, not `open_from_env`, and the difference is ONLY safe because this key is
    // folded under [`FoldTier::Resolved`]. `open_from_env` is a one-line wrapper that calls this
    // same `open_from_vars` over `PropertiesRecorder::env_snapshot` — a map holding just this one
    // variable, read from the PROCESS ENV — so swapping in a different map REPLACES the environment
    // read rather than widening it. `vars` is the credential store's map, which was never a tier
    // for this key, so an `or_insert` fold would have let a stale `VIKE_RECORD_PROPERTIES` line in
    // `secrets.env` beat an exported one in BOTH directions. The overwriting tier makes this map's
    // value exactly `vike_config::Flags`' answer — the file with the environment applied over it —
    // which is what lets `<project>/settings/flags.toml` turn the recorder on while an exported
    // `=0` still turns it off. `a_process_env_value_beats_a_file_value_for_every_wired_key` is the
    // proof; `PropertiesRecorder::gate` is the exact-`"1"` grammar it is read with.
    // ⚠ ALWAYS `None` since the daemon stopped writing the store (0084). `PropertiesRecorder`
    // needs the DataFusion backend the removed `record-feeds`/`materialize` features carried; the
    // recorder that survives runs inside the datahub, which opens its own store.
    ...
    let arming = vike_run::venue_arming(&vars, &mount_policy);
    ...
    warn_deadman_absent(policy, authority);
    ...
    for line in link_deadman_arming_report(policy, authority, &link_venues) {
        tracing::info!("{line}");
    }
    ...
    // Reconcile driver mount — the reference is `vike-app`'s `App::new` recon_driver
    ...
    // dropped the handles above) ⇒ this whole mount is byte-identically absent on a PAPER mount and
    // whenever the operator refused it. `spawn_recon` owns its own `vt-core-recon` thread and
    // respects the single-writer rule (it
    ...
    let recon_driver = match recon_ingredients {
        ...
        Some((clients, _trigger)) if clients.is_empty() => {
            ...
            // INTENT-based (`vike_run::armed_live_venues`' own doc says so), so a venue whose
...
/// byte-identical to this daemon before B5's residual closed. Deliberately NOT `config.state_dir`:
...
fn account_admin_source(
    ...
    let Some(barrier) = AccountBarrier::parse(Some(raw)) else {
        tracing::error!(
            value = raw,
            "config.toml `tradehub_account_admin` is not a value this daemon recognises, so \
        ...
        && let BindExposure::Public(exposed) = server::bind_exposure(resolved)
    {
        tracing::error!(
            %exposed,
            "config.toml `tradehub_account_admin = \"loopback\"` DECLARES that this node server is \
    ...
    // peer that wrote this declaration into config.toml still could not authenticate for the scope.
    //
    ...
    if !keys.as_ref().is_some_and(|k| k.has(Scope::Account)) {
        tracing::error!(
            ...
            "config.toml `tradehub_account_admin` declares a barrier, but there is no {} in this \
    ...
    let Some(settings_dir) = settings_dir else {
        tracing::error!(
            "config.toml `tradehub_account_admin` declares a barrier, but this daemon resolved NO \
        ...
        && let Err(e) = probe_writable(db_dir)
    {
        tracing::error!(
            error = %e,
            dir = %db_dir.display(),
            "config.toml `tradehub_account_admin` declares a barrier and the key is present, but \
             ...
             grant to `settings/`: that hands this daemon write access to the policy.toml that \
             caps it"
        );
        return None;
    }
    ...
    match barrier {
        AccountBarrier::Loopback => tracing::warn!(
            "ACCOUNT ADMINISTRATION ARMED (config.toml `tradehub_account_admin = \"loopback\"`) — \
        ...
        AccountBarrier::Contained => tracing::warn!(
            "ACCOUNT ADMINISTRATION ARMED (config.toml `tradehub_account_admin = \"contained\"`) — \
             an ADMIN-authenticated peer may add, rename, deactivate and REMOVE accounts on this \
...
mod tests {
    ...
    fn contained_arms_regardless_of_the_bind() {
        ...
        assert_eq!(armed.barrier, server::AccountBarrier::Contained);
    ...
    fn a_loopback_declaration_over_a_loopback_bind_arms_the_capability() {
        let armed = account_admin_source(
            Some("loopback"),
            &loopback_bind(),
            &admin_keys(),
            Some(any_dir()),
        ...
        assert_eq!(armed.barrier, server::AccountBarrier::Loopback);
    }
    ...
    fn unset_blank_and_off_are_all_the_default() {
        for decl in [None, Some(""), Some("   "), Some("off")] {
            assert!(
                account_admin_source(decl, &loopback_bind(), &admin_keys(), Some(any_dir()))
                    .is_none(),
    ...
    fn a_declaration_with_no_admin_key_arms_nothing() {
        ...
        assert!(
            account_admin_source(Some("loopback"), &loopback_bind(), &blank, Some(any_dir()))
                .is_none(),
    ...
    fn a_declaration_with_no_settings_directory_arms_nothing() {
        assert!(
            account_admin_source(Some("contained"), &loopback_bind(), &admin_keys(), None)
                .is_none(),
    ...
    fn arming_accounts_does_not_restore_a_control_key_the_control_gate_zeroed() {
        ...
        let zeroed = vike_tradehub_client::auth::NodeKeys::new(
            loaded.key_for(Scope::Read).to_vec(),
            Vec::new(),
        );
        ...
        assert!(
            armed.has(Scope::Account),
            "…while still granting the admin scope it was armed for"
        );
    }
    ...
    // `crates/vike-core/src/runtime/deadman_tests.rs`'s job (`trips_after_timeout_and_cancels_all_
    ...
    ///
    /// ⚠ **Every assertion here is about the `Authority::Files` rendering and is BYTE-IDENTICAL to
    /// what shipped before the remedy followed the store.** That is the point: the files box's text
    /// was already correct, and a class fix is not a licence to churn it. The store arm's
    /// assertions are the test directly below, and the pair is what makes either one meaningful —
    /// an assertion satisfied under BOTH authorities proves nothing, which is how the defect
    /// shipped.
    #[test]
    fn the_absent_key_warning_fires_for_none_alone_and_names_what_an_operator_needs() {
        let absent = vike_config::Policy::default();
        let msg = deadman_absent_warning(&absent, vike_config::Authority::Files)
            .expect("an absent key WARNS");
        ...
        assert!(msg.contains("deadman_timeout_ms = 60000\n"), "the paste-ready line: {msg}");
        assert!(msg.contains("deadman_timeout_ms = 0\n"), "…and the silencing one: {msg}");
        ...
        assert_eq!(
            deadman_absent_warning(&decided, vike_config::Authority::Files),
            None,
            "an explicit 0 is a decision"
        );
        ...
        assert_eq!(
            deadman_absent_warning(&armed, vike_config::Authority::Files),
            None,
            "an armed switch has nothing to warn"
        );
    }

    /// **THE DEFECT, as a test.** MEASURED in the live journal on the CI box, v0.1.33,
    /// 2026-09-23T02:41:43Z: this warning fired on a daemon whose next lines mounted a LIVE exec
    /// client, and it told the operator to add `deadman_timeout_ms` to
    /// `<project>/settings/policy.toml` — on a box whose own boot disclosure, 23 lines earlier,
    /// said `policy.toml ABSENT` and `the settings DATABASE answers for every key on this box`.
    /// Following the instruction creates a file nothing reads.
    ///
    /// So on an ADOPTED box the remedy must name the verb that reaches the ROWS, and must name no
    /// file as somewhere to write. The SUBSTANCE is unchanged and is asserted here too — this is
    /// still the only place that says the switch is off by omission rather than by decision.
    #[test]
    fn the_absent_key_warnings_remedy_follows_the_store_that_answered() {
        let absent = vike_config::Policy::default();
        let msg = deadman_absent_warning(&absent, vike_config::Authority::Store)
            .expect("an absent key WARNS whichever store answers");

        // The remedy, both halves, in the vocabulary of the store that answered.
        assert!(
            msg.contains("vike-cli config set policy.deadman_timeout_ms 60000 --confirm policy.deadman_timeout_ms"),
            "the arming line must be a command this box can run: {msg}"
        );
        assert!(
            msg.contains("vike-cli config set policy.deadman_timeout_ms 0 --confirm policy.deadman_timeout_ms"),
            "…and so must the one that records a decision AGAINST it: {msg}"
        );
        // ⚠ The bare TOML assignment is exactly what an adopted box must NOT be handed: it is the
        // instruction that creates an inert file. `\n` anchored so `config set policy.…_ms 0`
        // above cannot satisfy it.
        assert!(
            !msg.contains("deadman_timeout_ms = 60000"),
            "an adopted box may not be told to write a TOML line: {msg}"
        );
        assert!(
            !msg.contains("add this to <project>/settings/policy.toml"),
            "…nor to add anything to a file it does not read: {msg}"
        );
        // The file may still be NAMED — as the thing that is not read, which is the one fact an
        // operator staring at an absent `policy.toml` most needs.
        assert!(
            msg.contains("NOT read for resolution"),
            "the file is named as INERT rather than as a write target: {msg}"
        );
        assert!(
            msg.contains("the settings DATABASE does not carry `policy.deadman_timeout_ms`"),
            "the headline follows the store too — it used to credit the file: {msg}"
        );

        // …and the SUBSTANCE, unchanged. This half is deliberately identical to the files test
        // above: only the remedy moved.
        assert!(msg.contains("off by OMISSION, not by decision"), "the force is kept: {msg}");
        assert!(msg.contains("cancel every resting"), "says what it would do: {msg}");
        assert!(msg.contains("engage HALT"), "…and that the default action halts: {msg}");
        assert!(msg.contains("observes SILENCE, not the connection"), "states the cost: {msg}");
        assert!(
    ...
    /// ⚠ **The property no single-authority assertion can carry**: the two renderings must DIFFER,
    /// and each must be the one that box can follow. An assertion that passes under both
    /// authorities is the shape that let this defect live in a shipped release.
    #[test]
    fn the_two_authorities_render_different_deadman_remedies() {
        let absent = vike_config::Policy::default();
        let files = deadman_absent_warning(&absent, vike_config::Authority::Files).unwrap();
        let store = deadman_absent_warning(&absent, vike_config::Authority::Store).unwrap();
        assert_ne!(files, store, "the remedy must follow the store that answered");
        assert!(files.contains("add this to <project>/settings/policy.toml"), "{files}");
        assert!(!files.contains("vike-cli config set"), "{files}");
        assert!(store.contains("vike-cli config set"), "{store}");
        assert!(!store.contains("add this to <project>/settings/policy.toml"), "{store}");
    }

    /// The SIBLING half of the same warning, and the same class: `link_deadman_grace_ms = 0`'s way
    /// back is "delete the line" on a files box and a `config set` of the default on an adopted
    /// one, because an adopted box has no line to delete.
    #[test]
    fn the_link_grace_way_back_follows_the_store_too() {
        let both_off = vike_config::Policy {
            link_deadman_grace_ms: Some(vike_config::LINK_DEADMAN_DISABLED_MS),
            ..vike_config::Policy::default()
        };
        let files = deadman_absent_warning(&both_off, vike_config::Authority::Files).unwrap();
        let store = deadman_absent_warning(&both_off, vike_config::Authority::Store).unwrap();
        assert!(files.contains("Delete the line from <project>/settings/policy.toml"), "{files}");
        assert!(
            store.contains("Run `vike-cli config set policy.link_deadman_grace_ms 120000 --confirm policy.link_deadman_grace_ms`"),
            "{store}"
        );
        assert!(!store.contains("Delete the line"), "there is no line on an adopted box: {store}");
        // Both still say the mount has NO automatic stop of any kind — the substance is untouched.
        for msg in [&files, &store] {
    ...
    fn an_armed_venue_whose_lanes_disclose_nothing_here_is_not_armed() {
        let policy = vike_config::Policy::default();
        assert!(
            link_deadman_config_from_policy(&policy, &[unseen("binance"), unseen("bybit")])
                .is_none(),
    ...
    fn the_cex_arm_subscribes_only_the_bar_verb_on_the_status_bearing_handle() {
        ...
        let arm_start = FEEDS.find("VenuePlan::Cex { venue, mainnet } => {").expect(
            "the CEX arm's match head has moved — this scan is anchored on it and would otherwise \
             check nothing",
        );
    ...
    /// REAL table** — the join every other test here takes one leg of. A default `policy.toml`, a
    ...
    fn a_default_cex_mount_arms_the_link_deadman_and_says_so() {
        ...
        let lines = link_deadman_arming_report(&policy, vike_config::Authority::Files, &mounted);
        ...
        let with_deribit = [
            mounted[0].clone(),
            ("deribit".to_string(), mount_link_disclosure(&VenuePlan::Deribit)),
        ];
        let lines =
            link_deadman_arming_report(&policy, vike_config::Authority::Files, &with_deribit);
    ...
    fn the_arming_report_tells_the_four_off_reasons_apart() {
        ...
        let lines = link_deadman_arming_report(
            &vike_config::Policy::default(),
            vike_config::Authority::Files,
            &venues,
        );
        assert_eq!(lines.len(), 4, "one line per mounted venue");
        ...
        assert!(
            ...
            "the RESIDUAL reason — nothing in policy.toml can fix this one: {}",
            lines[2]
        );
        ...
        for line in link_deadman_arming_report(&off, vike_config::Authority::Files, &venues) {
            assert!(line.contains("`link_deadman_grace_ms = 0`"), "{line}");
            assert!(line.contains("Delete the line from <project>/settings/policy.toml"), "{line}");
        }
        // ⚠ …and on an ADOPTED box there is no line to delete. Same class as the silence
        // dead-man's own remedy; `crates/vike-config/src/remedy.rs` carries the measurement.
        for line in link_deadman_arming_report(&off, vike_config::Authority::Store, &venues) {
            assert!(
                line.contains("`link_deadman_grace_ms = 0` in the settings database"),
                "{line}"
            );
            assert!(
                line.contains("Run `vike-cli config set policy.link_deadman_grace_ms 120000 --confirm policy.link_deadman_grace_ms`"),
                "{line}"
            );
    ...
    fn the_run_profile_cannot_touch_the_link_deadman() {
        let profile =
            vike_core::RunProfile::from_toml_str(vike_core::run_profile::samples::LIVE_TOML)
    ...
    /// `policy.toml`'s protections — could disarm the switch. Driven through the shipped LIVE
    ...
    /// The reconcile FORCE-ON flag reads the FLAGS FILE, and the environment still wins over it.
    ///
    /// ⚠ **What this flag MEANS changed with S2, and the test name did not, deliberately** — it is
    /// still "the reconcile gate reads the file". `flags.reconcile` is no longer the default answer:
    /// a mount that arms a live venue account reconciles without it (see the composed test below).
    /// What it still does is force the driver on where the armed-live probe reports nothing, and
    /// that is what a `flags.toml` line has to keep being able to do.
    ///
    ...
    /// hand-built value would assert nothing about it. The env keys come from `vike_config`'s own
    /// constants so a rename cannot leave this test passing against a name nothing reads.
    #[test]
    fn the_reconcile_gate_reads_the_flags_file_and_the_env_still_wins() {
        ...
        assert!(from_file.flags.reconcile, "flags.toml must arm the force-on gate");
    ...
    /// The REFUSAL is reachable from both layers an operator has — a deployed `flags.toml` line and
    /// a one-run exported variable — because a default-on behaviour that can only be refused from a
    /// file cannot be refused at 3am, and one that can only be refused from the environment cannot
    /// be refused durably on a box whose unit somebody else owns.
    #[test]
    fn the_reconcile_refusal_is_reachable_from_the_file_and_from_the_environment() {
        let dir = tempfile::tempdir().expect("temp settings dir");
        std::fs::write(dir.path().join("flags.toml"), "reconcile_off = true\n").unwrap();
        assert!(vike_config::load(Some(dir.path()), &HashMap::new()).unwrap().flags.reconcile_off);

        let env =
            HashMap::from([(vike_config::flags::RECONCILE_OFF_ENV.to_string(), "1".to_string())]);
        ...
        assert!(!vike_config::load(Some(dir.path()), &back_on).unwrap().flags.reconcile_off);
    }

    /// **THE S2 property at this daemon's own edge**, composed over the REAL probe this mount uses
    /// (`vike_run::armed_live_venues`) rather than a hand-picked count: a mount that arms a venue
    ...
    fn a_live_armed_mount_reconciles_with_nothing_set_and_a_paper_one_does_not() {
        ...
        let paper = vike_run::MountPolicy::default();
        let none_armed = vike_run::armed_live_venues(&creds, &paper);
        ...
        let armed_policy = vike_run::MountPolicy {
            venues: vike_config::VenuePolicy::default()
                .declare(venue, vike_config::VenueMode::Demo),
            ..Default::default()
        };
        let armed = vike_run::armed_live_venues(&creds, &armed_policy);
    ...
    /// The `flags.toml` KEY for a folded variable, from the registry rather than from a second
    ...
    /// A settings tree holding one `flags.toml` line.
    fn settings_dir_with_flag(key: &str, value: bool) -> tempfile::TempDir {
        let dir = tempfile::tempdir().expect("a temp settings dir");
        std::fs::write(dir.path().join("flags.toml"), format!("{key} = {value}\n"))
            .expect("write flags.toml");
        dir
    }
    ...
    ///
    /// For each key: `<project>/settings/flags.toml` says `true`, the process environment says
    /// `"0"`, and `<project>/settings/secrets.env` — the third source nobody modelled — says `"1"`.
    /// The environment must win.
    ///
    /// It drives the REAL `vike_config::load`, so the file-then-env half is the shipped resolution
    /// and not a re-implementation, and then the REAL fold. What it asserts is per-TIER, because
    /// the two tiers answer the hostile credential line differently and both answers are correct:
    ///
    /// * `FoldTier::Resolved` — the value is OVERWRITTEN, so the map carries `"0"` and the reader
    ///   downstream sees the environment's answer. This is the tier for every key whose reader had
    ///   no credential-store tier before the wiring, which is both SAFETY OVERRIDES among them.
    /// * `FoldTier::CredentialStoreFirst` — the store line stands, because that store IS this
    ///   reader's documented second tier and was before this wiring existed. What makes it safe is
    ///   not the fold but `vike_config::refuse_credential_file_arming`, which REFUSES TO START on
    ///   exactly that line — asserted here rather than assumed, so the two halves of the argument
    ///   cannot drift apart.
    #[test]
    fn a_process_env_value_beats_a_file_value_for_every_wired_key() {
        for (var, _, tier) in folded_flag_rows(vike_config::Flags::default()) {
            let key = flags_key_for(var);
            let dir = settings_dir_with_flag(key, true);
            let env = HashMap::from([(var.to_string(), "0".to_string())]);
            let settings = vike_config::load(Some(dir.path()), &env)
                .unwrap_or_else(|e| panic!("{var}: the settings must load: {e}"));

            // Layer one: the environment beat the file inside `vike_config::load`.
            assert!(
                !resolved_row(settings.flags, var),
                "{var}=0 must beat `{key} = true` in flags.toml"
            );
            ...
            match tier {
                FoldTier::Resolved => assert_eq!(
                    vars.get(var).map(String::as_str),
                    Some("0"),
                    "{var}: an exported `0` must survive the fold — a `1` here is a credential \
                     file outranking the environment on a key whose reader never had that tier"
                ),
                FoldTier::CredentialStoreFirst => {
                    assert_eq!(
                        vars.get(var).map(String::as_str),
                        Some("1"),
                        "{var}: the documented credential-store tier is left exactly as it was"
                    );
                    let store = HashMap::from([(var.to_string(), "1".to_string())]);
                    let refusal = vike_config::refuse_credential_file_arming(&store)
                        .expect_err("...and THAT line refuses startup, which is why it may stand");
                    assert!(refusal.contains(var), "the refusal names the variable: {refusal}");
                }
            }
        }
    }
    ...
    /// over-correcting. Nothing exported, `flags.toml` says `true`, and every folded key reaches
    /// the map as `"1"`.
    #[test]
    fn a_file_value_reaches_the_map_when_the_environment_is_silent() {
        for (var, _, _) in folded_flag_rows(vike_config::Flags::default()) {
            let key = flags_key_for(var);
            let dir = settings_dir_with_flag(key, true);
            let settings = vike_config::load(Some(dir.path()), &HashMap::new())
                .unwrap_or_else(|e| panic!("{var}: the settings must load: {e}"));
            ...
            assert_eq!(
                vars.get(var).map(String::as_str),
                Some("1"),
                "{var}: `{key} = true` must reach the map — that IS the wiring"
            );
        }
    }

    /// The two SAFETY OVERRIDES, called out by name and carried into the READER rather than left
    /// at the map: exported `=0` plus `flags.toml` `true` must resolve to REFUSE.
    ///
    /// The withdraw half calls the real reader
    /// (`vike_bridge_core::key_permissions::allow_withdraw_keys`, this crate's own dependency).
    /// The preflight half asserts the exact string, because `vike_mount::startup` lives in a crate
    /// this one does not depend on directly — that side of the chain is proved in
    /// `crates/vike-mount/src/startup.rs`'s
    ...
    fn a_safety_override_refuses_when_the_environment_says_zero_and_the_file_says_true() {
        for var in
            [vike_config::flags::ALLOW_WITHDRAW_KEYS_ENV, vike_config::flags::PREFLIGHT_SKIP_ENV]
        {
            let key = flags_key_for(var);
            let dir = settings_dir_with_flag(key, true);
            let env = HashMap::from([(var.to_string(), "0".to_string())]);
            let settings = vike_config::load(Some(dir.path()), &env).expect("settings load");
            ...
            assert_eq!(
                vars.get(var).map(String::as_str),
                Some("0"),
                "{var} must be disarmed in the map the readers OR in"
            );
            assert!(
                !vike_bridge_core::key_permissions::allow_withdraw_keys(&vars),
                "{var}: the withdraw override must read false from the folded map"
            );
        }
    }

    /// **The tier column is not a judgement call.** Exactly the keys whose credential-store line is
    /// REFUSED AT STARTUP (`vike_config::arming::CREDENTIAL_FILE_ARMING_REFUSED`) may keep that
    /// store as a tier; every other folded key must be overwritten.
    ///
    /// Without this, the next folded key's tier is decided by whoever adds the row, and the wrong
    /// choice is silent: an `or_insert` on a `VIKE_*` toggle reintroduces a third source that
    /// outranks the environment and appears in no precedence model.
    #[test]
    fn the_credential_store_tier_is_exactly_the_refused_arming_set() {
        for (var, _, tier) in folded_flag_rows(vike_config::Flags::default()) {
            let refused =
                vike_config::arming::CREDENTIAL_FILE_ARMING_REFUSED.iter().any(|s| s.var == var);
            let expected =
                if refused { FoldTier::CredentialStoreFirst } else { FoldTier::Resolved };
            assert_eq!(
                tier, expected,
                "{var}: a key whose credential-file line is NOT refused at startup must be \
                 OVERWRITTEN by the fold, or that line becomes a third source outranking the \
                 environment; a key whose line IS refused may keep the store tier its reader \
                 documents"
            );
        }
    }
    ...
    /// `withhold_exec_credentials`, which strips `vars` by `{VENUE}_` prefix and reports the count
    ...
    fn the_process_env_beats_the_resolved_flag_in_the_reconcile_family() {
        ...
        for var in [
            ...
            assert_eq!(
                from_file.get(var).map(String::as_str),
                Some("1"),
                "{var}: the wiring works"
            );
        }
    }

    /// `config.journal_dir`, the one wired key that is not a flag: an exported `VIKE_JOURNAL_DIR`
    /// beats `config.toml`, and the file's value lands only where the variable is absent.
    #[test]
    fn the_process_env_beats_config_journal_dir() {
        ...
        assert_eq!(
            ...
            "an `Environment=VIKE_JOURNAL_DIR=...` line beats config.toml"
        );
