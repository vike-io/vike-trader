...
//! [`vike_ops::reconcile_config::reconcile_gate`], called once here and once in `vike-app` so the two
...
//! [`vike_ops::reconcile_config::quarantine_first_default`], and the pairing is load-bearing rather
...
pub fn run(env: &HashMap<String, String>, cwd: Option<&Path>, argv: &[String]) -> ExitCode {
    ...
    // ⚠ A DECLARED PARTIAL, disclosed rather than implied. The owner's ruling (the row wins) is
    // implemented for the DAEMON profile above, which is 0057's Phase 3. The RUN profile's BODY is
    // Phase 2's subject (`run-live.toml`'s `[risk]` as rows) and is not stored yet, so this binary
    // still resolves it from `--profile`/`VIKE_RUN_PROFILE` alone. A box that somehow carries an
    // active `run` row must be told that it is not deciding anything here, because a row that reads
    // as authoritative and does nothing is the failure this whole program exists to remove.
    {
        ...
        if active_run.is_some() {
            tracing::warn!(
                "the settings store holds an ACTIVE `run` profile row, and this binary does not \
                 read run-profile bodies from rows yet (0057 Phase 2). The [risk] ceilings still \
                 come from --profile / VIKE_RUN_PROFILE"
            );
        }
        let sel = crate::profile_rows::select_run_profile(
            None,
            args.profile_path.as_deref(),
            &run_profile_vars,
        );
        tracing::info!("{}", crate::profile_rows::selection_line("run profile", &sel));
    }
    ...
    let (handle, live_extras, live_venues): (
        ...
        let opts = vike_run::PaperMountOpts {
            ...
            journal: vike_core::journal_config_from(&journal_vars),
            ..Default::default()
        };
        let handle = if multi {
    ...
    let _materializer = maybe_spawn_materializer(&journal_vars);
...
/// [`vike_ops::reconcile_config`]'s module doc) with the QUARANTINE-FIRST default folded
/// in. Every `VIKE_RECONCILE_*` knob ([`vike_ops::reconcile_config::build_recon_config`]) is read
...
/// [`vike_ops::reconcile_config::quarantine_first_default`] — not tidiness: the S2 default-on gate
    ...
    mounted_venues
        ...
        .map(|(venue, disclosure)| {
            ...
            match (grace, row.off_reason(), disclosure) {
                (None, _, _) => format!(
                    "LINK DEAD-MAN off for {venue}: `link_deadman_grace_ms = 0` in \
                     <project>/settings/policy.toml turns it off for EVERY venue. Delete the line \
                     to restore the default grace"
                ),
...
pub fn deadman_absent_warning(policy: &vike_config::Policy) -> Option<String> {
    ...
    let sibling = match policy.link_deadman_grace_ms_effective() {
        Some(_) => {
            "\
        ...
        None => {
         ...
         any kind — not for a dead socket and not for a quiet feed. Deleting that line restores \
         the connection-state switch at its default grace; this key stays a separate decision."
        }
    };
    Some(format!(
        "THE SILENCE DEAD-MAN IS OFF: <project>/settings/policy.toml does not name \
         `deadman_timeout_ms`, so this live mount has no automatic stop for \"the feed went QUIET \
         and orders are still resting\". It is off by OMISSION, not by decision — nothing arms it \
         unless you write the key, and this line is the only place that says so.\n\n\
         {sibling}\n\n\
         ...
         To arm it for a venue that never closes, add this to <project>/settings/policy.toml \
...
fn live_mount_with(
    ...
    let properties_rec = vike_data::PropertiesRecorder::open_from_vars(&tick_store_root(), &vars);
    ...
    let mut node_cfg = NodeConfig {
        ...
        core_config: vike_core::CoreConfig {
            ...
            // ⚠ `journal_config_from`, over the map this binary resolved — that is what lets
            // `config.journal_dir` in `<project>/settings/config.toml` enable the WAL, which no
            // file could do while this was a `vike-core` library env read. `VIKE_JOURNAL_DIR` still
            // wins over the file, inside [`journal_vars`].
            journal: vike_core::journal_config_from(journal_vars),
...
/// here: `crates/vike-desktop/src/main.rs`'s `tick_store_root` is the same resolution for the desktop
...
/// can legitimately grant (`deploy/vike-datahub.service` already grants `<project>/market_data`
/// beside its log path), while the
...
fn maybe_spawn_materializer(
    journal_vars: &HashMap<String, String>,
) -> Option<vike_ops::journal_mat::MaterializerHandle> {
    let dir = vike_core::journal_config_from(journal_vars).map(|jc| jc.dir)?;
    let root = tick_store_root();
...
fn maybe_spawn_materializer(
    _journal_vars: &HashMap<String, String>,
) -> Option<vike_ops::journal_mat::MaterializerHandle> {
    None
}
...
fn start_observe_server(
    ...
    // leaves `Scope::Admin` empty, which is what makes that the default rather than a check.
    //
    ...
    // `scope == Scope::Control && !keys.has(Scope::Control)` refusal then admitted a Control peer to
    ...
    // cannot see it: it asserts only that `Scope::Admin` is absent, never that Control stays absent.
...
/// `scope == Scope::Control && !keys.has(Scope::Control)` refusal stopped refusing and a Control
...
fn account_admin_source(
    ...
    if !keys.as_ref().is_some_and(|k| k.has(Scope::Admin)) {
        tracing::error!(
...
mod tests {
    ...
    fn the_ordinary_node_key_read_leaves_the_admin_scope_absent() {
        ...
        assert!(
            !keys.has(Scope::Admin),
            "the two-name read must leave Scope::Admin ABSENT even when the store holds an admin \
        ...
        assert!(widened.has(Scope::Admin), "…and the explicit door is what grants it");
    }

    /// **Arming the account capability may not restore a control key the control gate ZEROED.**
    ///
    /// The test above asserts only about `Scope::Admin`, so it stayed green while the two steps
    ...
    /// `run_handshake`'s `!keys.has(Scope::Control)` gate had just refused it with — and
    ...
    fn arming_accounts_does_not_restore_a_control_key_the_control_gate_zeroed() {
        ...
        assert!(armed.has(Scope::Admin), "…while still granting the admin scope it was armed for");
    }
    ...
    fn the_absent_key_warning_fires_for_none_alone_and_names_what_an_operator_needs() {
        ...
        let msg = deadman_absent_warning(&absent).expect("an absent key WARNS");
    ...
    ///
    /// ⚠ The fold itself now lives in `vike_ops::reconcile_config::quarantine_first_default` (the
    ...
    fn daemon_reconcile_policy_defaults_to_quarantine() {
        ...
        assert!(
            vike_ops::reconcile_config::auto_applied_kinds(&cfg.policy).is_empty(),
