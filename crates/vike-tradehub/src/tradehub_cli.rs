...
/// it. The successor is named so the next reader does not re-derive it: a CONNECTION-state
/// dead-man (M13) is the coming default, and it is not this key.
pub fn deadman_absent_warning(policy: &vike_config::Policy) -> Option<String> {
    if policy.deadman_timeout_ms.is_some() {
        return None;
    }
    Some(format!(
        "DEAD-MAN SWITCH IS OFF: <project>/settings/policy.toml does not name \
         `deadman_timeout_ms`, so this live mount has NO automatic stop for \"the feed went silent \
         ...
         What it would do: after that many milliseconds with no venue event, tick, bar, quote, \
         trade or book update on ANY venue or symbol this daemon follows, cancel every resting \
         order and (with the default `deadman_action`) engage HALT — `Halted` on every engine plus \
         the HALT sentinel file, which an operator must delete before the next submit.\n\n\
         ...
         no outage at all. It is a tool for a 24/7 mount; a connection-state dead-man is the \
         coming default and is not this key.\n\n\
...
mod tests {
    ...
    fn the_absent_key_warning_fires_for_none_alone_and_names_what_an_operator_needs() {
        ...
        assert!(msg.contains("connection-state dead-man"), "names the successor: {msg}");
