    ...
    let remedy = if exec_live {
        ...
        Some(format!(
            "{store} — ⚠ aster has NO {upper}_MAINNET flag: the credential TIER is the network \
             choice and `make_engine` resolves LIVE-FIRST, so adding {upper}_LIVE_USER + \
             {upper}_LIVE_PRIVATE_KEY instead arms REAL-MONEY MAINNET exec",
            store = add_to_the_store(&keys)
        ))
    } else if mainnet {
        Some(format!(
            "{store} — ⚠ {upper}_MAINNET=1 is SET, so `make_engine` loads the LIVE (mainnet) key \
             set ONLY and a {upper}_DEMO_* key set arms NOTHING; to mount this venue on DEMO \
             instead, UNSET {upper}_MAINNET",
            store = add_to_the_store(&keys)
        ))
    } else {
