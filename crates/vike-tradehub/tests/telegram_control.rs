...
use vike_tradehub::telegram::{
    CONFIRM_WINDOW_MS, LedgerPaths, PendingConfirms, PollError, ReadVerb, TelegramConfig,
    TelegramDeps, TgUpdate, UpdateLedger, control_gates_open, maybe_spawn, poll_once, spawn,
};
...
fn accept_command_shared_by_both_surfaces() {
    ...
    let deps = StubDeps::new()
        ...
        .with_accept(Box::new(move |cmd, reason| {
            ...
            // `engines: &[]` — this stub surface publishes no engine roster, which
            // `venue_refusal` reads as UNKNOWN and refuses nothing. The routing gate has its own
            // suite (`tests/venue_routing.rs`); this one is about the SHARED limiter.
            accept_command(
                cmd,
                Some(reason),
