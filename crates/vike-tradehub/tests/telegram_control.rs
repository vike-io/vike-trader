...
use vike_tradehub::server::{ControlLimits, ControlLimitsConfig, accept_command};
...
fn accept_command_shared_by_both_surfaces() {
    ...
    let deps = StubDeps::new()
        ...
        .with_accept(Box::new(move |cmd, reason| {
            ...
            // `crates/vike-tradehub/src/server.rs`'s `account_refusal_tests` for the pure
...
// account this node does not run was Acked and misrouted. `server::accept_command` being the ONE
...
// one.** `ProdTelegramDeps::preview` calls `server::account_refusal` DIRECTLY and never enters
...
fn one_account_cell() -> std::sync::Arc<arc_swap::ArcSwap<vike_core::CoreSnapshot>> {
    let mut snap = vike_core::CoreSnapshot::empty("binance", "BTCUSDT");
