...
fn the_ledger_names_the_authenticating_key_and_never_carries_it() {
    ...
    let id = keys.key_id(Scope::Write).expect("a configured control key has an id");
...
fn the_two_scopes_keys_do_not_share_an_id() {
    ...
    assert!(control_only.key_id(Scope::Read).is_none());
    assert_eq!(control_only.key_id(Scope::Write), keys.key_id(Scope::Write));
}
