...
///
/// `apply_set_setting` runs on a CONNECTION THREAD with a peer on the far end, and it used to
/// inherit a ~3 s spin argued entirely for a CLI that is the whole process. The budget is now this
/// daemon's own ([`server::SETTINGS_LOCK_BUDGET`]), and two properties are asserted as SHAPE — the
/// arm taken and the number reported — rather than as elapsed time:
///
...
/// 2. with the directory genuinely HELD, the peer gets a REFUSAL naming the sentinel and reporting
///    exactly that budget — which is only possible if the production arm passed it — and no byte of
///    the file moves.
///
/// The lock is taken here the way another PROCESS would take it: by path, with the bare `std`
/// primitive, because `flock`/`LockFileEx` key on the open file description rather than on the
/// process.
#[test]
fn a_held_settings_directory_refuses_the_peer_with_this_daemons_own_budget() {
    assert!(
    ...
    .expect_err("a held settings directory must refuse the peer");
    let msg = err.to_string();
    assert!(msg.contains(vike_config::SETTINGS_LOCK_FILE), "it names the held sentinel: {msg}");
    assert!(
        msg.contains(&format!("waited {} ms", server::SETTINGS_LOCK_BUDGET.max_wait_ms())),
        "the refusal must report the budget THIS daemon chose ({} ms): {msg}",
        server::SETTINGS_LOCK_BUDGET.max_wait_ms()
    );
    ...
    assert_eq!(
        ...
        "a busy refusal changes NO byte"
    );
    drop(held);
