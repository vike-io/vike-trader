...
impl Daemon {
    ...
    fn start() -> Self {
        Self::start_with_control(true)
    }

    /// [`Daemon::start`] with the control gate as a PARAMETER.
    ///
    /// `false` exists for one caller — the test that proves [`authenticate_control`]'s detector
    /// actually fires. A detector nobody has seen refuse is a detector nobody has tested, and the
    /// collision it exists for is too rare to wait for.
    fn start_with_control(control: bool) -> Self {
        ...
        let mut child = Command::new(env!("CARGO_BIN_EXE_vike-tradehub"))
            ...
            .envs(control.then_some(("VIKE_TRADEHUB_CONTROL", "1")))
...
/// The answer a daemon gives when its own `flags.tradehub_control` is off.
///
/// ⚠ **This test's daemon can never give it**, and that is what makes the string usable as a
/// DETECTOR rather than a guess: [`Daemon::start`] sets `VIKE_TRADEHUB_CONTROL=1` on the child
/// unconditionally. An `AuthDenied` carrying this reason therefore proves the handshake reached a
/// daemon this test did not configure — see [`authenticate_control`].
const FOREIGN_DAEMON_REASON: &str = "control disabled on this node";

/// Complete the `Control` handshake, or say WHY not.
///
/// ⚠ **`Err` means one specific thing: the peer is not our daemon.** Returns `Err` only for an
/// `AuthDenied` whose reason is [`FOREIGN_DAEMON_REASON`]; every other outcome still panics, so a
/// genuine authentication regression fails here as loudly as it always did rather than being
/// retried until the attempts run out.
fn authenticate_control(stream: &mut TcpStream) -> Result<(), String> {
    ...
    let mac = auth::sign(CONTROL_KEY.as_bytes(), &nonce, NODE_PROTO_VERSION, Scope::Write);
    write_frame(stream, &Request::Auth { scope: Scope::Write, mac }).expect("auth");
    match read_frame::<_, Response>(stream).expect("authok") {
        Response::AuthOk { scope: Scope::Write } => Ok(()),
        Response::AuthDenied { reason } if reason == FOREIGN_DAEMON_REASON => Err(reason),
        other => panic!("expected AuthOk(Control), got {other:?}"),
    }
}

/// How many times [`daemon_with_control`] may re-spawn on a fresh port.
///
/// Each attempt claims a NEW ephemeral port, so a repeat needs the same collision to happen again
/// against a different number. Three is far past the point where a real refusal would be mistaken
/// for the race: the bound exists so a genuine break fails in seconds instead of hanging.
const SPAWN_ATTEMPTS: usize = 3;

/// **A daemon of OUR OWN, with an authenticated `Control` stream.**
///
/// ⚠ **This exists because of a race the spawn helper's own comment admits and accepts**, and which
/// has now reddened `main` twice. [`Daemon::start`] claims `127.0.0.1:0`, DROPS the listener, and
/// hands the number to a child that binds it — so between the drop and the bind a sibling test can
/// take the same port, and the handshake then reaches a daemon this test never configured. Measured
/// on 2026-09-09 (PR #1727) and again on 2026-09-18 (`834b06b59`), both times as
/// `expected AuthOk(Control), got AuthDenied { reason: "control disabled on this node" }`, both
/// times green on the prior commit of identical tradehub code.
///
/// The comment offered one alternative — parse the port back out of the daemon's log — and rejected
/// it for trading the race against a dependency on a log line's wording. **This is a third answer it
/// did not consider: do not avoid the collision, DETECT it.** The detection is exact rather than
/// heuristic, because the one answer that proves a foreign peer is an answer our own child is
/// configured never to give ([`FOREIGN_DAEMON_REASON`]).
///
/// ⚠ It is a RETRY and not a fix, and the difference is worth keeping visible: the window is still
/// there, and a sibling can still take the port. What changed is that losing the race now costs a
/// re-spawn instead of a red `main`. The real fix — hand the bound listener to the child, the shape
/// `settings_hot_reload.rs`'s `spawn_node` already uses for an in-process server — needs the daemon
/// to accept an inherited descriptor, which is a change to the BINARY rather than to this test.
fn daemon_with_control() -> (Daemon, TcpStream) {
    let mut refusals = Vec::new();
    for attempt in 1..=SPAWN_ATTEMPTS {
        let daemon = Daemon::start();
        let mut ctl = daemon.connect_when_listening();
        match authenticate_control(&mut ctl) {
            Ok(()) => return (daemon, ctl),
            Err(reason) => refusals.push(format!("attempt {attempt} on {}: {reason}", daemon.addr)),
        }
    }
    panic!(
        "every one of {SPAWN_ATTEMPTS} spawns reached a daemon that is not ours — each on its own \
         fresh ephemeral port, which a port collision alone cannot explain. Either this box is \
         unusually contended, or `VIKE_TRADEHUB_CONTROL=1` has stopped enabling control:\n  {}",
        refusals.join("\n  ")
    );
}
...
fn an_accepted_command_is_on_disk_even_at_file_level_warn() {
    let (daemon, mut ctl) = daemon_with_control();
...
/// **The detector FIRES** — spawned against a daemon with the control gate off, which is exactly
/// what a port collision hands [`daemon_with_control`].
///
/// ⚠ **Without this, the retry above is untested code.** The collision it exists for is rare — five
/// consecutive runs of the real test on a the CI box lane took the happy path every time — so waiting for
/// it to happen is waiting for the thing the retry was written to stop happening. This produces the
/// condition directly instead: a daemon started WITHOUT `VIKE_TRADEHUB_CONTROL`, which is what the
/// six sibling tests in this binary all are, and which is therefore precisely what a stolen port
/// puts on the other end of the socket.
///
/// It asserts the SHAPE the retry keys on, not merely that something failed: an `Err` carrying
/// [`FOREIGN_DAEMON_REASON`] verbatim. If the daemon ever reworded that refusal, the retry would
/// silently stop retrying and this test is what says so.
#[test]
fn the_foreign_daemon_detector_fires_on_a_daemon_without_control() {
    let daemon = Daemon::start_with_control(false);
    let mut ctl = daemon.connect_when_listening();
    match authenticate_control(&mut ctl) {
        Err(reason) => assert_eq!(
            reason,
            FOREIGN_DAEMON_REASON,
            "the detector must key on the daemon's ACTUAL wording, or the retry stops retrying \
             the day that wording changes; stderr:\n{}",
            daemon.log()
        ),
        Ok(()) => panic!(
            "a daemon started WITHOUT VIKE_TRADEHUB_CONTROL admitted a Control handshake — either \
             the gate stopped gating, or this fixture stopped withholding the variable; \
             stderr:\n{}",
            daemon.log()
        ),
    }
