...
//!   and reached the same way: `server::account_refusal` on `accept_command`'s step 1c, against
...
/// ⚠ **THE WIRING, which is the half a unit test cannot reach.** `server::account_refusal` has its
