...
use vike_tradehub::tradehub_cli::{deadman_absent_warning, warn_deadman_absent};
...
fn the_absent_key_warns_exactly_once_and_neither_zero_nor_an_armed_value_warns_at_all() {
    ...
    warn_deadman_absent(&armed, vike_config::Authority::Files);
    assert!(
    ...
    warn_deadman_absent(&decided, vike_config::Authority::Files);
    assert!(
        deadman_warnings().is_empty(),
    ...
    warn_deadman_absent(&base, vike_config::Authority::Files);
    ...
    // ⚠ The headline names WHICH switch. It read `DEAD-MAN SWITCH IS OFF` until the
    // connection-state switch (M13) shipped armed by default, at which point an unqualified
    // headline told an operator that their daemon had no automatic stop when it had one.
    assert!(msg.contains("THE SILENCE DEAD-MAN IS OFF"), "the headline: {msg}");
    ...
    // …and it says what IS armed, so "off" cannot be read as "unprotected".
    assert!(
        msg.contains("CONNECTION-state dead-man is armed by default"),
        "names the switch that IS on: {msg}"
    );
    assert!(
        !msg.contains("NO automatic stop"),
        "the sentence that became FALSE with M13 must not come back: {msg}"
    );
    assert!(
        msg.contains(&format!(
            "deadman_timeout_ms = {}\n",
    ...
    // ⚠ THE OTHER BOX: the operator turned the CONNECTION-state switch off too. The message may
    // not keep claiming the sibling is armed — on that box it is not, and the per-venue lines
    // below it say so. Asserted on the PURE decision because the emitter is `Once`-latched and has
    // already fired above.
    let both_off = vike_config::Policy {
        link_deadman_grace_ms: Some(vike_config::LINK_DEADMAN_DISABLED_MS),
        ..base.clone()
    };
    let msg = deadman_absent_warning(&both_off, vike_config::Authority::Files)
        .expect("an absent silence key still warns");
    assert!(
        !msg.contains("CONNECTION-state dead-man is armed by default"),
        "a switch the operator disabled must not be reported as armed: {msg}"
    );
    assert!(
        msg.contains("`link_deadman_grace_ms = 0`"),
        "names the line that turned the other one off: {msg}"
    );
    assert!(
        msg.contains("NO automatic stop of any kind"),
        "…and says plainly what that box has: {msg}"
    );
}
