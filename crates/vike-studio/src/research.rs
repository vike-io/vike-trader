...
use vike_model::runs::{BACKTEST_RUN_KIND, RunConfig};
use vike_studio_core::listing::{ListedStudy, RunListing, StudyTier, list_runs, list_studies};
use vike_studio_core::study_run::nonfinite_tag;
use vike_studio_core::{
    STUDY_METRICS_NOTE, StoreHandle, StudyRun, StudyRunPlan, empty_params, read_recipe, recipes,
};
...
pub struct StudyHost {
    ...
    pub scratch: PathBuf,
    /// The BINARY doing this, spelled literally — `vike_model::runs::RunManifest::produced_by`
...
/// the two it does — `vike_model::runs::RunManifest::kind`'s own doc says the set of kinds grows
...
fn path_line(p: &std::path::Path) -> String {
    ...
    if std::path::MAIN_SEPARATOR == '/' { s } else { s.replace('/', std::path::MAIN_SEPARATOR_STR) }
}
