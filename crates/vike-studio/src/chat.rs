...
use vike_ai::{AgentResult, LedgerPaths, Provider, develop_strategy_with_ledger, make_client};
...
impl ChatPane {
    ...
    pub fn send(
        ...
        let example = vike_script::TEMPLATES.first().map(|t| t.1).unwrap_or("");
    ...
    /// over a RUNNING vike-datahub server, so `datahub_addr` must be a DATAHUB address.
    /// ⚠ It is NOT the Studio's Remote-backend address, which this said until 2026-09-20: since
    /// ruling 7 that one is the COMPUTE daemon's, so threading it here points the run/list tools
    /// at a socket that refuses their verbs by plane. The Studio holds no datahub address, so its
    /// only caller passes `None`, which lets the server fall back to
