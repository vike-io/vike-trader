...
//! `StoreHandle = Arc<dyn HistStore + Send + Sync>`, and `crates/vike-desktop/src/main.rs`'s
...
//! `crates/vike-desktop/src/app_methods.rs`'s `refresh_stored` and the "Loading…" state on
//! `crates/vike-desktop/src/app_ui.rs`'s `stored_loading`. This is the same discipline for the
