...
impl SavedStrategy {
    ...
    ///
    /// ⚠ **`Plugin` rows are `None` — the migration has no shape for them, by design rather than by
    /// omission.** `vike_studio_core::user_strategies::migrate`'s own module doc commits to exactly
    /// TWO populations (a Rhai row IS a strategy folder; a Native row is a PRESET for a registry
    /// one) and `LegacyBody` is declared with exactly those two variants — growing a third would
    /// ripple into that crate's own exhaustive matches (`MigrationSkip`, `plan_migration`'s
    /// per-kind planning), which is out of this track's scope. A runtime-loaded plugin is neither
    /// population: it is not code IN a file (Rhai's shape) and not a preset FOR a compiled-in
    /// registry entry (Native's shape) — it names an artifact this migration was never handed a
    /// tree for.
    ///
    /// ⚠ **THE CONTRACT A `None` OBLIGES THE CALLER TO: leave this row exactly where it is, in
    /// `studio_strategies.json`, as the read-only fallback — never "skip" it in the sense of
    /// treating it as handled.** Whoever wires the migration SWEEP (iterating every saved strategy
    /// and calling this method) must not delete the row, must not mark it migrated, and must not
    /// let the ABSENCE of a planned file for it be read as evidence the row was processed —
    /// `load_strategies` must keep finding it exactly as before, forever, until a future record
    /// gives Plugin rows a shape to migrate INTO. Reading `None` as "drop this and move on" is the
    /// defect this comment exists to prevent: the row is not gone, it never entered the pipeline.
    #[must_use = "a None here means this row still needs its JSON fallback preserved — dropping \
                  the return value without acting on that is how a saved plugin strategy goes \
                  missing from a migration sweep that assumed every row plans a file"]
    pub fn legacy_entry(&self) -> Option<LegacyEntry> {
        let body = match self.source {
            ...
            StrategySource::Plugin => return None,
        };
        Some(LegacyEntry { name: self.name.clone(), body })
    }
}
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        ui.with_layout(egui::Layout::left_to_right(egui::Align::Center), |ui| {
                            ...
                            let (color, tip) = match s.source {
                                ...
                                // Same accent as Rhai — both are code the user wrote (Rust here,
                                // Rhai there); Native is the odd one out, a NAME into the binary.
                                StrategySource::Plugin => {
                                    (crate::theme::ACCENT, "Runtime-loaded Rust plugin")
                                }
                            };
                            ui.label(
