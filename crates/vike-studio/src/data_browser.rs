...
impl DataBrowserPane {
    ...
    /// frame. A scan failure clears the pane AND records the reason (it used to degrade to a
    /// silent `unwrap_or_default` empty, indistinguishable from a genuinely empty store).
    pub fn refresh(&mut self, store: &dyn HistStore) {
        match store.inventory() {
            Ok(inventory) => {
                self.tree = vike_data_manager::build_tree(inventory);
                self.error = None;
            }
            Err(e) => {
                self.tree = Vec::new();
