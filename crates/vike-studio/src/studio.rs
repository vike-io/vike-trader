...
use vike_studio_core::{
    ...
    StudyRun, StudyRunError, native_strategies, params_from_rows, spawn_compare_all,
    spawn_paramscan, spawn_run, spawn_study, spawn_walkforward,
};
...
pub struct StudioState {
    ...
    /// store, the default and byte-identical to the pre-PR-5 behavior), [`Backend::Remote`]
    /// (offloaded to the COMPUTE daemon over TCP — ⚠ this said `vike-datahub` until 2026-09-20 and
    /// the default address agreed with it, which is what made it a bug rather than a typo) or
    /// [`Backend::Named`] (a strategy that same COMPUTE
    /// daemon already holds — `docs/decisions/0064-a-named-run-carries-no-source.md`). A runtime
    /// toggle like `strategy_source`, held on the state struct; not disk-persisted.
    pub backend: Backend,
    ...
    /// `vike_datahub_client::node_auth::Scope::Observe` is hard-coded at the dial rather than
    /// chosen here.
    ///
...
impl StudioState {
    ...
    pub fn named_spec(&self) -> StrategySpec {
        match (&self.strategy_source, &self.named_strategy) {
            (StrategySource::Rhai, _) | (StrategySource::Native, None) => self.current_spec(),
    ...
    fn start_sweep(&mut self) {
        ...
        let rx = match &self.backend {
            Backend::Local => {
                let store = self.store.clone();
                spawn_paramscan(spec, slice, store, grid)
            }
            Backend::Remote { addr } => {
            ...
            // catch-all so a fourth backend cannot land here silently.
        ...
        egui::Panel::top("studio-toolbar").show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui
                    ...
                    && !is_remote
                {
                    self.backend = crate::remote::Backend::remote_default();
                }
                ...
                // `Scope::Control` because it compiles client-supplied Rhai, and `ListStrategies`
                ...
                if ui
                    ...
                    && !is_named
                {
                    self.backend = crate::remote::Backend::named_default();
                    ...
                    let edited = ui
                        ...
                        .on_hover_text(
                            "COMPUTE daemon address (`vike-backend backtest --addr`) — a DIFFERENT \
                             process and port from the datahub the Remote backend dials",
                        )
                        .changed();
                    if edited {
                        self.named_roster = None;
    ...
    fn chat_pane_ui(&mut self, ui: &mut egui::Ui) {
            ...
            .clicked()
        {
            ...
            let addr = match &self.backend {
                ...
                // ⚠ The Named backend's address is the COMPUTE daemon's, and MCP's run/list tools
                // dial the DATA daemon — a different process on a different port. Handing one over
                // would point those tools at a socket that refuses their verbs by PLANE, so this
                // arm passes `None` and lets the server use its own default, exactly as Local does.
                Backend::Named { .. } | Backend::Local => None,
            };
