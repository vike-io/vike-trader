...
use vike_backtest::report::{DEFAULT_PERIODS_PER_YEAR, periods_per_year_for_interval};
...
pub struct StudioState {
    ...
    /// The editor buffer [`Self::plugin_sha`] was built FROM, when this session built it.
    ///
    ...
    pub builder_keys: Option<vike_datahub_client::NodeKeys>,
    /// The in-flight build's receiver (`None` when idle) — the worker-thread twin of
    /// `run_rx`/`sweep_rx`/`wf_rx`, folded by [`Self::poll`] the same way. A build is a CARGO
    /// BUILD, so this is the one worker in this shell whose legitimate wait is minutes.
    build_rx: Option<Receiver<Result<String, String>>>,
    ...
    /// way `vike_app_core::md_session::MdSession::set_key_name` is, and `None` in a bare
    /// `cargo run -p vike-studio`.
    ///
    /// ⚠ **Observe and never Control, which is the whole point of the backend it serves.**
    /// `crates/vike-app-core/src/backend_registry.rs`'s `datahub_observe_key` states why this
    /// process must never hold the datahub CONTROL key — that scope also compiles client-supplied
    /// Rhai — so this field can only ever carry the weaker one, and
    /// `vike_datahub_client::node_auth::Scope::Read` is hard-coded at the dial rather than
    /// chosen here.
    ///
    ...
    pub named_run_keys: Option<vike_datahub_client::NodeKeys>,
    ...
    /// GUI rather than slowing it. Same rule `crates/vike-app-core/src/backfill_wire.rs` states for
    /// its own dial.
...
impl StudioState {
        ...
        match action {
            ...
            SavedAction::SaveCurrent => {
                ...
                let entry = match self.strategy_source {
                    ...
                    StrategySource::Plugin => SavedStrategy::plugin(
                        name.clone(),
                        self.plugin_name.clone(),
    ...
    pub fn start_plugin_build(&mut self) {
        ...
        self.build_rx = Some(crate::plugin_build::spawn_build(
            self.builder_addr.clone(),
            self.builder_keys.clone(),
            self.plugin_name.clone(),
            self.editor.source.clone(),
        ));
    }
    ...
    /// the `vike-app` precedent this copies (`refresh_stored` + `stored_loading`).
    ///
    ...
    pub fn poll(&mut self) {
        ...
        // launching anything.
        match poll_worker(&mut self.build_rx) {
            Delivery::Ready(Ok(sha)) => {
                ...
                self.plugin_built_source = Some(self.editor.source.clone());
                self.plugin_sha = Some(sha.clone());
                self.build_last = Some(Ok(sha));
            }
            Delivery::Ready(Err(e)) => {
        ...
        egui::Panel::top("studio-toolbar").show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                let (chip_color, chip_text, chip_tip) = match self.strategy_source {
                    ...
                    StrategySource::Plugin => (
                        ...
                        format!(
                            "\u{1F9E9} {}",
                            if self.plugin_name.is_empty() { "plugin" } else { &self.plugin_name }
                        ),
                ...
                ui.add_space(4.0);
                let run_blocked = self.run_blocked_reason();
                let can_run =
                    !self.running && self.picker.selected().is_some() && run_blocked.is_none();
                let run_tip = run_blocked.unwrap_or("Run backtest  (Ctrl+Enter)");
                if ui
                ...
                let refresh = ui
                    .add_enabled(
                        !scanning,
                        egui::Button::new(if scanning { "⟳ scanning…" } else { "⟳ Refresh" }),
                    )
            ...
            ui.horizontal(|ui| {
                ...
                if ui
                    ...
                    .on_hover_text(
                        "Offload the run to the COMPUTE daemon over TCP — `vike-backend backtest \
                         --addr`, default 127.0.0.1:7880. NOT the datahub: it refuses every Run* \
                         verb by plane.",
                    )
                    .clicked()
                    && !is_remote
                {
                    // KEEP the address the operator typed. `remote_default()` hard-codes
                    // `DEFAULT_COMPUTE_ADDR`, so switching mode used to silently reset a
                    // hand-entered host back to 127.0.0.1:7880 - and BOTH modes dial the
                    // SAME daemon, so there was never anything to reset it FOR. An
                    // operator pointing the Studio at another box lost it by clicking a
                    // mode button, in the one field this branch has already had to repair
                    // twice for carrying the wrong port.
                    self.backend =
                        crate::remote::Backend::Remote { addr: self.backend.addr().to_string() };
                }
                // ⚠ **THE NAMED BACKEND — the only one that works with the OBSERVE key this shell
                // holds** (`docs/decisions/0064-a-named-run-carries-no-source.md`). The hover text
                // carries the trade rather than hiding it: Remote above needs the CONTROL key,
                // which `crates/vike-app-core/src/backend_registry.rs`'s `datahub_observe_key`
                // says this process must never hold, so against a keyed backend it cannot work
                // from here at all. ⚠ This said "the datahub CONTROL key" until 2026-09-20 and
                // BOTH halves misled: the key pair is one pair for both daemons (one domain
                // separator), and the daemon Remote dials is the COMPUTE one — it only ever
                // DEFAULTED to the datahub's port. What is true is the SCOPE: every `Run*` verb is
                // `Scope::Write` because it compiles client-supplied Rhai, and `ListStrategies`
                // plus the named run are the Observe half this shell can reach.
                let is_named = matches!(self.backend, Backend::Named { .. });
                if ui
                    .selectable_label(is_named, "Named")
                    .on_hover_text(
                        "Run a strategy the SERVER already holds, over this shell's read-only \
                         key. No script, no sweep, no walk-forward — one strategy, one param \
                         set, one bounded window.",
                    )
                    .clicked()
                    && !is_named
                {
                    // Same as the Remote button: the address survives the mode switch,
                    // because both modes dial the same daemon.
                    self.backend =
                        crate::remote::Backend::Named { addr: self.backend.addr().to_string() };
                    // A backend switch invalidates the roster: it belongs to the daemon that
                    // answered, and a stale one would offer names the new box may not hold.
            ...
            if let Backend::Named { addr } = &self.backend {
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    if fetching {
                        ui.label(egui::RichText::new("asking…").weak().size(11.0));
                        return;
                    }
                    match &self.named_roster {
                        None => {
                            ui.label(egui::RichText::new("roster not fetched").weak().size(11.0));
                        }
                        Some(Err(e)) => {
                            ui.label(
    ...
    /// is `vike_backtest::report::periods_per_year_for_interval` (vike-analytics', re-exported),
                ...
                ui.allocate_ui_with_layout(
                    ...
                    |ui| {
                        ...
                        if ui
                            ...
                            .on_hover_text(self.run_blocked_reason().unwrap_or("Ctrl+Enter"))
                            .clicked()
                        {
                            self.start_run();
                        }
    ...
    fn strategy_pane_ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.selectable_value(&mut self.strategy_source, StrategySource::Plugin, "Plugin (Rust)")
                .on_hover_text(
                    "Build the editor buffer as a Rust plugin and run the compiled artifact — \
        ...
        match self.strategy_source {
            ...
            StrategySource::Plugin => {
                ...
                ui.horizontal(|ui| {
                    ui.label(egui::RichText::new("Builder").weak().size(11.0));
                    ui.add(egui::TextEdit::singleline(&mut self.builder_addr).desired_width(140.0))
                        .on_hover_text(
                ...
                let can_build = !building && !self.plugin_name.trim().is_empty();
                ui.horizontal(|ui| {
                    if ui
                    ...
                    if building {
                        ui.label(egui::RichText::new("building…").weak().size(11.0));
                    }
                });
                ui.add_space(4.0);
                match (&self.plugin_sha, self.run_blocked_reason()) {
                    (Some(sha), None) => {
                        crate::theme::chip(
                            ui,
                            crate::theme::OK,
                            &format!("\u{25cf} built {}", &sha[..sha.len().min(8)]),
                        )
                        .on_hover_text(sha.as_str());
                    }
                ...
                if let Some(Err(e)) = &self.build_last {
                    ...
                    ui.add(
                        egui::Label::new(
                            egui::RichText::new(e).monospace().size(11.0).color(crate::theme::ERR),
                        )
                        .wrap(),
                    );
                }
...
mod tests {
    ...
    fn the_sweep_autorun_seeds_and_widens_the_grid_before_dispatching() {
        ...
        for (name, csv) in &st.grid {
            ...
            assert!(
                n > 1,
                "{name}: a one-candidate axis is the single-combination sweep ({csv:?})"
            );
        }
    ...
    fn a_delivered_build_sha_unblocks_run_and_nothing_else_is_dispatched() {
        ...
        st.build_rx = Some(ready(Ok("c".repeat(64))));
    ...
    fn an_edit_after_a_build_blocks_run_until_it_is_rebuilt() {
        ...
        st.build_rx = Some(ready(Ok("d".repeat(64))));
        st.poll();
    ...
    fn a_failed_build_clears_the_sha_rather_than_leaving_a_stale_one() {
        ...
        st.build_rx = Some(ready(Err("error[E0425]: cannot find value `x`".to_string())));
        st.poll();
