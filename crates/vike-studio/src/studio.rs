...
pub struct StudioState {
    ...
    /// (offloaded to a `vike-datahub` server over TCP) or [`Backend::Named`] (a strategy the COMPUTE
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Panel::top("studio-toolbar").show(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                // carries the trade rather than hiding it: Remote above needs the datahub CONTROL
                // key, which `crates/vike-app-core/src/backend_registry.rs`'s
                // `datahub_observe_key` says this process must never hold, so against a keyed
                // backend it cannot work from here at all.
                ...
                {
                    self.backend =
                        Backend::Named { addr: crate::remote::DEFAULT_COMPUTE_ADDR.to_string() };
