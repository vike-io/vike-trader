...
use vike_studio_core::{
    DataSlice, RunError, RunOutcome, SliceKind, StrategySpec, StudioSweep, SweepEntry,
    spawn_outcome,
};
...
/// One rendered TOML value — `vike_datahub::wire_run`'s `to_wire_wf_window` wrote it with
...
mod tests {
    ...
    /// The render→parse PAIR, end to end. `vike_datahub::wire_run`'s `to_wire_wf_window` writes
