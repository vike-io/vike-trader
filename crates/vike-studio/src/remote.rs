...
impl Backend {
    /// The value the UI's **Remote** button installs.
    ///
    ...
    /// The value the UI's **Named** button installs. Same daemon as [`Backend::remote_default`] —
    /// see [`Backend::Named`] for what differs, which is the credential and the ask, not the port.
    #[must_use]
    pub fn named_default() -> Self {
        Self::Named { addr: DEFAULT_COMPUTE_ADDR.to_string() }
    }

    /// The address this backend dials, or `None` for [`Backend::Local`], which dials nothing.
    #[must_use]
    pub fn addr(&self) -> Option<&str> {
        match self {
            Self::Local => None,
            Self::Remote { addr } | Self::Named { addr } => Some(addr.as_str()),
        }
    }
}
...
fn connect(addr: &str) -> Result<DatahubClient, RunError> {
    DatahubClient::connect(addr)
        .map_err(|e| RunError::Data(format!("connect to the compute daemon at {addr} failed: {e}")))
}
...
mod tests {
    ...
    fn remote_store_tick_refusal_refuses_exactly_the_local_tick_cell() {
        let remote = Backend::Remote { addr: DEFAULT_COMPUTE_ADDR.to_string() };
...
mod named_run_tests {
    ...
    fn no_offloading_backend_defaults_to_the_datahub() {
        for backend in [Backend::remote_default(), Backend::named_default()] {
            let addr = backend.addr().expect("an offloading backend carries an address");
            assert_ne!(
                addr,
        ...
        assert_eq!(Backend::Local.addr(), None, "Local dials nothing");
    }
    ...
    fn a_search_is_refused_on_the_named_backend_and_nowhere_else() {
        ...
        for other in [Backend::Local, Backend::Remote { addr: DEFAULT_COMPUTE_ADDR.to_string() }] {
