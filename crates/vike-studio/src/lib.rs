...
mod indicators;
mod picker;
// Flow step 2 of the runtime-plugin design: the BUILDER client. A module beside `remote.rs`
// rather than inside it — that file is one daemon's client, and the builder is a third service
// with its own domain, protocol version and key. Its own doc carries the argument.
mod plugin_build;
mod remote;
mod research;
mod results;
...
pub use plugin_build::spawn_build;
