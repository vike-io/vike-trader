...
pub mod theme;
mod workspace;
pub use chat::{apply_result, summary_of, ChatApiKeys, ChatOutcome, ChatPane};
pub use data_browser::DataBrowserPane;
pub use editor::EditorPane;
pub use indicators::{compute_indicator, default_params, grouped_search, IndicatorsPane};
pub use picker::{
    SeriesRow, SlicePicker, DEPTH_NOT_REPLAYABLE, SERIES_SCAN_ADVICE, SERIES_SCAN_FAILED,
};
...
pub use remote::{
    remote_store_tick_refusal, spawn_run_remote, spawn_sweep_remote, spawn_walkforward_remote,
    Backend,
};
...
pub use research::{
    fact_rows, kind_color, metric_rows, run_rows, study_result_ui, tier_label, ResearchAction,
    ResearchPane, RunRow, StudyHost, BACKTEST_RUN_KIND, NO_HOST, NO_LEARNER, NO_METRICS, NO_RUNS,
    NO_STUDIES, RUNS_BLURB, STUDY_RESULT_TITLE,
};
pub use results::{perf_rows, results_ui, returns_hist, ResultsTab};
pub use saved::{
    comparison_rows, load_strategies, save_strategies, CompareRow, SavedAction, SavedPane,
    SavedStrategy, StrategySource,
};
pub use studio::{CenterView, RightTab, StudioState};
pub use templates_gallery::{gallery_ui, template_preview};
pub use workspace::{load_workspace, save_workspace, StudioWorkspace};
...
pub use vike_studio_core::{
    ...
    spawn_run, spawn_sweep, spawn_walkforward, templates, DataSlice, RunError, RunOutcome,
    SliceKind, StoreHandle, StrategySpec, StudioSweep, SweepEntry,
};
...
pub use vike_studio_core::{StudyRun, StudyRunError, STUDY_METRICS_NOTE, STUDY_RUN_KIND};
