...
/// The combo's selected-text when the last [`SlicePicker::refresh`] could not enumerate the store.
///
...
pub const SERIES_SCAN_ADVICE: &str =
    "Could not list the store's series. Fix the store (a remote datahub: is the server up?), then \
     ⟳ Refresh.";
...
pub const DEPTH_NOT_REPLAYABLE: &str =
    "Recorded, but not replayable. `kind=depth` is the CONFLATING L2 lane — periodic full \
...
pub struct SlicePicker {
    ...
    /// `Some(reason)` when the last [`Self::refresh`] could not enumerate the store's series —
    /// rendered by [`Self::ui`] instead of the "no data in store" text an EMPTY store earns.
...
impl SlicePicker {
    ...
    /// `HistStore::list_series` call, so a mixed outcome — bars listed, ticks refused — is
    /// unreachable in every impl in the tree; and a PARTIAL list would be the same defect wearing
    /// a smaller hat, since the combo is read as the store's complete inventory either way.
    pub fn refresh(&mut self, store: &dyn HistStore) {
        // All three filters live once in vike-studio-core (see `bar_series`/`tick_series`/
        // `depth_only_series` there) — including the last, which is `tick_series`'s complement and
        // must not be re-derived from a second reading of the same rule.
        let listed = vike_studio_core::bar_series(store).and_then(|bars| {
            let ticks = vike_studio_core::tick_series(store)?;
            let depth_only = vike_studio_core::depth_only_series(store)?;
            Ok((bars, ticks, depth_only))
        });
        let (bars, ticks, depth_only) = match listed {
            Ok(lists) => lists,
            Err(e) => {
        ...
        self.available = bars
            ...
            .chain(ticks.into_iter().map(|(venue, symbol)| SeriesRow {
                venue,
                symbol,
                interval: None,
            }))
