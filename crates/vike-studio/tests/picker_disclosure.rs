...
//! models: `vike_studio_core::series_lists` — the one call `SlicePicker::refresh` and the off-thread
//! path both fold — reads that one catalog verb once, which is why the picker
...
use common::{REASON, RefusingCatalogStore};
