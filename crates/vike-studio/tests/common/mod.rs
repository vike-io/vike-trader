...
//!
//! ⚠ **This double refuses `list_series` and NOTHING else**, which is the whole failure it models:
//! `vike_studio_core::bar_series`, `vike_studio_core::tick_series` and
//! `vike_studio_core::depth_only_series` all fold that one catalog verb, so the picker treats a
//! refusal as one indivisible outcome rather than listing whichever part answered.
//!
//! ⚠ ...and the consequence the SHELL test has to live with, because the shell reads more of the
//! store than the picker does. `crates/vike-data/src/hist.rs`'s `inventory` is a DEFAULTED trait
//! verb and is not overridden here, so it answers with that default's empty `Ok`: the Studio's
//! Data pane over this double reads as an EMPTY store while the picker reads as a FAILED one. A
//! real `RemoteHistStore` whose server went away would fail both. That is why the shell test
//! asserts the toolbar/central-panel half only and says nothing about the Data pane — an honest
//! half, not an unstated gap.
//!
