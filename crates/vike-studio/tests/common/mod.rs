...
use vike_studio::{ChatApiKeys, EditorPane, QaAutorun, RightTab, StoreHandle, StudioState};
...
/// Bind an ephemeral loopback listener, serve the COMPUTE verbs over `store` on a detached
/// thread, and return the address a [`vike_studio::Backend::Remote`] should dial.
///
...
/// leaves the process now, so a test that needs a RESULT needs something to answer it — and the
/// DEFAULT address is a real deployed daemon's. Without this, `start_run` dialled
/// `127.0.0.1:7880` and, on the CI box where that daemon runs, came back with its AUTH REFUSAL:
/// a test reaching a live service and reporting the rejection as its own failure.
///
...
/// in-process path it replaces: the run is marshalled over the real wire now.
///
/// ⚠ Nothing joins the thread. The listener dies with the process, which is what every other
/// ephemeral-server harness in this workspace does; a test that wants determinism about SHUTDOWN
/// needs a different tool than this one.
#[must_use]
pub fn spawn_compute_server(store: StoreHandle) -> String {
