...
pub mod node;
pub use node::{
    MOUNT_FAILED, MountAccount, Node, NodeConfig, NodeError, WIRED_MARKETS, account_symbols_for,
    armed_live_venues, build_node, build_node_with_preflight, journal_venue_mounts, mount_accounts,
    refuse_unarmed_live_venues,
};
/// The settings database's `account` table as the composition ROOT read it, re-exported for the
/// same reason [`MountPolicy`] is and to fill one of its fields: a daemon that builds a
/// [`NodeConfig`] must be able to NAME this type to hand its own store read over, and
/// `vike-tradehub` has no `vike-mount` edge of its own.
///
/// ⚠ A root that leaves it unread degrades exactly as an unmigrated box does — every LABELLED
/// dukascopy account refused by name, the default account byte-identical. It is not a silent
/// failure, but it IS a silent absence, so a root that mounts venues should fill it.
pub use vike_mount::AccountDirectory;
...
/// [`vike_mount::accounts_to_mount`] — **WHICH accounts of a venue actually get an engine**,
/// reached through this crate for the same reason the rows above are.
///
/// ⚠ Re-exported rather than left to each consumer's own predicate, and the predicate is the point:
/// it is not simply "effective is not paper" — the venue's DEFAULT account is mounted even on
/// paper, because a paper engine is still the engine that venue's strategies fold through. A
/// reporting surface that restated the rule would disagree with the mount the day the rule moved,
/// and a screen whose job is to describe what the mount will do cannot afford its own copy of it.
pub use vike_mount::accounts_to_mount;
...
///
/// ⚠ **It takes the whole [`vike_mount::MountPolicy`] since 2026-09-15, not its `venues` table.**
/// The producer needs `MountPolicy::accounts` — the settings database's `account` table as the
/// composition root read it — because the dukascopy row resolves an ACCOUNT out of it, and the mount
/// resolves the same account out of the same snapshot. Handing over the ceilings alone would leave
/// the screen unable to answer for that venue at all.
#[must_use]
pub fn venue_arming(
