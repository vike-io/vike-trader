...
pub struct NodeConfig {
    ...
    /// `vike_tradehub::reconcile_config::reconcile_enabled` over the REAL process env). Drives whether
...
pub struct Node {
    ...
    /// ⚠ ONE ROW PER ACCOUNT, not per venue — `vike_core::ReconLeg` carries the canonical venue
    /// and the account's route key as two separate facts, and the reconcile manager stamps both
    /// onto every payload. A box with no `[accounts]` table produces exactly the rows it always
    /// did, each with `route_key: None`.
...
pub fn armed_live_venues(
    ...
    WIRED_MARKETS
        .iter()
        .flat_map(|(venue, _)| vike_mount::venue_account_arming(venue, vars, Some(policy)))
...
fn refuse_unarmed_mount_accounts(
    ...
    for m in mounts {
        let Some(label) = m.account.as_ref().filter(|l| !l.is_default()) else { continue };
        let armed = vike_mount::venue_account_arming(&m.venue, &cfg.vars, Some(&cfg.policy))
            .into_iter()
...
/// CANONICAL venue id, and that account's own route key (`venue#LABEL`). The manager stamps both
/// onto every payload it enqueues, so the fold thread routes the pass by route key while every
/// venue-shaped job — the health probe, the ring notes, the held-alert dedup identity — keeps
/// reading a real `vike_model::VENUES` id.
///
/// ⚠ **This doc used to argue the opposite and the behaviour changed with it.** The handle was
/// keyed by route key ALONE, which reached `vike_core`'s per-venue health gate as an unknown venue;
/// that gate reads `Healthy` on a miss, and the argument offered for it was that a second account
/// "reconciling regardless of the first account's feed is the correct behaviour — they are
/// different books". They ARE different books, and that is beside the point the gate is making: the
/// probe answers whether THE EXCHANGE'S FEED is mid-gap, which is one fact about one venue that
/// both accounts' passes read through. A degraded binance feed now suppresses both binance legs,
/// which is what suppression was for.
...
fn mount_accounts_of(
    ...
    for (label, (extra_engine, extra_recon)) in mounted {
        ...
        if let Some(rc) = extra_recon {
            // BOTH facts: the canonical exchange id the health gate and every note key on, and
            // THIS account's route key, which is the only thing that can tell the fold thread
            // which of the venue's engines these reports belong to.
...
fn build_node_inner(cfg: NodeConfig) -> Result<Node, NodeError> {
    ...
    // `vike_fxcm::sdk_available()` is true; a build without the proprietary ForexConnect SDK compiles
    ...
    // ⚠ **Every row here is that venue's DEFAULT account**, whose route key IS its venue id — so
    // each becomes a `ReconLeg` with `route_key: None` and the payload the manager stamps is
    // byte-identical to the one it stamped before accounts existed. The extra accounts are
    // appended at the end as legs carrying BOTH facts.
    //
    // ⚠ This comment used to say the extras were keyed by route key "rather than a bug", and that
    // WAS the bug: one string cannot be both the health-probe key and the routing key. A leg
    // carries the two separately now — `vike_core::ReconLeg`'s doc argues both failure directions.
    let recon_clients: Vec<vike_core::ReconLeg> = [
    ...
    .into_iter()
    // `sole_account_of`: every row here IS its venue's default account, whose route key is the
    // bare venue id — so the leg carries no route key and its payload is byte-identical to the
    // one this assembly produced before accounts existed.
    .filter_map(|(venue, recon)| recon.map(|r| vike_core::ReconLeg::sole_account_of(venue, r)))
    .collect();
    ...
    if let Some(r) = ibkr_recon {
        recon_clients.push(vike_core::ReconLeg::sole_account_of("ibkr", r));
    }
    ...
    if let Some(r) = poly_recon {
        recon_clients.push(vike_core::ReconLeg::sole_account_of("polymarket", r));
    }
    ...
    if let Some(r) = fxcm_recon {
        recon_clients.push(vike_core::ReconLeg::sole_account_of("fxcm", r));
    }
    ...
    // ⚠ …and then EVERY account of a venue this process runs several ENGINES of NAMES ITSELF. The
    // default account's leg above was built by `sole_account_of`, which stamps `route_key: None` —
    // correct, and only correct, while that venue has one engine. This step is what makes `None`
    // mean exactly one thing on the wire, which is what lets `CoreThread::reconcile_reports` REFUSE
    // a `None` it cannot attribute (the spec's Class E refusal) instead of folding it against a
    // guess.
    //
    // ⚠ **The set handed in is the ENGINES, not the legs, and that is the 2026-09-14 correction.**
    // The refusal counts ENGINES (`CoreThread::engines_of_venue`), so the stamp has to count the
    // same thing or the two guards diverge — and [`mount_accounts_of`] makes them diverge BY
    // CONSTRUCTION: it pushes `AccountExtras::engines` unconditionally and `AccountExtras::recon`
    // only `if let Some(rc) = extra_recon`. A second ARMED account whose reconcile client came back
    // `None` (a ctrader/ibkr connect that failed at mount, an ig/oanda/deribit recon handshake that
    // returned `None`) is therefore a venue with two engines and ONE leg — and while this counted
    // legs, nothing was stamped and the fold thread then refused the SURVIVING default account's
    // own pass, every interval, for the life of the process. `build_node_inner` is the one place
    // that holds both halves, which is why the set is assembled here rather than inferred there.
    //
    // A no-op on every box with no `[accounts]` table: no venue has two engines there, so nothing
    // is stamped and the journal bytes are unchanged. `ReconLeg::name_accounts_of_shared_venues`
    // argues every half.
    //
    // ⚠ Spelled from `primary` + `extra` in exactly the order `spawn_core_multi` below takes them,
    // because that IS the engine set the core will hold — the same pair `assemble_core` computes
    // `CoreThread::multi_account` from. The borrow ends at the call below, before the move.
