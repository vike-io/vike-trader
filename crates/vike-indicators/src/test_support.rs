...
/// raw columns — where it is SHARPER than it is for bars, because a `vike_marketdata::Bar` cannot hold a
...
///   reading the clip allows) on a LARGE FRACTION — not a handful of rows — of an entire feature
///   family. At `lag = 1` that is exactly the row this shape produces.
...
///   instances of it, and that constant's own doc carries the argument: the standard deviation
