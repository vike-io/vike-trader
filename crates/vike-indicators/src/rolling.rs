...
///
/// A series bounded in `[-1, 1]` — a sentiment-style input, say — can pin near a bound for long
/// stretches: many consecutive hours of values sitting within ~5e-4 of the bound WITHOUT ever
/// being bit-equal. That collapses the trailing standard deviation while the numerator stays
/// ordinary, so the ratio explodes by orders of magnitude. Those are DENOMINATOR failures, not
/// large moves, and they dominate any effect size measured on the feature. Clipping preserves
/// ordering while bounding the range.
///
/// (The specific series and the specific `|z|` this was first measured on are deliberately not
/// named here — the regime is a property of ANY bounded input, and the generator cited below
/// reproduces it from in-repo data.)
///
...
mod tests {
    ...
    fn a_rolling_window_never_reaches_across_an_asset_boundary() {
        ...
        cols.insert("sig_a".to_string(), vec![1.0, 2.0, 3.0, 4.0, 10.0, 20.0, 30.0, 40.0]);
    ...
    fn a_missing_row_is_nan_and_not_the_clip_floor() {
        // The Task-1 regression, at panel level: one column's observation absent at the last
        // hour, with its own history behind it full. Without
        ...
        v[29] = f64::NAN;
        let mut cols = Columns::new();
        cols.insert("sig_a".to_string(), v);
        assert!(zscore_block(&cols, &f.span_ranges(), 24)["z24_sig_a"][29].is_nan());
    }
    ...
    fn every_z_value_is_inside_the_clip_band() {
        ...
        assert!(
            out["z24_sig_a"].iter().any(|x| x.abs() == Z_CLIP),
    ...
    fn a_block_preserves_its_input_column_order() {
        // ⚠ The insertion order is deliberately NOT alphabetical. These were once real column
        // names whose ordering happened to differ from their sort order; neutral names that sorted
        // as they were inserted would let a hash- or BTree-backed container pass this assertion by
        // accident, which is the failure the `Columns`-is-a-`Vec` decision exists to catch.
        let f = frame_of(&[("A", 30)]);
        let mut cols = Columns::new();
        ...
        assert_eq!(
            ...
            vec!["pr24_sig_c", "pr24_sig_a", "pr24_sig_b"]
        );
    }
