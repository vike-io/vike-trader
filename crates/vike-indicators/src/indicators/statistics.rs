...
//! → `f64::NAN`; builtin `sum()` over a window → naive `.iter().sum()` — ⚠ which is the ONE place
//! this catalog deliberately departs from the root `CLAUDE.md` parity rule, and
//! `crates/vike-indicators/src/lib.rs` carries the three measured reasons it is a choice rather
//! than a defect; do not "fix" this to `py_sum` without reading them — matching
