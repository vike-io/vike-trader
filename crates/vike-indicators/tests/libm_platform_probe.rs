...
//!
//! This file now holds THREE tests — the header said TWO until the SOURCE gate joined them, FOUR
//! once that gate's own cut got a non-vacuity arm, and THREE again once decision 0074 moved the
//! shared text parser to `vike_model::libm_walk` and the planted fixture that proves the cut to
//! `crates/vike-model/tests/libm_walk_selftest.rs` — one copy in place of the ten it replaces.
//! The difference between the first two is the difference between measuring a problem and holding
//! it closed; the third is what stops the other two from certifying a tree that drifted out from
//! under them:
//!
...
//!
//! # MEASURED 2026-08-25 — Windows 11 / MSVC CRT against a Linux / glibc box, rustc 1.96.0
//!
...
///
/// The banned set is `vike_model::libm_walk`'s `BANNED_FNS` rendered through [`banned_needles`],
/// which is where the argument for every name lives — including the two SPELLINGS each name gets.
/// Decision 0074 moved that list and the range walk out of this file and into one shared home, so
/// the general argument is no longer restated here. Until 2026-08-26 only the method form was
/// banned, so a fully-qualified `f64::exp(x)` matched nothing.
///
/// # ⚠ What the SHARED list cannot know — the per-crate evidence, measured here
///
/// The paragraphs below were harvested out of the doc comments on the parser this file used to
/// own. They are facts about `crates/vike-indicators/src`, not about the machinery, so they stayed
/// with the gate that rests on them rather than travelling to `vike_model::libm_walk`.
///
/// ⚠ **The inverse-trig and hyperbolic family was once MISSING from the banned list, and the
/// omission was LIVE in this crate rather than theoretical.**
/// `crates/vike-indicators/src/indicators/statistics.rs`'s `batch_linearreg_angle` calls
/// `libm::atan(b)` in a file that carries no `#[cfg(test)]` marker at all — every line of it is
/// production and every line of it was scanned — so reverting that one call to `b.atan()` passed
/// this gate cleanly, and the pinned `linearreg_angle` row would then have had to disagree across
/// platforms before anything went red. `tan` could not stand in for `atan`: a needle anchors a `(`
/// immediately after the name and a `.` or `::` immediately before it, so `.tan(` matches neither
/// `.atan(` nor `.tanh(`, and `atan2` needs its own entry rather than riding on `atan`.
///
/// ⚠ **The 2026-08-26 widening from 22 names to 26 — `asinh`, `acosh`, `atanh`, `sin_cos` —
/// changed no verdict HERE: none of the four appears anywhere under this crate's `src/`.** It
/// removed a gap the next edit could have walked into; it did not close a finding.
///
/// ⚠ **The three spellings a needle list cannot see are UNOCCUPIED in this crate today.** The
/// shared module declares the blind spot — the angle-bracket form (`f64>::exp(`), a call reached
/// through a generic bound (`T: Float`, `num_traits::Float::exp`), and a call split across two
/// lines by a `max_width` wrap — but whether it is occupied is a per-crate fact, and this is this
/// crate's answer: none of the three occurs under `crates/vike-indicators/src`. A list of
...
/// on** when it declares a square-through-`powi` unable to come back silently — and that claim
/// covers BOTH spellings, which it did not before the needle renderer existed.
///
/// ⚠ **`to_degrees` / `to_radians` are deliberately absent from the banned list and must stay
/// absent** — std lowers each to a single multiply by a constant, so they are ordinary arithmetic
/// rather than a libcall. There IS a separate reason `batch_linearreg_angle` must not use one:
/// `crates/vike-indicators/src/indicators/statistics.rs`'s module doc records that Rust's
/// precomputed constant can differ by a ULP from CPython's `math.degrees`, so the port spells
/// `atan(b) * (180.0 / PI)` by hand. That is ORACLE PARITY — a different contract with a different
/// owner — and folding it into the banned list would make that list mean two things at once.
///
...
/// `crates/vike-model/tests/libm_walk_selftest.rs` is the non-vacuity arm — a FIXTURE rather than
/// a tree measurement precisely because this tree cannot exercise the case, and since decision
/// 0074 ONE shared fixture rather than a private copy in each of the eleven probes.
///
/// ⚠ **The range walk ends an item by INDENTATION rather than by counting braces, and the
/// indentation rule was MEASURED rather than assumed.** 2026-08-26: all 52 `#[cfg(test)]` markers
/// under the three gated `src/` trees — this crate's among them — sit at column 0, and each is
/// followed either by a line ending in `{` or by a `;` (the `mod NAME;` declaration form, which
/// exists in `crates/vike-mm/src/lib.rs` and is why the `;` arm is not written for a
/// hypothetical).
///
...
fn production_code_calls_libm_not_the_platform() {
    // The banned set, in both spellings, lives on `vike_model::libm_walk`'s `BANNED_FNS` /
    // `banned_needles` — moved out of this function body so the argument for each name sits with
    // the name rather than in a comment the next widening has to be threaded through, and moved
    // out of this CRATE by decision 0074 so eleven probes share one parser instead of eleven.
        ...
        for entry in std::fs::read_dir(&dir).expect("crate has a src/ directory") {
            ...
            for (i, line) in lines.iter().enumerate() {
                if test_items.iter().any(|&(start, end, _)| i >= start && i < end) {
                    ...
                    // hiding two live call sites, and this test's own doc carries that
                    // measurement.
                    continue;
                }
