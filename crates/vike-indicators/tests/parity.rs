...
use vike_indicators::{make, make_with, registry, Indicator, WindowReach};
use vike_model::Bar;
...
fn the_finite_window_roster_is_pinned() {
    ...
    let patterns: Vec<&str> = registry()
        ...
        .map(|m| m.name)
        .collect();
    assert!(patterns.len() > 50, "the pattern family collapsed to {} — the filter is wrong, and a          vacuous family would make the assertion below prove nothing", patterns.len());
