...
//! [`every_pinned_row_is_non_vacuous`], [`production_code_calls_libm_not_the_platform`] and
//! [`the_self_exclusion_is_narrow`]) is an ORDINARY test that `--ignored` filters OUT. ⚠ This read
//! "and the two walk controls" until decision 0074: the second control was the planted-fixture
//! proof of the range walk, and it left this crate with the parser it proves —
//! `crates/vike-model/tests/libm_walk_selftest.rs` runs it once for all eleven probes now.
//!
//! A recording session driven by the second line alone therefore exits 0 having asserted nothing,
//! which is how a run can look green while the outstanding rows go unmeasured. Use the FIRST line
//! to record and gate; the second only to eyeball two boxes side by side.
//!
//! ```text
...
///
/// The banned set is `vike_model::libm_walk::BANNED_FNS` rendered through [`banned_needles`], so
/// the method spelling and the UFCS spelling of the same call can never fall out of step; both
/// carry the argument for what is on the list and what is deliberately off it.
///
/// ⚠ **The TEXT MACHINERY moved out of this file on decision 0074 and the GATE did not.** The name
/// list, the needle renderer and the range walk are one shared parser now
/// (`crates/vike-model/src/libm_walk.rs`), because eleven crates were carrying eleven copies of it
/// and three had already fallen behind the other eight. What stayed here is everything that is a
/// fact about VIKE-MM: this test, its `CARGO_MANIFEST_DIR`, its must-scan list, its self-exemption,
/// its failure message — and the measurements below, which were harvested out of those parsers'
/// doc comments rather than deleted with them.
///
...
///
/// # ⚠ RANGES, not a `break` — and the hole the `break` left had a MEASURED size
///
/// The scan used to stop at the first marker, which is right only for a file whose test module is
/// last, and five files in this crate put one in the middle. That `break` is why the module doc
/// said THIRTEEN sites when there are fifteen: the two it could not see were the two nobody
/// counted.
///
/// Measured over `crates/vike-mm/src` on 2026-08-26, the range walk scans **1,612 production lines
/// the `break` discarded** — `lib.rs` +972 (its `#[cfg(test)] mod platform_probe;` sits on line 129
/// of 1104, so 88% of the file was outside the gate), `avellaneda.rs` +633 (six test modules, the
/// first at line 394 of 1921), `own_book.rs` +5, `fits.rs` +1, `strategy_impl.rs` +1. Two of this
/// crate's fifteen `libm` sites live in that discarded region; see the module doc's CORRECTION.
/// The `scanned_past_a_test_module` assertion below is what holds that fix down.
///
/// ⚠ **Those two rows are evidence about `avellaneda.rs` and `lib.rs` as they are TODAY.**
/// Reorganise either file so its test module goes last and the assertion passes VACUOUSLY while the
/// mechanism goes unproven. The mechanism itself — production before a test module, a banned call
/// inside it, production after it, and the `;` header form no brace-matching walk would handle by
/// accident — is proved on a planted fixture in `crates/vike-model/tests/libm_walk_selftest.rs`,
/// which is where this file's own copy of that fixture went on 0074. Both halves are needed, and
/// only one of them is in this crate now.
///
/// # What the INDENTATION rule rests on here
///
/// The walk finds an item's end by indentation rather than by counting braces (the shared module
/// argues why). Measured 2026-08-26: all 52 `#[cfg(test)]` markers under the three gated `src/`
/// trees sit at column 0 — not one is indented — and every one is followed either by a line ending
/// in `{` (a `mod`/`impl` item) or by a `;`, and the `;` arm's real-tree instances are this crate's
/// own: `lib.rs`'s two `mod NAME;` declarations, so that arm is exercised rather than written for a
/// hypothetical.
///
/// ⚠ The comment filter runs BEFORE the marker test for a reason this crate supplies: a doc comment
/// spelling the attribute inside backticks is a HOUSE-STYLE HEADER here — `src/xemm/pricing.rs` and
/// three siblings open with one, forty lines above any real marker — and reading one as a marker
/// would blank the file. That is how the thirteenth site was found; the module doc carries it.
///
/// # What the needle list has been measured NOT to miss, under this crate's `src/`
///
/// * The four names the shared list gained on 2026-08-26 (`asinh`, `acosh`, `atanh`, `sin_cos`,
///   none of them reachable from the original 22 by substring) appear NOWHERE under this crate's
///   `src/` today, so that widening changed no verdict here — it removed a gap the next edit could
///   have walked into, which is the only moment a list like this can be widened for free.
/// * The three spellings `banned_needles` still cannot see — the angle-bracket qualified form, a
///   call reached through a generic bound (`T: Float`), and a call split across two lines by a
///   `max_width` wrap — not one of the three occurs under this crate's `src/` today, and the first
///   would be the cheapest to add the day it does. A list of spellings is an enumeration, and an
///   enumeration is never a proof, which is why this measurement is restated per crate instead of
///   being asserted once in the shared module.
#[test]
fn production_code_calls_libm_not_the_platform() {
    ...
    assert!(
        ...
        "a `#[cfg(test)]` item's closing line could not be located, so everything below it went \
         ...
         by a lone `}}`, or a `;` for a `mod NAME;` declaration. An empty \
         `#[cfg(test)] mod x {{}}` on one line, or a hand-formatted item rustfmt did not touch, \
         will land here.\n{}",
        unclosed.join("\n")
    );
    ...
    assert!(
        ...
        "production code must call the `libm` CRATE rather than `f64`'s platform-libm methods — \
         ...
         `vike_model::libm_walk::banned_needles`. A `powi` is banned too: on MSVC a `dev` build \
         makes it a `pow()` libcall, and its cure is a multiply rather than `libm`:\n{}",
        found.join("\n")
    );
}
...
///   this test exists to catch walks straight past it. The mirror-image spelling was live in this
///   very file until decision 0074 — the range walk's planted fixture opened with `let file = [`
///   and carried a banned call inside a string two lines down, so the crude filter was being
///   leaned on from both sides at once. That fixture now lives in
///   `crates/vike-model/tests/libm_walk_selftest.rs`, but the pressure has NOT gone away: the
///   `literal_row` control inside [`the_self_exclusion_is_narrow`] is the same spelling, planted
///   deliberately so this stripper keeps being tested against it.
///
...
/// one. This file is skipped because it spells out the gate's own needles — the names in
/// `vike_model::libm_walk::BANNED_FNS` and everything [`banned_needles`] renders from them — so the
/// exemption is only honest while the file contains no actual transcendental call of its own.
///
...
///
/// The needle list here is a SUBSET of `vike_model::libm_walk::BANNED_FNS` on purpose — it is a
/// tripwire on one file, not the tree-wide ban — but it tracks the widenings, so the 2026-08-26
/// four (`asinh`, `acosh`, `atanh`, `sin_cos`) and both UFCS spellings are here.
#[test]
fn the_self_exclusion_is_narrow() {
    ...
    assert!(
        ...
        "the stripper kept a needle that was inside a string literal — this test would then \
         report any planted source fixture in this file as a call. The range walk's own fixture \
         is the shape this guards against; it moved to \
         `crates/vike-model/tests/libm_walk_selftest.rs` on decision 0074, and the row above is \
         kept as its stand-in so the stripper is still measured against that spelling"
    );
