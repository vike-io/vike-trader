...
use vike_model::{
    AsParams, BookLevel, HorizonMode, KappaMode, PriceDomain, ReservationModel, SpreadModel,
    SpreadSource, TradeTick, VarianceMode,
};
...
impl AsState {
    ...
    fn fair_value(&self, view: &BookView) -> Option<f64> {
        let &BookLevel { price: bid_px, qty: bid_sz } = view.bids.first()?;
        let &BookLevel { price: ask_px, qty: ask_sz } = view.asks.first()?;
