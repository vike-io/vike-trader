...
impl OwnOrderBook {
    ...
    pub fn subtract_levels(
        &self,
        side: OwnSide,
        levels: &[vike_model::BookLevel],
        filter: &OwnQtyFilter,
    ) -> Vec<vike_model::BookLevel> {
        let mut out = Vec::with_capacity(levels.len());
        for &vike_model::BookLevel { price, qty: size } in levels {
            let own = self.qty_at(side, price, filter);
            if own <= 0.0 {
                // nothing of ours here — pass the level through untouched (no float op at all)
                out.push(vike_model::BookLevel::new(price, size));
                continue;
            }
            let remaining = size - own;
            if remaining > 0.0 {
                out.push(vike_model::BookLevel::new(price, remaining));
            }
...
mod tests {
    ...
    fn buffer_elapsed_includes_the_order_and_shifts_the_best() {
        ...
        assert_eq!(
            ...
            "partly ours ⇒ only our size subtracted"
        );
