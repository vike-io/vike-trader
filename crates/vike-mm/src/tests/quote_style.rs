...
fn each_style_prices_from_a_fixture_book() {
    let book = BookView {
        ...
        bids: vec![
            ...
            BookLevel::new(99.0, 7.0),
            BookLevel::new(98.0, 9.0),
        ], // best-first
        asks: vec![
            BookLevel::new(101.0, 4.0),
            BookLevel::new(102.0, 6.0),
            BookLevel::new(103.0, 8.0),
        ],
    };
    ...
    let half = BookView { tick_size: 1.0, bids: vec![BookLevel::new(100.0, 5.0)], asks: vec![] };
...
fn own_order_filtration_shifts_the_derived_best() {
    let book = BookView {
        tick_size: 0.5,
        bids: vec![BookLevel::new(100.0, 3.0), BookLevel::new(99.5, 8.0)],
        asks: vec![BookLevel::new(100.5, 2.0), BookLevel::new(101.0, 9.0)],
    };
    ...
    assert_eq!(f.bids[0].price.to_bits(), 99.5_f64.to_bits(), "best bid shifts past our own");
    assert_eq!(f.asks[0].price.to_bits(), 101.0_f64.to_bits(), "best ask shifts past our own");
...
fn own_order_filtration_reduces_size_when_not_alone() {
    let book = BookView {
        tick_size: 0.5,
        bids: vec![BookLevel::new(100.0, 10.0)],
        asks: vec![BookLevel::new(100.5, 10.0)],
    };
    let f = filter_own(&book, Some((100.0, 4.0)), None);
    assert_eq!(
        f.bids[0].price.to_bits(),
        100.0_f64.to_bits(),
        "best price unchanged when not alone"
    );
    assert_eq!(f.bids[0].qty.to_bits(), 6.0_f64.to_bits(), "our size subtracted from the level");
    assert_eq!(f.asks[0].qty.to_bits(), 10.0_f64.to_bits(), "untouched side is unchanged");
}
...
fn filtration_is_inert_without_a_tick_grid() {
    let book = BookView {
        tick_size: 0.0,
        bids: vec![BookLevel::new(100.0, 3.0)],
        asks: vec![BookLevel::new(101.0, 3.0)],
    };
...
fn on_order_book_depth_style_rests_deeper() {
    ...
    book.apply_snapshot(
        1,
        &[BookLevel::new(100.0, 5.0), BookLevel::new(99.5, 8.0), BookLevel::new(99.0, 3.0)],
        &[BookLevel::new(100.5, 4.0), BookLevel::new(101.0, 9.0), BookLevel::new(101.5, 2.0)],
    );
