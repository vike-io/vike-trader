...
//!    in a FABRICATED book instead (`vike_app_core::dom_math::synth_book`: 40 gapless, perfectly
...
use vike_ui_theme::palette::trading::{
    ACCENT as LAST, DOWN as ASK, FAINT, MUTED, PANEL, PANEL2, RULE, TXT, UP as BID, dim,
};

const ROW_H: f32 = 20.0;
const HEADER_H: f32 = 26.0;
/// The DOM's own-source strip — one always-present line naming WHERE this ladder's depth comes
/// from and what that link is doing. It is its own row rather than a header badge because the
/// header is already five items wide at the 560 px the tool window opens at, and the one fact a
/// crowded-out badge would drop is the one this strip exists to state.
const SOURCE_H: f32 = 18.0;
...
pub struct DomInputs<'a> {
    pub book: &'a L2Book,
    /// last traded price (drives the amber row + auto-center). `None` ⇒ this window has NO price
    /// at all, and the header says so with a dash: it must never print a plausible number that
    /// came from a table rather than from a venue.
    pub last: Option<f64>,
...
pub fn draw(ui: &mut egui::Ui, state: &mut DomState, inputs: &DomInputs) -> Vec<DomAction> {
    ...
    header(ui, state, last_opt, inputs.stale && !bookless, inputs.paper);

    // --- source strip: WHERE this ladder's depth comes from, and what that link is doing ---
    source_strip(ui, inputs.source);

    // --- toolbar: qty presets, grouping, recenter, reduce-only ---
    toolbar(ui, state);

    // --- ladder / heatmap region ---
    let region_h = (full.height() - HEADER_H - SOURCE_H - TOOLBAR_H - FOOTER_H).max(ROW_H * 5.0);
    ...
    //
    // The predecessor did the opposite. `vike_app_core::dom_math::synth_book` built 40 gapless,
    ...
    if bookless {
        no_book(ui, region, inputs.absence);
...
fn header(ui: &mut egui::Ui, state: &mut DomState, last: Option<f64>, stale: bool, paper: bool) {
    let (rect, _) =
    ...
    // `vike_app_core::dom_math::default_price`'s table value (62 800.00 for BTCUSDT) whenever no
    ...
    let (ptxt, pcol) = match last {
        Some(p) => (fmt_px(p, 0.5), LAST),
        None => ("—".to_string(), FAINT),
    };
    child.label(egui::RichText::new(ptxt).monospace().size(13.0).color(pcol)).on_hover_text(
        if last.is_some() {
...
/// The source strip's glance word for one parsed link state, plus its colour.
///
/// ⚠ **Deliberately NOT the `LIVE`/`PAPER` vocabulary the header badge uses.** That badge is about
...
fn source_word(state: vike_model::feed_status::ConnectionState) -> (&'static str, Color32) {
    use vike_model::feed_status::ConnectionState as C;
    match state {
        C::Connected => ("FEED UP", BID),
        C::Connecting => ("FEED DIALLING", LAST),
        C::Disconnected => ("FEED DOWN", ASK),
        C::Error => ("FEED FAULT", ASK),
        C::Unknown => ("FEED ?", MUTED),
    }
}
...
/// venue.
fn source_strip(ui: &mut egui::Ui, source: &str) {
    let (rect, _) =
        ui.allocate_exact_size(Vec2::new(ui.available_width(), SOURCE_H), Sense::hover());
    ui.painter().rect_filled(rect, 0.0, PANEL);
    ui.painter().line_segment([rect.left_bottom(), rect.right_bottom()], Stroke::new(1.0, RULE));
    let mut child = ui.new_child(
        egui::UiBuilder::new()
            .max_rect(rect.shrink2(Vec2::new(8.0, 1.0)))
            .layout(egui::Layout::left_to_right(egui::Align::Center)),
    );
    child.spacing_mut().item_spacing.x = 5.0;
    let trimmed = source.trim();
    let (word, col) = source_word(vike_model::feed_status::parse_feed_status(trimmed));
    child.label(egui::RichText::new("depth").monospace().size(10.0).color(FAINT));
    child.label(egui::RichText::new(word).monospace().size(10.0).color(col));
    let detail =
        if trimmed.is_empty() { "source unknown".to_string() } else { trimmed.to_string() };
    child.label(egui::RichText::new(detail).monospace().size(10.0).color(MUTED)).on_hover_text(
...
fn no_book(ui: &mut egui::Ui, region: Rect, absence: Option<BookAbsence<'_>>) {
    // A dashed-looking inset frame: unmistakably NOT the ladder's own painted grid.
    ui.painter().rect_stroke(region.shrink(6.0), 3.0, Stroke::new(1.0, RULE), StrokeKind::Inside);
    ...
    child.label(egui::RichText::new(headline).monospace().size(13.0).color(ASK));
    child.label(egui::RichText::new(NO_BOOK_SUBLINE).monospace().size(10.5).color(MUTED));
    if !a.cause.is_empty() {
        child.label(egui::RichText::new(a.cause).monospace().size(10.5).color(TXT));
    }
    if !a.next_step.is_empty() {
        child.label(egui::RichText::new(a.next_step).monospace().size(10.5).color(FAINT));
    }
}
