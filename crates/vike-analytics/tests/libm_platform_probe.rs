...
//! the day the SOURCE half joined it. Two tests below gate, neither of them `#[ignore]`d:
//! [`converted_functions_are_platform_invariant`] (the hashes agree across platforms) and
//! [`production_code_calls_libm_not_the_platform`] (no platform spelling in `src/`, which is what
//! covers the sites the corpus never drives). ⚠ There was a THIRD — a planted-fixture proof
//! that the second one's notion of "production" is not silently eating the file — and it did not
//! go away, it MOVED: decision 0074 took the text parser these gates share into
//! `vike_model::libm_walk`, and the fixture went with it to
...
/// same `break` was actively wrong is `vike-mm`, whose in-`src` gate carries the measurement.
/// `crates/vike-mm/src/platform_probe.rs`'s `cfg_test_ranges` is the same SHARED function this
/// gate calls since 0074 — that file used to hold a private copy, and its doc records what that
/// copy's `break` cost: 1,612 production lines discarded across five files, two `libm` sites
/// among them, and a module doc that had copied the resulting undercount into prose.
/// `vike_model::libm_walk::cfg_test_ranges`
/// is the shared shape — shared literally since decision 0074, where it had been one of eleven
/// private copies — and its non-vacuity arm is a planted FIXTURE,
/// `crates/vike-model/tests/libm_walk_selftest.rs`, rather than a tree measurement, precisely
/// because THIS tree cannot exercise the case: with no mid-file test module anywhere under this
/// crate's `src/` (measured just above), a tree-shaped assertion — "some production line below a
/// marker was scanned" — would be FALSE here, and could only be written by weakening it into
...
///
/// # The per-crate evidence for the SHARED banned list, harvested here when its copy left
///
/// The banned names, and the two spellings each one renders into, now live once — in
/// `vike_model::libm_walk::BANNED_FNS` and `vike_model::libm_walk::banned_needles` (decision
/// 0074, which overturned a nine-file refusal to share them). That module carries the GENERAL
/// argument: why `sqrt` and `to_degrees`/`to_radians` are absent, why `powi` is present, why the
/// UFCS half is not decoration. What it deliberately does NOT carry is everything below, because
/// each line of it is a fact about `crates/vike-analytics/src` rather than about the parser —
/// and each was measured, so it would have died with the copy.
///
/// ⚠ **Widening `.powi(` from the square to EVERY arity cost this crate nothing:** its one
/// `.powi(` sits inside a `#[cfg(test)]` module. So the widening bought the next edit, which has
/// no such luck. The cure here is a MULTIPLICATION and never `libm` — the operation is already
/// correctly rounded, and the problem was only ever that the compiler is free to reach it
/// through a libcall.
///
/// ⚠ **The inverse-trig and hyperbolic family (`atan` .. `hypot`) joined at the same zero cost:**
/// `crates/vike-analytics/src` contains not one call to any of the nine today, so that widening
/// changed no verdict — it removed a gap the next edit could walk into rather than closing a leak
/// that was moving numbers. ⚠ Of the nine, exactly ONE is MEASURED rather than argued, and it is
/// measured in THIS file: Part A's `atan/ols-slope` row sweeps `f64::atan` against `libm::atan`
/// and reports the divergence. The other eight rest on the IEEE 754 argument alone, which is the
/// same argument the original thirteen names rest on for every domain Part A does not sweep
/// either.
///
/// ⚠ **The 2026-08-26 widening from 22 names to 26** — `asinh`, `acosh`, `atanh`, `sin_cos` —
/// likewise changed no verdict here: none of the four appears anywhere under this crate's `src/`.
///
/// ⚠ **This crate's answer to the three spellings the needle list cannot see.**
/// `vike_model::libm_walk::banned_needles` states that blind spot and leaves the answer to each
/// gate, because the answer is a per-crate fact. The three are `<f64>::exp(x)` (what appears is
/// `f64>::exp(`, and no needle ends in `>`), a call reached through a generic bound (`T: Float`,
/// `num_traits::Float::exp`), and a call split across two lines by a `max_width` wrap. **None of
/// the three occurs under this crate's `src/` today.** A list of spellings is an enumeration, and
/// an enumeration is never a proof.
///
/// ⚠ **The indentation assumption the range walk rests on, MEASURED rather than assumed.**
/// 2026-08-26: all 52 `#[cfg(test)]` markers under the three gated `src/` trees — this crate's,
/// `vike-indicators`' and `vike-mm`'s — sat at column 0, and each was followed either by a line
/// ending in `{` or by a `;` (the `mod NAME;` declaration form, which exists in
