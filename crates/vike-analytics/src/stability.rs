...
//!
//! ⚠ **"Port" means PORTED FROM, not bit-identical to CPython.** [`parameter_stability`]'s mean is
//! a naive `.sum::<f64>()` where the Python twin writes builtin `sum()`, which the root
//! `CLAUDE.md` would send to `vike_model::py_sum`. It stays naive for the three reasons
//! `crates/vike-analytics/src/overfit.rs`'s module doc sets out in full — the oracle is retired,
//! nothing compares this crate to Python (its fixture was deleted as an orphan), and switching
//! would move every number with no test to catch a mistake. `crates/vike-ops/tests/parity_fold_gate.rs`
//! pins the count so it cannot grow quietly.
//!
