...
pub enum JournalRecord {
    ...
    /// re-applied on replay (`crate::replay`'s tail lowers it as `Command::Order(Submit(req))`), so
    ...
    ///
    /// ⚠ **Recorded and not yet consumed**: `crate::replay::replay_journal` refuses a multi-engine
...
mod tests {
    ...
    fn margin_call_liquidate_record_roundtrips() {
        ...
        match &recs[0] {
            JournalRecord::MarginCallLiquidate {
                seq: got, now_ms, req: got_req, mount_id, ..
            } => {
                assert_eq!(*got, seq);
                assert_eq!(*now_ms, 1_000);
    ...
    fn a_pre_v16_write_ahead_frame_reads_back_as_the_sole_account() {
        ...
        match serde_json::from_value::<JournalRecord>(minted)
            .expect("a v15 frame is a valid v16 one")
        {
