...
pub(crate) fn frame_begin(app: &mut App, ui: &mut egui::Ui) -> (egui::Context, FrameSnapshot) {
        ...
        && ctx.input_mut(|i| i.consume_key(egui::Modifiers::ALT, egui::Key::L))
    {
        ...
        if let Some(id) = target_id {
            if let Some(w) = app.wins.iter_mut().find(|w| {
                w.kind == workspace::WinKind::Chart && w.open && !w.minimized && w.id == id
            }) {
                w.scale =
                    if w.scale == ScaleMode::Log { ScaleMode::Linear } else { ScaleMode::Log };
            }
        }
    }
...
pub(crate) fn draw_chrome(app: &mut App, ui: &mut egui::Ui, ctx: egui::Context) {
    ...
    if menu.new_window {
        ...
        // `crates/vike-app/src/chart_window.rs`'s symbol-picker quick-pick list, so it is
        // vike-app's constant, not the planner's. Only the cascade slot and the `WinState`
    ...
    egui::Panel::bottom("statusbar")
        ...
        .show(ui, |ui| {
            ...
            if dismissed {
                if let Some(ctrl) = app.remote_ctrl() {
...
pub(crate) fn draw_windows(app: &mut App, ctx: egui::Context, snap: FrameSnapshot) {
    ...
    if let Some(dispatch) = dispatch {
        ...
        // hardcoded `("binance", "BTCUSDT")`. Byte-identical on the local FAT build (that IS the
        // primary `vike_run::build_node` mounts), and CORRECT for a control-enabled `--observe`
        // client, whose remote daemon's primary is polymarket/hyperliquid — where the Trade card
        // rendered `snap.symbol` while the BUY button submitted binance/BTCUSDT.
    ...
    // Data Manager "Stored" view (Task 3): per-series Delete — a FRESH store handle (mirrors
    // `open_local_hist_store`'s own-tokio-runtime cost model; the Stored view doesn't keep a
    // handle open between actions), then a forced reload so the tree reflects the removal.
    // `delete_series` is idempotent/irreversible; the GUI has already confirmed by this point
    // (see `tool_views::stored_tool_content`'s modal — the ONE deferred-action site it feeds).
    if !stored_deletes.is_empty() {
        ...
        for (venue, symbol, kind, interval) in stored_deletes {
            ...
            match open_local_hist_store() {
                Ok(store) => {
                    let id = vike_data::SeriesId::per_symbol(kind, venue, symbol, interval);
                    if let Err(e) = store.delete_series(&id) {
                        tracing::warn!("Stored delete {id:?} failed: {e}");
                    }
                }
                Err(e) => tracing::warn!("Stored delete: open store failed: {e}"),
            }
            // THIN build: a thin grid only ever LOADS in remote mode, where the gate above
            // refuses — a local-mode thin delete could only come from an empty grid, i.e.
            // never; the binding keeps the arm compiling.
            #[cfg(not(feature = "fat"))]
