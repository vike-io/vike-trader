...
fn main() -> eframe::Result<()> {
    ...
    // `App::new`'s observe branch). The SCAN stays here (argv is the binary's own fact — the arg
    // immediately after `--observe` is the addr); the LADDER it feeds is
    // `vike_app_core::backend_conn::startup_backend`, whose doc carries every rung and the reason
    // there is no "not connected" outcome to represent.
    let observe_backend: Option<vike_app_core::backend_registry::BackendRecord> = {
        let mut args = std::env::args();
        let mut found = None;
        while let Some(a) = args.next() {
            if a == "--observe" {
                // A bare `--observe` in a thin build is not an error: that build can do nothing
                // else, so the word alone means "the configured node", and a blank flag is not a
                // rung — the ladder falls through to the registry.
                found = args.next();
                break;
            }
        }
        Some(vike_app_core::backend_conn::startup_backend(found.as_deref()))
    };
    ...
    // local-core arm is compiled out and it must have an address — but `observe_addr` above now
    // always yields one (flag, else the active backend, else the local default), so the arm it
    ...
    // act on; `exit(2)` before a window existed was not.
