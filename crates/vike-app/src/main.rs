...
// `reconcile_config::reconcile_gate` — ON by DEFAULT since S2 once this mount arms a live venue
// account (see the `recon_driver` block after `spawn_core_multi`, below); this binary
...
struct App {
    ...
    /// Reconcile driver: `Some` once `vike_core::spawn_recon` is mounted — the S2 gate said yes
    /// (`vike_ops::reconcile_config::reconcile_gate`: ON by DEFAULT once this mount arms a live
    /// venue account) AND at least one live venue's `ReconClient` was built (see
    /// `build_recon_client`). `None` on a paper mount, in `--observe`, and whenever the operator
    /// refused it with `flags.reconcile_off`. Shut down in `on_exit`, alongside `forwarder_stop`:
...
// `vike_ops::reconcile_config::reconcile_enabled` — a function this binary no longer calls. It was
// also never a gate: this file is outside CI's crate roster, so its `#[cfg(test)]` modules are
// compiled and never RUN. Since S2 the mount's condition is
// `vike_ops::reconcile_config::reconcile_gate` over `flags.reconcile` / `flags.reconcile_off` and
// the armed-live COUNT, and every property it has lives in three places that DO run:
// `vike_ops::reconcile_config`'s own gate tests (the truth table, the quarantine pairing, and the
// empty auto-apply set), `vike_config::flags`' `env_turns_a_flag_on_and_off` /
// `a_truthy_typo_is_an_error_not_a_silent_false`, and `vike-tradehub`'s
// `a_live_armed_mount_reconciles_with_nothing_set_and_a_paper_one_does_not`, which composes the
// same gate over the REAL `vike_run::armed_live_venues` probe this file calls.
// ⚠ The residual is unchanged and is the reason the daemon carries the composed test: nothing
// executes THIS root's wiring, so "the GUI calls the gate with the armed set" rests on review.
...
impl App {
    ...
    fn new(
        ...
        let (
            ...
            // always takes the observe arm above — `vike_app_core::split_plane::observes` is
            // unconditionally true there, so `observe_backend` is always `Some` — and the `not(fat)`
            // twin below is genuinely unreachable. ⚠ In a FAT build this arm is the ORDINARY one: it
            // runs whenever nobody asked to observe, which is the whole of #1610's regression.
            #[cfg(feature = "fat")]
            {
                ...
                // `recon_env` — the REAL process env, NOT `vars` (the credential map `make_engine` uses;
                // see `reconcile_config`'s module doc for why a shell-exported flag is invisible to it) —
                // supplies the whole `VIKE_RECONCILE_*` family to `build_recon_config` below, including the
                ...
                //
                // ⚠ IT GOES THROUGH `quarantine_first_default`, AND UNTIL S2 IT DID NOT. `vike-tradehub`
                // had folded that policy default in since the daemon's reconcile landed; this root built
                // its `ReconConfig` off a raw sweep, so a GUI operator who armed reconciliation got
                // `hybrid` — the policy that AUTO-APPLIES `PositionDrift`, rewriting local position size
                // onto the venue's number and booking realized PnL at the venue's average price with no
                // operator in front of it. One home, both roots.
                let recon_env: HashMap<String, String> =
                    reconcile_config::quarantine_first_default(std::env::vars().collect());
                let flags = settings().flags;

                // The per-venue ARMING CEILING and this box's armed-live set — computed HERE, before the
                // reconcile gate, because the S2 default keys off it. ⚠ ONE call, THREE consumers: the
                // reconcile gate below, the B11 live-account lock claim further down (whose own comment
                // block carries why the claim must precede `build_node`), and `build_node`'s ceilings.
                // Asking twice would let the set this process LOCKS and the set it reconciles disagree.
                let mount_policy = vike_run::MountPolicy::from(&settings().policy);
                let armed_live = vike_run::armed_live_venues(&vars, &mount_policy);

                // THE RECONCILE MASTER GATE (S2) — ON by DEFAULT for a mount that arms a live venue
                // account, paired with the `quarantine` policy folded in above so it folds NOTHING. The
                // decision is `vike_ops::reconcile_config::reconcile_gate`, called identically by
                // `vike-tradehub`'s `live_mount_with`: **the default is the same in the GUI and in the
                // daemon, on purpose**. This binary mounts the same twelve venues with the same
                // credentials and signs the same orders, so a GUI that restarted without asking its
                // venues what they held would be trading against a belief in exactly the way the daemon
                // was. A default that silently differed per binary would be worse than either answer.
                //
                // ⚠ From here a live GUI issues AUTHENTICATED READ calls against every armed account at
                // startup and then every `VIKE_RECONCILE_INTERVAL_MS` (default 60 s) — including, on a box
                // that arms aster, MAINNET reads, since only LIVE credentials are configured for that
                // venue (its testnet exists and is routed; the store simply holds no testnet keys).
                // `reconcile_gate`'s own doc carries the argument and the three ways to refuse;
                // `flags.reconcile_off` / `VIKE_RECONCILE_OFF=1` is the one that keeps the venue armed.
                let recon_gate = reconcile_config::reconcile_gate(
                    flags.reconcile,
                    flags.reconcile_off,
                    armed_live.len(),
                );
                ...
                //                             exist) behind `reconcile_config::reconcile_gate`; ON by
                //                             DEFAULT for a live mount since S2. `recon_clients`
                ...
                //
                // ⚠ THE GATE IS `recon_enabled` — the S2 verdict (`reconcile_config::reconcile_gate` over
                // `flags.reconcile`/`flags.reconcile_off` and the armed-live count), the SAME one the
                // `recon_driver` mount further below reads. It used to
                            ...
                            Err(e) => {
                                tracing::error!(
                                    ...
                                    "journal cross-check DEGRADED to a two-way reconcile: the hist store \
                                     ...
                                     MissingFill instead of JournalDivergence — HELD for you under the \
                                     default `quarantine` policy, and auto-applied with no operator in \
                                     front of it if this mount named `hybrid` in \
                ...
                //
                // ⚠ `mount_policy`/`armed_live` are the bindings computed ABOVE, beside the reconcile
                // gate — not a second probe. The claim still happens HERE, before `build_node`, which is
                // the property this comment block is about and the one
                // `crates/vike-ops/tests/live_lock_claim_order_gate.rs` measures; only the pure
                // computation moved earlier, so the set that is LOCKED and the set that is RECONCILED
                ...
                let mut node_cfg = vike_run::NodeConfig {
                    ...
                    core_config: vike_core::CoreConfig {
                        seed_cash: 10_000.0,
                        // This deployment's origin claim (`config.instance_origin`), stamped into
                        // the client order id of every order this core mints so a SECOND instance
                        // on the same venue account is recognisable on reconcile. `None` — the
                        // default — is byte-identical to before the key existed.
                        instance_origin: settings().config.instance_origin.clone(),
                ...
                // Reconcile driver mount: gated on `recon_enabled` — the S2 verdict resolved up above,
                // ON by DEFAULT once this mount arms a live venue account. Its `recon_env` is read from the
                // REAL process env, deliberately NOT `vars` (the workspace `.env` file parse `make_engine`
                // uses for venue credentials, above); see `reconcile_config`'s module doc for why a
                // shell-exported `VIKE_RECONCILE_*` knob would be invisible to `load_workspace_dotenv()`.
                ...
                // latter isn't captured until `binance_feed` exists, above. On a PAPER mount `recon_enabled`
                // reads false, so `recon_driver` stays `None` and NOTHING below runs — byte-identical to the
                // pre-reconcile GUI. When enabled but every venue is paper
                ...
                let recon_driver = if recon_enabled {
                    if recon_clients.is_empty() {
                        // ⚠ WARN, not info, and reachable in a new way since S2: the armed-live probe
                        // is INTENT-based, so a venue whose factory declines a present-but-bad key turns
                        // the default gate on and then hands back no client. "reconcile is on" and
                        // "reconcile reached a venue" are different facts.
                        tracing::warn!(
                            "reconcile is ON but no venue produced a ReconClient (every armed \
                             venue fell back to paper); the driver is inert and NOTHING is being \
                             reconciled"
                        );
                        None
                    } else {
            ...
            {
                unreachable!(
                    "thin build has no local trading core — `split_plane::observes` is \
                     unconditionally true without the `fat` feature, so `observe_backend` is \
                     always Some and the observer arm always wins"
                )
            }
        };
        ...
        let materializer = if observe_backend.is_some() {
            None
        } else {
            #[cfg(feature = "fat")]
            {
...
fn main() -> eframe::Result<()> {
    ...
    // `--observe [ADDR]`: the read-only observer arms — watch a headless `vike-tradehub` daemon's
    // observe server instead of spawning a local trading core (see `App::new`'s observe branch).
    ...
    // ⚠ THE MODE, and the only thing that decides it: `vike_app_core::split_plane::observes` —
    // `Some` puts `App::new` in the OBSERVER arm (no local core, read-only order path), so it must
    // answer "was observing REQUESTED", never "did an address resolve". The ladder above always
    // resolves one, by design; wrapping THAT in `Some` is #1610's regression, which cost this app
    // its local trading core for three days. A thin build always observes — it has no core.
    let observe_backend: Option<vike_app_core::backend_registry::BackendRecord> =
        vike_app_core::split_plane::observes(cfg!(feature = "fat"), startup.requested)
    ...
    // ⚠ A thin build no longer REFUSES to start. It has no local trading core, so `App::new`'s
    // local-core arm is compiled out and it must have an address — and in THAT build `observes`
    // above is unconditionally true, so `observe_addr` always yields one (flag, else the active
    // backend, else the local default) and the arm it could have fallen into is unreachable by
    // construction rather than by an early exit. The window opens and the status bar says whether
    // it connected, which is the thing a person can act on; `exit(2)` before a window existed was
    // not. ⚠ In a FAT build `observe_addr` is `None` on an ordinary launch — that is the local-core
    // arm, and the title below then says "live" rather than naming a node nobody asked to watch.
