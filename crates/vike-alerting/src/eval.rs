    ...
    fn recon_alert_rule_fires_on_presence_edge_with_optional_kind_filter() {
        ...
        with_alert.recon = ReconBlock {
            alerts: vec![ReconAlertView {
                ...
                // A venue with one account: the account label IS the exchange id. This rule keys
                // on PRESENCE and (optionally) on `kind`, never on the account.
                account: "binance".into(),
            }],
            last_pass_ts: 100,
        };
