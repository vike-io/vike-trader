...
fn maybe_fire(
    ...
    if rule.cooldown_ms > 0 {
        if let Some(last) = st.last_fired_ms {
...
mod tests {
    ...
    fn price_above_fires_only_on_the_upward_crossing() {
        ...
        assert!(eval_snapshot_rule(&rule, &mut st, &snap_with_mark("binance", "BTCUSDT", 99.0), 1)
            .is_none());
        // Crosses up 99 -> 101: fires once.
        assert!(eval_snapshot_rule(
            &rule,
            &mut st,
            &snap_with_mark("binance", "BTCUSDT", 101.0),
            2
        )
        .is_some());
        ...
        assert!(eval_snapshot_rule(
            &rule,
            &mut st,
            &snap_with_mark("binance", "BTCUSDT", 102.0),
            3
        )
        .is_none());
        // Drops back under then crosses up again: re-arms and fires.
        assert!(eval_snapshot_rule(&rule, &mut st, &snap_with_mark("binance", "BTCUSDT", 98.0), 4)
            .is_none());
        assert!(eval_snapshot_rule(
            &rule,
            &mut st,
            &snap_with_mark("binance", "BTCUSDT", 105.0),
            5
        )
        .is_some());
    }
    ...
    fn price_below_fires_on_the_downward_crossing_and_ignores_the_wrong_symbol() {
        ...
        assert!(eval_snapshot_rule(&rule, &mut st, &snap_with_mark("binance", "ETHUSDT", 1.0), 1)
            .is_none());
        ...
        .is_none());
        assert!(eval_snapshot_rule(&rule, &mut st, &snap_with_mark("binance", "BTCUSDT", 99.0), 3)
            .is_some());
    }
    ...
    fn fill_rule_scoping_fires_only_for_the_matching_instrument() {
        ...
        assert!(eval_event_rule(&any, &mut RuleState::default(), &fill_event("okx", "SOLUSDT"), 4)
            .is_some());
    ...
    fn order_rejected_rule_fires_on_both_reject_and_deny() {
        ...
        assert!(eval_event_rule(
            &rule,
            &mut RuleState::default(),
            &fill_event("binance", "BTCUSDT"),
            3
        )
        .is_none());
    }
