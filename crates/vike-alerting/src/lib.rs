...
//! [`RuleTrigger::SeriesStale`] and [`RuleTrigger::SeriesSlow`] with default features on (see
//! `vike_recorder::alerts`).
//!
...
//!   decorator is where its only bound lives). It also owns [`FiredAlert`], the sink payload: that
//!   is the ONE type both halves share, so it lives in the default half where a vike-free consumer
//!   can build and deliver one.
//!
...
pub use delivery::{
    ...
    QueuedSinkStop, STOP_POLL, UreqTransport, WebhookConfig, WebhookKind, WebhookSink,
    WebhookTransport, queued_ureq_webhook_sinks, ureq_webhook_sinks, webhook_configs_from_env,
};
