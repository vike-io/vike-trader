...
use vike_model::{
    BookLevel, Broker, Fill, HftBroker, L2Book, QuoteTick, Strategy, SymbolProperties, TradeTick,
};
...
fn snap(ts: i64, seq: u64, bids: Vec<BookLevel>, asks: Vec<BookLevel>) -> Tick {
...
fn delta(ts: i64, seq: u64, bids: Vec<BookLevel>, asks: Vec<BookLevel>) -> Tick {
    Tick::Book(vike_model::BookUpdate {
        ts,
        local_ts: 0,
        seq,
...
fn none_keeps_touch_equals_fill_and_risk_adverse_withholds_it() {
    ...
    let ticks = || {
        vec![
            snap(1, 1, vec![BookLevel::new(99.0, 10.0)], vec![BookLevel::new(101.0, 5.0)]),
            tr(2, 99.0, 0.5),
        ]
    };
...
fn partial_fills_account_only_the_excess_beyond_the_front() {
    ...
    let ticks = vec![
        snap(1, 1, vec![BookLevel::new(99.0, 5.0)], vec![BookLevel::new(101.0, 5.0)]),
        tr(2, 99.0, 7.0),
        tr(3, 99.0, 1.0),
        tr(4, 99.0, 5.0),
    ];
...
fn a_trade_through_the_level_fills_in_full_despite_the_front() {
    let ticks = vec![
        snap(1, 1, vec![BookLevel::new(99.0, 50.0)], vec![BookLevel::new(101.0, 5.0)]),
...
fn depth_cancellation_clears_the_front_then_a_quote_touch_fills() {
    let with_cancel = vec![
        ...
        delta(3, 2, vec![BookLevel::new(99.0, 0.0)], vec![]), // the level ahead cancels → front clamps to 0
...
fn prob_power_model_reestimates_front_on_depth_decrease_then_trade_excess_fills() {
    ...
    let ticks = vec![
        ...
        delta(3, 2, vec![BookLevel::new(99.0, 2.0)], vec![]),
        tr(4, 99.0, 3.0),
    ];
    let (eng, fills) =
...
fn a_tag_repriced_mid_pass_is_gated_at_its_new_level() {
    ...
    let fills = Rc::clone(&strategy.fills);
    let ticks = vec![
        snap(1, 1, vec![BookLevel::new(100.0, 8.0)], vec![BookLevel::new(100.0, 1.0)]),
        tr(2, 100.0, 3.0),
    ];
    let params =
        EngineParams { queue_model: Some(QueueModelKind::RiskAdverse), ..Default::default() };
...
fn tick_fill_model_quote_triggering_composes_with_the_queue_gate() {
    ...
    let ticks = || {
        vec![
            ...
            delta(3, 2, vec![BookLevel::new(99.0, 0.0)], vec![]),
...
fn a_book_integrity_break_leaves_states_alone_then_min_clamps_on_re_anchor() {
    ...
    let ticks = vec![
        ...
        delta(3, 9, vec![BookLevel::new(99.0, 4.0)], vec![]), // seq 9 after 1 → integrity break, book dropped
        tr(4, 99.0, 3.0), // bookless: front 8 → 5, still no fill
        snap(5, 10, vec![BookLevel::new(99.0, 1.0)], vec![BookLevel::new(101.0, 5.0)]), // re-anchor: min-clamp 5 → 1
