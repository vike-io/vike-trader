...
use vike_model::{
    BookLevel, BookUpdate, BookUpdateKind, Fill, HftBroker, L2Book, QuoteTick, Strategy, TradeTick,
};
...
fn snap(ts: i64, seq: u64, bid: f64, ask: f64) -> Tick {
    Tick::Book(BookUpdate {
        ...
        tick_size: 0.01,
        bids: vec![BookLevel::new(bid, 100.0)],
        asks: vec![BookLevel::new(ask, 100.0)],
        symbol: SYM.to_string(),
    })
}
