...
use vike_backtest::{
    AlmgrenChriss, EngineParams, ImpactInputs, ImpactModel, SimBroker, StrategyEngine,
    adverse_fill_price, broker_sim::MIN_ADVERSE_FACTOR, window_stats,
};
...
fn an_unmeasurable_window_falls_back_to_flat_slippage() {
    ...
    // ...and the fallback is COUNTED. Falling back is correct here; being silent about it is not.
    assert_eq!(e.core.impact_unpriced, 1, "the skipped charge must be counted, not just taken");
}

/// A ZERO-VOLUME series disables a configured impact model at every coefficient — silently, before
/// this counter existed.
///
/// This is the shape measured on a real the CI box store on 2026-09-07: two sweeps differing only in
/// `[engine.impact]` came back bit-identical, and raising the coefficients 5,000x moved nothing,
/// because every bar carried `volume = 0`. `crate::impact::measure_series` answers `None` on a
/// non-positive mean size, so the context is unbuildable however long the window gets.
///
/// ⚠ The second half is the anti-vacuity control and it is what makes the first half mean
/// something: the SAME fixture with real volume must count ZERO. Without it this test would pass
/// just as well if the counter incremented unconditionally.
#[test]
fn a_zero_volume_series_leaves_a_configured_impact_model_unpriced_and_says_so() {
    let count_for = |volume: f64| {
        let bars = vec![(SYM.to_string(), mk_bars(SYM, N_BARS, volume))];
        let params = EngineParams {
            cash: 100_000_000.0,
            slippage: 0.002,
            impact: Some(Arc::new(AlmgrenChriss::default())),
        ...
        let mut e = StrategyEngine::new(bars, strat, params);
        let res = e.run();
        // The broker's counter and the mirror onto the result must agree — the mirror is the half
        // an operator can actually read, and a diagnostic that reaches nobody was the whole defect.
        assert_eq!(e.core.impact_unpriced, res.impact_unpriced, "broker and result must agree");
        res.impact_unpriced
    };

    assert!(count_for(0.0) > 0, "a zero-volume series cannot price impact, and must say so");
    assert_eq!(count_for(VOL), 0, "with real volume nothing is skipped — the anti-vacuity control");
}
...
impl ImpactModel for FixedImpact {
    /// The REQUIRED method is the terms-aware one (see `vike_backtest::ImpactModel`), and this
    /// double deliberately ignores `terms`: what it exists to drive is the engine's behaviour at
    /// an oversized estimate, and making that depend on which lane asked would make the saturation
    /// tests below lane-sensitive for no reason. The lane split's own arithmetic is gated in
    /// `crates/vike-backtest/src/impact.rs` and `crates/vike-backtest/tests/fills/impact_lanes.rs`.
    fn impact_frac_for(&self, _input: &ImpactInputs, _terms: vike_backtest::ImpactTerms) -> f64 {
        self.0
    }
}
