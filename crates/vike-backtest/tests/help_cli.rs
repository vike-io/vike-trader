...
/// ⚠ **The usage text names the section by the name the profile FIELD has**, and it did not.
///
/// Ruling R2 renamed the parameter-search section `[sweep]` → `[paramscan]`
/// (`crates/vike-backtest/src/harness/profile.rs`'s `BacktestProfile::paramscan`), and this
/// binary's own `--help` went on telling operators to write `[sweep]` — deliberately at the time,
/// because the rename's operator-facing scope was `vike-cli`. A help text that names the OLD
/// spelling of a section is how an operator learns the old spelling, so the exclusion was
/// withdrawn and this is what holds it.
///
/// ⚠ **It asserts nothing about what the binary ACCEPTS**, and must not: `[sweep]` is a PERMANENT
/// serde alias and every profile written with it still loads —
/// `crates/vike-backtest/tests/search_persist_cli.rs`'s fixture is spelled that way on purpose so
/// the alias is exercised rather than merely promised. The help text mentioning the alias by name
/// is therefore correct and is not what this gate forbids; SPELLING THE SECTION AS `[sweep]` while
/// never naming `[paramscan]` is.
#[test]
fn the_usage_names_the_paramscan_section_rather_than_its_old_spelling() {
    let stdout = String::from_utf8_lossy(&run(&["--help"]).stdout).to_string();
    assert!(
        stdout.contains("[paramscan]"),
        "`backtest --help` must name the section as the profile field spells it: {stdout:?}"
    );
    assert!(
        stdout.contains("alias"),
        "…and must still tell an operator that their existing [sweep] profiles keep working, \
         because the alias is permanent and a silent rename reads as a breaking change: {stdout:?}"
    );
}
