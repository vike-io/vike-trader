...
use vike_model::{BookLevel, BookUpdate, BookUpdateKind, L2Book, Strategy};
...
fn bu(ts: i64, seq: u64, kind: BookUpdateKind, bids: Vec<BookLevel>, asks: Vec<BookLevel>) -> Tick {
...
fn book_fold_applies_anchors_deltas_and_drops_on_gap() {
    let ticks = vec![
        ...
        bu(2, 2, BookUpdateKind::Delta, vec![BookLevel::new(0.46, 3.0)], vec![]), // best bid moves to 0.46
        bu(3, 4, BookUpdateKind::Delta, vec![BookLevel::new(0.48, 1.0)], vec![]), // seq GAP (3 missing) → dropped
        bu(4, 5, BookUpdateKind::Delta, vec![BookLevel::new(0.49, 1.0)], vec![]), // still no book → ignored
        bu(
            5,
            6,
            BookUpdateKind::Snapshot,
            vec![BookLevel::new(0.44, 2.0)],
            vec![BookLevel::new(0.50, 2.0)],
        ), // re-anchor
    ];
...
fn gap_start_marker_drops_book_until_snapshot() {
    let ticks = vec![
        ...
        bu(3, 2, BookUpdateKind::Delta, vec![BookLevel::new(0.46, 3.0)], vec![]), // after gap marker → ignored
        bu(4, 0, BookUpdateKind::LiveResume, vec![], vec![]),
        bu(
            5,
            3,
            BookUpdateKind::Snapshot,
            vec![BookLevel::new(0.40, 1.0)],
            vec![BookLevel::new(0.60, 1.0)],
        ),
    ];
    let strategy = BookSpy::default();
    let tops = Rc::clone(&strategy.tops);
