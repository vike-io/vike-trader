...
use vike_backtest::harness::{BacktestProfile, BacktestReport, HarnessError, expand_paramscan};
...
/// against `expand_paramscan`'s own expansion of the same profile rather than against a hand-written
...
fn every_searched_window_records_a_point_of_the_grid_it_searched() {
    let p = profile(GRID);
    let obj = total_return_objective();
    let points = expand_paramscan(&p).expect("the grid expands");
...
/// degenerated into the control: `expand_paramscan` answers a table-less profile with ONE candidate, so
...
/// SEARCH, not the profile. The empty-table spelling is covered too — `is_paramscan()` reads a present
...
fn a_grid_search_over_a_profile_with_no_sweep_table_is_refused_by_the_driver_too() {
    ...
    // A present-but-EMPTY table is the same refusal: `is_paramscan()` reads it as "not a sweep".
    let p = profile("[sweep]\n");
    let err = optimized_with(&p, WindowSearch::Sweep, WalkMode::Anchored, &obj).unwrap_err();
