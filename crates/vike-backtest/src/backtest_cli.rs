...
pub fn run(
    ...
    if archive.is_some() && arg(args, "--store").is_some() {
        eprintln!(
    ...
    // NONE when the run reads an archive: there is no `DataFusionHist` behind it, and the two
    // inherent-method consumers below take the absence as the fact it is.
    let concrete = match &archive {
        Some(_) => None,
        None => match DataFusionHist::open(&root) {
            Ok(h) => Some(h),
            Err(e) => {
    ...
    let concrete = concrete.map(Arc::new);
    ...
    if profile.data.explain {
        let rung = (resolved_root.rung.as_str(), resolved_root.rung.why());
    ...
    if profile.is_paramscan() {
        ...
        let (store_data, search_data, input_addr) =
            match collect_data_fingerprint(concrete.as_deref(), &profile, &root) {
                Ok(fp) => {
    ...
    let data_fingerprint = match collect_data_fingerprint(concrete.as_deref(), &profile, &root) {
        Ok(fp) => Some(fp),
        Err(why) => {
...
fn collect_data_fingerprint(
    store: Option<&DataFusionHist>,
    profile: &BacktestProfile,
    store_root: &Path,
) -> Result<run_fingerprint::DataFingerprint, String> {
    // ⚠ `Option`, because `--archive` opens an `ArchiveParquetHistStore` and there is no
    // `DataFusionHist` to ask. `series_facts` — the per-series coverage + part-file list this
    // fingerprint is made of — is INHERENT to that type rather than a `HistStore` method, so an
    // archive run genuinely cannot be witnessed this way.
    //
    ...
    let Some(store) = store else {
        return Err("the run reads archive Parquet in place (--archive), and a data fingerprint \
                    needs the part-file list only a DataFusionHist store can report"
            .to_string());
    };
...
fn run_serve(
    ...
    // be silently skipped.
    // ⚠ THROUGH THE SETTINGS SOURCE — this arm is a SIXTH, UNDECLARED composition root (it walks
    // for its own settings directory and opens a socket), and it is invisible to
    // `crates/vike-boot/tests/one_owner.rs` only because that gate's `production_half` truncates
    // this file at its `#[cfg(test)]` module, roughly 1500 lines above this call. The hole is real
    // and this is what fell into it: a store-blind `vike_config::load` on a box that has run
    // `vike-cli config adopt` reads `config.*` out of files nothing resolves from.
    ...
    // **`preferences.sweep_threads` reaches the sweep pool from HERE** — the composition root's one
    // job for that key. The value is the loader's resolved answer (`env > file > default`), and
    ...
    match crate::compute_server::serve_authed(
        listener,
        Arc::new(store),
        studio,
        study,
...
"#;
    ...
    fn a_series_the_store_does_not_hold_is_recorded_as_missing_rather_than_as_all_zeros() {
        ...
        let fp = collect_data_fingerprint(Some(&store), &profile, &root).unwrap();
