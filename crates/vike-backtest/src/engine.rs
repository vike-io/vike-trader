...
pub use sim_broker::{
    DEFAULT_IMPACT_WINDOW, EngineParams, EquitySampling, FillModelKind, MirrorFill, MirrorFunding,
...
impl<S: Strategy<SimBroker>> StrategyEngine<S> {
    pub fn new(bars_by_symbol: Vec<(String, Vec<Bar>)>, strategy: S, p: EngineParams) -> Self {
        ...
        // `expect("granular symbol unknown")` below, rather than returning a named error. No
        // profile can reach it TODAY — nothing in `crate::harness::profile` sets
        // `EngineParams::granular_by_symbol` from TOML — but it must become a named error before
        // any `[engine]` key does, exactly as `timeframes` did above.
