...
pub use sim_broker::{
    ...
    MirrorFunding, OptionExpirySource, OptionRight, OptionSpec, ResolutionSource, SettlementFill,
    SimBroker, VariationSettlement,
};
...
pub struct StrategyEngine<S: Strategy<SimBroker>> {
    ...
    /// The EFFECTIVE warm-up gate in steps, resolved ONCE in [`Self::new`] as
    /// `p.warmup.unwrap_or(0).max(strategy.warmup())` — the number both [`Self::run`] and
    /// [`Self::run_ticks`] compare their step index against before dispatching ANY strategy
    /// callback, and the number [`BacktestResult::warmup`] reports.
    ///
    /// ⚠ It is resolved here rather than read per step for two reasons, and the second is the
    /// load-bearing one. Cheaply: `strategy.warmup()` was a virtual call inside the fold loop.
    /// Correctly: the two REPORTING sites used to publish `strategy.warmup()` while the two GATE
    /// sites read the same expression, so the report could not disagree with the gate by
    /// construction — and the moment an outside FLOOR ([`EngineParams::warmup`]) exists, keeping
    /// that property means one resolution feeding both. A report naming a warm-up the run did not
    /// actually apply is worse than no report: `vike_analytics::zero_trade` diagnoses a
    /// zero-trade run FROM this number.
    warmup: usize,
}
...
impl<S: Strategy<SimBroker>> StrategyEngine<S> {
    pub fn new(bars_by_symbol: Vec<(String, Vec<Bar>)>, strategy: S, p: EngineParams) -> Self {
        ...
        // ⚠ This assert is a LAST line of defence, not the refusal an operator sees.
        // `harness::run::refuse_ragged_series` catches a ragged universe at load and names both
        // symbols and both lengths. Reaching HERE means a caller bypassed the harness loader, so
        // the message stays terse on purpose — it is addressed to a developer, not a user.
        ...
        //
        // ⚠ A THIRD undocumented-abort panic lives here, the same shape as the two recorded on
        // `crates/vike-backtest/src/engine/sim_broker.rs`'s module doc (`SimBroker::idx` on an
        // unknown symbol; `crates/vike-backtest/src/bar_buffer.rs`'s `BarSeriesBuffer`): a
        // `p.granular_by_symbol` entry
        // naming a symbol not present in this run's `symbols` aborts the process at the
        // `expect("granular symbol unknown")` below, rather than returning a named error.
        //
        // ⚠ **A PROFILE CAN NOW REACH THIS FIELD, and this comment said it could not.** The
        // `data.detail_interval` key (the intrabar detail tape) loads it through
        // `crates/vike-backtest/src/harness/run.rs`'s `load_profile_detail_bars`. The panic stays
        // unreachable FROM THAT DOOR by CONSTRUCTION rather than by a check: that loader builds
        // its entries from `DataCfg::resolved_series`, the same list the coarse `load_profile_bars`
        // walks, so a key it emits is a symbol of the run by definition and no comparison exists to
        // rot. What is still open is a DIRECT `EngineParams` caller — a test, a bin, a future
        // loader that assembles the pair from two sources — for which this remains a process abort
        // where `timeframes` gives a named refusal. Converting it means `StrategyEngine::new`
        // returning a `Result`, which is a signature change across every construction site in the
        // tree; it has not been done, and a second profile key that reaches this field must not be
        // written on the assumption that it was.
        ...
        // ...with the exceptions, and ⚠ **THE ROUTING IS THE OTHER WAY ROUND NOW**: the two shapes
        // that may FLATTEN are NAMED here, and every other shape is carried to the fill site
        // verbatim and applied through `FeeSchedule::commission` at the price the fill actually
        // transacted at. It read the other way — `ProbabilityScaled` named, everything else
        // flattened — and the inversion is a SAFETY property rather than a tidy-up, because
        // `maker_taker_rates()` answers `(0.0, 0.0)` for every shape it cannot express: under the
        // old routing a `PerShareWithFloor` schedule (the IBKR per-share + per-order MINIMUM +
        // notional cap) charged exactly ZERO while looking configured, which is the same
        // prediction-market misreport port backlog G7 named, wearing the equities shape. Naming
        // the flat side means a shape ADDED to `FeeSchedule` later defaults to being charged
        // exactly rather than to being charged nothing.
        //
        // What the two named arms are, and why flattening them is not a loss:
        //   * `PercentMakerTaker` — a flat fraction of notional IS the whole shape, and
        //     `commission` computes `((qty*px) * (bps/1e4))`, which is `fee(size, price, rate,
        //     mult)`'s own associativity with `rate = bps/1e4`. Identical arithmetic, one fewer
        //     per-fill branch.
        //   * `Free` — zero either way.
        // `PercentOfUnderlying` moved to the exact side and is byte-identical on every real
        // Deribit configuration: `commission` is `min(bps × premium, cap × premium)` and
        // `bps/1e4 ≪ cap`, so the `min` is a no-op there and the product is the same expression.
        // It matters only where the cap genuinely binds — which the flat fraction could not
        // express at all, and understating a fee is the dangerous direction.
        let fee_curve = match p.fee_schedule {
            None | Some(FeeSchedule::PercentMakerTaker { .. } | FeeSchedule::Free) => None,
            Some(exact) => Some(exact),
        };
        // The cross-section's walk order, resolved ONCE here (before `symbols` moves into the
        // struct) because `symbols` is fixed for the life of the engine. Identity under the
        // default `DecideMode::Sequential`, so every fold routed through
        // `SimBroker::cross_section` sums exactly the bits it summed before the field existed.
        let cross_section = p.decide.cross_section_order(&symbols);
        let core = SimBroker {
            symbols,
            bars,
            n,
            ...
            // sized by `run_ticks` only when an impact model is configured (see there); EMPTY
            // here and on every bar/vector path, which is what keeps `slippage_for`'s bar-window
            // read the frozen line
            impact_ticks: Vec::new(),
            slippage_saturations: 0,
            impact_unpriced: 0,
            // The realised maker/taker mix and the realised commission total — see the field docs
            // on `SimBroker`; both start at zero and are folded at the charge site.
            maker_fills: 0,
            taker_fills: 0,
            fees_paid: 0.0,
        ...
        // The warm-up gate, resolved ONCE (see the `warmup` field doc). `max`, not replace: an
        // outside floor may RAISE what the strategy declared and may never lower it, so a caller
        // passing a smaller number than `strategy.warmup()` changes nothing — which is what makes
        // `EngineParams::warmup` safe to hand an operator's profile key.
        let warmup = p.warmup.unwrap_or(0).max(strategy.warmup());
        StrategyEngine {
            core,
            strategy,
            queue,
    ...
    /// Granular sub-bar fill phase (`_fill_pending_granular`) — and, since
    /// `crates/vike-backtest/src/harness/run.rs`'s `load_profile_detail_bars` gave it a profile
    /// door (`data.detail_interval`), the engine's answer to the ONE thing a coarse bar cannot
    /// say.
    ///
    /// # What this lane resolves that [`Self::fill_pending`] can only guess
    ///
    /// A bar carries open/high/low/close and no PATH, so when a stop and a target both sit inside
    /// it, OHLC cannot say which was touched first. [`Self::fill_pending`] hands the triggered
    /// pair to `vike_fills::resolve_intrabar_fills`, which orders them ADVERSE-FIRST and caps the
    /// total reduction to the position — the pessimistic bound, not the truth — and counts the bar
    /// in `SimBroker::intrabar_both_hit`. This lane instead walks the step's sub-bars IN TIME
    /// ORDER and fills whichever level the finer series actually reaches first, so the sequence is
    /// OBSERVED. It never calls `resolve_intrabar_fills` at all.
    ///
    /// ⚠ **So `intrabar_both_hit` does not increment for a symbol that has a detail series, no
    /// matter how ambiguous its bars were.** Read that counter as "bars whose order the engine had
    /// to guess", never "ambiguous bars": with a detail tape on, a `0` says the guessing stopped,
    /// not that the ambiguity was absent. Comparing the two configurations is a PnL comparison.
    ///
    /// ⚠ **It is a finer PRINT tier, not only a finer FILL tier**, so turning the tape on can move
    /// a run that never had an ambiguous bracket at all: `note_print` is called per sub-bar (the
    /// opt-in staleness discipline therefore measures age against the sub-bar, and a fully-printing
    /// step never defers where a quiet stretch WITHIN a step does), [`Self::check_stop`] is
    /// evaluated once per sub-bar, and the coarse lane's `size <= 1e-12` dust guard is deliberately
    /// absent here — preserved from the frozen engine rather than chosen.
    ///
    /// ⚠ **KNOWN HOLE, stated because a profile key can now reach this lane: the TAGGED maker lane
    /// is not run here.** [`Self::fill_pending`] calls [`Self::fill_tagged`] before its pending
    /// loop and this function has no equivalent call, so a resting TAGGED quote
    /// (`vike_model::HftBroker`) on a symbol that HAS a detail tape never fills at all — it is not
    /// filled late, it is not filled on the coarse bar either, because the coarse lane is skipped
    /// for that symbol on that step. That predates the tape (the lane was unreachable from any
    /// profile), but the tape is what makes it reachable, and it cannot be refused from a profile
    /// because nothing in a profile says whether the strategy quotes with tags. So:
    /// `data.detail_interval` is for STOP/TARGET realism on a directional strategy, and a maker
    /// backtest belongs on the tick lane's `engine.queue_model` (whose `fill_tagged_queued` is the
    /// queue-gated twin of the tagged lane). Closing it means giving this lane a `fill_tagged` call
    /// per sub-bar, which is a fill-behaviour change to a frozen path and needs its own proof.
    ...
    /// weight desc, unfundable dropped.
    ///
    /// **This is the engine's one cross-sectional allocator** — it collects the whole step's
    /// triggered orders across every symbol before admitting any of them, which is what makes a
    /// joint decision possible here and nowhere else. The TIE-BREAK is therefore the decision,
    /// and [`EngineParams::decide`] chooses it: trigger order (the frozen answer, which is the
    /// order the symbols were listed) or the cross-section rank (the symbol's own name). See
    /// [`DecideMode`] for why the tie-break is not a detail — no strategy in this tree sets
    /// `weight`, so every order ties and the tie-break decides everything.
    fn fill_step_gated(&mut self, cur_idx: usize) {
        ...
        // THE CROSS-SECTION RANK — `symbols` position -> position in `SimBroker::cross_section`,
        // built once for this step and used as the tie-break of BOTH phases below.
        //
        // Empty (and every read of it skipped) under `DecideMode::Sequential`, so neither phase
        // allocates and both keep their frozen collection-order sequence.
        let ranked = self.core.decide.is_simultaneous();
        let rank: Vec<usize> = if ranked {
            let cs = self.core.cross_section();
            // `cs` is a permutation of `0..n`, so every symbol index is assigned exactly once
            // and no entry is left at the fill value.
            let mut r = vec![0usize; cs.len()];
            for (pos, &si) in cs.iter().enumerate() {
                r[si] = pos;
            }
            r
        } else {
        ...
        //
        // ⚠ Their ORDER matters even though none of them can be refused, and this is the subtler
        // half of the list-order defect. Each `apply_fill` moves the shared `cash` and pushes a
        // `Trade`, so the sequence decides (a) the f64 accumulation order of the cash the opens
        // are then judged against — addition is not associative, and the cash gate below is an
        // inequality against that exact value, so an ULP can flip an admission — and (b) the
        // order of `trades`, `MirrorFill` and every `Strategy::on_fill` callback, all of which a
        // report and a fixture compare positionally. Under `Simultaneous` the sequence is the
        // cross-section's; under the default it is the collection order, unchanged.
        if ranked {
            frees.sort_by(|a, b| rank[a.0].cmp(&rank[b.0]).then(a.3.cmp(&b.3)));
        }
        ...
        // Opens/adds: highest weight first. THE TIE-BREAK IS THE CROSS-SECTION DECISION, and
        // under the default mode it is the run's own hidden parameter.
        //
        // `weight` is `WorkingOrder::weight`, the declared cross-symbol fill priority — and
        // NOTHING in this tree sets it: `vike_model::Broker::submit_market`/`submit_limit` pass
        // `0.0`, and so does `SimBroker::strategy_order_target_percent`. So on every strategy
        // that exists here every order ties at `0.0`, and the whole verdict of this allocator —
        // which symbol gets the last of the shared cash and the last open slot — falls to the
        // tie-break.
        //
        // * `DecideMode::Sequential` breaks it on `seq`, the collection order, which is the order
        //   `symbols` was handed to `StrategyEngine::new`, which is the order the operator typed
        //   `data.symbols`. Kept verbatim (Python `sorted` = stable) because it is the frozen
        //   answer every parity fixture pins.
        // * `DecideMode::Simultaneous` breaks it on the CROSS-SECTION RANK — each order's
        //   position in `SimBroker::cross_section`, i.e. its symbol's own name — so permuting
        //   `data.symbols` permutes nothing here. The rank is looked up rather than compared by
        //   name so that this comparator agrees with every fold by construction: both read the
        //   one canonical order, and a future ordering rule changes `cross_section_order` alone.
        opens.sort_by(|a, b| {
            let tie =
                if ranked { rank[a.0].cmp(&rank[b.0]).then(a.3.cmp(&b.3)) } else { a.3.cmp(&b.3) };
            b.1.weight.partial_cmp(&a.1.weight).unwrap_or(std::cmp::Ordering::Equal).then(tie)
        });
        for (si, o, fp, _) in opens {
            ...
            // `.0` — the skip flag is deliberately DROPPED here. This is the cash pre-check, which
            // prices the same fill `apply_fill` will price again; counting in both would double
            // every entry, exactly as `SimBroker::slippage_saturations`' doc records for its twin.
            let slipped = adverse_fill_price(
                fp,
                o.side,
                self.core.slippage_for(si, fill_size, bar_ts, is_maker).0,
            );
    ...
    fn check_liquidation(&mut self, cur_idx: usize) {
        ...
        // Held positions in CROSS-SECTION order (`symbols` order under the default
        // `DecideMode::Sequential`, so this is the frozen list index for index). It is routed
        // through the canonical order because THREE separate answers below are functions of this
        // sequence: `eq_adv`/`notional_adv`/`maint_adv` are naive `+=` folds (deliberately naive
        // — the parity rule — and therefore order-sensitive in the last bits, which decides an
        // inequality), and `candidates` is handed to `vike_model::cross_liquidation_plan`, whose
        // losers-first sort breaks a TIE on candidate order. Two positions with equal unrealized
        // loss would otherwise be liquidated in the order the operator typed them.
        let held: Vec<usize> = self
            .core
            .cross_section()
            .iter()
            .copied()
            .filter(|&si| self.core.sym[si].pos.size != 0.0)
            .collect();
        if held.is_empty() {
            return;
        }
    ...
    pub fn run(&mut self) -> BacktestResult {
        ...
        // THE SHARED-CASH ADMISSION PHASE is reached by either door, and this is the ONE place
        // the disjunction is spelled: `EngineParams::decide = Simultaneous` IMPLIES it, because
        // `fill_step_gated` is the only lane in this engine where a cross-section exists to be
        // ordered at all — the per-symbol `fill_pending` loop consumes the open-position caps in
        // list order inside `dispatch_fill` and offers no site where a ranking could apply. So a
        // "simultaneous" mode on the non-gated lane would be a name with nothing behind it; see
        // `DecideMode::Simultaneous`.
        let gated = self.core.cash_gate || self.core.decide.is_simultaneous();
        ...
        for i in 0..self.core.n {
            ...
            granular.clear();
            granular.extend((0..n_sym).map(|si| !gated && !self.core.sym[si].sub[i].is_empty()));
            ...
            for si in 0..n_sym {
                    ...
                    && self.core.sym[si].pos.size != 0.0
                {
                    ...
                    if fc != 0.0
                        && let Some(sink) = &mut self.core.mirror_funding
                    {
            ...
            if i >= self.warmup {
                // Strategy._on_step: fan the bundle out per symbol, in CROSS-SECTION order —
                // `symbols` order under the default `DecideMode::Sequential`, so this loop is the
                // frozen one index for index.
                //
                // ⚠ The ENGINE's own reads do not move during this fan-out (no fill lands here,
                // so `cash`, `equity_now` and `open_risk` are the same for every symbol in the
                // step), which is why re-ordering it is not what closes the list-order defect —
                // the folds and the allocator are. It is re-ordered anyway because a STRATEGY may
                // carry cross-sectional state of its own (a rank, a budget it hands out, a "first
                // signal this bar wins" latch), and leaving the fan-out in list order would keep
                // a hidden argv dependency alive on the one side the engine cannot see. Under
                // `Simultaneous` the sequence a strategy observes is a property of the universe.
                ...
                for k in 0..n_sym {
                    let si = self.core.cross_section()[k];
        ...
        BacktestResult {
            ...
            impact_unpriced: self.core.impact_unpriced,
            ...
            // The realised maker/taker mix and the commission total, mirrored off the engine with
            // the diagnostics above — a stamp on a ranked answer cannot be read off the equity
            // curve, and this is the run that actually charged it.
            maker_fills: self.core.maker_fills,
            taker_fills: self.core.taker_fills,
            fees_paid: self.core.fees_paid,
        }
    }
    ...
    pub fn run_ticks(&mut self, ticks_by_symbol: &[(String, Vec<Tick>)]) -> BacktestResult {
        ...
        // ARM the tick lane's market context for the opt-in impact model, for the duration of this
        // replay only — the same discipline as `latency` / `shadow_pos` / `books` above, and for
        // the same reason: a window left behind would let a later bar `run` or a caller inspecting
        // the broker be answered off a stale tape.
        //
        // Sized ONLY when a model is configured, so a default replay allocates nothing and
        // `SimBroker::impact_context`'s `Vec::get` misses exactly as it does on the bar path.
        // Without this the tick lane was inert rather than absent: `run_ticks` is routinely driven
        // with an EMPTY per-symbol bar series, so the bar-window read measured `None` and an
        // operator who configured `[engine.impact]` got a silent zero.
        self.core.impact_ticks = if self.core.impact.is_some() {
            vec![crate::impact::TickWindow::with_capacity(self.core.impact_window); n_sym]
        } else {
            Vec::new()
        };
        ...
        loop {
            ...
            // quote/trade and three (symbol + both `Vec<BookLevel>`) per book event, on every tick of
            ...
            if let Tick::Book(b) = tick {
                ...
                if let Some(pre) = pre
                    && let Some(tracker) = self.queue.as_mut()
                {
            ...
            // Record this TRADE PRINT into the impact model's tick-lane market context — AFTER
            // EVERY FILL SITE THIS TICK REACHES, never before one. That ordering IS the
            // no-lookahead rule on this lane: an order pricing on this tick must not be charged
            // against a print that includes its own execution, exactly as the bar lane excludes
            // the bar its `ts` falls inside.
            // ⚠ It sits below `check_liquidation_tick`, not merely below `fill_pending_tick`,
            // and that is the CORRECTION of where it first landed. A forced close is a fill like
            // any other and is priced through the same `slippage_for`, so recording the print
            // between the two fill sites left liquidations — and only liquidations — charged
            // against a window containing their own tick, while the module claimed the exclusion
            // unconditionally. The strategy callback below may only SUBMIT; its orders fill on a
            // later tick, so nothing after this line prices anything against this print.
            // Quotes are deliberately not recorded (see `crate::impact::TickWindow`): they carry
            // no traded volume, so admitting them would push `avg_volume` toward zero and the
            // participation rate toward infinity on a quote-heavy tape. EMPTY vector = no model
            // configured, so the default replay pays one `is_empty` branch per tick.
            if !self.core.impact_ticks.is_empty()
                && let Tick::Trade(t) = tick
            {
                self.core.impact_ticks[si].push(t.price, t.size.abs());
            }
        ...
        if self.core.resolution.is_some() {
            ...
            if self.final_resolution_sweep(&last_ts)
                && let Some(last) = equity_curve.last_mut()
            {
                *last = self.core.equity_now();
            }
        }
        ...
        // The impact model's tick window is the same kind of state and is dropped for the same
        // reason — leaving it behind would make a later BAR run price its fills off this tape's
        // prints instead of its own bars, which is the one way this extension could reach a lane
        // it was never wired into.
        self.core.books = Vec::new();
        self.core.impact_ticks = Vec::new();
        self.strategy.on_stop(&mut self.core);
        let per_symbol_pnl: Vec<(String, f64)> = (0..n_sym)
            .map(|si| {
                let st = &self.core.sym[si];
...
/// The permutation proof for [`DecideMode`] — the property the mode exists to establish, checked
/// end to end through the real engine rather than argued in prose.
///
/// ⚠ The load-bearing test here is [`tests::sequential_is_order_dependent_the_defect_itself`],
/// not the invariance one. An invariance assertion over a fixture that is symmetric anyway passes
/// for no reason and reports a property nobody obtained; pinning that the DEFAULT mode genuinely
/// answers differently under the two symbol orders is what proves the fixture reaches the
/// mechanism. If the defect test ever goes green-by-accident, the invariance test beside it has
/// stopped meaning anything and both need a new fixture.
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn run_order(order: &[&str], decide: DecideMode) -> crate::BacktestResult {
    ...
    fn pnl_by_name(r: &crate::BacktestResult) -> Vec<(String, u64)> {
