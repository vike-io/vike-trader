...
impl LatencyRow {
    ...
    pub fn entry_ns(&self) -> i64 {
        if self.is_rejection() { -(self.resp_ts - self.req_ts) } else { self.exch_ts - self.req_ts }
    }
    ...
    pub fn response_ns(&self) -> i64 {
        if self.is_rejection() { self.resp_ts - self.req_ts } else { self.resp_ts - self.exch_ts }
    }
}
    ...
    pub fn in_flight_market_signed(&self, si: usize) -> f64 {
        ...
        for f in &self.queue {
            if let InFlightAction::Push { si: s, order } = &f.action
                && *s == si
                && order.kind == vike_model::OrderKind::Market
            {
