...
use super::{run_backtest, BacktestProfile, BacktestReport, HarnessError};
...
fn point_row(
    ...
    match run_backtest(profile, store.clone()) {
        Ok(r) => SweepRow {
            overrides,
...
pub(crate) fn eval_scored_point(
    ...
    let mut row = point_row(base, store, profile, overrides);
...
"#
    ...
    fn all_losing_grid_never_crowns_the_worst_point() {
        use crate::objective::{multi_metric_score, MultiMetricParams};
    ...
    fn run_sweep_with_ranks_by_objective() {
        use crate::objective::{multi_metric, MultiMetricParams};
        use vike_data::DataFusionHist;
