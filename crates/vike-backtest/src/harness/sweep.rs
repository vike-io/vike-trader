...
// and `expand_sweep` has out-of-crate consumers (`crates/vike-studio-core/src/wire_run.rs`,
...
pub fn expand_sweep(base: &BacktestProfile) -> Result<Vec<SweepPoint>, HarnessError> {
    ...
    expand_sweep_overrides(base)?
        .into_iter()
        .map(|point| {
            let (profile, overrides) = profile_with_overrides(base, point)?;
            Ok(SweepPoint { overrides, profile })
        })
        .collect()
}

/// [`expand_sweep`]'s ODOMETER half: the same cartesian product in the same deterministic order,
...
/// touch [`expand_sweep`] or [`SweepPoint`] at all: those two are consumed OUTSIDE this module
...
/// — so dropping `SweepPoint`'s profile field would turn N profile builds done ONCE into
/// N x `n_splits`. Strictly additive: `expand_sweep` is now this plus `profile_with_overrides` per
/// point, which is the fold every searched point already went through anyway.
pub fn expand_sweep_overrides(base: &BacktestProfile) -> Result<Vec<Candidate>, HarnessError> {
    if !base.is_sweep() {
...
/// [`expand_sweep_overrides`] and submits it as ONE batch, which is exactly what the deleted
...
impl Optimizer for GridSearch {
    ...
    fn search(
        ...
        let rows = eval
            .evaluate(expand_sweep_overrides(base)?)
            .into_iter()
            .map(|e| e.row)
            .collect::<Vec<_>>();
...
/// table nor `--json` can tell. For a custom objective use [`run_sweep_with`].
pub fn run_sweep(
    base: &BacktestProfile,
    store: Arc<dyn HistStore + Send + Sync>,
    rank_by: RankMetric,
...
///
/// The objective-path twin of [`run_sweep_exec`]'s adapter: an objective [`StoreEvaluator`] driven
