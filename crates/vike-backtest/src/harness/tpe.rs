...
use super::sweep::{SweepExec, SweepReport, SweepRow, numeric_sweep_axes, sort_scored_rows};
...
impl Optimizer for TpeSearch {
    ...
    fn search(
        ...
        let mut rows: Vec<SweepRow> = Vec::with_capacity(self.cfg.n_trials);
        for _ in 0..self.cfg.n_trials {
            ...
            // what keeps that map in scope: `domain.to_toml` ROUNDS an integral coordinate, so
...
/// ⚠ The evaluator is built `SweepExec::Sequential` DELIBERATELY, and it changes nothing: TPE's
...
pub fn run_tpe(
    ...
    let eval = StoreEvaluator::new(base, store, objective, label, SweepExec::Sequential)?;
