...
"#,
    ...
    fn euler_sweep_runs_and_reports_its_budget() {
        ...
        let (report, budget) =
            run_paramscan_euler(&base, store, &objective, "sharpe", &cfg).unwrap();
    ...
    fn euler_parallel_and_sequential_are_byte_identical() {
        ...
        let run = |exec| {
            run_paramscan_euler_exec(&base, store.clone(), &objective, "sharpe", &cfg, exec)
                .unwrap()
        };
    ...
    fn euler_and_tpe_rank_rows_exactly_like_run_sweep_with() {
        ...
        for w in ranks.windows(2) {
            assert!(
                w[0] <= w[1],
                "tpe rows must follow run_paramscan_with's rank order: {ranks:?}"
            );
        }
        for row in &tpe.rows {
            assert_eq!(
