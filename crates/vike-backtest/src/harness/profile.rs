...
/// The `[walkforward]` section: how many ANCHORED out-of-sample windows to walk this profile's
/// bar slice over ([`super::walkforward::run_walkforward`]).
///
/// Only the split count is configurable, deliberately: the mode (`WalkMode::Anchored`) and the
/// annualization ([`super::report::periods_per_year`]) are DERIVED, exactly as the Studio's
/// walk-forward derives them — one knob, no way for a profile to disagree with the Studio on what
/// "walk-forward" means.
#[derive(Debug, Clone, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct WalkforwardCfg {
    /// Number of anchored out-of-sample windows. Must be `>= 1` ([`BacktestProfile::validate`]
    /// rejects `0`); a negative TOML integer fails to deserialize into `usize` as a parse error.
    pub n_splits: usize,
}
...
pub struct EngineCfg {
    ...
    /// [`BacktestProfile::validate`] (the mirror of the bar-only `impact` rule). The funding lookup
    ...
    /// ignored (the mirror of the `impact` rule, which is bar-mode only).
    #[serde(default)]
    ...
    /// Absent (the default) = the frozen flat-slippage cost, byte-identical. BAR MODE ONLY —
    /// the tick lane replays a real book and needs no model, so a profile that sets this with
    /// `data.kind = "tick"` is rejected by [`BacktestProfile::validate`] rather than silently
    /// ignored.
    #[serde(default)]
...
pub struct ImpactCfg {
    ...
    pub model: String,
    /// execution horizon in bar-periods (see [`crate::impact::AlmgrenChriss::exec_time`])
    #[serde(default = "default_exec_time")]
    pub exec_time: f64,
    /// rolling sigma/volume lookback in bars
...
impl ImpactCfg {
    ...
    pub fn build(&self) -> Result<Arc<dyn crate::impact::ImpactModel>, HarnessError> {
        ...
        match self.model.as_str() {
            "almgren_chriss" => {
                Ok(Arc::new(crate::impact::AlmgrenChriss::with_exec_time(self.exec_time)))
            }
            other => Err(HarnessError::Validation(format!(
...
impl BacktestProfile {
    ...
    pub fn validate(&self) -> Result<(), HarnessError> {
        ...
        if let Some(r) = &self.risk {
            if r.max_orders_per_window.is_some() {
        ...
        if let Some(imp) = &self.engine.impact {
            ...
            if self.data.kind == DataKind::Tick {
                return Err(HarnessError::Validation(
                    "engine.impact is bar-mode only: the tick lane replays a real book, which \
                     already prices size better than any model"
                        .to_string(),
                ));
            }
        }
        ...
        // fold — so it is bar-mode only (the mirror of the bar-only `impact` rule).
        ...
        // A zero-split walk-forward is always wrong (there is no window to test on), so it fails at
        // LOAD. The bar-mode/single-symbol rules are NOT checked here on purpose: `[walkforward]`
        // is inert for a plain `run_backtest`/`run_sweep` run, and rejecting a profile that merely
        // CARRIES the section would fail runs that never ask for a walk-forward.
        // `super::walkforward::run_walkforward` enforces those rules where they apply.
    ...
    fn validate_data_slice(&self) -> Result<(), HarnessError> {
        ...
        if self.data.kind == DataKind::Bar {
            if let Some(s) = self.data.series.iter().find(|s| s.kind != SeriesKind::Tick) {
                return Err(HarnessError::Validation(format!(
...
"#;
    ...
    fn queue_model_parses_resolves_and_is_tick_only() {
        ...
        assert!(BacktestProfile::from_toml_str(TICK_TOML)
            .unwrap()
            .engine
            .queue_model_kind()
            .unwrap()
...
"#;
    ...
    /// The tick lane replays a real book; a model there would be strictly worse than what the
    /// engine already does, so the combination is rejected rather than ignored.
    #[test]
    fn impact_on_the_tick_lane_is_rejected() {
    ...
    /// combination is rejected rather than silently ignored (the mirror of the bar-only `impact`
    /// rule above).
    #[test]
...
"#;
    ...
    fn a_relative_resolution_path_resolves_against_the_profile_dir() {
        ...
        assert!(p
            ...
            .as_ref()
            .unwrap()
            .build(p.base_dir.as_deref(), &symbols)
            .is_ok());
    }
