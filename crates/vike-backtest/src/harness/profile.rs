...
use crate::engine::{DecideMode, EquitySampling, FillModelKind};
...
pub struct DataCfg {
    ...
    /// start, and [`crate::BacktestResult::warmup`] then reports the number the run actually
    ...
    /// `max(this, strategy.warmup())` (`crate::StrategyEngine::new` resolves it once). A profile
    ...
    /// (`crate::engine::SimBroker`'s bar reads), not out of state accumulated inside gated
    ...
    /// counts each such bar in [`crate::BacktestResult::intrabar_both_hit`] — so the engine has
    /// always known exactly which bars it was guessing on. With a detail series present that
    /// symbol's step instead runs `crate::StrategyEngine::fill_pending_granular`, which walks the
    ...
    /// opt-in staleness discipline (`crate::staleness`) measures age against the sub-bar rather
...
impl DataCfg {
    ...
    /// a strategy callback, or `None` when the key is absent (⇒ `crate::EngineParams::warmup` is
    ...
    /// absent (⇒ `crate::EngineParams::granular_by_symbol` stays empty ⇒ every symbol fills on
    ...
    /// mode is refused because the granular lane is `crate::StrategyEngine::run`'s own bar step:
...
pub struct EngineCfg {
    ...
    /// [`crate::DecideMode`], and this doc deliberately does not restate it.
    ///
...
///
/// [`EngineCfg::fee_rate`] is one flat fraction of notional, and `vike_model::fees` expresses
...
///   `crates/vike-model/src/fees.rs`'s `fee_schedule_for` `"ibkr"` arm): a per-share fee, a
...
///
/// [`crate::StrategyEngine::new`] routes a schedule one of two ways (its own comment is the
...
impl FeeCfg {
                ...
                if self.premium_cap_pct.is_some_and(|c| c <= 0.0) {
                    return Err(HarnessError::Validation(format!(
                        "engine.fee.premium_cap_pct must be > 0, got {} — a zero cap is not \
                         ...
                         fee-modelled. crates/vike-model/src/fees.rs's fee_schedule_for carries \
    ...
    /// (`crates/vike-model/src/fees.rs`'s `fee_schedule_for` `"binance"`/`"binance-perp"` arms
        ...
        match lanes.len() {
            ...
            _ => Err(HarnessError::Validation(format!(
                "engine.fee.kind = \"venue\" resolves ONE schedule for the whole run, and this \
                 ...
                 completely differently (crates/vike-model/src/fees.rs's fee_schedule_for \
...
"#;
    ...
    fn the_percent_maker_taker_shape_says_the_two_sides_apart() {
        let schedule =
            fee_cfg("kind = \"percent_maker_taker\"\nmaker_bps = 2.0\ntaker_bps = 5.0\n")
                .build()
    ...
    fn the_venue_kind_resolves_the_perp_lane_from_the_runs_own_symbol() {
        ...
        let perp = cfg
            .build_for(&fee_series(&[("binance", "BTCUSDT.P")]))
            .expect("the perp lane resolves");
        assert_eq!(
            perp,
            vike_model::fee_schedule_for(vike_catalog::fee_lane("binance", "BTCUSDT.P"))
        );
        let spot =
...
"#;
    ...
    fn a_refused_slice_does_not_produce_a_second_refusal_about_the_symbols_it_never_resolved() {
        ...
        let p: BacktestProfile =
            toml::from_str(&toml).expect("it parses; the duplicate is semantic");
        let all = p.validate_all();
