...
use crate::validation::{Split, WalkMode};
...
"#;
    ...
    ///
    /// ⚠ `vike_data::MemHistStore`, not a temp-dir `DataFusionHist` — so this one rides
    /// `hist-replay` where [`seeded_store`] below needs `datafusion-store`. That is available
    /// because `crates/vike-backtest/Cargo.toml` declares
    ...
    fn a_quoted_bare_number_window_is_refused_by_name() {
        ...
        .unwrap_err();
        assert!(matches!(unquoted, HarnessError::Parse(_)), "{unquoted}");
    }
    ...
    /// the trait-only `hist-replay` lane.
    ///
