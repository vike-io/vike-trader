//! `backtest --fetch-starter` — download the published starter dataset into the hist store.
//!
//! # Why this exists beside `--fetch`
//!
//! `--fetch` reaches the venue directly, which is the right answer when the venue is reachable. It
...
//! (`github.com/vike-io/vike`), which is a plain HTTPS download with no credentials and no venue
//! involved. `scripts/publish_starter_data.sh` produces it, from this workspace's own `--fetch` and
//! `--export`, so the dataset is exactly what a user's own fetch would have written.
//!
...
pub const STARTER_BASE: &str = "https://github.com/vike-io/vike/releases/download/starter-data";
...
";
    ...
    fn the_base_url_is_the_public_mirror_over_https() {
        ...
        assert!(
            STARTER_BASE.contains("github.com/vike-io/vike/"),
