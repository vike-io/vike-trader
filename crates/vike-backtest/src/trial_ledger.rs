...
pub struct SearchHeader {
    ...
    pub run_id: String,
    /// `none` | `scalars`. The third spelling the design names, `series`, is refused at the flag —
    /// `crates/vike-backtest/src/backtest_cli.rs`'s `parse_keep_trials` carries the argument.
...
pub struct TrialsDocument {
    ...
    /// The parent run's id.
    pub run_id: String,
    /// `none` | `scalars`, as [`SearchHeader::keep_trials`].
    pub keep_trials: String,
