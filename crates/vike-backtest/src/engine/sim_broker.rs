...
//!
//! ⚠ Two panics in this file's neighbourhood are deliberately NOT addressed by the `bars_for`
//! change: `SimBroker::idx` panics on an unknown symbol, and
//! `crates/vike-backtest/src/bar_buffer.rs`'s `BarSeriesBuffer` carries its own copy of the same
//! parse-and-lookup pair. `BarSeriesBuffer` is `pub use`d from the crate root and has ZERO
//! constructors in this repository, so it is a public divergent copy rather than a live path.
...
/// WHEN a multi-symbol step is decided: one symbol at a time in the order the caller listed
/// them, or every symbol against one shared account state ([`EngineParams::decide`]).
///
/// # The defect this type names: the symbol list is a hidden strategy parameter
///
/// [`crate::StrategyEngine::run`] fans `Strategy::on_bar` out over `symbols` in the order the
/// caller handed them in, and three of the ENGINE's own reads are a function of that order rather
/// than of the SET of instruments. A run's answer is supposed to be a function of the universe,
/// the tape and the strategy; each of these makes it a function of a fourth input nobody chose,
/// no profile records and no report shows:
///
/// 1. **The shared-cash allocator's tie-break.** `StrategyEngine::fill_step_gated`
///    already does the hard half: it collects the whole step's triggered orders across every
///    symbol, fills the reductions first (freeing cash), and only then ranks the opens against
///    the shared pool. But it ranks on [`vike_model::WorkingOrder::weight`] and breaks a tie on
///    COLLECTION order, which is symbol-list order — and **nothing in this tree ever sets
///    `weight`**: [`vike_model::Broker::submit_market`] and `Broker::submit_limit` pass `0.0`,
///    and so does [`SimBroker::strategy_order_target_percent`]. So every order ties, and the
///    allocator's entire verdict — who gets the last of the cash and the last open slot — is the
///    argv order. [`Self::Simultaneous`] replaces that tie-break with the symbol's own name.
/// 2. **Every cross-sectional fold.** [`SimBroker::equity_now`], `SimBroker::open_risk`,
///    `SimBroker::margin_in_use_pending_aware` and `SimBroker::cap_to_leverage` all sum with
///    [`vike_model::py_sum`] over `0..symbols.len()`. f64 addition is not associative and
///    Neumaier compensation does not make it so, so permuting the list moves `equity_now` by an
///    ULP — which moves [`vike_model::units_from_percent`], which can move a fill by a whole lot
///    or a whole tick. **This is the one that bites the DEFAULT configuration**: with no
///    `[risk]` table, no `leverage` and no position caps, the non-gated fill lane gates nothing
///    at all, so the fold order is the *only* order dependency a plain portfolio run has.
/// 3. **The pre-trade gate's pending-aware budget.** `SimBroker::margin_in_use_pending_aware`
///    and the exposure fold in `SimBroker::gate_order` sum the `pending` set, which ACCUMULATES
///    during the `on_bar` fan-out: the symbol judged first faces the step's full budget and every
///    later symbol faces what is left of it. That term is not a bug — its own doc argues why a
///    pre-trade cap must model the local view — but it means the DECISION, not merely the
///    allocation, is taken in list order. Closing it needs a step-boundary budget snapshot, which
///    is **not built**; [`Self::Simultaneous`] is therefore REFUSED at load beside an armed
///    `[risk]` table or `engine.leverage` rather than silently claiming a cross-section it did
///    not compute (`BacktestProfile::refusals`).
///
/// # Why the cure is a canonical ORDER rather than a joint solve
///
/// A genuinely joint allocation — pro-rata scaling every open by one factor so the step's demand
/// fits the pool — is a different and larger design, and it cannot express an indivisible
/// resource: `max_open_positions` is a slot count, and half a slot is not a thing. What every
/// order dependency above has in common is narrower and fixable: the walk is indexed by LIST
/// POSITION, an accident of how the universe was typed. Ordering the cross-section by the
/// instrument's own identity makes each of them a property of the SET, so permuting `data.symbols`
/// cannot move the answer — which is exactly the property a portfolio run is supposed to have.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub enum DecideMode {
    /// One symbol at a time, in the order the caller listed them — the frozen behaviour, and
    /// **byte-identical to the engine before this type existed**: `cross_section_order` returns
    /// the identity permutation, so every fold visits the same indices in the same sequence and
    /// sums to the same bits, and the allocator keeps its collection-order tie-break.
    ///
    /// It is named rather than left implicit because an unnamed default cannot be reported. The
    /// list-order dependency was invisible in every profile and every report; a run that carries
    /// `decide = "sequential"` says out loud which of the two answers it computed.
    #[default]
    Sequential,
    /// One decision over the whole cross-section: every fold and the shared-cash allocator walk
    /// the symbols in a canonical order derived from their own names, so no permutation of
    /// `data.symbols` can change the result.
    ///
    /// ⚠ It **implies the shared-cash admission phase** ([`EngineParams::cash_gate`]) and
    /// therefore inherits both of that mode's consequences: the step routes through
    /// `StrategyEngine::fill_step_gated` instead of the per-symbol `fill_pending` loop, and the
    /// granular sub-bar lane is unconditionally disabled. That is not a coupling of convenience —
    /// without an allocator there is no cross-section to order. The per-symbol lane consumes the
    /// position caps in list order inside `StrategyEngine::dispatch_fill` and has no place where
    /// a ranking could be applied at all, so a "simultaneous" mode on that lane would be a name
    /// with nothing behind it.
    Simultaneous,
}

impl DecideMode {
    /// `true` for [`Self::Simultaneous`] — the one branch every consumer needs, spelled once so
    /// a third variant cannot silently read as sequential at some call site that matched on
    /// `Simultaneous` alone.
    pub fn is_simultaneous(self) -> bool {
        matches!(self, DecideMode::Simultaneous)
    }

    /// The order every cross-sectional walk takes over `symbols`, as indices INTO `symbols`.
    ///
    /// * [`Self::Sequential`] ⇒ the identity permutation `0..symbols.len()`. Load-bearing: it is
    ///   what makes the default path byte-identical rather than merely equivalent — the folds
    ///   visit the same indices in the same sequence, so the f64 sums are bit-for-bit the frozen
    ///   ones and no parity fixture moves.
    /// * [`Self::Simultaneous`] ⇒ indices sorted by the symbol's own name, ascending, with the
    ///   list position as the tie-break for a duplicated name.
    ///
    /// ⚠ **Sorting by NAME is arbitrary, and that is the point — it is arbitrary in a way the
    /// caller cannot influence.** Any total order intrinsic to the instrument would do; what
    /// matters is that it is not the list position, because the list position is the one input a
    /// portfolio run's answer must not depend on. A duplicated symbol falls back to list
    /// position, spelled as an EXPLICIT `.then(a.cmp(b))` rather than left to `sort_by`'s
    /// stability — a total comparator is what makes the result a function of the input and not of
    /// the sort implementation. A duplicate universe is already degenerate (two slots of one
    /// instrument sharing one price series), so there is nothing better to order it by, and
    /// taking the earlier slot at least keeps the answer a permutation, which
    /// `StrategyEngine::fill_step_gated`'s rank inversion relies on.
    pub fn cross_section_order(self, symbols: &[String]) -> Vec<usize> {
        let mut order: Vec<usize> = (0..symbols.len()).collect();
        if self.is_simultaneous() {
            order.sort_by(|a, b| symbols[*a].cmp(&symbols[*b]).then(a.cmp(b)));
        }
        order
    }
}
...
pub struct EngineParams {
    ...
    ///
    /// **Which shapes flatten is now a CLOSED list, and the other direction is the default.**
    /// [`StrategyEngine::new`] names exactly two shapes as flattenable —
    /// [`FeeSchedule::PercentMakerTaker`] and [`FeeSchedule::Free`], where a flat fraction of
    /// notional IS the whole shape and `commission` computes the same expression in the same
    /// associativity as `broker_sim::fee` — and carries EVERY other shape to the fill site
    /// verbatim, applying it through `FeeSchedule::commission` at the price the fill transacted
    /// at (see [`SimBroker::fee_curve`]).
    ///
    /// ⚠ **It read the other way round, and the inversion is a safety property.** Only
    /// [`FeeSchedule::ProbabilityScaled`] was named as exact — on the true observation that
    /// `maker_taker_rates()` answers `(0.0, 0.0)` for it, so flattening would charge ZERO while
    /// looking configured (the prediction-market misreport port backlog G7 calls out). But that
    /// method answers `(0.0, 0.0)` for [`FeeSchedule::PerShareWithFloor`] as well, so the IBKR
    /// equities shape — a per-share fee, a per-order MINIMUM and a notional cap — was the SAME
    /// bug wearing an equities hat, and it became reachable from a profile the moment
    /// `crate::harness::profile`'s `FeeCfg` learned to name that shape. Naming the FLAT side
    /// means a shape added to `FeeSchedule` later defaults to being charged exactly rather than
    /// to being charged nothing.
    ///
    /// `POLYMARKET_PROB_CURVE`'s all-zero rates still charge zero, so every existing consumer of
    /// that constant is numerically unchanged; so is every `PercentOfUnderlying` configuration
    /// whose premium cap does not bind (which is every real Deribit row — `bps/1e4 ≪ cap`, so
    /// `commission`'s `min` is a no-op and the surviving product is `fee`'s own expression).
    ///
    /// ⚠ The exact path multiplies the schedule's answer by the symbol's CONTRACT MULTIPLIER,
    /// which is right for a rate-shaped or notional-capped term and WRONG for a currency-shaped
    /// one: a `PerShareWithFloor` `$1.00` minimum is one dollar, not one dollar per contract. It
    /// is exact at the default `multiplier = 1.0`, which is what an equities book runs at.
    ...
    pub liq_buffer: f64,
    pub cash_gate: bool,
    /// WHEN the cross-section is decided — see [`DecideMode`], which carries the whole argument.
    /// [`DecideMode::Sequential`] (the default) is byte-identical to the engine before this field
    /// existed. [`DecideMode::Simultaneous`] IMPLIES [`Self::cash_gate`]: the engine reads
    /// `cash_gate || decide.is_simultaneous()` wherever it used to read `cash_gate` alone, so
    /// selecting it routes the bar step through the shared-cash admission phase (and disables the
    /// granular sub-bar lane) without the caller also having to set the older flag.
    ///
    /// ⚠ BAR MODE ONLY, for the same reason `cash_gate` is: the allocator it orders lives in
    /// `StrategyEngine::fill_step_gated`, which only `StrategyEngine::run` calls.
    /// `StrategyEngine::run_ticks` never consults either field, so a tick profile asking for it
    /// would change nothing — `BacktestProfile::refusals` refuses that pair rather than accepting
    /// it silently.
    pub decide: DecideMode,
    ...
    /// A FLOOR on the warm-up gate, in STEPS — the door a caller (today the harness's
    /// `data.warmup` key) uses to raise the `index >= warmup` dispatch gate above what the
    /// strategy's own [`vike_model::Strategy::warmup`] declares. `None` (the default) is
    /// byte-identical to before this field existed: the gate is exactly `strategy.warmup()`.
    ///
    /// ⚠ **`max`, never replace, and never downwards.** [`crate::StrategyEngine::new`] resolves
    /// the effective number ONCE as `warmup.unwrap_or(0).max(strategy.warmup())` and both
    /// `run` and `run_ticks` gate on that; [`crate::BacktestResult::warmup`] reports it, so
    /// `vike_analytics::zero_trade`'s `warmup-shortfall` cause diagnoses a warm-up that
    /// swallowed the whole slice whichever side asked for it. A caller passing a SMALLER number
    /// than the strategy declares changes nothing, deliberately: `warmup()` is the strategy's
    /// statement about its own indicators, and letting an outside number lower it would let a
    /// run trade on indicators that had not converged — the failure this gate exists to prevent.
...
pub struct SimBroker {
    ...
    pub maker_fee: f64,
    pub taker_fee: f64,
    /// The EXACT per-fill fee schedule, set by [`StrategyEngine::new`] for every
    /// [`EngineParams::fee_schedule`] shape EXCEPT the two it names as flattenable
    /// ([`FeeSchedule::PercentMakerTaker`] and [`FeeSchedule::Free`], for which a flat fraction
    /// is the whole shape and the arithmetic is identical). `None` there — and for the default
    /// `fee_schedule: None` — keeps the frozen `fee(size, price, rate, mult)` fold
    /// byte-identical.
    ///
    /// ⚠ The field name still says "curve" because `ProbabilityScaled`'s `p(1−p)` curve was the
    /// first shape to need it; it now carries a floor and a cap too. The name is left alone
    /// deliberately — it is `pub(crate)` and renaming it would churn call sites for a word,
    /// while `crate::StrategyEngine::new`'s own comment is the authority on what reaches it.
    ...
    pub liq_buffer: f64,
    pub cash_gate: bool,
    /// see [`EngineParams::decide`] / [`DecideMode`]
    pub decide: DecideMode,
    /// The order every cross-sectional walk takes, as indices into [`Self::symbols`] — built
    /// ONCE at construction by [`DecideMode::cross_section_order`] and read through
    /// [`Self::cross_section()`].
    ///
    /// Stored rather than recomputed because [`Self::equity_now`] is on the per-step path and is
    /// also called from inside `size_entry` for every submit: sorting `n` strings there would put
    /// a comparison-sort in the sizing path for a value that cannot change after construction
    /// (`symbols` is fixed by `StrategyEngine::new`). Under [`DecideMode::Sequential`] it is the
    /// identity permutation, which is what keeps every fold's f64 bits frozen.
    pub(crate) cross_section: Vec<usize>,
    ...
    /// ORDER FILLS booked as MAKER, and its taker twin below — the REALISED maker/taker mix,
    /// counted where the fee is charged so the two can never disagree.
    ///
    /// ⚠ **It records the `is_maker` the FEE was computed under, and nothing else.** The dispatch
    /// sites set that from the order KIND rather than from crossing aggressiveness (the caveat
    /// [`Self::apply_fill`] carries in its own source), so the count inherits exactly that
    /// classification. That is the property worth having: a consumer comparing "what mix did this
    /// run realise" against "what schedule did it resolve" must read the mix the CHARGE used, not
    /// a second derivation of it.
    ///
    /// Incremented in [`Self::fold_fill`] and ONLY for an order fill — a settlement
    /// ([`Self::settle_at_payout`]) folds through the same function with `settlement = true`, no
    /// order behind it and a zero fee, so counting it would report a taker fill nobody placed.
    /// Mirrored onto [`crate::BacktestResult::maker_fills`] at end-of-run, like
    /// [`Self::impact_unpriced`] and for the same reason: a diagnostic only tests can read tells
    /// an operator nothing.
    pub maker_fills: u64,
    /// ORDER FILLS booked as TAKER — the other half of [`Self::maker_fills`], same rules, same
    /// increment site.
    pub taker_fills: u64,
    /// The running TOTAL of per-fill commission, the same `fee` [`Self::fold_fill`] debits from
    /// [`Self::cash`] — summed rather than recomputed, so the reported number is the charged one.
    /// Signed: a rebate-bearing schedule contributes negative terms. Order fills only, for the
    /// reason [`Self::maker_fills`] gives. Mirrored onto [`crate::BacktestResult::fees_paid`].
    pub fees_paid: f64,
...
impl SimBroker {
    ...
    /// THE cross-sectional walk order — every fold over the whole book goes through here rather
    /// than over `0..symbols.len()` directly, so there is ONE place that decides whether a
    /// portfolio read is a property of the instrument SET or of the list the caller typed.
    ///
    /// ⚠ Under [`DecideMode::Sequential`] this is the identity permutation, so a fold routed
    /// through it visits the same indices in the same sequence as the raw range it replaced and
    /// the result is bit-for-bit the frozen one. That equality is the whole reason the indirection
    /// is safe to add to a parity-frozen path: nothing about the default answer changes, only
    /// where the order comes from.
    #[inline]
    pub(crate) fn cross_section(&self) -> &[usize] {
        &self.cross_section
    }

    /// cash + Σ pos·price·mult (Python builtin sum → py_sum, cross-section order — the symbols
    /// order under the default [`DecideMode::Sequential`]).
    pub fn equity_now(&self) -> f64 {
        self.cash
            + py_sum(self.cross_section().iter().map(|&si| {
    ...
    /// Total $ risk currently open across the book (builtin sum → py_sum, cross-section order).
    ...
    fn margin_in_use_pending_aware(&self, order_im: f64) -> f64 {
        ...
        py_sum(self.cross_section().iter().map(|&i| {
    ...
    fn cap_to_leverage(&self, si: usize, side_sign: i32, size: f64) -> f64 {
        ...
        let cur = py_sum(self.cross_section().iter().map(|&i| {
            vike_model::gross_notional(
                self.sym[i].pos.size,
                self.sym[i].price,
                self.multiplier_of_idx(i),
        ...
        let pending = py_sum(self.cross_section().iter().map(|&i| {
            let mut signed = py_sum(
                self.sym[i]
                    .pending
                    .iter()
    ...
    /// The completed coarse bars of `tf` for `symbol`, look-ahead safe: only windows that have
    /// fully elapsed at `self.now` are visible.
    ///
    /// `None` means the timeframe is not registered — declare it in `[engine] timeframes`.
    /// ⚠ An unknown SYMBOL still panics, in `idx`; that is a separate defect.
    ///
    /// ⚠ **This `None` is a SUBSTITUTE for an unmet spec clause, not the clause itself.**
    /// `docs/superpowers/specs/2026-09-12-backtest-cli-surface-design.md` §9.1(2) required the
    /// undeclared-timeframe failure to become a LOAD-TIME refusal — "checked against the
    /// strategy's declared timeframe requirement" — before the `timeframes` profile key shipped.
    /// That check was not built (strategies declare no timeframe requirement, and giving them one
    /// is a different plan), and the loud `panic!("timeframe {tf:?} not registered")` this method
    /// used to carry became this quiet `None`. So a strategy asking for a timeframe the profile
    /// did not declare now runs on NO higher-TF data instead of aborting. The trade is accepted
    /// because it is latent — this method and [`Self::forming_for`] have zero callers anywhere in
    /// this tree — and it closes when a strategy-declared timeframe requirement exists.
    pub fn bars_for(&self, symbol: &str, tf: &str) -> Option<&[Bar]> {
        let si = self.idx(symbol);
        let (_, ms, coarse) = self.sym[si].tf.iter().find(|(name, _, _)| name == tf)?;
        let window_start = self.now - self.now.rem_euclid(*ms);
        let idx = coarse.partition_point(|b| b.ts < window_start);
        Some(&coarse[..idx])
    }

    /// The partially-formed coarse bar of `tf` for `symbol`, or `None`.
    ///
    /// ⚠ `None` now carries TWO meanings: no base bars have landed in the current window yet, and
    /// the timeframe is not registered at all. They were one panic and one `None` before; a caller
    /// that must tell them apart should ask `bars_for` first, which answers only the second.
    pub fn forming_for(&self, symbol: &str, tf: &str) -> Option<Bar> {
        let si = self.idx(symbol);
        let (_, ms, _) = self.sym[si].tf.iter().find(|(name, _, _)| name == tf)?;
        let window_start = self.now - self.now.rem_euclid(*ms);
    ...
    pub(crate) fn apply_fill(
        ...
        let mult = self.multiplier_of_idx(si);
        // `fee_curve` is `Some` for every schedule shape but the two `StrategyEngine::new` names
        // as flattenable (`PercentMakerTaker`, `Free`) — see `EngineParams::fee_schedule`. An
        // unset schedule, and those two, take the frozen `size * price * rate * mult` fold below,
        // byte-identical. ⚠ The `* mult` is right for a rate-shaped or notional-capped term and
        // wrong for a CURRENCY-shaped one (`PerShareWithFloor`'s per-order minimum is a dollar
        // amount, not a dollar per contract); it is exact at the default `multiplier = 1.0`.
    ...
    fn fold_fill(
        ...
        // The REALISED mix and the realised total, recorded beside the charge itself (see
        // `maker_fills`). Two `u64` increments and one add on the fill path, after every gate has
        // already passed — this is the backtest engine, not the live fold the latency gate
        // measures. Settlements are excluded: they are venue events with a zero fee, and a
        // `settlement` folded into the taker count would describe a fill nobody placed.
        if !settlement {
            if is_maker {
                self.maker_fills += 1;
            } else {
...
/// `bars_for`/`forming_for` have zero callers anywhere in this tree (`SimBroker` implements only
/// `Broker`/`HftBroker`, neither of which declares them, and no Rhai surface binds them either) —
/// this module carries no other test double for `SimBroker`, so these two build one through the
/// real constructor path (`StrategyEngine::new(..).core`) with a no-op strategy, matching the
/// benches' `NoopStrategy` idiom.
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn broker(p: EngineParams) -> SimBroker {
        crate::StrategyEngine::new(vec![("BTCUSDT".to_string(), Vec::new())], NoopStrategy, p).core
    }

    #[test]
    fn an_unregistered_timeframe_is_none_not_a_panic() {
        let p = EngineParams { timeframes: vec!["4h".to_string()], ..Default::default() };
        let broker = broker(p);
        assert!(broker.bars_for("BTCUSDT", "1d").is_none());
        assert!(broker.forming_for("BTCUSDT", "1d").is_none());
    }

    #[test]
    fn a_registered_timeframe_still_answers() {
        let p = EngineParams { timeframes: vec!["4h".to_string()], ..Default::default() };
        let broker = broker(p);
        assert!(broker.bars_for("BTCUSDT", "4h").is_some());
    }

    fn universe() -> Vec<String> {
        ["ETHUSDT", "BTCUSDT", "SOLUSDT"].iter().map(|s| (*s).to_string()).collect()
    }

    /// The DEFAULT mode's order is the IDENTITY, and that is not a detail: it is the whole reason
    /// routing every cross-sectional fold through `SimBroker::cross_section` cannot move a
    /// parity fixture. Same indices, same sequence, same f64 bits.
    #[test]
    fn sequential_is_the_identity_permutation() {
        let syms = universe();
        assert_eq!(DecideMode::Sequential.cross_section_order(&syms), vec![0, 1, 2]);
        assert_eq!(DecideMode::default(), DecideMode::Sequential);
    }

    /// The simultaneous order is derived from the symbols' own NAMES, so it is a function of the
    /// instrument SET — which is what makes every consumer of it (the folds, the allocator's
    /// tie-break, the `on_bar` fan-out) blind to how the universe was typed.
    #[test]
    fn simultaneous_orders_by_name_not_by_list_position() {
        let syms = universe();
        // BTCUSDT(1) < ETHUSDT(0) < SOLUSDT(2)
        assert_eq!(DecideMode::Simultaneous.cross_section_order(&syms), vec![1, 0, 2]);
    }

    /// THE PROPERTY: two permutations of one universe induce the same VISITING SEQUENCE of
    /// symbols. Asserted on the names rather than on the index vectors, because the indices are
    /// necessarily different — that difference is exactly what the mode is absorbing.
    #[test]
    fn a_permuted_universe_is_visited_in_the_same_sequence() {
        let forward: Vec<String> = ["AAA", "BBB", "CCC"].iter().map(|s| (*s).to_string()).collect();
        let reversed: Vec<String> =
            ["CCC", "BBB", "AAA"].iter().map(|s| (*s).to_string()).collect();
        let seq = |syms: &[String]| -> Vec<String> {
            DecideMode::Simultaneous
                .cross_section_order(syms)
                .iter()
                .map(|&i| syms[i].clone())
                .collect()
        };
        assert_eq!(seq(&forward), seq(&reversed));
        assert_eq!(seq(&forward), vec!["AAA", "BBB", "CCC"]);
        // ...and the SEQUENTIAL order is not invariant — the defect, so the assertion above is
        // known to be testing something.
        let seq_seq = |syms: &[String]| -> Vec<String> {
        ...
        assert_ne!(seq_seq(&forward), seq_seq(&reversed));
    }

    /// Whatever the mode, the answer is a PERMUTATION of `0..n` — every symbol visited exactly
    /// once. `StrategyEngine::fill_step_gated` inverts it into a rank table on that assumption
    /// (`r[si] = pos` over every entry, with no fill value left standing), and a duplicated or
    /// missing index there would silently give two symbols the same allocation priority.
    #[test]
    fn every_mode_yields_a_permutation_even_with_a_duplicated_symbol() {
        let dup: Vec<String> =
            ["BTCUSDT", "AAA", "BTCUSDT"].iter().map(|s| (*s).to_string()).collect();
        for mode in [DecideMode::Sequential, DecideMode::Simultaneous] {
            let mut order = mode.cross_section_order(&dup);
            assert_eq!(order.len(), dup.len());
            order.sort_unstable();
            assert_eq!(order, vec![0, 1, 2], "{mode:?} did not yield a permutation");
        }
        // The duplicate keeps LIST position as its tie-break, so the pair stays in the order it
        // was given — a degenerate universe is still ordered deterministically.
        assert_eq!(DecideMode::Simultaneous.cross_section_order(&dup), vec![1, 0, 2]);
    }
}
