...
use vike_model::{
    py_sum, Bar, Broker, FeeSchedule, Fill, FillKind, HftBroker, OrderKind, OrderRequest, Position,
    SessionCalendar, SymbolProperties, Trade, TradeFold, WorkingOrder,
};
...
pub struct EngineParams {
    ...
    /// Opt-in **market-impact slippage model** for the BAR path (see [`crate::impact`]). `None`
    /// (default) keeps the frozen flat-`slippage` cost byte-identical — the wire-in
    ...
    /// average-volume window measured off the symbol's own bars, and the estimate is ADDED to
    /// `slippage`. MAKER fills (`OrderKind::Limit`, including the `tagged` HFT lane) are exempt —
    /// impact is the cost of demanding liquidity, and a resting order supplies it. So this knob
    /// is safe to leave on for a maker strategy: it prices its aggressive legs and leaves its
    /// passive ones alone.
    ///
    /// Deliberately NOT consulted by the vectorized kernel ([`crate::VectorBacktestEngine`]),
    ...
    /// Lookback (in bars) for the rolling `sigma`/`avg_volume` the `impact` model is priced
    /// against; read only when `impact` is `Some`. Defaults to [`DEFAULT_IMPACT_WINDOW`].
...
impl SimBroker {
    ...
    fn atr(&self, si: usize, n: usize) -> f64 {
        ...
        if trs.is_empty() {
            ...
            py_sum(trs.iter().copied()) / trs.len() as f64
        }
    }
    ...
    ///
    /// MAKER FILLS ARE NOT CHARGED IMPACT. `is_maker` returns the flat `slippage` alone, on the
    /// same early-return shape as the `None` arm. Almgren–Chriss temporary impact is the price
    /// concession paid for DEMANDING liquidity; an order that rested and got hit was SUPPLYING
    /// it, and charging it impact would fill a resting limit THROUGH its own limit price — an
    /// impossible fill that silently destroys any maker strategy's edge (the `tagged` HFT lane,
    /// `vike-mm`-shaped quoting, and every `OrderKind::Limit` fill in the gated lane). The flat
    /// `slippage` still applies to makers exactly as it did on the frozen path; only the
    /// size-dependent addend is taker-only.
    ///
    /// With a model configured, the market context (`sigma`, `avg_volume`) is measured over the
    /// last `impact_window` bars of this symbol that CLOSED BEFORE `ts` — the bar `ts` falls
    /// INSIDE is excluded, because its own close and volume are not knowable at the moment the
    /// order prices, and its volume would in part be this very order. The window end is therefore
    /// `partition_point(|b| b.ts <= ts) - 1` (the index of the bar containing `ts`), NOT
    /// `partition_point(|b| b.ts < ts)`: the two agree on the coarse bar lanes, where `ts` is
    /// exactly the landing bar's own `ts`, but only the former holds on the granular sub-bar lane
    /// (`dispatch_fill` is called with `sub.ts`, strictly INSIDE the coarse bar) and on the tick
    /// lane (`event.ts`, later than every seeded bar). That keeps the cost model free of
    /// lookahead on ALL THREE lanes, the same discipline the signal path is held to. On the tick
    /// lane the rule is mildly conservative — with no bar interval to consult, a final seeded bar
    /// that has genuinely closed is still dropped — which errs toward less information, never more.
    ///
    /// A window too short or degenerate to measure ([`crate::impact::window_stats`] returning
    /// `None` — the first bars of a run, a volume-less resampled series, the tick path where
    /// `bars` may be empty) charges the flat slippage alone rather than guessing.
    ///
    ...
    pub(crate) fn slippage_for(&self, si: usize, size: f64, ts: i64, is_maker: bool) -> f64 {
        if is_maker {
        ...
        let Some(stats) = crate::impact::window_stats(&bars[..hi], self.impact_window) else {
            return self.slippage;
        };
        let extra = model.impact_frac(&crate::impact::ImpactInputs {
            qty: size.abs(),
    ...
    pub(crate) fn apply_fill(
        ...
        let slip = self.slippage_for(si, size, ts, is_maker);
