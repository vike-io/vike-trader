//! `sport_taker_run` — drive [`vike_strategy::SportTaker`] over the RAW on-chain Polymarket tape.
//!
...
//!    [`vike_strategy::SignalTracker`] and writes every conviction crossing to `--triggers-out`.
...
use vike_model::{Bar, SymbolProperties, TradeTick, py_sum};
use vike_strategy::sport_taker::{
    CONVICTION_USD, STAKE_USD, SignalTracker, SportTaker, WALLETS, WalletBuy, flat_stake_pnl,
    market_symbol, signal_symbol,
};
