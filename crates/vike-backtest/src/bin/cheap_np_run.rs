//! `cheap_np_run` — drive [`vike_strategy::CheapNp`] over the REAL hist store, window by window.
//!
...
//!
//! ```sh
//! cheap_np_run --from 2026-06-08 --to 2026-06-14 \
//!     --resolutions resolutions.csv [--store DIR] [--poly-venue polymarket] [--spot-venue spot] \
...
//! the datahub, so without that flag this bin discovers its windows and reads its tape over RPC
//! from `$VIKE_DATAHUB_ADDR` (default `127.0.0.1:7878`) rather than opening Parquet itself. Pass
//! `--store DIR` to read local files anyway — when the data daemon is down, or when a
//! whole-catalogue `list_series` plus a per-token scan is not worth the transfer. Node keys come
//! from the credential STORE, not the environment, so a keyed datahub needs nothing exported.
//! The run names the route it took on stderr before it reports a window count, because an empty
//! datahub and an empty directory both report "0 windows".
//!
...
use vike_strategy::fair_value::{SIGMA_LOOKBACK_S, THETA, fee};
use vike_strategy::{CheapNp, CheapNpMode, TokenId};
...
fn run_window(
    ...
    let sts_ms = sts * 1000;
    let res_ms = (sts + UPDOWN_WINDOW_SECS) * 1000;
...
fn main() -> ExitCode {
    ...
    let (Some(from), Some(to), Some(res_path)) =
        ...
        eprintln!(
            "usage: cheap_np_run --from YYYY-MM-DD --to YYYY-MM-DD \
             --resolutions FILE.csv [--store DIR] [--poly-venue polymarket] \
             [--spot-venue spot] [--spot-symbol BTCUSDT] [--mode hold|flip] [--theta 0.055] \
             [--size 1.0] [--sigma-scale 1.0] [--latency-ms 0,1000,5000] [--max-windows N] \
             [--signals-out FILE.csv] [--json]\n\n\
             Without --store, history is read over the wire from the datahub \
             ($VIKE_DATAHUB_ADDR, default 127.0.0.1:7878) — see decision 0084."
        );
    ...
    // opens and deliberately does NOT select the route, the break `history_route` states for the
    // compute daemon and which holds here for the same reason: a flag on the line is a decision
    // somebody made for one invocation, an inherited variable is one nobody remembers making.
    //
    // The ADDRESS ladder is `$VIKE_DATAHUB_ADDR` > `vike_config::DEFAULT_DATAHUB_ADDR`, resolved
    // by that same function, so this bin carries no second copy of the blank-value filter or the
    // default. It is a narrower ladder than `vike-cli`'s and that is not an omission: a bare
    // research bin loads no settings file, so `config.datahub_addr` is not a rung it can reach.
    //
    // `root` is still resolved on BOTH arms, because the Local arm is what opens it: `--store` >
    // `$VIKE_HIST_STORE` > `<repo>/market_data/hist`, where the repo-root default replaces the old
    // CWD-relative `market_data/hist` (run from elsewhere, `DataFusionHist::open` silently CREATED
    // an empty store and the run reported "0 windows").
    let vars: std::collections::HashMap<String, String> = std::env::vars().collect();
    let store_flag = arg(&args, "--store").map(PathBuf::from);
    let route = history_route(store_flag.as_deref(), datahub_addr_for_bin(&vars));
    let root: PathBuf = store_root(store_flag, &vars);
    ...
    // ⚠ Every `list_series`/`scan_*` below goes over the wire on the default arm, and
    // `discover_windows` opens with a WHOLE-CATALOGUE `list_series`. That is the cost 0084 buys
    // the single-reader property with; `--store DIR` is the documented way back to local files.
    let store = match open_routed_history(&route, &root, &vars) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("{e}");
            return ExitCode::FAILURE;
    ...
    // ⚠ The route is DISCLOSED, and it names the ADDRESS on the wire arm rather than a path: the
    // resolved root would be the SERVER's, so a directory printed here is a guess about another
    // box's filesystem. This bin has no JSON provenance field to carry it, so the line is all
    // there is — without it a run against an empty datahub and a run against an empty directory
    // both report "0 windows" and read identically.
    eprintln!("cheap_np_run: history from {}", route.label(&root));

    let mut windows = match discover_windows(store.as_ref(), &poly_venue, from_s, to_s) {
        Ok(w) => w,
        Err(e) => {
    ...
    if windows.is_empty() {
        // ⚠ This used to end "— was the tape ingested with --symbol-key slug?", naming
        // `clickhouse_poly_backfill`'s flag. That bin was RETIRED on 2026-09-19 (it read a local
        // ClickHouse database directly, which the owner's rule forbids) and its module was DELETED
        // on 2026-09-20 with the read-side bridge beside it, taking the `TradeSymbolKey::SlugOutcome`
        // builder with it. It was the ONLY producer of a slug-keyed tape: `vike_archive_backfill`,
        // which replaces its LANES, reads `symbol=<token_id>` per row out of the archive Parquet and
        // has no slug column to key on. So the honest answer to an empty window set is "nothing in
        // this tree writes that shape", not a flag to re-run with, and the shape is not recoverable
        // by reverting anything here. `crates/vike-data/src/backtest_store.rs`'s module doc
        // records the loss — this bin's windows are one of the two things that deletion cost.
        eprintln!(
            "no `{poly_venue}` kind=trade series parse as <slug>#<outcome> in [{from}, {to}] \
             — this store holds no slug-keyed tape, and no tool in this tree writes one: the last \
             producer was clickhouse_poly_backfill, retired 2026-09-19 and deleted 2026-09-20; see \
             crates/vike-data/src/backtest_store.rs"
        );
        return ExitCode::FAILURE;
    }
