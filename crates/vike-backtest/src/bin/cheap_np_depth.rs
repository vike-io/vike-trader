//! `cheap_np_depth` — measure the TRUE resting ask depth available to [`vike_strategy::CheapNp`]
...
//!
//! ```sh
//! cheap_np_depth --entries FILE.csv \
//!     [--store DIR] [--venue polymarket] [--sizes 10,50,100,500,1000] [--theta 0.055] \
...
//! gate all live in [`vike_data::cheap_np_book`] — hoisted out of this bin when
//! `cheap_np_askgate` needed the same book, and moved again into the DATA crate, where reading the
//! archive belongs. Read that module for why a pure delta replay is wrong
...
use vike_backtest::cheap_np_match::{POLYGON_BLOCK_MS, match_clob_print};
use vike_data::cheap_np_book::{Checkpoints, fold_until, verify_checkpoints};
use vike_data::{HistStore, TsRange};
use vike_datahub_client::route::{datahub_addr_for_bin, history_route, open_routed_history};
use vike_model::{BookLevel, BookUpdate, py_sum};
use vike_strategy::cheap_np_ask::price_at_edge;
use vike_strategy::fair_value::{CHEAP_HI, THETA, fee};
...
fn sweep(levels: &[BookLevel], qty: f64, limit: f64) -> (f64, f64, Option<f64>) {
    let (mut filled, mut notional, mut worst) = (0.0, 0.0, None);
    for &BookLevel { price: px, qty: lvl } in levels {
...
fn measure(
    ...
    let asks: Vec<BookLevel> = f.book.top_n(1 << 16).1;
    if asks.is_empty() {
        agg.empty_ask += 1;
        return;
    }
    agg.measured += 1;
    if asks.iter().any(|&BookLevel { qty: q, .. }| q < 0.0) {
    ...
    let cum = |limit: f64| -> f64 {
        py_sum(
            asks.iter()
                .filter(|&&BookLevel { price: px, .. }| px <= limit)
...
fn main() -> ExitCode {
    ...
    let Some(entries_path) = arg("--entries").map(PathBuf::from) else {
        eprintln!(
            "usage: cheap_np_depth --entries FILE.csv [--store DIR] [--venue polymarket] \
    ...
    //
    // `root` is still resolved on BOTH arms, because the Local arm is what opens it and the
    // disclosure below needs something to name: `--store` > `$VIKE_HIST_STORE` >
    // `<repo>/market_data/hist`, where the repo-root default replaces the old CWD-relative
    // `market_data/hist` (run from elsewhere, `DataFusionHist::open` silently CREATED an empty
    // store and the run reported zero entries measured).
    let vars: std::collections::HashMap<String, String> = std::env::vars().collect();
    let store_flag = arg("--store").map(PathBuf::from);
    ...
    let store_label = route.label(&root);
    ...
    let store: Box<dyn HistStore + Send + Sync> = match open_routed_history(&route, &root, &vars) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("{e}");
            return ExitCode::FAILURE;
    ...
    for (i, e) in entries.iter().enumerate() {
        ...
        // "Two clocks" and `cheap_np_match::match_clob_print`).
    ...
    if json {
        ...
        println!("cheap_np resting-depth — {} entries, history from {store_label}", entries.len());
...
mod tests {
    ...
    /// `vike_data::cheap_np_book` / `vike_strategy::cheap_np_ask` (and their tests with them) when
    ...
    fn sweep_respects_the_price_cap_and_clamps_negative_displayed_size() {
        let levels = [
            ...
            BookLevel::new(0.25, 100.0),
            BookLevel::new(0.90, 10_000.0),
        ];
        ...
        let (f, _, _) = sweep(&[BookLevel::new(0.20, -5.0), BookLevel::new(0.21, 7.0)], 100.0, 1.0);
        assert_eq!(f, 7.0);
    }
