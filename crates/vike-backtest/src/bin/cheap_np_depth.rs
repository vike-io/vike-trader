...
use vike_backtest::cheap_np_book::{
    fold_until, match_clob_print, verify_checkpoints, Checkpoints, POLYGON_BLOCK_MS,
};
use vike_backtest::fair_value::{fee, CHEAP_HI, THETA};
use vike_data::{DataFusionHist, HistStore, TsRange};
use vike_model::{py_sum, BookUpdate};
...
fn mean(v: &[f64]) -> f64 {
    if v.is_empty() {
        ...
        py_sum(v.iter().copied()) / v.len() as f64
    }
}
