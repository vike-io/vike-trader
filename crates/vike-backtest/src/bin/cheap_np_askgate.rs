...
//!     [--store DIR] [--venue polymarket] [--theta 0.055] [--size 1] [--delay-ms 0,250] \
...
//! * the edge re-score and the θ-clearing limit are [`vike_strategy::cheap_np_ask`], which
...
use vike_data::cheap_np_book::{Checkpoints, PRICE_GRID, ask_ladder_at, verify_checkpoints};
...
use vike_strategy::cheap_np_ask::{edge_at, price_at_edge, prob_wc};
use vike_strategy::fair_value::{THETA, fee};
...
fn main() -> ExitCode {
    ...
    // store and the run reported zero entries priced).
    ...
    for (i, e) in entries.iter().enumerate() {
        ...
        // The block-vs-CLOB clock anchor (see `cheap_np_match::match_clob_print`).
