//! `backtest --fetch` — pull REAL bars from a venue's public history into the hist store.
//!
...
pub fn parse_spec(raw: &str) -> Result<FetchSpec, String> {
    ...
    if parts.len() != 3 || parts.iter().any(|p| p.is_empty()) {
        return Err(format!(
            "--fetch takes VENUE:SYMBOL:INTERVAL (e.g. binance:BTCUSDT:1h), not {raw:?}"
        ));
    }
...
pub fn window(
    ...
    match (days, from, to) {
        ...
        (None, None, None) => {
            Err("--fetch needs a window: --days N, or --from LABEL --to LABEL".to_string())
        }
        _ => {
            Err("--fetch takes EITHER --days N OR both --from and --to, not a mixture".to_string())
        }
    }
}
