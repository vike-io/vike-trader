...
pub fn store_root(explicit: Option<PathBuf>, vars: &HashMap<String, String>) -> PathBuf {
    ...
    let resolved = resolve_with(explicit, &repo_default(), cwd.as_deref(), vars);
...
mod tests {
    ...
    fn the_call_site_reaches_the_project_rung_when_this_box_has_no_checkout() {
        use vike_model::store_path::{
            StoreRootRung, HOME_VAR, LOCALAPPDATA_VAR, XDG_DATA_HOME_VAR,
        };
        ...
        let got = resolve_with(None, no_checkout, Some(&project), &vars);
    ...
    fn the_call_site_still_prefers_what_was_stated() {
        ...
        assert_eq!(
            resolve_with(Some(PathBuf::from("/x/explicit")), no_checkout, None, &vars).root,
            PathBuf::from("/x/explicit")
        );
        assert_eq!(
            resolve_with(None, no_checkout, None, &vars).root,
            PathBuf::from("/y/env"),
