...
/// `VENUE:SYMBOL:INTERVAL` and it always writes `kind=bar` per symbol — its own module doc says it
/// "has no other shape to express". A suggested command for a grouped or tick series would name a
/// series the fetch cannot write, which is worse than offering nothing.
