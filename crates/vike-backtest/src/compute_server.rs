...
//! server's.** Every `Run*` verb reaches a Rhai compiler — `RunBacktest`/`RunParamscanProfile`/
...
use vike_datahub_client::proto::{
    FEATURE_AUTH, FEATURE_SEARCH_METHOD, FEATURE_STUDY, PROTO_VERSION, Plane, Request, Response,
    VerbScope, WireSearch, WireStudy, plane_of, read_frame_raw, read_frame_raw_capped,
...
pub fn serve_with_studio(
    ...
    serve_authed(listener, store, studio, None, None)
}
            ...
            Ok(request) => match authed {
                Some(scope) => {
                    ...
                    if scope_admits(scope, needed) {
                        handle_request(request, &store, studio, study)
                    } else {
                        let kind = request_kind(&request);
                        tracing::info!(
                            ?peer,
                ...
                None => handle_request(request, &store, studio, study),
            },
