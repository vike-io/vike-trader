...
//! `to_strategy_spec`'s `WireSpec::Rhai` arm — so every one of them is `Scope::Control`, and a
...
//! that record — is `VerbScope::Observe`: it carries `NamedParam`, which has no variant a script
                ...
                Some(scope) => {
                    ...
                    if scope_admits(scope, needed) {
                        ...
                        Response::Error(match needed {
                            ...
                            // reader: `RunNamed` is `VerbScope::Observe`
...
fn handle_request(
    ...
    match request {
        ...
        // The chart-gap seed. Its SCOPE is unlike its neighbours here (`VerbScope::Observe`,
