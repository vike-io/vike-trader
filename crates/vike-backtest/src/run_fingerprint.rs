...
/// on the REMOTE route, where the store is an `Arc<dyn HistStore>` behind `hist-replay` while
/// `backtest_cli` needs `datafusion-store` to compile at all. A copy would have been a second
...
pub struct SeriesFingerprint {
    ...
    /// part that holds each key's rows (`crates/vike-data/src/datafusion_hist/manifest.rs`'s
