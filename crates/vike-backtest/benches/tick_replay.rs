...
use vike_model::{
    Bar, BookLevel, BookUpdate, BookUpdateKind, EquitySample, QuoteTick, Strategy,
    ...
    (0..n)
        .map(|i| {
            ...
            let (kind, bids, asks) = if i == 0 {
                let bids =
                    (0..10).map(|l| BookLevel::new(mid - 0.01 * (l + 1) as f64, 1.0)).collect();
                let asks =
                    (0..10).map(|l| BookLevel::new(mid + 0.01 * (l + 1) as f64, 1.0)).collect();
                (BookUpdateKind::Snapshot, bids, asks)
            } else {
                let bids = vec![
                    BookLevel::new(mid - 0.01, 1.0 + (i % 5) as f64),
                    BookLevel::new(mid - 0.02, 2.0),
                ];
                let asks = vec![
                    BookLevel::new(mid + 0.01, 1.0 + (i % 3) as f64),
                    BookLevel::new(mid + 0.02, 2.0),
                ];
