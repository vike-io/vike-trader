...
use serde_json::json;
use vike_backtest::{metrics, overfit, EngineParams, SimBroker, StrategyEngine};
...
use crate::ledger::{
    append_learning, append_trial, load_learnings, load_trials, now_ms, prior_work_summary,
    Learning, LearningScope, LedgerPaths, Trial, PROMPT_TOP_K,
};
        ...
        match RhaiStrategy::<SimBroker>::compile(&code) {
            ...
            Ok(strat) => {
                ...
                if res.n_trades == 0 {
                    ...
                    r.accepted = true;
                    r.oos_sharpe = metrics::sharpe(&res.equity_curve, PPY);
...
/// (`metrics::sharpe`, PPY=252) — a different quantity, for display only, that must never reach
/// `overfit::`. Using `sharpe_moments` for BOTH the observed value AND every trial keeps the
