...
fn remote_hist_store(addr: String, key_name: &str) -> vike_datahub_client::RemoteHistStore {
    let (keys, notices) = vike_app_core::backend_registry::datahub_observe_keys(
...
impl App {
    ...
    fn new(
        ...
        let (
            ...
            //
            //  * `App::direct_bars` STAYS `None`. `split_plane::direct_bars_mount(ObserveOnly)` is
            //    false and `core_sync` takes `direct_bars.is_some()` AS the render-source mode
            //    signal, so binding it `Some` would reassign every kline series on the five CEX
            //    venues to a store nothing fills. This wire serves no bar lane at all (design
            //    §10), and the sink's own bar store is private to it and never written.
