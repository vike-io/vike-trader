...
use vike_app_core::backfill_plan::{BackfillJob, plan_backfill_jobs};
...
// and were then DELETED outright on 2026-09-15 — the DOM no longer fabricates a ladder or a price
// when it has no book, it says so; `vike_app_core::dom_math`'s module doc carries the argument.
...
use vike_app_core::series_follow;
...
const BAR_PLANE: vike_app_core::split_plane::BarPlane =
    vike_app_core::split_plane::bar_plane(vike_app_core::split_plane::AppMode::ObserveOnly);
...
struct App {
    ...
    // Read by three surfaces, all in `vike_app_core::series_follow`: the adoption planner, the
    // symbol picker's backend section, and the title-bar badge.
    published_series: Vec<vike_app_core::series_follow::PublishedSeries>,
    ...
    // THE MARKET-DATA SESSION (`vike_app_core::md_session`): one connection to the BACKEND's
    ...
    md_session: std::sync::Arc<vike_app_core::md_session::MdSession>,
    ...
    // read out of the BACKEND'S OWN hist store by `vike_app_core::store_bars` — the only way a
    ...
    // (`vike_app_core::chart_seed::render_chart_seed_status`), written by the background read
    ...
    /// (`vike_app_core::catalog_refresh`). It holds the SENDER half of `catalog_rx`, so a refresh
    ...
    catalog: Arc<vike_app_core::catalog_refresh::CatalogRefresh>,
...
/// ⚠ **The BODY moved to `vike_app_core::backend_registry::remote_hist_store`** — beside
...
/// (`vike_app_core::store_bars`) dials identically without importing this binary. What is left
...
fn remote_hist_store(addr: String, key_name: &str) -> vike_datahub_client::RemoteHistStore {
    vike_app_core::backend_registry::remote_hist_store(
        addr,
...
/// environment (`crates/vike-ops/tests/settings_registry.rs`), and the resolution behind it opens
...
/// `crate::remote::connect_observe` puts straight into the Studio's error banner. Re-surfacing the
/// same notice twice per frame would be noise, and this is called once per Studio construction.
fn named_run_observe_keys(key_name: &str) -> Option<vike_datahub_client::NodeKeys> {
    vike_app_core::backend_registry::datahub_observe_keys(
        SETTINGS_DIR.get().and_then(Option::as_deref).and_then(std::path::Path::to_str),
...
/// (`crates/vike-ops/tests/settings_registry.rs`), so the SWEEP belongs in the composition root even
/// though everything it feeds lives one crate down in `vike_app_core::chart_seed`.
fn chart_seed_dial(
    addr: String,
    key_name: &str,
    note: std::sync::Arc<std::sync::Mutex<Option<String>>>,
) -> vike_app_core::chart_seed::SeedDial {
    vike_app_core::chart_seed::SeedDial::resolve(
        addr,
        SETTINGS_DIR.get().and_then(Option::as_deref).and_then(std::path::Path::to_str),
        PROCESS_ENV.get_or_init(|| std::env::vars().collect()),
        key_name,
...
/// (`crates/vike-ops/tests/settings_registry.rs`), so the SWEEP belongs in the composition root
/// even though every rule it feeds lives one crate down in `vike_app_core::catalog_wire`.
///
...
fn catalog_dial(addr: String, key_name: &str) -> vike_app_core::catalog_wire::CatalogDial {
    vike_app_core::catalog_wire::CatalogDial::resolve(
        addr,
        SETTINGS_DIR.get().and_then(Option::as_deref).and_then(std::path::Path::to_str),
        PROCESS_ENV.get_or_init(|| std::env::vars().collect()),
        key_name,
...
impl App {
    ...
    fn new(
        ...
        let (
            ...
            // constructor since the `fat` deletion — so the DOM ladder, the trade tape and the
            ...
            //    filled by `vike_app_core::store_bars`' reads of the BACKEND's own hist store —
            ...
            let md_mount = vike_app_core::md_session::build(
                SETTINGS_DIR.get().and_then(Option::as_deref).and_then(std::path::Path::to_str),
                // ⚠ `PROCESS_ENV`, NOT `vars`. `vars` is `workspace_credentials()` — the VENUE
                // store (`secrets.env`) plus the two TRADEHUB node keys gap-filled from the
                ...
                PROCESS_ENV.get_or_init(|| std::env::vars().collect()),
                vike_app_core::backend_registry::datahub_observe_key_name(observe_backend),
                books.clone(),
                trades.clone(),
                {
        ...
        let catalog = Arc::new(vike_app_core::catalog_refresh::CatalogRefresh::new(
            ...
            vike_model::state_path::bin_dir_beside(SETTINGS_DIR.get().and_then(Option::as_deref))
...
// `crates/vike-app-core/src/workspace/title_bar.rs`'s `tool_title_bar`. It moved for the reason
// that module's own doc argues: a title bar that now RESERVES the Connections window's tab slot is
// making geometry decisions, and this crate is in `xtask::ci::tables`' `EXCLUDE_FROM_CI`, so every
// one of them was gated by nothing. `crates/vike-app-core/tests/connections_tabs.rs`'s
// `the_tab_slot_lives_inside_the_title_bar_and_ends_on_its_hairline` runs them. The
...
fn tool_content(
    ...
    // tool's segmented control. Only the Connections arm reads it; every other kind answers
    // `WinKind::carries_title_tabs() == false` and the slot is inert.
    ...
    catalog: &vike_app_core::catalog_refresh::CatalogRefresh,
) {
    ...
    match kind {
                    ...
                    .show(ui.ctx(), |ui| {
                        ...
                        ui.horizontal(|ui| {
                            let (fill, label) = if ticket.side > 0 {
                                (palette::UP, "Confirm Buy")
                            } else {
                                (palette::DOWN, "Confirm Sell")
                            };
                            if ui
                                .add(
                                    egui::Button::new(
        ...
        // ⚠ The CHECKED read, not the plain one: this tool RENDERS A COUNT of what the store
        // holds, and the plain loader's empty map is the same for an absent store and one that
        // would not open. One read, both answers — see `workspace_credentials_checked`.
        Connections => {
            let (creds, health) = workspace_credentials_checked();
            // ⚠ **The one place this box's credential set reaches the Instruments screen**, and it
            // is here rather than at `App::new` because credentials are EDITED on this very screen:
            // a set resolved once at startup would tell an operator who had just saved their alpaca
            // keys that no keys were saved (`docs/decisions/0066` decision 9's arming). This arm
            // already performs the FRESH read — the comment above says only this arm pays for it —
            // so the push costs one derivation over a map already in hand, and the set is
            // `credential_status`' own, which is what the grid beside it renders.
            catalog.set_credentialed_venues(vike_connections::credentialed_venues(&creds));
            tool_views::connections_tool_content(
                ui,
                &tool_ctx(),
                tv,
                &creds,
            ...
            if studio.is_none() && studio_error.is_none() {
                match open_studio_store(resolved_datahub_addr.map(str::to_string), datahub_key_name)
                {
                    ...
                    Ok((store, remote_addr)) => {
                        ...
                        // holds for its store and market-data planes, and deliberately never a
                        // Control one — `vike_app_core::backend_registry`'s `datahub_observe_key`
                        // states why this process must not hold the key that compiles
...
enum Dispatch<'a> {
    ...
    /// `vike_app_core::tradehub_control::may_send_to_backend`, one crate down, where CI runs it.
...
impl Dispatch<'_> {
    ...
    fn send(&self, cmd: vike_exec::Command) {
        match self {
            ...
            // ⚠ **The client-side ROUTING gate fires here**, at the one choke point rather than at
            // each button, and it has to fire before the write: a backend that does not advertise
            ...
            // error anywhere — so there is no reply to check afterwards. The DECISION and its log
            // line are `may_send_to_backend`, one crate down, where CI runs them; what stays here
            // is the two facts only this binary holds.
            Dispatch::Remote(ctrl, snap) => {
                match vike_app_core::tradehub_control::wire_from_command(&cmd) {
                    Some(wire) => {
                        if !vike_app_core::tradehub_control::may_send_to_backend(
                            &wire,
                            snap,
...
fn install_visuals(ctx: &egui::Context) {
    ...
    let bg = palette::BG; // hsl(215,28,7)
    let surface = palette::SURFACE;
    let hover = palette::HOVER;
    let border = palette::BORDER;
    let text = palette::TEXT_UI;
    let text2 = palette::TEXT2;
...
fn status_bar(
    ...
    ui.horizontal_centered(|ui| {
        ...
        if let Some(control) = control {
            ui.add_space(12.0);
            ui.label(RichText::new("│").color(palette::BORDER).size(12.0));
            ui.add_space(12.0);
...
// written — was referenced by nothing in the tree. `vike_app_core::catalog_refresh::CatalogRefresh`
...
/// **The credential store: `<project>/settings/secrets.env`, plus the NODE store beside it.**
///
...
///
/// ⚠ **The two TRADEHUB node keys do NOT come out of the venue file above** — they come from
...
/// resolution is `vike_app_core::backend_registry::fill_node_keys`, one crate down where CI runs
...
-> (HashMap<String, String>, vike_bridge_core::credentials::StoreHealth) {
    ...
    vike_app_core::backend_registry::fill_node_keys(
        &mut creds,
        SETTINGS_DIR.get().and_then(Option::as_deref).and_then(std::path::Path::to_str),
        env,
    );
...
fn main() -> eframe::Result<()> {
    ...
    match vike_app_core::workspace::persist::declare_project_state_dir(booted.state_dir.clone()) {
        Ok(Some(moved)) => tracing::warn!("{moved}"),
        Ok(None) => {}
        Err(e) => tracing::error!("{e}"),
    }

    // …and the STUDIO's sweep pool takes this process's resolved `preferences.sweep_threads`.
    //
    // The Studio's `Backend::Local` sweep enters `vike_backtest::harness`'s BOUNDED rayon pool
    // through `vike_studio_core::run_paramscan_slice` -> `map_bounded`, and that pool reads its
    // worker count at CONSTRUCTION, deep inside a fan-out no caller here can reach — so the number
    // arrives through a process-wide handle a root installs, never as a parameter
    // (`vike_backtest::harness::install_sweep_threads` carries the argument; `vike-studio` re-exposes
    // it because this binary does not link vike-backtest). Until this call existed only
    // `VIKE_SWEEP_THREADS` capped the GUI's sweeps and the FILE key was inert —
    // `docs/decisions/0057-the-seven-settings-files-answered-one-at-a-time.md`'s Phase 0 names that
    // as owed. Here rather than in `App::new` for the same reason as the two declarations above:
    // one root, one installation, before anything can start a sweep.
    vike_studio::install_sweep_threads(settings().preferences.sweep_threads);
    ...
    // unreachable now; it is left standing because `observe_backend` is still an `Option` and its
    ...
    // ⚠ THE NAME LEADS, and the address is the parenthetical. A window title is the one identity
    // surface an operator reads with two production boxes side by side in a taskbar, and both
    // the CI box listeners bind loopback — so `OBSERVING 127.0.0.1:7879` was byte-identical on every
    // box. `vike_app_core::backend_identity` is the one authority for the spelling (an unnamed
    // record renders as `unnamed backend (…)`, never as an invented one).
    let window_title = match &observe_backend {
        Some(record) => {
            format!("Vike Trader — OBSERVING {}", vike_app_core::backend_identity::one_line(record))
        }
