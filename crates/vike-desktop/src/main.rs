...
type StoredLoad = vike_app_core::stored_load::StoredLoadOutcome;
...
/// a remote `Scope::Write` channel is mounted. Both guards are the `VIKE_DOM_TESTORDER` block's,
            ...
            if studio.is_none() && studio_error.is_none() {
                match open_studio_store(resolved_datahub_addr.map(str::to_string), datahub_key_name)
                {
                    ...
                    Ok((store, remote_addr)) => {
                        ...
                        // read from. So the key comes from the environment, `crate::plugin_build`
                        // names the variable when it is absent (from that service's own constant,
                        // never a second spelling), and widening it is a decision with a record
