...
pub(crate) fn draw_chrome(app: &mut App, ui: &mut egui::Ui, ctx: egui::Context) {
    ...
    egui::Panel::top("caption").frame(egui::Frame::NONE).show_separator_line(false).show(
        ...
        |ui| {
            ...
            ui.painter().rect_filled(cap, 0.0, palette::BG);
            ...
            ui.scope_builder(
                ...
                |ui| {
                    ...
                    ui.painter().rect_filled(br, 5.0, palette::ACCENT);
                    ui.painter().text(
                        br.center(),
            ...
            ui.scope_builder(
                ...
                |ui| {
                    ...
                    // hsl(148,50,42)≈(54,161,104); radius 6, 12px. (egui's default focus ring is blue.)
                    {
                        ...
                        v.extreme_bg_color = palette::SURFACE; // RAISE field bg
    ...
    // bounds it lives with the windows, in `vike_app_core::workspace`'s `WinState::arena_bounded`.
...
pub(crate) fn draw_windows(app: &mut App, ctx: egui::Context, snap: FrameSnapshot) {
    ...
    let mut to_ensure: Vec<(String, String, String, Option<vike_model::AssetClass>)> = Vec::new();
        ...
        };
    {
        ...
        // What the connected node publishes — a disjoint field from `app.wins` below, bound here
        // beside `charts` for the same split-borrow reason. Read-only inside the loop; it decides
        ...
        for (wi, w) in app.wins.iter_mut().enumerate() {
            ...
            if w.kind == workspace::WinKind::Chart {
                ...
                if workspace::show_window(&ctx, w, desktop, |ui, _bounds| {
                    ...
                    series_follow::paint_empty_hint(ui, feed, chart_seed_note.as_deref());
                ...
                if let Some(s) = act.new_symbol {
                    ...
                    w.venue = act
                        .new_venue
                        .take()
                        .unwrap_or_else(|| workspace::DEFAULT_VENUE.to_string());
                ...
                if workspace::show_window(&ctx, w, desktop, |ui, bounds| {
                    // ⚠ `tabs` is the rect this bar RESERVED between the window title and its
                    // `─ □ ✕` controls for a tabbed tool's segmented control (today: Connections).
                    ...
                    egui::Frame::new().inner_margin(workspace::TOOL_BODY_MARGIN).show(ui, |ui| {
                        ...
                        // dispatches the two `fills` TOOL kinds — DOM and Polymarket — and for
                        ...
                        // would discard every egui-persisted widget state inside the DOM ladder
                        ...
                        bounds.show(ui, |ui| {
                            tool_content(
                                ...
                                &tool_views::BackendPicker {
                                    ...
                                    reported: vike_app_core::backend_identity::SelfReport::of(
                                        daemon_self_report
                                            .as_ref()
                                            .map(|id| id.advertise_addr.as_str()),
                                    ),
                                },
                                &mut backend_action,
                                &mut backend_editor,
                ...
                if kind == workspace::WinKind::Polymarket {
                    ...
                    let up_px = dom_book.as_ref().and_then(|b| b.best_ask()).map(|l| l.price);
                    let dn_px = dom_book.as_ref().and_then(|b| b.best_bid()).map(|l| 1.0 - l.price);
    ...
    // and the argument for how narrow it is — is `vike_app_core::series_follow::follow_backend`;
    ...
    // is `vike_app_core::store_bars::plan_store_reads`; what belongs here is the wiring: the
    ...
    if let (Some(addr), Some(bars)) = (resolved_datahub_addr.clone(), app.direct_bars.clone())
        && !reads.is_empty()
    {
        for r in &reads {
        ...
        // to fetch. The whole rule and every message is `vike_app_core::chart_seed`; what belongs
        ...
        // this binary can never send to a keyed server because it may not hold a control key.
        let dial = chart_seed_dial(addr.clone(), &datahub_key_name, app.chart_seed_note.clone());
        store_bars::spawn_store_seed(
            reads,
            remote_hist_store(addr, &datahub_key_name),
            bars,
    ...
    // `vike_app_core::tradehub_control::may_send_to_backend`, applied in `Dispatch::send`.
    let dispatch = match (&app.core, remote_ctrl) {
        (Some(core), _) => Some(Dispatch::Local(core)),
        (None, Some(ctrl)) => Some(Dispatch::Remote(ctrl, &core_snap)),
        (None, None) => None,
    };
    if let Some(dispatch) = dispatch {
        ...
        // Scope::Write channel exists. `app.live_venues` is ALWAYS EMPTY in observe mode, so
        ...
        if app.dom_test_pending && app.shot_n >= 320 && app.remote_ctrl().is_none() {
            ...
            if let Some((venue, canonical)) = target {
                ...
                if app.live_venues.contains(vstr) {
                    ...
                    && let Some(vike_model::BookLevel { price: bid, .. }) = book.best_bid()
                {
