...
impl App {
    /// The active backend's Scope::Write write channel, if any — the read every command/status
    ...
    pub(crate) fn apply_backend_action(
        ...
        backend_conn::switch_backend(
            ...
            backend_conn::SwitchSlots {
                ...
                published: &mut self.published_series,
        ...
        // Another backend is another STORE: `clear_session_state` emptied the bar store under
        // `BarPlane::BackendStore`, so the record of what was already asked for must go with it or
        // B's charts would never be fetched at all.
        self.store_asked.clear();
        // ...and the seed note with it: it names a SWITCH on the server that was just swapped out,
        // so carrying it over would send the operator to the wrong box.
        if let Ok(mut n) = self.chart_seed_note.lock() {
            *n = None;
        }
    }
    ...
    pub(crate) fn ensure_feed_on(
        ...
        symbol: &str,
        interval: &str,
        asset_class: Option<vike_model::AssetClass>,
    ) {
    ...
    /// per-frame either.
    ///
    /// ⚠ **This doc used to end "rather than leaving the UI stuck on 'Loading…' forever", and that
    /// clause was the defect rather than a description of it.** Only a FAILURE degraded; a HANG did
    /// not, because the thread spawned below is detached and there was nothing to time it out —
    /// a walk that never returned never sent, `stored_loading` never cleared, and the window sat on
    /// *Loading stored data…* for the life of the process, with nothing logged at any level to say
    /// which of "hung", "empty" or "slow" it was. The bound and the boundary logging now live in
    /// `crates/vike-app-core/src/stored_load.rs` — beside the degrade contract they make keepable,
    /// and where CI can RUN them, which this crate's one `app-check` job cannot. This body is the
    /// thread and the send, and deliberately nothing else.
    pub(crate) fn refresh_stored(&mut self, ctx: &egui::Context) {
        ...
        if let Some(addr) = mode.remote_addr {
            ...
            // A retry must not keep showing the PREVIOUS attempt's reason beside a fresh spinner —
            // the load about to run may well succeed, and a stale reason under "Loading…" reads as
            // the new attempt having already failed.
            self.stored_error = None;
            let tx = self.stored_tx.clone();
            let c = ctx.clone();
            // ⚠ Resolved HERE, on the frame thread, and moved in — not inside the closure. The name
            // is a read of `self.active_backend`, which the spawned thread cannot borrow; taking it
            // beside the address is also what keeps the two from being read at different instants
            // (see `App::datahub_observe_key_name`). Resolving the key VALUE stays on the worker,
            // where the file read belongs.
            let key_name = self.datahub_observe_key_name();
            std::thread::spawn(move || {
                // `Arc<dyn HistStore + Send + Sync>` rather than the plain value this used to hand
                // over: `load_stored` runs each RPC on a helper thread it may ABANDON, which a
                // borrow could not outlive. That is the one call-site consequence of bounding the
                // walk at all, and `RemoteHistStore` is built for it (it holds an address and a
                // key — its own module doc names this exact `Arc` as the seam it drops into).
                let store: std::sync::Arc<dyn vike_data::HistStore + Send + Sync> =
                    std::sync::Arc::new(remote_hist_store(addr, &key_name));
                // ⚠ The LABEL, never the address. A host, a port or a path in a log line is a
                // string `scripts/publish_mirror.sh`'s forbidden-token scan has to chase, and a
                // machine name that reaches a shipped artifact fails a gate at TAG time — where the
                // only remedy is re-cutting the tag. The kind is the half an operator needs anyway.
                let load = vike_app_core::stored_load::load_stored(
                    &store,
                    "datahub",
                    vike_app_core::stored_load::RPC_DEADLINE,
                );
                let _ = tx.send(load);
                c.request_repaint();
            });
    ...
    /// [`vike_app_core::backfill_plan::plan_backfill_jobs`] (each selected series' known
    ...
    ///
    /// ⚠ The PLAN no longer depends on that route — 0059 Phase 3 deleted this build's own venue
    /// roster, so `kind == "bar"` is the whole client-side gate and venue support is the server's
    /// question on both arms. The route picks the executor and nothing else:
    ///
    /// - **unset** → [`Self::spawn_stored_backfill_local`]: a TOMBSTONE. It ran `vike-backfill`'s
    ///   keyless kline backfillers into a LOCAL store under `fat`; that feature and that store are
    ///   gone, so it reports the missing engine in the status slot and runs nothing.
    ...
    pub(crate) fn maybe_spawn_stored_backfill(
        ...
        // The PLAN is route-independent since 0059 Phase 3 deleted the GUI's own venue roster
        // (`vike_app_core::backfill_plan`'s module doc carries why); `route` below still picks the
        // EXECUTOR.
        let (jobs, skipped) =
            plan_backfill_jobs(&selected, &self.stored_gaps, now_ms, DEFAULT_LOOKBACK_MS);
        if jobs.is_empty() {
