...
//! to app-local types (`TitleActions`, the `SYMS`/`IVLS`/… constants, `font::`, `palette::`), so it
...
use vike_app_core::workspace::DEFAULT_VENUE;
...
pub(crate) fn title_bar(
    ...
    backend: &[vike_app_core::series_follow::PublishedSeries],
    // What the badge may claim — `● LIVE` only when bars have actually arrived.
    feed: vike_app_core::series_follow::ChartFeed,
        ...
        |ui| {
            ...
            egui::Popup::menu(&bresp).id(id.with("style_popup")).show(|ui| {
                ...
                for (sec, styles) in chart::STYLE_SECTIONS {
                    ...
                    for &st in *styles {
                        ...
                        if rresp.hovered() {
                            ui.painter().rect_filled(rr, 4.0, palette::HOVER);
                        }
                        ...
                        let tcol = if style == st { palette::ACCENT } else { palette::TEXT_UI };
                        ui.painter().text(
            ...
            egui::Popup::menu(&sresp).id(id.with("sym_popup")).show(|ui| {
                ...
                egui::ScrollArea::vertical().max_height(300.0).show(ui, |ui| {
                    // THE BACKEND'S OWN SERIES, first and unfiltered by the search box: this is
                    // the only place in the GUI a node's venue is reachable at all (see the
                    // `backend` parameter), and it is short by construction — one row per mounted
                    // series. Picking one takes the venue AND the interval, unlike a catalog hit.
                    if !backend.is_empty() {
                        ui.weak("  On this backend");
                        for s in backend {
                            let cur =
                                s.venue == venue && s.symbol == symbol && s.interval == interval;
                            if ui.selectable_label(cur, s.label()).clicked() {
                                a.new_symbol = Some(s.symbol.clone());
                                a.new_venue = Some(s.venue.clone());
                                a.new_interval = Some(s.interval.clone());
                            }
                        }
                        ui.separator();
                    }
            ...
            // (`main.rs`'s `of_wanted` collection). Label tints `palette::ACCENT` while either toggle
            ...
            // `WinState::add_compare`) so the overlay is visible. Label tints `palette::ACCENT`
            ...
            let tone = if feed.is_live() { palette::UP } else { ui.visuals().weak_text_color() };
            ui.label(RichText::new(feed.badge()).color(tone).size(12.0)).on_hover_text(feed.hint());
        },
    );
