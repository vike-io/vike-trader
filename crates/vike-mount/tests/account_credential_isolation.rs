//! **Every venue `vike_mount`'s `arm_addresses_accounts` says YES to reads ITS OWN account's
//! credentials** — the gate behind that answer, and the one that fails if a bespoke loader stops
//! being account-aware.
//!
//! ⚠ That list now carries every `vike_model::VENUES` id, so "every venue it says yes to" is the
//! whole roster; the three that are covered somewhere else rather than here are named at the end of
//! this doc, each with the reason it cannot be driven through this projection.
//!
...
//!   the vars map at all. Its arm resolves an account from the settings database's `account` table
//!   (`vike_mount`'s `dukascopy` module: the row's credential-key OWNER PREFIX picks the broker),
//!   so the isolation property here — *a labelled account never reads the default account's keys* —
//!   is held by that module's own unit tests, which drive the resolution over planted `Accounts`
//!   and `AccountKeys` values and assert that an account the store cannot identify is REFUSED
//!   rather than coerced onto `DUKASCOPY_DEMO1_*`. The projection half is
