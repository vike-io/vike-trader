...
///
/// ⚠ This is paid in full on every refusal, so it is the wall-clock cost of this file. It must
/// still be long enough that "the CI box was busy" cannot masquerade as "refused" — `fake_run` emits
/// its marker synchronously the instant a command lands, so the only thing being waited on is a
/// thread wake-up.
///
/// ⚠ It used to say this was "paid in full on every refusal (a blocked `modify` produces no event
/// at all, by design)", and BOTH halves have stopped being true. Only the two `modify` rows were
/// ever event-free — an opening submit's refusal has always sent a synthesized `OrderRejected` down
/// this same lane — and since kill-switch gap 5 closed, a halted modify sends a non-terminal
/// `OrderModifyRejected` too. Neither is in the admitted set, so both rows still resolve to
/// `Refused`, just without the wait. **No row in `CASES` reaches this bound today**, and it is kept
/// rather than shortened precisely because that is a property of the clients rather than of this
/// harness: a client that went back to silence must still be measured as refused and not as slow.
...
/// **Kill-switch gap 5, closed and pinned across the seam it was open on.**
///
/// A blocked `modify` used to emit NOTHING on both of these clients, so it was
/// byte-indistinguishable from a modify the adapter had lost: same absence of events, same resting
/// order at its old terms. An operator amending under an engaged halt could not tell a working kill
/// switch from a wedged one, which is a bad thing for the operator to be unsure about while holding
/// the switch. cTrader was the one client of four that said so.
///
/// Both now emit exactly one NON-terminal `OrderModifyRejected` carrying the SHARED
/// `HALT_REJECT_REASON` — shared so a single recogniser (a GUI row, a journal grep, a test) matches
/// every enforcing client. This belongs beside the verdict table rather than in either crate's own
/// tests for the reason the whole file exists: the two must not drift, and each crate can only ever
/// assert about itself.
///
/// ⚠ It asserts the ADVISORY only. The VERDICT — refused, with no reducing exemption on either
/// client — is `CASES`' two `modify` rows above and stays their job; a client that started ADMITTING
/// halted modifies while emitting this event would pass here and fail there.
///
/// ⚠ Mutation proof: delete either `push_back`/`blocking_send` of the advisory and the matching half
/// goes red naming which client went quiet.
#[test]
fn a_halted_modify_is_refused_out_loud_on_both_clients() {
    // LIVE — the shared `ExecActor` boundary eleven venues inherit.
    let (_live_dir, live_sentinel) = unique_sentinel("advisory-live");
    std::fs::write(&live_sentinel, b"").expect("engage the sentinel");
    let (events, mut rx) = event_channel(16);
    let mut live =
        ExecActor::spawn("advisory-test", events.clone(), move |rx| fake_run(events, rx))
            .with_halt_path(live_sentinel.clone());
    live.submit(&order("advisory-1", false));
    let _ = drain_ingest(&mut rx);
    live.modify(&order("advisory-1", false), Some(2.0), Some(101.0));
    let live_events = drain_ingest(&mut rx);
    match live_events.as_slice() {
        [Event::OrderModifyRejected(m)] => {
            assert_eq!(m.client_order_id, "advisory-1");
            assert_eq!(m.reason, vike_bridge_core::halt::HALT_REJECT_REASON);
        }
        other => panic!(
            "the shared ExecActor boundary must SAY it refused the amend — silence here is a \
             blocked modify wearing the costume of a lost one. Got: {other:?}"
        ),
    }

    // PAPER — the book an operator rehearses the switch against.
    let (_paper_dir, paper_sentinel) = unique_sentinel("advisory-paper");
    let mut paper = PaperExecutionClient::with_fee_schedule(VENUE, SYMBOL, 0.0, FeeSchedule::Free)
        .with_halt_path(paper_sentinel.clone());
    paper.submit(&order("advisory-1", false));
    while paper.poll_events().is_some() {}
    std::fs::write(&paper_sentinel, b"").expect("engage the sentinel");
    paper.modify(&order("advisory-1", false), Some(2.0), Some(101.0));
    let mut paper_events = Vec::new();
    while let Some(e) = paper.poll_events() {
        paper_events.push(e);
    }
    match paper_events.as_slice() {
        [Event::OrderModifyRejected(m)] => {
            assert_eq!(m.client_order_id, "advisory-1");
            assert_eq!(
                m.reason,
                vike_bridge_core::halt::HALT_REJECT_REASON,
                "the same bytes as the live client above — that is the point of asserting them here"
            );
        }
        other => panic!(
            "the paper mount must say it too, or the rehearsal teaches that a halted amend goes \
             quiet when the live one does not. Got: {other:?}"
        ),
    }
}
