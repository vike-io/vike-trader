...
/// The hard ceilings [`crate::make_engine`] applies — plus ONE fact that is not a ceiling and had
/// nowhere else to ride, [`Self::accounts`], which carries its own argument.
///
...
/// let a caller give the two sides different tables.
///
...
pub struct MountPolicy {
    ...
    /// **The settings database's `account` table, as the composition root read it** — and the ONE
    /// field of this struct that is not a ceiling.
    ///
    ...
    /// resolve a dukascopy account IDENTICALLY (`crate::dukascopy`: the row's credential-key owner
    /// prefix decides which LEGAL ENTITY an order reaches), and one snapshot on the object they both
...
mod tests {
    ...
    fn a_policy_per_order_ceiling_reaches_nothing_the_mount_carries() {
        let bare =
            |n| MountPolicy::from(&Policy { max_notional_per_order: n, ..Policy::default() });
        assert_eq!(
            bare(Some(1.0)),
