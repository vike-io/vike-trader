...
pub fn make_engine_for_account(
    ...
    let client: Box<dyn vike_exec::ExecutionClient + Send> = match (venue, creds) {
        ...
        ("dukascopy", _) => {
            // ⚠ DEMO1 ONLY, and the restriction is deliberate rather than unfinished.
            //
            // DEMO1 is Dukascopy Bank SA, DEMO2 is Dukascopy Europe IBS AS — two different brokers
            // with two different credential sets, which is why the bridge models them as an enum.
            // Mapping the venue-account LABEL onto that enum looks like a one-line convenience and
            // is not: `crates/vike-mount/src/arming.rs`'s `arm_addresses_accounts` refuses this
            // venue for TWO independent reasons, and only the first ("no live arm at all") is what
            // this change removes. The second stands — the `DUKASCOPY_DEMO1_*` shape bakes an
            // account INDEX into the TIER token, `vike_model::account_keys` pins it as
            // non-conforming ON PURPOSE, and that file states in its own words that reconciling the
            // two multi-account spellings is "a design question rather than a mechanical port".
            //
            // So this arm mounts the DEFAULT account against DEMO1 and addresses no second account.
            // A box wanting DEMO2 today points `DUKASCOPY_DEMO1_*` at those credentials. Deciding
            // the addressing properly is its own change, with the account-key question settled
            // first — not something a mount arm should quietly answer.
            match vike_dukascopy::load_dukascopy_config_from(
                vike_dukascopy::DukascopyAccount::Demo1,
                vars,
            ) {
                Some(cfg) => {
                    // `<project>/bin`, derived from the state directory the boot already resolved.
                    let project_bin = vike_bridge_core::halt::declared_project_state_dir()
                        .and_then(|state| {
                    ...
                    let tools =
                        vike_dukascopy::resolve_dukascopy_tools(vars, project_bin.as_deref());
                    match vike_dukascopy::DukascopyExecutionClient::spawn(
                        ...
                        Ok(client) => {
                            ...
                            tracing::warn!(
                                venue,
                                "dukascopy: DEMO credentials present and the JForex sidecar started \
                                 → LIVE exec client (real demo-account orders)"
                            );
                            // Derived from the client, so it must be built BEFORE the client moves
                            // into the box — the sidecar owns the one connection both share.
                            let rc = client.recon_client();
                            recon = recon_if_enabled(recon_enabled, || Some(Box::new(rc)));
                            Box::new(client)
                        }
                        Err(e) => {
                            // Includes the absent-jar case, which `spawn` has already logged with
                            // the path it looked for. Staying paper is the live gate doing its job.
                            tracing::error!(
                                venue,
                                error = ?e,
