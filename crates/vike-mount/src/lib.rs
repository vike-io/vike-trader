...
use arming::{
    ...
    cex_mainnet_enabled, margin_mode_grid, multiplier_grid, report_unaddressable_accounts,
...
pub fn make_engine_accounts(
    ...
    report_unaddressable_accounts(vars, policy);
    ...
    let rows = venue_account_arming(venue, vars, policy);
    ...
    let report = vike_config::shared_book_report(
        shared_books_for(venue, &rows, vars, policy),
...
pub fn make_engine_for_account(
    ...
    if account_arming_under(venue, account, vars, mode, policy).0 != vike_config::VenueMode::Paper {
    ...
    let client: Box<dyn vike_exec::ExecutionClient + Send> = match (venue, creds) {
        ...
        ("hyperliquid", _) => {
            match hyperliquid::hyperliquid_live_client(
                ...
                live_permitted,
                // …and the `account` table, for the ONE thing this arm can do that no other venue's
                // can: its `userRole` probe is a real handshake, so when the venue names the book
                // this mount trades, the operator is told whether the store records it — and, when
                // it does not, the exact `vike-cli secrets set-book` line that would. The arm
                // REPORTS; only the composition root may write the store.
        ...
        ("dukascopy", _) => {
            ...
            //
            // WHICH account is decided by `crate::dukascopy::resolve_account`, keyed on the settings
            // database's `account` row — its credential-key OWNER PREFIX picks the broker, its
            // `venue_account_id` is what an operator writes to address it, and its `label` is used
            // only if somebody wrote one (the owner refused the migration's provisional labels
            // twice). That module's doc carries the whole argument, including why
            // `AccountLabel::Default` is `Demo1` on every box whatever the database holds.
            //
            ...
            // the claim — `arming::dukascopy_holds_sidecar`, the same call
            ...
            match dukascopy::resolve_in(account, arming::directory_of(policy)) {
                ...
                Ok(mount) => {
                    match vike_dukascopy::load_dukascopy_config_from(mount.account, vars) {
                        ...
                        // lost and to whom.
                        Some(_) if !arming::dukascopy_holds_sidecar(account, vars, policy) => {
                            let refusal = dukascopy::DukascopyRefusal::SidecarHeldByAnother {
                                label: account.to_string(),
                                holder: arming::dukascopy_sidecar_holder(vars, policy).to_string(),
                            };
                            tracing::error!(venue, "{refusal}");
                            Box::new(paper_client(venue, symbol, static_default))
                        }
                        Some(cfg) => {
                            ...
                            match dukascopy::claim_sidecar() {
                                None => {
                                    let refusal =
                                        dukascopy::DukascopyRefusal::SidecarAlreadyClaimed {
                                            label: account.to_string(),
                                        };
                                    tracing::error!(venue, "{refusal}");
                                    Box::new(paper_client(venue, symbol, static_default))
                                ...
                                Some(claim) => {
                                    // `<project>/bin`, derived from the state directory the boot resolved.
                                    let project_bin =
        ...
        // opens `libfcshim.so` instead, so the same `vike` that runs everywhere arms FXCM exactly
        // on the boxes that have the shim installed. There is no "stub build" any more.
        //
                ...
                Some(cfg) => {
                    if vike_fxcm::sdk_available() {
                        ...
                        tracing::warn!(
                            ...
                            // WHICH shim answered, not merely that one did. The loader's ladder
                            // ends at the dynamic loader's own search, so on a box with an
                            // installed `<root>/lib/libfcshim.so` AND a stale copy on
                            // `LD_LIBRARY_PATH` this line is the only record of which file the
                            // session was made through.
    ...
    //
    // ⚠ This is the CEX half of what the `("hyperliquid", _)` arm above already does. That
    // arm asks `userRole` and folds the answer through the same recorder; the difference is that HL
    // can compute its book offline from the key and the CEX venues cannot — `book_identity`'s
    // bybit/okx rows say the store names no account and only an authenticated call can. This is
    // that call, for the venues where it is the only one there is.
...
mod preconnect_tests {
    ...
    fn live_arming_cases() -> &'static [(&'static str, &'static [(&'static str, &'static str)])] {
        &[
            ...
            // name. `SERVER` is deliberately absent here — `load_dukascopy_config_from` gates on
            // login and password alone and lets the server default, so a row carrying it would
            // assert a requirement the loader does not have.
            //
    ...
    /// BOTH values of it.
    ///
    /// The end-to-end `would_mount_live` answer is asserted against `sdk_available()` rather than
    ...
    /// summary line above it did not. What remains here is the FEATURE-off half: polymarket's arm
    /// exists only under its feature.
    ///
    ...
    fn a_venue_this_build_cannot_mount_says_so_instead_of_blaming_credentials() {
        ...
        assert_eq!(
            ...
            (
                Mode::Paper,
                if vike_fxcm::sdk_available() { Block::NoCredentials } else { Block::SdkAbsent }
            ),
        );
    ...
    fn the_roster_projection_covers_every_venue_and_carries_its_ceiling() {
        ...
        let policy = crate::MountPolicy {
            venues: VenuePolicy::default()
                .declare("bybit", Mode::Live)
                .declare("binance", Mode::Demo),
            ..Default::default()
        };
    ...
    fn a_labelled_account_in_the_store_gets_its_own_unarmed_row() {
        ...
        let policy = crate::MountPolicy {
            venues: VenuePolicy::default().declare("bybit", Mode::Live),
            ..Default::default()
        };
