...
//! `recon_enabled` (the caller's already-resolved `VIKE_RECONCILE=1` verdict) through
...
//! lane a Polymarket fill/cancel ever arrives on — and `POLY_RECONCILE=1` independently mounts the
//! `ReconClient`. With both on, ONE `vike_polymarket::live_mount_from_vars` call builds them over
...
pub use arming::{
    known_accounts, resolve_fee_schedule, shared_books_for, symbol_for_account,
...
pub use symbol_grid::{declared_grid_source, DeclaredGridSource};
...
/// the number they wrote. A cross-symbol lane would need the position book threaded into
/// `check()`, which is inside the `p99 < 10µs` fold — a separate epic, not a doc fix.
...
pub fn make_engine_for_account(
    ...
    use vike_bridge_core::credentials::{load_credentials_for_account, Environment};
    ...
    if mode == vike_config::VenueMode::Paper {
        ...
        let (mut engine, recon) = paper_engine(venue, symbol, declared_legs, risk_profile);
    ...
    let client: Box<dyn vike_exec::ExecutionClient + Send> = match (venue, creds) {
        ...
        ("polymarket", _) => {
            let want_recon = vike_polymarket::poly_reconcile_enabled(vars);
...
mod fee_schedule_tests {
    ...
    fn polymarket_without_the_gates_is_inert_and_offline() {
        ...
        // `recon_enabled: true` on purpose: polymarket's reconcile decision is its OWN inner gate
        // (`POLY_RECONCILE=1`), never the global one, and passing `true` is what proves that.
        let (engine, recon) = super::make_engine(
            "polymarket",
