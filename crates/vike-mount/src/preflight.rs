...
//!
//! Two bounds, both named and both observable in the report:
//!
//! - `crate::server_time`'s `CLOCK_READ_TIMEOUT` (3 s) — the PER-READ ceiling, on a dedicated
//!   bounded agent rather than the 30 s order-path one.
...
//! `MAX_CLOCK_BUDGET_MS + CLOCK_READ_TIMEOUT` = **18 s** and today **11.4 s**, against 630 s. ⚠ The
...
pub(crate) const REMEDY_CLOCK: &str =
    "sync the host clock (NTP / w32tm) — the host's own time is wrong, whatever the venue does \
     with it";
/// ② — the venue publishes a clock and did not give us one.
const REMEDY_CLOCK_UNREACHABLE: &str =
    "this venue PUBLISHES a server-time endpoint and it did not answer — check egress and the \
...
const REMEDY_CLOCK_UNMEASURED: &str =
    "verify this host's clock by other means before trading this venue live (`timedatectl` / \
...
const REMEDY_CLOCK_BUDGET: &str =
    "an earlier venue's clock read consumed the leg's whole budget — check that venue's row, or \
     raise PreflightConfig::clock_budget_ms if every venue here is genuinely this slow";
const REMEDY_CREDENTIALS: &str =
    "this venue is MOUNTED PAPER for this session — check its credentials in the store \
...
const REMEDY_CREDENTIALS_UNANSWERED: &str =
    "this venue is STILL MOUNTED LIVE — a probe that did not answer proves nothing about the keys, \
...
const REMEDY_CREDENTIALS_BUDGET: &str =
    "an earlier venue's credential probe consumed the leg's whole budget — check that venue's row; \
...
mod tests {
    ...
    use vike_bridge_core::{NetProbe, NetProbeConfig, DEFAULT_PROBE_INTERVAL};
