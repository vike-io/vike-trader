...
use crate::{
    arm_universal_defaults, margin_mode_grid, merge_operator_budget, multiplier_grid,
    resolve_fee_schedule, symbol_grid, would_mount_live, EngineAndRecon, MountPolicy,
};
...
/// `recon_driver` `spawn_recon` mount in `App::new` (Task 6), gated on `VIKE_RECONCILE=1` — off by
/// default, so building it here is otherwise inert (no new behavior for the engine itself).
///
...
///
/// `recon_enabled` is the GLOBAL reconciliation on/off — the caller's already-resolved
/// `VIKE_RECONCILE=1` verdict (`vike_run::NodeConfig::recon_enabled`) — and it exists SEPARATELY
...
/// hyperliquid's recon client is inseparable from the exec client it is built with; polymarket has
/// its own equivalent inner gate (`POLY_RECONCILE=1`, checked BEFORE its factory).
///
