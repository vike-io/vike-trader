...
/// **The venue answered, so the store can be told** — the hyperliquid twin of
/// `crate::dukascopy`'s `record_confirmation`, over the SAME mechanism rather than a second one.
///
...
/// two venues from answering differently about what a handshake means.
///
...
fn record_hyperliquid_confirmation(
    ...
    let Some(record) = crate::book_identity::confirmation_for_account(
        "hyperliquid",
        label,
        bound_tier,
        master,
        ...
        tracing::info!(
            account = %label,
            book = %master,
            "hyperliquid: the venue named this account's book, and this box cannot record it — no \
    ...
    match verdict(&record.handshake_account_id, record.observed_book.as_deref()) {
        Verdict::Confirms => tracing::info!(
            ...
            "hyperliquid: the venue CONFIRMED the book this account row names"
        ),
        Verdict::Learns => tracing::info!(
            account = %label,
            row = ?record.observed_row,
            book = %record.handshake_account_id,
            "hyperliquid: the venue NAMED this account's book, which the store did not know — \
        ...
        Verdict::Disagrees { stored } => tracing::error!(
            ...
            "hyperliquid: ⚠ THE STORE AND THE VENUE DISAGREE about which account this key is. The \
             settings database says this row's venue account is `{stored}` and the venue's own \
             `userRole` answered `{}`. NOTHING WILL BE WRITTEN — not the book, and not \
    ...
    // `crate::book_identity::confirmation_for_account` already answers the UNREAD-store way for.
    //
    ...
    if let Err(e) = vike_model::account_confirmation::park(Some(&state), record) {
        ...
        tracing::warn!(
            ...
            error = %e,
            state = %state.display(),
            "hyperliquid: the venue's account confirmation could not be parked for \
