        ...
        match self {
            MountError::MissingRiskBudget { venue, missing, profile_supplied } => {
                ...
                if *profile_supplied {
                    ...
                    writeln!(f, "    mode = \"live\"")?;
                    writeln!(f)?;
                    // ⚠ `[event_source]` and `[broker]` were PRINTED here, with a note saying they
                    // were schema-required while a live venue mount ignored both. Both tables are
                    // DELETED from `RunProfile` and a profile carrying either is now refused by
                    // name, so printing them would hand the operator a paste-ready file that fails
                    // startup — the worst possible copy in a message whose whole job is to be
                    // pasted. A `mode` + `[risk]` file is the complete minimal profile now.
                    writeln!(f, "    [risk]")?;
