...
//!
//! **`Account::id` is never an address.** It is a SQLite rowid with no `AUTOINCREMENT`, stable only
...
///
/// ⚠ **The address is the KEY PREFIX, never `mount.row`.** An `account.id` is a SQLite rowid with
/// no `AUTOINCREMENT`, stable only for the life of ONE database FILE, and this tree documents a
/// recovery that deletes the database and migrates again — after which the same accounts come back
/// numbered differently. A parked record outlives that, so an id-keyed one would let a fold stamp a
/// stranger's row, which on this venue is the wrong legal entity. The prefix is the store's own
/// account identity (`crates/vike-secrets/src/schema.rs`'s `Classification::owner_prefix`) and is
/// what [`row_account`] already keys the broker mapping on, so the mount and the fold cannot
/// disagree about which row is which. The row id rides along as EVIDENCE only.
fn confirmation_for(
