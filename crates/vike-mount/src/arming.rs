...
use crate::{
    AccountLabel, CexCredChoice, MountError, MountPolicy, WithdrawGate, ARMED_MAX_ORDERS_PER_WINDOW,
};
...
/// arm landed** and is now a feature-gated row like ibkr's, with the extra `sdk_linked` conjunct
...
fn known_accounts_in(
    ...
    if let Some(p) = policy {
        for (v, label, _) in p.accounts() {
            if v == venue {
                if let Ok(parsed) = AccountLabel::parse(label) {
                    labels.push(parsed);
                }
            }
        }
...
pub(crate) fn account_arming_under(
    ...
    match venue {
        ...
        // No live arm in ANY build: dukascopy (no arm in `make_engine` today) and every unknown
        // venue — both reach only the paper `_` arm.
...
/// SDK linkage is `sdk_linked`, describe a mount that will actually go LIVE?
///
...
/// * **`sdk_linked`** — a STUB build's arm refuses and lands on paper no matter what the store
...
pub(crate) fn fxcm_live_intent(
    ...
    sdk_linked && vike_fxcm::load_fxcm_config_for_account(Environment::Demo, label, vars).is_some()
}
...
pub(crate) fn binance_withdraw_gate(
    ...
    use vike_bridge_core::key_permissions::{allow_withdraw_keys, KeyPermissionProbe};
    if !mainnet {
...
pub(crate) fn binance_withdraw_verdict(
    ...
    use vike_bridge_core::key_permissions::{withdraw_gate, KeyPermissions};
