...
//! to give up — at the top of `vike_run::build_node`, before one venue is up, over a check whose
...
use vike_bridge_core::credentials::{Environment, load_credentials_from};
...
///
/// ⚠ **"Reports itself first" holds for six of the seven wired fetchers, and the seventh is a
/// declared residual.** Every fetcher in `crate::server_time` makes exactly ONE request on its
/// bounded agent — except `hyperliquid_time`, which goes through
/// `crates/bridges/hyperliquid/src/transport.rs`'s `info`: a RETRY LOOP above ureq
/// (`INFO_MAX_ATTEMPTS` attempts, each on the 3 s agent, with a 200 ms then 400 ms backoff), so a
/// venue answering slow `429`s/`5xx`s can legitimately occupy that fetcher for longer than this
/// ceiling before its third attempt succeeds. Such a read is abandoned here with our generic
/// "did not answer" where the venue's own error text — or its eventual answer — would have been
/// truer. Accepted rather than sized around: the retry fires only on a fast-answering throttle or
/// server error (an ambiguous timeout returns on the first attempt, per that method's own doc), the
/// worst outcome either way is outcome ② (a WARN that degrades nothing), and widening this ceiling
/// to cover the loop would cost every OTHER venue's wedged-resolver case the same seconds on every
/// mount.
...
/// from `crate::server_time`'s `CLOCK_SOURCES` table with no I/O, so there is nothing to abandon and
/// bounding it would spawn a thread to run a `match`. Only a read that can BLOCK is spawned, which
/// is also what keeps a mount whose armed venues are all DECLARED at zero threads.
///
...
pub fn authed_read_probes(
    ...
    //
    // ⚠ **THIS LEG ALSO RECORDS WHICH ACCOUNT THE KEY IS**, and it is the site that covers a venue
    // this box ARMS but never MOUNTS. MEASURED on the CI box 2026-09-20: 16 `account` rows across 13
    // venues, and a `tradehub.toml` that mounts exactly one. `make_engine_for_account`'s own call
    // would have recorded that one and left the store asking about the rest.
    // `crate::book_identity::record_authenticated_account` carries the whole argument, including
    // why deribit is NOT reachable from here and needs the mount site too.
    //
    ...
    for (venue, client) in authed_read_clients(vars, policy) {
        ...
        // The SAME fold `authed_read_clients` used to pick the tier this client signs with — so the
        // row the record addresses and the keys the probe authenticated with can never disagree
        // about which tier this is.
        let bound_tier = if probe_mainnet(&venue, vars, policy) {
            vike_config::VenueMode::Live
        } else {
            vike_config::VenueMode::Demo
        };
        let directory = directory.clone();
        let venue_id = venue.clone();
        out.insert(
            venue,
            Arc::new(move || {
                let guard =
                    client.lock().map_err(|_| "preflight probe lock poisoned".to_string())?;
                // ⚠ ORDERING, and it is the same rule `make_engine_for_account` states at its own
                // call site: the BALANCE first. On binance spot the account id rides
                // `/api/v3/account`, which is exactly the body this read pulls, so recording after
                // it costs that venue nothing — and recording before it would spend a second signed
                // request to fetch the body this line was about to fetch anyway.
                guard.fetch_balance()?;
                // ⚠ The probe's VERDICT is the balance read alone. A venue that answered its
                // balance and then could not name its account is a HEALTHY credential — demoting it
                // over a bookkeeping field would turn a live venue into paper for a reason that has
                // nothing to do with whether the key works. `record_authenticated_account` returns
                // `()` and warns on its own failures, so there is no way for this line to say no.
...
/// `the_environment_overrides_the_file_for_every_flag`) and the fold OVERWRITES rather than
...
/// is the only production root that reaches `vike_run::build_node`, and it folds.
...
pub fn run_startup_preflight(
    ...
    let probes = FnProbes::new()
        .with_venue_server_time_ms({
            ...
            // abandon it), so the map is shared by refcount instead of copied per venue per sample.
            let vars = Arc::new(vars.clone());
            move |venue: &str| bounded_server_time_ms(venue, &vars)
        })
...
mod tests {
    ...
    fn a_prompt_credential_probe_answers_through_the_bound() {
        ...
        assert_eq!(
            bounded_probe(&ok, CREDENTIAL_PROBE_TIMEOUT, CREDENTIAL_PROBE_THREAD),
            Ok(Ok(()))
        );
    ...
    fn a_declared_clock_venue_is_answered_without_the_probe() {
        ...
        for venue in ["ctrader", "oanda", "alpaca"] {
            assert_eq!(
                bounded_server_time_ms(venue, &vars),
                venue_server_time_ms(venue, &vars),
...
mod preflight_skip_precedence_tests {
    ...
    /// refusing and an un-resolved `<project>/settings/secrets.env` line arming. Asserted as
