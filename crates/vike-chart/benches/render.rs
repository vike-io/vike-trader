...
/// `n` closed bars as the wire type `vike_marketdata::Bar` — what `sync_from_core` reads FROM.
fn synth_raw_bars(n: usize) -> Arc<Vec<vike_marketdata::Bar>> {
    Arc::new(
        (0..n)
            .map(|i| {
...
fn synth_raw_bars_shifted(n: usize, epoch_shift_ms: i64) -> Arc<Vec<vike_marketdata::Bar>> {
    Arc::new(
        (0..n)
            .map(|i| {
                let (ts, o, h, l, c, v) = wave(i);
...
fn main() {
    ...
    // (a) the CURRENT full ChartState bar rebuild: `crates/vike-desktop/src/main.rs`'s
    ...
    // (`crates/vike-desktop/src/main.rs`'s `sync_from_core`), which is O(1) on forming-ticks and
    // O(N) only on a bar CLOSE — measured separately in row (a2); rows (a) + (a2) together
    // represent the full per-snapshot cost.
    let mut cs = vike_chart::model::ChartState::default();
    let t_rebuild = time_it(REPEATS, WARMUP, || {
        cs.bars.clear();
        let render = |i: usize, b: &vike_marketdata::Bar| ModelBar {
            t: i as f64,
            ot: b.ts,
            o: b.open,
            h: b.high,
    ...
    // (a2) refresh_caches() on a bar CLOSE: `crates/vike-desktop/src/main.rs`'s `sync_from_core`
    // ends with `cs.refresh_caches()`. Its O(1) key-guard (key = (closed_len, first ot, last ot))
    ...
    let mut growing: Vec<vike_marketdata::Bar> = raw.as_ref().clone();
    let mut closed_steps: Vec<Arc<Vec<vike_marketdata::Bar>>> = Vec::with_capacity(steps);
    let mut forming_steps: Vec<vike_marketdata::Bar> = Vec::with_capacity(steps);
    for k in 0..steps {
        let (ts, o, h, l, c, v) = wave(N + k);
        growing.push(vike_marketdata::Bar {
            ts,
            open: o,
            high: h,
            low: l,
        ...
        forming_steps.push(vike_marketdata::Bar {
            ts: fts,
            open: fo,
            high: fh,
            low: fl,
    ...
    println!(
        "  external bench binary — timed via `heikin_ashi_before`, a byte-for-byte copy in this file."
    );
