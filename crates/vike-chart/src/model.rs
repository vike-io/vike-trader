...
use crate::chart::{ChartStyle, heikin_ashi, heikin_ashi_bar};
...
pub struct ChartState {
    ...
    /// `(closed.len(), closed.first().ts, closed.last().ts)` of the `vike_marketdata::Bar`
...
/// Map one wire-format `vike_marketdata::Bar` (venue/core-side OHLCV) to the render-model
...
fn render_bar(i: usize, b: &vike_marketdata::Bar) -> Bar {
...
impl ChartState {
    ...
    pub fn sync(
        &mut self,
        closed: &std::sync::Arc<Vec<vike_marketdata::Bar>>,
