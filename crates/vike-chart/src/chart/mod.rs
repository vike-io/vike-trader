...
use crate::interact::{
    AutoYEvent, follow_engaged, follow_shift, next_auto_y, resolve_ty, visible_slice, x_drag_zoom,
    y_drag_zoom,
};
...
use crate::sync::{SyncIn, nearest_index_by_ts, ts_range_to_index_bounds};
...
use extent::{Bounds, default_bounds, map_pad_view, visible_ts_bounds, y_raw_ext};
use fmt::{fmt_scaled, fmt_scaled_view};
use marks::{XAxisCtx, attach_shared_x_axis};
use overlay::{
...
use panes_layout::{PaneLayout, pane_separator, resolve_pane_layout};
use series::{RenderSeries, resolve_series};
...
pub use crate::panes::{MoveTarget, PaneFractions, PaneKey, PaneTarget, resolve_add_target};
...
pub fn draw(ui: &mut egui::Ui, inp: ChartInputs<'_>) -> ChartActions {
    ...
    ui.visuals_mut().widgets.noninteractive.fg_stroke.color = palette::TEXT3;
    ...
    // sub-pane (they share this `ui`). Default `bg` == palette::BG, the value
    ...
    let resp = plot.show(ui, |plot_ui| {
        ...
        if !interacted
            && let Some((rx0, rx1)) =
                ...
                && ((rx0 - cx0).abs() > 1e-9 || (rx1 - cx1).abs() > 1e-9) {
                    cx0 = rx0;
                    cx1 = rx1;
        ...
        if !force_fresh_ty
            && let Some((prev_view, prev_anchor_bits)) = follow.last_scale {
        ...
        if auto_y
            && let Some((mut lo, mut hi)) = y_raw_ext(vis_now) {
                if owned.is_none() {
