...
// ---- the three grid ROLES that are not palette slot names ----
//
// Every colour this grid paints comes from the ONE canonical `vike_ui_theme::palette` and is named
// there — `palette::TEXT`, `palette::BORDER`, `palette::ACCENT` and the rest are spelled at the
// call site, because a second name for a palette slot is an alias and the 2026-09-18 ruling
// retired those. The three below survive because each names a role IN THIS GRID that the palette
// slot does not: the bid/ask family tint pair (Deribit-style — see `cell`), which also paints a
// long/short position, and the raised centre Strike band.
const GREEN: Color32 = palette::UP;
const RED: Color32 = palette::DOWN;
const SPINE: Color32 = palette::HOVER; // raised centre Strike band
...
pub fn draw(ui: &mut egui::Ui, inp: OptionChainInputs<'_>) -> OptionChainActions {
    ...
    ui.horizontal(|ui| {
        ...
        let pill = |ui: &mut egui::Ui, t: &str| -> egui::Response {
            ui.add(
                egui::Button::new(RichText::new(t).color(palette::TEXT).size(13.0))
                    .fill(SPINE)
        ...
        for u in inp.underlyings {
            ...
            let resp = ui.add(
                egui::Button::new(
                    RichText::new(u)
                        .color(if on { palette::TEXT } else { palette::TEXT2 })
                        .size(13.0),
                )
                .fill(if on { SPINE } else { Color32::TRANSPARENT })
                .stroke(egui::Stroke::new(1.0, if on { palette::ACCENT } else { palette::BORDER }))
                .min_size(egui::vec2(0.0, 34.0)),
            );
            if resp.clicked() {
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ui.label(RichText::new(&expiry_label).color(palette::TEXT2));
            ui.label(RichText::new("·").color(palette::TEXT3));
            ui.label(RichText::new("deribit").color(palette::TEXT2));
            ui.label(RichText::new("·").color(palette::TEXT3));
            if let Some(s) = spot {
                ui.label(
                    RichText::new(format!(
    ...
    let cell = |c: &Col, row: &vike_options::StrikeRow| -> (String, Color32) {
        match c {
            Col::Strike => (ocols::fmt_strike(row.strike), palette::TEXT),
            Col::Iv => {
                ...
                (ocols::fmt(iv, "iv"), if iv.is_some() { palette::TEXT } else { palette::TEXT3 })
            }
    ...
    egui::ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
        if rows.is_empty() {
            ui.label(RichText::new("loading…").color(palette::TEXT3));
        }
            ...
            for (i, c) in cols.iter().enumerate() {
                if let Col::Field { field: "volume", call } = c.2 {
                    ...
                    if v > 0.0 {
                        ...
                        let bar = if call { palette::BLUE } else { RED };
                        p.rect_filled(
            ...
            for is_call in [true, false] {
                ...
                if let Some(first) = book.working.first() {
                    ...
                    p.text(
                        ...
                        &txt,
                        small.clone(),
                        if hot { palette::TEXT } else { palette::BLUE },
                    );
                    if resp.clicked() {
