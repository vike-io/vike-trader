...
use vike_core::{spawn_core, CoreConfig, CoreSnapshot, JournalConfig, ReconBlock};
...
//     while read -r r; do
//       j=$(gh api "repos/vike-io/vike_trader_rust/actions/runs/$r/jobs" \
//             --jq '.jobs[] | select(.name=="latency") | .id')
//       [ -n "$j" ] && gh api "repos/vike-io/vike_trader_rust/actions/jobs/$j/logs" |
