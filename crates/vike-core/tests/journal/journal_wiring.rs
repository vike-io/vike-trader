...
use vike_core::{
    CoreConfig, JournalConfig, LiveBroker, StrategyMount, spawn_core, strategy_state::mount_id_of,
};
...
fn journals_before_fold_and_snapshots_on_shutdown() {
    ...
    let recs = vike_journal::CommandJournal::read_all(&dir).unwrap();
...
// ---------------------------------------------------------------------------------------------
// The DEFAULT account on a JOURNALED command
// ---------------------------------------------------------------------------------------------

/// ⚠ **A documented, gate-approved wire value used to KILL the fold thread, and this is the gate
/// for it.**
///
/// `vike_model::account_keys::AccountLabel`'s `Serialize` returns `Err` for
/// `AccountLabel::Default`, deliberately: `DEFAULT` is exactly the spelling `AccountLabel::parse`
/// refuses, so emitting it would produce a document that type cannot read back. Every other carrier
/// of the label obeys that by rendering the default account as an ABSENT field — `vike_core::
/// snapshot::VenueBlock::account` says so in those words and filters with `!l.is_default()`.
///
/// `vike_model::OrderRequest::account` is the FIRST carrier that cannot: `Some(Default)` and `None`
/// route DIFFERENTLY (on a two-account venue the first reaches the unlabelled book and the second
/// is refused as ambiguous), and the spec's §8 ruling 1 requires the journal to carry which account
/// an order was for. So the field had to gain a REPRESENTATION that round-trips — it renders
/// through `AccountLabel`'s `Display` and reads back through `parse_wire_account`, the one parser
/// that admits `DEFAULT` — rather than a value the journal could omit.
///
/// Before that, a journaling daemon that accepted `{"account":"DEFAULT"}` (which
/// `vike_tradehub`'s `account_refusal` PASSES by design on a single-account node — the bare venue
/// id IS that account's route key) reached `serde_json::to_vec(…).expect("JournalRecordRef
/// serializes")` and panicked. The core died and every later command answered `Gone`.
///
/// ⚠ **This drives the REAL writer, not the serializer**, because the defect is not that a
/// serializer returns `Err` — it is supposed to — but that a live command path reaches an
/// `.expect` on it. It asserts the WRITE-AHEAD `Cmd` record and the `MintedSubmit` record, which
/// are two different serialization sites for the same field, and it reads the account back rather
/// than merely counting records: a representation that wrote something unparseable would satisfy a
/// count and fail the replay this journal exists for.
#[test]
fn a_submit_naming_the_default_account_journals_and_reads_back() {
    let dir = crate::scratch::Scratch::reserved("acct-default");
    let engine = ExecutionEngine::new(
        Account::new(1.0, VENUE, None, BalanceMode::Delta),
        RiskGate::new(RiskLimits::new()),
        RecordingClient::default(),
    ...
    let cfg = CoreConfig {
        ...
        journal: Some(JournalConfig {
            dir: dir.to_path_buf(),
            file: vike_journal::JournalFileConfig { segment_bytes: 1024 * 1024, flush_every: 1 },
    ...
    // TWO submits, because the two records have DIFFERENT triggers and each is its own
    // serialization of this field. The write-ahead `Cmd` is written for every command; the
    // `MintedSubmit` is written ONLY when the runtime MINTED the coid, i.e. for an EMPTY-coid
    // submit (`apply_intent`'s `if minted` guard) — so an explicit-coid order alone would never
    // reach the second site and a test asserting it would be measuring the wrong thing.
    let mut named = order_req("op-default");
    named.account = Some(vike_model::account_keys::AccountLabel::Default);
    handle.send_command(Command::Order(OrderIntent::Submit(Box::new(named))));

    let mut minted = order_req("");
    minted.account = Some(vike_model::account_keys::AccountLabel::Default);
    handle.send_command(Command::Order(OrderIntent::Submit(Box::new(minted))));
    ...
    // (1) the WRITE-AHEAD `Cmd` record — what `append_cmd` serializes, the site the panic was
    //     reported at.
    let write_ahead = recs
        .iter()
        .find_map(|r| match r {
            JournalRecord::Cmd {
                msg: Ingest::Command(Command::Order(OrderIntent::Submit(req))),
                ..
            } if req.client_order_id == "op-default" => Some(req.clone()),
            _ => None,
        })
        .expect(
            "the write-ahead Cmd record must exist — an absent one means the fold thread died \
             serializing it, which is the defect",
        );
    assert_eq!(
        write_ahead.account,
        Some(vike_model::account_keys::AccountLabel::Default),
        "…and it must READ BACK as the DEFAULT account, not as an absent field: `Some(Default)` \
         and `None` route differently, so a journal that collapsed them could not be reconciled"
    );

    // (2) the MintedSubmit record — `append_minted_submit`, a SECOND serialization of the same
    //     field from a different record type, written for the EMPTY-coid submit above.
    let minted = recs
        .iter()
        .find_map(|r| match r {
            JournalRecord::MintedSubmit { req, .. } => Some(req.clone()),
            _ => None,
        })
        .expect("the minted-submit record must exist too — it serializes the OrderRequest again");
    assert_eq!(
        minted.account,
        Some(vike_model::account_keys::AccountLabel::Default),
        "the second site must agree with the first"
    );
    assert!(
        !minted.client_order_id.is_empty(),
        "sanity: this record exists BECAUSE the runtime minted the coid — an empty one here would \
         mean the record was written from some other path and proves nothing about `if minted`"
    );

    // (3) …and the core survived to write BOTH. Under the defect the very first serialization
    //     killed the fold thread, so the second submit was never folded at all and every later
    //     command answered `Gone`.
    assert_eq!(
        recs.iter()
            .filter(|r| matches!(
                r,
                JournalRecord::Cmd {
                    msg: Ingest::Command(Command::Order(OrderIntent::Submit(_))),
                    ..
                }
            ))
            .count(),
        2,
        "both submits reached the journal: the fold thread was alive for the second one"
    );
}
