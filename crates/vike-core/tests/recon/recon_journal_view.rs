...
//!
//! The walk it drives used to live inside the GUI shell's `journal_view_provider` closure
//! (`crates/vike-desktop/src/main.rs`, then spelled `vike-app`) — the workspace's ONE production
//! `JournalViewHook`, in the one workspace member the merge gate does not RUN. Nothing in this
//! repository executed a line of it. Two defects were sitting there, and each one has a test below
//! that reddens on it:
//!
...
//!
//! ⚠ **That closure is GONE ENTIRELY now — do not go looking for it.** The desktop cut removed the
//! GUI's local trading core (all orders and trading leave the shell through the backend), and the
//! `journal_view_provider` hook went with the reconcile mount it fed; the citation above survives
//! only as a tombstone comment in that file. So the workspace now has NO production
//! `JournalViewHook` at all — `CoreConfig::journal_view_provider` is `None` at every live mount —
//! and these tests are the only thing that drives the lifted walk. Nothing below changes:
//! the two defects are properties of the WALK, which is what these tests own, and the defect-free
//! walk is what a daemon-side hook would take when one is wired.
//!
...
use vike_exec::recon::{Divergence, DivergenceKind, LocalCash, LocalView, ReconPolicy, diff};
...
fn a_journal_window_narrower_than_the_venue_window_turns_an_alert_into_an_auto_fold() {
    ...
    assert!(
        ...
        "…and that downgrade is not merely a quieter message: `hybrid` — the policy an operator \
         reaches by NAMING it, since an unset variable now folds to quarantine at both live \
         mounts — AUTO-APPLIES MissingFill, so the narrow window folds the divergence with no \
