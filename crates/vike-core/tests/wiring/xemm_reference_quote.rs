...
use vike_model::{BookLevel, Broker, HftBroker, L2Book, OrderRequest, QuoteTick, Strategy};
...
fn book(bid: f64, ask: f64) -> Arc<L2Book> {
    ...
    b.apply_snapshot(
        1,
        &[BookLevel::new(bid, 3.0), BookLevel::new(bid - 1.0, 9.0)],
        &[BookLevel::new(ask, 4.0), BookLevel::new(ask + 1.0, 9.0)],
    );
    Arc::new(b)
}
