...
/// example — it starts from the real process env and inserts the file's `journal_dir` only where
/// the variable is ABSENT, so an `Environment=` line in a unit beats `config.toml` exactly as it
/// did when this function could only read the environment.
///
