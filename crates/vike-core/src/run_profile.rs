...
pub struct Sinks {
    ...
    /// already being SET, hardcoded to one second, in `crates/vike-app/src/main.rs`: the target
    /// landed and the key was never pointed at it, while the doc kept saying the target did not
    /// exist. That is the exact failure this workspace deleted `Policy::max_total_exposure` for.
...
///
/// The bound is stated in prose on that field and argued again at both live roots'
/// `submit_ack_timeout` literals (`crates/vike-app/src/main.rs`, `crates/vike-tradehub/src/tradehub_cli.rs`);
/// this is the one place it is COMPUTED, so a run profile can no longer invalidate those arguments
/// in silence.
...
impl RunProfile {
    ...
    ///   timeout BOTH live roots arm — `2·5s = 10s`, needed `> 30/2 + 2·5 = 25s` for a two-hop
    ...
    ///   profile that never mentioned the grace. `crates/vike-app/src/main.rs`'s
    ...
    ///   alongside stage 1" confuses the PROFILE's stage 1 with the CORE's: `vike-app` and
    ///   `vike-tradehub` both set `submit_ack_timeout: Some(30s)` in their own `CoreConfig`
    ///   literals, so the watchdog is armed whether or not the profile mentions it.
    ///
...
mod consumption_gate {
    ...
    /// A `CoreConfig` shaped like the one BOTH live roots build: the binary's own 30 s stage-1
    /// timeout, and a confirm grace nobody has touched. `crates/vike-app/src/main.rs` and
    /// `crates/vike-tradehub/src/tradehub_cli.rs` each construct exactly this before calling
    ...
    /// The stage-1 timeout both live roots arm, in their own `CoreConfig` literals
    /// (`crates/vike-app/src/main.rs`'s and `crates/vike-tradehub/src/tradehub_cli.rs`'s
    /// `submit_ack_timeout`). Restated here because `vike-core` cannot read a binary above it; the
    /// citation is the link.
