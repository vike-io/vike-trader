...
mod tests {
    ...
    const DAY0: i64 = 20_650 * MS_PER_DAY; // 2026-07-xx 00:00:00 UTC
    ...
    fn registry_establishes_then_fires_each_instant_once() {
        ...
        assert!(s.check_due(DAY0 + MS_PER_DAY - 1).is_empty());
        // next midnight: fires
        assert_eq!(s.check_due(DAY0 + MS_PER_DAY), vec!["recalibrate".to_string()]);
        // same day again: no re-fire
        assert!(s.check_due(DAY0 + MS_PER_DAY + 3_600_000).is_empty());
        // following midnight: fires again
        assert_eq!(s.check_due(DAY0 + 2 * MS_PER_DAY), vec!["recalibrate".to_string()]);
    }
    ...
    fn missed_instants_coalesce_to_one_fire() {
        ...
        assert_eq!(s.check_due(DAY0 + 3 * MS_PER_DAY), vec!["daily".to_string()]);
        // and the next day fires again (the latch advanced to DAY0 + 3 days)
        assert_eq!(s.check_due(DAY0 + 4 * MS_PER_DAY), vec!["daily".to_string()]);
    }
}
