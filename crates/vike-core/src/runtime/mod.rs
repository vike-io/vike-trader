...
use vike_model::{
    amount_to_order, build_bracket, units_from_percent, units_from_value, Bar, BracketSpec, Broker,
    Clock, EquitySample, FeedStatus, Fill, FlowToxicity, HftBroker, LiveClock, MarkTick,
    OrderRequest, Strategy, StrategyParams,
};
...
impl<C: ExecutionClient> CoreThread<C> {
    fn run(mut self, mut rx: mpsc::Receiver<Ingest>) {
        ...
        if catch_unwind(AssertUnwindSafe(|| {
            ...
            self.dispatch_applied_fills(); // pending on_fill deliveries while the client is attached
                                           ...
                                           // point a cancel can still reach the venue. One line below, `engine.shutdown()`
                                           // detaches and the same sweep would go nowhere. OFF by default ⇒ one bool read, and the
                                           // teardown stays byte-identical to the version that had no such field.
    ...
    fn dispatch(&mut self, msg: Ingest) {
        ...
        match msg {
            ...
            Ingest::BarClose(update) => {
                ...
                let append = match last_ts {
                    ...
                    Some(t) if bar.ts == t => {
                        if let Some(series) = self.bars.get_mut(&key) {
                            if let Some(last) = Arc::make_mut(&mut series.closed).last_mut() {
                                *last = bar.clone();
                            }
                        }
                        false
                ...
                if let Some(series) = self.bars.get_mut(&key) {
                    if series.forming.as_ref().is_some_and(|f| f.ts <= bar.ts) {
                        series.forming = None;
                    }
                }
                self.dirty = true;
            ...
            Ingest::Quote(qu) => {
                ...
                {
                    if self.eng(eidx).accepts_symbol(&symbol) {
                        self.eng_mut(eidx)
                            .price_board
            ...
            Ingest::Book(bu) => {
                ...
                if let Some(px) = book.mid() {
                    ...
                    if let (Some((bb, _)), Some((ba, _))) = top {
                            ...
                            self.engine_idx_for_route_key(RouteKey::sole_account_of(&venue))
                        {
                            if self.eng(eidx).accepts_symbol(&symbol) {
                                self.eng_mut(eidx)
                                    .price_board
                                    .set_quote(&venue, &symbol, bb, ba, now);
                            }
                        }
                    }
                    ...
                    if self.any_mount_ref {
                        if let (Some((bb, bq)), Some((ba, aq))) = top {
                            self.drive_strategy_reference_quote(
                                &venue, &symbol, bb, ba, bq, aq, now,
                            );
                        }
                    }
                }
    ...
    fn mount_strategy_runtime(&mut self, spec: vike_exec::MountSpec) {
        ...
        if let Some(dir) = self.config.state_dir.as_ref() {
            if let Err(e) = crate::mount_topology::upsert(
                dir,
    ...
    fn unmount_strategy_runtime(&mut self, controller_id: &str) {
        ...
        // re-mount or restart.
        if let Some(dir) = self.config.state_dir.as_ref() {
            if let Some(mount) = self.mounts[i].as_ref() {
        ...
        if let Some(dir) = self.config.state_dir.as_ref() {
            if let Err(e) = crate::mount_topology::remove(dir, &want) {
                tracing::warn!(
                    target: "vike_core::mount_topology",
                    mount_id = %want,
                    error = %e,
    ...
    fn eng_mut(&mut self, idx: usize) -> &mut ExecutionEngine<C> {
        if idx == 0 {
            ...
            &mut self.extra_engines[idx - 1].1
        }
    }
        ...
        if let Some(v) = venue {
            ...
            if self.multi_account {
                ...
                if let Some(key) = event_route_key(ev) {
                    if let Some(i) = self.engine_idx_for_route_key(RouteKey::declared(key)) {
                        return i;
                    }
                }
                ...
                if let Some(coid) = event_coid(ev) {
                    if let Some(&i) = self.coid_venue.get(coid) {
                        return i;
                    }
                }
                if let Some(symbol) = event_symbol(ev) {
                    if let Some(i) = self.engine_idx_for_venue_symbol(v, symbol) {
                        return i;
                    }
                }
            }
