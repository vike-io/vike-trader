...
use vike_model::{
    ...
    HftBroker, LiveClock, MarkTick, OrderRequest, Strategy, StrategyParams, amount_to_order,
    build_bracket, units_from_percent, units_from_value,
};
...
///   whichever venue `vike_run::build_node` mounted first, in practice binance — so a strategy
...
///   first, loudly and by name (`vike_run::refuse_unarmed_mount_accounts`); this is the backstop no
...
fn mount_engine_resolution(
    ...
    match account.filter(|l| !l.is_default()) {
        None => idx_of(RouteKey::sole_account_of(venue).as_str()).ok_or_else(|| {
            format!(
                "names venue `{venue}`, and this core runs no engine for it (mounted: \
                 {mounted:?}). It must NOT fall through to ENGINE ZERO — on a live daemon that is \
                 whichever venue was mounted first, so every order this strategy places would be \
                 signed and sent on a book its author did not choose. That is the same failure \
                 the account arm refuses, wearing a venue instead of a label. Mount `{venue}` on \
                 this core, or drop the mount"
            )
        }),
        Some(label) => {
            let key = vike_model::account_keys::route_key_of(venue, label);
            idx_of(RouteKey::declared(&key).as_str()).ok_or_else(|| {
                format!(
                    "names account `{label}` of venue `{venue}`, and this core runs no engine with \
                     ...
                     not choose is silent and unrecoverable. Arm the account \
                     (`policy.accounts.{venue}.{label}` in <project>/settings/policy.toml, plus \
                     its `__{label}` credential keys), or drop `account` from the mount"
                )
            })
        }
    }
...
/// spelled for it.
fn mounted_route_keys<'a, C: ExecutionClient>(
...
fn assemble_core<C: ExecutionClient>(
    ...
    // engines than it armed (dukascopy's `sidecar_holder` is the live example: two credential sets,
...
struct CoreThread<C: ExecutionClient> {
    ...
    link_deadman: Option<LinkDeadMan>,
...
impl<C: ExecutionClient> CoreThread<C> {
    ...
    fn accounts_of_venue(&self, venue: &str) -> usize {
        let primary = self.engine_idx_for_route_key(RouteKey::sole_account_of(venue));
    ...
    fn routed_engine(&self, route: EngineRoute, payload_venue: &str) -> Option<usize> {
    ...
    /// * **`venue: Some(v)`, route names no engine** (every EXTERNAL command path — a DOM click, a
    ///   tradehub ticket, a CLI verb — which can name no account) — EVERY engine of that EXCHANGE,
    ...
    /// * **`venue: Some(v)`, route names an engine** (a labelled mount's own intent, or a
    ///   core-internal producer that already resolved one — [`super::EngineRoute::Engine`]) — that
    ///   ONE engine. A strategy's exit must not reach another account's book, and
    ///   `sweep_link_deadman` addresses each account explicitly for exactly this reason.
    ///
    ...
    /// number of accounts this process runs on one exchange (one, on every core with no
    /// `policy.toml` `[accounts]` table).
    fn exit_scope_engines(&self, route: EngineRoute, venue: Option<&str>) -> Vec<usize> {
    ...
    fn route_event(&self, ev: &Event) -> Option<usize> {
        let venue = match ev {
    ...
    fn journal_route_key(&self, eidx: usize, payload_venue: &str) -> Option<String> {
    ...
    fn refuse_ambiguous(&mut self, verb: &str, venue: &str, candidates: &[String]) {
    ...
    /// and gated by `crates/vike-ops/tests/wallet_frame_venues_gate.rs`, re-run from the bridge
    ...
    fn reconcile_reports(&mut self, reports: ReconcileReports) {
        ...
        // `vike_run::mount_accounts_of` mounts an engine for every armed account while pushing a
        ...
        // `None` (provider unset — e.g. no store, or a mount whose reconcile gate answered no)
        // keeps this pass byte-identical to the two-way (local-vs-venue) comparison. A bounded
        // lookback query on this OCCASIONAL path, not the hot fold.
        ...
        // `crates/vike-ops/tests/wallet_frame_venues_gate.rs`; read it there rather than trusting a
        ...
        //        having performed it (`crates/vike-exec/src/recon/diff.rs`'s
        ...
        if reconcile_balance {
            ...
            if let Some(b) = balance_seed {
                ...
                // (`crates/vike-app-core/src/tradehub_control.rs`'s `wire_from_command` returns
                ...
                // `crates/vike-ops/tests/wallet_frame_venues_gate.rs` — deliberately not restated
        ...
        for a in recon.alerts {
            ...
            // leading text byte-identical for `crates/vike-ops/tests/held_divergence_is_visible_gate.rs`,
