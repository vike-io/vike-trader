...
//!   under the default `hybrid` policy `PositionDrift` auto-applies, so each pass rewrites the
//!   local position onto whichever account answered last.
...
/// against, and B's venue truth lands in A's book. Under the default `hybrid` policy that is not a
