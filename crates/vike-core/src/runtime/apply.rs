...
impl<C: ExecutionClient> CoreThread<C> {
    ...
    pub(crate) fn market_exit_flatten_legs(
        ...
        for idx in self.exit_scope_engines(route, venue) {
            ...
            for ((pv, symbol, side), pos) in self.eng(idx).account.positions.iter() {
                ...
                out.push((
                    idx,
                    OrderIntent::Flatten { venue: pv.to_string(), symbol: symbol.to_string() },
                ));
            }
        }
        out
            ...
            OrderIntent::MassCancel { venue, symbol } => {
                match (venue, symbol) {
                    ...
                    (Some(v), sym) => {
                        ...
                        for eidx in self.exit_scope_engines(route, Some(&v)) {
    ...
    fn submit_resolved(&mut self, req: &OrderRequest, now: i64) {
        ...
        // stage may not make. It belongs with Stage 2's wire key.
        let eidx = self
            .coid_venue
    ...
    ///
    /// **Residual, declared:** where the caller DID name one engine (a labelled mount's own
    /// venue-scoped mass-cancel), this still clears the other account's held exits. Narrowing it
    /// needs the account on the leg, and the replay twin that mirrors these semantics
    /// (`crate::replay`'s `fold_intent_scope`) folds a journaled intent that carries no route at
    /// all — so an account-scoped clear would diverge from the determinism fence. That is the
    /// journal's landing, not this one.
    ...
    /// the same declared residual for a caller that named ONE engine.
