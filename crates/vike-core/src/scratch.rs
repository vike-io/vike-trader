...
//! journal's own design rather than from many files: `crates/vike-journal/src/segment.rs`'s
...
//!
//! `crates/vike-journal/src/writer.rs`'s `open` and the reader in
//! `crates/vike-journal/src/read.rs` both call `create_dir_all` themselves, so most suites
