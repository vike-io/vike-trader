...
//!
//! The workspace's ONE production [`crate::JournalViewHook`] was built in the GUI shell's
//! `main.rs` (`crates/vike-desktop/src/main.rs`, then spelled `vike-app`), which is the one
//! workspace member the merge gate does not RUN (`xtask/src/ci/tables.rs`'s `EXCLUDE_FROM_CI`, and
//! the ratchet `crates/vike-ops/tests/ci_excluded_gui_shell_ratchet.rs` exists because of it). The
//! store walk, the `kind=`/`venue` filter, the window-scoped scans and the status parse are all
//! PURE over the `HistStore` trait, so none of them needed to be there — and while they were, no
//! test in this repository ever executed them.
//!
//! ⚠ **That hook is now GONE rather than merely elsewhere, and lifting the walk down here is what
//! made the deletion free.** The desktop cut took the local trading core out of the GUI — all
//! orders and trading leave the shell through the backend — so the `journal_view_provider` closure
//! went with the reconcile mount it fed, and [`crate::CoreConfig::journal_view_provider`] is `None`
//! at every live mount today. The logic and both of the defects it was carrying are covered by
//! `crates/vike-core/tests/recon/recon_journal_view.rs`, so a daemon-side hook can be wired to a
//! TESTED walk rather than re-derived inside a binary the merge gate does not run.
//!
...
//! (`vike_tradehub::reconcile_config::lookback_ms`), never a literal of its own.
