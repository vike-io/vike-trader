...
use vike_data::{exec_index, HistStore};
        ...
        Err(e) => {
            tracing::error!(
                ...
                "journal cross-check DEGRADED to a two-way reconcile for this venue: the hist \
                 ...
                 fill this process has lost will now surface as MissingFill (auto-applied under \
                 the default `hybrid` policy) instead of JournalDivergence"
            );
            return view;
        }
    };
