...
pub struct VenueBlock {
    ...
    ///
    /// Derived from the engine's own `route_key` through
    /// `vike_model::account_keys::label_of_route_key`, so it cannot disagree with [`Self::route_key`]
    /// about which account this block is. `None` on every single-account process — which is every
    /// process with no `[accounts]` table — so a consumer that ignores it reads what it read before
    /// the field existed.
...
mod account_fields_tests {
    ...
    use vike_model::account_keys::{AccountLabel, route_key_of};
    ...
    fn two_accounts_of_one_exchange_publish_two_distinguishable_blocks() {
        ...
        assert_eq!(
            venues,
            vec!["binance", "binance"],
            "…which is exactly why `venue` cannot answer"
        );
    ...
    fn a_built_snapshot_publishes_its_own_engines_epoch() {
        let snap =
            build_of(&engine("binance", "binance"), &[(1.0, engine("binance", "binance#ALT"))]);
        assert_eq!(
            snap.accounts_epoch,
            CoreSnapshot::accounts_epoch_of(["binance", "binance#ALT"]),
        );
        assert_ne!(
            snap.accounts_epoch,
