...
pub mod account_ambiguity;
pub mod control;
pub mod core_sink;
pub mod counters;
pub mod emulator;
// ⚠ `pub mod journal;` and `pub mod journal_lock;` stood here until 2026-09-23. Both are the
// `vike-journal` crate now (layer 25, under this one): the write-ahead command journal and the
// single-writer directory lock that nothing outside it ever named.
//
// No rank was unblocked by the move and none was the reason for it. `vike-report` named THIS
// crate for exactly two things and both of them were the journal — a reporting crate linking the
// live trading core to read a log file — and the journal is the part of this crate that grows
// with execution volume. A thing that keeps growing is better off with its own boundary than
// inside a crate named for something else.
//
// ⚠ The append path is STILL in this crate's gated fold: `tests/runtime_latency.rs` has three
// variants and two are journal-on. The crate boundary is new, the `JOURNAL_P99_NS` budget is not.
// The store-backed builder for `CoreConfig::journal_view_provider`'s third reconcile leg. NOT the
// WAL (`vike-journal`) — that is the command journal; this reads the MATERIALIZED exec log back.
...
pub use recon_manager::{
    HealthProbe, ReconConfig, ReconDriver, ReconHealth, ReconLeg, ReconManager, spawn_recon,
};
pub use run_profile::{
    ConfirmGraceHazard, GridSource, Guards, GuardsReport, Mode, ProfileError, ProfileMarginCall,
    ProfileRisk, ProfileTradingState, RunProfile, Sinks, journal_config_from,
    journal_config_from_env, resolve_profile,
};
pub use runtime::{
