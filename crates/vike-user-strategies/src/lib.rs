...
/// The COMPILER-FREE resolver a NAMED RUN goes through, and the roster it serves — the FENCE
/// `docs/decisions/0064-a-named-run-carries-no-source.md`'s decision 2 rests on. It lives HERE
/// because this is the only crate that can see BOTH halves of a named run's roster (the portable
/// registry and the generated user one) and can see no compiler at all.
pub mod named_run;
