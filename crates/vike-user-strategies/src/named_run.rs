...
//!
//! * `vike_strategy::strategy_by_name` — `vike-strategy` and `vike-script` declare the SAME layer
//!   rank, and `crates/vike-ops/tests/layer_gate.rs` fails a PR whose normal `vike-*` dependency
//!   does not declare a STRICTLY LOWER one. `crates/vike-backtest/src/harness/registry.rs`'s
//!   `"rhai"` arm cites that fact at the arm as the reason it lives in `vike-backtest` at all.
