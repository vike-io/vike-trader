...
//! log nothing was reading, and on 2026-08-05 a Polymarket series collapsed from ~250–370k rows/min
//! to **18/min for 19 minutes** — ~3.8M rows lost — with nothing said. So the assertion that
//! matters here is not "the condition was detected" (the unit tests in `liveness` already pin
//! that): it is that a `FiredAlert` reached a SINK.
//!
...
use vike_recorder::alerts::{watchdog_tick, RecorderAlerts};
...
fn a_series_that_never_starts_alerts_after_its_grace() {
    ...
    assert!(watchdog_tick(&mut watch, &mut alerts, &expect(&[series]), &live(&[]), 0, SILENT_SECS)
        .is_empty());
...
fn every_silent_series_of_one_episode_reaches_the_pager() {
    ...
    let silent =
        watchdog_tick(&mut watch, &mut alerts, &names, &live(&rows), 1_000 + 600_000, SILENT_SECS);
...
fn a_persisting_outage_pages_once_but_keeps_reporting_every_tick() {
    ...
    for tick in 0..10 {
        ...
        let silent =
            watchdog_tick(&mut watch, &mut alerts, &expect(&[series]), &rows, now, SILENT_SECS);
