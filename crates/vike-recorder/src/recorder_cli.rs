...
/// The BOUNDED half of the teardown — the flush and the compaction join — is hard-capped at this.
///
...
/// sub-second on the measured the CI box tape. What does NOT have to fit is the `MaintenanceScheduler`
/// join — a compaction pass merges up to `max_merge_rows` and can genuinely take minutes — which is
/// exactly why it is the LAST step of the sequential tail: a hard cap abandons the pass, which is
/// restartable and costs nothing, and never the rows.
///
...
fn run(
    ...
    // tasks: both steps are load-bearing SEQUENTIAL, so it is all the `then` tail.
    ...
    match run_with_deadline(Vec::new(), teardown, budget) {
        ...
        ShutdownOutcome::HardCapped { .. } => tracing::warn!(
            ...
            "recorder: teardown hard-capped at its deadline — a compaction pass was abandoned \
             (restartable); if a flush was still running, check the store's manifest on next open"
        ),
    }
    ...
    fn the_whole_stop_fits_inside_the_units_stop_timeout() {
        ...
        assert!(
            ...
            "the TOTAL stop budget ({TOTAL_STOP_BUDGET_SECS}s = {FEED_STOP_BUDGET_SECS}s \
             unsubscribing the feeds + {SHUTDOWN_DEADLINE_SECS}s flushing and joining compaction) \
