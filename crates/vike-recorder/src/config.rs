...
pub struct RecorderProfile {
    /// Store root. The recorder is the only writer here; `vike-datahub` reads the same root from a
    /// SEPARATE process (spec §8.1), which is why the store's `SeriesLock` is a lockfile.
    pub store: PathBuf,
    #[serde(default)]
    ...
    fn validate(&self) -> Result<(), ProfileError> {
        ...
        for (i, s) in self.subscribe.iter().enumerate() {
            match (&s.family, s.symbols.is_empty()) {
                ...
                (Some(_), false) => {
                    return Err(ProfileError::FamilyAndSymbols { index: i, venue: s.venue.clone() })
                }
                _ => {}
            }
            if let Some(fam) = &s.family {
                if seen.insert((&s.venue, fam), ()).is_some() {
