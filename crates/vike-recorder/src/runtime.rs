...
impl RecorderRuntime {
    ...
    /// `crates/vike-recorder/src/venues/binance.rs`'s `family_with_source` declares one too, so a
    /// binance family subscription folds exactly like a Polymarket one. `Membership`'s own doc
