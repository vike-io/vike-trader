...
use vike_exec::{event_channel, EventSender, ExecutionClient, Ingest, ManagedOrder, OrderStatus};
...
const DEFERRED: &[(&str, &str)] = &[
    ("polymarket", "behind the `polymarket` feature (EIP-712); default build is an empty crate — no mapper reachable"),
    ("dukascopy", "Java sidecar over JSON-lines (proto.rs), not a private-WS/JSON mapper; fake_jforex_bridge covers its lifecycle"),
    ("ibkr", "stateful `EventMapper` (private module) whose fills need a two-message execDetails+commissionReport join keyed by exec_id and typed (non-JSON) reports — no stateless `decode(&Value)` shape; covered by its own tests/ibkr_lifecycle.rs"),
    ("ctrader", "cTrader Open API is protobuf-framed — `exec_event_to_events` takes `ProtoOaExecutionEvent` structs + a `SymbolMap`, not JSON; wiring it needs a JSON→proto reconstruction axis beyond this PR"),
