...
use vike_bridge_core::exec_actor::{run_loop, ExecCommand};
...
use vike_exec::{
    event_channel, Account, BalanceMode, EventHandler, ExecutionEngine, Ingest, Outbox, RiskGate,
    RiskLimits,
};
...
fn resync_across_reconnects(history: Vec<Event>, reconnects: u64) -> Vec<Event> {
    ...
    for n in 1..=reconnects {
        gen.fetch_add(1, Ordering::Relaxed); // a reconnect: the pump bumps the session gen
