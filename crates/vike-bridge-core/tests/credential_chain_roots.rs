...
use vike_bridge_core::credentials::{
    load_credentials_from, load_workspace_dotenv, load_workspace_secrets_at,
    load_workspace_secrets_from_env, Environment,
};
...
fn a_named_settings_directory_supplies_the_credentials() {
    ...
    assert!(std::fs::read_to_string(settings.join("secrets.env"))
        .unwrap()
