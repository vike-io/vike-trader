...
//!   never transformed. ⚠ A second consumer of those labels was named here —
//!   `metrics.rs`'s `binary_logloss`, which widened them and took `libm::log` on the result — and
//!   that module is DELETED. The point it supported is unchanged and now rests on the two sites
//!   above: an `f32` label is hashed and widened in this tree, never arithmetic'd at `f32` width.
...
//!
//! ⚠ **This heading read "EVERY pinned constant below reads `RECORD`" long after every row had
//! been recorded, and the paragraph below still described how the file LANDED in the present
//! tense.** Both are corrected here rather than deleted, because the convention they describe is
//! live and is what a future re-record has to obey.
//!
...
//!
//! This whole file was WRITTEN on a box that could not run cargo, so every row landed unrecorded;
//! they have since been recorded and [`PINNED`] carries real hashes today. The rule that got them
//! there is unchanged and binds every future re-record:
...
/// what needs fixing.
///
/// ⚠ **A row reading `"RECORD"` is the placeholder convention** — see the module doc for it, and
/// for why a plausible-looking hex literal invented here would be the worst possible thing to
/// write. Record by RUNNING on both boxes; a mismatch panics with a paste-ready replacement block.
/// (This paragraph read "EVERY row is `RECORD`" long after every row had been recorded. It
/// described how the file LANDED, in the present tense, which is the same failure the quantizer
/// record in `crates/vike-bridge-core/tests/format_props.proptest-regressions` exists to correct.)
///
/// ⚠ **RESIDUAL — [`STEPS`] STOPS AT 1e-8 AND SHOULD NOT.** okx serves a 1e-12 tick on nine live
/// order-placing markets, and 1e-11 carries the deepest per-rung loss rate measured anywhere in
/// the workspace, so four rungs below 1e-8 are quantized on real orders and pinned by nothing
/// here. `crates/vike-bridge-core/tests/grid_multiple_survives_the_wire.rs`'s `LADDER` was
/// extended to cover them and re-measures them every run, so the DEFECT is watched either way —
/// what is missing is this file's PLATFORM claim over those steps. Extending [`STEPS`] is not a
/// prose edit: every row below indexes it with `i % STEPS.len()`, so four more rungs reassign
/// every sample's step and move every hash — case (a) of the panic message, an intended change
/// needing a full re-record on BOTH boxes. The Linux half was already measured on the CI box on
/// 2026-09-14 with the four rungs present and is recorded beside the rung table in
/// `crates/vike-bridge-core/tests/format_props.proptest-regressions`, so whoever picks this up
/// only has to run Windows and compare. ⚠ Do not promote those figures into this array on their
/// own — a pin copied from one box asserts something nobody measured, which is the exact thing
