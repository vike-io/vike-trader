...
//!   `@depth` DIFFS fold. ⚠ **TWO grammars, two rows** ([`DepthGrammar`]) — the spot stream's
//!   `U == prev u + 1` rule, and the USDⓈ-M futures stream's `pu` back-pointer, which the frame
//!   itself declares by carrying (or not carrying) that field. Either way a break is
//!   `MdEvent::Resync`. [`BookModel::DeltaSync`]. L1 rides a SEPARATE `@bookTicker` channel (not
//!   derived from the book), so invariant 3 is N/A. This suite ran only the spot shape until
//!   2026-09-10, which is why it stayed green through forty days of a mis-synced perp lane.
...
trait MarketDataBridge {
    ...
    /// What the matrix and a failure message CALL this row. Defaults to [`Self::venue`] and is
    /// overridden only where one venue speaks more than one wire GRAMMAR and each needs its own
    /// row — binance's spot vs USDⓈ-M depth streams, whose difference cost forty days of a
    /// mis-synced perp book. The roster gate keys on `venue()`, never on this, so several rows may
    /// share a venue without unbalancing the COVERED/DEFERRED partition.
...
fn wire_levels(levels: &[BookLevel]) -> Value {
    Value::Array(
        levels
            .iter()
            .map(|&BookLevel { price: px, qty }| json!([format!("{px}"), format!("{qty}")]))
            .collect(),
    )
}
...
fn okx_wire_levels(levels: &[BookLevel]) -> Value {
    Value::Array(
        levels
            .iter()
            .map(|&BookLevel { price: px, qty }| {
                json!([format!("{px}"), format!("{qty}"), "0", "1"])
            })
            .collect(),
    )
}
...
/// Which of the family's TWO depth grammars a row drives — the distinction this harness did not
/// make, and whose absence let a real defect stay green for forty days.
///
/// `family::depth::apply_depth_event` reads the grammar off the FRAME: a `pu` (previous final
/// update id) field means the USDⓈ-M futures chain rule, its absence means the spot `U == prev u+1`
/// rule. Every fixture in this file (and in both venues' own `route_frame` tests) used to synthesise
/// `Contiguous` as `U = last + 1` with no `pu` — the SPOT shape — so the suite certified the spot
/// half twice over and the futures half never, while the CI box's `kind=depth/binance/BTCUSDT.P` series
/// reconnect-looped at 4 % of its expected rate.
#[derive(Clone, Copy)]
enum DepthGrammar {
    /// `stream.binance.com` — no `pu`; `U` is exactly the previous event's `u + 1`.
    Spot,
    /// `fstream.binance.com` (and BOTH aster planes) — carries `pu`; `U` runs far ahead of
    /// `prev.u + 1` (measured 2026-09-10 from the CI box: median 93 ids, max 781, contiguous on 0 of 199
    /// consecutive pairs), so ONLY `pu` proves continuity.
    Futures,
}

struct Binance(DepthGrammar);
impl MarketDataBridge for Binance {
    ...
    fn label(&self) -> &'static str {
        match self.0 {
            DepthGrammar::Spot => "binance/spot",
            DepthGrammar::Futures => "binance/perp",
        }
    }
    fn model(&self) -> BookModel {
        BookModel::DeltaSync
    ...
    fn delta(
        ...
        // Binance diffs carry a `U`(firstUpdateId)/`u`(finalUpdateId) SPAN, and — on the futures
        // grammar only — a `pu` back-pointer. apply_depth_event:
        //   final_u <= last_seq                        → Stale (both grammars);
        //   `pu` present: pu == last_seq || U <= last_seq   → Apply, else Gap;
        //   `pu` absent:  U <= last_seq + 1                 → Apply, else Gap.
        let last = book.last_seq;
        let mut data = match intent {
            // ⚠ The two grammars must express "in sequence" DIFFERENTLY, or this row is a copy of
            // the other one. Spot's next event opens at `last + 1`; a futures event opens wherever
            // it likes and PROVES continuity with `pu` — so the futures fixture deliberately jumps
            // `U` a hundred ids ahead, which is the shape that was read as a gap on every real
            // perp session.
            SeqIntent::Contiguous => match self.0 {
                DepthGrammar::Spot => json!({"U": last + 1, "u": last + 3}),
                DepthGrammar::Futures => {
                    json!({"U": last + 100, "u": last + 200, "pu": last})
                }
            },
            // final_u <= last_seq → already reflected. Same test on both grammars, so the futures
            // row still carries a `pu` (a real frame always does) to prove the Stale arm is
            // reached BEFORE the grammar split.
            SeqIntent::Stale => match self.0 {
                DepthGrammar::Spot => json!({"U": last, "u": last}),
                DepthGrammar::Futures => {
                    json!({"U": last.saturating_sub(5), "u": last, "pu": last.saturating_sub(9)})
                }
            },
            // frames were dropped: spot's span skips the next seq; the futures span opens past the
            // anchor AND its `pu` names the event that vanished.
            SeqIntent::Gapped => match self.0 {
                DepthGrammar::Spot => json!({"U": last + 5, "u": last + 10}),
                DepthGrammar::Futures => {
                    json!({"U": last + 50, "u": last + 90, "pu": last + 40})
                }
            },
        };
        let obj = data.as_object_mut().expect("the fixture is an object");
        obj.insert("b".into(), wire_levels(bids));
        obj.insert("a".into(), wire_levels(asks));
        let frame = json!({"stream": "btcusdt@depth", "data": data}).to_string();
...
struct Bybit;
impl Bybit {
    fn book_frame(msg_type: &str, seq: u64, bids: &[BookLevel], asks: &[BookLevel]) -> String {
        json!({
...
struct Okx;
impl Okx {
    fn books5_frame(seq: u64, bids: &[BookLevel], asks: &[BookLevel]) -> String {
        json!({
...
impl Deribit {
    ...
    fn levels(levels: &[BookLevel]) -> Value {
        Value::Array(
            levels.iter().map(|&BookLevel { price: px, qty }| json!(["new", px, qty])).collect(),
        )
    }
    fn frame(
        kind: &str,
...
fn covered_bridges() -> Vec<Box<dyn MarketDataBridge>> {
    vec![
        Box::new(Binance(DepthGrammar::Spot)),
        // The row this suite was missing. It is RED against the pre-2026-09-10
        // `family::depth::apply_depth_event` — `Contiguous` answers `Resync` — which is what makes
        // it worth having; a second spot-shaped row would have stayed green through the defect.
        Box::new(Binance(DepthGrammar::Futures)),
        Box::new(Bybit),
        Box::new(Okx),
        Box::new(Deribit),
    ]
...
const DEFERRED: &[(&str, &str)] = &[
    (
        "aster",
        "market-data feed forks binance verbatim — `vike_aster::market_data` re-exports `family::depth::{route_frame, MdEvent}`; the shared DeltaSync normalizer + its gap arm are exercised through the two `binance` rows (a dedicated aster row would re-run byte-identical asserts; pump_spec groups `binance|aster` for the same reason). ⚠ Aster speaks the FUTURES grammar on BOTH planes — measured 2026-09-10 from the CI box, 100 frames each: fstream 100/100 carry `pu` with `U == prev.u + 1` on 0/99, and sstream ALSO 100/100 carry `pu` with `U == prev.u + 1` on only 18/99 — so the `binance/perp` row, not the `binance/spot` one, is the row that covers aster's spot lane. That is a property of the WIRE and the shared normalizer reads it off the frame, which is why this deferral still holds",
    ),
    (
        "hyperliquid",
...
const SEED_BIDS: &[BookLevel] = &[BookLevel::new(60000.0, 5.0), BookLevel::new(59999.0, 2.0)];
const SEED_ASKS: &[BookLevel] = &[BookLevel::new(60001.0, 4.0)];
...
fn inv_monotonic_seq(b: &dyn MarketDataBridge) -> Result<Cell, String> {
    ...
    let BookLevel { qty: seed_bid_qty, .. } = book.best_bid().ok_or("seed produced no best bid")?;
    ...
    match b.model() {
        BookModel::DeltaSync => {
            ...
            let out =
                b.delta(&mut book, SeqIntent::Contiguous, &[BookLevel::new(60000.0, 8.0)], &[]);
            ...
            let BookLevel { qty: q, .. } =
                book.best_bid().ok_or("no best bid after contiguous delta")?;
            ...
            let out = b.delta(&mut book, SeqIntent::Stale, &[BookLevel::new(60000.0, 999.0)], &[]);
            ...
            let BookLevel { qty: q, .. } =
                book.best_bid().ok_or("no best bid after stale delta")?;
        ...
        BookModel::SnapshotFull => {
            ...
            let out = b.snapshot(
                &mut book,
                200,
                &[BookLevel::new(60000.0, 8.0)],
                &[BookLevel::new(60001.0, 4.0)],
            );
            ...
            let BookLevel { qty: q, .. } =
                book.best_bid().ok_or("no best bid after full snapshot")?;
...
fn inv_gap_resync(b: &dyn MarketDataBridge) -> Result<Cell, String> {
    match b.model() {
        ...
        BookModel::DeltaSync => {
            ...
            let BookLevel { price: bid_px_before, qty: bid_qty_before } =
                book.best_bid().ok_or("seed produced no best bid")?;
            let BookLevel { price: ask_px_before, .. } =
                book.best_ask().ok_or("seed produced no best ask")?;
            ...
            let out = b.delta(&mut book, SeqIntent::Gapped, &[BookLevel::new(60000.0, 999.0)], &[]);
            ...
            let BookLevel { price: bid_px, qty: bid_qty } =
                book.best_bid().ok_or("gap dropped the best bid")?;
            check!(
            ...
            let BookLevel { price: ask_px, .. } =
                book.best_ask().ok_or("gap dropped the best ask")?;
...
fn inv_derived_l1(b: &dyn MarketDataBridge) -> Result<Cell, String> {
    ...
    let BookLevel { price: bb, qty: bbq } = book.best_bid().ok_or("seed produced no best bid")?;
    let BookLevel { price: ba, qty: baq } = book.best_ask().ok_or("seed produced no best ask")?;
...
fn coverage_matrix() {
    ...
    out.push_str(&format!("{:<14}", "lane"));
    for inv in Invariant::ALL {
        out.push_str(&format!(" | {:<14}", inv.label()));
    }
    out.push('\n');
    out.push_str(&"-".repeat(14 + Invariant::ALL.len() * 17));
    out.push('\n');

    let mut failures: Vec<String> = Vec::new();
    for b in &bridges {
        out.push_str(&format!("{:<14}", b.label()));
        for inv in Invariant::ALL {
            let cell = match run_cell(&**b, inv) {
                Ok(Cell::Pass) => "PASS".to_string(),
                Ok(Cell::NotApplicable(_)) => "N/A".to_string(),
                Err(why) => {
                    failures.push(format!("[{} × {}] {why}", b.label(), inv.label()));
                    "FAIL".to_string()
                }
            };
    ...
    out.push_str(&format!(
        "\nCovered: {} lane(s) × {} invariant(s). Deferred: {} venue(s).\n",
        bridges.len(),
        Invariant::ALL.len(),
        DEFERRED.len(),
    ));
