...
//!    that reason. The other shipped units deliberately do NOT: `deploy/vike-datahub.service` and
//!    `deploy/vike-backtest.service` construct no `ExecutionClient` and so have no submit boundary
//!    to read it at — recording a tape is not an execution path.
//!
//!    ⚠ **NO TRACKED UNIT EXERCISES RUNG 2 ANY MORE, AND THE ONE THAT DID WAS A FILE RATHER THAN
//!    A BOX.** Until the 2026-09-16 unit collapse this repo shipped a second tradehub unit with
//!    the CI box's root filled in, and it OMITTED this line deliberately — so the built-in default was
//!    what a real deployment used, and that was the whole evidence rung 2 worked outside a test. A
//!    daemon has ONE unit file now, and it sets the line. What still exercises rung 2 is the
//!    INSTALLED unit on the CI box, which omits it (measured 2026-09-16) and is not a tracked artifact
//!    anyone can read from here. Treat the rung as covered by
//!    `crates/vike-bridge-core/tests/halt_default_path.rs` and by that install, not by a file in
//!    `deploy/`.
...
//!   them, so a single recogniser matches any of them.
//!
//!   ⚠ Three of the four used to do it SILENTLY and cTrader was the exception
//!   (`docs/ops/kill-switches.md` section 5, now closed). Worth knowing because a binary older than
//!   that change still returns nothing here, and because it names the property the fix was for: a
//!   silent refusal is byte-indistinguishable from a modify the adapter LOST. The divergence was
//!   only ever in the NOTIFICATION, never in the verdict.
//!
