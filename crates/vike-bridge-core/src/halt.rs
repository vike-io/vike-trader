...
//!    `deploy/vike-datahub-record.service` construct no `ExecutionClient` and so have no submit
//!    boundary to read it at — recording a tape is not an execution path — and
//!    `deploy/vike-tradehub-project.service` omits it to exercise rung 2.
