...
//! against one shape.
//!
//! ⚠ **…and since 2026-09-22 they all return the same CONTENT, which they did not before.** Ruling
//! 10 moved ten config-shaped names out of the `credential` table into `venue_setting`, and the
//! fold that renders them back into the names a loader looks up used to live in THIS file — so it
//! was applied by the rows above and by nothing below them. `vike_secrets::load_workspace_dotenv_from`
//! and `vike_secrets::load_workspace_dotenv_scoped`, which the ibkr smokes and
...
//! operator rows read by nothing. The grammar moved DOWN into `vike_secrets::venue_setting_names`
//! and the store folds inside its own front door, so the difference between these doors is now
//! only what they REPORT, never what they resolve. `load_workspace_dotenv_from`'s own doc carries
//! the measurement.
//!
...
///
/// ⚠ **The `venue_setting` FOLD is not performed here any more, and that is the point of the
/// 2026-09-22 move.** It used to be — this function called a `fold_venue_settings_reported` that
/// lived in this file, because the name renderer did too. That made the fold reachable only from
/// this layer, so `vike_secrets::load_workspace_dotenv_from` and
/// `vike_secrets::load_workspace_dotenv_scoped` — two readers BELOW it, one of them the polymarket
/// egress path — resolved the pre-move answer in silence. The renderer moved down into
/// `vike_secrets::venue_setting_names` and the fold now happens inside the store, so this function
/// gets a folded map without asking and every reader gets the same one.
...
pub fn try_load_workspace_secrets_scoped_at(
    ...
    // The fourth, same as the whole-map reader — and computed INSIDE the scope, so it names only
    // collisions on names this process declared. See `report_venue_setting_collisions`.
    report_venue_setting_collisions(&scoped.collisions);
    Ok(scoped)
}
...
pub fn try_load_workspace_secrets_at(
    ...
    // ⚠ THE FOURTH FINDING, and the one that used to be computed here rather than reported here.
    // The store folds ruling 10's `venue_setting` rows itself now (`vike_secrets::resolve_store_in`)
    // and hands back the names BOTH tables answered for; this crate is where the logging dependency
    // is, so this is where the half-done move is said out loud. NAMES only — the value is never
    // printed, exactly as for the three findings above.
    report_venue_setting_collisions(&resolved.collisions);
    Ok((resolved.secrets.into_map(), resolved.source))
}

/// **Say out loud that BOTH tables answer for a name** — ruling 10's move is half done.
///
/// One spelling, called by every loud reader, so the whole-map path and the scoped path cannot
/// word this differently. Empty is the normal state in both directions and says nothing.
///
/// ⚠ NAMES, never values. A credential name is already public knowledge (it is
/// `vike_ops::settings::SETTINGS`' whole subject matter); the value is the credential.
fn report_venue_setting_collisions(collisions: &[String]) {
    if collisions.is_empty() {
        return;
    }
    tracing::error!(
        names = ?collisions,
        "⚠ the credential store and the venue_setting rows BOTH answer for these names — ruling \
         10's move is HALF DONE. The CREDENTIAL value is the one in force, which is what this box \
         resolved before the move started; nothing is guessed at. `vike-cli secrets \
...
/// `vike_secrets::venue_setting_names` on 2026-09-22 and took its pure-grammar tests with it —
...
mod venue_setting_renderer_tests {
    ...
    use vike_secrets::venue_setting_key;
...
/// **The scoped credential read, with the half-done-move finding REPORTED.**
///
/// `crates/bridges/polymarket/src/egress.rs`'s `dotenv_proxy_vars` reads the store through this
/// path rather than through a composition root's map, so this is the one door the five polymarket
/// proxy keys go through.
///
/// ⚠ **It is a FUNCTION here, not a re-export, and what it adds CHANGED on 2026-09-22.** It used
/// to add the FOLD: this crate owned the name renderer, so this was the only layer that could
/// apply it, and `vike_secrets::load_workspace_dotenv_scoped` below was the unfolded primitive.
/// That was the defect rather than the design — the primitive is what `egress.rs` and every other
/// crate below this one can reach, and an unfolded one is a reader that resolves the pre-move
/// answer in silence. The renderer moved DOWN into `vike_secrets::venue_setting_names`, the store
/// folds inside its own front door, and both spellings now answer identically.
///
/// What is left here is the REPORT. `vike-secrets` carries no logging dependency and hands the
/// collision back as DATA (`ScopedSecrets::collisions`); this crate has `tracing`, so this is
/// where a half-done move is said out loud. That is a genuine behavioural difference, not an
/// alias: a caller that wants the finding without a subscriber names the `vike-secrets` function.
///
/// ⚠ **A folded name is added only where the scope DECLARED it** — unchanged, and now enforced one
/// layer down, inside `vike_secrets::resolve_store_scoped_in`. The scope is the read's own
/// narrowing (owner ruling, 2026-09-16: a process materialises only what it asked for), and a fold
/// that ignored it would quietly hand a reader keys it never asked for.
#[must_use]
pub fn load_workspace_dotenv_scoped(scope: &KeyScope) -> ScopedSecrets {
    let base = vike_secrets::load_workspace_dotenv_scoped(scope);
    report_venue_setting_collisions(&base.collisions);
    base
}
