...
fn fold_venue_settings(
    ...
    // decoding here — `VenueSettingRow::value` is a plain string, where a `setting` row's is a JSON
    // scalar.
    for row in &rows.venue {
        for name in venue_setting_names(&row.venue, row.tier.as_deref(), &row.field) {
            match map.entry(name.clone()) {
                std::collections::hash_map::Entry::Occupied(_) => collisions.push(name),
                std::collections::hash_map::Entry::Vacant(slot) => {
                    // VERBATIM — `venue_setting.value` is a plain string. No codec, because the
                    // column is not inside a typed schema that would parse it.
