        ...
        for name in venue_setting_names(&venue, tier.as_deref(), &field) {
            match map.entry(name.clone()) {
                ...
                std::collections::hash_map::Entry::Vacant(slot) => {
                    // ⚠ DECODED — the row holds a JSON scalar and this map holds plain strings.
                    // Reading it raw is half of the pair that broke the CI box; see
                    // [`decode_setting_scalar`], which also carries why a raw row still reads.
                    slot.insert(decode_setting_scalar(&row.value));
                }
            }
        }
    }
...
/// **A `setting` row's value is ONE JSON SCALAR; the credential map holds PLAIN strings.** This is
/// the encoder for the boundary between them, and its twin is [`decode_setting_scalar`].
///
/// ⚠ **Writing the plain string straight into that column broke a live box.** MEASURED on the CI box
/// 2026-09-21: `secrets move-venue-config` stored `https://…` where the column wanted `"https://…"`,
/// `vike_config`'s loader hit `expected value at line 1 column 1` on the FIRST such row and stopped
/// parsing the WHOLE `config` section, and every setting fell back to its compiled-in default —
/// which caps every venue at `paper` with nothing recording that an arming was ever stated. The
/// running daemon still held what it booted with, so nothing traded differently; the next restart
/// would have brought the fleet up on paper, in silence.
///
/// ⚠ **Both halves were wrong in a MATCHING way, which is why no test caught it.** The fold read
/// the same column back raw, so the venue bridges saw exactly the value they expected and only the
/// settings loader — a different reader of the same row — ever objected. A round trip through
/// `vike_config`'s parser is the check that bites; a write-then-read through this pair is not.
///
/// It lives HERE rather than in `vike-secrets` because that crate declares exactly ONE external
/// dependency (rusqlite) and both call sites already link `serde_json` — the same reason
/// `vike_secrets::move_pending_rows` takes its classifier as a closure.
#[must_use]
pub fn encode_setting_scalar(value: &str) -> String {
    // Infallible for a `&str`; the fallback keeps the signature total rather than panicking on a
    // path that cannot be reached.
    serde_json::to_string(value).unwrap_or_else(|_| format!("{value:?}"))
}

/// The inverse of [`encode_setting_scalar`] — a stored JSON scalar back to the plain string a
/// credential reader expects.
///
/// ⚠ **A value that does not parse is returned AS-IS, deliberately.** A box that already ran the
/// unencoded move holds raw rows, and refusing them would take the venue configuration away from
/// every reader on exactly the boxes that need it back. `docs/decisions/0013`'s degrade-rather-than-
/// refuse rule, applied to a row this workspace itself mis-wrote.
///
/// A non-string scalar (`4002`, `true`) renders as its literal, which is what the credential map —
/// a `HashMap<String, String>` — has always carried for these keys.
#[must_use]
pub fn decode_setting_scalar(stored: &str) -> String {
...
/// **The JSON-scalar boundary between a `setting` row and the credential map**, which broke a live
/// box because both sides were wrong in the same direction and therefore agreed.
#[cfg(test)]
mod setting_scalar_codec_tests {
    use super::*;

    /// A round trip returns the value unchanged — including the shapes a URL, a host and a port
    /// actually take on the box this was measured on.
    #[test]
    fn every_moved_shape_survives_the_round_trip() {
        for value in [
            ...
            "cpapi",
            "",
            // The ones a hand-rolled quoter would get wrong.
            "a value with \"quotes\" in it",
            "back\\slash",
            "line\nbreak\ttab",
        ] {
            let stored = encode_setting_scalar(value);
            assert_eq!(decode_setting_scalar(&stored), value, "round trip failed for {value:?}");
        }
    }

    /// ⚠ **The assertion the missing test would have made.** The stored form must be something
    /// `serde_json` can parse as a scalar — that is the property `vike_config`'s loader needs, and
    /// the one a write-then-read through this pair alone would NOT have caught, because a raw
    /// string round-trips through a raw reader perfectly well.
    #[test]
    fn the_stored_form_is_a_json_scalar_which_is_what_the_settings_loader_needs() {
        let stored = encode_setting_scalar("https://example.invalid/x.jnlp");
        assert!(stored.starts_with('"') && stored.ends_with('"'), "not quoted: {stored}");
        let parsed: serde_json::Value =
            serde_json::from_str(&stored).expect("the settings loader must be able to parse it");
        assert!(parsed.is_string(), "{parsed:?}");

        // …and the shape that actually shipped is refused by that same parser, which is the whole
        // incident in one line.
        assert!(
            serde_json::from_str::<serde_json::Value>("https://example.invalid/x.jnlp").is_err(),
            "the raw form must NOT parse — if it does, this test is not about the bug"
        );
    }

    /// ⚠ A row written by the BROKEN move still reads, because a box that already ran it holds
    /// raw rows and taking its venue configuration away would be the worse failure.
    #[test]
    fn a_raw_row_written_before_the_fix_still_reads() {
        assert_eq!(decode_setting_scalar("127.0.0.1"), "127.0.0.1");
        assert_eq!(decode_setting_scalar("cpapi"), "cpapi");
        // ⚠ …but a bare number IS valid JSON, so it decodes through the scalar arm rather than the
        // fallback. Same answer, different route — asserted so the arm is not mistaken for dead.
        assert_eq!(decode_setting_scalar("4002"), "4002");
        assert_eq!(decode_setting_scalar("true"), "true");
    }
}
