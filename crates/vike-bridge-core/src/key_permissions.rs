...
/// unfuzzy idiom as `reconcile_enabled` (`"true"`/`"yes"`/`"0"` do NOT enable).
///
/// ⚠ **Which map to pass, and the one that is never acceptable.** The REAL process env
/// (`std::env::vars().collect()`) is the primary source and must always be one of them, so a
/// shell-exported override is visible — exactly as the `VIKE_RECONCILE` master gate is read. A
/// caller may OR in a SECOND map only when that map's value for this key is a RESOLVED
/// `vike_config::Flags` value (file, with the environment applied over it) written by a
/// composition root — `crates/vike-mount/src/arming.rs`'s `withdraw_override_allowed` is the one
/// such caller, and `crates/vike-tradehub/src/tradehub_cli.rs`'s `fold_flags_into_vars` is what
/// puts the value there. A RAW workspace `.env` credential map is NOT that: it is a third source
/// that outranks nothing and answers to nobody, and OR-ing it in would let an appended line arm a
/// live-money gate past an operator who exported `=0`.
