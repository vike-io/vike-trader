...
/// rewrote it. `vike_ops::reconcile_config`'s `health_from_feed_status` reads that text and
...
/// feed-status vocabulary, the healthy spelling is per-venue, `vike_app_core::status_dot`'s
...
/// journal LEVEL on it. `vike_app_core::observe_bridge`'s `observing_status` already states the
