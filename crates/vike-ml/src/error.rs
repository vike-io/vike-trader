        ...
        match self {
            ...
            Self::UnsupportedVersion(v) => write!(
                ...
                "model text declares `version={v}`; this walker implements `{}` only, and is \
                 ...
                 binary (`just lightgbm-build`), re-run `tests/train_infer_equality.rs` on the CI box \
                 — THAT is the proof, not this check — and regenerate the golden fixtures with \
            ...
            Self::Trainer(what) => write!(
                ...
                "the LightGBM trainer is not usable: {what}. Build the pinned upstream binary with \
                 `just lightgbm-build` (the CI box only — it clones and compiles upstream C++), or pass \
                 the path to one that already exists. Inference needs no binary at all and is \
                 always available."
            ),
