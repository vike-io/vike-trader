...
//! precedent was `crates/vike-backfill/src/clickhouse_poly/ingest.rs`'s `run_export`, which shelled
//! `clickhouse-client` rather than linking a client. ⚠ That file is DELETED (2026-09-20, under the
//! owner's rule that data is fetched by API or from the venue directly, never by reaching
//! ClickHouse) and this driver is now the only subprocess-instead-of-a-crate site left in the
//! workspace. The precedent is cited rather than repointed because it is the EVIDENCE that the
//! shape was accepted here before, and a citation cannot point at itself.
//!
...
mod tests {
    ...
    /// pointed a store at a deliberately nonexistent client and asserted the error named it). ⚠ That
    /// file is DELETED (2026-09-20) and the idiom has no in-tree twin left; the citation stays
    /// because it is where this constant's shape came from, and the `DEAD_PATH_EXCEPTIONS` row in
    /// `crates/vike-ops/tests/citation_gate.rs` is what excuses it.
