...
pub struct ProfileRisk {
    ...
    ///
    /// ⚠ **`<project>/settings/policy.toml` carries a ceiling of this exact NAME that judges a
    /// DIFFERENT ACT**, and the two must not be kept "in step" by a comment — that is precisely
    /// what was MEASURED on a live box. That one guards only the EDGE surfaces a human types at;
    /// `vike_mount::MountPolicy::from` drops it before it can reach a [`RiskLimits`]. So every
    /// order it touches is also judged by THIS key, and the reverse is false — nothing compares
    /// the two numbers at load, at mount or at submit. `vike_config::ceilings::PRE_TRADE_CEILINGS`
    /// is the authority for both rows, and `vike-cli config show` renders it.
    ///
...
impl ProfileRisk {
    ...
    pub fn to_risk_limits(&self) -> RiskLimits {
        RiskLimits {
            ...
            // ⚠ NOT a profile field, and deliberately never one (the drift alarm fired as
            // designed). The ACCOUNT-aggregate ceiling is a property of the BOX and the wallet its
            // credentials open, not of the strategy run pointed at it — one number for every
            // engine the process mounts on that account, however many profiles come and go. It
            // arrives from `<project>/settings/policy.toml` through `vike_mount::MountPolicy`, and
            // `vike_config::Policy::max_account_exposure` carries the whole argument for that home.
            // Accepting it here as well would give one ceiling two authorities that disagree, and
            // the profile is the one an operator swaps with a `--profile` flag.
            max_account_exposure: None,
            // ⚠ NOT a profile field either, and for the same reason spelled out directly above:
            // the SIZING-EQUITY ceiling bounds what this BOX's engines may size and admit against,
            // and giving it a second authority an operator swaps with `--profile` would be the
            // defect the policy home exists to prevent. `vike_config::Policy::max_sizing_equity`.
    ...
    ///   collar `price_collar`/`collar_by_symbol`; and `max_account_exposure`, which belongs to a
    ///   THIRD owner — the policy FILE, reaching the mount through `vike_mount::MountPolicy` —
    ///   rather than to the venue or to this profile): carried over from `base` untouched. This
    ...
    pub fn apply_to(&self, base: RiskLimits, grid: GridSource) -> Result<RiskLimits, ProfileError> {
        ...
        Ok(RiskLimits {
            ...
            // file's, via `vike_mount::MountPolicy`), so it belongs to the same "neither side's
            // concern" group as the collar above. Carrying it is what makes the mount's fold order
            // a readability choice instead of a load-bearing one: were this `None`, merging a
            // profile that never mentions the key would silently disarm the box's account ceiling,
            // which is the exact `im_requirement` hazard `make_engine_for_account`'s merge block
            // records.
            max_account_exposure: base.max_account_exposure,
            // Carried from `base` for the identical reason as the line above — a THIRD owner's
            // field (the policy file's, via `vike_mount::MountPolicy`), so a profile that never
            // mentions it must not silently disarm the box's equity ceiling.
    ...
    pub fn apply_operator_budget_only(&self, base: RiskLimits) -> RiskLimits {
        ...
        RiskLimits {
            ...
            collar_by_symbol: base.collar_by_symbol,
            grid_by_symbol: base.grid_by_symbol,
            // From `base`, exactly as in `apply_to` above — see that line. This path is the one
            // reached when a profile was REJECTED for setting a venue-owned field, and a
            // rejected profile must not take the box's account ceiling down with it.
            max_account_exposure: base.max_account_exposure,
            // From `base`, exactly as in `apply_to` above — and this path is the REJECTED-profile
            // one, where dropping it would let a bad profile take the box's equity ceiling with it.
            max_sizing_equity: base.max_sizing_equity,
        }
    }
}
