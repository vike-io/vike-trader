...
pub trait ReconClient: Send {
    ...
    /// **WHICH ACCOUNT this client's key trades, as the VENUE names it** — the answer
    /// `vike_secrets::Account::venue_account_id` is for, and the one thing the credential store
    /// cannot supply for a venue whose key is an opaque HMAC string.
    ///
    /// Default `Ok(None)` = not wired for this venue, exactly like [`Self::fetch_fee_rates`] above.
    /// `vike_mount::book_identity`'s `Undeterminable` rows say only that *an authenticated call*
    /// could answer; this is that call, once a venue has one.
    ///
    /// # ⚠ THE ONE VALUE ON THIS TRAIT THAT MAY BE CACHED, and the rule is IMMUTABILITY
    ///
    /// An account's id does not change for the life of a key. A balance changes every second and a
    /// fee schedule changes when a tariff does — so those two hit the network on EVERY call, and
    /// must keep doing so:
    ///
    /// * [`Self::fetch_balance`] is what the startup preflight probes with, and it is how a REFUSED
    ///   or EXPIRED key is discovered (measured 2026-09-20: a bybit key answered *Your api key has
    ///   expired* and the preflight degraded that venue to paper). Caching it would silently retire
    ///   that check;
    /// * [`Self::fetch_fee_rates`] feeds a live fee schedule that a tariff change moves.
    ///
    /// So an implementation may remember THIS answer and only this one.
    ///
    /// # ⚠ …and a cache cannot answer before the first fetch
    ///
    /// Which is the whole design, not a caveat: the rule is **whichever call pulls the venue's
    /// account body first populates it**. On binance that body is `/api/v3/account` — already
    /// fetched by `fetch_balance` at every startup preflight and by `fetch_fee_rates` at every live
    /// mount — so recording the account costs NO extra request whichever of them runs first. On okx
    /// nothing fetches `/api/v5/account/config` at all, so the first call here is a genuinely new
    /// request and the cache saves only the repeats.
    ///
    /// The cache lives as long as the CLIENT, which is rebuilt at every mount. So a session sees one
    /// fetch at most, and every restart re-asks — which is what keeps an expired key discoverable.
