    ...
    pub fn describe(&self) -> String {
        match self {
            ...
            Divergence::MissingTerminal { order } => {
                format!("{} {} coid={:?} status={:?}", order.venue, order.symbol, order.client_order_id, order.status)
            }
            Divergence::OrphanLocalOrder { client_order_id } => format!("coid={client_order_id}"),
            Divergence::UnknownOrder(o) => {
                format!("{} {} coid={:?} status={:?}", o.venue, o.symbol, o.client_order_id, o.status)
            }
            ...
            Divergence::OrphanLocalPosition { venue, symbol, position_side, local_qty } => {
                format!("{venue} {symbol} {position_side} local_qty={local_qty} — the venue reports none")
            }
