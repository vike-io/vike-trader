...
impl Divergence {
    ...
    ///
    /// ⚠ This is the ORIGIN-BLIND answer. [`Self::origin_for`] is the one
    /// [`crate::recon::mode_applies_divergence`] actually asks, because a coid can now NAME the
    ...
    ///
    /// `MissingFill` is the interesting row — a `FillReport`'s coid echo is `Option`, and `None`
    /// is already the "foreign order's fill" signal [`Self::origin`] refines on.
    /// `OrphanLocalPosition`/`PositionDrift`/`PositionOnlyExternal`/`BalanceDrift` are position-
    /// and cash-shaped and name no order at all; `JournalDivergence` carries a recoverable venue
    /// order but its mode is decided before any of this is consulted (`resolve` short-circuits
    /// it), so it deliberately answers `None` rather than inviting a fold decision on it.
    ...
    ///
    /// This is the cross-MACHINE half of the duplicate-instance interlock
    /// (`crates/vike-journal/src/lock.rs` is the same-machine half, and its module doc lists
    /// what it cannot reach). Two deployments sharing one venue API key each hold their own
    /// journal lock and each believe they are alone; if both declare an
    /// [`vike_model::InstanceOrigin`], every id they mint says which one placed it, and this is
    /// where that claim is read.
    ///
    /// `false` unless the answer is positively yes — see
    /// `crates/vike-model/src/instance_origin.rs`'s `coid_is_foreign` for why the asymmetry is the
    /// safety property. In particular an UNTAGGED id is never foreign: an instance that turns the
    /// feature on while a sibling has not is in exactly the position it was in before, not in a
    /// worse one.
    ...
    ///
    /// It names BOTH tags, because the useful question is not "is this foreign" but "which of my
    /// deployments placed it" — an operator with three tagged instances needs the answer to know
    /// which box to look at. Neither tag is a secret: an origin is an identity that already rides
    /// on the public wire inside every client order id.
    #[must_use]
    ...
    ///
    /// It can only ever move a divergence TOWARD External — never away from it — which is what
    /// makes it safe to consult under every policy rather than behind an opt-in: the sole
    /// consumer ([`crate::recon::mode_applies_divergence`]) turns External into a HOLD, so the
    /// worst a mis-read can do is surface something for an operator that would otherwise have
    /// folded. `local: None` (nothing configured — the default) is definitionally
    /// [`Self::origin`].
...
pub struct ReconPolicy {
    ...
    /// THIS instance's own origin claim — the identity every fold decision below is made RELATIVE
    /// TO. `None` (the default, and every constructor's value) is byte-identical to before the
    /// field existed: with nothing to compare against, [`Divergence::origin_for`] is
    /// [`Divergence::origin`] and [`crate::recon::mode_applies_divergence`] is unchanged.
    ///
    /// ⚠ **It is not a policy KNOB, and no operator sets it.**
    /// `crates/vike-core/src/runtime/mod.rs`'s `reconcile_reports` stamps it, per pass, from the
    /// tag its OWN `ClientOrderIdGenerator` is minting under — not from configuration. Two
    /// reasons, and the second is the load-bearing one:
    ///
    /// - The comparison is then against the identity that is ACTUALLY on the wire. A core whose
    ///   resumed `coid_session` predates the operator turning the origin on is still minting
    ///   untagged ids, and reconcile must judge by what it minted, not by what it was told.
    ...
    ///   reaches the same fold decision — where reading configuration would have made a replay's
    ///   folds depend on the box replaying them, which is exactly the determinism fence
    ///   `crates/vike-journal/src/lock.rs` exists to protect.
    ///
    /// The field is `pub` and settable so a pure `resolve` caller (the offline `run_pass`, a test,
    /// a future non-core consumer) can supply the identity explicitly.
    ///
    /// Serde: `default` + skipped-when-absent, so an old journaled `Command::ReconcileReports`
    /// replays unchanged and an unconfigured instance's policy still serializes byte-identically
    /// to its pre-origin form.
...
impl ReconPolicy {
    ...
    /// not disagree about what `"quarantine"` builds: `vike_tradehub::reconcile_config::parse_policy`
