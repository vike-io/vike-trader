...
impl<C: ExecutionClient> ExecutionEngine<C> {
    ...
    fn risk_ctx(&self, request: &OrderRequest, now_ms: i64) -> RiskContext {
        ...
        let (equity, margin_used, closing_credit) =
            if let Some(im_req) = self.gate.limits.im_for(&request.symbol) {
                ...
                (self.resolved_equity(self.equity_seed, &self.price_cfg), used, credit)
            } else {
                (0.0, 0.0, 0.0)
            };
        RiskContext {
    ...
    /// liquidation watchdog, drawdown latch, armed margin gate, strategy `ctx.equity`, and the
    /// portfolio-snap journal all read equity through THIS entry point, so an auto-liquidation
    /// acts on exactly the number the snapshot/sampler display. SAME resolver chain, per-position
...
impl<C: ExecutionClient> EventHandler for ExecutionEngine<C> {
    fn on_event(&mut self, event: &Event, _outbox: &mut Outbox) -> Fold {
        if let Event::Fill(fill) = event {
            ...
            if self.collect_applied_fills {
                ...
                self.applied_fills.push(AppliedFill {
                    ...
                    equity_after: self.account.equity_all(self.equity_seed),
                    fill: fill.clone(),
                });
            }
            return Fold::Applied;
        ...
        if let Some(coid) = Self::lifecycle_coid(event) {
            ...
            if let Some(tid) = tid {
                if self.seen_fsm_trade_ids.contains(tid.as_str()) {
            ...
            if self.collect_applied_fills {
                if let Some(lc) = vike_model::strategy::OrderLifecycle::from_event(event) {
                    if let Some((venue, symbol)) = self
                        .registry
                        .get(&coid)
        ...
        if let Event::PositionLiquidated(ev) = event {
            ...
            if let Some(coid) = self.coid_for_position(ev) {
                if let Some(mo) = self.registry.get_mut(&coid) {
