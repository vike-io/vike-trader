...
use vike_model::{OrderRequest, f64_from_hex_bits, f64_to_hex_bits};
...
fn limits_from(v: &serde_json::Value) -> RiskLimits {
    ...
    RiskLimits {
        ...
        // the ACCOUNT-aggregate ceiling stays off for the same reason as the three knobs above —
        // the oracle fixtures predate it, and it reads a context field the fixtures cannot supply
        // (this harness builds a `RiskContext` from fixture scalars, so the account's OTHER
        // symbols are not in the row at all). Off ⇒ the r5 verdicts are byte-identical.
        max_account_exposure: None,
        // ⚠ the SIZING-EQUITY ceiling stays off, and for a reason of its own rather than the
        // "fixtures predate it" one above: this harness feeds `RiskContext::equity` from the
        // fixture row directly, so the ceiling — which bounds the equity the ENGINE resolves
        // before the gate ever sees it (`ExecutionEngine::sizing_equity`) — has nothing to act on
        // here. Off ⇒ the r5 verdicts are byte-identical, as they must be against a frozen oracle.
