...
fn ctx(pos: f64, mark: f64, equity: f64, margin_used: f64, closing_credit: f64) -> RiskContext {
    RiskContext {
        ...
        closing_credit,
        multiplier: 1.0,
        // The ACCOUNT-aggregate ceiling's term, and `0.0` is the right value here rather than a
        // stand-in: this file's gates never arm `max_account_exposure`, so the lane that reads it
        // does not run and the number could not change a verdict below. Spelled out rather than
        // reached through `..RiskContext::default()` because the whole point of this builder is
        // that every field a margin verdict depends on is visible in one place.
