...
fn apply_snapshot_reseed_moves_the_cash_reconcile_baseline() {
    use vike_exec::recon::{BalanceCheck, BalanceTol, diff_balance};
    ...
    assert_eq!(
        diff_balance(&view, Some(1000.0), "USDT", BalanceTol::default(), 0),
        // `Anchored { drift: None }` and NOT `Adopt`: the reseed left a venue-anchored baseline, so
        // the pass DIFFS and agrees — it does not fall back to taking the venue's word. Telling
        // those two apart is the whole reason `BalanceCheck` replaced an `Option`.
        BalanceCheck::Anchored { expected: 1000.0, drift: None },
