...
//! nobody. Under `hybrid` — `parse_policy`'s own fallback, and what an operator gets by NAMING it
//! (an unset variable now folds to `quarantine` at both live mounts) — a live
...
/// none for the coid local is resting. Under `hybrid` — `parse_policy`'s own fallback, which an
/// operator reaches by NAMING it (an unset variable folds to `quarantine` at both live mounts
/// since S2) — that now surfaces
