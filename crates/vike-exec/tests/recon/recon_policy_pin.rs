...
/// THE pin, updated. Under `hybrid` — `parse_policy`'s own fallback, which an operator now reaches
/// by NAMING it (an unset variable folds to `quarantine` at both live mounts since S2),
