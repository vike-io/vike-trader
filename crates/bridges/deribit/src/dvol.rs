...
//! `VIKE_RECORD_DVOL=1` and `record_dvol = true` in `<project>/settings/flags.toml` are two
//! spellings of ONE resolved value rather than two arming paths, and a LIBRARY reading the variable
//! itself would be a SECOND source of truth for one toggle, the shape `vike_ops::settings`' STEP-2
//! ledger exists to retire. ⚠ **This sentence said both spellings "still arm it" and neither does
//! — see the ⚠ paragraph below.** Resolving a value is not passing it: no composition root passes
//! this one to [`DvolRecorder::from_flag`] or to [`spawn_deribit_dvol_feed`], so the parameter is
//! never supplied and the recorder is never built. The correction is stated HERE, fifteen lines
//! above the paragraph that carries the argument, because that is exactly the distance at which
//! the two contradicted each other. Only the cadence
...
//!
//! ⚠ **NO PRODUCTION WIRING — the flag is a knob with nothing on the other end again, and this
//! paragraph went on describing the mount for months after it was deleted.** It read: "`vike-app`'s
//! `main.rs` spawns [`spawn_deribit_dvol_feed`] … **only when `flags.record_dvol` is on**". That
//! mount was this flag's ONE production consumer and it went with the desktop shell's whole local
//! DATA plane. Nothing in the tree calls [`spawn_deribit_dvol_feed`] and nothing constructs a
//! [`DvolRecorder`] outside this file's own `#[cfg(test)]` module, so `VIKE_RECORD_DVOL=1` records
//! nothing and `record_dvol = true` in `<project>/settings/flags.toml` records nothing — exactly
//! the state the ⚠ note at the end of this paragraph used to describe as HISTORY.
//!
...
//!
//! `vike_config::CONSUMPTION`'s `flags.record_dvol` row is the gated authority (the table's one
//! `Reader::Nothing`: there is no library reader left either, only this crate's settings loader
//! resolving the field), and it argues why the field is KEPT rather than deleted — the feature is
//! unmounted, not removed.
//!
//! What survives, for whoever mounts it next: [`DvolRecorder::from_flag`] and
