...
use vike_bridge_core::market_pump::{run_market_feed, FrameOutcome, MarketPumpOpts, PumpBackoff};
use vike_data::{HistStore, LiveDataSink};
use vike_model::{now_ms, TradeTick};
...
impl DvolRecorder {
    ...
    pub fn from_flag(store: Arc<dyn HistStore + Send + Sync>, enabled: bool) -> Self {
        let cadence_ms = std::env::var(RECORD_DVOL_CADENCE_ENV)
            .ok()
...
fn dvol_feed_body(
    ...
    run_market_feed(
        ...
        || {},
        |e| tracing::warn!(target: "vike_deribit::dvol", error = %e, "dvol feed session error"),
    );
}
...
mod tests {
    ...
    fn parse_rejects_non_dvol_and_malformed_frames() {
        ...
        assert!(parse_dvol_frame(&serde_json::json!({
            "method": "subscription",
    ...
    /// The FLAG gate + the cadence parse. The cadence env is owned entirely by THIS test (no other
    /// test in the workspace touches `VIKE_RECORD_DVOL_CADENCE_MS`), so parallel scheduling can
    /// never race the set/remove.
    #[test]
    fn from_flag_gate_and_cadence() {
        let store = Arc::new(CaptureStore::default());
        std::env::remove_var(RECORD_DVOL_CADENCE_ENV);
        ...
        assert!(!DvolRecorder::from_flag(store.clone(), false).enabled(), "flag off → disabled");
        assert!(DvolRecorder::from_flag(store.clone(), true).enabled(), "flag on → enabled");

        // cadence parse/clamp (read independently of the enable gate)
        assert_eq!(
            DvolRecorder::from_flag(store.clone(), true).cadence_ms(),
            DEFAULT_DVOL_CADENCE_MS
        );
        std::env::set_var(RECORD_DVOL_CADENCE_ENV, "5000");
        assert_eq!(DvolRecorder::from_flag(store.clone(), true).cadence_ms(), 5_000);
        std::env::set_var(RECORD_DVOL_CADENCE_ENV, "0");
        assert_eq!(
            DvolRecorder::from_flag(store.clone(), true).cadence_ms(),
            1,
            "0 clamps to 1 ms"
        );
        std::env::set_var(RECORD_DVOL_CADENCE_ENV, "not-a-number");
        assert_eq!(
            DvolRecorder::from_flag(store.clone(), true).cadence_ms(),
            DEFAULT_DVOL_CADENCE_MS,
        ...
        assert_eq!(
            DvolRecorder::from_flag(store.clone(), false).cadence_ms(),
            DEFAULT_DVOL_CADENCE_MS
        );
