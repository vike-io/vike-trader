...
//! as a PARAMETER ([`DvolRecorder::from_flag`]), never from `std::env::var`. A LIBRARY reading the
//! variable itself would be a SECOND source of truth for one toggle, the shape
...
//! positive confirmation of something false. Writing `record_dvol` in `flags.toml` now fails
//! startup by name, and exporting `VIKE_RECORD_DVOL` warns that it configures nothing. What is
//! deleted is the SETTING; the parameter below is unchanged, and a root that mounts this feed
//! passes whatever it decides — a key of its own, or a plain `true`. Only the cadence
...
///
/// ⚠ There is deliberately no `RECORD_DVOL_ENV` beside it, and now there is no settings key for
/// one either: the ENABLE gate is a plain PARAMETER on [`DvolRecorder::from_flag`], and the
/// `flags.record_dvol` key that used to fill it is DELETED (`vike_config::DEAD_FLAG_KEYS`). A
...
impl DvolRecorder {
    ...
    /// The app-root constructor: `enabled` is the mounting root's own decision, taken as a
    /// parameter so this crate has no opinion about where it came from. Cadence from
    /// `VIKE_RECORD_DVOL_CADENCE_MS` when set and parsable, else 1 minute.
    ///
    /// ⚠ This replaced a `from_env` that read `VIKE_RECORD_DVOL` itself, and for a while the
    /// parameter was fed by `vike_config::Flags::record_dvol`. That key is now DELETED
    /// (`vike_config::DEAD_FLAG_KEYS`) because no root ever passed it here — so the parameter
    /// currently has no caller, and the next root to mount this feed chooses its own gate. The
    /// shape is what matters and it is unchanged: ONE authority, supplied by the caller, never a
    /// library reaching for the environment behind that caller's back.
