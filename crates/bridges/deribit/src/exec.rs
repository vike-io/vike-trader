...
pub fn parse_get_instrument(resp: &serde_json::Value) -> Option<SymbolProperties> {
    ...
    let properties = SymbolProperties {
        ...
        // A row the venue calls `spot`/`future_combo`/`option_combo`, or one publishing no `kind`,
        // leaves this ABSENT — the grid is still returned and still usable; only the class is
        // withheld, which is the honest answer rather than a guess.
