...
/// is what `vike-mount`'s deribit arm and the PIT properties recorder actually call. This function
/// has no caller under `src/` at all: its live callers are this crate's smoke tests (each of which
/// asks the venue for `kind=option` in the REQUEST) plus the r6 parity test.
...
mod properties_recorder_tests {
    ...
    fn deribit_disabled_recorder_writes_nothing() {
        ...
        assert!(
            store.scan_symbol_properties("deribit", SYMBOL, TsRange::all()).unwrap().is_empty()
        );
    }
}
