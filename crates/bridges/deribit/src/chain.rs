...
use vike_options::{
    AssetClass, Expiry, OptionChain, OptionKind, OptionQuote, StrikeRow, enrich_quote, expiry_ms,
    limit_strikes, make_expiry, years_to_expiry,
};
...
pub struct DeribitOptionsProvider {
    ...
    /// aspirational)"). It described `vike-app`'s `App::new` calling
    /// `vike_data::ChainRecorder::open_from_env(tick_store_root())` under `VIKE_RECORD_CHAINS=1`
    /// and handing the result to `vike_app_core::tools::spawn_tool_fetchers`. That root went with
    /// the desktop cut: `crates/vike-desktop/src/main.rs` now binds `chain_rec` to `None` outright,
    /// and `ChainRecorder::open_from_env` has no call site anywhere in the tree. So
    /// `VIKE_RECORD_CHAINS=1` records nothing in any binary — the flag's own
    /// `vike_config::CONSUMPTION` row (`flags.record_chains`, a `Reader::Uncalled`) is the gated
    /// authority for that, and this comment contradicting it is how the claim survived.
    ///
