...
use vike_data::{
    require_live_verb, DataClient, FeedRegistry, LiveDataError, LiveDataSink, SubscriptionId,
};
use vike_model::{now_ms, Bar, L2Book, LiveVerb};

use crate::market_data::{
    book_channel, chart_channel, classify_rpc_reply, parse_book_snapshot, parse_chart_bar,
    parse_quote, parse_trades, public_subscribe_frame, quote_channel, route_frame, trades_channel,
    MdEvent, RpcReply,
};
...
fn bars_main(symbol: String, interval: String, ctx: FeedCtx) {
    ...
    match fetch_seed(&symbol, &interval) {
        Ok(mut bars) => {
            ...
            ctx.set_status("LIVE · Deribit".into());
        }
...
fn quotes_main(symbol: String, _interval: String, ctx: FeedCtx) {
    ctx.set_status(format!("LIVE · Deribit quotes {symbol}"));
    ...
    run_market_feed(
        ...
        || {},
        |e| ctx.set_status(format!("{symbol} quotes ws error (reconnecting): {e}")),
    );
}
...
fn trades_main(symbol: String, _interval: String, ctx: FeedCtx) {
    ctx.set_status(format!("LIVE · Deribit trades {symbol}"));
    ...
    run_market_feed(
        ...
        || {},
        |e| ctx.set_status(format!("{symbol} trades ws error (reconnecting): {e}")),
    );
}
...
fn book_main(symbol: String, _interval: String, ctx: FeedCtx) {
    ctx.set_status(format!("LIVE · Deribit book {symbol}"));
    ...
    run_market_feed(
        ...
        || {},
        |e| ctx.set_status(format!("{symbol} book ws error (reconnecting): {e}")),
    );
}
