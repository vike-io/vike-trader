...
fn main() {
    ...
    let seed_bids: Vec<BookLevel> =
        (0..1000).map(|i| BookLevel::new(60_000.0 - i as f64 * 0.1, 1.0)).collect();
    let seed_asks: Vec<BookLevel> =
        (0..1000).map(|i| BookLevel::new(60_000.1 + i as f64 * 0.1, 1.0)).collect();
