...
//! tick off a limit price on a live order.
//!
//! ⚠ That condition is NECESSARY and not sufficient — it is what
//! [`f64_image_sits_below_its_decimal`] computes, and that function's own doc carries the
//! monotonicity argument for the provable half. Which multiples of an exposed tick actually drift,
//! and how many, is MEASURED per rung rather than derived:
//! `crates/vike-bridge-core/tests/grid_multiple_survives_the_wire.rs` sweeps the wire ladder every
//! run, and `crates/vike-bridge-core/tests/format_props.proptest-regressions` carries the full
//! `1e-0..=1e-22` sweep. The chain is the production one —
...
use vike_model::{BookLevel, SymbolProperties, round_to_step};
...
///
/// ⚠ **This is the NECESSARY half only, and the selection below relies on that being understood.**
/// The monotonicity argument makes a `false` here conclusive — such a tick provably cannot
/// demonstrate the defect, so passing it over costs no coverage. A `true` proves nothing on its
/// own: which multiples of an exposed tick actually drift is measured, not derived, which is why
/// selection goes on to require a multiple inside the resting band that genuinely lands below the
/// grid point's float (`ulps_below`) rather than resting on this verdict alone. The two most
/// common live exposed grids are 1e-6 and 1e-7; the per-rung counts are in
...
fn a_fine_tick_grid_price_reaches_the_venue_whole() {
    ...
    let best_ask = |symbol: &str| match vike_binance::market_data::fetch_depth_snapshot(
        ...
        Ok((_, _, asks)) => {
            ...
            let lowest = asks
                .iter()
                .filter(|BookLevel { price: px, qty }| *qty > 0.0 && px.is_finite() && *px > 0.0)
                .map(|BookLevel { price: px, .. }| *px)
