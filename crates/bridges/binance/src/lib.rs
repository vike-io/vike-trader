...
#[cfg(feature = "exec")]
pub mod history;
/// ⚠ FEEDS side, deliberately: `data`'s route reads it, and a keyless market-data build
/// (`scripts/ci_feature_suite.sh`'s `bridges-feeds` arm) must be able to reach the COIN-M book.
pub mod instruments;
#[cfg(feature = "exec")]
