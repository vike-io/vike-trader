...
use vike_model::{AssetClass, OrderRequest, SymbolProperties, TimeInForce};
...
/// The asset class of ONE `/fapi` `/exchangeInfo` `symbols[]` entry, from the venue's own
/// `contractType` — the field binance publishes precisely to distinguish a perpetual from a dated
/// contract, so this reads it rather than re-deriving the same fact from the symbol text
/// (`docs/decisions/0061-an-instrument-names-its-kind.md`).
///
/// `PERPETUAL` is the only value that means perpetual. Every other non-empty value is a DATED
/// contract — `CURRENT_QUARTER`, `NEXT_QUARTER`, and `PERPETUAL_DELIVERING`, which is a perpetual
/// binance has scheduled for delivery and so is settling like a future, not rolling like a perp.
/// An ABSENT or empty field is `None`: this venue's answer is missing, and
/// [`vike_model::SymbolProperties::asset_class`]'s doc argues at length why a missing answer must
...
fn perp_asset_class(entry: &Value) -> Option<AssetClass> {
    match entry.get("contractType").and_then(|c| c.as_str()).unwrap_or("") {
        "" => None,
        "PERPETUAL" => Some(AssetClass::CryptoPerp),
        _ => Some(AssetClass::CryptoFuture),
    }
}
        ...
        out.insert(
            ...
            PerpInstrument {
                properties: SymbolProperties {
                    ...
                    // The class is the VENUE'S OWN answer, read off the same entry the grid came
                    // from — never inferred from the symbol text, which is the implicit encoding
                    // `docs/decisions/0061-an-instrument-names-its-kind.md` exists to remove.
                    // `/fapi` `/exchangeInfo` is NOT a perp-only endpoint: it lists delivery
                    // futures (`CURRENT_QUARTER`/`NEXT_QUARTER`, symbols like `BTCUSDT_250926`)
                    // beside the perpetuals, so "this parser is the perp parser" would be a wrong
                    // answer for part of its own output.
