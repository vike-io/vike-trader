...
//! [`vike_catalog::AssetClass::CryptoPerp`] into linear and inverse, so a caller's class claim
...
/// [`vike_catalog::AssetClass::CryptoPerp`] for a perpetual — 0061 REFUSES to split that variant
...
pub fn fetch_klines_range_classed(
    ...
    start_ms: i64,
    end_ms: i64,
    class: Option<vike_catalog::AssetClass>,
) -> Result<Vec<Bar>, String> {
...
fn range_target(
    symbol: &str,
    claimed: Option<vike_catalog::AssetClass>,
...
fn route_target<'a>(
    ...
    let (wire, suffixed) = vike_catalog::split_perp(symbol);
...
fn unaddressable_class_refusal(symbol: &str, class: vike_catalog::AssetClass) -> String {
    format!(
...
fn contradicting_claim_refusal(symbol: &str, class: vike_catalog::AssetClass) -> String {
    format!(
...
fn dated_contract_refusal(wire: &str, dated: crate::instruments::CoinMContract) -> String {
    format!(
        "binance lists {wire:?} as a COIN-M {} contract — a DATED delivery future — and it was \
         ...
         a dated future is `vike_catalog::AssetClass::CryptoFuture`, `vike_catalog::addressing_for`'s \
...
mod tests {
    ...
    fn a_spot_route_never_consults_the_coin_m_listing() {
        ...
        let (base, _, _) =
            route_target("BTCUSDT", Some(vike_catalog::AssetClass::CryptoSpot), &counting)
