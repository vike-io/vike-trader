...
pub fn depth_main(
    ...
    //
    // ⚠ The split is asked OF THE VENUE, and this lane is why that function exists.
    // `docs/decisions/0061`'s Phase 1 pins that "every bridge that uses the suffix calls
    // `split_perp` unconditionally without ever asking whether its venue is a suffix venue" — and
    // this is a FAMILY driver: the venue is a runtime value two lines up, `ctx.spec.venue`, and a
    // third venue joining the family (the whole point of the module) would have had its symbols
    // stripped on a convention it does not use. Byte-identical for the two venues on it today,
    // binance and aster, both `vike_catalog::Naming::PerpSuffix`.
    let (api_sym, is_perp) = vike_catalog::split_perp_at(venue, &symbol);
