...
use vike_bridge_core::market_pump::{
    FrameOutcome, MarketPumpOpts, MarketStream, SessionStatus, connect_market_socket,
    run_market_feed_on,
};
...
use vike_bridge_core::ws::{TungsteniteStream, WsSocket, is_timeout};
...
pub fn run_trades_feed(
    ...
    run_market_feed_on(
        ...
        |s| match s {
            // THE FIX for this lane, which was a full bybit twin: the `"LIVE · …"` write used to
            // sit inside the connect closure under `if !started`, so after the one-time splice
            // EVERY reconnect skipped it and only error strings could reach the shared status
            // string again. Moved out here, `started` keeps governing exactly what it was for —
            // the warmup/drain/splice dance — and the healthy claim is made per SESSION, when the
            // venue has actually delivered a frame. The ` trades {sym}` suffix is dropped per
            // `super::market_feed::live_status`'s one-string-per-venue rule.
            SessionStatus::Live => {
                set_status(super::market_feed::live_status(spec));
                wake();
            }
