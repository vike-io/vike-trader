...
/// schema, invisible to `crates/vike-data/src/store_kind.rs` (which declares the column, not its
...
mod tests {
    ...
    fn the_fallback_pacer_reports_no_discovered_pace() {
        ...
        assert!(
            !pacer_for(&spec(150, Some("https://host/exchangeInfo")), Some("not json"))
                .is_discovered()
        );
        assert!(
    ...
    fn the_volume_column_is_the_one_the_book_puts_the_base_asset_in() {
        ...
        assert_eq!(
            parse_klines_with(coin_m, VolumeColumn::Index7).unwrap()[0].volume,
            776.291_743_26
        );
        assert_eq!(parse_klines_with(coin_m, VolumeColumn::Index5).unwrap()[0].volume, 595_752.0);
        let ratio = 595_752.0 / 776.291_743_26;
        assert!(
            ratio > 700.0 && ratio < 800.0,
            "the unit error this field exists to stop: {ratio}"
        );
