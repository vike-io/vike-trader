...
mod sync_tests {
    ...
    fn send_order_request_captures_and_yields() {
        ...
        let sub =
            MessageBus::send_order_request(&stub, 7, b"order-bytes").expect("send_order_request");
    ...
    fn send_shared_request_uses_shared_receiver() {
        let stub = MessageBusStub::with_responses(vec!["1|9001|".to_string()]);
        let sub = MessageBus::send_shared_request(
            &stub,
    ...
    fn cancel_shared_subscription_captures_packet() {
        let stub = MessageBusStub::default();
        MessageBus::cancel_shared_subscription(
            &stub,
            OutgoingMessages::RequestMarketData,
            b"cancel-shared",
        )
        .expect("cancel_shared_subscription");
    ...
    fn cancel_order_subscription_releases_tracker() {
        ...
        MessageBus::cancel_order_subscription(&stub, 0, b"cancel-order")
            .expect("cancel_order_subscription");
        assert_eq!(stub.request_messages(), vec![b"cancel-order".to_vec()]);

        let _second =
            MessageBus::create_order_update_subscription(&stub).expect("re-subscribe after cancel");
    }
...
mod async_tests {
    ...
    async fn drain_async(
        sub: &mut crate::transport::AsyncInternalSubscription,
    ) -> Vec<ResponseMessage> {
        let mut out = Vec::new();
    ...
    async fn send_request_captures_and_yields() {
        let stub = MessageBusStub::with_responses(vec!["1|9001|".to_string()]);
        let mut sub =
            AsyncMessageBus::send_request(&stub, 1, b"req".to_vec()).await.expect("send_request");
    ...
    async fn send_shared_request_captures_and_yields() {
        let stub = MessageBusStub::with_responses(vec!["1|9001|".to_string()]);
        let mut sub = AsyncMessageBus::send_shared_request(
            &stub,
            OutgoingMessages::RequestMarketData,
            b"shared".to_vec(),
        )
    ...
    async fn create_order_update_subscription_then_already_subscribed_and_cleanup() {
        ...
        let sub = AsyncMessageBus::create_order_update_subscription(&stub)
            .await
