...
fn test_decode_head_timestamp_proto() {
    let proto_msg =
        crate::proto::HeadTimestamp { req_id: Some(1), head_timestamp: Some("1609459200".into()) };
...
fn test_decode_histogram_data_proto() {
    let proto_msg = crate::proto::HistogramData {
        ...
        histogram_data_entries: vec![
            crate::proto::HistogramDataEntry { price: Some(100.5), size: Some("50".into()) },
            crate::proto::HistogramDataEntry { price: Some(101.0), size: Some("75".into()) },
        ],
    };
...
fn test_decode_historical_schedule_unknown_timezone_errors() {
    ...
    let err = decode_historical_schedule_proto(&proto_msg.encode_to_vec())
        .expect_err("unknown tz must error");
...
fn test_decode_historical_data_update_proto_missing_bar_defaults() {
    let proto_msg =
        crate::proto::HistoricalDataUpdate { req_id: Some(1), historical_data_bar: None };
...
fn test_decode_historical_ticks_mid_point_rejects_text_framing() {
    ...
    let err =
        decode_historical_ticks_mid_point(&message).expect_err("text framing must be rejected");
    assert!(matches!(err, Error::UnexpectedResponse(_)), "got: {err:?}");
}
