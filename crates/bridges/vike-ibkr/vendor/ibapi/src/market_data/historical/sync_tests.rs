...
use crate::common::test_utils::helpers::{
    assert_proto_msg_id, assert_request, assert_request_msg_id, count_proto_msgs,
...
use crate::testdata::builders::market_data::{
    head_timestamp_request, head_timestamp_response, histogram_data_request,
    histogram_data_response, histogram_entry, historical_data_bar, historical_data_daily_bar,
    historical_data_end_response, historical_data_request, historical_data_response,
    historical_data_update_response, historical_schedule_response, historical_tick_bid_ask,
    historical_tick_last, historical_tick_mid, historical_ticks_bid_ask_response,
...
fn tick_sub_from_routed<T: TickDecoder<T>>(
    items: Vec<RoutedItem>,
) -> (TickSubscription<T>, channel::Receiver<Signal>) {
    ...
    drop(sender);
    let internal =
        SubscriptionBuilder::new().receiver(receiver).signaler(signaler).request_id(9300).build();
...
fn test_historical_data() {
    ...
    assert_eq!(
        historical_data.bars[1].date,
        BarTimestamp::Date(date!(2023 - 04 - 14)),
        "daily bar.date"
    );
    ...
    let message_bus = Arc::new(MessageBusStub::with_ordered_responses(vec![
        proto_response(
            ...
            historical_ticks_last_response()
                .tick(
                    historical_tick_last(1_681_133_400, 11.63, 24_547, "ISLAND")
                        .special_conditions(" O X"),
                )
...
fn test_tick_subscription_owned_iterator() {
    let message_bus = Arc::new(MessageBusStub::with_ordered_responses(vec![proto_response(
        ...
        historical_ticks_last_response()
            .tick(
                historical_tick_last(1_681_133_400, 11.70, 24_547, "ISLAND")
                    .special_conditions(" O X"),
            )
...
fn test_tick_subscription_bid_ask() {
    ...
    // Collect ticks
    let ticks: Vec<TickBidAsk> =
        tick_subscription.iter_data().map(|t| t.expect("decode error")).collect();
...
fn test_historical_data_streaming_with_updates() {
    ...
    let mut client =
        Client::stubbed(message_bus.clone(), server_versions::PROTOBUF_REST_MESSAGES_3);
...
fn test_tick_subscription_sends_cancel_on_drop() {
    ...
    {
        let _subscription: TickSubscription<TickLast> =
            TickSubscription::new(internal, 9100, message_bus.clone());
...
fn test_tick_subscription_explicit_cancel_prevents_duplicate_on_drop() {
    ...
    {
        let subscription: TickSubscription<TickLast> =
            TickSubscription::new(internal, 9101, message_bus.clone());
        subscription.cancel();
    }
...
fn test_tick_subscription_drop_after_done_does_not_cancel() {
    ...
    {
        let subscription: TickSubscription<TickLast> =
            TickSubscription::new(internal, 9102, message_bus.clone());
...
fn test_head_timestamp_end_of_stream() {
    ...
    let message_bus = Arc::new(MessageBusStub::default());
    let client = Client::stubbed(message_bus, server_versions::SIZE_RULES);
    let result = client.head_timestamp(
        &Contract::stock("MSFT").build(),
        WhatToShow::Trades,
        TradingHours::Regular,
    );
...
fn test_historical_schedule_end_of_stream() {
    ...
    let result =
        client.historical_schedules(&Contract::stock("MSFT").build(), Duration::days(1)).fetch();
    assert!(
        matches!(result, Err(Error::UnexpectedEndOfStream)),
        "expected UnexpectedEndOfStream, got {result:?}"
    );
...
fn test_tick_subscription_try_next_drains_buffer() {
    ...
    let ticks: Vec<TickLast> =
        subscription.try_iter_data().map(|t| t.expect("decode error")).collect();
...
fn test_tick_subscription_error_surfaces_via_err_arm() {
    ...
    let (subscription, _signal_rx) =
        tick_sub_from_routed::<TickLast>(vec![RoutedItem::Error(Error::Simple("injected".into()))]);
...
fn test_tick_subscription_iter_data_filters_notice_keeps_error() {
    ...
    match data.next() {
        Some(Err(Error::Simple(msg))) => {
            assert_eq!(msg, "boom", "error survives the notice filter")
        }
...
fn test_tick_subscription_midpoint_try_iter_and_timeout_iter() {
    ...
    let ticks_try: Vec<TickMidpoint> =
        subscription.try_iter_data().map(|t| t.expect("decode error")).collect();
