...
use super::{
    BarSize, Duration, HistogramEntry, HistoricalBarUpdate, HistoricalData, Schedule, TickDecoder,
    WhatToShow,
};
...
impl Client {
    ...
    pub fn head_timestamp(
        ...
        let request = encoders::encode_request_head_timestamp(
            builder.request_id(),
            contract,
            what_to_show,
        ...
        match subscription.next() {
            Some(Ok(message)) if message.message_type() == IncomingMessages::HeadTimestamp => {
                Ok(decoders::decode_head_timestamp(&message)?)
            }
            Some(Ok(message)) => Err(Error::unexpected_response(&message)),
            Some(Err(Error::ConnectionReset)) => {
                self.head_timestamp(contract, what_to_show, trading_hours)
            }
            Some(Err(e)) => Err(e),
    ...
    pub fn historical_data<'a>(
        ...
        contract: &'a Contract,
        bar_size: BarSize,
    ) -> super::HistoricalDataBuilder<'a, Self> {
    ...
    /// }
    /// ```
    pub fn historical_schedules<'a>(
        &'a self,
        contract: &'a Contract,
        duration: Duration,
    ) -> super::HistoricalScheduleBuilder<'a, Self> {
    ...
    /// # let _ = quotes;
    /// ```
    pub fn historical_ticks<'a>(
        &'a self,
        contract: &'a Contract,
        number_of_ticks: i32,
    ) -> super::HistoricalTicksBuilder<'a, Self> {
    ...
    pub fn histogram_data(
        ...
        loop {
            let builder = self.request();
            let request = encoders::encode_request_histogram_data(
                builder.request_id(),
                contract,
                trading_hours.use_rth(),
                period,
...
pub(crate) fn historical_data(
    ...
    for _ in 0..MAX_RETRIES {
        ...
        match subscription.next() {
            ...
            Some(Ok(message)) if message.message_type() == IncomingMessages::Error => {
...
impl<T: TickDecoder<T>> TickSubscription<T> {
    ...
    ) -> TickSubscriptionTimeoutIter<'_, T> {
        TickSubscriptionTimeoutIter { subscription: self, timeout: duration }
    }
    ...
    pub fn timeout_iter_data(
        &self,
        duration: std::time::Duration,
    ) -> FilterData<TickSubscriptionTimeoutIter<'_, T>> {
