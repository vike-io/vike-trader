...
use crate::market_data::historical::{
    Bar, BarTimestamp, HistogramEntry, HistoricalData, Schedule, Session, TickAttributeBidAsk,
    TickAttributeLast, TickBidAsk, TickLast, TickMidpoint,
};
...
fn parse_unix_seconds_str(s: &str) -> Result<OffsetDateTime, Error> {
    let mk_err = |e: &dyn std::fmt::Display| {
        Error::parse_field(s, format!("invalid unix-second timestamp: {e}"))
    };
...
pub(crate) fn decode_historical_data(message: &ResponseMessage) -> Result<HistoricalData, Error> {
    ...
    Ok(HistoricalData { start: OffsetDateTime::UNIX_EPOCH, end: OffsetDateTime::UNIX_EPOCH, bars })
}
...
pub(crate) fn decode_historical_ticks_bid_ask(
    message: &ResponseMessage,
) -> Result<(Vec<TickBidAsk>, bool), Error> {
    decode_historical_ticks_bid_ask_proto(message.require_proto()?)
}

pub(crate) fn decode_historical_ticks_mid_point(
    message: &ResponseMessage,
) -> Result<(Vec<TickMidpoint>, bool), Error> {
    decode_historical_ticks_proto(message.require_proto()?)
}

pub(crate) fn decode_historical_ticks_last(
    message: &ResponseMessage,
) -> Result<(Vec<TickLast>, bool), Error> {
    decode_historical_ticks_last_proto(message.require_proto()?)
}

pub(crate) fn decode_histogram_data(
    message: &ResponseMessage,
...
pub(crate) fn decode_historical_ticks_proto(
    bytes: &[u8],
) -> Result<(Vec<TickMidpoint>, bool), Error> {
...
pub(crate) fn decode_historical_ticks_last_proto(
    bytes: &[u8],
) -> Result<(Vec<TickLast>, bool), Error> {
...
pub(crate) fn decode_historical_ticks_bid_ask_proto(
    bytes: &[u8],
) -> Result<(Vec<TickBidAsk>, bool), Error> {
...
pub(crate) fn decode_historical_data_end_proto(
    bytes: &[u8],
) -> Result<(OffsetDateTime, OffsetDateTime), Error> {
...
pub(crate) fn decode_historical_schedule_proto(bytes: &[u8]) -> Result<Schedule, Error> {
    ...
    let sessions = p
        ...
        .map(|s| -> Result<Session, Error> {
            Ok(Session {
                start: parse_schedule_date_time(
                    s.start_date_time.as_deref().unwrap_or(""),
                    time_zone,
                )?,
