...
use crate::testdata::builders::market_data::{
    ...
    historical_data_update_response, historical_schedule_response, historical_session,
    historical_tick_bid_ask, historical_tick_last, historical_tick_mid,
    historical_ticks_bid_ask_response, historical_ticks_last_response, historical_ticks_request,
...
async fn test_historical_data_error_response() {
    ...
    let result =
        client.historical_data(&contract, BarSize::Hour).duration(Duration::days(1)).fetch().await;
    ...
    let message_bus = Arc::new(MessageBusStub::with_ordered_responses(vec![
        proto_response(
            ...
            historical_ticks_bid_ask_response()
                .tick(
                    historical_tick_bid_ask(1_678_838_400, 185.50, 186.00, 100, 200)
                        .bid_past_low(true),
                )
                .tick(
        ...
        proto_response(
            ...
            historical_ticks_bid_ask_response()
                .tick(
                    historical_tick_bid_ask(1_678_838_500, 185.75, 186.25, 150, 250)
                        .bid_past_low(true),
                )
                .done(true)
                .encode_proto(),
...
async fn test_tick_subscription_buffer_and_iteration() {
    ...
    let ticks: Vec<_> =
        subscription.filter_data().map(|r| r.expect("decode error")).collect().await;
...
async fn test_tick_subscription_bid_ask() {
    ...
    let message_bus = Arc::new(MessageBusStub::with_ordered_responses(vec![proto_response(
        ...
        historical_ticks_bid_ask_response()
            .tick(
                historical_tick_bid_ask(1_678_838_400, 185.50, 186.00, 100, 200).bid_past_low(true),
            )
            .done(true)
            .encode_proto(),
    )]));
...
async fn test_historical_ticks_trade() {
    let message_bus = Arc::new(MessageBusStub::with_ordered_responses(vec![proto_response(
        ...
        historical_ticks_last_response()
            .tick(
                historical_tick_last(1_678_838_400, 185.50, 100, "ISLAND")
                    .special_conditions("APR"),
            )
            .done(true)
            .encode_proto(),
    )]));
...
async fn test_historical_data_time_zone_handling() {
    let message_bus = Arc::new(MessageBusStub::with_ordered_responses(vec![
        ...
        proto_response(
            IncomingMessages::HistoricalDataEnd,
            historical_data_end_response().encode_proto(),
        ),
    ]));
...
async fn test_historical_data_streaming_with_updates() {
    ...
    let Some(Ok(SubscriptionItem::Data(HistoricalBarUpdate::Historical(data)))) =
    ...
    let Some(Ok(SubscriptionItem::Data(HistoricalBarUpdate::Update(bar)))) =
        subscription.next().await
    else {
...
async fn test_historical_data_streaming_error_response() {
    ...
    assert!(
        err.to_string().contains("No market data permissions"),
...
async fn test_historical_data_connection_reset_after_retries() {
    ...
    assert!(
        ...
        "expected ConnectionReset, got {result:?}"
    );
...
async fn test_cancel_historical_ticks_version_check() {
    assert_version_check_fails(Features::CANCEL_CONTRACT_DATA, |c| async move {
...
async fn test_tick_subscription_cancel_idempotent() {
    ...
    let subscription: TickSubscription<TickLast> =
        TickSubscription::new(internal, 9200, message_bus.clone());
    subscription.cancel().await;
    subscription.cancel().await;
...
async fn test_tick_subscription_error_surfaces_via_err_arm() {
    ...
    let mut subscription: TickSubscription<TickLast> =
        tick_sub_from_routed(vec![RoutedItem::Error(Error::ConnectionReset)]);
