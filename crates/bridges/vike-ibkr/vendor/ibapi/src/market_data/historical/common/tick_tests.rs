...
use crate::testdata::builders::market_data::{
    historical_tick_last, historical_ticks_last_response,
};
...
fn classify_decode_failure_returns_error() {
    ...
    let malformed = RoutedItem::Response(proto_response(
        IncomingMessages::HistoricalTickLast,
        vec![0xFF, 0xFF, 0xFF],
    ));
    assert!(
...
fn classify_end_of_stream_error_returns_end_of_stream() {
    assert!(
        matches!(
            classify::<TickLast>(RoutedItem::Error(Error::EndOfStream)),
