...
mod sync_tests {
    ...
    fn trade_terminal_round_trip() {
        let (client, bus) = create_blocking_test_client_with_responses_and_version(
            vec![TRADE_RESPONSE.to_owned()],
    ...
    fn mid_point_terminal_round_trip() {
        let (client, bus) = create_blocking_test_client_with_responses_and_version(
            vec![MID_POINT_RESPONSE.to_owned()],
            server_versions::HISTORICAL_TICKS,
        );
        let contract = Contract::stock("AAPL").build();
...
mod async_tests {
    ...
    async fn trade_terminal_round_trip() {
        let (client, bus) = create_test_client_with_responses_and_version(
            vec![TRADE_RESPONSE.to_owned()],
            server_versions::HISTORICAL_TICKS,
        );
        let contract = Contract::stock("AAPL").build();
