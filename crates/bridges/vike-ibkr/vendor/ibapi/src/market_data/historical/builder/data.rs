...
use crate::market_data::historical::{
    BarSize, Duration, HistoricalBarUpdate, HistoricalData, WhatToShow,
};
...
impl<'a, C> HistoricalDataBuilder<'a, C> {
        ...
        match (self.between, self.duration, self.ending) {
            ...
            (Some((start, end)), None, None) => {
                ...
                if seconds > i32::MAX as i64 {
                    return Err(Error::InvalidArgument(
                        "historical_data: .between(start, end) range exceeds i32::MAX seconds"
                            .to_owned(),
                    ));
                }
    ...
    fn resolve_for_stream(&self) -> Result<Duration, Error> {
        ...
        self.duration.ok_or_else(|| {
            Error::InvalidArgument("historical_data().stream(): must set .duration()".to_owned())
        })
    }
}
...
impl<'a> HistoricalDataBuilder<'a, crate::client::r#async::Client> {
    ...
    pub async fn stream(
        self,
    ) -> Result<crate::subscriptions::Subscription<HistoricalBarUpdate>, Error> {
        let duration = self.resolve_for_stream()?;
