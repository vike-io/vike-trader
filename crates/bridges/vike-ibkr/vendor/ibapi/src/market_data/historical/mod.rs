...
impl FromStr for BarTimestamp {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let secs: i64 =
            s.parse().map_err(|e: std::num::ParseIntError| Error::parse_field(s, e.to_string()))?;
...
impl FromStr for Duration {
    ...
    fn from_str(val: &str) -> Result<Self, HistoricalParseError> {
        if val.is_empty() {
            return Err(HistoricalParseError::Duration(
                val.to_string(),
        ...
        match val.to_uppercase().rsplit_once(' ') {
            Some((value_part, unit_part)) => {
                ...
                match unit_part {
                    ...
                    _ => Err(HistoricalParseError::Duration(
                        val.to_string(),
                        "Unsupported unit".to_string(),
                    )),
                }
            }
            None => Err(HistoricalParseError::Duration(
                val.to_string(),
...
impl StreamDecoder<HistoricalBarUpdate> for HistoricalBarUpdate {
    ...
    fn decode(_context: &DecoderContext, message: &mut ResponseMessage) -> Result<Self, Error> {
        match message.message_type() {
            IncomingMessages::HistoricalData => {
                Ok(Self::Historical(common::decoders::decode_historical_data(message)?))
            }
            IncomingMessages::HistoricalDataUpdate => {
                Ok(Self::Update(common::decoders::decode_historical_data_update(message)?))
            }
            IncomingMessages::HistoricalDataEnd => {
...
#[cfg(all(feature = "sync", not(feature = "async")))]
pub use sync::{
    TickSubscription, TickSubscriptionIter, TickSubscriptionOwnedIter, TickSubscriptionTimeoutIter,
