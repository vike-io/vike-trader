...
mod sync_tests {
    ...
    use crate::common::test_utils::helpers::{
        assert_proto_msg_id, assert_request, proto_response, TEST_REQ_ID_FIRST,
    };
    ...
    use crate::testdata::builders::market_data::{
        market_data_request, tick_price, tick_size, tick_snapshot_end,
    };
    ...
    fn test_market_data_builder_default() {
        ...
        let _subscription =
            client.market_data(&contract).subscribe().expect("Failed to create subscription");
    ...
    fn test_snapshot_once_collects_until_snapshot_end() {
        let message_bus = Arc::new(MessageBusStub::with_ordered_responses(vec![
            proto_response(
                ...
                tick_price().tick_type(4).price(185.50).encode_proto(),
            ),
            proto_response(
                IncomingMessages::TickSize,
                tick_size().tick_type(5).size(100.0).encode_proto(),
            ),
