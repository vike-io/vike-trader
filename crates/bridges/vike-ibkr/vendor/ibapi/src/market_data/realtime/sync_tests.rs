...
use crate::common::test_utils::helpers::{
    assert_request, proto_response, request_message_count, TEST_REQ_ID_FIRST,
};
use crate::contracts::tick_types::TickType;
use crate::contracts::{
    ComboLeg, Contract, Currency, DeltaNeutralContract, Exchange, LegAction, SecurityType, Symbol,
};
...
use crate::testdata::builders::market_data::{
    bid_ask_tick, depth_market_data_description, market_data_request,
    market_depth_exchanges_request, market_depth_request, market_depth_response, mid_point_tick,
    mkt_depth_exchanges_response, realtime_bar_tick, realtime_bars_request, tick_by_tick_request,
    tick_generic, tick_price, tick_size, tick_string, trade_tick,
};
...
fn test_validate_tick_by_tick_request() {
    ...
    let client =
        Client::stubbed(Arc::new(MessageBusStub::default()), server_versions::TICK_BY_TICK - 1);
    ...
    let client =
        Client::stubbed(Arc::new(MessageBusStub::default()), server_versions::TICK_BY_TICK);
...
fn test_realtime_bars() {
    ...
    let received_bars: Vec<Bar> =
        bars.iter_data().take(2).map(|r| r.expect("subscription error")).collect();
...
fn test_tick_by_tick_all_last() {
    ...
    // Test receiving data
    let received_trades: Vec<Trade> =
        trades.iter_data().take(2).map(|r| r.expect("subscription error")).collect();
...
fn test_market_depth() {
    ...
    // Test receiving data
    let received_depth: Vec<MarketDepths> =
        depth.iter_data().take(2).map(|r| r.expect("subscription error")).collect();
...
fn test_market_depth_exchanges() {
    ...
    let exchanges =
        client.market_depth_exchanges().expect("Failed to request market depth exchanges");
...
fn test_tick_by_tick_bid_ask() {
    ...
    let received_ticks: Vec<BidAsk> =
        subscription.iter_data().take(1).map(|r| r.expect("subscription error")).collect();
...
fn test_tick_by_tick_midpoint() {
    ...
    // Test receiving data
    let received_midpoints: Vec<MidPoint> =
        midpoints.iter_data().take(2).map(|r| r.expect("subscription error")).collect();
...
fn test_basic_market_data() {
    let message_bus = Arc::new(MessageBusStub::with_ordered_responses(vec![
        ...
        proto_response(
            ...
            tick_size().tick_type(0).size(150.0).encode_proto(),
        ),
        ...
        proto_response(
            ...
            tick_generic().tick_type(23).value(20.5).encode_proto(),
        ),
    ]));
    ...
    let received_ticks: Vec<TickTypes> =
        subscription.iter_data().take(4).map(|r| r.expect("subscription error")).collect();
...
fn test_market_data_with_delta_neutral() {
    ...
    contract.delta_neutral_contract =
        Some(DeltaNeutralContract { contract_id: 12345, delta: 0.5, price: 100.0 });
...
fn test_tick_by_tick_last() {
    ...
    let received_trades: Vec<Trade> =
        trades.iter_data().take(1).map(|r| r.expect("subscription error")).collect();
