...
use super::{
    Bar, DepthMarketDataDescription, MarketDepthBuilder, MarketDepths, RealtimeBarsBuilder,
    TickByTickBuilder, TickTypes, WhatToShow,
};
...
impl Client {
    ...
    /// }
    /// ```
    pub async fn switch_market_data_type(
        &self,
        market_data_type: crate::market_data::MarketDataType,
    ) -> Result<(), Error> {
        self.check_server_version(
            server_versions::REQ_MARKET_DATA_TYPE,
            "It does not support market data type requests.",
        )?;

        let message =
            crate::market_data::encoders::encode_request_market_data_type(market_data_type)?;
    ...
    /// }
    /// ```
    pub fn tick_by_tick<'a>(
        &'a self,
        contract: &'a Contract,
        number_of_ticks: i32,
    ) -> TickByTickBuilder<'a, Self> {
...
pub(super) fn validate_tick_by_tick_request(
    client: &Client,
    _contract: &Contract,
    number_of_ticks: i32,
    ignore_size: bool,
...
pub(crate) async fn market_depth(
    ...
    let request = encoders::encode_request_market_depth(
        builder.request_id(),
        contract,
        number_of_rows,
        is_smart_depth,
    ...
    builder
        .send_with_context::<MarketDepths>(
            request,
...
pub(crate) async fn tick_by_tick<T: StreamDecoder<T> + Send + 'static>(
    ...
    let builder = client.request();
    let request = encoders::encode_tick_by_tick(
        builder.request_id(),
        contract,
        tick_type,
        number_of_ticks,
