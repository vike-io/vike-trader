...
use crate::market_data::realtime::{
    ...
    TickAttribute, TickGeneric, TickPrice, TickPriceSize, TickRequestParameters, TickSize,
    TickString, TickType, TickTypes, Trade, TradeAttribute,
};
...
pub(crate) fn decode_market_depth_l2(
    message: &mut ResponseMessage,
) -> Result<MarketDepthL2, Error> {
    decode_market_depth_l2_proto(message.require_proto()?)
}

pub(crate) fn decode_market_depth_exchanges(
...
pub(crate) fn decode_tick_option_computation(
    message: &mut ResponseMessage,
) -> Result<OptionComputation, Error> {
    decode_tick_option_computation_proto(message.require_proto()?)
}

pub(crate) fn decode_tick_request_parameters(
    message: &mut ResponseMessage,
) -> Result<TickRequestParameters, Error> {
    decode_tick_request_parameters_proto(message.require_proto()?)
}

pub(crate) fn decode_market_data_type(
    message: &mut ResponseMessage,
) -> Result<MarketDataType, Error> {
...
pub(crate) fn decode_mid_point_tick_proto(bytes: &[u8]) -> Result<MidPoint, Error> {
    ...
    Ok(MidPoint { time: ts(t.time.unwrap_or_default()), mid_point: t.price.unwrap_or_default() })
}

pub(crate) fn decode_market_depth_exchanges_proto(
    bytes: &[u8],
) -> Result<Vec<DepthMarketDataDescription>, Error> {
...
pub(crate) fn decode_tick_request_parameters_proto(
    bytes: &[u8],
) -> Result<TickRequestParameters, Error> {
...
pub(crate) fn decode_tick_price_proto(bytes: &[u8]) -> Result<TickTypes, Error> {
    ...
    match (size_tick_type, size) {
        (TickType::Unknown, _) | (_, None) => {
            Ok(TickTypes::Price(TickPrice { tick_type, price, attributes }))
        }
...
pub(crate) fn decode_tick_option_computation_proto(
    bytes: &[u8],
) -> Result<OptionComputation, Error> {
