...
impl<'a, C> MarketDepthBuilder<'a, C> {
    pub(crate) fn new(client: &'a C, contract: &'a Contract, number_of_rows: i32) -> Self {
        Self { client, contract, number_of_rows, smart_depth: SmartDepth::default() }
    }
...
impl<'a> MarketDepthBuilder<'a, crate::client::sync::Client> {
    ...
    pub fn subscribe(
        self,
    ) -> Result<crate::subscriptions::sync::Subscription<MarketDepths>, Error> {
        crate::market_data::realtime::sync::market_depth(
            self.client,
            self.contract,
            self.number_of_rows,
            self.smart_depth,
...
impl<'a> MarketDepthBuilder<'a, crate::client::r#async::Client> {
    ...
    pub async fn subscribe(
        self,
    ) -> Result<crate::subscriptions::Subscription<MarketDepths>, Error> {
        crate::market_data::realtime::r#async::market_depth(
            self.client,
            self.contract,
            self.number_of_rows,
            self.smart_depth,
