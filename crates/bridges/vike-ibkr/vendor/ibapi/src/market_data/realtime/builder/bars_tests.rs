...
mod sync_tests {
    use crate::common::test_utils::helpers::{
        assert_request, create_blocking_test_client, request_message_count,
    };
    ...
    fn options_round_trip() {
        let (client, bus) = create_blocking_test_client();
        let contract = Contract::stock("AAPL").build();
        let options = vec![TagValue { tag: "XYZ".to_owned(), value: "1".to_owned() }];
...
mod async_tests {
    ...
    async fn full_chain_async() {
        let (client, bus) = create_test_client();
        let contract = Contract::stock("AAPL").build();
        let options = vec![TagValue { tag: "K".to_owned(), value: "V".to_owned() }];
