...
impl StreamDecoder<MarketDepths> for MarketDepths {
    const RESPONSE_MESSAGE_IDS: &[IncomingMessages] =
        &[IncomingMessages::MarketDepth, IncomingMessages::MarketDepthL2];

    fn decode(_context: &DecoderContext, message: &mut ResponseMessage) -> Result<Self, Error> {
        match message.message_type() {
            IncomingMessages::MarketDepth => {
                Ok(MarketDepths::MarketDepth(common::decoders::decode_market_depth(message)?))
            }
            IncomingMessages::MarketDepthL2 => {
                Ok(MarketDepths::MarketDepthL2(common::decoders::decode_market_depth_l2(message)?))
            }
            _ => Err(Error::unexpected_response(message)),
        }
    }
    ...
    fn cancel_message(
        ...
        common::encoders::encode_cancel_market_depth(
            request_id,
            context.map(|c| c.is_smart_depth).unwrap_or(false),
        )
    }
}
...
impl StreamDecoder<TickTypes> for TickTypes {
    ...
    fn decode(_context: &DecoderContext, message: &mut ResponseMessage) -> Result<Self, Error> {
        match message.message_type() {
            ...
            IncomingMessages::TickSize => {
                common::decoders::decode_tick_size(message).map(TickTypes::Size)
            }
            IncomingMessages::TickString => {
                common::decoders::decode_tick_string(message).map(TickTypes::String)
            }
            IncomingMessages::TickGeneric => {
                common::decoders::decode_tick_generic(message).map(TickTypes::Generic)
            }
            IncomingMessages::TickOptionComputation => {
                common::decoders::decode_tick_option_computation(message)
                    .map(TickTypes::OptionComputation)
            }
            IncomingMessages::TickReqParams => {
                common::decoders::decode_tick_request_parameters(message)
                    .map(TickTypes::RequestParameters)
            }
            IncomingMessages::MarketDataType => {
                common::decoders::decode_market_data_type(message).map(TickTypes::MarketDataType)
            }
