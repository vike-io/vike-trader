...
use crate::testdata::builders::wsh::{
    cancel_wsh_event_data_request, cancel_wsh_metadata_request, wsh_event_data_request,
    wsh_metadata_request, wsh_metadata_response,
};
...
async fn test_wsh_event_data_by_filter_subscription_table() {
    ...
    for test_case in subscription_test_cases() {
        ...
        let client =
            Client::stubbed(message_bus, crate::server_versions::WSH_EVENT_DATA_FILTERS_DATE);
        ...
        for (i, (received, expected)) in
            received_events.iter().zip(test_case.expected_events.iter()).enumerate()
        {
...
async fn test_wsh_event_data_by_filter_integration_table() {
    use crate::wsh::common::test_tables::{
        event_data_by_filter_integration_test_cases, IntegrationExpectedResult,
    };
    ...
    for test_case in event_data_by_filter_integration_test_cases() {
        ...
        let result = match test_case.name {
            "successful filter request with autofill" => {
                ...
                let auto_fill = AutoFill { competitors: true, portfolio: false, watchlist: true };
                let result =
                    client.wsh_event_data_by_filter(filter, Some(100), Some(auto_fill)).await;
                if result.is_ok() {
                    assert_request(
                        &message_bus,
                        0,
...
async fn test_subscription_integration_table() {
    ...
    for test_case in subscription_integration_test_cases() {
        ...
        for (i, (received, expected)) in
            events.iter().zip(test_case.expected_events.iter()).enumerate()
        {
            assert_eq!(received, expected, "Test '{}' event {} mismatch", test_case.name, i);
        }
    }
