...
pub const WSH_METADATA_DECODE_TESTS: &[DecodeTestCase] =
    &[DecodeTestCase { name: "valid metadata", req_id: 9000, data_json: r#"{"test":"metadata"}"# }];
...
pub const WSH_EVENT_DATA_DECODE_TESTS: &[DecodeTestCase] =
    &[DecodeTestCase { name: "valid event data", req_id: 9000, data_json: r#"{"test":"event"}"# }];
...
pub fn wsh_metadata_test_cases() -> Vec<ApiTestCase> {
    vec![
        ApiTestCase {
            ...
            response_messages: vec![metadata_response(
                9000,
                r#"{"validated":true,"data":{"metadata":"test"}}"#,
            )],
...
pub fn event_data_by_contract_test_cases() -> Vec<EventDataByContractTestCase> {
    vec![
        EventDataByContractTestCase {
            ...
            limit: Some(100),
            auto_fill: Some(AutoFill { competitors: true, portfolio: false, watchlist: true }),
            response_messages: vec![event_data_response(
                9001,
                r#"{"validated":true,"data":{"events":[]}}"#,
            )],
            expected_result: ApiExpectedResult::Success {
