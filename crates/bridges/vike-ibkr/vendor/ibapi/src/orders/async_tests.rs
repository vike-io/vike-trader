...
use crate::testdata::builders::orders::{
    cancel_order_request, commission_report, completed_order, completed_orders_end,
    completed_orders_request, execution_data, execution_data_end, executions_request,
    global_cancel_request, next_valid_order_id_request, open_order, open_order_end,
...
async fn test_place_order() {
    ...
    let mut subscription =
        client.place_order(1, &contract, &order).await.expect("failed to place order");
    ...
    assert!(
        matches!(
            commission_report,
            Some(Ok(SubscriptionItem::Data(PlaceOrder::CommissionReport(_))))
        ),
    ...
    assert_request(
        &message_bus,
        0,
        &place_order_request().order_id(1).contract(&contract).order(&order),
    );
}
...
async fn test_completed_orders() {
    ...
    let mut subscription =
        client.completed_orders(true).await.expect("failed to get completed orders");
...
async fn test_submit_order() {
    ...
    assert_request(
        &message_bus,
        0,
        &place_order_request().order_id(2).contract(&contract).order(&order),
    );
}
...
async fn test_exercise_options() {
    ...
    assert!(
        matches!(
            exercise_response,
            Some(Ok(SubscriptionItem::Data(ExerciseOptions::OpenOrder(_))))
        ),
...
async fn test_order_update_stream_drop_releases_subscription() {
    ...
    let stream1 =
        client.order_update_stream().await.expect("failed to create initial order update stream");
    drop(stream1);
