...
use crate::orders::{
    Action, AuctionStrategy, OcaType, Order, OrderComboLeg, TagValue, TimeInForce, VolatilityType,
...
pub fn market_order(action: Action, quantity: f64) -> Order {
    Order { action, order_type: "MKT".to_owned(), total_quantity: quantity, ..Order::default() }
}
...
pub fn market_on_close(action: Action, quantity: f64) -> Order {
    Order { action, order_type: "MOC".to_owned(), total_quantity: quantity, ..Order::default() }
}
...
pub fn box_top(action: Action, quantity: f64) -> Order {
    Order { action, order_type: "BOX TOP".to_owned(), total_quantity: quantity, ..Order::default() }
}
...
pub fn market_to_limit(action: Action, quantity: f64) -> Order {
    Order { action, order_type: "MTL".to_owned(), total_quantity: quantity, ..Order::default() }
}
...
pub fn market_with_protection(action: Action, quantity: f64) -> Order {
    Order { action, order_type: "MKT PRT".to_owned(), total_quantity: quantity, ..Order::default() }
}
...
fn tag_order_non_guaranteed(mut order: Order) -> Order {
    ...
    order
        ...
        .push(TagValue { tag: "NonGuaranteed".to_owned(), value: "1".to_owned() });
    order
}
...
pub fn one_cancels_all(
    oca_group: &str,
    mut oca_orders: Vec<Order>,
    oca_type: OcaType,
) -> Vec<Order> {
