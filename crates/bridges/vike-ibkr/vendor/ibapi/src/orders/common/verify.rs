...
pub(crate) fn verify_order(
    ...
    if order.what_if {
        client
            .check_version(server_versions::WHAT_IF_ORDERS, "It does not support what-if orders.")?
    }
    ...
    if order.not_held {
        client
            .check_version(server_versions::NOT_HELD, "It does not support not_held parameter.")?
    }

    if order.exempt_code != -1 {
        client
            .check_version(server_versions::SSHORTX, "It does not support exempt_code parameter.")?
    }
...
pub(crate) fn verify_order_contract(
    client: &impl VersionedClient,
    contract: &Contract,
    _order_id: i32,
) -> Result<(), Error> {
