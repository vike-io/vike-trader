...
impl Client {
    ...
    pub async fn next_valid_order_id(&self) -> Result<i32, Error> {
        ...
        let mut internal_subscription =
            self.send_shared_request(OutgoingMessages::RequestIds, message).await?;
    ...
    pub async fn completed_orders(&self, api_only: bool) -> Result<Subscription<Orders>, Error> {
        ...
        let internal_subscription =
            self.send_shared_request(OutgoingMessages::RequestCompletedOrders, request).await?;
    ...
    pub async fn open_orders(&self) -> Result<Subscription<Orders>, Error> {
        ...
        let internal_subscription =
            self.send_shared_request(OutgoingMessages::RequestOpenOrders, request).await?;
        Ok(Subscription::new_from_internal_simple::<Orders>(
            internal_subscription,
            self.message_bus.clone(),
            self.decoder_context(),
    ...
    pub async fn all_open_orders(&self) -> Result<Subscription<Orders>, Error> {
        ...
        let internal_subscription =
            self.send_shared_request(OutgoingMessages::RequestAllOpenOrders, request).await?;
        Ok(Subscription::new_from_internal_simple::<Orders>(
            internal_subscription,
            self.message_bus.clone(),
            self.decoder_context(),
    ...
    pub async fn auto_open_orders(&self, auto_bind: bool) -> Result<Subscription<Orders>, Error> {
        ...
        let internal_subscription =
            self.send_shared_request(OutgoingMessages::RequestAutoOpenOrders, request).await?;
        Ok(Subscription::new_from_internal_simple::<Orders>(
            internal_subscription,
            self.message_bus.clone(),
            self.decoder_context(),
