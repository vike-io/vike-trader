...
use super::{
    CancelOrder, ExecutionFilter, Executions, ExerciseAction, ExerciseOptions, OrderUpdate, Orders,
    PlaceOrder,
};
...
impl Client {
    ...
    pub fn all_open_orders(&self) -> Result<Subscription<Orders>, Error> {
        ...
        let subscription =
            self.send_shared_request(OutgoingMessages::RequestAllOpenOrders, request)?;
    ...
    pub fn auto_open_orders(&self, auto_bind: bool) -> Result<Subscription<Orders>, Error> {
        ...
        let subscription =
            self.send_shared_request(OutgoingMessages::RequestAutoOpenOrders, request)?;
    ...
    pub fn completed_orders(&self, api_only: bool) -> Result<Subscription<Orders>, Error> {
        self.check_server_version(
            server_versions::COMPLETED_ORDERS,
            "It does not support completed orders requests.",
        )?;

        let request = encoders::encode_completed_orders(api_only)?;
        let subscription =
            self.send_shared_request(OutgoingMessages::RequestCompletedOrders, request)?;
    ...
    pub fn global_cancel(&self) -> Result<(), Error> {
        self.check_server_version(
            server_versions::REQ_GLOBAL_CANCEL,
    ...
    pub fn open_orders(&self) -> Result<Subscription<Orders>, Error> {
        ...
        let subscription =
            self.send_shared_request(OutgoingMessages::RequestOpenOrders, request)?;
