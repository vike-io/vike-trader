...
use crate::contracts::{
    ComboLeg, Contract, Currency, Exchange, LegAction, OptionRight, SecurityType, Symbol,
};
use crate::messages::IncomingMessages;
use crate::orders::{Action, ExecutionFilterSide, ExecutionSide, OrderStatusKind};
use crate::stubs::MessageBusStub;
use crate::testdata::builders::orders::{
    all_open_orders_request, auto_open_orders_request, cancel_order_request, commission_report,
    completed_order, completed_orders_end, completed_orders_request, execution_data,
    executions_request, global_cancel_request, next_valid_order_id_request, open_order,
...
fn place_order() {
    let message_bus = Arc::new(MessageBusStub::with_ordered_responses(vec![
        ...
        proto_response(
            IncomingMessages::OpenOrder,
            open_order().status(OrderStatusKind::Filled).encode_proto(),
        ),
        proto_response(
            IncomingMessages::OrderStatus,
            order_status()
...
fn completed_orders() {
    ...
    if let Some(Ok(Orders::OrderData(order_data))) = results.next_data() {
        ...
        assert_eq!(
            order_data.contract.security_type,
            SecurityType::Stock,
        ...
        assert_eq!(
            order_data.order_state.completed_status, "Filled",
