...
use crate::orders::{
    Action, BracketOrderBuilder, BracketOrderIds, Order, OrderBuilder, OrderData, OrderId,
    OrderState, OrderStatus, OrderStatusKind, OrderUpdate, PlaceOrder, TimeInForce,
};
...
impl<'a> OrderBuilder<'a, AsyncMockClient> {
    ...
    pub async fn submit_with_updates(
        self,
...
async fn test_async_submit_oca_orders() {
    ...
    let order_ids =
        client.submit_oca_orders(vec![(contract1, order1), (contract2, order2)]).await.unwrap();
