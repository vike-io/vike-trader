...
impl<'a, C> OrderBuilder<'a, C> {
    ...
    pub fn algo_param(mut self, key: impl Into<String>, value: impl Into<String>) -> Self {
        self.algo_params.push(TagValue { tag: key.into(), value: value.into() });
        self
    }
    ...
    pub fn build(self) -> Result<Order, ValidationError> {
        ...
        let quantity_raw =
            self.quantity.ok_or(ValidationError::MissingRequiredField("quantity"))?;
        let order_type =
            self.order_type.ok_or(ValidationError::MissingRequiredField("order_type"))?;
        ...
        let limit_price = if order_type.requires_limit_price() {
            let price_raw =
                self.limit_price.ok_or(ValidationError::MissingRequiredField("limit_price"))?;
        ...
        let stop_price = match order_type {
            OrderType::Stop | OrderType::StopLimit => {
                let price_raw =
                    self.stop_price.ok_or(ValidationError::MissingRequiredField("stop_price"))?;
                Some(Price::new(price_raw)?)
            }
            _ => {
...
impl<'a, C> BracketOrderBuilder<'a, C> {
    ...
    pub fn build(mut self) -> Result<Vec<Order>, ValidationError> {
        ...
        let take_profit_raw =
            self.take_profit_price.ok_or(ValidationError::MissingRequiredField("take_profit"))?;
        let stop_loss_raw =
            self.stop_loss_price.ok_or(ValidationError::MissingRequiredField("stop_loss"))?;
        ...
        match self.entry_type {
            BracketEntryType::None => {
                return Err(ValidationError::MissingRequiredField(
                    "entry (use entry_limit() or entry_market())",
                ));
            }
