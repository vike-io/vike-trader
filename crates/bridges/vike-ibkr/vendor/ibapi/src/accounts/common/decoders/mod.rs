...
use super::super::{
    AccountMultiValue, AccountPortfolioValue, AccountSummary, AccountUpdate, AccountUpdateTime,
    AccountValue, FaConfig, FaDataType, FamilyCode, PnL, PnLSingle, Position, PositionMulti,
    ReplaceFaResult, UserInfo, VerificationChallenge, VerificationResult,
};
...
pub(crate) fn decode_account_portfolio_value(
    message: &ResponseMessage,
) -> Result<AccountPortfolioValue, Error> {
    decode_account_portfolio_value_proto(message.require_proto()?)
}

pub(crate) fn decode_account_update_time(
    message: &ResponseMessage,
) -> Result<AccountUpdateTime, Error> {
...
pub(crate) fn decode_server_time_millis(
    message: &ResponseMessage,
...
pub(crate) fn decode_server_time_proto(bytes: &[u8]) -> Result<OffsetDateTime, Error> {
    ...
    OffsetDateTime::from_unix_timestamp(timestamp)
        .map_err(|e| Error::parse_proto("current_time", e.to_string()))
}
...
pub(crate) fn decode_account_multi_value(
    message: &ResponseMessage,
) -> Result<AccountMultiValue, Error> {
...
pub(crate) fn decode_position_proto(bytes: &[u8]) -> Result<Position, Error> {
    ...
    let contract =
        p.contract.as_ref().map(proto::decoders::decode_contract).transpose()?.unwrap_or_default();
    Ok(Position {
...
pub(crate) fn decode_account_portfolio_value_proto(
    bytes: &[u8],
) -> Result<AccountPortfolioValue, Error> {
...
pub(crate) fn decode_account_update_time_proto(bytes: &[u8]) -> Result<AccountUpdateTime, Error> {
    ...
    Ok(AccountUpdateTime { timestamp: p.time_stamp.unwrap_or_default() })
}
...
pub(crate) fn decode_soft_dollar_tiers(
    message: &ResponseMessage,
) -> Result<Vec<SoftDollarTier>, Error> {
...
pub(in crate::accounts) fn decode_soft_dollar_tiers_message(
    message: &ResponseMessage,
) -> Result<Vec<SoftDollarTier>, Error> {
...
pub(crate) fn decode_user_info_proto(bytes: &[u8]) -> Result<UserInfo, Error> {
    ...
    Ok(UserInfo { white_branding_id: p.white_branding_id.unwrap_or_default() })
}

pub(in crate::accounts) fn decode_user_info_message(
    message: &ResponseMessage,
...
pub(crate) fn decode_receive_fa_proto(bytes: &[u8]) -> Result<FaConfig, Error> {
    ...
    Ok(FaConfig { fa_data_type, xml: p.xml.unwrap_or_default() })
}
...
pub(crate) fn decode_replace_fa_end_proto(bytes: &[u8]) -> Result<ReplaceFaResult, Error> {
    ...
    Ok(ReplaceFaResult { text: p.text.unwrap_or_default() })
}

pub(in crate::accounts) fn decode_replace_fa_end_message(
    message: &ResponseMessage,
) -> Result<ReplaceFaResult, Error> {
    match message.message_type() {
...
pub(crate) fn decode_verify_message_api(
    message: &ResponseMessage,
) -> Result<VerificationChallenge, Error> {
    decode_verify_message_api_proto(message.require_proto()?)
}

pub(crate) fn decode_verify_message_api_proto(
    bytes: &[u8],
) -> Result<VerificationChallenge, Error> {
    let p = proto::VerifyMessageApi::decode(bytes)?;
    Ok(VerificationChallenge { api_data: p.api_data.unwrap_or_default() })
}

pub(crate) fn decode_verify_completed(
    message: &ResponseMessage,
...
pub(crate) fn decode_account_update_message(
    message: &ResponseMessage,
) -> Result<AccountUpdate, Error> {
    match message.message_type() {
        IncomingMessages::AccountValue => {
            decode_account_value(message).map(AccountUpdate::AccountValue)
        }
        IncomingMessages::PortfolioValue => {
            decode_account_portfolio_value(message).map(AccountUpdate::PortfolioValue)
        }
        IncomingMessages::AccountUpdateTime => {
            decode_account_update_time(message).map(AccountUpdate::UpdateTime)
        }
