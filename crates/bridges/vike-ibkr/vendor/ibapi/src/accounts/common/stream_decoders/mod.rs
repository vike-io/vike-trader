...
impl StreamDecoder<AccountSummaryResult> for AccountSummaryResult {
    ...
    fn decode(_context: &DecoderContext, message: &mut ResponseMessage) -> Result<Self, Error> {
        match message.message_type() {
            IncomingMessages::AccountSummary => {
                Ok(AccountSummaryResult::Summary(decoders::decode_account_summary(message)?))
            }
...
impl StreamDecoder<PnL> for PnL {
    const RESPONSE_MESSAGE_IDS: &'static [IncomingMessages] =
        &[IncomingMessages::PnL, IncomingMessages::Error];
    ...
    fn decode(_context: &DecoderContext, message: &mut ResponseMessage) -> Result<Self, Error> {
        match message.message_type() {
            IncomingMessages::Position => {
                Ok(PositionUpdate::Position(decoders::decode_position(message)?))
            }
...
impl StreamDecoder<PositionUpdateMulti> for PositionUpdateMulti {
    ...
    fn decode(_context: &DecoderContext, message: &mut ResponseMessage) -> Result<Self, Error> {
        match message.message_type() {
            IncomingMessages::PositionMulti => {
                Ok(PositionUpdateMulti::Position(decoders::decode_position_multi(message)?))
            }
...
impl StreamDecoder<AccountUpdate> for AccountUpdate {
    ...
    fn decode(_context: &DecoderContext, message: &mut ResponseMessage) -> Result<Self, Error> {
        match message.message_type() {
            IncomingMessages::AccountValue => {
                Ok(AccountUpdate::AccountValue(decoders::decode_account_value(message)?))
            }
            IncomingMessages::PortfolioValue => Ok(AccountUpdate::PortfolioValue(
                decoders::decode_account_portfolio_value(message)?,
            )),
            IncomingMessages::AccountUpdateTime => {
                Ok(AccountUpdate::UpdateTime(decoders::decode_account_update_time(message)?))
            }
...
impl StreamDecoder<AccountUpdateMulti> for AccountUpdateMulti {
    ...
    fn decode(_context: &DecoderContext, message: &mut ResponseMessage) -> Result<Self, Error> {
        match message.message_type() {
            IncomingMessages::AccountUpdateMulti => Ok(AccountUpdateMulti::AccountMultiValue(
                decoders::decode_account_multi_value(message)?,
            )),
    ...
    fn cancel_message(
        ...
        let request_id = error_helpers::require_request_id_for(
            request_id,
            "encode cancel account updates multi",
        )?;
