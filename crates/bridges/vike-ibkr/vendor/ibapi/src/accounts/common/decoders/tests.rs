...
fn test_decode_account_update_time_proto() {
    ...
    let proto_msg = crate::proto::AccountUpdateTime { time_stamp: Some("12:34:56".into()) };
...
fn test_decode_managed_accounts_proto() {
    use prost::Message;
    let proto_msg =
        crate::proto::ManagedAccounts { accounts_list: Some("DU1111,DU2222,DU3333".into()) };
    let mut bytes = Vec::new();
    proto_msg.encode(&mut bytes).unwrap();
...
fn test_decode_managed_accounts_proto_skips_empty() {
    ...
    let proto_msg = crate::proto::ManagedAccounts { accounts_list: Some("DU1111,,DU2222,".into()) };
    let mut bytes = Vec::new();
    proto_msg.encode(&mut bytes).unwrap();
...
fn test_decode_user_info_proto_round_trip() {
    use prost::Message;
    let p = crate::proto::UserInfo { req_id: Some(1), white_branding_id: Some("brand-xyz".into()) };
...
fn test_decode_receive_fa_rejects_invalid_fa_data_type() {
    use prost::Message;
    let p = crate::proto::ReceiveFa { fa_data_type: Some(2), xml: Some("<bad/>".into()) };
...
fn test_decode_receive_fa_rejects_missing_fa_data_type() {
    use prost::Message;
    let p = crate::proto::ReceiveFa { fa_data_type: None, xml: Some("<groups/>".into()) };
    let err = super::decode_receive_fa_proto(&p.encode_to_vec()).unwrap_err();
    assert!(matches!(err, super::Error::Parse(_, _, _)), "got {err:?}");
}
...
fn test_decode_replace_fa_end_proto_round_trip() {
    use prost::Message;
    let p = crate::proto::ReplaceFaEnd { req_id: Some(7), text: Some("done".into()) };
...
fn test_decode_verify_message_api_proto_round_trip() {
    use prost::Message;
    let p = crate::proto::VerifyMessageApi { api_data: Some("payload".into()) };
