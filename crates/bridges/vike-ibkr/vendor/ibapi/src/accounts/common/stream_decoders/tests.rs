...
use crate::testdata::builders::accounts::{
    ...
    account_update_multi_end, account_update_time, account_value, portfolio_value,
};
...
mod account_summary_tests {
    ...
    fn test_decode_error_message() {
        ...
        let mut message =
            proto_error_response(123, 10089, "Requested market data is not subscribed");
    ...
    fn test_cancel_message() {
        let bytes =
            AccountSummaryResult::cancel_message(TEST_SERVER_VERSION, Some(TEST_REQUEST_ID), None)
                .unwrap();
...
mod pnl_single_tests {
    ...
    fn test_cancel_message() {
        let bytes =
            PnLSingle::cancel_message(TEST_SERVER_VERSION, Some(TEST_REQUEST_ID), None).unwrap();
    ...
    fn test_response_message_ids() {
        assert_eq!(
            PnLSingle::RESPONSE_MESSAGE_IDS,
            &[IncomingMessages::PnLSingle, IncomingMessages::Error]
        );
    }
...
mod position_update_multi_tests {
    ...
    fn test_cancel_message() {
        let bytes =
            PositionUpdateMulti::cancel_message(TEST_SERVER_VERSION, Some(TEST_REQUEST_ID), None)
                .unwrap();
...
mod account_update_tests {
    ...
    fn test_decode_portfolio_value() {
        let mut message = proto_response(
            ...
            portfolio_value().contract_id(12345).encode_proto(),
        );
    ...
    fn test_decode_account_download_end() {
        let mut message = proto_response(
            ...
            account_download_end().encode_proto(),
        );
...
mod account_update_multi_tests {
    ...
    fn test_cancel_message() {
        let bytes =
            AccountUpdateMulti::cancel_message(TEST_SERVER_VERSION, Some(TEST_REQUEST_ID), None)
                .unwrap();
...
mod edge_cases {
    ...
    fn test_context_parameter_ignored() {
        ...
        let context = DecoderContext::new(TEST_SERVER_VERSION, None)
        ...
        let result1 =
            AccountSummaryResult::cancel_message(TEST_SERVER_VERSION, Some(TEST_REQUEST_ID), None)
                .unwrap();
        let result2 = AccountSummaryResult::cancel_message(
            TEST_SERVER_VERSION,
