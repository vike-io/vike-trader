...
use crate::testdata::builders::accounts::{
    ...
    account_update_multi_end, account_value, cancel_account_summary, cancel_account_updates_multi,
    cancel_pnl, cancel_pnl_single, current_time, fa_replace_request, fa_request, family_codes,
    managed_accounts, receive_fa, replace_fa_end, request_account_summary, request_account_updates,
    request_account_updates_multi, request_current_time, request_family_codes,
    request_managed_accounts, request_pnl, request_pnl_single, set_server_log_level_request,
    soft_dollar_tiers, soft_dollar_tiers_request, user_info, user_info_request, verify_completed,
    verify_message_api, verify_message_request, verify_request,
};
...
async fn test_account_summary() {
    ...
    let mut subscription =
        client.account_summary(&group, tags).await.expect("request account_summary failed");
...
async fn test_pnl() {
    ...
    let subscription1 =
        client.pnl(&account, model_code.as_ref()).await.expect("request pnl failed");
    drop(subscription1);
...
async fn test_pnl_single() {
    ...
    let subscription2 =
        client.pnl_single(&account, contract_id, None).await.expect("request pnl_single failed");
    drop(subscription2);
    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
...
async fn test_managed_accounts() {
    let (client, message_bus) =
        create_test_client_with_ordered_proto_responses(vec![proto_response(
    ...
    for test_case in VERSION_TEST_CASES {
        ...
        let result = match test_case.function_name {
            ...
            "Account Summary" => client
                .account_summary(&group, &[AccountSummaryTags::ACCOUNT_TYPE])
                .await
                .map(|_| ()),
            "Positions Multi" => client.positions_multi(Some(&account), None).await.map(|_| ()),
            "Account Updates Multi" => {
                client.account_updates_multi(Some(&account), None).await.map(|_| ())
            }
    ...
    for test_case in test_cases {
        ...
        if test_case.expect_responses {
            ...
            assert!(
                matches!(
                    first_update,
                    Some(Ok(SubscriptionItem::Data(AccountSummaryResult::Summary(_))))
                ),
...
async fn test_soft_dollar_tiers_version_error() {
    let (client, message_bus) =
        create_test_client_with_version(server_versions::SOFT_DOLLAR_TIER - 1);
...
async fn test_replace_fa_version_error() {
    ...
    let (client, message_bus) =
        create_test_client_with_version(server_versions::REPLACE_FA_END - 1);
...
async fn test_set_server_log_level() {
    ...
    let (client, message_bus) = create_test_client();
    client.set_server_log_level(ServerLogLevel::Detail).await.expect("set_server_log_level failed");
    assert_eq!(request_message_count(&message_bus), 1);
    assert_request(
        &message_bus,
        0,
