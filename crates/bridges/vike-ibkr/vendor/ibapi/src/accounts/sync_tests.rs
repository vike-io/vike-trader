...
use crate::testdata::builders::accounts::{
    account_download_end, account_summary, account_summary_end, account_update_multi,
    account_update_multi_end, account_value, cancel_account_summary, cancel_account_updates,
    cancel_account_updates_multi, cancel_pnl, cancel_pnl_single, current_time,
    current_time_in_millis, fa_replace_request, fa_request, family_codes, managed_accounts,
    receive_fa, replace_fa_end, request_account_summary, request_account_updates,
    request_account_updates_multi, request_current_time, request_current_time_in_millis,
    request_family_codes, request_managed_accounts, request_pnl, request_pnl_single,
    set_server_log_level_request, soft_dollar_tiers, soft_dollar_tiers_request, user_info,
    user_info_request, verify_completed, verify_message_api, verify_message_request,
    verify_request,
};
use crate::testdata::builders::positions::{
    cancel_positions, cancel_positions_multi, position, position_end, position_multi,
    position_multi_end, request_positions, request_positions_multi,
};
...
fn test_pnl() {
    ...
    assert_request(
        &message_bus,
        2,
        &request_pnl().request_id(TEST_REQ_ID_FIRST + 1).no_model_code(),
    );
...
fn test_pnl_single() {
    ...
    let model_code = Some(ModelCode(TEST_MODEL_CODE.to_string()));
    let _ =
        client.pnl_single(&account, contract_id, model_code.as_ref()).expect("request pnl failed");
    ...
    assert_request(
        &message_bus,
        2,
        &request_pnl_single().request_id(TEST_REQ_ID_FIRST + 1).no_model_code(),
    );
...
fn test_account_summary() {
    ...
    let subscription =
        client.account_summary(&group, tags).expect("request account_summary failed");
...
fn test_managed_accounts() {
    let (client, message_bus) =
        create_blocking_test_client_with_ordered_proto_responses(vec![proto_response(
            IncomingMessages::ManagedAccounts,
            managed_accounts().accounts([TEST_ACCOUNT, TEST_ACCOUNT_2]).encode_proto(),
        )]);
    let accounts =
        client.managed_accounts().expect("request managed accounts failed for valid response");
    ...
    let accounts_no_msg =
        client_no_msg.managed_accounts().expect("request managed accounts failed for no message");
    assert!(
        accounts_no_msg.is_empty(),
        "Accounts list should be empty when no message is received"
    );
}
...
fn test_family_codes() {
    ...
    assert_eq!(
        codes[0],
        FamilyCode { account_id: "ACC1".to_string(), family_code: "FC1".to_string() }
    );
    assert_eq!(
        codes[1],
        FamilyCode { account_id: "ACC2".to_string(), family_code: "FC2".to_string() }
    );
    assert_eq!(request_message_count(&message_bus), 1);
...
fn test_account_updates_multi() {
    let message_bus = Arc::new(MessageBusStub::with_ordered_responses(vec![
        ...
        proto_response(
            ...
            account_update_multi_end().encode_proto(),
        ),
    ]));
...
fn test_server_version_errors() {
    ...
    for test_case in VERSION_TEST_CASES {
        ...
        let result = match test_case.function_name {
            ...
            "Account Summary" => {
                client.account_summary(&group, &[AccountSummaryTags::ACCOUNT_TYPE]).map(|_| ())
            }
            "Positions Multi" => client.positions_multi(Some(&account), None).map(|_| ()),
            "Account Updates Multi" => {
                client.account_updates_multi(Some(&account), None).map(|_| ())
            }
        ...
        if test_case.expect_responses {
            let message_bus = Arc::new(MessageBusStub::with_ordered_responses(vec![
                ...
                proto_response(
                    ...
                    account_summary_end().encode_proto(),
                ),
            ]));
            let client = Client::stubbed(message_bus.clone(), server_versions::SIZE_RULES);
    ...
    for test_case in &test_cases {
        match &test_case.subscription_type {
            SubscriptionType::PnL { account, model_code } => {
                ...
                let sub = client.pnl(&account_id, model.as_ref()).unwrap_or_else(|_| {
                    panic!("PnL subscription failed for {}", test_case.description)
                });
                drop(sub);
            }
            SubscriptionType::Positions => {
                let sub = client.positions().unwrap_or_else(|_| {
                    panic!("Positions subscription failed for {}", test_case.description)
                });
                drop(sub);
            }
            SubscriptionType::AccountSummary { group, tags } => {
                ...
                let sub = client.account_summary(&group_id, &tag_refs).unwrap_or_else(|_| {
                    panic!("Account Summary subscription failed for {}", test_case.description)
                });
                drop(sub);
            }
            SubscriptionType::PositionsMulti { account, model_code } => {
                ...
                let sub = client
                    ...
                    .unwrap_or_else(|_| {
                        panic!("Positions Multi subscription failed for {}", test_case.description)
                    });
                drop(sub);
            }
            SubscriptionType::PnLSingle { account, contract_id, model_code } => {
                let account_id = AccountId(account.clone());
                let contract = ContractId(*contract_id);
                let model = model_code.as_ref().map(|s| ModelCode(s.clone()));
                let sub =
                    client.pnl_single(&account_id, contract, model.as_ref()).unwrap_or_else(|_| {
                        panic!("PnL Single subscription failed for {}", test_case.description)
                    });
                drop(sub);
            }
        }
...
fn test_server_time_millis_version_error() {
    let (client, message_bus) =
        create_blocking_test_client_with_version(server_versions::CURRENT_TIME_IN_MILLIS - 1);
...
fn test_server_time_millis_no_response() {
    let (client, message_bus) =
        create_blocking_test_client_with_version(server_versions::CURRENT_TIME_IN_MILLIS);
...
fn test_soft_dollar_tiers_version_error() {
    let (client, message_bus) =
        create_blocking_test_client_with_version(server_versions::SOFT_DOLLAR_TIER - 1);
...
fn test_user_info_version_error() {
    let (client, message_bus) =
        create_blocking_test_client_with_version(server_versions::USER_INFO - 1);
...
fn test_replace_fa() {
    ...
    let result =
        client.replace_fa(FaDataType::AccountAliases, "<aliases/>").expect("replace_fa failed");
...
fn test_replace_fa_version_error() {
    ...
    let (client, message_bus) =
        create_blocking_test_client_with_version(server_versions::REPLACE_FA_END - 1);
...
fn test_set_server_log_level() {
    ...
    assert_request(
        &message_bus,
        0,
        &set_server_log_level_request().log_level(ServerLogLevel::Detail as i32),
    );
}
...
fn test_verify_request_version_error() {
    let (client, message_bus) =
        create_blocking_test_client_with_version(server_versions::LINKING - 1);
