...
pub fn encode_contract_with_order(contract: &Contract, order: Option<&Order>) -> proto::Contract {
    proto::Contract {
        ...
        delta_neutral_contract: contract
            ...
            .as_ref()
            .map(encode_delta_neutral_contract),
        combo_legs: encode_combo_legs(contract, order),
        last_trade_date: None,
    }
}

fn encode_delta_neutral_contract(
    dnc: &contracts::DeltaNeutralContract,
...
fn encode_combo_legs(contract: &Contract, order: Option<&Order>) -> Vec<proto::ComboLeg> {
    ...
    contract
        ...
        .map(|(i, leg)| {
            let per_leg_price =
                order.and_then(|o| o.order_combo_legs.get(i)).and_then(|ocl| ocl.price);
...
pub fn tag_values_to_map(
    tags: &[crate::contracts::TagValue],
