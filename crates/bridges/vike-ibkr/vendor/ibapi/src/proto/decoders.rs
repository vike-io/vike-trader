use crate::contracts::{
    ComboLeg, ComboLegOpenClose, Contract, ContractDetails, Currency, DeltaNeutralContract,
    Exchange, FundAssetType, FundDistributionPolicyIndicator, IneligibilityReason, LegAction,
    SecurityIdType, SecurityType, Symbol, TagValue,
};
use crate::orders::conditions::TriggerMethod;
use crate::orders::{
    Action, Execution, ExecutionSide, Liquidity, OcaType, Order, OrderAllocation, OrderCondition,
    OrderOpenClose, OrderOrigin, OrderState, ReferencePriceType, Rule80A, ShortSaleSlot,
    SoftDollarTier, TimeInForce, VolatilityType,
};
use crate::proto;
use crate::Error;
...
pub(crate) fn optional_string_f64(opt: &Option<String>) -> Option<f64> {
    opt.as_deref().and_then(|s| s.parse::<f64>().ok()).and_then(|v| {
        if v == f64::MAX {
            None
        } else {
            Some(v)
...
pub(crate) fn tag_values(map: &std::collections::HashMap<String, String>) -> Vec<TagValue> {
    map.iter().map(|(k, v)| TagValue { tag: k.clone(), value: v.clone() }).collect()
}
...
pub fn decode_contract(proto: &proto::Contract) -> Result<Contract, Error> {
    Ok(Contract {
        ...
        delta_neutral_contract: proto
            .delta_neutral_contract
            .as_ref()
            .map(decode_delta_neutral_contract),
...
pub fn decode_order(proto: &proto::Order) -> Order {
    ...
    order.override_percentage_constraints =
        proto.override_percentage_constraints.unwrap_or_default();
    ...
    order.is_pegged_change_amount_decrease =
        proto.is_pegged_change_amount_decrease.unwrap_or_default();
    ...
    order.soft_dollar_tier =
        proto.soft_dollar_tier.as_ref().map(decode_soft_dollar_tier).unwrap_or_default();
    ...
    order.discretionary_up_to_limit_price =
        proto.discretionary_up_to_limit_price.unwrap_or_default();
...
fn decode_order_condition(proto: &proto::OrderCondition) -> OrderCondition {
    ...
    match condition_type {
        ...
        3 => OrderCondition::Time(TimeCondition { time: s(&proto.time), is_more, is_conjunction }),
...
pub fn decode_order_state(proto: &proto::OrderState) -> Result<OrderState, Error> {
    Ok(OrderState {
        ...
        equity_with_loan_before_outside_rth: optional_f64(
            proto.equity_with_loan_before_outside_rth,
        ),
        ...
        equity_with_loan_change_outside_rth: optional_f64(
            proto.equity_with_loan_change_outside_rth,
        ),
...
pub fn decode_contract_details(
    proto_contract: &proto::Contract,
    proto_details: &proto::ContractDetails,
    ...
    Ok(ContractDetails {
        ...
        fund_closed_for_new_investors: proto_details
            .fund_closed_for_new_investors
            .unwrap_or_default(),
        ...
        fund_asset_type: FundAssetType::from(
            proto_details.fund_asset_type.as_deref().unwrap_or(""),
        ),
        // bond fields
        ...
        ineligibility_reasons: proto_details
            ...
            .iter()
            .map(|r| IneligibilityReason { id: s(&r.id), description: s(&r.description) })
            .collect(),
