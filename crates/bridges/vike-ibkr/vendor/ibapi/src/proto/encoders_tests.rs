...
fn test_encode_contract_with_multiplier() {
    let contract = Contract { multiplier: "100".to_string(), ..Default::default() };
...
fn test_encode_delta_neutral() {
    let dnc = contracts::DeltaNeutralContract { contract_id: 123, delta: 0.5, price: 45.0 };
    let contract = Contract { delta_neutral_contract: Some(dnc), ..Default::default() };
...
fn test_encode_condition_price_default_trigger_method_is_emitted() {
    ...
    let proto = encode_condition(&cond);
    assert_eq!(
        proto.trigger_method,
        Some(0),
        "Default trigger_method must be emitted, not omitted"
    );
}
...
fn test_tag_values_to_map() {
    let tags = vec![
        crate::contracts::TagValue { tag: "k1".to_string(), value: "v1".to_string() },
        crate::contracts::TagValue { tag: "k2".to_string(), value: "v2".to_string() },
    ];
