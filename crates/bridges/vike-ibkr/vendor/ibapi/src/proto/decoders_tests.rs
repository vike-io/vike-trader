...
fn parse_required_none_errors_with_label() {
    ...
    match err {
        Error::Parse(_, _, msg) => {
            assert!(msg.contains("OrderStatus"), "expected label in message, got: {msg}")
        }
...
fn parse_optional_unknown_propagates_fromstr_err() {
    assert!(matches!(
        parse_optional::<OrderStatusKind>(Some("Garbage")),
        Err(Error::Parse(_, _, _))
    ));
}
...
fn decode_contract_propagates_bad_combo_leg() {
    let proto_contract =
        proto::Contract { combo_legs: vec![proto_leg(Some("NOTAVARIANT"))], ..Default::default() };
