...
impl Features {
    ...
    pub const POSITIONS: ProtocolFeature =
        ProtocolFeature::new("positions", server_versions::POSITIONS);
    /// Enables the account summary API (`reqAccountSummary`).
    pub const ACCOUNT_SUMMARY: ProtocolFeature =
        ProtocolFeature::new("account summary", server_versions::ACCOUNT_SUMMARY);
    /// Grants access to account family metadata via `reqFamilyCodes`.
    pub const FAMILY_CODES: ProtocolFeature =
        ProtocolFeature::new("family codes", server_versions::REQ_FAMILY_CODES);
    ...
    pub const UNREALIZED_PNL: ProtocolFeature =
        ProtocolFeature::new("unrealized PnL", server_versions::UNREALIZED_PNL);
    /// Required to request realized PnL values.
    pub const REALIZED_PNL: ProtocolFeature =
        ProtocolFeature::new("realized PnL", server_versions::REALIZED_PNL);
    /// Indicates support for model-code scoped requests.
    pub const MODELS_SUPPORT: ProtocolFeature =
        ProtocolFeature::new("models support", server_versions::MODELS_SUPPORT);

    /// Enables streaming real-time bars through `reqRealTimeBars`.
    pub const REAL_TIME_BARS: ProtocolFeature =
        ProtocolFeature::new("real-time bars", server_versions::REAL_TIME_BARS);
    /// Allows switching market data modes (live, frozen, delayed).
    pub const MARKET_DATA_TYPE: ProtocolFeature =
        ProtocolFeature::new("market data type", server_versions::REQ_MARKET_DATA_TYPE);
    /// Required for tick-by-tick market data.
    pub const TICK_BY_TICK: ProtocolFeature =
        ProtocolFeature::new("tick-by-tick data", server_versions::TICK_BY_TICK);
    /// Allows omitting the size field in tick-by-tick subscriptions.
    pub const TICK_BY_TICK_IGNORE_SIZE: ProtocolFeature = ProtocolFeature::new(
        "tick-by-tick ignore size parameter",
        server_versions::TICK_BY_TICK_IGNORE_SIZE,
    );
    /// Required to request histogram data snapshots.
    pub const HISTOGRAM: ProtocolFeature =
        ProtocolFeature::new("histogram data", server_versions::REQ_HISTOGRAM);
    /// Enables historical tick downloads.
    pub const HISTORICAL_TICKS: ProtocolFeature =
        ProtocolFeature::new("historical ticks", server_versions::HISTORICAL_TICKS);
    /// Allows requesting the head timestamp for historical data.
    pub const HEAD_TIMESTAMP: ProtocolFeature =
        ProtocolFeature::new("head timestamp", server_versions::REQ_HEAD_TIMESTAMP);
    /// Provides synthetic real-time bars for illiquid contracts.
    pub const SYNT_REALTIME_BARS: ProtocolFeature =
        ProtocolFeature::new("synthetic real-time bars", server_versions::SYNT_REALTIME_BARS);
    /// Enables retrieval of historical trading schedules.
    pub const HISTORICAL_SCHEDULE: ProtocolFeature =
        ProtocolFeature::new("historical schedule", server_versions::HISTORICAL_SCHEDULE);
    /// Required to stream SMART depth data.
    pub const SMART_DEPTH: ProtocolFeature =
        ProtocolFeature::new("SMART depth", server_versions::SMART_DEPTH);
    /// Exposes the primary exchange in market depth responses.
    pub const MKT_DEPTH_PRIM_EXCHANGE: ProtocolFeature = ProtocolFeature::new(
        "market depth primary exchange",
    ...
    pub const WHAT_IF_ORDERS: ProtocolFeature =
        ProtocolFeature::new("what-if orders", server_versions::WHAT_IF_ORDERS);
    /// Enables the order container flag when placing orders.
    pub const ORDER_CONTAINER: ProtocolFeature =
        ProtocolFeature::new("order container", server_versions::ORDER_CONTAINER);
    /// Allows auto-cancelling parent orders when children fill.
    pub const AUTO_CANCEL_PARENT: ProtocolFeature =
        ProtocolFeature::new("auto cancel parent", server_versions::AUTO_CANCEL_PARENT);
    ...
    pub const CASH_QTY: ProtocolFeature =
        ProtocolFeature::new("cash quantity", server_versions::CASH_QTY);
    /// Enables MiFID decision maker and execution trader fields.
    pub const DECISION_MAKER: ProtocolFeature =
        ProtocolFeature::new("decision maker", server_versions::DECISION_MAKER);
    /// Enables MiFID execution information fields.
    pub const MIFID_EXECUTION: ProtocolFeature =
        ProtocolFeature::new("MiFID execution", server_versions::MIFID_EXECUTION);
    /// Allows setting manual order time stamps.
    pub const MANUAL_ORDER_TIME: ProtocolFeature =
        ProtocolFeature::new("manual order time", server_versions::MANUAL_ORDER_TIME);
    /// Required for the completed orders endpoint.
    pub const COMPLETED_ORDERS: ProtocolFeature =
        ProtocolFeature::new("completed orders", server_versions::COMPLETED_ORDERS);
    /// Enables global cancel requests affecting all open orders.
    pub const REQ_GLOBAL_CANCEL: ProtocolFeature =
        ProtocolFeature::new("global cancel", server_versions::REQ_GLOBAL_CANCEL);

    /// Exposes trading class metadata in contract details.
    pub const TRADING_CLASS: ProtocolFeature =
        ProtocolFeature::new("trading class", server_versions::TRADING_CLASS);
    /// Provides size rules per exchange for contract validation.
    pub const SIZE_RULES: ProtocolFeature =
        ProtocolFeature::new("size rules", server_versions::SIZE_RULES);
    /// Adds issuer identifiers to bond contract details.
    pub const BOND_ISSUERID: ProtocolFeature =
        ProtocolFeature::new("bond issuer ID", server_versions::BOND_ISSUERID);
    /// Adds support for security identifier types (ISIN, CUSIP, etc.).
    pub const SEC_ID_TYPE: ProtocolFeature =
        ProtocolFeature::new("security ID type", server_versions::SEC_ID_TYPE);
    /// Required for smart components (routing map) queries.
    pub const SMART_COMPONENTS: ProtocolFeature =
        ProtocolFeature::new("smart components", server_versions::REQ_SMART_COMPONENTS);
    ...
    pub const SOFT_DOLLAR_TIER: ProtocolFeature =
        ProtocolFeature::new("soft dollar tier", server_versions::SOFT_DOLLAR_TIER);

    /// Allows fetching available news providers.
    pub const NEWS_PROVIDERS: ProtocolFeature =
        ProtocolFeature::new("news providers", server_versions::REQ_NEWS_PROVIDERS);
    /// Enables downloading full news articles.
    pub const NEWS_ARTICLE: ProtocolFeature =
        ProtocolFeature::new("news article", server_versions::REQ_NEWS_ARTICLE);
    /// Enables historical news queries.
    pub const HISTORICAL_NEWS: ProtocolFeature =
        ProtocolFeature::new("historical news", server_versions::REQ_HISTORICAL_NEWS);
    /// Allows filtering historical news by origin.
    pub const NEWS_QUERY_ORIGINS: ProtocolFeature =
        ProtocolFeature::new("news query origins", server_versions::NEWS_QUERY_ORIGINS);
    ...
    pub const FUNDAMENTAL_DATA: ProtocolFeature =
        ProtocolFeature::new("fundamental data", server_versions::FUNDAMENTAL_DATA);

    /// Adds Wall Street Horizon earnings calendar support.
    pub const WSHE_CALENDAR: ProtocolFeature =
        ProtocolFeature::new("WSHE Calendar", server_versions::WSHE_CALENDAR);
    ...
    pub const WSH_EVENT_DATA_FILTERS_DATE: ProtocolFeature = ProtocolFeature::new(
        "WSH event data filters with date",
    ...
    pub const REPLACE_FA_END: ProtocolFeature =
        ProtocolFeature::new("replace FA end", server_versions::REPLACE_FA_END);
    /// Required to request market rule metadata.
    pub const MARKET_RULES: ProtocolFeature =
        ProtocolFeature::new("market rules", server_versions::MARKET_RULES);
    ...
    pub const REQ_CALC_IMPLIED_VOLAT: ProtocolFeature = ProtocolFeature::new(
    ...
    pub const SEC_DEF_OPT_PARAMS_REQ: ProtocolFeature = ProtocolFeature::new(
    ...
    pub const CURRENT_TIME_IN_MILLIS: ProtocolFeature =
        ProtocolFeature::new("current time in millis", server_versions::CURRENT_TIME_IN_MILLIS);
    /// Required for user info queries.
    pub const USER_INFO: ProtocolFeature =
        ProtocolFeature::new("user info", server_versions::USER_INFO);
    /// Enables cancelling in-flight contract data and historical ticks requests.
    pub const CANCEL_CONTRACT_DATA: ProtocolFeature =
        ProtocolFeature::new("cancel contract data", server_versions::CANCEL_CONTRACT_DATA);
}
