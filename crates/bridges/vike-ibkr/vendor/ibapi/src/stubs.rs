...
static ORDER_UPDATE_SUBSCRIPTION_TRACKER: LazyLock<Mutex<HashSet<usize>>> =
...
impl MessageBusStub {
    pub fn with_responses(response_messages: Vec<String>) -> Self {
        Self { request_messages: RwLock::new(vec![]), response_messages, ordered_responses: vec![] }
    }
    ...
    pub fn with_ordered_responses(ordered_responses: Vec<ResponseMessage>) -> Self {
        Self { request_messages: RwLock::new(vec![]), response_messages: vec![], ordered_responses }
    }
