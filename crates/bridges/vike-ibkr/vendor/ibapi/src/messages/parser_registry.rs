    ...
    fn parse(&self, parts: &[&str]) -> Vec<ParsedField> {
        ...
        for field_def in &self.fields {
            if let Some(value) = parts.get(field_def.index) {
                ...
                result
                    .push(ParsedField { name: field_def.name.to_string(), value: processed_value });
            }
        }
...
impl MessageParserRegistry {
    ...
    pub fn new() -> Self {
        let mut registry =
            Self { request_parsers: HashMap::new(), response_parsers: HashMap::new() };
    ...
    pub fn register_request_parser(
        &mut self,
    ...
    pub fn register_response_parser(
        &mut self,
...
pub fn parse_generic_message(parts: &[&str]) -> Vec<ParsedField> {
    ...
    if let Some(msg_type) = parts.first() {
        fields.push(ParsedField { name: "message_type".to_string(), value: msg_type.to_string() });
    }
    ...
    for (i, part) in parts.iter().skip(1).enumerate() {
        ...
        fields.push(ParsedField { name: format!("field_{}", i + 2), value: part.to_string() });
    }
