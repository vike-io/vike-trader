...
#[test]
fn test_notice() {
    let message = crate::common::test_utils::helpers::proto_error_response(
        -1,
        2107,
        ...
        match (&test_case.parse_type, &test_case.expected) {
            (ParseType::Int, ParseResult::Int(expected)) => match message.next_int() {
                ...
                Err(e) => panic!(
                    ...
                    test_case.name, expected, e
                ),
            },
            (ParseType::Int, ParseResult::Error) => {
                assert!(
                    message.next_int().is_err(),
...
fn test_order_id_protobuf_open_order() {
    let bytes =
        crate::proto::OpenOrder { order_id: Some(42), ..Default::default() }.encode_to_vec();
...
fn test_order_id_protobuf_execution_data_nested() {
    ...
    let bytes = crate::proto::ExecutionDetails {
        req_id: Some(7),
        contract: None,
        execution: Some(crate::proto::Execution { order_id: Some(123), ..Default::default() }),
    }
    .encode_to_vec();
    let message = ResponseMessage::from_protobuf(IncomingMessages::ExecutionData as i32, bytes);
...
fn test_request_message_index_out_of_bounds() {
    let message = RequestMessage { fields: vec!["field0".to_string()] };
...
fn test_field_based_parser() {
    ...
    let test_cases = vec![
        ...
        TestCase {
            name: "missing_fields",
            fields: vec![
                FieldDef::new(0, "field1"),
                FieldDef::new(1, "field2"),
                FieldDef::new(5, "field6"),
            ],
            input: vec!["val1", "val2"],
            expected_count: 2,
    ...
    for test_case in test_cases {
        ...
        for (field, (expected_name, expected_value)) in
            result.iter().zip(test_case.expected_values.iter())
        {
            assert_eq!(
                field.name, *expected_name,
                "Test '{}' failed: wrong field name",
                test_case.name
            );
            assert_eq!(
                field.value, *expected_value,
                "Test '{}' failed: wrong field value",
                test_case.name
            );
        }
    }
...
fn test_message_parser_registry() {
    ...
    for test_case in test_cases {
        ...
        for (field, (expected_name, expected_value)) in
            result.iter().zip(test_case.expected_fields.iter())
        {
            assert_eq!(
                field.name, *expected_name,
                "Test '{}' failed: wrong field name",
...
fn test_response_parser_registry() {
    ...
    let test_cases = vec![
        ...
        TestCase {
            ...
            input: vec![
                "61", "3", "DU12345", "12345", "AAPL", "STK", "", "0", "", "", "NASDAQ", "USD",
                "AAPL", "NMS", "100", "150.50",
            ],
...
fn test_response_message_next_methods_edge_cases() {
    ...
    let test_cases = vec![
        TestCase { name: "empty_message", input: "", test_fn: |msg| msg.next_int().is_err() },
        TestCase {
...
fn test_exclusive_one_shot_response_types() {
    ...
    for included in [
        ...
        assert!(
            one_shot.contains(&included),
            "{included:?} should be a one-shot error response type"
        );
    }
    ...
    for excluded in [
        ...
        assert!(
            !one_shot.contains(&excluded),
            "{excluded:?} is streaming and must not be fail-fast"
        );
    }
}
...
fn test_notice_edge_cases() {
    ...
    let test_cases = vec![
        ...
        TestCase { name: "empty_message", code: 1000, msg: "" },
        TestCase { name: "negative_code", code: -500, msg: "Negative error code" },
    ];
    ...
    for test_case in test_cases {
        ...
        assert_eq!(
            notice.code, test_case.code,
            "Test '{}' failed: wrong error code",
            test_case.name
        );
        assert_eq!(
            notice.message, test_case.msg,
...
fn test_notice_is_order_rejection() {
    ...
    for code in [start, start + 1, ORDER_CANCELLED_CODE, end - 1, end] {
        assert!(
            notice_with_code(code).is_order_rejection(),
            "code {code} should be order rejection"
        );
    }

    for code in
        [start - 1, end + 1, 100, *WARNING_CODE_RANGE.start(), SYSTEM_MESSAGE_CODES[0], 10000]
    {
        assert!(
            !notice_with_code(code).is_order_rejection(),
...
fn test_connectivity_status_subset_of_warning() {
    ...
    for &code in all_farm {
        ...
        assert!(
            ...
            "farm code {code} should be inside WARNING_CODE_RANGE"
        );
    }
}
