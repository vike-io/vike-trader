    ...
    pub(super) fn validate(self) -> Result<ValidatedPieces, Error> {
        Ok(ValidatedPieces {
            address: self.address.ok_or_else(|| {
                Error::InvalidArgument("ClientBuilder: address is required".into())
            })?,
            client_id: self.client_id.ok_or_else(|| {
                Error::InvalidArgument("ClientBuilder: client_id is required".into())
            })?,
...
pub mod sync_impl {
    ...
    impl ClientBuilder {
        ...
        ///     .connect();
        /// ```
        pub fn startup_callback(
            mut self,
            callback: impl Fn(StartupMessage) + Send + Sync + 'static,
        ) -> Self {
