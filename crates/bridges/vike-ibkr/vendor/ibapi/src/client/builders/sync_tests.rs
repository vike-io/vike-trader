...
fn create_test_client() -> Client {
    let client =
        Client::stubbed(Arc::new(MessageBusStub::default()), server_versions::PROTOBUF_SCAN_DATA);
...
fn test_subscription_builder_with_context() {
    ...
    let builder: SubscriptionBuilder<Bar> =
        SubscriptionBuilder::new(&client).with_context(context.clone());
...
fn test_subscription_builder_with_smart_depth() {
    let client = create_test_client();
    let builder: SubscriptionBuilder<Bar> =
        SubscriptionBuilder::new(&client).with_smart_depth(true);
