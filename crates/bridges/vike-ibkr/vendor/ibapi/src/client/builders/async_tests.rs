...
async fn test_subscription_builder_with_context() {
    ...
    let message_bus = client.message_bus.clone();
    let builder: SubscriptionBuilder<Bar> =
        SubscriptionBuilder::new_with_components(client.decoder_context(), message_bus)
...
async fn test_subscription_builder_with_smart_depth() {
    ...
    let message_bus = client.message_bus.clone();
    let builder: SubscriptionBuilder<Bar> =
        SubscriptionBuilder::new_with_components(context, message_bus).with_smart_depth(true);
...
async fn test_response_context_modifications() {
    ...
    for tc in test_cases {
        ...
        let context = client.decoder_context();
        let message_bus = client.message_bus.clone();
        let mut builder: SubscriptionBuilder<Bar> =
            SubscriptionBuilder::new_with_components(context, message_bus);
        ...
        if let Some(request_type) = tc.set_request_type {
            let context =
                DecoderContext::new(builder.context.server_version, builder.context.time_zone)
...
async fn test_request_builder_send_raw() {
    ...
    let message =
        crate::messages::encode_protobuf_message(OutgoingMessages::RequestCurrentTime as i32, &[]);
...
async fn test_shared_request_builder_send_raw() {
    ...
    let message = crate::messages::encode_protobuf_message(
        OutgoingMessages::RequestManagedAccounts as i32,
        &[],
    );
...
async fn test_order_request_builder_send() {
    ...
    let message =
        crate::messages::encode_protobuf_message(OutgoingMessages::PlaceOrder as i32, &[]);
