...
fn test_is_connection_error() {
    ...
    let test_cases = vec![
        ...
        TestCase { name: "connection_reset", error: Error::ConnectionReset, expected: true },
        TestCase {
        ...
        TestCase { name: "cancelled_not_connection", error: Error::Cancelled, expected: false },
        TestCase {
...
fn test_is_timeout_error() {
    ...
    let test_cases = vec![
        ...
        TestCase { name: "non_io_error", error: Error::Cancelled, expected: false },
    ];
...
fn test_is_transient_error() {
    ...
    let test_cases = vec![
        ...
        TestCase { name: "fatal_error_not_transient", error: Error::Shutdown, expected: false },
    ];
...
fn test_is_fatal_error() {
    ...
    let test_cases = vec![
        TestCase { name: "shutdown", error: Error::Shutdown, expected: true },
        TestCase {
            name: "invalid_argument",
            error: Error::InvalidArgument("test".to_string()),
            expected: true,
        },
        TestCase { name: "not_implemented", error: Error::NotImplemented, expected: true },
        TestCase {
            name: "server_version",
            error: Error::ServerVersion(100, 90, "feature".to_string()),
            expected: true,
        },
        TestCase { name: "already_subscribed", error: Error::AlreadySubscribed, expected: true },
        TestCase {
            name: "connection_reset_not_fatal",
            error: Error::ConnectionReset,
            expected: false,
        },
        TestCase { name: "cancelled_not_fatal", error: Error::Cancelled, expected: false },
        TestCase {
...
fn test_error_message() {
    ...
    let test_cases = vec![
        ...
        TestCase { name: "shutdown", error: Error::Shutdown, expected: "Client is shutting down" },
        TestCase {
            name: "cancelled",
...
fn test_error_categorization() {
    ...
    let test_cases = vec![
        ...
        // Cancelled category
        TestCase { name: "cancelled", error: Error::Cancelled, expected: ErrorCategory::Cancelled },
        // Fatal category
        TestCase { name: "shutdown", error: Error::Shutdown, expected: ErrorCategory::Fatal },
        TestCase {
...
fn test_is_connection_io_error() {
    ...
    let test_cases = vec![
        ...
        TestCase { name: "unexpected_eof", error_kind: ErrorKind::UnexpectedEof, expected: true },
        TestCase { name: "broken_pipe", error_kind: ErrorKind::BrokenPipe, expected: true },
        TestCase {
...
fn test_is_timeout_io_error() {
    ...
    let test_cases = vec![
        TestCase { name: "would_block", error_kind: ErrorKind::WouldBlock, expected: true },
        TestCase { name: "timed_out", error_kind: ErrorKind::TimedOut, expected: true },
        TestCase {
            name: "interrupted_not_timeout",
            error_kind: ErrorKind::Interrupted,
            expected: false,
        },
        TestCase { name: "other_not_timeout", error_kind: ErrorKind::Other, expected: false },
    ];
