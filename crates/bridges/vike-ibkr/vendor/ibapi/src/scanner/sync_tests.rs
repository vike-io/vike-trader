...
use crate::testdata::builders::scanner::{
    cancel_scanner_subscription_request, scanner_data, scanner_data_row, scanner_parameters,
    scanner_parameters_request, scanner_subscription_request,
};
...
fn test_scanner_subscription() {
    ...
    let filter = vec![
        TagValue { tag: "scannerType".to_string(), value: "TOP_PERC_GAIN".to_string() },
        TagValue { tag: "numberOfRows".to_string(), value: "10".to_string() },
    ];
    ...
    let scanner_data: Vec<Vec<ScannerData>> =
        subscription.iter_data().map(|r| r.expect("error getting scanner results")).collect();
    ...
    assert_request(
        &message_bus,
        1,
        &cancel_scanner_subscription_request().request_id(TEST_REQ_ID_FIRST),
    );
}
