...
impl MessageRecorder {
    ...
    pub fn from_env() -> Self {
        match env::var("IBAPI_RECORDING_DIR") {
            Ok(dir) => {
                if dir.is_empty() {
                    MessageRecorder { enabled: false, recording_dir: String::from("") }
                } else {
                    let format = format_description!("[year]-[month]-[day]-[hour]-[minute]");
                    let now = OffsetDateTime::now_utc();
                    let instance_id = RECORDER_ID.fetch_add(1, Ordering::SeqCst);
                    let recording_dir =
                        format!("{}/{}-{}", dir, now.format(&format).unwrap(), instance_id);
            ...
            _ => MessageRecorder { enabled: false, recording_dir: String::from("") },
        }
    }
    ...
    pub fn record_response(&self, message: &ResponseMessage) {
        ...
        let record_id = RECORDING_SEQ.fetch_add(1, Ordering::SeqCst);
        if let Err(err) =
            fs::write(self.response_file(record_id), message.encode().replace('\0', "|"))
        {
