...
async fn test_shared_channel_fan_out_for_open_orders() {
    ...
    let mut sub_open =
        bus.send_shared_request(OutgoingMessages::RequestOpenOrders, vec![]).await.unwrap();
    let mut sub_all =
        bus.send_shared_request(OutgoingMessages::RequestAllOpenOrders, vec![]).await.unwrap();
    let mut sub_auto =
        bus.send_shared_request(OutgoingMessages::RequestAutoOpenOrders, vec![]).await.unwrap();
    ...
    stream.push_inbound(binary_proto(
        ...
        &crate::proto::OpenOrder { order_id: Some(42), ..Default::default() },
    ));
    bus.read_and_route_message().await.unwrap();
...
async fn test_shared_channel_routing_current_time() {
    ...
    let mut sub =
        bus.send_shared_request(OutgoingMessages::RequestCurrentTime, vec![]).await.unwrap();
...
async fn test_request_less_warning_does_not_fail_one_shot() {
    ...
    assert!(
        ...
        "warning must not fail a one-shot shared request"
    );
}
...
use crate::subscriptions::{
    DecoderContext, StreamDecoder, SubscriptionItem, SubscriptionItemStreamExt,
};
...
impl StreamDecoder<NoticeTestData> for NoticeTestData {
    fn decode(
        ...
        _msg: &mut ResponseMessage,
    ) -> Result<NoticeTestData, Error> {
        Ok(NoticeTestData)
    }
}

async fn make_request_subscription(
    request_id: i32,
) -> (MemoryStream, Arc<AsyncTcpMessageBus<MemoryStream>>, Subscription<NoticeTestData>) {
    let (stream, bus) = make_bus();
    let internal = bus.send_request(request_id, vec![]).await.unwrap();
    let sub = Subscription::new_from_internal::<NoticeTestData>(
        internal,
        bus.clone(),
        Some(request_id),
        None,
...
async fn make_order_subscription(
    order_id: i32,
) -> (MemoryStream, Arc<AsyncTcpMessageBus<MemoryStream>>, Subscription<NoticeTestData>) {
    let (stream, bus) = make_bus();
...
async fn next_item(
    sub: &mut Subscription<NoticeTestData>,
) -> Option<Result<SubscriptionItem<NoticeTestData>, Error>> {
    tokio::time::timeout(TICK, sub.next()).await.expect("subscription got no item before timeout")
}
...
async fn test_completed_order_routes_to_shared_channel() {
    let (stream, bus) = make_bus();
    let mut sub =
        bus.send_shared_request(OutgoingMessages::RequestCompletedOrders, vec![]).await.unwrap();
