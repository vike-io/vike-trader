...
impl Io for MockSocket {
    fn read_message(&self) -> Result<Vec<u8>, Error> {
        ...
        while self.read_call_count.load(Ordering::SeqCst)
            >= self.responses_len.load(Ordering::SeqCst)
        {
...
impl Exchange {
    fn new(request: Vec<u8>, responses: Vec<ResponseMessage>) -> Self {
        Self { request, responses: VecDeque::from(responses), is_handshake: false }
    }
...
fn managed_accounts_response(accounts: &str) -> ResponseMessage {
    use prost::Message;
    let bytes =
        crate::proto::ManagedAccounts { accounts_list: Some(accounts.to_string()) }.encode_to_vec();
...
fn test_bus_send_order_request() -> Result<(), Error> {
    ...
    let events = vec![
        ...
        Exchange::new(
            start_api_bytes,
            vec![managed_accounts_response("DU1234567"), next_valid_id_response(5)],
        ),
        Exchange::new(
            request.clone(),
            vec![
    ...
    let connection = Connection::with_socket(
        stream,
        28,
        None,
        std::sync::Arc::new(crate::transport::sync::NoticeBroadcaster::new()),
    );
...
fn test_dispatcher_thread_exits_promptly_on_shutdown() {
    ...
    assert!(
        elapsed < Duration::from_millis(500),
        "ensure_shutdown took {elapsed:?}, expected <500ms"
    );
}
...
impl crate::subscriptions::StreamDecoder<NoticeTestData> for NoticeTestData {
    fn decode(
        _context: &crate::subscriptions::DecoderContext,
...
fn wrap_subscription(
    ...
    crate::subscriptions::sync::Subscription::new(
        bus,
        internal,
        crate::subscriptions::DecoderContext::default(),
    )
}
...
fn test_cleanup_thread_processes_drop_signals() -> Result<(), Error> {
    ...
    while Instant::now() < deadline
        && (bus.requests.contains(&42)
            || bus.orders.contains(&99)
...
fn test_warning_with_orphan_request_id_logs() -> Result<(), Error> {
    ...
    assert!(
        notice_stream.try_next().is_none(),
        "global notice stream got a routed-but-orphan notice"
    );
    Ok(())
}
...
fn test_reset_notifies_all_channel_categories() -> Result<(), Error> {
    ...
    for (name, sub) in [("request", &req), ("order", &order), ("shared", &shared)] {
        let resp =
            sub.next_timeout(TICK).unwrap_or_else(|| panic!("{name} sub got no notification"));
