...
use crate::messages::{
    shared_channel_configuration, IncomingMessages, OutgoingMessages, ResponseMessage,
};
use crate::Error;

use super::common::log_orphan;
use super::routing::{
    classify_error, determine_routing, order_routing_strategy, DecodedError, ErrorDisposition,
    OrderRoutingStrategy, RoutingDecision,
};
...
pub trait AsyncMessageBus: Send + Sync {
    ...
    #[allow(dead_code)]
    async fn cancel_order_subscription(&self, order_id: i32, message: Vec<u8>)
        -> Result<(), Error>;
...
pub struct AsyncTcpMessageBus<S: AsyncStream = AsyncTcpSocket> {
    ...
    shared_channel_receivers:
        Arc<RwLock<HashMap<OutgoingMessages, broadcast::Receiver<RoutedItem>>>>,
...
impl<S: AsyncStream> AsyncTcpMessageBus<S> {
            ...
            while let Some(signal) = receiver.recv().await {
                match signal {
                    ...
                    CleanupSignal::Shared(message_type) => {
                        ...
                        debug!(
                            "Subscription for shared channel {:?} ended (channel remains active)",
                            message_type
                        );
                    }
    ...
    async fn deliver_to_request_id(
        &self,
        request_id: i32,
        item: RoutedItem,
        sent_to_update_stream: bool,
    ) {
        {
            let channels = self.request_channels.read().await;
    ...
    async fn route_to_request_channel(
        &self,
        request_id: i32,
        message: ResponseMessage,
    ) -> Result<(), Error> {
    ...
    async fn route_to_order_channel(
        &self,
        order_id: i32,
        message: ResponseMessage,
    ) -> Result<(), Error> {
    ...
    async fn route_to_shared_channel(
        &self,
        message_type: IncomingMessages,
        message: ResponseMessage,
    ) -> Result<(), Error> {
...
impl<S: AsyncStream> AsyncMessageBus for AsyncTcpMessageBus<S> {
    ...
    async fn cancel_order_subscription(
        &self,
        order_id: i32,
        message: Vec<u8>,
    ) -> Result<(), Error> {
    ...
    fn notice_subscribe(&self) -> crate::subscriptions::notice_stream::async_impl::NoticeStream {
        crate::subscriptions::notice_stream::async_impl::NoticeStream::new(
            self.connection.notice_sender.subscribe(),
        )
    }
