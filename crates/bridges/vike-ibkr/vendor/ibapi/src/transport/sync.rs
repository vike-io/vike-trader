...
use crate::messages::{
    shared_channel_configuration, IncomingMessages, Notice, OutgoingMessages, ResponseMessage,
};
...
impl SharedChannels {
    ...
    pub fn new() -> Self {
        let mut instance = Self { senders: HashMap::new(), receivers: HashMap::new() };
...
impl<S: Stream> MessageBus for TcpMessageBus<S> {
    ...
    fn cancel_shared_subscription(
        &self,
        _message_type: OutgoingMessages,
        message: &[u8],
    ) -> Result<(), Error> {
        self.connection.write_message(message)?;
        // TODO send cancel
