...
fn empty_ctx<'a>() -> StartupHandshakeContext<'a> {
    ...
    StartupHandshakeContext { startup: None, notice_sink: &SINK }
}

fn startup_ctx<'a>(cb: &'a (dyn Fn(StartupMessage) + Send + Sync)) -> StartupHandshakeContext<'a> {
    static SINK: DiscardingSink = DiscardingSink;
    StartupHandshakeContext { startup: Some(cb), notice_sink: &SINK }
}
...
fn full_ctx<'a>(
    cb: &'a (dyn Fn(StartupMessage) + Send + Sync),
    sink: &'a CapturingSink,
) -> StartupHandshakeContext<'a> {
    StartupHandshakeContext { startup: Some(cb), notice_sink: sink }
}
...
fn test_dispatch_unsolicited_order_status_decode_failure_emits_notice() {
    ...
    assert_eq!(notice.code, HANDSHAKE_DECODE_FAILURE_CODE);
    assert!(
        notice.message.contains("OrderStatus"),
...
fn test_dispatch_unsolicited_notice_only_fires_notice_sink() {
    ...
    dispatch_unsolicited_message(
        TEST_SERVER_VERSION,
        &mut message,
        &StartupHandshakeContext { startup: Some(&startup_cb), notice_sink: &sink },
    );
...
fn test_parse_account_info_callback_not_invoked_for_managed_accounts() {
    ...
    let handler = ConnectionHandler::default();
    let bytes =
        crate::proto::ManagedAccounts { accounts_list: Some("DU123".to_string()) }.encode_to_vec();
...
fn test_require_protobuf_support_accepts_minimum() {
    require_protobuf_support(server_versions::PROTOBUF_REST_MESSAGES_3)
        .expect("floor version must be accepted");
}

#[test]
fn test_require_protobuf_support_accepts_newer() {
    require_protobuf_support(server_versions::PROTOBUF_REST_MESSAGES_3 + 5)
        .expect("newer versions must be accepted");
}
...
fn test_require_protobuf_support_rejects_older() {
    ...
    let expected_required =
        format!("server version {} required", server_versions::PROTOBUF_REST_MESSAGES_3);
...
fn test_require_protobuf_support_rejects_previous_scan_data_floor() {
    ...
    let err =
        require_protobuf_support(previous_floor).expect_err("previous floor must now be rejected");
    match err {
...
fn test_non_utf8_handshake_response() {
    ...
    let gb2312_bytes: Vec<u8> = vec![
        ...
        214, 208, 185, 250, 177, 234, 215, 188, 202, 177, 188,
        228, // GB2312: 中国标准时间
        0,   // null terminator
    ];
...
fn test_startup_message_message_type_typed_variants() {
    ...
    assert_eq!(
        StartupMessage::AccountUpdate(AccountUpdate::AccountValue(AccountValue::default()))
            .message_type(),
        IncomingMessages::AccountValue
    );
    assert_eq!(
        StartupMessage::AccountUpdate(AccountUpdate::PortfolioValue(
            AccountPortfolioValue::default()
        ))
        .message_type(),
        IncomingMessages::PortfolioValue
    );
    assert_eq!(
        StartupMessage::AccountUpdate(AccountUpdate::UpdateTime(AccountUpdateTime::default()))
            .message_type(),
    ...
    assert_eq!(
        StartupMessage::CompletedOrdersEnd.message_type(),
...
fn test_parse_account_info_managed_accounts_protobuf() {
    ...
    let handler = ConnectionHandler::default();
    let proto = crate::proto::ManagedAccounts { accounts_list: Some("DU111,DU222".to_string()) };
    let bytes = proto.encode_to_vec();
    let mut message = proto_response(IncomingMessages::ManagedAccounts, bytes);
...
fn proto_frame<B: crate::testdata::builders::ResponseProtoEncoder>(
