...
use super::common::{
    parse_connection_time, parse_raw_message, require_protobuf_support, AccountInfo,
    ConnectionHandler, ConnectionProtocol, StartupHandshakeContext, StartupMessage,
};
...
impl<S: AsyncStream> AsyncConnection<S> {
    pub(crate) fn with_socket(
        ...
        Self {
            client_id,
            socket,
            connection_metadata: Mutex::new(ConnectionMetadata { client_id, ..Default::default() }),
    ...
    pub(crate) fn stubbed(socket: S, client_id: i32) -> Self {
        let (notice_sender, _) =
            broadcast::channel(crate::transport::r#async::BROADCAST_CHANNEL_CAPACITY);
    ...
    async fn reset_connection_metadata(&self) {
        ...
        *connection_metadata =
            ConnectionMetadata { client_id: self.client_id, ..Default::default() };
    }
    ...
    pub(crate) async fn handshake(&self) -> Result<(), Error> {
        ...
        match ack {
            Ok(mut response) => {
                let handshake_data =
                    self.connection_handler.parse_handshake_response(&mut response)?;
    ...
    pub(crate) async fn receive_account_info(&self) -> Result<(), Error> {
        ...
        loop {
            ...
            let info =
                self.connection_handler.parse_account_info(server_version, &mut message, &ctx)?;
            ...
            attempts += 1;
            if (account_info.next_order_id.is_some() && account_info.managed_accounts.is_some())
