...
use crate::messages::{
    encode_length, encode_protobuf_message, IncomingMessages, Notice, OutgoingMessages,
...
pub trait ConnectionProtocol {
    ...
    ) -> Result<HandshakeData, Self::Error>;
...
pub(crate) fn dispatch_unsolicited_message(
    _server_version: i32,
    message: &mut ResponseMessage,
    ctx: &StartupHandshakeContext<'_>,
) {
    use crate::accounts::common::decode_account_update_message;
    use crate::orders::common::{
        decode_commission_report, decode_completed_order, decode_execution_data, decode_open_order,
        decode_order_status,
    };
    ...
    match kind {
        ...
        IncomingMessages::OpenOrder => {
            dispatch_typed(ctx, kind, || decode_open_order(message), StartupMessage::OpenOrder)
        }
        IncomingMessages::OrderStatus => {
            dispatch_typed(ctx, kind, || decode_order_status(message), StartupMessage::OrderStatus)
        }
        ...
        | IncomingMessages::AccountDownloadEnd => dispatch_typed(
            ctx,
            kind,
            || decode_account_update_message(message),
            StartupMessage::AccountUpdate,
        ),
        IncomingMessages::ExecutionData => {
            dispatch_typed(ctx, kind, || decode_execution_data(message), StartupMessage::Execution)
        }
        IncomingMessages::CommissionsReport => dispatch_typed(
            ctx,
            kind,
            || decode_commission_report(message),
            StartupMessage::CommissionReport,
        ),
        IncomingMessages::CompletedOrder => dispatch_typed(
            ctx,
            kind,
            || decode_completed_order(message),
            StartupMessage::CompletedOrder,
        ),
        IncomingMessages::ExecutionDataEnd => dispatch_unit(ctx, StartupMessage::ExecutionDataEnd),
        IncomingMessages::CompletedOrdersEnd => {
            dispatch_unit(ctx, StartupMessage::CompletedOrdersEnd)
        }
        _ => {
...
pub fn parse_connection_time(
    connection_time: &str,
) -> Result<(Option<OffsetDateTime>, Option<&'static Tz>), Error> {
...
pub fn parse_raw_message(data: &[u8]) -> (ResponseMessage, Option<String>) {
    ...
    if msg_id > PROTOBUF_MSG_ID {
        ...
        let message = ResponseMessage { i: 0, fields, raw_bytes: None };
