...
use crate::common::test_utils::helpers::{
    error_frame, managed_accounts_frame, next_valid_id_frame,
};
...
async fn make_client() -> Client {
    ...
    bus.clone().process_messages(server_version, Duration::from_secs(0)).expect("process_messages");
...
async fn handshake_callbacks_and_notice_stream_survive_reconnect() {
    ...
    connection.startup_callback =
        Some(Arc::new(move |_msg: crate::connection::common::StartupMessage| {
    ...
    assert_eq!(
        *startup_count.lock().unwrap(),
        1,
        "startup callback should fire on first handshake"
    );
    ...
    assert_eq!(
        *startup_count.lock().unwrap(),
        2,
        "startup callback should fire on reconnect handshake"
    );
