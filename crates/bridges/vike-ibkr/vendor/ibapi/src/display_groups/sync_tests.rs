...
fn stubbed_subscription(
    responses: Vec<crate::messages::ResponseMessage>,
) -> (Arc<MessageBusStub>, DisplayGroupSubscription) {
...
fn assert_first_data_eq(
    item: Option<Result<SubscriptionItem<DisplayGroupUpdate>, Error>>,
    expected_contract_info: &str,
) {
...
fn test_update_display_group() {
    ...
    let (message_bus, subscription) =
        stubbed_subscription(vec![display_group_update_response("265598@SMART")]);
