...
fn test_decode_display_group_update() {
    ...
    let result =
        DisplayGroupUpdate::decode(&test_context(), &mut message).expect("decoding failed");
...
fn test_cancel_message() {
    ...
    let message =
        DisplayGroupUpdate::cancel_message(176, Some(9000), None).expect("cancel message failed");
