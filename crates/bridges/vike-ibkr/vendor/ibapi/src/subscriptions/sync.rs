...
impl<T: StreamDecoder<T>> Subscription<T> {
    ...
    pub fn cancel(&self) {
        ...
        if let Some(request_id) = self.request_id {
            if let Ok(message) =
                T::cancel_message(self.context.server_version, self.request_id, Some(&self.context))
            {
    ...
    fn handle_response(
        &self,
        response: Option<RoutedItem>,
    ) -> NextAction<Result<SubscriptionItem<T>, Error>> {
        match response {
    ...
    pub fn timeout_iter_data(
        &self,
        timeout: Duration,
    ) -> FilterData<SubscriptionTimeoutIter<'_, T>> {
