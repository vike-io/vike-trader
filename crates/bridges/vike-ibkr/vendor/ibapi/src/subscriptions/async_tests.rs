...
async fn test_subscription_with_decoder() {
    ...
    // Send a test message
    let msg = ResponseMessage::from(
        "1\09000\020241231 12:00:00\0100.5\0101.0\0100.0\0100.25\01000\0100.2\05\00",
    );
...
async fn test_subscription_new_from_internal_simple() {
    ...
    let subscription: Subscription<TestItem> = Subscription::new_from_internal_simple::<TestItem>(
        internal,
        message_bus,
        DecoderContext::default(),
    );
...
fn collect_subscription(
    items: Vec<RoutedItem>,
    keep_open: bool,
) -> (Subscription<CollectItem>, Option<broadcast::Sender<RoutedItem>>) {
    let message_bus = Arc::new(MessageBusStub::default());
    let (tx, rx) = broadcast::channel(100);
    let internal = AsyncInternalSubscription::new(rx);
    for item in items {
    ...
    let sub = Subscription::<CollectItem>::with_decoder(
        internal,
        message_bus,
        CollectItem::decode,
        None,
...
async fn test_collect_for_stops_at_snapshot_end() {
    let (mut sub, _keep) = collect_subscription(
        vec![collect_data(10), collect_data(20), collect_data(-1), collect_data(30)],
        true,
    );
...
async fn test_collect_until_stops_on_predicate() {
    let (mut sub, _keep) = collect_subscription(
        vec![collect_data(10), collect_data(20), collect_data(30), collect_data(40)],
        true,
    );
...
async fn test_collect_for_returns_prefix_on_terminal_error() {
    let (mut sub, _keep) = collect_subscription(
        vec![collect_data(10), RoutedItem::Error(Error::ConnectionReset), collect_data(20)],
        true,
    );
...
async fn test_collect_for_drains_to_stream_end() {
    let (mut sub, _keep) =
        collect_subscription(vec![collect_data(10), collect_data(20), collect_data(30)], false);
...
async fn test_collect_for_filters_notices() {
    ...
    let (mut sub, _keep) =
        collect_subscription(vec![collect_data(10), notice, collect_data(20)], false);
