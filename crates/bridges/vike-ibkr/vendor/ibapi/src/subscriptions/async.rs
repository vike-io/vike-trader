...
use super::common::{
    filter_notice, process_decode_result, DecoderContext, ProcessingResult, RoutedItem,
    SubscriptionItem,
};
...
type CancelFn =
    Box<dyn Fn(i32, Option<i32>, Option<&DecoderContext>) -> Result<Vec<u8>, Error> + Send + Sync>;
type DecoderFn<T> =
    Arc<dyn Fn(&DecoderContext, &mut ResponseMessage) -> Result<T, Error> + Send + Sync>;
...
enum SubscriptionInner<T> {
    ...
    WithDecoder { subscription: AsyncInternalSubscription, decoder: DecoderFn<T> },
...
impl<T> Subscription<T> {
    ...
    {
        let mut sub =
            Self::with_decoder(internal, message_bus, D::decode, request_id, order_id, context);
        ...
        loop {
            match tokio::time::timeout_at(deadline, self.next()).await {
                ...
                Ok(Some(Ok(SubscriptionItem::Notice(notice)))) => {
                    warn!("ib notice on subscription: {notice}")
                }
                Ok(Some(Err(e))) => {
...
impl<T: Send + 'static> Stream for Subscription<T> {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let Subscription { inner, context, stream_ended, snapshot_ended, snapshot_end_fn, .. } =
            this;
        loop {
            match inner {
                SubscriptionInner::WithDecoder { subscription, decoder } => {
                ...
                SubscriptionInner::PreDecoded { receiver } => {
                    return match receiver.poll_recv(cx) {
                        Poll::Ready(Some(Ok(t))) => {
                            Poll::Ready(Some(Ok(SubscriptionItem::Data(t))))
                        }
