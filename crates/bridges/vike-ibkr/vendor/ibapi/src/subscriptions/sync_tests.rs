...
fn test_routed_item_error_terminates_subscription() {
    ...
    let internal =
        SubscriptionBuilder::new().receiver(receiver).signaler(signaler).request_id(1).build();
...
fn collect_subscription(
    ...
    keep_open: bool,
) -> (Subscription<CollectItem>, Option<channel::Sender<RoutedItem>>) {
    let (sender, receiver) = channel::unbounded::<RoutedItem>();
...
fn test_collect_for_returns_prefix_on_terminal_error() {
    ...
    let (sub, _keep) = collect_subscription(
        vec![data(10), RoutedItem::Error(Error::ConnectionReset), data(20)],
        true,
    );
