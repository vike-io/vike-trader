...
fn test_assert_request_msg_id() {
...

    {
        ...
        request_messages
            .push(encode_protobuf_message(OutgoingMessages::RequestAccountSummary as i32, &[]));
    }
...
fn assert_request_proto_matches_expected_body() {
...

    {
        let mut request_messages = message_bus.request_messages.write().unwrap();
        request_messages
            .push(encode_protobuf_message(OutgoingMessages::RequestManagedAccounts as i32, &body));
    }
...
fn test_create_test_client_with_responses_and_version() {
    ...
    let (client, message_bus) =
        create_test_client_with_responses_and_version(responses.clone(), custom_version);
    assert_eq!(client.server_version(), custom_version);
    assert_eq!(message_bus.response_messages, responses);
}
...
fn assert_request_proto_panics_on_body_mismatch() {
    ...
    let on_wire = AccountSummaryRequest { req_id: Some(7), ..Default::default() };
    let mut body = Vec::new();
...

    {
        let mut request_messages = message_bus.request_messages.write().unwrap();
        request_messages
            .push(encode_protobuf_message(OutgoingMessages::RequestAccountSummary as i32, &body));
    }

    let expected = AccountSummaryRequest { req_id: Some(99), ..Default::default() };
