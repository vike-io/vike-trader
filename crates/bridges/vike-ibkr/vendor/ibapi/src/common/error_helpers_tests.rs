...
fn test_require_with_none_value() {
    ...
    assert!(result.is_err());
    assert!(
        matches!(result.unwrap_err(), Error::InvalidArgument(msg) if msg == "Custom error message")
    );
}
...
fn test_require_request_id_none() {
    ...
    assert!(result.is_err());
    assert!(
        matches!(result.unwrap_err(), Error::InvalidArgument(msg) if msg == "Request ID required")
    );
}
...
fn test_require_request_id_for_none() {
    ...
    assert!(result.is_err());
    assert!(
        matches!(result.unwrap_err(), Error::InvalidArgument(msg) if msg == "Request ID required to cancel subscription")
    );
}
...
fn test_require_range_too_low() {
    ...
    assert!(result.is_err());
    assert!(
        matches!(result.unwrap_err(), Error::InvalidArgument(msg) if msg == "value must be between 1 and 10, got 0")
    );
}
...
fn test_require_range_too_high() {
    ...
    assert!(result.is_err());
    assert!(
        matches!(result.unwrap_err(), Error::InvalidArgument(msg) if msg == "value must be between 1 and 10, got 15")
    );
}
...
fn test_require_not_empty_invalid() {
    ...
    assert!(result.is_err());
    assert!(
        matches!(result.unwrap_err(), Error::InvalidArgument(msg) if msg == "name cannot be empty")
    );
}
...
fn test_require_not_empty_vec_invalid() {
    ...
    assert!(result.is_err());
    assert!(
        matches!(result.unwrap_err(), Error::InvalidArgument(msg) if msg == "numbers must contain at least one element")
    );
}
...
fn test_map_error_err() {
    ...
    assert!(result.is_err());
    assert!(
        matches!(result.unwrap_err(), Error::InvalidArgument(msg) if msg == "Failed to process: internal error")
    );
}
...
fn test_map_error_with_err() {
    ...
    assert!(result.is_err());
    assert!(
        matches!(result.unwrap_err(), Error::InvalidArgument(msg) if msg == "Could not find resource: not found")
    );
}
