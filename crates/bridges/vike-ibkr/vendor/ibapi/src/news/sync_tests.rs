...
use crate::common::test_utils::helpers::{
    assert_request, proto_response, TEST_CONTRACT_ID, TEST_REQ_ID_FIRST,
};
...
use crate::testdata::builders::news::{
    historical_news, historical_news_end, historical_news_request, news_article,
    news_article_request, news_bulletin, news_bulletins_request, news_providers,
...
fn test_news_bulletins() {
    ...
    let bulletin =
        subscription.next_data().expect("expected news bulletin").expect("subscription error");
...
fn test_historical_news() {
    ...
    let article =
        subscription.next_data().expect("expected news article").expect("subscription error");
...
fn test_contract_news() {
    ...
    let subscription =
        client.contract_news(&contract, &["BZ", "DJ"]).expect("request contract news failed");
