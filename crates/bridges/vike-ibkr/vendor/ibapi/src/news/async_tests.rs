use crate::common::test_utils::helpers::{
    assert_request, assert_request_msg_id, proto_response, request_message_count, TEST_CONTRACT_ID,
    TEST_REQ_ID_FIRST,
};
...
use crate::testdata::builders::news::{
    cancel_news_bulletins_request, historical_news, historical_news_end, historical_news_request,
    news_article, news_article_request, news_bulletin, news_bulletins_request, news_providers,
    news_providers_request, tick_news,
};
...
async fn test_news_bulletins() {
    ...
    let mut subscription =
        client.news_bulletins(true).await.expect("request news bulletins failed");
...
async fn test_contract_news() {
    ...
    let mut subscription =
        client.contract_news(&contract, &["BZ", "DJ"]).await.expect("request contract news failed");
...
async fn test_broad_tape_news() {
    ...
    let mut subscription =
        client.broad_tape_news("BZ").await.expect("request broad tape news failed");
