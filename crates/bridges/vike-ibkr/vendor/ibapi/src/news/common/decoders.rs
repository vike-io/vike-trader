...
pub(in crate::news) fn decode_news_providers(
    message: &ResponseMessage,
) -> Result<Vec<NewsProvider>, Error> {
...
pub(in crate::news) fn decode_news_bulletin(
    message: &ResponseMessage,
) -> Result<NewsBulletin, Error> {
    decode_news_bulletin_proto(message.require_proto()?)
}

pub(in crate::news) fn decode_historical_news(
    message: &ResponseMessage,
) -> Result<NewsArticle, Error> {
...
pub(in crate::news) fn decode_news_article(
    message: &ResponseMessage,
...
pub(crate) fn decode_tick_news_proto(bytes: &[u8]) -> Result<NewsArticle, Error> {
    ...
    let time = OffsetDateTime::from_unix_timestamp_nanos((millis as i128) * 1_000_000)
...
pub(crate) fn decode_historical_news_proto(bytes: &[u8]) -> Result<NewsArticle, Error> {
    ...
    let time =
        p.time.as_deref().and_then(try_parse_time_as_utc).unwrap_or(OffsetDateTime::UNIX_EPOCH);
