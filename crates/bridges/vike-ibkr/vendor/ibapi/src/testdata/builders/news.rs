...
impl RequestEncoder for NewsBulletinsRequestBuilder {
    ...
    fn to_proto(&self) -> Self::Proto {
        proto::NewsBulletinsRequest { all_messages: some_bool(self.all_messages) }
    }
}
...
impl NewsProvidersResponse {
    pub fn provider(mut self, code: impl Into<String>, name: impl Into<String>) -> Self {
        self.providers.push(NewsProviderEntry { code: code.into(), name: name.into() });
        self
    }
}
...
impl ResponseProtoEncoder for HistoricalNewsEndResponse {
    ...
    fn to_proto(&self) -> Self::Proto {
        proto::HistoricalNewsEnd { req_id: Some(self.request_id), has_more: None }
    }
}
...
impl Default for NewsArticleResponse {
    fn default() -> Self {
        Self { request_id: TEST_REQ_ID_FIRST, article_type: 0, article_text: String::new() }
    }
}
