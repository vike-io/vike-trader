...
use crate::common::test_utils::helpers::{
    assert_request, request_message_count, text_response, TEST_REQ_ID_FIRST,
};
...
use crate::testdata::builders::contracts::{
    calculate_implied_volatility_request, calculate_option_price_request,
    cancel_contract_data_request, contract_data_request, market_rule_request,
...
fn test_contract_details() {
    for test_case in contract_details_test_cases() {
        let message_bus =
            Arc::new(MessageBusStub::with_ordered_responses(test_case.ordered_responses.clone()));

        let client = Client::stubbed(message_bus.clone(), server_versions::SIZE_RULES);
        let result = client.contract_details(&test_case.contract);

        assert_eq!(
            request_message_count(&message_bus),
            1,
            "Test '{}' request count",
            test_case.name
        );
...
fn test_market_rule() {
    for test_case in market_rule_test_cases() {
        ...
        assert_request(
            &message_bus,
            0,
            &market_rule_request().market_rule_id(test_case.market_rule_id),
        );
...
fn test_option_calculations() {
    for test_case in option_calculation_test_cases() {
        ...
        let result = if let Some(volatility) = test_case.volatility {
            let res = client.calculate_option_price(
                &test_case.contract,
                volatility,
                test_case.underlying_price,
            );
            assert_request(
                &message_bus,
                0,
            ...
            let res = client.calculate_implied_volatility(
                &test_case.contract,
                option_price,
                test_case.underlying_price,
            );
    ...
    for test_case in stream_decoder_test_cases() {
        ...
        match &test_case.expected_result {
            StreamDecoderResult::OptionComputation { price, delta } => {
                let result = OptionComputation::decode(
                    &DecoderContext::new(server_versions::SIZE_RULES, None),
                    &mut message,
                );
                assert!(result.is_ok(), "Test '{}' failed: {:?}", test_case.name, result.err());
                let computation = result.unwrap();
            ...
            StreamDecoderResult::Error(expected_error) => {
                if test_case.name == "option chain end of stream" {
                    ...
                    let opt_result = OptionComputation::decode(
                        &DecoderContext::new(server_versions::SIZE_RULES, None),
                        &mut message.clone(),
                    );
                    let chain_result = OptionChain::decode(
                        &DecoderContext::new(server_versions::SIZE_RULES, None),
                        &mut message,
                    );
                    assert!(
...
fn test_cancel_messages() {
    for test_case in cancel_message_test_cases() {
        let context =
            test_case.request_type.map(|rt| DecoderContext::default().with_request_type(rt));

        let result = match test_case.decoder_type {
            "OptionComputation" => OptionComputation::cancel_message(
...
fn test_client_methods() {
    for test_case in client_method_test_cases() {
        ...
        let client = Client::stubbed(
            ...
            match &test_case.test_type {
                ClientMethodTest::CalculateOptionPrice { .. } => {
                    server_versions::REQ_CALC_OPTION_PRICE
                }
        ...
        let result = match &test_case.test_type {
            ClientMethodTest::CalculateOptionPrice { contract, volatility, underlying_price } => {
        ...
        let computation = result.unwrap();
        match &test_case.expected_result {
            ClientMethodResult::OptionComputation { option_price, implied_volatility } => {
                assert_eq!(
                    computation.option_price, *option_price,
                    "Test '{}' option price mismatch",
                    test_case.name
                );
                assert_eq!(
...
fn test_matching_symbols_rejects_unexpected_message() {
    let message_bus =
        Arc::new(MessageBusStub::with_ordered_responses(vec![text_response("10|9000|")]));
    let client = Client::stubbed(message_bus, server_versions::BOND_ISSUERID);
