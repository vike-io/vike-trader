...
fn test_security_type_from() {
    ...
    assert_eq!(
        SecurityType::from("FOP"),
        SecurityType::FuturesOption,
        "FOP should be FuturesOption"
    );
    ...
    match SecurityType::from("UNKNOWN") {
        SecurityType::Other(name) => {
            assert_eq!(name, "UNKNOWN", "Other should contain original string")
        }
...
fn test_tag_value_to_field() {
    ...
    let tag_values = vec![
        TagValue { tag: "TAG1".to_string(), value: "VALUE1".to_string() },
        TagValue { tag: "TAG2".to_string(), value: "VALUE2".to_string() },
        TagValue { tag: "TAG3".to_string(), value: "VALUE3".to_string() },
    ];
    ...
    let single_tag_value =
        vec![TagValue { tag: "SINGLE_TAG".to_string(), value: "SINGLE_VALUE".to_string() }];
    ...
    let empty_fields = vec![TagValue { tag: "".to_string(), value: "".to_string() }];
...
fn test_is_bag() {
    ...
    let spread_contract = Contract { security_type: SecurityType::Spread, ..Default::default() };
    assert!(spread_contract.is_bag(), "Spread contract should be a bag");

    // Test with an explicitly set BAG security type
    let bag_contract = Contract { security_type: SecurityType::from("BAG"), ..Default::default() };
