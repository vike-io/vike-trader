...
fn test_futures_multiplier() {
    ...
    let custom =
        Contract::futures("ES").expires_in(ContractMonth::new(2024, 3)).multiplier(50).build();
    assert_eq!(custom.multiplier, "50");

    let cl =
        Contract::futures("CL").expires_in(ContractMonth::new(2024, 3)).multiplier(1000).build();
