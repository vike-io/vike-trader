...
use crate::common::test_utils::helpers::{
    assert_request, assert_tws_error_message, proto_error_response, proto_response,
    request_message_count, text_response, TEST_REQ_ID_FIRST,
};
...
use crate::testdata::builders::contracts::{
    ...
    cancel_contract_data_request, contract_data, contract_data_request, market_rule_request,
...
async fn contract_details_rejects_unexpected_message() {
    let message_bus =
        Arc::new(MessageBusStub::with_ordered_responses(vec![text_response("79|9000|0|")]));
    let client = Client::stubbed(message_bus, server_versions::SIZE_RULES);
