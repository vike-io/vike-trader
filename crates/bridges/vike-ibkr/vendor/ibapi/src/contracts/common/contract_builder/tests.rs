...
fn test_contract_builder_build_stock_success() {
    let contract =
        ContractBuilder::stock("AAPL", "NASDAQ", "USD").contract_id(12345).build().unwrap();
...
fn test_contract_builder_delta_neutral_contract() {
    let delta_neutral = DeltaNeutralContract { contract_id: 12345, delta: 0.5, price: 100.0 };
...
fn setter_parity_with_contract_fields() {
    ...
    let built = ContractBuilder::new()
        ...
        .delta_neutral_contract(DeltaNeutralContract { contract_id: 7, delta: 0.5, price: 100.0 })
