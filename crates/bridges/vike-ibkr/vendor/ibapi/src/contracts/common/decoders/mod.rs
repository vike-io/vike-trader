...
use crate::contracts::{
    ContractDescription, ContractDetails, MarketRule, OptionChain, SmartComponent,
};
...
pub(in crate::contracts) fn decode_contract_details(
    _server_version: i32,
...
pub(in crate::contracts) fn decode_market_rule(
    message: &mut ResponseMessage,
) -> Result<MarketRule, Error> {
    decode_market_rule_proto(message.require_proto()?)
}

pub(in crate::contracts) fn decode_option_chain(
    message: &mut ResponseMessage,
    ...
    p.contract_descriptions
        ...
        .map(|d| {
            let contract = d
                ...
                .transpose()?
                .unwrap_or_default();
            Ok(ContractDescription { contract, derivative_security_types: d.derivative_sec_types })
        })
        .collect()
}
...
pub(crate) fn decode_smart_components(
    message: &ResponseMessage,
) -> Result<Vec<SmartComponent>, Error> {
...
pub(in crate::contracts) fn decode_smart_components_message(
    message: &ResponseMessage,
) -> Result<Vec<SmartComponent>, Error> {
