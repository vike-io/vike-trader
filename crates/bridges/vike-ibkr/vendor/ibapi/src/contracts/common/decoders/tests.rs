...
fn test_decode_market_rule_proto() {
    let proto_msg = crate::proto::MarketRule {
        ...
        price_increments: vec![
            crate::proto::PriceIncrement { low_edge: Some(0.0), increment: Some(0.01) },
            crate::proto::PriceIncrement { low_edge: Some(1000.0), increment: Some(0.05) },
        ],
    };
    let mut bytes = Vec::new();
    proto_msg.encode(&mut bytes).unwrap();
...
fn test_decode_contract_details_rejects_text_framing() {
    ...
    let err = decode_contract_details(server_versions::PROTOBUF_REST_MESSAGES_3, &mut message)
        .expect_err("text framing must be rejected");
    assert!(
...
fn test_decode_contract_descriptions_rejects_text_framing() {
    let mut message =
        ResponseMessage::from("79\09000\01\012345\0AAPL\0STK\0NASDAQ\0USD\00\0APPLE INC\0\0");
    let err = decode_contract_descriptions(server_versions::PROTOBUF_REST_MESSAGES_3, &mut message)
        .expect_err("text framing must be rejected");
    assert!(
        matches!(err, Error::UnexpectedResponse(_)),
        "expected Error::UnexpectedResponse, got {err:?}"
...
fn test_decode_option_chain_rejects_text_framing() {
    let mut message =
        ResponseMessage::from("75\09000\0SMART\0265598\0AAPL\0100\01\020260619\01\0150.0\0");
