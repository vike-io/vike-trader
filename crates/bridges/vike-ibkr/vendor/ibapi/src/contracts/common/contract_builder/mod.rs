use super::super::{
    ComboLeg, Contract, Currency, DeltaNeutralContract, Exchange, OptionRight, SecurityIdType,
    SecurityType, Symbol,
};
use crate::Error;
    ...
    pub fn build(self) -> Result<Contract, Error> {
        ...
        if self.symbol.is_none() && self.local_symbol.is_none() && self.contract_id.is_none() {
            return Err(Error::InvalidArgument(
                "Symbol, local_symbol, or contract_id is required".into(),
            ));
        }
        ...
        if security_type == SecurityType::Option || security_type == SecurityType::FuturesOption {
            ...
            if self.last_trade_date_or_contract_month.is_none() {
                return Err(Error::InvalidArgument(
                    "Expiration date is required for options".into(),
                ));
            }
        }
        ...
        if (security_type == SecurityType::Future || security_type == SecurityType::FuturesOption)
            && self.last_trade_date_or_contract_month.is_none()
        {
