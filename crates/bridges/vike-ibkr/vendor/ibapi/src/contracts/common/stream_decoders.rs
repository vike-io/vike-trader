...
impl StreamDecoder<OptionComputation> for OptionComputation {
    const RESPONSE_MESSAGE_IDS: &'static [IncomingMessages] =
        &[IncomingMessages::TickOptionComputation, IncomingMessages::Error];
    ...
    fn decode(_context: &DecoderContext, message: &mut ResponseMessage) -> Result<Self, Error> {
        match message.message_type() {
            IncomingMessages::TickOptionComputation => {
                decoders::decode_tick_option_computation(message)
            }
    ...
    fn cancel_message(
        ...
        match context.and_then(|c| c.request_type) {
            Some(OutgoingMessages::ReqCalcImpliedVolat) => {
                encoders::encode_cancel_option_computation(
...
impl StreamDecoder<OptionChain> for OptionChain {
    ...
    fn decode(
        ...
        match message.message_type() {
            IncomingMessages::SecurityDefinitionOptionParameter => {
                Ok(decoders::decode_option_chain(message)?)
            }
