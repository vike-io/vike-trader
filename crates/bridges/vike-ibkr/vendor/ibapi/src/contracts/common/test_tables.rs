...
use crate::contracts::{
    Contract, ContractDetails, Currency, Exchange, OptionRight, SecurityIdType, SecurityType,
    Symbol,
};
use crate::messages::{IncomingMessages, OutgoingMessages, ResponseMessage};
use crate::server_versions;
use crate::testdata::builders::contracts::{
    contract_data, market_rule, option_chain, smart_components, symbol_samples,
    symbol_samples_entry,
};
        ...
        MatchingSymbolsTestCase {
            ...
            ordered_responses: vec![proto_response(
                ...
                symbol_samples()
                    .request_id(9000)
                    .entry(
                        symbol_samples_entry(12345, "AAPL")
                            .primary_exchange("NASDAQ")
        ...
        MatchingSymbolsTestCase {
            ...
            ordered_responses: vec![proto_response(
                ...
                symbol_samples()
                    ...
                    .entry(
                        symbol_samples_entry(12346, "AAPL")
                            .primary_exchange("SMART")
                            .description("APPLE INC"),
                    )
                    .encode_proto(),
        ...
        ordered_responses: vec![
            proto_response(
                ...
                option_chain()
                    ...
                    .expirations(
                        [
                            "20230120", "20230217", "20230317", "20230421", "20230519", "20230616",
                            "20230721", "20230818", "20231215", "20240119", "20240621", "20250117",
                        ]
                        .iter()
                        .map(|s| s.to_string())
                        .collect(),
...
pub enum ClientMethodTest {
    CalculateOptionPrice { contract: Contract, volatility: f64, underlying_price: f64 },
    CalculateImpliedVolatility { contract: Contract, option_price: f64, underlying_price: f64 },
}

#[cfg(feature = "sync")]
pub enum ClientMethodResult {
    OptionComputation { option_price: Option<f64>, implied_volatility: Option<f64> },
}
