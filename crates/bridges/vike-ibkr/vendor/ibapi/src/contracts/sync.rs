...
impl Client {
        ...
        while let Some(response) = responses.next() {
            ...
            match response {
                Ok(mut message) if message.message_type() == IncomingMessages::ContractData => {
                    let decoded =
                        decoders::decode_contract_details(self.server_version, &mut message)?;
                    contract_details.push(decoded);
                }
                Ok(message) if message.message_type() == IncomingMessages::ContractDataEnd => {
                    return Ok(contract_details)
                }
                Ok(message) if message.message_type() == IncomingMessages::Error => {
    ...
    pub fn market_rule(&self, market_rule_id: i32) -> Result<MarketRule, Error> {
        ...
        let subscription =
            self.shared_request(OutgoingMessages::RequestMarketRule).send_raw(request)?;
        ...
        if let Some(Ok(mut message)) = subscription.next() {
            match message.message_type() {
                IncomingMessages::SymbolSamples => {
                    return decoders::decode_contract_descriptions(
                        self.server_version,
                        &mut message,
                    );
                }
    ...
    pub fn calculate_option_price(
        ...
        let builder = self.request();
        let request_id = builder.request_id();
        let message = encoders::encode_calculate_option_price(
            request_id,
            contract,
            volatility,
            underlying_price,
        ...
        match subscription.next() {
            Some(Ok(mut message)) => {
                OptionComputation::decode(&self.decoder_context(), &mut message)
            }
            Some(Err(e)) => Err(e),
            None => Err(Error::UnexpectedEndOfStream),
        }
    ...
    pub fn calculate_implied_volatility(
        ...
        let builder = self.request();
        let request_id = builder.request_id();
        let message = encoders::encode_calculate_implied_volatility(
            request_id,
            contract,
            option_price,
            underlying_price,
    ...
    pub fn option_chain(
        ...
        request_helpers::blocking::request_with_id(
            self,
            Features::SEC_DEF_OPT_PARAMS_REQ,
            |request_id| {
                encoders::encode_request_option_chain(
                    request_id,
                    symbol,
                    exchange,
                    security_type,
