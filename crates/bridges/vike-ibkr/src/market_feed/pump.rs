...
pub(crate) fn quotes_pump(symbol: String, _interval: String, ctx: FeedCtx) {
    let contract = match parse_simplified(&symbol) {
        Ok(c) => c,
        Err(why) => {
            tracing::warn!(%symbol, reason = %why, "ibkr quotes: unusable symbol");
...
fn apply_depth_row(
    ladder: &mut Vec<vike_model::BookLevel>,
    op: i32,
    pos: i32,
    price: f64,
    size: f64,
    ...
    match op {
        0 => {
            ...
            if idx <= ladder.len() {
                ladder.insert(idx, BookLevel { price, qty: size });
                ladder.truncate(cap);
            }
        }
        1 => {
            ...
            // way in practice).
            if idx < ladder.len() {
                ladder[idx] = BookLevel { price, qty: size };
            } else if idx == ladder.len() && ladder.len() < cap {
                ladder.push(BookLevel { price, qty: size });
            }
        }
...
pub(crate) fn trades_pump(symbol: String, _interval: String, ctx: FeedCtx) {
    let contract = match parse_simplified(&symbol) {
        Ok(c) => c,
        Err(why) => {
            tracing::warn!(%symbol, reason = %why, "ibkr trades: unusable symbol");
            return;
        }
    };
    let ib = super::pump_contract(&contract);
...
pub(crate) fn bars_pump(symbol: String, interval: String, ctx: FeedCtx) {
    let contract = match parse_simplified(&symbol) {
        Ok(c) => c,
        Err(why) => {
            tracing::warn!(%symbol, reason = %why, "ibkr bars: unusable symbol");
            return;
        }
    };
    let ib = super::pump_contract(&contract);
...
pub(crate) fn depth_pump(symbol: String, _interval: String, ctx: FeedCtx) {
    ...
    let contract = match parse_simplified(&symbol) {
        Ok(c) => c,
        Err(why) => {
            tracing::warn!(%symbol, reason = %why, "ibkr depth: unusable symbol");
            return;
        }
    };
    let ib = super::pump_contract(&contract);
    ...
    // is the identity.
    let mut bids: Vec<vike_model::BookLevel> = Vec::with_capacity(ROWS);
    let mut asks: Vec<vike_model::BookLevel> = Vec::with_capacity(ROWS);
    let mut seq = 0u64;
    while !ctx.stop.load(Ordering::Relaxed) {
        match sub.next_timeout(POLL) {
            Some(Ok(SubscriptionItem::Data(d))) => {
                ...
                seq += 1;
                let bid_lv: Vec<vike_model::BookLevel> = bids.iter().take(ROWS).copied().collect();
                let ask_lv: Vec<vike_model::BookLevel> = asks.iter().take(ROWS).copied().collect();
...
mod depth_tests {
    ...
    fn update_replaces_row_no_ghost_level() {
        ...
        assert_eq!(
            bids,
            vec![BookLevel::new(100.06, 12.0)],
            "stale 100.05 must not survive an update"
        );
    }
    ...
    fn insert_shifts_down_delete_shifts_up_bounded() {
        ...
        assert_eq!(
            asks,
            vec![
                BookLevel::new(100.09, 2.0),
                BookLevel::new(100.095, 3.0),
                BookLevel::new(100.10, 1.0)
            ]
        );
        // A 4th insert would exceed cap=3 → truncated to the top 3 rows.
        apply_depth_row(&mut asks, 0, 0, 100.08, 4.0, 3);
        assert_eq!(asks.len(), 3);
        assert_eq!(asks[0], BookLevel::new(100.08, 4.0));
        // Delete the top row → rows below shift up.
        apply_depth_row(&mut asks, 2, 0, 0.0, 0.0, 3);
        assert_eq!(asks, vec![BookLevel::new(100.09, 2.0), BookLevel::new(100.095, 3.0)]);
    }

    #[test]
    fn out_of_range_position_is_ignored() {
        let mut bids = vec![BookLevel::new(100.0, 1.0)];
        ...
        assert_eq!(bids, vec![BookLevel::new(100.0, 1.0)]);
    }
}
