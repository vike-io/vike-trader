...
mod ffi {
    ...
    // Edition 2024 requires the `unsafe` qualifier on an extern block: declaring a foreign
    // signature is itself the unsafe act, since nothing checks it against the real symbol. The
    // block sits behind `#[cfg(fcsdk)]`, so no CI box compiles it — the SDK is never staged there.
    unsafe extern "C" {
        pub fn fc_login(
            user: *const c_char,
            pw: *const c_char,
