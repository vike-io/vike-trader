...
impl RouteTable {
    ...
    fn remove(&mut self, venue_order_id: &str) {
        if self.map.remove(venue_order_id).is_some() {
            if let Some(pos) = self.inserted.iter().position(|id| id == venue_order_id) {
...
#[cfg(test)]
mod tests {
    use super::{coid_of, to_fxcm_instrument, RouteTable, MAX_ROUTES};
