...
# `docs/ops/fxcm-forexconnect-the CI box.md`. `grep -c` reads to EOF, so nothing is left writing into a
...
production outage — docs/ops/fxcm-forexconnect-the CI box.md records the measurement.)
