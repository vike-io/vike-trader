...
//! `crates/bridges/fxcm/src/lib.rs`'s `sdk_linked` false, so `vike_mount::make_engine`'s
...
fn every_probed_path_is_also_a_declared_rerun_input() {
    ...
    assert!(
        ...
        "build.rs stats {unwatched:?} but never declares {unwatched:?} as a \
         ...
         when that path appears or disappears. Staging the SDK then leaves `sdk_linked()` false and \
