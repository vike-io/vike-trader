    ...
    let Some(client) = recon_client(&c, SYMBOL) else {
        assert!(
            ...
            "FXCM credentials are configured and the ForexConnect SDK IS linked, yet the reconcile \
             client could not be built — the demo login FAILED. This used to print a skip note and \
             return, which reported an unreachable venue as a GREEN test. See \
             `vike_fxcm::sdk_available()`.\n\n\
             ⚠ The reason is NOT in this message and does not need guessing at any more: the \
             reconcile session thread logs it at `error` on the `vike_fxcm` target the moment the \
             login fails, carrying the SDK's own words (`crates/bridges/fxcm/src/recon_client.rs`'s \
             `run_session`). Re-run with `--nocapture` and read that line; \
             `docs/ops/fxcm-forexconnect.md`'s triage table says what each answer means."
        );
