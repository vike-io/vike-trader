...
//!
//! ⚠ **THE RELEASE NOW SHIPS A JRE AS WELL AS A JAR, AND IT ADDS NO FIFTH PIN.** Until 2026-09-23
//! every pin above was about the JDK that COMPILES the jar and none was about the JVM that RUNS it,
//! because no release carried one — measured on the CI box that day: no `java` on `PATH`, no
//! `JAVA_HOME`, and Dukascopy working entirely off an unpacked Temurin somebody had placed by hand
//! at the path `crates/bridges/dukascopy/src/config.rs`'s `resolve_java` rung 2 reads. The release
//! closes that by CALLING pin 3's own script rather than downloading for itself, so the version and
//! the Adoptium digest stay in one file; [`the_release_ships_a_jre_through_the_pinned_provisioner`]
//! is what refuses the alternative, and also refuses the asset quietly ceasing to be attached.
//!
...
/// The INSTALLING half — the table that decides which release asset lands at which path under
/// `<project>/bin`. Read for the `jre` row alone, so this gate writes the asset name down nowhere.
const TOOL_TABLE_SCRIPT: &str = "scripts/fetch_release_tools.sh";
...
/// The RELEASE must ship a JRE, and must obtain it through the PINNED provisioner rather than a
/// fifth pin of its own.
///
/// ⚠ **The jar had no JVM to run it on a fresh install, and that was invisible from here.** Every
/// assertion above is about the JDK that COMPILES the jar; nothing was about the JRE that RUNS it,
/// because until 2026-09-23 no release carried one. MEASURED on the CI box that day: no `java` on
/// `PATH`, no `JAVA_HOME`, and Dukascopy working entirely off
/// `crates/bridges/dukascopy/src/config.rs`'s `resolve_java` rung 2 — `<project>/bin/jre/*/bin/java`
/// — from an unpacked Temurin somebody had placed by hand. The rung the code was designed around
/// was the one in use and nothing shipped it.
///
/// Two failures this closes, each with no other detector:
///
///   1. **A fifth pin.** `release.yml` inlining a `curl` with a hand-copied version and digest
///      would be a pin [`every_jdk_pin_in_the_repo_agrees`] cannot see, because that test reads
///      four named files and a new one is not among them. Same shape, same cure, as
///      [`the_sidecar_provisioner_delegates_instead_of_carrying_a_second_pin`] one rung down.
///   2. **The asset quietly stops being attached.** `scripts/fetch_release_tools.sh` would then
///      report a missing asset to every install and Dukascopy would degrade to paper — correctly,
///      silently, and forever. This is the twin of
///      [`the_release_builds_the_jar_this_gate_proves`], which exists for exactly that reason on
///      the jar side.
///
/// ⚠ The asset NAME is read out of the tool table rather than written here, so this gate carries no
/// copy of it to rot — the same rule the `SHADOW_TASK` assertions follow.
///
/// ⚠ Honest limit, and it is the module doc's: this is a check on TEXT. It cannot prove the release
/// step runs, that Adoptium answers, or that the archive unpacks — the release step's own
/// `java -version` is what proves that, on the runner, over the bytes about to be published.
/// ⚠ SECOND honest limit, declared rather than assumed away: `xtask::ci::gate_crates_for` keys on
/// source and prose, not on YAML or shell, so a PR touching ONLY `release.yml` and
/// `fetch_release_tools.sh` selects no crate and does not run this test. That is the same residual
/// [`the_release_builds_the_jar_this_gate_proves`] papers over with
/// `.github/workflows/jforex-bridge.yml`'s path filter, and it is not widened here: making the jar
/// REPRODUCIBILITY job fire on every edit to a download script would be a cost paid on the wrong
/// change class.
#[test]
fn the_release_ships_a_jre_through_the_pinned_provisioner() {
    let release = read(RELEASE);

    assert!(
        release.contains(SH),
        "{RELEASE} no longer calls `{SH}`, so the JRE it publishes is obtained some other way. The \
         only other way is a download with a version and a digest spelled HERE — a fifth pin that \
         `every_jdk_pin_in_the_repo_agrees` reads none of the files of, and that therefore agrees \
         with the other four only until somebody bumps one of them."
    );
    for (needle, what) in [
        ("api.adoptium.net", "an Adoptium download URL"),
        ("JDK_VERSION=", "a version pin"),
        ("$JdkVersion", "a version pin"),
    ] {
        assert!(
            !release.contains(needle),
            "{RELEASE} declares `{needle}` — {what} that belongs to {SH} alone. The download, the \
             pin and the verify-before-extract live there so ONE file answers 'which Temurin'; a \
             copy in the workflow that publishes the bytes is the worst place for the second one, \
             because what people DOWNLOAD would then be pinned somewhere no gate reads."
        );
    }

    // The asset the release attaches must be the one the tool table installs. Read from the table,
    // never restated — a literal here would be a third spelling of the same name.
    let table = read(TOOL_TABLE_SCRIPT);
    let jre_row = table
        .lines()
        .map(str::trim)
        .find(|l| l.starts_with("jre|"))
        .unwrap_or_else(|| panic!(
            "{TOOL_TABLE_SCRIPT} has no `jre|` row in its TOOL_TABLE, so nothing installs the JVM \
             the sidecar needs and `resolve_java`'s rung 2 is unfilled on every fresh install — \
             which is the state the CI box was measured in on 2026-09-23."
        ));
    let fields: Vec<&str> = jre_row.split('|').collect();
    assert_eq!(
        fields.len(),
        4,
        "{TOOL_TABLE_SCRIPT}'s jre row is not `<tool>|<asset>|<path>|<mode>`: {jre_row}"
    );
    let (asset, path, mode) = (fields[1], fields[2], fields[3]);
    assert!(
        release.contains(asset),
        "{TOOL_TABLE_SCRIPT} installs the release asset `{asset}`, and {RELEASE} never names it — \
         so every install would ask for a file no release carries. The fetch script would `die` on \
         it and Dukascopy would stay on paper, both correctly and neither usefully."
    );
    assert_eq!(
        path, "jre",
        "the jre row lands at `{path}` under <project>/bin. `resolve_java`'s rung 2 globs \
         `<project>/bin/jre/*/bin/java` (one level of nesting, because Temurin unpacks into a \
         versioned directory), so any other destination is invisible to the code that reads it."
    );
    assert_eq!(
        mode, "tar",
        "the jre row installs with mode `{mode}`, not `tar`. The asset is a directory TREE — \
         Temurin's `jdk-<v>-jre/` — and a file mode would chmod an archive into place instead of \
         extracting it."
    );
}
