...
use vike_dukascopy::{
    DukascopyAccount, DukascopyConfig, DukascopyExecutionClient, DukascopyTools,
    load_dukascopy_config_from, resolve_dukascopy_tools,
};
...
///
/// Spent three times, deliberately: on the credential store above and on BOTH project walks below
/// (`bin/` and `settings/state`, which this variable moves independently of each other). Reading it
/// once and threading it is what keeps those answers from disagreeing — the split
...
fn live_tools() -> DukascopyTools {
    ...
    let override_dir = settings_dir_override();
    let bin = vike_model::state_path::project_bin_dir_from(override_dir.as_deref(), &cwd);
    // The STATE directory in its own right — `$VIKE_SETTINGS_DIR` moves it without moving `bin/`,
    // which is why it is a second resolver call rather than a derivation from `bin`. It is what
    // the sidecar's JVM `user.home` is built from.
    let state = vike_model::state_path::project_state_dir_from(override_dir.as_deref(), &cwd);
    let tools = resolve_dukascopy_tools(&env, bin.as_deref(), state.as_deref());
    tracing::info!(
        target: "vike_dukascopy",
        jar = %tools.bridge_jar.display(),
        java = %tools.java,
        jforex_home = ?tools.jforex_home,
