...
//! three runtime facts the sidecar is spawned with — the JVM, the jar and that JVM's `user.home`
//! ([`resolve_dukascopy_tools`]). `exec.rs`
...
pub use catalog::{DukascopyCatalog, bundled_instruments};
pub use config::{
    BRIDGE_JAR_FILE, DukascopyAccount, DukascopyConfig, DukascopyTools, JAVA_HOME_ENV,
    JFOREX_BRIDGE_JAR_ENV, JFOREX_HOME_SUBDIR, JFOREX_TOOL_DIR, JRE_TOOL_DIR,
    dukascopy_env_var_names, load_dukascopy_config_from, resolve_dukascopy_tools,
};
pub use data::{
    Tick, decode_ticks, decompress, fetch_bars_range, fetch_hour, fetch_ticks_range, hour_url,
    point_divisor, ticks_to_bars,
};
pub use exec::{DukascopyError, DukascopyExecutionClient};
pub use proto::{Command, Envelope, encode_line, parse_command, parse_envelope};
