...
//! `<project>/bin` ([`vike_model::state_path::project_bin_dir_from`]) and
//! `<project>/settings/state` ([`vike_model::state_path::project_state_dir_from`]). This module
...
impl DukascopyAccount {
    ...
    /// `crates/vike-ops/tests/settings_registry.rs` demanded a row for each, which would have made
    ...
    /// `DUKASCOPY_DEMO2_`. `crates/vike-secrets/src/schema.rs`'s `Classification::owner_prefix`
    ...
    /// `vike_model::account_keys::AccountLabel::parse` refuses `alt`: a spelling this function fixes
...
/// why there is no second, separator-less spelling of the prefix any more.
pub fn dukascopy_env_var_names(account: DukascopyAccount) -> (String, String, String) {
    let p = account.key_prefix();
    (format!("{p}LOGIN"), format!("{p}PASSWORD"), format!("{p}SERVER"))
}
...
pub struct DukascopyTools {
    ...
    ///   because that box's `policy.toml` carries no dukascopy row and the venue stays paper;
    ...
    /// `crates/vike-mount/src/dukascopy.rs`'s `sidecar_holder` exists because two JVMs of one user
    /// share one platform cache; moving that cache moves it for both sidecars of one process, so
    /// they would still share it. What retires the refusal is the `IClient` cache setter,
    /// unchanged.
...
/// `project_bin` is [`vike_model::state_path::project_bin_dir_from`]'s answer and `project_state`
/// is [`vike_model::state_path::project_state_dir_from`]'s. Nothing here reads the process
...
/// [`vike_model::state_path::PROJECT_BIN_DIR`]'s own doc already makes for the directory above it:
...
mod tests {
    ...
    fn a_key_prefix_names_exactly_one_account_in_both_spellings() {
        for account in DukascopyAccount::ALL {
            ...
            let (login, password, server) = dukascopy_env_var_names(account);
    ...
    fn the_jforex_home_follows_the_state_directory_not_the_tool_directory() {
        ...
        let tools =
            resolve_dukascopy_tools(&HashMap::new(), Some(&project_bin), Some(&project_state));
