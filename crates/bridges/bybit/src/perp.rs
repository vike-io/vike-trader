...
pub struct BybitInstrument {
    ...
    /// **The venue's own `contractType` for this row, verbatim** — `"LinearPerpetual"`,
    /// `"InversePerpetual"`, `"LinearFutures"`, `"InverseFutures"`. `None` when the row carries
    /// none (a spot row, or a shape the venue changed).
    ///
    /// ⚠ **It is parsed because the exec plane needs it to REFUSE, not because anything routes on
    /// it.** MEASURED 2026-09-16: `instruments-info?category=linear&symbol=XRPUSD` ANSWERS, with
    /// `contractType: "InversePerpetual"` and `settleCoin: "XRP"` — so the exec grid fetch's
    /// `category: "linear"` literal does not fail loudly on an inverse symbol, it succeeds and
    /// returns a coin-settled grid (`qtyStep=1`, `minOrderQty=1` — USD contracts) that
    /// `SymbolProperties::multiplier()` then folds at 1.0 because `contract_size` stays unset. Every
    /// notional the RiskGate computes is wrong by roughly the price. `crates/bridges/bybit/src/
    /// exec.rs`'s `non_linear_perpetual_refusal` is the consumer, and it exists so that arm stops
    /// being the one that does not fail loudly.
    pub contract_type: Option<String>,
}

/// The asset class of ONE `instruments-info` `result.list[]` row, from the venue's own
/// `contractType` — the SAME word [`crate::exec::non_linear_perpetual_refusal`] refuses on and the
/// same word `crates/bridges/bybit/src/catalog.rs`'s `parse_perpetuals` gates its picker rows with,
/// read off the row rather than re-derived from the symbol text
/// (`docs/decisions/0061-an-instrument-names-its-kind.md`; bybit is a venue where the text cannot
/// answer anyway — `BTCUSD` is an INVERSE perpetual and `BTCUSDT` a linear one).
///
/// ⚠ **`contractType` is the ROW's word; `category` is the REQUEST's.** A survey named `category`
/// as this venue's field and it is not one: `category` is the query parameter
/// [`crate::exec::fetch_bybit_instrument`] sends and the venue echoes ONCE at `result.category`,
/// beside the list — so a per-row read of it does not exist, and the one value it carries is the
/// caller's own literal `"linear"`, which MEASURED 2026-09-16 answers for inverse symbols too.
/// Reading that back would tag an `InversePerpetual` row with the question rather than the answer.
///
/// Both settlement modes map to [`AssetClass::CryptoPerp`] and both dated shapes to
/// [`AssetClass::CryptoFuture`]: linear-vs-inverse is a SETTLEMENT distinction, which this taxonomy
/// does not spell (`crates/bridges/deribit/src/catalog.rs`'s `parse_instruments` carries the same
/// note for its own two modes) and which bybit already publishes verbatim on
/// [`BybitInstrument::contract_type`] for anything that needs it.
///
/// An absent/empty `contractType` is `None` — the shape of a SPOT row, or of a row whose shape the
/// venue changed. An unrecognised non-empty word is `None` too, deliberately: the four values are a
/// closed set today, and a fifth one bybit invents means something this parser has not read about.
fn perp_asset_class(entry: &Value) -> Option<AssetClass> {
    match entry.get("contractType").and_then(|c| c.as_str()).unwrap_or("") {
        "LinearPerpetual" | "InversePerpetual" => Some(AssetClass::CryptoPerp),
        "LinearFutures" | "InverseFutures" => Some(AssetClass::CryptoFuture),
        _ => None,
    }
}
        ...
        out.insert(
            ...
            BybitInstrument {
                properties: SymbolProperties {
                    ...
                    // The class is the VENUE'S OWN `contractType`, read off the same row the grid
                    // came from — see [`perp_asset_class`], which also records why it is NOT the
                    // `category` a survey named.
                    asset_class: perp_asset_class(entry),
                ...
                max_leverage: entry
                    ...
                    .and_then(json_num),
                // Carried, never interpreted here. Its one consumer is the exec thread's refusal.
                contract_type: entry
                    .get("contractType")
                    .and_then(|v| v.as_str())
                    .map(str::to_string),
