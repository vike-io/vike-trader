...
//! dead-man (`vike_core::LinkDeadManConfig`) can hear a bybit link die on that daemon. The venue's
//! other emitter, the DOM depth feed (`crates/bridges/bybit/src/market_feed.rs`'s `depth_main`),
//! discloses through a `LiveDataSink`; this one goes onto the tick lane, because that is the seam
//! this pump has.
...
use vike_model::{BookLevel, DeltaDecision, L2Book, QuoteTick, SeqPolicy, TradeTick};

const VENUE: &str = "bybit";
const READ_TIMEOUT: Duration = Duration::from_secs(2);
/// ⚠ **THIS LANE STAYS LINEAR-ONLY, DELIBERATELY, and it is the one host site
/// `docs/decisions/0061-an-instrument-names-its-kind.md` phase 4 did not widen.**
///
/// `crates/bridges/bybit/src/market_feed.rs` now picks between THREE sockets
/// (`market_feed::ws_host`), and this file's own const could join them. It does not, because this
/// lane exists to feed the DAEMON's tick plane for the symbol its exec engine trades, and
/// `crates/bridges/bybit/src/exec.rs`'s `non_linear_perpetual_refusal` refuses to trade anything but
/// a `LinearPerpetual` — so an inverse symbol cannot reach a mount that spawns this. Widening the
/// host here without widening exec would build a tick lane for a book no order can be placed in,
/// and widening exec is the signed-plane work that refusal names.
///
/// It is written down rather than left implied because the two files would otherwise look like an
/// oversight: **when the exec category is threaded through `perp.rs`/`recon_client.rs`/`funding.rs`,
/// this const is part of the same change**, and `market_feed::ws_host` is the chooser to reuse
...
fn levels(side: Option<&Value>) -> Vec<BookLevel> {
...
pub fn quote_from_book(book: &L2Book, symbol: &str) -> Option<QuoteTick> {
    let BookLevel { price: bid, qty: bid_size } = book.best_bid()?;
    let BookLevel { price: ask, qty: ask_size } = book.best_ask()?;
    Some(QuoteTick {
        ts: 0,
        // TODO(book-recording plan): stamp at WS receive
        local_ts: 0,
...
pub fn parse_orderbook_snapshot(text: &str) -> Option<(u64, Vec<BookLevel>, Vec<BookLevel>)> {
...
/// The stream label stamped on this pump's [`StreamStatusUpdate`]s — informational (the
/// connection-state dead-man keys on `(venue, symbol)` and destructures the label away), but it is
/// what separates this socket from the venue's DOM lane (`market_feed.rs`'s `depth_main`, label
/// `"depth"`) in an `on_feed_status` trace.
const LINK_STREAM_LABEL: &str = "ticks";

/// Disclose this pump's TRANSPORT state onto the core's tick lane — the bybit twin of
/// `crates/bridges/binance/src/family/depth.rs`'s `disclose_link`, whose doc carries the argument
/// for why the tick lane is where a CEX daemon hears a dead link at all. No `LiveDataSink` is
/// involved: this pump owns a bare [`TickSender`], and `vike_model::FeedStatus` is what
/// `vike_core::CoreLaneSink` converts a sink-side `StreamStatus` into anyway
/// ([`HealthEvent::feed_status`] is the one map). A `CoreGone` send failure is dropped exactly as
/// every other push on this loop drops it.
fn disclose_link(ticks: &TickSender, symbol: &str, ev: HealthEvent) {
...
///
/// The reconnect loop owns ONE [`StreamHealth`] for the whole feed lifetime (across reconnects),
/// as `vike_bridge_core::depth`'s `run_depth_feed` does: an outage is disclosed as exactly one
/// `Gap`/`Live` pair however many re-dials it takes. Transport half only — this loop runs no
/// freshness watchdog, hence [`StreamHealth::transport_only`].
    ...
    let join = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            while !st.load(Ordering::Relaxed) {
                if run_session(&sym, tick_size, &ticks, &st, &mut health).is_ok() {
                    return;
                }
                // The outage, disclosed ONCE. A failed CONNECT lands here too (the `?` on the bounded
                // `connect_ws` dial below), which is what makes the FIRST link death of a
                // mount visible — the pump that never comes up discloses a disconnect and never a
                // recovery, and that is the outage the dead-man exists for.
                if let Some(ev) = health.enter_gap(vike_model::clock::now_ms()) {
                    disclose_link(&ticks, &sym, ev);
                }
...
fn run_session(
    ...
    // `connect_ws` arm the market pump, the bridge-core depth driver and the binance/aster twin of
    // THIS pump already take. It was a bare `tungstenite::connect` until 2026-09-06, which applies
    // no connect bound of any kind: on a black-holed route this thread sat out the OS's own SYN
    // ladder (~127 s on Linux defaults) with `stop` already raised behind it. The binance family
    // pump had that fixed on 2026-08-08 and this one was missed; it was found by
    // `crates/vike-ops/tests/feed_stop_windows_gate.rs`, which classifies a file as feed-path from
    // the drivers it names. ⚠ It matters more here than it did: this pump is now the lane the
    // CONNECTION-state dead-man hears a bybit link die on, and 127 s of unstoppable dial is longer
    // than that switch's own default grace.
    ...
    loop {
        ...
        // TRANSPORT RECOVERY on the first inbound frame, NOT at connect — the argument (a venue
        // that accepts a socket and immediately drops it would otherwise reset the dead-man's
        // grace on every cycle and never trip) is on `crates/bridges/binance/src/family/depth.rs`'s
        // `run_session`, and this is the same rule. One `Option::take` per inbound frame on THIS
        // thread; `vike-core`'s fold is untouched.
        if let Some(ev) = health.recover() {
            disclose_link(ticks, symbol, ev);
        }
        match msg {
            Message::Text(txt) => {
...
mod tests {
    ...
    fn orderbook_snapshot_then_delta() {
        ...
        assert_eq!(book.best_bid(), Some(BookLevel::new(60000.0, 5.0)));
        assert_eq!(book.best_ask(), Some(BookLevel::new(60001.0, 4.0)));
        ...
        assert_eq!(book.best_bid(), Some(BookLevel::new(60000.0, 8.0)));
        ...
        assert_eq!(book.best_bid(), Some(BookLevel::new(60000.0, 8.0)), "stale must not apply");
    }
    ...
    fn dropped_frame_is_a_resync_and_the_delta_is_not_folded() {
        ...
        assert_eq!(
            book.best_bid(),
            Some(BookLevel::new(60000.0, 5.0)),
            "a gapped delta must NOT fold"
        );
    ...
    fn u_regression_is_a_resync_not_a_stale_drop() {
        ...
        assert_eq!(
            book.best_bid(),
            Some(BookLevel::new(60000.0, 5.0)),
            "a regressed delta must NOT fold"
        );
