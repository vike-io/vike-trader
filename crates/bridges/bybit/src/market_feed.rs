...
/// topic shapes as spot; only the stream URL differs. A `.P`-suffixed `subscribe_bars` symbol whose
/// symbol the venue lists on `category=linear` routes its kline WS here — see [`ws_host`].
...
/// owning wrapper over [`vike_catalog::split_perp`], which is where that split is defined for every
/// venue.
///
...
pub fn pump_opts(subscribe: &str) -> MarketPumpOpts<'_> {
...
/// `subscribe_depth` is the chart DOM (`vike_app_core::feed_lifecycle`'s `ensure_depth`), while
...
mod tests {
    use super::{
        ...
        PUBLIC_WS_SPOT, mark_subscribe_frame, parse_trades, perp_split, relabel_series,
        route_frame, route_mark_frame, subscribe_frame, trades_subscribe_frame, ws_host,
    };
    use std::sync::Arc;
