...
use vike_bybit::transport::{unwrap_envelope, BybitTransport, UreqBybitTransport};
...
fn bybit_isolated_wallet_probe() {
    ...
    match probe.call(
        ...
        Err(e) => {
            println!("=== switch-isolated FAILED (code {}: {}) — trying account-level set-margin-mode ISOLATED_MARGIN ===", e.code, e.msg);
            match probe.call(
    ...
    if isolated_manufactured {
        println!("=== probe complete: inspect the ISOLATED-OPEN dump above for positionBalance/positionIM/tradeMode semantics ===");
    } else {
        println!("=== probe complete: ISOLATION VENUE-BLOCKED on this account — the CROSS control dump above shows what positionBalance/positionIM/tradeMode carry for a cross row (a populated positionBalance there disproves the isolated-wallet reading) ===");
    }
}
