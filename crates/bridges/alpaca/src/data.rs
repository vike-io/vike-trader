...
use vike_data::live::{require_live_verb, DataClient, LiveDataError, LiveDataSink, SubscriptionId};
        ...
        match self.conns.get_mut(&class) {
            Some(conn) => {
                ...
                if conn.reg.add(verb, sym.clone()) == WireAction::Subscribe {
                    if let Err(e) = conn.cmd_tx.send(ConnCmd::Sub(verb, sym.clone())) {
...
impl DataClient for AlpacaDataClient {
    ...
    fn unsubscribe(&mut self, id: SubscriptionId) {
        ...
        if teardown {
            if let Some(conn) = self.conns.remove(&class) {
