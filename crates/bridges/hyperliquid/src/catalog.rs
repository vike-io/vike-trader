...
impl CatalogProvider for HyperliquidCatalog {
    ...
    fn list_instruments(&self) -> Result<Vec<Instrument>, CatalogError> {
        ...
        //
        // ⚠ **`load`, so this reads the PROCESS ENV only — it cannot see `flags.hyperliquid_hip3`.**
        // That key was wired by giving the mount `HyperliquidInstruments::load_from_vars` and having
        // the composition root fold the resolved flag into the `vars` map; this provider takes no
        // map, so a file-set flag would leave the picker's universe disagreeing with the mount's
        // symbology about HIP-3 markets. It is inert today — no binary constructs this provider
        // since the desktop cut (`crates/vike-desktop/src/main.rs`'s `spawn_catalog_fetcher` builds
        // `vike_deribit::DeribitCatalog` alone) — and a binary that re-links it must thread a map
        // here and call `load_from_vars`, not this line. Named in `vike_config::CONSUMPTION`'s
        // `flags.hyperliquid_hip3` row as a known unreached reader, so it is a declared residual
        // rather than a hole somebody finds later.
