...
// SDK-spec-faithful, still with NO golden vector — but the DIGEST below is testnet-verified as of
// 2026-09-14: the venue rebuilt this typed data from a posted payload and recovered the signing
// key's own address, which it can only do if its digest equals this one. See `crate::transfer`.
...
// SDK-spec-faithful, still with NO golden vector — but the DIGEST below is testnet-verified as of
// 2026-09-14, by the same address-recovery oracle as its `usdClassTransfer` twin. See
// `crate::builder_fee`.
...
/// fee — an agent/API wallet's signature is accepted for orders but not for this grant, MEASURED
/// on testnet 2026-09-14 by
/// `crates/bridges/hyperliquid/tests/hyperliquid_permissioning_smoke.rs`); the caller is
/// responsible for passing a `Signer` built from the master private key, not an agent's. ⚠ The
/// same is true of [`sign_usd_class_transfer`], whose own module doc once claimed the opposite.
