...
impl HyperliquidInstruments {
    ...
    /// [`Self::load`] over a CALLER-SUPPLIED map — the shape a settings file can reach.
    ///
    /// `HYPERLIQUID_HIP3` was a process-env read several layers below any binary, which is why
    /// `vike_config::CONSUMPTION`'s `flags.hyperliquid_hip3` row was a written admission. The
    /// composition root folds the RESOLVED flag into the `vars` map the mount already threads
    /// everywhere (`crates/vike-tradehub/src/tradehub_cli.rs`'s `fold_flags_into_vars`), and
    /// RESOLVED is the load-bearing word twice over: `vike_config::load` applies the environment
    /// layer OVER the file layer (`crates/vike-config/tests/flag_registry.rs`'s
    /// `the_environment_overrides_the_file_for_every_flag`), and the fold OVERWRITES this key
    /// rather than deferring to whatever the credential store held — this reader took no
    /// credential-store tier before the wiring, and leaving one standing would have made
    /// `<project>/settings/secrets.env` a third source outranking the environment. So the OR below
    /// is a second SOURCE rather than a widening.
    ///
    /// A caller whose map holds no such key gets [`Self::load`] exactly — the process-env read is
    /// still there and still first.
    ///
    /// ⚠ **[`crate::HyperliquidCatalog`] is a KNOWN reader this does not reach**, and it is named
    /// here rather than left to be discovered: `HyperliquidCatalog::list_instruments` calls
    /// [`Self::load`], which sees `hip3_enabled` and the process env alone, so a file-set
    /// `flags.hyperliquid_hip3` would give the picker's universe a different HIP-3 answer from the
    /// mount's symbology. It is INERT today — no binary constructs that provider since the desktop
    /// cut left `crates/vike-desktop/src/main.rs`'s `spawn_catalog_fetcher` with
    /// `vike_deribit::DeribitCatalog` alone — so the divergence is latent rather than shipping. A
    /// binary that re-links the catalog must give it a map and call THIS constructor.
    pub fn load_from_vars(
        transport: &HyperliquidTransport,
        recorder: Option<&PropertiesRecorder>,
        vars: &std::collections::HashMap<String, String>,
    ) -> Result<Self, VenueApiError> {
        let hip3 = hip3_enabled() || hip3_flag(vars.get(HIP3_ENV).map(String::as_str));
