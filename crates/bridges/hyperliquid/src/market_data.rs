...
use vike_model::{Bar, BookLevel, QuoteTick, TradeTick};
...
fn level_px_sz(lvl: &Value) -> Option<BookLevel> {
    Some(BookLevel::new(fstr(lvl.get("px"))?, fstr(lvl.get("sz"))?))
}

/// One side of an l2Book (`[{px,sz,n},…]`) → `Vec<BookLevel>`, skipping any malformed level. An absent
/// / non-array side yields an empty vec.
fn side_levels(side: Option<&Value>) -> Vec<BookLevel> {
...
pub fn bbo_to_quote(frame: &Value) -> Option<QuoteTick> {
    ...
    let BookLevel { price: bid, qty: bid_size } = level_px_sz(bbo.first()?)?;
    let BookLevel { price: ask, qty: ask_size } = level_px_sz(bbo.get(1)?)?;
    Some(QuoteTick {
...
mod tests {
    ...
    fn l2book_full_snapshot_splits_bids_and_asks() {
        ...
        assert_eq!(snap.bids, vec![BookLevel::new(60000.0, 5.0), BookLevel::new(59999.0, 3.0)]);
        assert_eq!(snap.asks, vec![BookLevel::new(60001.0, 4.0)]);
