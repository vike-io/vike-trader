...
impl ExecutionClient for HyperliquidExecutionClient {
    ...
    fn modify(&mut self, order: &OrderRequest, new_qty: Option<f64>, new_price: Option<f64>) {
        ...
        //
        // ⚠ It used to return SILENTLY, which made a halted modify indistinguishable from a lost
        // one (`docs/ops/kill-switches.md` gap 5). The verdict is unchanged; the advisory is the
        // same non-terminal event, carrying the same shared reason, that this client already emits
        // for an unknown coid and a dead command channel — and that `ExecActor`, the paper book and
        // cTrader now all emit here too.
        //
        // ⚠ DECLARED RESIDUAL: this client's halt arms — this one and the two in `submit`/
        // `submit_batch` — are covered by NO test, and never have been. `HyperliquidExecutionClient`
        // cannot be constructed without a `Signer` and a spawned private-WS pump, and it has no
        // `with_halt_path` seam, so there is nothing for a test to drive that does not dial the
        // venue. The cross-client pin (`crates/vike-mount/tests/halt_paper_live_agree.rs`) covers
        // the shared `ExecActor` boundary and the paper book; this copy rides on being three lines
        // that read identically to the one beside them. Do not read the green as covering it.
        // Closing it means a constructor seam, not another assertion.
