...
use vike_bridge_core::exec_actor::{cancel_event, CancelOutcome};
...
use vike_model::{now_ms, OrderRequest, TimeInForce};

use crate::config::{Network, Product};
use crate::consts::VENUE;
use crate::event_mapper::{cloid_from_client_order_id, map_order_response, SubmittedOrder};
use crate::funding::{fetch_funding, HlFundingPoller};
    ...
    fn build_order_wire(
        ...
        let (limit_px, order_kind) = if is_trigger {
            ...
            if let Some(source) = req.trigger_by {
                use vike_bridge_core::trigger::{venue_trigger_by, TriggerByOutcome};
...
mod tests {
    ...
    fn cancel_uses_native_cancel_by_cloid() {
        ...
        assert!(evs
            .iter()
            .any(|e| matches!(e, Event::OrderCanceled(c) if c.client_order_id == "c1")));
    }
