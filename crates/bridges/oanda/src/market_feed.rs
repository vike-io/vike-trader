...
use vike_bridge_core::poller::{sleep_stop_aware, STOP_POLL_SLICE};
...
use vike_model::{now_ms, Bar, LiveVerb, QuoteTick};

use crate::config::OandaConfig;
use crate::data::{granularity, to_oanda_instrument};
use crate::market_data::{decode_pricing_frame, parse_forming_candle, PricingFrame};
        ...
        match rest.get(&ctx.config.rest_base, &path, &query) {
            Ok(v) => {
                ...
                if !seeded {
                    ...
                    ctx.set_status(format!("LIVE · OANDA bars {series}@{interval}"));
                } else {
                    for b in fresh {
