...
//! dead-man (`vike_core::LinkDeadManConfig`) can hear an okx link die on that daemon. The venue's
//! other emitter, the DOM depth feed (`crates/bridges/okx/src/market_feed.rs`'s `depth_main`),
...
/// All levels of an OKX bids/asks array → `BookLevel`s.
...
/// Disclose this pump's TRANSPORT state onto the core's tick lane — the okx twin of
...
fn disclose_link(ticks: &TickSender, inst: &str, ev: HealthEvent) {
    ...
    let join = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            while !st.load(Ordering::Relaxed) {
                if run_session(&inst, tick_size, &ticks, &st, &mut health).is_ok() {
                    return;
                }
...
fn run_session(
    ...
    // CONNECTION-state dead-man hears an okx link die on, and 127 s of unstoppable dial is longer
...
mod tests {
    ...
    fn books5_rebuilds_the_top() {
        ...
        assert_eq!(book.best_bid(), Some(BookLevel::new(60000.5, 1.0)));
    ...
    fn books_deep_snapshot_then_update() {
        ...
        match parse_books_frame(&snap) {
            BooksFrame::Snapshot { seq, bids, asks } => {
                ...
                assert_eq!(asks[0], BookLevel::new(60001.0, 4.0));
            }
        ...
        match parse_books_frame(&upd) {
            BooksFrame::Update { seq, prev_seq, bids, asks } => {
                ...
                assert_eq!(bids[0], BookLevel::new(60000.0, 8.0));
                assert_eq!(asks[0], BookLevel::new(60001.0, 0.0)); // sz 0 → the folder removes it
            }
