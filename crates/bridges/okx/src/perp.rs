...
use vike_model::{AssetClass, MarginMode, OrderRequest, SymbolProperties};
...
/// The asset class of ONE `public/instruments` `data[]` row, from the venue's own `instType` — the
/// field OKX publishes to say WHICH universe a row belongs to, and the same word
/// `crates/bridges/okx/src/catalog.rs`'s `parse_insts` is handed one instType at a time as its
/// `class` argument. Read here rather than re-derived from the `instId` text, the implicit encoding
/// `docs/decisions/0061-an-instrument-names-its-kind.md` exists to remove (`-SWAP`/`-250926` tails
/// are exactly the shape it names).
///
/// ⚠ **This parser does not only ever see SWAP rows, which is why the word is read per ROW.**
/// [`crate::exec::fetch_okx_instrument_with_cap`] asks `instType=SWAP`, but the function is a pure
/// parse over whatever `data[]` it is handed, and OKX's four instTypes share one row shape.
///
/// An unrecognised non-empty `instType` is `None`, NOT the neighbouring variant. That is where this
/// deliberately differs from `crates/bridges/binance/src/perp.rs`'s `perp_asset_class`, whose
/// `contractType` is a BINARY distinction (perpetual, or a dated contract) so "everything else" has
/// exactly one meaning. `instType` is an open list of NAMED universes — `MARGIN` is spot-margin,
/// and OKX may add more — so "everything else" here names nothing, and
/// [`vike_model::SymbolProperties::asset_class`]'s doc argues why a missing answer must stay
/// missing rather than become a guess.
fn okx_asset_class(entry: &Value) -> Option<AssetClass> {
    match entry.get("instType").and_then(|t| t.as_str()).unwrap_or("") {
        "SWAP" => Some(AssetClass::CryptoPerp),
        "FUTURES" => Some(AssetClass::CryptoFuture),
        ...
        out.insert(
            ...
            OkxInstrument {
                properties: SymbolProperties {
                    ...
                    min_qty: f("minSz"),
                    max_qty: f("maxMktSz"),
                    // The class is the VENUE'S OWN `instType`, read off the same row the grid came
                    // from — see [`okx_asset_class`] for why an unrecognised word stays `None`.
                    asset_class: okx_asset_class(entry),
