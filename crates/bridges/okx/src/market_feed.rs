...
pub fn route_frame(text: &str) -> KlineEvent {
    ...
    if confirmed {
        KlineEvent::Closed(bar)
    } else {
        KlineEvent::Forming(bar)
    }
}
...
fn mark_main(inst: String, _interval: String, ctx: FeedCtx) {
    ...
    run_market_feed(
        ...
        || {},
        |e| ctx.set_status(format!("{inst} mark ws error (reconnecting): {e}")),
    );
}
...
fn trades_main(inst: String, _interval: String, ctx: FeedCtx) {
    ctx.set_status(format!("LIVE · OKX trades {inst}"));
    ...
    run_market_feed(
        ...
        |e| ctx.set_status(format!("{inst} trades ws error (reconnecting): {e}")),
    );
}
...
use crate::market_data::{
    parse_books_frame, parse_raw_books, BooksFrame, PUBLIC_WS as MD_PUBLIC_WS,
};
use vike_bridge_core::depth::{infer_tick_size, run_depth_feed, BookOp};
...
mod tests {
    use super::{
        mark_subscribe_frame, okx_depth_decode, parse_trades, route_frame, route_mark_frame,
        subscribe_frame, trades_subscribe_frame, FeedCtx, Feeds, KlineEvent, MarkEvent,
    };
    use crate::book_checksum::ChecksumBook;
