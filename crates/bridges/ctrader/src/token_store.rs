...
//! [`vike_secrets::save_credentials_to_store`] — the byte-preserving UPSERT that replaces exactly
//! the named keys in place and leaves every comment, blank line, unrelated key and their ordering
//! verbatim, written back atomically. That is the ONE sanctioned way to write the store, and it is
//! what the workspace rule actually forbids: rewriting the user's only copy of their keys
//! WHOLESALE, never amending one key it was asked to amend.
//!
//! ⚠ **"ONE home" stopped meaning "one FILE" on `docs/decisions/0054`'s credential half**, and this
//! site is the one where the difference bites hardest. `<project>/settings/db/vike.db` shadows
//! `secrets.env` wherever it exists, and a write that named the file there would succeed, change the
//! file, record the rotation and return `Ok` while the grant every later process reads stayed the
//! SPENT one — a token cTrader has already exchanged. `save_credentials_to_store` asks
//! `vike_secrets::backend_in` which store answers, which is the same question every reader asks; its
//! file branch is the same upsert this module has always used.
//!
...
pub fn persist(
    ...
    // ⚠ **The store that ANSWERS, not the file** — and this is the site where getting it wrong is
    // sharpest in the whole workspace. `docs/decisions/0054`'s credential half lets
    // `<project>/settings/db/vike.db` shadow `secrets.env`; a write that named the file would
    // succeed, change the file, record the rotation in the ledger and return `Ok` — while the grant
    // every later process reads stayed the SPENT one. cTrader rotates the refresh token on every
    // refresh, so the next restart would authenticate with a token already exchanged, and that
    // session cannot recover on its own.
    //
    // The settings directory is `persist.store`'s parent, which is where the composition root
    // joined the file name on; `save_credentials_to_store` then asks `vike_secrets::backend_in`
    // about it — the same question every reader asks — and its file branch is the same
    // byte-preserving upsert this call site has always made.
    let dir = vike_secrets::settings_dir_of_store(&persist.store);
    vike_secrets::save_credentials_to_store(
        &dir,
        vike_secrets::Table::Credential,
        &updates,
        // Schema 2 needs a NEW credential name classified into an account; this writer only ever
        // REPLACES a name the store already holds (the venue rotates the grant under the same two
        // keys), so the classifier is here for the first-ever write of the pair rather than for the
        // rotation. `vike_bridge_core::credentials::classify_credential_name` is the one
        // implementation; this crate already links it.
