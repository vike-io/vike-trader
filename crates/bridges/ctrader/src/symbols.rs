...
impl SymbolMap {
    ...
    pub fn risk_properties(&self, symbol: &str) -> Option<vike_model::SymbolProperties> {
        ...
        Some(vike_model::SymbolProperties {
            ...
            // ⚠ `asset_class` stays absent, and this is what was looked for and NOT found
            // (`docs/decisions/0061-an-instrument-names-its-kind.md`; the field's own doc on
            // `vike_model::SymbolProperties::asset_class` argues why a missing answer must not
            // become a guess):
            //
            // - `ProtoOaSymbol` — the FULL symbol this map's `digits`/`VolumeGrid` come from —
            //   carries no class field of any kind. Its nearest neighbours are `trading_mode`
            //   (open/close-only/disabled) and `measurement_units`, neither of which says what
            //   the instrument IS.
            // - `ProtoOaLightSymbol` carries `symbol_category_id`, and that is a BROKER-DEFINED
            //   id, not a vocabulary. Resolving it needs two more requests this bridge's
            //   handshake never sends (`crates/bridges/ctrader/src/conn.rs` sends
            //   `ProtoOaSymbolsListReq` then `ProtoOaSymbolByIdReq` and stops): a
            //   `ProtoOaSymbolCategory` list to reach `asset_class_id`, then a
            //   `ProtoOaAssetClass` list to reach a NAME the broker chose. Neither is stored on
            //   `SymbolMap`, so nothing here could read one even if it had been fetched.
            // - `crates/bridges/ctrader/src/catalog.rs`'s `classify` DOES answer Fx-vs-Cfd for the
            //   picker — off the symbol NAME (`len == 6` and uppercase ⇒ Fx). That is precisely
            //   the implicit encoding 0061 exists to remove, so it is not borrowed here: this
            //   grid is persisted as `kind=properties`, and a stored class read out of a ticker is
            //   a guess a consumer can no longer tell from a fetch.
            //
            // Closing it means fetching the two lists at handshake and keeping the
            // `(category_id → asset class name)` mapping on `SymbolMap` — a connection-level
            // change, not a parser-level one.
            //
            // Everything else absent too: no min-notional, no contract multiplier, a flat grid
...
mod tests {
    ...
    /// ⚠ **`asset_class` is `None` here on PURPOSE, and this pins it as an answer rather than an
    /// oversight** (`docs/decisions/0061-an-instrument-names-its-kind.md`). `EURUSD` is an FX pair
    /// and `US500` a CFD, and the taxonomy has both variants — but neither `ProtoOaSymbol` nor
    /// `ProtoOaLightSymbol` says so, and the only local signal is the symbol NAME, which is the
    /// encoding 0061 removes. `risk_properties`'s own comment carries the two requests that would
    /// close it; this test is what turns green the day one of them is wired, so the change cannot
    /// land without a decision about this field.
    #[test]
    fn ctrader_names_no_class_because_the_handshake_fetches_none() {
        let light = vec![
            ProtoOaLightSymbol {
                symbol_id: 1,
                symbol_name: Some("EURUSD".to_string()),
                // The broker's own grouping id — present on the wire, and NOT a class: it
                // resolves only through `ProtoOaSymbolCategory` → `ProtoOaAssetClass`, neither of
                // which this handshake requests or this map stores.
                symbol_category_id: Some(7),
                ..Default::default()
            },
            ProtoOaLightSymbol {
                symbol_id: 2,
                symbol_name: Some("US500".to_string()),
                symbol_category_id: Some(9),
                ..Default::default()
            },
        ];
        let full = vec![
            ProtoOaSymbol { symbol_id: 1, digits: 5, ..Default::default() },
            ProtoOaSymbol { symbol_id: 2, digits: 1, ..Default::default() },
        ];
        let map = SymbolMap::from_symbols(&light, &full);
        for symbol in ["EURUSD", "US500"] {
            assert_eq!(
                map.risk_properties(symbol).expect("known symbol → a grid").asset_class,
                None,
                "{symbol}: cTrader publishes no class on either symbol message"
            );
        }
    }
}
