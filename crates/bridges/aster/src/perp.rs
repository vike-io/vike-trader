        ...
        out.insert(
            ...
            PerpInstrument {
                properties: SymbolProperties {
                    ...
                    // Aster's perp API is a Binance-`/fapi` fork, so the class is read the same way
                    // its twin reads it — off the entry's OWN `contractType`
                    // (`crates/bridges/binance/src/perp.rs`'s `perp_asset_class` argues the
                    // mapping), never off the symbol text, which is the implicit encoding
                    // `docs/decisions/0061-an-instrument-names-its-kind.md` exists to remove.
                    // ⚠ Deliberately NOT a blanket `CryptoPerp` even though this venue lists only
                    // perpetuals today: whether Aster's fork actually publishes the field is NOT
                    // measured here, and an entry that omits it must stay UNCLASSIFIED rather than
                    // acquire a class nobody stated — see
                    // `vike_model::SymbolProperties::asset_class` for why a guess is worse than an
                    // absence. A live fetch showing the field always present is a measurement to
                    // record here, not a default to assume now.
                    asset_class: match entry
                        .get("contractType")
                        .and_then(|c| c.as_str())
                        .unwrap_or("")
                    {
