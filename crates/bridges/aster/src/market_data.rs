...
use vike_binance::family::depth::{PumpSpec, combined_stream_url, parse_depth_snapshot};
...
pub use vike_binance::family::depth::{
    DepthOutcome, MarketDataFeed, MdEvent, apply_depth_event, decode_book_ticker, decode_trade,
    route_frame,
};
...
/// the SAME [`vike_catalog::split_perp_at`] the kline/trades/depth lanes in `market_feed.rs` already
...
fn pump_wire(env: Environment, symbol: &str) -> (String, String) {
    let (api_symbol, _) = vike_catalog::split_perp_at(VENUE, symbol);
...
mod tests {
    ...
    fn depth_sync_snapshot_delta_stale_and_gap() {
        ...
        book.apply_snapshot(100, &[BookLevel::new(60000.0, 5.0)], &[BookLevel::new(60001.0, 5.0)]);
        ...
        assert_eq!(book.best_bid(), Some(BookLevel::new(60000.0, 5.0)), "stale must not apply");
