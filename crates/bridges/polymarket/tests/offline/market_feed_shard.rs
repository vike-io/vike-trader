...
use vike_polymarket::{
    MarketStream, PumpMode, PumpTiming, TokenSlot, TokenState, Watchdog, run_session,
    run_shard_session, subscribe_message,
};
...
fn interleaved_frames_route_to_their_own_token() {
    ...
    assert_eq!(
        slots[0].state.book.best_ask(),
        Some(BookLevel::new(54.0, 90.0)),
        "A's ask delta hit A's book"
    );
    assert_eq!(
        slots[0].state.book.best_bid(),
        Some(BookLevel::new(51.0, 100.0)),
        "…and A's bid is untouched"
    );
    assert_eq!(
        slots[1].state.book.best_bid(),
        Some(BookLevel::new(11.0, 7.0)),
        "B's bid delta hit B's book"
    );
    assert_eq!(
        slots[1].state.book.best_ask(),
        Some(BookLevel::new(12.0, 3.0)),
        "…and B's ask is untouched"
    );
}
