...
//! This client IS now wired into `vike_mount::make_engine`'s `("polymarket", _)` arm, but behind a
//! venue-local opt-in ON TOP of the workspace-wide `VIKE_RECONCILE=1`: see
...
//! configuration for a recon-only mount remains `VIKE_RECONCILE=1` + `POLY_RECONCILE=1` +
//! `VIKE_RECONCILE_POLICY=quarantine` — for a real reason now, not the false one. With
//! `POLY_RECONCILE` unset — the default — `make_engine` builds nothing and makes no network call.
//!
...
use crate::config::{first_token, PolymarketCreds, CLOB_BASE, DATA_API_BASE};
...
/// The SECOND gate on Polymarket reconciliation, on top of the workspace-wide `VIKE_RECONCILE=1`:
/// the EXACT string `"1"` in the process env **or** in the workspace `.env` map, default OFF.
///
...
///   **`VIKE_RECONCILE_POLICY=quarantine` is required**, on the `PositionDrift` ground.
