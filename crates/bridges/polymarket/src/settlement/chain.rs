...
/// ruling 2026-09-16, a process materialises only what it asked for. The four names that survive
/// are the four that always did; what changes is that no other venue secret is materialised even
/// for the instant between the load and the filter. [`crate::egress`]'s `dotenv_proxy_vars` carries
/// the full argument, including why `into_map` rather than the three-state
/// `vike_secrets::ScopedSecrets::get` is the right end of it on an allow-list whose purpose is a
/// REFUSAL.
fn dotenv_chain_vars() -> &'static HashMap<String, String> {
    ...
    CACHE.get_or_init(|| {
        vike_bridge_core::credentials::load_workspace_dotenv_scoped(
            &vike_bridge_core::credentials::KeyScope::of(CHAIN_KEYS),
        )
        .into_map()
    })
}
...
fn chain_var(key: &str) -> Option<String> {
    // The key-parameterised `std::env::var(key)` read stays HERE, exactly as
    // `crates/vike-ops/tests/settings_registry.rs`'s DYNAMIC_ALLOWLIST row for this file describes.
    chain_var_from(std::env::var(key).ok(), dotenv_chain_vars().get(key).cloned())
}

/// The PURE two-tier resolution [`chain_var`] wraps: the process value first, the `.env` value
/// second, first token only, empty is absent. Split out so the tiers can be tested without
/// `std::env::set_var` (an `unsafe fn` since edition 2024, which this workspace forbids) and
/// without depending on what the box's `.env` happens to carry.
fn chain_var_from(process: Option<String>, dotenv: Option<String>) -> Option<String> {
    process.or(dotenv).map(|v| crate::config::first_token(&v).to_string()).filter(|v| !v.is_empty())
}
...
mod tests {
    ...
    fn chain_watch_is_off_by_default_and_exact() {
        // Over the PURE two-tier resolver with the `.env` tier supplied explicitly — so every case
        // is asserted unconditionally rather than "only when the dev box's `.env` lacks the key",
        // and nothing touches the process env (`std::env::set_var` is an `unsafe fn` since edition
        // 2024, which this workspace forbids).
        let on = |process: Option<&str>, dotenv: Option<&str>| {
            chain_var_from(process.map(str::to_string), dotenv.map(str::to_string)).as_deref()
                == Some("1")
        };
        assert!(!on(None, None), "unset ⇒ OFF");
        assert!(!on(Some(""), None), "empty must not enable the watcher");
        for off in ["0", "true", "yes", "on", "11", "1x"] {
            assert!(!on(Some(off), None), "{off} must not enable the watcher");
        }
        assert!(on(Some("1"), None));
        // The live `.env` annotates values with trailing comments the shared parser keeps.
        assert!(on(Some("1   # on-chain settlement watcher"), None));
        // The `.env` tier is a real input: it enables on its own, and a set process value outranks it.
        assert!(on(None, Some("1")), ".env alone enables");
        assert!(!on(Some("0"), Some("1")), "a set process value outranks the .env tier");
        // ⚠ PINNED, not designed: an EMPTY process value SHADOWS the `.env` tier rather than
        // falling through to it — `Some("")` survives `Option::or`, and only the first-token filter
        // empties it afterwards. The previous version of this test claimed the opposite in a
        // comment and never asserted it either way. Recorded so a change to fall-through is made
        // deliberately rather than by accident.
        assert!(!on(Some(""), Some("1")), "empty process value shadows the .env tier (pinned)");
    }
