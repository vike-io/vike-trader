...
pub fn pm_resolve_enabled() -> bool {
    std::env::var("VIKE_PM_RESOLVE").as_deref() == Ok("1")
}
...
pub fn payout_for(entry: &WatchEntry, winners: &BTreeSet<String>) -> f64 {
    if winners.contains(&entry.token_id) {
        WINNER_PAYOUT
    } else {
        LOSER_PAYOUT
    }
...
mod tests {
    ...
    fn resolve_is_off_by_default() {
        std::env::remove_var("VIKE_PM_RESOLVE");
        assert!(!pm_resolve_enabled(), "unset ⇒ OFF");
        std::env::set_var("VIKE_PM_RESOLVE", "true");
        assert!(!pm_resolve_enabled(), "only the EXACT string \"1\" enables it");
        std::env::set_var("VIKE_PM_RESOLVE", "1");
        assert!(pm_resolve_enabled());
        std::env::remove_var("VIKE_PM_RESOLVE");
    }
