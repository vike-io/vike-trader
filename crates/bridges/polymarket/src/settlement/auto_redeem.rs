...
use crate::positions::{payout_of, redeemable_by, Payout, Position, PositionsClient, WinnerSource};
use crate::redeem::Era;
use crate::redeem_confirm::{ChainRedeemConfirmer, RedeemConfirmation};
use crate::redeem_ledger::RedeemLedger;
use crate::redeem_relayer::{submit_redeem, RedeemKind, RedeemResult};
...
pub fn auto_redeem_enabled() -> bool {
    std::env::var("POLY_AUTO_REDEEM").as_deref() == Ok("1")
}
...
pub fn kill_switch_tripped(halt_file: &Path) -> bool {
    std::env::var_os("POLY_REDEEM_HALT").is_some() || halt_file.exists()
}
...
mod tests {
    ...
    fn spawn_returns_none_without_opt_in() {
        // POLY_AUTO_REDEEM unset -> never starts (default OFF), even with creds present.
        std::env::remove_var("POLY_AUTO_REDEEM");
        assert!(!auto_redeem_enabled());
    }
    ...
    fn kill_switch_env_or_file() {
        ...
        std::env::remove_var("POLY_REDEEM_HALT");
        assert!(!kill_switch_tripped(&halt), "neither env nor file set");

        std::fs::write(&halt, "").unwrap();
        assert!(kill_switch_tripped(&halt), "halt file present trips it");
        std::fs::remove_file(&halt).unwrap();
        assert!(!kill_switch_tripped(&halt), "removing the file un-trips it");

        std::env::set_var("POLY_REDEEM_HALT", "1");
        assert!(kill_switch_tripped(&halt), "env present trips it");
        std::env::remove_var("POLY_REDEEM_HALT");
        assert!(!kill_switch_tripped(&halt), "removing the env un-trips it");
    }
