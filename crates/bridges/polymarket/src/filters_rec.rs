    ...
    if let Some(r) = rec {
        r.record(
            ...
            SymbolProperties {
                tick_size,
                taker_hold_ms,
                // Every row this venue writes is a binary prediction market — the venue IS the
                // class, so the producer knows it without reading the token id (0061 STEP 2).
                asset_class: Some(AssetClass::PredictionMarket),
                ..Default::default()
            },
            ts_ns,
        );
...
mod tests {
    ...
    fn poly_records_tick_as_properties() {
        ...
        assert_eq!(
            f.asset_class,
            Some(vike_model::AssetClass::PredictionMarket),
            "the venue IS the class here, so even the tick-only row names it"
        );
    }
