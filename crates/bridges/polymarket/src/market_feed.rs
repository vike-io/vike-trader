...
//! `crates/vike-marketdata/src/orderbook.rs`); a qty-0 delta level is a removal (also the book's own behavior,
...
use vike_bridge_core::user_data::StreamError;
...
// The stream seam is the SHARED driver's now (dedup A6) — re-exported so `raw_tap`'s
// `TappedStream`, the scripted tests, and vike-backfill's `poly_reparse` (which implements
// `MarketStream` over captured gz frames) keep their seam: `read_frame` returns exactly one TEXT
// frame per call (Ping auto-ponged, control frames swallowed, every inbound frame stamping the
// idle-watchdog liveness clock), `StreamError::Timeout` is the stop-flag poll tick, `Closed`
// carries the reconnect reason.
//
// ⚠ The ERROR shape is named canonically, `vike_bridge_core::user_data::StreamError`. It used to
// be re-exported here as `StreamError` — a pure abbreviation that gave one type a second name, which
// the 2026-09-18 alias ruling retired; the seam itself is unchanged.
...
fn handle_update(
    ...
    match up {
        ...
        {
            let bids: Vec<BookLevel> = changes
                .iter()
                .filter(|c| c.is_bid)
                .map(|c| BookLevel { price: c.price, qty: c.size })
                .collect();
            let asks: Vec<BookLevel> = changes
                .iter()
                .filter(|c| !c.is_bid)
                .map(|c| BookLevel { price: c.price, qty: c.size })
                .collect();
...
fn after_book_change(
    ...
    if (mode == PumpMode::Book || mode == PumpMode::Quotes)
        && let (
            Some(BookLevel { price: bid, qty: bid_size }),
            Some(BookLevel { price: ask, qty: ask_size }),
...
impl<S: MarketStream> MarketStream for ShardStream<S> {
    fn read_frame(&mut self) -> Result<String, StreamError> {
        if self.membership.epoch() != self.subscribed_at {
            return Err(StreamError::Closed("token set changed — resubscribing".into()));
        }
    ...
    run_market_feed_on(
        || {
            ...
            tapped.send_text(&subscribe_message(&tokens)).map_err(|e| match e {
                ...
                StreamError::Timeout => "read timeout".to_string(),
            })?;
