...
use vike_bridge_core::market_pump::{run_market_feed_on, run_market_session, MarketPumpOpts};
...
use super::ws::{decode_market, subscribe_message, MarketUpdate};
...
/// (`crates/vike-recorder/src/recorder_cli.rs`'s `FEED_STOP_BUDGET_SECS`). And polymarket is
...
/// `crates/vike-recorder/src/recorder_cli.rs`'s `FEED_STOP_BUDGET_SECS` — and the count is not
...
fn shard_main(mode: PumpMode, membership: Arc<ShardMembership>, ctx: FeedCtx) {
    ...
    // RefCell shims: the connect / on_text / on_tick / on_session_error closures all touch the
    ...
    run_market_feed_on(
        ...
        |e| {
            ctx.set_status(format!("polymarket {} ws error (reconnecting): {e}", label.borrow()));
            let now = now_ms();
...
#[cfg(test)]
mod tests {
    use super::{shard_label, FeedCtx, Feeds, PumpMode, ShardMembership};
