...
use vike_bridge_core::exec_actor::{
    CancelOutcome, ConfirmFn, ExecActor, ExecCommand, cancel_batch_undeclared, cancel_event,
};
...
use crate::stream::{DealRefMap, DealRefs, LsParams, stream_trades};
...
pub const CLOSE_MARKET_ONLY: &str = "ig: a reduce_only close is MARKET-only (measured: IG's close endpoint cannot rest an order — \
